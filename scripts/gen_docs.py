...
def build_nav_nodes():
    """Build the nested navigation structure mirroring the API toctree.

    Each node is {"t": title, "u": docname, "c": [children]}.  Docnames are
    relative to the documentation source root (e.g. "api/svd/.../gesvd") so a
    single shared asset can serve every page; the browser prepends the
    per-page path-to-root at runtime.
    """
    def leaf(page, url_prefix):
        return {"t": page, "u": f"{url_prefix}/{page}"}

    families = []
    for family_name, (title, children) in HIERARCHY.items():
        fam_url = f"api/{family_name}"
        if isinstance(children, list):
            kids = [leaf(page, fam_url) for page in children]
        else:
            kids = []
            for sub_name, (sub_title, pages) in children.items():
                sub_url = f"api/{family_name}/{sub_name}"
                kids.append({
                    "t": sub_title,
                    "u": sub_url,
                    "c": [leaf(page, sub_url) for page in pages],
                })
        families.append({"t": title, "u": fam_url, "c": kids})
    return families


def write_nav_data(dry_run=False, stats=None):
    """Write the shared navigation asset consumed by semicolon-nav.js."""
    payload = json.dumps(build_nav_nodes(), separators=(",", ":"),
                         ensure_ascii=False)
    content = ("// Auto-generated by scripts/gen_docs.py -- do not edit.\n"
               f"window.SEMICOLON_NAV = {payload};\n")
    static_dir = os.path.join(PROJECT_ROOT, "doc", "source", "_static")
    write_file(os.path.join(static_dir, "semicolon_nav_data.js"),
               content, dry_run, stats)
...
def main():
    ...
    # Write the shared client-side navigation asset
    write_nav_data(dry_run, stats)
