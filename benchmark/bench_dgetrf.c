/**
 * @file bench_dgetrf.c
 * @brief Benchmark for dgetrf (LU factorization) — semicolon-lapack.
 *
 * Usage:
 *   ./bench_dgetrf [m] [n] [iters]        Single size benchmark
 *   ./bench_dgetrf --sweep [iters]         Sweep sizes, print table
 *   ./bench_dgetrf --sweep [iters] --csv   Same but CSV output
 *
 * Profiling:
 *   samply record ./bench_dgetrf 1000 1000 50
 */
...
int main(int argc, char* argv[])
{
    ...
    if (do_sweep) {
        ...
        if (do_csv) {
            ...
            printf("Benchmarking dgetrf: m=%d, n=%d, iters=%d (warmup=%d)\n",
