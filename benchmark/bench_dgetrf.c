/**
 * @file bench_dgetrf.c
 * @brief Benchmark for dgetrf (LU factorization).
 *
 * Usage:
 *   ./bench_dgetrf [n] [iterations]    Single size benchmark
 *   ./bench_dgetrf --sweep             Sweep n=32..4096, print table
 *   ./bench_dgetrf --sweep --csv       Same but CSV output
 *   ./bench_dgetrf 500 50 --csv        Single size, CSV output
 *
 * Profiling:
 *   samply record ./bench_dgetrf 1000 50
 */
...
static double bench_one(int n, int iters)
{
    ...
    for (int iter = 0; iter < iters; iter++) {
        ...
        dgetrf(n, n, A, lda, ipiv, &info);
        if (info != 0 && iter == 0) {
            fprintf(stderr, "dgetrf failed with info=%d\n", info);
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    if (do_sweep) {
        ...
        if (do_csv) {
            ...
            printf("Benchmarking dgetrf: n=%d, iterations=%d (warmup=%d)\n",
