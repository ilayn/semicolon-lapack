/**
 * @file bench_dgeqrf_fortran.c
 * @brief Benchmark for vendor dgeqrf via Fortran LAPACK interface.
 *
 * Usage:
 *   ./bench_dgeqrf_fortran [m] [n] [iters]        Single size benchmark
 *   ./bench_dgeqrf_fortran --sweep [iters]         Sweep sizes, print table
 *   ./bench_dgeqrf_fortran --sweep [iters] --csv   Same but CSV output
 *   ./bench_dgeqrf_fortran [m] [n] [iters] --check Run correctness check
 *
 * Profiling:
 *   samply record ./bench_dgeqrf_fortran 1000 1000 50
 */
...
extern void dormqr_(const char* side, const char* trans,
                    int* m, int* n, int* k,
                           ...
                           const double* A_orig)
{
    ...
    double res_norm = dlange_("F", &m, &n, C, &lda, work_norm);

    /* ||A_orig||_F */
    double a_norm = dlange_("F", &m, &n, A_orig, &lda, work_norm);
                     ...
                     double* out_min, double* out_med)
{
    ...
    for (int iter = 0; iter < iters; iter++) {
        ...
        times[iter] = t1 - t0;
        if (info != 0 && iter == 0)
            fprintf(stderr, "dgeqrf_ failed with info=%d\n", info);
    }
...
int main(int argc, char* argv[])
{
    ...
    if (do_sweep) {
        ...
        if (do_csv) {
            ...
            printf("Benchmarking vendor dgeqrf_: m=%d, n=%d, iters=%d (warmup=%d)\n",
