 ...
 *
 * Usage:
 *   ./bench_dgeqrf_fortran [n] [iterations]    Single size benchmark
 *   ./bench_dgeqrf_fortran --sweep              Sweep n=32..4096, print table
 *   ./bench_dgeqrf_fortran --sweep --csv        Same but CSV output
 *
 * Profiling:
 *   samply record ./bench_dgeqrf_fortran 1000 50
 */
...
static double bench_one(int n, int iters)
{
    ...
    double work_query;
    int lwork = -1;
    dgeqrf_(&n, &n, A, &lda, tau, &work_query, &lwork, &info);
...
int main(int argc, char* argv[])
{
    ...
    if (do_sweep) {
        ...
        if (do_csv) {
            ...
            printf("Benchmarking vendor dgeqrf_: n=%d, iterations=%d (warmup=%d)\n",
