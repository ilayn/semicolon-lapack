/**
 * @file test_dchkqr.c
 * @brief Comprehensive test suite for QR factorization (DQR) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/dchkqr.f to C using CMocka.
 * Tests DGEQRF, DORGQR, and DORMQR.
 *
 * Each (m, n, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from dchkqr.f:
 *   TEST 1-2: QR factorization via dqrt01 (norm(R - Q'*A), norm(I - Q'*Q))
 *   TEST 3-6: DORMQR tests via dqrt03 (applying Q from left/right with trans)
 *   TEST 7: Least squares solve via dgels and dget02
 *   TEST 8-9: DGEQRFP tests (positive diagonal R)
 *
...
#define NTYPES  8
#define NTESTS  9   /* Tests 1-2 (dqrt01), 3-6 (dqrt03), 7 (dgels), 8-9 (dqrt01p) */
#define THRESH  30.0
...
extern void dqrt01(const int m, const int n, const double* A, double* AF,
                   double* Q, double* R, const int lda, double* tau,
...
extern void dqrt01p(const int m, const int n, const double* A, double* AF,
                    double* Q, double* R, const int lda, double* tau,
                    double* work, const int lwork, double* rwork,
                    double* result);
extern int dgennd(const int m, const int n, const double* A, const int lda);
...
typedef struct {
    ...
    double* R;      /* Upper triangular R (NMAX x NMAX) */
    double* C;      /* Workspace for dqrt03 (NMAX x NMAX) */
    double* CC;     /* Another workspace for dqrt03 (NMAX x NMAX) */
...
static int group_setup(void** state)
{
    ...
    g_workspace->R = malloc(NMAX * NMAX * sizeof(double));
...
static void run_dchkqr_single(int m, int n, int imat, int inb_idx)
{
    ...
    for (int ik = 0; ik < nk; ik++) {
        ...
        if (ik == 0) {
            /*
             * TEST 1-2: Test DGEQRF via dqrt01
             * Computes:
            ...
            dqrt01(m, n, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
            ...
            /*
             * TEST 8-9: Test DGEQRFP via dqrt01p
             * DGEQRFP produces QR factorization with non-negative diagonal R.
             * Test 8: same as tests 1-2 but using DGEQRFP
             * Test 9: check that diagonal of R is non-negative (via dgennd)
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 8 (DGEQRFP factorization)", m, n, imat, k);
            set_test_context(ctx);
            dqrt01p(m, n, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                    ws->WORK, lwork, ws->RWORK, &result[7]);

            assert_residual_below(result[7], THRESH);
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 8b (DGEQRFP orthogonality)", m, n, imat, k);
            set_test_context(ctx);
            assert_residual_below(result[8], THRESH);

            /* Test 9: Check that AF has non-negative diagonal */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 9 (DGEQRFP non-negative diagonal)", m, n, imat, k);
            set_test_context(ctx);
            if (!dgennd(m, n, ws->AF, lda)) {
                result[8] = 2 * THRESH;  /* Fail the test */
            }
            assert_residual_below(result[8], THRESH);
        } else if (m >= n) {
            /*
             * TEST 1-2: Test DORGQR via dqrt02
             * Uses factorization from dqrt01
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 1-2 (DORGQR)", m, n, imat, k);
            set_test_context(ctx);
            dqrt02(m, n, k, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                   ws->WORK, lwork, ws->RWORK, result);
        ...
        if (m >= k) {
            /*
             * TEST 3-6: Test DORMQR via dqrt03
             * Tests Q from left/right with 'N'/'T' transpose options
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 3-6 (DORMQR)", m, n, imat, k);
            set_test_context(ctx);
            dqrt03(m, n, k, ws->AF, ws->C, ws->CC, ws->Q, lda, ws->TAU,
            ...
            /*
             * TEST 7: If M >= N and K == N and INB == 1, test DGELS least squares solve
            ...
            if (k == n && inb_idx == 0) {
                 ...
                 * and we need to preserve AF for subsequent dqrt02/dqrt03 tests */
