/**
 * @file test_ddrvsy_rook.c
 * @brief DDRVSY_ROOK tests the driver routine DSYSV_ROOK.
 *
...
/* Routine under test */
extern void dsysv_rook(const char* uplo, const int n, const int nrhs,
...
extern void dsyt01_rook(const char* uplo, const int n,
...
static void run_ddrvsy_rook_single(int n, int imat, int iuplo)
{
    ...
    if (zerot) {
        ...
        dsytrf_rook(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK,
                    ws->lwork, &info);

        dlacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        dsytri_rook(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, &info);
    ...
    dsysv_rook(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    /* Check error code from DSYSV_ROOK */
    if (info != k) {
        fail_msg("DSYSV_ROOK UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    dsyt01_rook(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (int i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("DSYSV_ROOK UPLO=%s test %d failed: result=%e >= thresh=%e",
