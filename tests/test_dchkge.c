...
extern void dgecon(const char* norm, const int n, const double* A,
...
extern void dget01(const int m, const int n, const double* A, const int lda,
                   double* AFAC, const int ldafac, const int* ipiv,
...
extern void dget02(const char* trans, const int m, const int n, const int nrhs,
                   ...
                   double* rwork, double* resid);
extern void dget03(const int n, const double* A, const int lda,
                   const double* AINV, const int ldainv, double* work,
                   const int ldwork, double* rwork, double* rcond, double* resid);
...
extern void dget07(const char* trans, const int n, const int nrhs,
                   ...
                   const double* XACT, const int ldxact, const double* ferr,
                   const int chkferr, const double* berr, double* reslts);
...
static int group_setup(void** state)
{
    ...
    g_workspace->RWORK = malloc(2 * NMAX * sizeof(double));
...
static void run_dchkge_single(int m, int n, int imat, int inb)
{
    ...
    int lda = (m > 1) ? m : 1;
    int trfcon;
    double anormo, anormi, rcondo, rcondi, rcond, rcondc;
    ...
    if (m == n && info == 0) {
        ...
        double ainvnm = dlange("I", n, n, ws->AINV, lda, ws->RWORK);
