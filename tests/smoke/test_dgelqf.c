/**
 * @file test_dgelqf.c
 * @brief CMocka test suite for dgelqf (LQ factorization).
 *
 * Verification:
 *   dlqt01: norm(L - A*Q') / (N * norm(A) * eps) and
 *           norm(I - Q*Q') / (N * eps)
 */
...
extern void dlqt01(const int m, const int n,
...
static int lq_setup(void **state, int m, int n)
{
    ...
    fix->L = calloc(fix->lda * maxmn, sizeof(double));
...
static void run_lqt01(lq_fixture_t *fix, int imat)
{
    ...
    dlqt01(fix->m, fix->n, fix->A, fix->AF, fix->Q, fix->L, fix->lda,
