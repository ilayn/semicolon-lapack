/**
 * @file test_schktz.c
 * @brief Comprehensive test suite for STZRZF (trapezoidal RZ factorization).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/dchktz.f to C using CMocka.
 * Tests STZRZF for various matrix types.
 *
 * Test structure from dchktz.f:
 *   TEST 1: norm(svd(R) - svd(A)) / (||svd(A)|| * eps * max(M,N)) via sqrt12
 *   TEST 2: norm(A - R*Q) / (||A|| * eps * max(M,N)) via srzt01
 *   TEST 3: norm(Q'*Q - I) / (eps * max(M,N)) via srzt02
 *
...
/* Routines under test */
extern void stzrzf(const int m, const int n, f32* A, const int lda,
...
extern f32 srzt01(const int m, const int n, const f32* A, const f32* AF,
                     const int lda, const f32* tau, f32* work, const int lwork);
extern f32 srzt02(const int m, const int n, const f32* AF, const int lda,
 ...
 */
typedef struct {
    f32* A;      /* Working matrix (NMAX x NMAX) */
    f32* COPYA;  /* Copy of original matrix (NMAX x NMAX) */
...
static void run_dchktz_single(int m, int n, int imode)
{
    ...
    if (mode == 0) {
        /* Zero matrix */
        slaset("F", m, n, ZERO, ZERO, ws->A, lda);
        ...
        sgeqr2(m, n, ws->A, lda, ws->WORK, &ws->WORK[mnmin], &info);

        /* Zero out below the diagonal */
        if (m > 1) {
            slaset("L", m - 1, n, ZERO, ZERO, &ws->A[1], lda);
        }

        /* Sort singular values in decreasing order */
        slaord("D", mnmin, ws->S, 1);
    }

    /* Save A */
    slacpy("A", m, n, ws->A, lda, ws->COPYA, lda);

    /* Call STZRZF to reduce the upper trapezoidal matrix to upper triangular form */
    stzrzf(m, n, ws->A, lda, ws->TAU, ws->WORK, lwork, &info);

    /* TEST 1: norm(svd(R) - svd(A)) / (||svd(A)|| * eps * max(M,N))
     * R is the m-by-m upper triangular factor from STZRZF (stored in A(0:m-1, 0:m-1)) */
    snprintf(ctx, sizeof(ctx), "m=%d n=%d type=%d TEST 1 (SVD comparison)", m, n, imode);
    set_test_context(ctx);
    result[0] = sqrt12(m, m, ws->A, lda, ws->S, ws->WORK, lwork);
    assert_residual_below(result[0], THRESH);

    /* TEST 2: norm(A - R*Q) / (||A|| * eps * max(M,N)) */
    snprintf(ctx, sizeof(ctx), "m=%d n=%d type=%d TEST 2 (factorization)", m, n, imode);
    set_test_context(ctx);
    result[1] = srzt01(m, n, ws->COPYA, ws->A, lda, ws->TAU, ws->WORK, lwork);
    assert_residual_below(result[1], THRESH);

    /* TEST 3: norm(Q'*Q - I) / (eps * max(M,N)) */
    snprintf(ctx, sizeof(ctx), "m=%d n=%d type=%d TEST 3 (orthogonality)", m, n, imode);
    set_test_context(ctx);
    result[2] = srzt02(m, n, ws->A, lda, ws->TAU, ws->WORK, lwork);
...
static void build_test_array(void)
{
    ...
    for (int im = 0; im < (int)NM; im++) {
        ...
        for (int in = 0; in < (int)NN; in++) {
            ...
            /* STZRZF only applies when M <= N (underdetermined systems) */
            if (m > n) {
                continue;
            }
