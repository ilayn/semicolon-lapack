 ...
 *
 * Tests SGGQRF and SGGRQF - the generalized QR and RQ factorizations:
 *   SGGQRF: A = Q*R, B = Q*T*Z  (N-by-M matrix A, N-by-P matrix B)
 *   SGGRQF: A = R*Q, B = Z*T*Q  (M-by-N matrix A, P-by-N matrix B)
 *
 * Each (im, ip, in, imat) combination is registered as a separate CMocka test.
 * For each combination, both SGGRQF (via sgrqts) and SGGQRF (via sgqrts) are tested.
 *
...
static int group_setup(void** state)
{
    ...
    g_ws->AQ    = malloc(lda * NMAX * sizeof(f32));
    g_ws->AR    = malloc(lda * NMAX * sizeof(f32));
    g_ws->taua  = malloc(NMAX * sizeof(f32));
    g_ws->B     = malloc(ldb * NMAX * sizeof(f32));
    g_ws->BF    = malloc(ldb * NMAX * sizeof(f32));
    g_ws->BZ    = malloc(ldb * NMAX * sizeof(f32));
    g_ws->BT    = malloc(ldb * NMAX * sizeof(f32));
    g_ws->BWK   = malloc(ldb * NMAX * sizeof(f32));
    g_ws->taub  = malloc(NMAX * sizeof(f32));
...
static void test_dckgqr_case(void** state)
{
    ...
    sgrqts(m, p, n, g_ws->A, g_ws->AF, g_ws->AQ, g_ws->AR, lda,
    ...
    distb_str[0] = distb;
    slatms(n, p, distb_str, type_str, g_ws->rwork, modea, cndnma,
           bnorm, klb, kub, "N", g_ws->B, ldb, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    sgqrts(n, m, p, g_ws->A, g_ws->AF, g_ws->AQ, g_ws->AR, lda,
           g_ws->taua, g_ws->B, g_ws->BF, g_ws->BZ, g_ws->BT,
           g_ws->BWK, ldb, g_ws->taub, g_ws->work, lwork,
           g_ws->rwork, result);
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dckgqr", g_tests, g_num_tests,
