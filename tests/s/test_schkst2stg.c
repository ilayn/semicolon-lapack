/**
 * @file test_schkst2stg.c
 * @brief Symmetric tridiagonal eigenvalue test driver (2-stage) - port of LAPACK
 *        TESTING/EIG/dchkst2stg.f
 *
 * Tests the symmetric eigenvalue problem routines using the 2-stage
 ...
 * standard 1-stage reduction SSYTRD. Tests 1 and 2 remain to verify
 ...
 *   SSYTRD_2STAGE       - 2-stage symmetric to tridiagonal reduction
 ...
 *   Tests  1-2 : Reduction quality (SSYTRD/SORGTR upper)
 *   Tests  3-4 : 2-stage eigenvalue comparison (SSYTRD_2STAGE upper/lower)
 *   Tests  5-8 : Packed reduction quality (SSPTRD/SOPGTR)
...
static dchkst2stg_ws_t* g_ws = NULL;
...
static int group_setup(void** state)
{
    ...
    g_ws = calloc(1, sizeof(dchkst2stg_ws_t));
    if (!g_ws) return -1;
    ...
    /* Ensure workspace is large enough for ssytrd_2stage.
    ...
    {
        INT kd_2stg = ilaenv2stage(1, "SSYTRD_2STAGE", "N", nmax, -1, -1, -1);
        INT ib_2stg = ilaenv2stage(2, "SSYTRD_2STAGE", "N", nmax, kd_2stg, -1, -1);
        INT lhmin = ilaenv2stage(3, "SSYTRD_2STAGE", "N", nmax, kd_2stg, ib_2stg, -1);
        INT lwmin = ilaenv2stage(4, "SSYTRD_2STAGE", "N", nmax, kd_2stg, ib_2stg, -1);
...
static void test_dchkst2stg_case(void** state)
{
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSYTRD(U) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[0] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SORGTR(U) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[1] = ulpinv;
        goto L280;
    }
     ...
     * Compute D1 from 1-stage SSYTRD(U) as reference, then compare
     * with SSYTRD_2STAGE upper (D2) and lower (D3).
     * (dchkst2stg.f lines 969-1075)
     * ================================================================ */
    ...
    ntest = 3;
    ssteqr("N", n, D1, work, NULL, ldu, NULL, &iinfo);
    if (iinfo != 0) {
        fprintf(stderr, "SSTEQR(N) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        if (iinfo < 0) {
            ws->result[2] = ulpinv;
            return;
        }
    ...
    slacpy("U", n, n, A, lda, V, ldu);
    ssytrd_2stage("N", "U", n, V, ldu, SD, SE, TAU,
    ...
    ssytrd_2stage("N", "L", n, V, ldu, SD, SE, TAU,
                  work, lh, work + lh, lw, &iinfo);

    cblas_scopy(n, SD, 1, D3, 1);
    if (n > 0) cblas_scopy(n - 1, SE, 1, work, 1);

    ssteqr("N", n, D3, work, NULL, ldu, NULL, &iinfo);
    if (iinfo != 0) {
        fprintf(stderr, "SSTEQR(N) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        if (iinfo < 0) {
            ws->result[3] = ulpinv;
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSPTRD(U) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[4] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SOPGTR(U) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[5] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSPTRD(L) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[6] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SOPGTR(L) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[7] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSTEQR(V) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[8] = ulpinv;
        goto L280;
    }
    ...
    if (jtype > 15) {
        ...
        if (iinfo != 0) {
            fprintf(stderr, "SPTEQR(V) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
            ws->result[13] = ulpinv;
            goto L280;
        }
        ...
        if (iinfo != 0) {
            fprintf(stderr, "SPTEQR(N) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
            ws->result[15] = ulpinv;
            goto L280;
        }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSTEIN failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[19] = ulpinv;
        ws->result[20] = ulpinv;
        goto L280;
    }
    ...
    sstedc("I", n, D1, work, Z, ldu, work + n, lwedc - n,
           iwork, liwedc, &iinfo);
    if (iinfo != 0) {
        fprintf(stderr, "SSTEDC(I) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[21] = ulpinv;
        goto L280;
    }
    ...
    sstedc("V", n, D1, work, Z, ldu, work + n, lwedc - n,
           iwork, liwedc, &iinfo);
    if (iinfo != 0) {
        fprintf(stderr, "SSTEDC(V) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[23] = ulpinv;
        goto L280;
    }
    ...
    sstedc("N", n, D2, work, Z, ldu, work + n, lwedc - n,
           iwork, liwedc, &iinfo);
    if (iinfo != 0) {
        fprintf(stderr, "SSTEDC(N) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[25] = ulpinv;
        goto L280;
    }
    ...
    if (SRANGE) {
        ...
        if (iinfo != 0) {
            fprintf(stderr, "SSTEMR(V,I) failed: info=%d n=%d jtype=%d\n",
                          iinfo, n, jtype);
            ws->result[28] = ulpinv;
            goto L280;
        }
        ...
        if (iinfo != 0) {
            fprintf(stderr, "SSTEMR(N,I) failed: info=%d n=%d jtype=%d\n",
                          iinfo, n, jtype);
            ws->result[30] = ulpinv;
            goto L280;
        }
        ...
        if (iinfo != 0) {
            fprintf(stderr, "SSTEMR(V,V) failed: info=%d n=%d jtype=%d\n",
                          iinfo, n, jtype);
            ws->result[31] = ulpinv;
            goto L280;
        }
        ...
        if (iinfo != 0) {
            fprintf(stderr, "SSTEMR(N,V) failed: info=%d n=%d jtype=%d\n",
                          iinfo, n, jtype);
            ws->result[33] = ulpinv;
            goto L280;
        }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSTEMR(V,A) failed: info=%d n=%d jtype=%d\n",
                      iinfo, n, jtype);
        ws->result[34] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSTEMR(N,A) failed: info=%d n=%d jtype=%d\n",
                      iinfo, n, jtype);
        ws->result[36] = ulpinv;
        goto L280;
    }
...
static void build_test_array(void)
{
    ...
    for (size_t in = 0; in < NNVAL; in++) {
        ...
        for (INT jtype = 1; jtype <= MAXTYP; jtype++) {
            ...
            dchkst2stg_params_t* p = &g_params[g_num_tests];
            p->n = n;
            p->jtype = jtype;
            snprintf(p->name, sizeof(p->name),
                     "dchkst2stg_n%d_type%d", n, jtype);

            g_tests[g_num_tests].name = p->name;
            g_tests[g_num_tests].test_func = test_dchkst2stg_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("dchkst2stg", g_tests, g_num_tests,
