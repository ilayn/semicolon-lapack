 ...
 *
 * Tests the nonsymmetric eigenvalue problem expert driver SGEESX.
 *
...
typedef struct {
    ...
    f32* H;      /* Copy modified by SGEESX */
    f32* HT;     /* Copy for comparison */
    f32* VS;     /* Schur vectors */
    f32* VS1;    /* Schur vectors (backup) */

    /* Eigenvalues */
    f32* wr;     /* Real parts */
    f32* wi;     /* Imaginary parts */
    f32* wrt;    /* Real parts (temp) */
    f32* wit;    /* Imaginary parts (temp) */
    f32* wrtmp;  /* Real parts (comparison) */
    f32* witmp;  /* Imaginary parts (comparison) */
...
static int group_setup(void** state)
{
    ...
    g_ws->wrtmp  = malloc(nmax * sizeof(f32));
    g_ws->witmp  = malloc(nmax * sizeof(f32));
    ...
    /* IWORK dimension: N*N (sget24.c line 144: liwork = n*n) */
...
static void run_ddrvsx_random(ddrvsx_params_t* params)
{
    ...
    /* Call sget24 with COMP=0 (ddrvsx.f lines 786-790) */
    INT info = 0;
    INT any_fail = 0;

    sget24(0, jtype, THRESH, n, A, lda, H, HT,
...
static void run_ddrvsx_precomp(ddrvsx_params_t* params)
{
    ...
    /* Call sget24 with COMP=1 (ddrvsx.f lines 848-851) */
    sget24(1, 22, THRESH, n, A, lda, H, HT,
           wr, wi, wrt, wit, wrtmp, witmp,
           VS, ldvs, VS1,
           pc->rcdein, pc->rcdvin,
           pc->nslct, pc->islct,
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("ddrvsx", g_tests, g_num_tests,
