/**
 * @file test_sdrvsy_aa_2stage.c
 * @brief DDRVSY_AA_2STAGE tests the driver routine SSYSV_AA_2STAGE.
 *
...
/* Routine under test */
extern void ssysv_aa_2stage(const char* uplo, const int n, const int nrhs,
                            f32* A, const int lda,
                            f32* TB, const int ltb,
                            int* ipiv, int* ipiv2,
                            f32* B, const int ldb,
...
static int group_setup(void** state)
{
    ...
    /* Also need workspace for slatms */
    ...
    g_workspace->TB = calloc(ltb, sizeof(f32));
...
static void run_ddrvsy_aa_2stage_single(int n, int imat, int iuplo)
{
    ...
    ssysv_aa_2stage(uplo, n, NRHS, ws->AFAC, lda,
                    ws->TB, ltb,
                    ws->IWORK, ws->IPIV2,
    ...
    /* Check error code from SSYSV_AA_2STAGE */
    if (info != k) {
        fail_msg("SSYSV_AA_2STAGE UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    for (int i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("SSYSV_AA_2STAGE UPLO=%s test %d failed: result=%e >= thresh=%e",
