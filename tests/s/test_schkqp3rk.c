/**
 * @file test_schkqp3rk.c
 * @brief Comprehensive test suite for SGEQP3RK (QR with column pivoting, rank-revealing).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/dchkqp3rk.f to C using CMocka.
 * Tests SGEQP3RK for various matrix types and block sizes.
 *
 * Test structure from dchkqp3rk.f:
 *   TEST 1: norm(svd(R) - svd(A)) / (||svd(A)|| * eps * max(M,N)) via sqrt12
 *           (only when KFACT == MINMN)
 *   TEST 2: norm(A*P - Q*R) / (||A|| * eps * max(M,N)) via sqpt01
 *   TEST 3: norm(Q'*Q - I) / (eps * M) via sqrt11
...
/* Routine under test */
extern void sgeqp3rk(const int m, const int n, const int nrhs, const int kmax,
                     f32 abstol, f32 reltol, f32* A, const int lda,
                     int* K, f32* maxc2nrmk, f32* relmaxc2nrmk,
                     int* jpiv, f32* tau, f32* work, const int lwork,
                     int* iwork, int* info);

/* Verification routines */
extern f32 sqrt11(const int m, const int k, const f32* A, const int lda,
...
extern f32 sqpt01(const int m, const int n, const int k,
...
extern void slaord(const char* job, const int n, f32* X, const int incx);
 ...
 */
typedef struct {
    f32* A;       /* Working matrix (NMAX * (NMAX + NSMAX)) - holds A and appended B */
    f32* COPYA;   /* Copy of original A (NMAX * NMAX) */
    f32* B;       /* RHS matrix (NMAX * NSMAX) */
    f32* COPYB;   /* Copy of original B (NMAX * NSMAX) */
    f32* S;       /* Singular values (NMAX) */
    f32* TAU;     /* Scalar factors of elementary reflectors (NMAX) */
...
static int group_setup(void** state)
{
    ...
    g_workspace->A = malloc(NMAX * (NMAX + NSMAX) * sizeof(f32));
    g_workspace->COPYA = malloc(NMAX * NMAX * sizeof(f32));
    g_workspace->B = malloc(NMAX * NSMAX * sizeof(f32));
    g_workspace->COPYB = malloc(NMAX * NSMAX * sizeof(f32));
 ...
 */
static int generate_matrix(int m, int n, int imat, f32* COPYA, int lda,
                           f32* S, f32* WORK, uint64_t rng_state[static 4])
{
    const f32 ZERO = 0.0f;
    int minmn = (m < n) ? m : n;
    int info;
    ...
    if (imat == 1) {
        ...
        slaset("F", m, n, ZERO, ZERO, COPYA, lda);
    ...
    if ((imat >= 2 && imat <= 4) || (imat >= 14 && imat <= 19)) {
        /* Matrices 2-4, 14-19: Generate via slatb4 + slatms */
        slatb4("DQK", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    if (minmn >= 2 && imat >= 5 && imat <= 13) {
        ...
        slaset("F", m, nb_zero, ZERO, ZERO, COPYA, lda);

        /* Generate an M-by-NB_GEN matrix in COPYA starting at column NB_ZERO */
        slatb4("DQK", imat, m, nb_gen, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);

        char dist_str[2] = {dist, '\0'};
        char type_str[2] = {type, '\0'};
        int ind_offset_gen = nb_zero * lda;
        slatms(m, nb_gen, dist_str,
               type_str, S, mode, cndnum, anorm,
        ...
        if (imat == 6 || imat == 7 || imat == 8 || imat == 10 || imat == 11) {
            ...
            for (int j = 0; j < jb_zero - 1; j++) {
                cblas_sswap(m, &COPYA[(nb_zero + j) * lda], 1,
                            &COPYA[j * lda], 1);
            }
            ...
            for (int j = 0; j < nb_gen; j++) {
                ...
                cblas_sswap(m, &COPYA[ind_out], 1, &COPYA[ind_in], 1);
            }
        }
...
static void run_dchkqp3rk_single(int m, int n, int nrhs, int imat, int inb, int kmax)
{
    ...
    /* Generate RHS matrix B (COPYB) with slatb4/slatms for IMAT=14 parameters */
    {
        char type, dist;
        int kl, ku, mode;
        f32 anorm, cndnum;
        slatb4("DQK", 14, m, nrhs, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
        char dist_str[2] = {dist, '\0'};
        char type_str[2] = {type, '\0'};
        slatms(m, nrhs, dist_str,
     ...
     */
    slacpy("A", m, n, ws->COPYA, lda, ws->A, lda);
    slacpy("A", m, nrhs, ws->COPYB, lda, &ws->A[lda * n], lda);
    slacpy("A", m, nrhs, ws->COPYB, lda, ws->B, lda);
    for (int i = 0; i < n; i++) {
        ws->IWORK[n + i] = ws->IWORK[i];
    }

    /* Workspace size for SGEQP3RK */
    ...
    sgeqp3rk(m, n, nrhs, kmax, abstol, reltol, ws->A, lda,
             &kfact, &maxc2nrmk, &relmaxc2nrmk, &ws->IWORK[n], ws->TAU,
             ws->WORK, lw, &ws->IWORK[2 * n], &info);

    if (info < 0) {
        snprintf(ctx, sizeof(ctx), "m=%d n=%d nrhs=%d kmax=%d type=%d nb=%d: SGEQP3RK failed (info=%d)",
    ...
    if (kfact == minmn) {
        result[0] = sqrt12(m, n, ws->A, lda, ws->S, ws->WORK, ws->lwork);
    ...
    /* TEST 2: norm(A*P - Q*R) / (||A|| * eps * max(M,N)) via sqpt01 */
    result[1] = sqpt01(m, n, kfact, ws->COPYA, ws->A, lda, ws->TAU,
    ...
    /* TEST 3: norm(Q'*Q - I) / (eps * M) via sqrt11 */
    result[2] = sqrt11(m, kfact, ws->A, lda, ws->TAU, ws->WORK, ws->lwork);
    ...
    if (ktest4 >= 2) {
        result[3] = ZERO;
        f32 r11 = ws->A[0];  /* R(0,0) */
        if (fabsf(r11) > ZERO) {
            for (int j = 0; j < kfact - 1; j++) {
                /* R(j,j) is at A[j + j*lda], R(j+1,j+1) is at A[(j+1) + (j+1)*lda] */
                f32 rjj = fabsf(ws->A[j + j * lda]);
                f32 rjj1 = fabsf(ws->A[(j + 1) + (j + 1) * lda]);
                f32 dtemp = (rjj - rjj1) / fabsf(r11);
                if (dtemp < ZERO) {
                    result[3] = BIGNUM;
                    break;
                }
     ...
     *         Only when MINMN > 0 */
    if (minmn > 0) {
        /* B already contains COPYB; A[lda*n:] contains Q**T * COPYB from sgeqp3rk */
        /* Apply Q**T to B using sormqr */
        int lwork_mqr = (nrhs > 1) ? nrhs : 1;
        sormqr("L", "T", m, nrhs, kfact, ws->A, lda, ws->TAU,
               ws->B, lda, ws->WORK, lwork_mqr, &info);

        if (info == 0) {
            /* Compute B := B - A[lda*n:] (where A[lda*n:] is Q**T * COPYB from sgeqp3rk) */
            for (int i = 0; i < nrhs; i++) {
                cblas_saxpy(m, NEGONE, &ws->A[lda * n + i * lda], 1,
                            &ws->B[i * lda], 1);
            }

            /* result[4] = ||B|| / (M * eps) */
            f32 rdummy[1];
            f32 bnorm = slange("1", m, nrhs, ws->B, lda, rdummy);
