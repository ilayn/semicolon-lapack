/**
 * @file slarge.c
 * @brief SLARGE pre- and post-multiplies a real general n by n matrix A
 *        with a random orthogonal matrix: A = U*A*U'.
 *
 * Faithful port of LAPACK TESTING/MATGEN/slarge.f
 */
            ...
            f32* work, int* info, uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    for (i = n - 1; i >= 0; i--) {
        ...
        slarnv_rng(3, n - i, work, state);
        wn = cblas_snrm2(n - i, work, 1);
        wa = (work[0] >= 0.0f) ? wn : -wn;
        if (wn == ZERO) {
            tau = ZERO;
        } else {
            wb = work[0] + wa;
            cblas_sscal(n - i - 1, ONE / wb, work + 1, 1);
            work[0] = ONE;
            tau = wb / wa;
        }

        cblas_sgemv(CblasColMajor, CblasTrans, n - i, n, ONE,
                    A + i, lda, work, 1, ZERO, work + n, 1);
        cblas_sger(CblasColMajor, n - i, n, -tau, work, 1,
                   work + n, 1, A + i, lda);

        cblas_sgemv(CblasColMajor, CblasNoTrans, n, n - i, ONE,
                    A + i * lda, lda, work, 1, ZERO, work + n, 1);
        cblas_sger(CblasColMajor, n, n - i, -tau, work + n, 1,
                   work, 1, A + i * lda, lda);
    }
}
