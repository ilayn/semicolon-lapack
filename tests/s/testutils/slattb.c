/**
 * @file slattb.c
 * @brief SLATTB generates a triangular test matrix in 2-dimensional banded
 *        storage.
 */
...
/**
 * SLATTB generates a triangular test matrix in 2-dimensional banded storage.
            ...
            f32* B, f32* work, int* info, uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    if (upper) {
        slatb4("STB", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
        ku = kd;
    ...
    if (imat <= 5) {
        ...
        slatms(n, n, dstr, symm, B, mode, cndnum, anorm,
        ...
        if (kd == 1) {
            if (upper) {
                AB[0 + 1 * ldab] = (rng_uniform_f32(state) < 0.5f ? -tnorm : tnorm);
                lenj = (n - 3) / 2;
                slarnv_rng(2, lenj, work, state);
                for (j = 0; j < lenj; j++) {
                    AB[0 + 2 * (j + 2) * ldab] = tnorm * work[j];
                }
            } else {
                AB[1 + 0 * ldab] = (rng_uniform_f32(state) < 0.5f ? -tnorm : tnorm);
                lenj = (n - 3) / 2;
                slarnv_rng(2, lenj, work, state);
                for (j = 0; j < lenj; j++) {
            ...
            star1 = (rng_uniform_f32(state) < 0.5f ? -tnorm : tnorm);
            sfac = sqrtf(tnorm);
            plus1 = (rng_uniform_f32(state) < 0.5f ? -sfac : sfac);
            for (j = 0; j < n; j += 2) {
                plus2 = star1 / plus1;
                work[j] = plus1;
                work[n + j] = star1;
                if (j + 1 < n) {
                    ...
                    rexp = rng_uniform_f32(state) * 2.0f - 1.0f;
                    if (rexp < ZERO) {
                        star1 = -powf(sfac, ONE - rexp);
                    } else {
                        star1 = powf(sfac, ONE + rexp);
                    }
                }
            ...
            if (upper) {
                cblas_scopy(n - 1, work, 1, &AB[(kd - 1) + 1 * ldab], ldab);
                cblas_scopy(n - 2, &work[n], 1, &AB[(kd - 2) + 2 * ldab], ldab);
            } else {
                cblas_scopy(n - 1, work, 1, &AB[1 + 0 * ldab], ldab);
                cblas_scopy(n - 2, &work[n], 1, &AB[2 + 0 * ldab], ldab);
            }
        }
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                ...
                slarnv_rng(2, lenj, &AB[(kd + 1 - lenj) + j * ldab], state);
            ...
            for (j = 0; j < n; j++) {
                ...
                if (lenj > 0)
                    slarnv_rng(2, lenj, &AB[0 + j * ldab], state);
                AB[0 + j * ldab] = (AB[0 + j * ldab] >= ZERO ? TWO : -TWO);
            }
        }

        slarnv_rng(2, n, B, state);
        iy = cblas_isamax(n, B, 1);
        bnorm = fabsf(B[iy]);
        bscal = bignum / (ONE > bnorm ? ONE : bnorm);
        cblas_sscal(n, bscal, B, 1);
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                lenj = (j + 1 < kd + 1 ? j + 1 : kd + 1);
                slarnv_rng(2, lenj, &AB[(kd + 1 - lenj) + j * ldab], state);
                cblas_sscal(lenj - 1, tscal, &AB[(kd + 1 - lenj) + j * ldab], 1);
            ...
            for (j = 0; j < n; j++) {
                ...
                slarnv_rng(2, lenj, &AB[0 + j * ldab], state);
                if (lenj > 1)
                    cblas_sscal(lenj - 1, tscal, &AB[1 + j * ldab], 1);
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                lenj = (j < kd ? j : kd);
                slarnv_rng(2, lenj, &AB[(kd - lenj) + j * ldab], state);
                AB[kd + j * ldab] = (f32)(j + 1);
            }
        } else {
            for (j = 0; j < n; j++) {
                ...
                if (lenj > 0)
                    slarnv_rng(2, lenj, &AB[1 + j * ldab], state);
                AB[0 + j * ldab] = (f32)(j + 1);
            }
        }
        ...
        tleft = bignum / (ONE > (f32)kd ? ONE : (f32)kd);
        tscal = bignum * ((f32)kd / (f32)(kd + 1));
        if (upper) {
            for (j = 0; j < n; j++) {
                lenj = (j + 1 < kd + 1 ? j + 1 : kd + 1);
                slarnv_rng(2, lenj, &AB[(kd + 1 - lenj) + j * ldab], state);
    ...
    if (!(trans[0] == 'N' || trans[0] == 'n')) {
        if (upper) {
            for (j = 0; j < n / 2; j++) {
                ...
                cblas_sswap(lenj, &AB[kd + j * ldab], ldab - 1,
                            &AB[(kd + 1 - lenj) + (n - 1 - j) * ldab], -1);
            }
        } else {
            for (j = 0; j < n / 2; j++) {
                lenj = (n - 2 * j - 1 < kd + 1 ? n - 2 * j - 1 : kd + 1);
                cblas_sswap(lenj, &AB[0 + j * ldab], 1,
