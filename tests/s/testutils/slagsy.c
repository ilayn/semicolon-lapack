/**
 * @file slagsy.c
 * @brief SLAGSY generates a real symmetric matrix A, by pre- and post-
 *        multiplying a real diagonal matrix D with a random orthogonal matrix.
 *
 * Faithful port of LAPACK TESTING/MATGEN/slagsy.f
    ...
    uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    for (i = n - 2; i >= 0; i--) {
        ...
        wn = cblas_snrm2(len, work, 1);
        wa = (work[0] >= 0.0f) ? wn : -wn;
        if (wn < safmin) {
            tau = ZERO;
        } else {
            wb = work[0] + wa;
            cblas_sscal(len - 1, ONE / wb, &work[1], 1);
            work[0] = ONE;
            tau = wb / wa;
        }
        ...
        /* Compute y := tau * A * u */
        cblas_ssymv(CblasColMajor, CblasLower, len, tau,
                    &A[i + i * lda], lda, work, 1, ZERO, &work[n], 1);

        /* Compute v := y - 1/2 * tau * (y, u) * u */
        alpha = -HALF * tau * cblas_sdot(len, &work[n], 1, work, 1);
        cblas_saxpy(len, alpha, work, 1, &work[n], 1);

        /* Apply the transformation as a rank-2 update to A(i:n-1, i:n-1) */
        cblas_ssyr2(CblasColMajor, CblasLower, len, -ONE, work, 1,
    ...
    for (i = 0; i < n - 1 - k; i++) {
        ...
        int len = n - k - i;
        wn = cblas_snrm2(len, &A[k + i + i * lda], 1);
        wa = (A[k + i + i * lda] >= 0.0f) ? wn : -wn;
        if (wn < safmin) {
            tau = ZERO;
        } else {
            wb = A[k + i + i * lda] + wa;
            cblas_sscal(len - 1, ONE / wb, &A[k + i + 1 + i * lda], 1);
            A[k + i + i * lda] = ONE;
            tau = wb / wa;
        }
        ...
        if (k - 1 > 0) {
            cblas_sgemv(CblasColMajor, CblasTrans, len, k - 1, ONE,
                        &A[k + i + (i + 1) * lda], lda,
                        &A[k + i + i * lda], 1, ZERO, work, 1);
            cblas_sger(CblasColMajor, len, k - 1, -tau,
        ...
        /* Compute v := y - 1/2 * tau * (y, u) * u */
        alpha = -HALF * tau * cblas_sdot(len, work, 1, &A[k + i + i * lda], 1);
        cblas_saxpy(len, alpha, &A[k + i + i * lda], 1, work, 1);

        /* Apply symmetric rank-2 update to A(k+i:n-1, k+i:n-1) */
        cblas_ssyr2(CblasColMajor, CblasLower, len, -ONE,
                    &A[k + i + i * lda], 1, work, 1,
                    &A[k + i + (k + i) * lda], lda);
