/**
 * @file slattp.c
 * @brief SLATTP generates a triangular test matrix in packed storage.
 *
 * Port of LAPACK TESTING/LIN/slattp.f to C.
 *
 * IMAT and UPLO uniquely specify the properties of the test matrix:
 *   IMAT 1-6:   Non-unit triangular generated via SLATMS
 *   IMAT 7:     Unit triangular identity
 *   IMAT 8-10:  Non-trivial unit triangular with controlled condition
 *   IMAT 11-19: Pathological test cases for SLATPS
 */
            ...
            INT* info, uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    if (upper) {
        slatb4("STP", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
        packit = 'C';
    } else {
        slatb4("STP", -imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
        packit = 'R';
    ...
    /* IMAT <= 6: Non-unit triangular matrix via SLATMS */
    if (imat <= 6) {
        char symm[2] = "N";
        symm[0] = type;
        char dstr[2] = "S";
    ...
    /* IMAT 8-10: Non-trivial unit triangular with controlled condition */
        ...
        if (upper) {
            ...
            for (j = 0; j < n - 1; j++) {
                ...
                if (j > 0) {
                    cblas_srot(j, &A[jcnext], 1, &A[jc], 1, -c, -s);
                }
            ...
            for (j = 0; j < n - 1; j++) {
                ...
                /* Multiply by [ c -s;  s  c] on the right */
                if (n > j + 2) {
                    cblas_srot(n - j - 2, &A[jcnext + 1], 1, &A[jc + 2], 1, c, -s);
                }
     ...
     * In type 12, the offdiagonal elements are small (CNORM(j) < 1). */
        ...
        if (upper) {
            jc = 0;
            for (j = 0; j < n; j++) {
                slarnv_rng(2, j, &A[jc], state);
                cblas_sscal(j, tscal, &A[jc], 1);
                A[jc + j] = (rng_uniform_f32(state) >= 0.5f) ? ONE : -ONE;
                jc += j + 1;
            }
            A[n * (n + 1) / 2 - 1] = smlnum;
        } else {
            jc = 0;
            for (j = 0; j < n; j++) {
                slarnv_rng(2, n - j - 1, &A[jc + 1], state);
                cblas_sscal(n - j - 1, tscal, &A[jc + 1], 1);
                A[jc] = (rng_uniform_f32(state) >= 0.5f) ? ONE : -ONE;
                jc += n - j;
            }
            A[0] = smlnum;
        }
     ...
     * every other step. */
        ...
        if (upper) {
            ...
            jc = 0;
            for (j = 0; j < n - 1; j += 2) {
                A[jc + n - j - 1] = -tscal / (f32)(n + 1);
                A[jc] = ONE;
                B[j] = texp * (ONE - ulp);
                jc += n - j;
                A[jc + n - j - 2] = -(tscal / (f32)(n + 1)) / (f32)(n + 2);
                A[jc] = ONE;
                B[j + 1] = texp * (f32)(n * n + n - 1);
                texp *= TWO;
                jc += n - j - 1;
