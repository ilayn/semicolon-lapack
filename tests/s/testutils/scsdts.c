/**
 * @file scsdts.c
 * @brief SCSDTS tests SORCSD and SORCSD2BY1 (CS decomposition).
 */
            ...
            f32* rwork, f32* result)
{
    ...
    f32 eps2;
    if (m > 0) {
        eps2 = MAX(ulp, slange("1", m, m, work, ldx, rwork) / (f32)m);
    } else {
        eps2 = ulp;
    }
    ...
    for (INT i = 0; i < MIN(p, q) - r; i++)
        XF[i + i * ldx] -= one;
    for (INT i = 0; i < r; i++)
        XF[(MIN(p, q) - r + i) + (MIN(p, q) - r + i) * ldx] -= cosf(theta[i]);
    ...
    resid = slange("1", p, q, &XF[0], ldx, rwork);
    result[0] = (resid / (f32)MAX3(1, p, q)) / eps2;

    resid = slange("1", p, m - q, &XF[q * ldx], ldx, rwork);
    result[1] = (resid / (f32)MAX3(1, p, m - q)) / eps2;

    resid = slange("1", m - p, q, &XF[p], ldx, rwork);
    result[2] = (resid / (f32)MAX3(1, m - p, q)) / eps2;

    resid = slange("1", m - p, m - q, &XF[p + q * ldx], ldx, rwork);
    ...
    if (m > 0) {
        eps2 = MAX(ulp, slange("1", q, q, work, ldx, rwork) / (f32)m);
    } else {
        eps2 = ulp;
    }
