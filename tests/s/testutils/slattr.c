/**
 * @file slattr.c
 * @brief SLATTR generates a triangular test matrix.
 *
 * Port of LAPACK TESTING/LIN/slattr.f to C.
 *
 ...
 *   IMAT 11-19: Pathological test cases for SLATRS
 */
            ...
            f32* B, f32* work, int* info, uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    if (upper) {
        slatb4("STR", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    } else {
        slatb4("STR", -imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    }
    ...
    /* IMAT 8-10: Non-trivial unit triangular with controlled condition */
        ...
        for (j = 0; j < n; j += 2) {
            ...
            if (j + 1 < n) {
                ...
                work[n + j + 1] = ZERO;
                plus1 = star1 / plus2;
                /* Generate a random value in (-1, 1) using slarnv with idist=2 */
                rng_fill_f32(state, 2, 1, &rexp);
                star1 = star1 * powf(sfac, rexp);
                if (rexp < ZERO) {
    ...
    /* IMAT = 11: Well-conditioned with large RHS */
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                rng_fill_f32(state, 2, j + 1, &A[j * lda]);
                A[j * lda + j] = (A[j * lda + j] >= ZERO) ? TWO : -TWO;
            }
        } else {
            for (j = 0; j < n; j++) {
                rng_fill_f32(state, 2, n - j, &A[j * lda + j]);
                A[j * lda + j] = (A[j * lda + j] >= ZERO) ? TWO : -TWO;
            }
        }

        /* Set RHS so largest value is BIGNUM */
        rng_fill_f32(state, 2, n, B);
    ...
    /* IMAT = 12: Small diagonal, small off-diagonal (CNORM < 1) */
        ...
        if (upper) {
            ...
            for (j = 0; j < n; j++) {
                rng_fill_f32(state, 2, n - j, &A[j * lda + j]);
                if (j < n - 1) {
                    cblas_sscal(n - j - 1, tscal, &A[j * lda + j + 1], 1);
                }
                A[j * lda + j] = (A[j * lda + j] >= ZERO) ? ONE : -ONE;
            }
            A[0] = smlnum * A[0];
    ...
    /* IMAT = 18: Unit triangular with large RHS */
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                if (j > 0) {
                    rng_fill_f32(state, 2, j, &A[j * lda]);
                }
                A[j * lda + j] = ZERO;
            }
        } else {
            for (j = 0; j < n; j++) {
                if (j < n - 1) {
                    rng_fill_f32(state, 2, n - j - 1, &A[j * lda + j + 1]);
                }
                A[j * lda + j] = ZERO;
            }
        }
    ...
    /* IMAT = 19: Large elements causing column norm to exceed BIGNUM */
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                rng_fill_f32(state, 2, j + 1, &A[j * lda]);
                for (i = 0; i <= j; i++) {
                    f32 sign = (A[j * lda + i] >= ZERO) ? ONE : -ONE;
