/**
 * @file sget01.c
 * @brief SGET01 reconstructs a matrix A from its L*U factorization and
 ...
 *                        by sgetrf. Overwritten with the reconstructed matrix,
 ...
 *                        The pivot indices from sgetrf. 0-based indexing.
    ...
    f32 *resid)
{
    ...
    for (k = n - 1; k >= 0; k--) {
        if (k >= m) {
            ...
            if (k + 1 < m) {
                ...
                cblas_sscal(m - k - 1, t, &AFAC[k + 1 + k * ldafac], 1);
                // DGEMV: AFAC(k+1:m-1, k) += L(k+1:m-1, 0:k-1) * U(0:k-1, k)
                if (k > 0) {
                    cblas_sgemv(CblasColMajor, CblasNoTrans, m - k - 1, k, ONE,
            ...
            if (k > 0) {
                AFAC[k + k * ldafac] = t + cblas_sdot(k, &AFAC[k], ldafac,
                                                       &AFAC[k * ldafac], 1);
            }
    ...
    // Apply inverse row permutation: SLASWP with incx = -1
    slaswp(n, AFAC, ldafac, 0, minmn - 1, ipiv, -1);
