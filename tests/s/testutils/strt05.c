/**
 * @file strt05.c
 * @brief STRT05 tests the error bounds from iterative refinement for triangular systems.
 *
 * Port of LAPACK TESTING/LIN/strt05.f to C.
 */
            ...
            const f32* ferr, const f32* berr, f32* reslts)
{
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        xnorm = fabsf(X[j * ldx + imax]);
        if (xnorm < unfl) xnorm = unfl;

        diff = ZERO;
        for (i = 0; i < n; i++) {
            f32 d = fabsf(X[j * ldx + i] - XACT[j * ldxact + i]);
            if (d > diff) diff = d;
        }
        ...
        for (i = 0; i < n; i++) {
            ...
            if (upper) {
                if (!notran) {
                    ...
                    for (j = 0; j < i - ifu + 1; j++) {
                        tmp += fabsf(A[i * lda + j]) * fabsf(X[k * ldx + j]);
                    }
                    if (unit)
                        tmp += fabsf(X[k * ldx + i]);
                } else {
                    ...
                    if (unit)
                        tmp += fabsf(X[k * ldx + i]);
                    for (j = i + ifu; j < n; j++) {
                        tmp += fabsf(A[j * lda + i]) * fabsf(X[k * ldx + j]);
                    }
                }
            } else {
                /* Lower triangular */
