/**
 * @file slagge.c
 * @brief SLAGGE generates a real general m by n matrix A, by pre- and post-
 *        multiplying a real diagonal matrix D with random orthogonal matrices.
 *
 * Faithful port of LAPACK TESTING/MATGEN/slagge.f
    ...
    *info = 0;
    ...
    for (i = minmn - 1; i >= 0; i--) {
        if (i < m - 1) {
            ...
            cblas_sgemv(CblasColMajor, CblasTrans, len, n - i, ONE,
                        &A[i + i * lda], lda, work, 1, ZERO, &work[m], 1);
            cblas_sger(CblasColMajor, len, n - i, -tau, work, 1,
        ...
        if (i < n - 1) {
            ...
            cblas_sgemv(CblasColMajor, CblasNoTrans, m - i, len, ONE,
                        &A[i + i * lda], lda, work, 1, ZERO, &work[n], 1);
            cblas_sger(CblasColMajor, m - i, len, -tau, &work[n], 1,
        ...
        if (kl <= ku) {
            ...
            if (i < m - 1 - kl && i < n) {
                ...
                if (wn == ZERO) {
                    ...
                    wb = A[kl + i + i * lda] + wa;
                    cblas_sscal(len - 1, ONE / wb, &A[kl + i + 1 + i * lda], 1);
                    A[kl + i + i * lda] = ONE;
                    tau = wb / wa;
                }
                ...
                if (n - i - 1 > 0) {
                    cblas_sgemv(CblasColMajor, CblasTrans, len, n - i - 1, ONE,
                                &A[kl + i + (i + 1) * lda], lda,
                                &A[kl + i + i * lda], 1, ZERO, work, 1);
                    cblas_sger(CblasColMajor, len, n - i - 1, -tau,
            ...
            if (i < n - 1 - ku && i < m) {
                ...
                if (wn == ZERO) {
                    ...
                    wb = A[i + (ku + i) * lda] + wa;
                    cblas_sscal(len - 1, ONE / wb, &A[i + (ku + i + 1) * lda], lda);
                    A[i + (ku + i) * lda] = ONE;
                    tau = wb / wa;
                }
                ...
                if (m - i - 1 > 0) {
                    cblas_sgemv(CblasColMajor, CblasNoTrans, m - i - 1, len, ONE,
                                &A[i + 1 + (ku + i) * lda], lda,
                                &A[i + (ku + i) * lda], lda, ZERO, work, 1);
                    cblas_sger(CblasColMajor, m - i - 1, len, -tau, work, 1,
                               &A[i + (ku + i) * lda], lda,
                               &A[i + 1 + (ku + i) * lda], lda);
                }
                A[i + (ku + i) * lda] = -wa;
