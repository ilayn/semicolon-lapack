/**
 * @file slavsy_rook.c
 * @brief SLAVSY_ROOK performs one of the matrix-vector operations x := A*x or
 *        x := A'*x, where A is a factor from the block U*D*U' or L*D*L'
 *        factorization computed by SSYTRF_ROOK.
 */
 ...
 *                       as determined by SSYTRF_ROOK. Integer array, dimension (n).
 *                       0-based indexing.
            ...
            while (k < n) {
                if (ipiv[k] >= 0) {
                    ...
                    if (k > 0) {
                        ...
                        cblas_sger(CblasColMajor, k, nrhs, ONE, &A[0 + k * lda], 1,
                                   &B[k], ldb, &B[0], ldb);
                    ...
                    if (k > 0) {
                        ...
                        cblas_sger(CblasColMajor, k, nrhs, ONE, &A[0 + k * lda], 1,
                                   &B[k], ldb, &B[0], ldb);
                        cblas_sger(CblasColMajor, k, nrhs, ONE, &A[0 + (k + 1) * lda], 1,
                                   &B[k + 1], ldb, &B[0], ldb);
            ...
            while (k >= 0) {
                ...
                if (ipiv[k] >= 0) {
                    ...
                    if (k != n - 1) {
                        ...
                        cblas_sger(CblasColMajor, n - k - 1, nrhs, ONE, &A[(k + 1) + k * lda], 1,
                                   &B[k], ldb, &B[k + 1], ldb);
                    ...
                    if (k != n - 1) {
                        ...
                        cblas_sger(CblasColMajor, n - k - 1, nrhs, ONE, &A[(k + 1) + k * lda], 1,
                                   &B[k], ldb, &B[k + 1], ldb);
                        cblas_sger(CblasColMajor, n - k - 1, nrhs, ONE, &A[(k + 1) + (k - 1) * lda], 1,
                                   &B[k - 1], ldb, &B[k + 1], ldb);
