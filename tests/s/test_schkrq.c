/**
 * @file test_schkrq.c
 * @brief Comprehensive test suite for RQ factorization (SRQ) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/dchkrq.f to C using CMocka.
 * Tests SGERQF, SORGRQ, and SORMRQ.
 *
 * Each (m, n, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from dchkrq.f:
 *   TEST 1-2: RQ factorization via srqt01 (norm(R - A*Q'), norm(I - Q*Q'))
 *   TEST 3-6: SORMRQ tests via srqt03 (applying Q from left/right with trans)
 *   TEST 7: Least squares solve via sgerqs and sget02
 *
...
#define NTYPES  8
#define NTESTS  7   /* We test 1-2 (srqt01), 3-6 (srqt03), 7 (sgerqs) */
#define THRESH  30.0f
...
extern void srqt01(const int m, const int n, const f32* A, f32* AF,
                   f32* Q, f32* R, const int lda, f32* tau,
                   f32* work, const int lwork, f32* rwork,
                   f32* result);
extern void srqt02(const int m, const int n, const int k,
                   const f32* A, const f32* AF, f32* Q, f32* R,
                   const int lda, const f32* tau,
                   f32* work, const int lwork, f32* rwork,
                   f32* result);
extern void srqt03(const int m, const int n, const int k,
...
/* SGERQS - solve using RQ factorization (not a standard LAPACK routine,
 * but used in LAPACK testing). We implement it inline here. */
static void sgerqs(const int m, const int n, const int nrhs,
                   ...
                   f32* work, const int lwork, int* info)
{
    *info = 0;
    ...
    strtrs("U", "N", "N", m, nrhs, &A[0 + (n - m) * lda], lda,
           B, ldb, info);
    if (*info != 0) return;
...
typedef struct {
    ...
    f32* C;      /* Workspace for srqt03 (NMAX x NMAX) */
    f32* CC;     /* Another workspace for srqt03 (NMAX x NMAX) */
...
static void run_dchkrq_single(int m, int n, int imat, int inb)
{
    ...
    slatb4("SRQ", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    for (int ik = 0; ik < nk; ik++) {
        ...
        if (ik == 0) {
            /*
             * TEST 1-2: Test SGERQF via srqt01
             * Computes:
            ...
            srqt01(m, n, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
            ...
            /*
             * TEST 1-2: Test SORGRQ via srqt02
             * Uses factorization from srqt01
             * Note: For RQ, we test when m <= n (wide/square matrices)
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 1-2 (SORGRQ)", m, n, imat, k);
            set_test_context(ctx);
            srqt02(m, n, k, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                   ws->WORK, lwork, ws->RWORK, result);
        ...
        if (n >= k) {
            /*
             * TEST 3-6: Test SORMRQ via srqt03
             * Tests Q from left/right with 'N'/'T' transpose options
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 3-6 (SORMRQ)", m, n, imat, k);
            set_test_context(ctx);
            srqt03(m, n, k, ws->AF, ws->C, ws->CC, ws->Q, lda, ws->TAU,
            ...
            /*
             * TEST 7: If M <= N and K == M, test SGERQS least squares solve
            ...
            if (k == m && m <= n && m > 0 && inb == 0) {
                ...
                snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d nrhs=%d TEST 7 (SGERQS)", m, n, imat, k, nrhs);
                set_test_context(ctx);

                /* Generate right-hand side */
                slarhs("SRQ", "N", "F", "N", m, n, 0, 0, nrhs,
                       ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

                slacpy("F", m, nrhs, ws->B, lda, ws->X, lda);

                /* Use the factorization already in AF */
                sgerqs(m, n, nrhs, ws->AF, lda, ws->TAU, ws->X, lda,
                       ws->WORK, lwork, &info);
                assert_int_equal(info, 0);
