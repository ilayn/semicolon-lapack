...
static void test_dchkst_case(void** state)
{
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SSYTRD(L) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[2] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "SORGTR(L) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[3] = ulpinv;
        goto L280;
    }
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dchkst", g_tests, g_num_tests,
