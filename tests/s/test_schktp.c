...
static void test_latps(void** state)
{
    ...
    for (INT itran = 0; itran < 3; itran++) {
        ...
        if (info != 0) {
            fprintf(stderr, "SLATPS returned INFO=%d for n=%d, imat=%d, uplo=%s, trans=%s\n",
        ...
        if (info != 0) {
            fprintf(stderr, "SLATPS returned INFO=%d for n=%d, imat=%d, uplo=%s, trans=%s, normin=Y\n",
                         info, n, imat, uplo, trans);
        }
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dchktp", g_tests, (size_t)g_num_tests,
