/**
 * @file test_sdrvrf3.c
 * @brief Test suite for STFSM (RFP triangular solve).
 *
 * Port of LAPACK's TESTING/LIN/ddrvrf3.f to C using CMocka.
 * Tests STFSM by comparing it against cblas_dtrsm on the same input:
...
extern void stfsm(const char* transr, const char* side, const char* uplo,
                  ...
                  const f32 alpha, const f32* A, f32* B, const int ldb);
extern void sgeqrf(const int m, const int n, f32* A, const int lda,
                   f32* tau, f32* work, const int lwork, int* info);
extern void sgelqf(const int m, const int n, f32* A, const int lda,
                   f32* tau, f32* work, const int lwork, int* info);
                                ...
                                int iside, int itrans, int idiag, int ialpha)
{
    ...
    if (iuplo == 0) {
        sgeqrf(na, na, A, lda, TAU, D_WORK_DGEQRF, lda, &info);
        ...
        sgelqf(na, na, A, lda, TAU, D_WORK_DGEQRF, lda, &info);
    ...
    for (int j = 0; j < n; j++)
        for (int i = 0; i < m; i++) {
            B1[i + j * lda] = rng_uniform_symmetric_f32(rng_state);
    ...
    cblas_strsm(CblasColMajor, sideC, uploC, transC, diagC, m, n,
                alpha, A, lda, B1, lda);
    ...
    f32 result = slange("I", m, n, B1, lda, D_WORK_DLANGE);
