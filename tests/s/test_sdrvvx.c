 ...
 *
 * Tests the nonsymmetric eigenvalue problem expert driver SGEEVX.
 *
...
extern void sgeevx(const char* balanc, const char* jobvl, const char* jobvr,
                   ...
                   f32* wr, f32* wi, f32* VL, const int ldvl,
                   f32* VR, const int ldvr, int* ilo, int* ihi,
                   f32* scale, f32* abnrm, f32* rconde, f32* rcondv,
...
static void run_ddrvvx_random(ddrvvx_params_t* params)
{
    ...
    for (int ibal = 0; ibal < 4; ibal++) {
        sget23(0, BAL[ibal], jtype, THRESH, n,
...
static void run_ddrvvx_precomp(ddrvvx_params_t* params)
{
    ...
    const svx_precomputed_t* pc = &DVX_PRECOMPUTED[idx];
    int n = pc->n;
    ...
    /* Call sget23 with COMP=1 (ddrvvx.f lines 923-927) */
    sget23(1, "N", 22, THRESH, n,
           A, lda, H, WR, WI, WR1, WI1,
           VL, ldvl, VR, ldvr, LRE, ldlre,
           rcondv, rcndv1, rcdvin, rconde, rcnde1, rcdein,
           scale_, scale1, result, work, nnwork, iwork, &info);
