...
static void run_ddrvev_single(ddrvev_params_t* params)
{
    ...
    if (iinfo != 0) {
        ws->result[0] = ulpinv;
        fprintf(stderr, "SGEEV(V,V) failed with info=%d\n", iinfo);
        assert_info_success(iinfo);
        return;
    }
    ...
    if (iinfo != 0) {
        ws->result[4] = ulpinv;
        fprintf(stderr, "SGEEV(N,N) failed with info=%d\n", iinfo);
    } else {
    ...
    if (iinfo != 0) {
        ws->result[5] = ulpinv;
        fprintf(stderr, "SGEEV(N,V) failed with info=%d\n", iinfo);
    } else {
        /* Test 5 again */
        for (INT j = 0; j < n; j++) {
            if (WR[j] != WR1[j] || WI[j] != WI1[j]) {
    ...
    if (iinfo != 0) {
        ws->result[6] = ulpinv;
        fprintf(stderr, "SGEEV(V,N) failed with info=%d\n", iinfo);
    } else {
        /* Test 5 again */
        for (INT j = 0; j < n; j++) {
            if (WR[j] != WR1[j] || WI[j] != WI1[j]) {
