...
extern void sgeev(const char* jobvl, const char* jobvr, const int n,
                  f32* A, const int lda, f32* wr, f32* wi,
...
/**
 * Compute 2-norm using LAPACK's slapy2 pattern.
 */
static f32 slapy2(f32 x, f32 y)
{
    f32 xabs = fabsf(x);
    f32 yabs = fabsf(y);
    f32 w = (xabs > yabs) ? xabs : yabs;
    f32 z = (xabs < yabs) ? xabs : yabs;

    if (z == 0.0f) {
        return w;
    }
    f32 temp = z / w;
    return w * sqrtf(1.0f + temp * temp);
}
