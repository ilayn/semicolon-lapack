 ...
 *   Type 1: Random orthogonal matrix via SLAROR
...
static void dlacsg(const INT m, const INT p, const INT q,
                   const f32* theta, f32* X, const INT ldx,
                   f32* work, uint64_t rng[static 4])
{
    INT r = MIN4(p, m - p, q, m - q);
    INT info;

    slaset("Full", m, m, 0.0f, 0.0f, X, ldx);

    for (INT i = 0; i < MIN(p, q) - r; i++)
        X[i + i * ldx] = 1.0f;
    for (INT i = 0; i < r; i++)
        X[(MIN(p, q) - r + i) + (MIN(p, q) - r + i) * ldx] = cosf(theta[i]);

    for (INT i = 0; i < MIN(p, m - q) - r; i++)
        X[(p - 1 - i) + (m - 1 - i) * ldx] = -1.0f;
    for (INT i = 0; i < r; i++) {
        INT k = MIN(p, m - q) - r;
        X[(p - k - 1 - i) + (m - k - 1 - i) * ldx] = -sinf(theta[r - 1 - i]);
    }

    for (INT i = 0; i < MIN(m - p, q) - r; i++)
        X[(m - 1 - i) + (q - 1 - i) * ldx] = 1.0f;
    for (INT i = 0; i < r; i++) {
        INT k2 = MIN(m - p, q) - r;
        X[(m - k2 - 1 - i) + (q - k2 - 1 - i) * ldx] = sinf(theta[r - 1 - i]);
    }
    ...
    for (INT i = 0; i < r; i++) {
        ...
        X[(p + k3 + i) + (q + k3 + i) * ldx] = cosf(theta[i]);
    }

    slaror("Left", "No init", p, m, X, ldx, work, &info, rng);
    slaror("Left", "No init", m - p, m, &X[p], ldx, work, &info, rng);
    slaror("Right", "No init", m, q, X, ldx, work, &info, rng);
    slaror("Right", "No init", m, m - q, &X[q * ldx], ldx, work, &info, rng);
}
...
static int group_setup(void** state)
{
    ...
    g_ws->X     = malloc((size_t)ldx * mmax * sizeof(f32));
    g_ws->XF    = malloc((size_t)ldx * mmax * sizeof(f32));
    g_ws->U1    = malloc((size_t)ldx * mmax * sizeof(f32));
    g_ws->U2    = malloc((size_t)ldx * mmax * sizeof(f32));
    g_ws->V1T   = malloc((size_t)ldx * mmax * sizeof(f32));
    g_ws->V2T   = malloc((size_t)ldx * mmax * sizeof(f32));
    g_ws->theta = malloc((size_t)mmax * sizeof(f32));
    g_ws->iwork = malloc((size_t)mmax * sizeof(INT));
    g_ws->rwork = malloc((size_t)mmax * sizeof(f32));
...
static void run_dckcsd_single(dckcsd_params_t* params)
{
    ...
    if (imat == 1) {
        slaror("Left", "I", m, m, X, ldx, work, &iinfo, rng);
        if (m != 0 && iinfo != 0) {
            snprintf(ctx, sizeof(ctx),
                     "SLAROR in SCKCSD: M=%d, INFO=%d", m, iinfo);
        ...
        slaset("Full", m, m, 0.0f, 1.0f, X, ldx);
        for (INT i = 0; i < m; i++) {
            INT j = (INT)(rng_uniform_f32(rng) * m);
            if (j != i) {
                cblas_srot(m, &X[i * ldx], 1, &X[j * ldx], 1, 0.0f, 1.0f);
            }
        }
    }
    ...
    scsdts(m, p, q, X, XF, ldx, U1, ldx, U2, ldx,
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dckcsd", all_tests, ntests,
