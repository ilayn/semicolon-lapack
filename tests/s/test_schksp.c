...
extern void ssptri(const char* uplo, const int n, f32* AP, const int* ipiv,
                   f32* work, int* info);
extern void ssptrs(const char* uplo, const int n, const int nrhs,
                   const f32* AP, const int* ipiv, f32* B, const int ldb, int* info);
extern void ssprfs(const char* uplo, const int n, const int nrhs,
                   const f32* AP, const f32* AFP, const int* ipiv,
                   const f32* B, const int ldb,
                   f32* X, const int ldx, f32* ferr, f32* berr,
                   f32* work, int* iwork, int* info);
extern void sspcon(const char* uplo, const int n, const f32* AP, const int* ipiv,
...
typedef struct {
    ...
    f32* C;      /* Full format workspace for sspt01 */
...
static void run_dchksp_single(int n, int iuplo, int imat)
{
    ...
    ssptrf(uplo_str, n, ws->AFAC, ipiv, &info);
    ...
    sspt01(uplo_str, n, ws->A, ws->AFAC, ipiv, ws->C, lda, ws->RWORK, &result[0]);
    ...
    if (!trfcon) {
        ...
        ssptri(uplo_str, n, ws->AINV, ipiv, ws->WORK, &info);
        assert_int_equal(info, 0);
    ...
    for (int irhs = 0; irhs < (int)NNS; irhs++) {
        ...
        slarhs("SSP", "N", uplo_str, " ", n, n, kl, ku, nrhs,
               ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

        slacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        ssptrs(uplo_str, n, nrhs, ws->AFAC, ipiv, ws->X, lda, &info);
        assert_int_equal(info, 0);
        ...
        ssprfs(uplo_str, n, nrhs, ws->A, ws->AFAC, ipiv,
...
test8:
    ...
    sspcon(uplo_str, n, ws->AFAC, ipiv, anorm, &rcond, ws->WORK,
           ws->IWORK + n, &info);
    assert_int_equal(info, 0);
