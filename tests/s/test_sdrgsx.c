...
static void test_ddrgsx(void** state)
{
    ...
    if (linfo != 0 && linfo != mplusn + 2) {
        fprintf(stderr, "SGGESX returned INFO=%d for sense=%s type=%d m=%d n=%d\n",
...
static void test_ddrgsx_readin(void** state)
{
    ...
    if (linfo != 0 && linfo != mplusn + 2) {
        fprintf(stderr, "SGGESX returned INFO=%d for read-in example #%d\n",
                      linfo, ci + 1);
        result[0] = ulpinv;
        ...
        if (g_ws->alphai[j] == 0.0f) {
            ...
            if (!ilabad) {
                ...
                sget53(&g_ws->AI[i1 + i1 * lda], lda,
                       ...
                       &temp2, &iinfo);
                if (iinfo >= 3) {
                    fprintf(stderr, "SGET53 returned INFO=%d for eigenvalue %d "
                                  "(read-in #%d)\n", iinfo, j + 1, ci + 1);
                }
            } else {
                temp2 = ulpinv;
...
int main(void)
{
    ...
    (void)cmocka_run_group_tests_name("ddrgsx", all_tests, group_setup,
                                       group_teardown);
    return 0;
}
