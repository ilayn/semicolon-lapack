 ...
 *
 * Tests SGGLSE - the linear equality constrained least squares solver:
...
static int group_setup(void** state)
{
    ...
    g_ws->C     = malloc(NMAX * sizeof(f32));
    g_ws->CF    = malloc(NMAX * sizeof(f32));
    g_ws->D     = malloc(NMAX * sizeof(f32));
    g_ws->DF    = malloc(NMAX * sizeof(f32));
    g_ws->X     = malloc(NMAX * sizeof(f32));
...
static void test_dcklse_case(void** state)
{
    ...
    /* Test SGGLSE */
    f32 result[2];
    slsets(m, p, n, g_ws->A, g_ws->AF, lda, g_ws->B, g_ws->BF, ldb,
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dcklse", g_tests, g_num_tests,
