 ...
 *
 * Tests SGGGLM - the generalized linear model solver:
...
static void test_dckglm_case(void** state)
{
    ...
    slatms(n, p, distb_str, type_str, g_ws->rwork, modeb, cndnmb,
           bnorm, klb, kub, "N", g_ws->B, ldb, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    /* Generate random left hand side vector of GLM */
    slarnv_rng(2, n, g_ws->D, rng_state);

    /* Test SGGGLM */
    f32 resid;
    sglmts(n, m, p, g_ws->A, g_ws->AF, lda, g_ws->B, g_ws->BF, ldb,
           g_ws->D, g_ws->DF, g_ws->X, g_ws->U,
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dckglm", g_tests, g_num_tests,
