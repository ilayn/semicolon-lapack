/**
 * @file test_sgetc2.c
 * @brief CMocka test suite for sgetc2 (LU factorization with complete pivoting).
 *
...
typedef struct {
    int n;
    int lda;
    f32 *A;       /* Factored matrix (overwritten by sgetc2) */
 ...
 */
static void apply_row_perm(int n, f32 *A, int lda, const int *ipiv, int dir)
{
    if (dir > 0) {
        for (int i = 0; i < n - 1; i++) {
            if (ipiv[i] != i) {
                cblas_sswap(n, &A[i], lda, &A[ipiv[i]], lda);
            }
        }
    } else {
        for (int i = n - 2; i >= 0; i--) {
 ...
 */
static void apply_col_perm(int n, f32 *A, int lda, const int *jpiv, int dir)
{
    if (dir > 0) {
        for (int j = 0; j < n - 1; j++) {
            if (jpiv[j] != j) {
                cblas_sswap(n, &A[j * lda], 1, &A[jpiv[j] * lda], 1);
            }
        }
    } else {
        for (int j = n - 2; j >= 0; j--) {
 ...
 */
static f32 verify_lu(int n, const f32 *A_orig, const f32 *LU,
                        int lda, const int *ipiv, const int *jpiv)
{
    f32 eps = slamch("E");
    f32 *lwork = malloc(n * sizeof(f32));
    f32 *L = calloc(n * n, sizeof(f32));
    f32 *U = calloc(n * n, sizeof(f32));
    f32 *PLU = malloc(n * n * sizeof(f32));
    f32 *PLUQ = malloc(n * n * sizeof(f32));
    ...
    f32 anorm = slange("1", n, n, A_orig, lda, lwork);
    f32 resid_norm = slange("1", n, n, PLUQ, n, lwork);
 ...
 */
static f32 run_dgetc2_test(dgetc2_fixture_t *fix, int imat)
{
    char type, dist;
    int kl, ku, mode;
    f32 anorm, cndnum;
    ...
    /* info > 0 means matrix was perturbed; not an error for sgetc2 */
...
static void test_dgetc2_illcond(void **state)
{
    dgetc2_fixture_t *fix = *state;
    fix->seed = g_seed++;
    f32 resid = run_dgetc2_test(fix, 8);
    assert_residual_ok(resid);
}
...
static void test_dgetc2_identity(void **state)
{
    ...
    f32 resid = verify_lu(n, A_orig, A, n, ipiv, jpiv);
    assert_residual_ok(resid);
...
static void test_dgetc2_simple(void **state)
{
    ...
    f32 resid = verify_lu(3, A_orig, A, 3, ipiv, jpiv);
    assert_residual_ok(resid);
}
