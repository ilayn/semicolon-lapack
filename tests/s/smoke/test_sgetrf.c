/**
 * @file test_sgetrf.c
 * @brief CMocka test suite for sgetrf (LU factorization with partial pivoting).
 *
 * Tests the blocked LU factorization routine sgetrf using LAPACK's
 ...
 *
 * Verification: sget01 computes ||L*U - A|| / (N * ||A|| * eps)
 *
...
/**
 * Run the sgetrf test for a single (m, n, imat) combination.
 */
static void run_dgetrf_single(INT m, INT n, INT imat)
{
    ...
    if (zerot) {
        ...
        if (imat < 7) {
            ...
            slaset("F", m, n - izero + 1, 0.0f, 0.0f, &ws->A[ioff], lda);
        }
    }
    ...
    /* Verify factorization using sget01 */
    f32 resid;
    f32* AFAC_copy = malloc(lda * n * sizeof(f32));
    assert_non_null(AFAC_copy);
    memcpy(AFAC_copy, ws->AFAC, lda * n * sizeof(f32));

    sget01(m, n, ws->A, lda, AFAC_copy, lda, ws->IPIV, ws->RWORK, &resid);
    free(AFAC_copy);
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("dgetrf", g_tests, g_num_tests,
