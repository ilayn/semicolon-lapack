/**
 * @file test_sgtrfs.c
 * @brief CMocka test suite for sgtrfs (iterative refinement for tridiagonal systems).
 *
 * Tests the iterative refinement routine sgtrfs which improves the computed
 * solution and provides error bounds.
 *
 * Verification: sgtt05 tests the error bounds from iterative refinement.
...
typedef struct {
    ...
    f32 *X;       /* Computed/refined solution */
    f32 *B;       /* Right-hand side */
    f32 *ferr;    /* Forward error estimates */
    f32 *berr;    /* Backward error estimates */
    f32 *work;    /* Workspace for sgtrfs */
    int *iwork;      /* Integer workspace for sgtrfs */
                           ...
                           f32 *reslts)
{
    ...
    memcpy(fix->X, fix->B, ldx * nrhs * sizeof(f32));
    sgttrs(trans, n, nrhs, fix->DLF, fix->DF, fix->DUF, fix->DU2, fix->ipiv,
            fix->X, ldx, &info);
    assert_info_success(info);

    /* Apply iterative refinement */
    sgtrfs(trans, n, nrhs, fix->DL, fix->D, fix->DU, fix->DLF, fix->DF, fix->DUF,
            fix->DU2, fix->ipiv, fix->B, ldb, fix->X, ldx, fix->ferr, fix->berr,
            fix->work, fix->iwork, &info);
    assert_info_success(info);

    /* Verify error bounds using sgtt05 */
    sgtt05(trans, n, nrhs, fix->DL, fix->D, fix->DU, fix->B, ldb, fix->X, ldx,
