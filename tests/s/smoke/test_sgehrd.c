/**
 * @file test_sgehrd.c
 * @brief Test Hessenberg reduction routine sgehrd.
 *
...
typedef struct {
    ...
    f32* H;       /* Hessenberg result */
    f32* Q;       /* Orthogonal matrix */
    f32* work;    /* Workspace */
 ...
 */
static void generate_test_matrix(INT itype, INT n, f32* A, INT lda, f32 anorm,
                                 uint64_t state[static 4])
{
    ...
    f32 ulp = slamch("P");
    (void)ulp;  /* Used in some matrix types via slatm4 */
    ...
    switch (itype) {
        ...
        case 6:
            ...
            /* For simplicity, use slatm4 for eigenvalue test matrices */
            {
                INT mode = (itype == 4) ? 4 : (itype == 5) ? 3 : 1;
                slatm4(mode, n, 0, 0, 1, anorm, ulp, ZERO, 2, A, lda, state);
            }
            break;
        ...
        case 8:
            ...
            {
                f32 scale = (itype == 7) ? sqrtf(slamch("O")) * ulp / (f32)n
                                            : sqrtf(slamch("U")) * (f32)n / ulp;
                slatm4(4, n, 0, 0, 1, scale, ulp, ZERO, 2, A, lda, state);
            }
            break;
...
static void test_hessenberg_reduction(dgehrd_fixture_t* fix, INT itype)
{
    ...
    slacpy(" ", n, n, fix->A, lda, fix->H, lda);
    ...
    /* Generate the orthogonal matrix Q using sorghr */
    sorghr(n, ilo, ihi, fix->Q, lda, fix->tau, fix->work, lwork, &info);
    assert_info_success(info);

    /* Test: | A - Q*H*Q' | / ( |A| n ulp ) and | I - Q*Q' | / ( n ulp ) */
    INT lwork_verify = 2 * n * n;
    shst01(n, ilo, ihi, fix->A, lda, fix->H, lda, fix->Q, lda,
...
static void test_workspace_query(void** state)
{
    ...
    sgehrd(n, 0, n - 1, fix->H, n, fix->tau, &work_query, -1, &info);
    assert_info_success(info);
    assert_true(work_query >= (f32)n);

    /* Also test sorghr workspace query */
    slacpy(" ", n, n, fix->A, n, fix->H, n);
    sgehrd(n, 0, n - 1, fix->H, n, fix->tau, fix->work, n * n, &info);
    assert_info_success(info);

    slacpy(" ", n, n, fix->H, n, fix->Q, n);
    sorghr(n, 0, n - 1, fix->Q, n, fix->tau, &work_query, -1, &info);
    assert_info_success(info);
    assert_true(work_query >= (f32)n);
}
 ...
 * (as would come from sgebal with 'P' or 'B' balancing).
 *
 ...
 * outside [ilo, ihi]. This is the standard use case - sgebal permutes
...
static void test_partial_range(void** state)
{
     ...
     * This simulates what sgebal produces when eigenvalues are isolated at corners.
     *
     * Structure:
    ...
    fix->A[0 + 0 * lda] = rng_uniform_symmetric_f32(fix->rng_state) + 2.0f;
    fix->A[(n-1) + (n-1) * lda] = rng_uniform_symmetric_f32(fix->rng_state) + 2.0f;

    /* Upper part of row 0 (can be non-zero) */
    for (INT j = 1; j < n; j++) {
        fix->A[0 + j * lda] = rng_uniform_symmetric_f32(fix->rng_state);
    }
    ...
    for (INT i = ilo; i <= ihi; i++) {
        fix->A[i + (n - 1) * lda] = rng_uniform_symmetric_f32(fix->rng_state);
    }
    ...
    sgehrd(n, ilo, ihi, fix->H, lda, fix->tau, fix->work, n * n, &info);
    assert_info_success(info);
    ...
    /* Generate Q from Householder vectors using sorghr */
    sorghr(n, ilo, ihi, fix->Q, lda, fix->tau, fix->work, n * n, &info);
    assert_info_success(info);

    /* Verify using shst01: | A - Q*H*Q' | / (|A| n ulp) and | I - Q*Q' | / (n ulp) */
    INT lwork_verify = 2 * n * n;
    shst01(n, ilo, ihi, fix->A, lda, fix->H, lda, fix->Q, lda,
           fix->work, lwork_verify, result);
