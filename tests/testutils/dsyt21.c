/**
 * @file dsyt21.c
 * @brief DSYT21 checks a decomposition of the form A = U S U'
 *        where A is symmetric, U is orthogonal, and S is diagonal or tridiagonal.
 *
 * Port of LAPACK's TESTING/EIG/dsyt21.f to C.
 */
            ...
            f64* const restrict work, f64* restrict result)
{
    ...
    if (itype == 3) {
        ...
        anorm = dlansy("1", &cuplo, n, A, lda, work);
        if (anorm < unfl)
            anorm = unfl;
    }
    ...
    if (itype == 1) {
        ...
        wnorm = dlansy("1", &cuplo, n, work, n, &work[n * n]);
        ...
        if (lower) {
            ...
            for (j = n - 2; j >= 0; j--) {
                ...
                vsave = V[(j + 1) + j * ldv];
                V[(j + 1) + j * ldv] = ONE;
                dlarfy("L", n - j - 1, &V[(j + 1) + j * ldv], 1, tau[j],
            ...
            for (j = 0; j < n - 1; j++) {
                ...
                vsave = V[j + (j + 1) * ldv];
                V[j + (j + 1) * ldv] = ONE;
                dlarfy("U", j + 1, &V[(j + 1) * ldv], 1, tau[j],
                       work, n, &work[n * n]);
                V[j + (j + 1) * ldv] = vsave;
                work[(n + 1) * (j + 1)] = D[j + 1];
            }
        ...
        if (lower) {
            dorm2r("R", "T", n, n - 1, n - 1, &V[1], ldv, tau,
                   &work[n], n, &work[n * n], &iinfo);
        } else {
            dorm2l("R", "T", n, n - 1, n - 1, &V[ldv], ldv, tau,
