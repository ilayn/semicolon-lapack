/**
 * @file dlavsy.c
 * @brief DLAVSY performs one of the matrix-vector operations
 *        x := A*x or x := A'*x, where A is a factor from the
 *        block U*D*U' or L*D*L' factorization computed by DSYTRF.
 */
 ...
 * @param[in]     A      The factored matrix from DSYTRF.
            ...
            while (k < n) {
                if (ipiv[k] >= 0) {
                    ...
                    if (k > 0) {
                        ...
                        cblas_dger(CblasColMajor, k, nrhs, ONE,
            ...
            while (k >= 0) {
                if (ipiv[k] >= 0) {
                    ...
                    if (k < n - 1) {
                        ...
                        /* Apply the transformation. */
                        cblas_dger(CblasColMajor, n - k - 1, nrhs, ONE,
