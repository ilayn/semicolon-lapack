/**
 * @file dsyt01_aa.c
 * @brief DSYT01_AA reconstructs a symmetric indefinite matrix A from its
 *        block L*D*L' or U*D*U' factorization (Aasen's method) and computes the residual.
 *
 * Port of LAPACK TESTING/LIN/dsyt01_aa.f to C.
 */
    ...
    f64* resid)
{
    ...
    dlaset("F", n, n, ZERO, ZERO, C, ldc);

    /* Copy diagonal of AFAC to diagonal of C:
     * Fortran: DLACPY('F', 1, N, AFAC(1,1), LDAFAC+1, C(1,1), LDC+1)
    ...
    if (n > 1) {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            ...
            cblas_dtrmm(CblasColMajor, CblasLeft, CblasUpper, CblasTrans, CblasUnit,
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            ...
            cblas_dtrmm(CblasColMajor, CblasRight, CblasUpper, CblasNoTrans, CblasUnit,
    ...
    for (j = n - 1; j >= 0; j--) {
        i = ipiv[j];
        if (i != j) {
            cblas_dswap(n, &C[j], ldc, &C[i], ldc);
        }
    }
    for (j = n - 1; j >= 0; j--) {
        i = ipiv[j];
        if (i != j) {
            cblas_dswap(n, &C[j * ldc], 1, &C[i * ldc], 1);
        }
    }
