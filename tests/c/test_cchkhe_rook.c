/**
 * @file test_cchkhe_rook.c
 * @brief Comprehensive test suite for Hermitian indefinite ROOK (ZHR) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkhe_rook.f to C using CMocka.
 * Tests CHETRF_ROOK, CHETRI_ROOK, CHETRS_ROOK, and CHECON_ROOK.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkhe_rook.f:
 *   TEST 1: LDL^H factorization residual via chet01_rook
 *   TEST 2: Matrix inverse residual via cpot03
 *   TEST 3: Largest element in U or L (growth factor bound)
 *   TEST 4: Largest 2-Norm of 2-by-2 diagonal blocks (condition bound)
 *   TEST 5: Solution residual via cpot02 (using chetrs_rook)
 *   TEST 6: Solution accuracy via cget04
...
} zchkhe_rook_workspace_t;

static zchkhe_rook_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchkhe_rook_workspace_t));
...
/**
 * Run the full zchkhe_rook test battery for a single (n, uplo, imat, inb) combination.
 *
 * Following LAPACK's zchkhe_rook.f:
 ...
 */
static void run_zchkhe_rook_single(INT n, INT iuplo, INT imat, INT inb)
{
    const f32 ZERO = 0.0f;
    const f32 ONE = 1.0f;
    const f32 ONEHALF = 0.5f;
    const f32 EIGHT = 8.0f;
    const f32 SEVTEN = 17.0f;
    zchkhe_rook_workspace_t* ws = g_workspace;
    ...
    chetrf_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV,
    ...
    chet01_rook(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        chetri_rook(uplo_str, n, ws->AINV, lda, ws->IPIV, ws->WORK, &info);
        if (info == 0) {
            cpot03(uplo_str, n, ws->A, lda, ws->AINV, lda, ws->WORK, lda,
    ...
    {
        f32 dtemp;
        f32 const_val = ((alpha * alpha - ONE) / (alpha * alpha - ONEHALF))
                        / (ONE - alpha);
        ...
        if (iuplo == 0) {
            ...
            while (k >= 1) {
                if (ws->IPIV[k] >= 0) {
                    ...
                    dtemp = clange("M", k, 1, &ws->AFAC[k * lda], lda, ws->RWORK);
                } else {
                    /* 2x2 block: max abs value from columns k-1..k, rows 0..k-2 */
                    dtemp = clange("M", k - 1, 2, &ws->AFAC[(k - 1) * lda], lda, ws->RWORK);
                    k--;
                }
            ...
            while (k < n - 1) {
                if (ws->IPIV[k] >= 0) {
                    ...
                    dtemp = clange("M", n - k - 1, 1, &ws->AFAC[k * lda + k + 1], lda, ws->RWORK);
                } else {
                    /* 2x2 block: max abs value from columns k..k+1, rows k+2..n-1 */
                    dtemp = clange("M", n - k - 2, 2, &ws->AFAC[k * lda + k + 2], lda, ws->RWORK);
                    k++;
                }
                dtemp = dtemp - const_val + THRESH;
                if (dtemp > result[2])
        ...
        if (iuplo == 0) {
            ...
            while (k >= 1) {
                if (ws->IPIV[k] < 0) {
                    ...
                    c64 block[4];
                    f32 sv[2], rwork_svd[5];
                    c64 work_svd[10];
                    INT svd_info;

                    block[0] = ws->AINV[(k - 1) + (k - 1) * lda];
                    block[1] = ws->AINV[(k - 1) + k * lda];
                    block[2] = conjf(block[1]);
                    block[3] = ws->AINV[k + k * lda];

                    cgesvd("N", "N", 2, 2, block, 2, sv,
                           NULL, 1, NULL, 1,
                           work_svd, 6, rwork_svd, &svd_info);
...
test7:
    ...
    set_test_context(ctx);
    anorm = clanhe("1", uplo_str, n, ws->A, lda, ws->RWORK);
    checon_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV, anorm, &rcond,
                ws->WORK, &info);
    assert_int_equal(info, 0);
...
/**
 * CMocka test function - dispatches to run_zchkhe_rook_single based on prestate.
 */
static void test_zchkhe_rook_case(void** state)
{
    zchkhe_rook_params_t* params = *state;
    run_zchkhe_rook_single(params->n, params->iuplo, params->imat, params->inb);
}
...
static zchkhe_rook_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    /* Store parameters */
                    zchkhe_rook_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->inb = inb;
                    snprintf(p->name, sizeof(p->name), "zchkhe_rook_n%d_%c_type%d_nb%d_%d",
                             n, UPLOS[iuplo], imat, nb, inb);

                    /* Create CMocka test entry */
                    g_tests[g_num_tests].name = p->name;
                    g_tests[g_num_tests].test_func = test_zchkhe_rook_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkhe_rook", g_tests, g_num_tests,
