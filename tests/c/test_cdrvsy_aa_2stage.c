/**
 * @file test_cdrvsy_aa_2stage.c
 * @brief ZDRVSY_AA_2STAGE tests the driver routine CSYSV_AA_2STAGE.
 *
...
static int group_setup(void** state)
{
    ...
    g_workspace->TB = calloc(ltb, sizeof(c64));
...
static void run_zdrvsy_aa_2stage_single(INT n, INT imat, INT iuplo)
{
    ...
    csysv_aa_2stage(uplo, n, NRHS, ws->AFAC, lda,
                    ws->TB, ltb,
                    ws->IWORK, ws->IPIV2,
    ...
    if (info != k) {
        fail_msg("CSYSV_AA_2STAGE UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CSYSV_AA_2STAGE UPLO=%s test %d failed: result=%e >= thresh=%e",
