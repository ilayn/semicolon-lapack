 ...
 *   Tests  1-8 : Reduction quality (CHETRD/CUNGTR, CHPTRD/CUPGTR)
...
static void test_zchkst_case(void** state)
{
    ...
    if (iinfo != 0) {
        fprintf(stderr, "CHETRD(L) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[2] = ulpinv;
        goto L280;
    }
    ...
    if (iinfo != 0) {
        fprintf(stderr, "CUNGTR(L) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        ws->result[3] = ulpinv;
        goto L280;
    }

    chet21(2, "L", n, 1, A, lda, SD, SE, U, ldu, V, ldu, TAU, work,
           rwork, ws->result + 2);
    chet21(3, "L", n, 1, A, lda, SD, SE, U, ldu, V, ldu, TAU, work,
           rwork, ws->result + 3);
