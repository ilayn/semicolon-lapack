/**
 * @file test_cchkge.c
 * @brief Comprehensive test suite for complex general matrix (CGE) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkge.f to C using CMocka.
 * Tests CGETRF, CGETRI, CGETRS, CGERFS, and CGECON.
 */
...
static void run_zchkge_single(INT m, INT n, INT imat, INT inb)
{
    ...
    clatb4("CGE", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    if (zerot) {
        ...
        if (imat < 7) {
            ...
            claset("F", m, n - izero + 1, ZERO, ZERO, &ws->A[ioff], lda);
        }
    } else {
        izero = 0;
    }

    clacpy("F", m, n, ws->A, lda, ws->AFAC, lda);

    cgetrf(m, n, ws->AFAC, lda, ws->IPIV, &info);
    ...
    /* TEST 1 */
    clacpy("F", m, n, ws->AFAC, lda, ws->AINV, lda);
    cget01(m, n, ws->A, lda, ws->AINV, lda, ws->IPIV, ws->RWORK, &result[0]);
    assert_residual_below(result[0], THRESH);

    /* TEST 2 */
    if (m == n && info == 0) {
        clacpy("F", n, n, ws->AFAC, lda, ws->AINV, lda);
        INT lwork = NMAX * 3;
        cgetri(n, ws->AINV, lda, ws->IPIV, ws->WORK, lwork, &info);
        assert_int_equal(info, 0);

        cget03(n, ws->A, lda, ws->AINV, lda, ws->WORK, lda,
               ws->RWORK, &rcondo, &result[1]);
        anormo = clange("O", m, n, ws->A, lda, ws->RWORK);

        anormi = clange("I", m, n, ws->A, lda, ws->RWORK);
        f32 ainvnm = clange("I", n, n, ws->AINV, lda, ws->RWORK);
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        for (INT itran = 0; itran < (INT)NTRAN; itran++) {
            ...
            /* TEST 3 */
            clarhs("CGE", "N", " ", trans_arr, n, n, kl, ku, nrhs,
                   ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

            clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
            cgetrs(trans_arr, n, nrhs, ws->AFAC, lda, ws->IPIV, ws->X, lda, &info);
            assert_int_equal(info, 0);

            clacpy("F", n, nrhs, ws->B, lda, ws->WORK, lda);
            cget02(trans_arr, n, n, nrhs, ws->A, lda, ws->X, lda,
...
test8:
    ...
    if (m == n) {
        for (INT itran = 0; itran < 2; itran++) {
            ...
            cgecon(norm_arr, n, ws->AFAC, lda, anorm, &rcond, ws->WORK,
                   ws->RWORK, &info);
            assert_int_equal(info, 0);
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkge", g_tests, g_num_tests,
