/**
 * @file test_cchkq3.c
 * @brief Comprehensive test suite for CGEQP3 (QR with column pivoting).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkq3.f to C using CMocka.
 * Tests CGEQP3 for various matrix types and block sizes.
 *
...
typedef struct {
    ...
    f32* RWORK;   /* Real workspace (2*NMAX for cgeqp3 + extra for cqrt12) */
...
static int group_setup(void** state)
{
    ...
    /* RWORK needs max(2*N for CGEQP3, 2*mn + 4*mn for CQRT12 D/E/sbdsqr) */
...
static void run_zchkq3_single(INT m, INT n, INT imode, INT inb)
{
    ...
    /* Determine MODE for clatms based on IMODE */
    INT mode = imode;
    if (imode > 3) {
        mode = 1;
    }
    ...
    if (imode == 1) {
        /* Zero matrix */
        claset("F", m, n, CMPLXF(0.0f, 0.0f), CMPLXF(0.0f, 0.0f), ws->COPYA, lda);
    ...
    /* Workspace size for CGEQP3 */
    INT lw = (2 * n + nb * (n + 1) > 1) ? (2 * n + nb * (n + 1)) : 1;

    /* Compute QR factorization with pivoting */
    cgeqp3(m, n, ws->A, lda, &ws->IWORK[n], ws->TAU, ws->WORK, lw, ws->RWORK, &info);
    ...
    set_test_context(ctx);
    result[2] = cqrt11(m, mnmin, ws->A, lda, ws->TAU, ws->WORK, lwork);
...
{
    build_test_array();
    (void)_cmocka_run_group_tests("zchkq3", g_tests, g_num_tests,
