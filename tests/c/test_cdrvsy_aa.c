/**
 * @file test_cdrvsy_aa.c
 * @brief ZDRVSY_AA tests the driver routine CSYSV_AA.
 *
...
static void run_zdrvsy_aa_single(INT n, INT imat, INT iuplo)
{
    ...
    csysv_aa(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    if (info != k) {
        fail_msg("CSYSV_AA UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    csyt01_aa(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CSYSV_AA UPLO=%s test %d failed: result=%e >= thresh=%e",
