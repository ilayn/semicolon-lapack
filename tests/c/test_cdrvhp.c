/**
 * @file test_cdrvhp.c
 * @brief ZDRVHP tests the driver routines CHPSV and CHPSVX.
 *
...
static void run_zdrvhp_single(INT n, INT imat, INT iuplo, INT ifact)
{
    ...
    INT kl, ku, mode;
    f32 anorm, cndnum;
    clatb4("CHP", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    if (zerot) {
        ...
        f32 anrm = clanhp("1", uplo, n, ws->A, ws->RWORK);

        memcpy(ws->AFAC, ws->A, npp * sizeof(c64));
        chptrf(uplo, n, ws->AFAC, ws->IWORK, &info);

        memcpy(ws->AINV, ws->AFAC, npp * sizeof(c64));
        chptri(uplo, n, ws->AINV, ws->IWORK, ws->WORK, &info);
        f32 ainvnm = clanhp("1", uplo, n, ws->AINV, ws->RWORK);
    ...
    char xtype = 'N';
    clarhs("CHP", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (ifact == 1) {
        ...
        chpsv(uplo, n, NRHS, ws->AFAC, ws->IWORK, ws->X, lda, &info);
        ...
        if (info != k) {
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("CHPSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    f32 rcond;
    chpsvx(fact, uplo, n, NRHS, ws->A, ws->AFAC, ws->IWORK,
    ...
    /* Check error code from CHPSVX */
    if (info != k) {
        fail_msg("CHPSVX FACT=%s UPLO=%s info=%d expected=%d (n=%d imat=%d)",
    ...
    /* TEST 6: Compare RCOND from CHPSVX with computed value */
    result[5] = sget06(rcond, rcondc);

    /* Check results */
    for (INT i = k1 - 1; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CHPSVX FACT=%s UPLO=%s test %d: result=%e >= thresh=%e",
