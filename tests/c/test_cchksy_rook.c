 ...
 *   TEST 5: Solution residual via csyt02 (using csytrs_rook)
...
static void run_zchksy_rook_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    csytrf_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV,
    ...
    csyt01_rook(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        csytri_rook(uplo_str, n, ws->AINV, lda, ws->IPIV, ws->WORK, &info);
        if (info == 0) {
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 5: Solve and compute residual for A * X = B using csytrs_rook
         */
        ...
        csytrs_rook(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV,
                    ws->X, lda, &info);
        assert_int_equal(info, 0);
...
test7:
    ...
    csycon_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV, anorm, &rcond,
                ws->WORK, &info);
    assert_int_equal(info, 0);
