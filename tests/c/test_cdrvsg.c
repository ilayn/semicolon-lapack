/**
 * @file test_cdrvsg.c
 * @brief ZDRVSG checks the complex Hermitian generalized eigenproblem drivers.
 *
 * Port of LAPACK TESTING/EIG/zdrvsg.f
 */
...
static zdrvsg_workspace_t* g_ws = NULL;
...
static int group_setup(void** state)
{
    ...
    g_ws = malloc(sizeof(zdrvsg_workspace_t));
    if (!g_ws) return -1;
    ...
    g_ws->AB = malloc(n2 * sizeof(c64));
    g_ws->BB = malloc(n2 * sizeof(c64));
    g_ws->AP = malloc(n2 * sizeof(c64));
    g_ws->BP = malloc(n2 * sizeof(c64));
...
static void pack_hermitian(INT n, const char* uplo, const c64* A, INT lda,
                           c64* AP)
{
    INT ij = 0;
...
static void dense_to_band(INT n, INT kd, const char* uplo, const c64* A,
                          INT lda, c64* AB, INT ldab)
{
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        for (INT j = 0; j < n; j++) {
 ...
 * Port of zdrvsg.f lines 544-650.
 */
                      ...
                      INT* ka_out, INT* kb_out)
{
    ...
    if (itype == 1) {
        ...
        kb = 0;
        INT idumma[1] = {1};
        clatmr(n, n, "S", "H", work, 6, 1.0f, CONE, "T", "N",
 ...
 * Port of zdrvsg.f lines 478-1306.
 */
static void run_zdrvsg_single(zdrvsg_params_t* params)
{
    const c64 CZERO = CMPLXF(0.0f, 0.0f);
    ...
    for (INT ibtype = 1; ibtype <= 3; ibtype++) {
        for (INT ibuplo = 0; ibuplo < 2; ibuplo++) {
            ...
            clacpy(" ", n, n, A, lda, Z, lda);
            clacpy(uplo, n, n, B, lda, BB, lda);
            ...
            csgt01(ibtype, uplo, n, n, A, lda, B, lda, Z, lda,
            ...
            chegvx(ibtype, "V", "A", uplo, n, AB, lda, BB, lda,
            ...
            vl = 0.0f;
            vu = anorm;
            chegvx(ibtype, "V", "V", uplo, n, AB, lda, BB, lda,
                   vl, vu, il, iu, abstol, &m, D, Z, lda,
                   work, nwork, rwork, iwork + n, iwork, &iinfo);
            if (iinfo != 0) {
            ...
            csgt01(ibtype, uplo, n, m, A, lda, B, lda, Z, lda,
                   D, work, rwork, &ws->result[ntest - 1]);
            ...
            chegvx(ibtype, "V", "I", uplo, n, AB, lda, BB, lda,
                   vl, vu, il, iu, abstol, &m, D, Z, lda,
                   work, nwork, rwork, iwork + n, iwork, &iinfo);
            if (iinfo != 0) {
...
L100:
            ...
            chpgvx(ibtype, "V", "A", uplo, n, AP, BP,
                   vl, vu, il, iu, abstol, &m, D, Z, lda,
            ...
            vl = 0.0f;
            vu = anorm;
            chpgvx(ibtype, "V", "V", uplo, n, AP, BP,
                   vl, vu, il, iu, abstol, &m, D, Z, lda,
                   work, rwork, iwork + n, iwork, &iinfo);
            if (iinfo != 0) {
            ...
            chpgvx(ibtype, "V", "I", uplo, n, AP, BP,
                   vl, vu, il, iu, abstol, &m, D, Z, lda,
                   work, rwork, iwork + n, iwork, &iinfo);
            if (iinfo != 0) {
...
L310:
            ...
            if (ibtype == 1) {
                ...
                claset("F", lda, n, CZERO, CZERO, AB, lda);
                claset("F", lda, n, CZERO, CZERO, BB, lda);
                ...
                INT ldq = (n > 1) ? n : 1;
                chbgvx("V", "A", uplo, n, ka, kb, AB, lda, BB, lda,
                ...
                vl = 0.0f;
                vu = anorm;
                chbgvx("V", "V", uplo, n, ka, kb, AB, lda, BB, lda,
                       BP, ldq, vl, vu, il, iu, abstol, &m, D, Z, lda,
                       work, rwork, iwork + n, iwork, &iinfo);
                if (iinfo != 0) {
                ...
                chbgvx("V", "I", uplo, n, ka, kb, AB, lda, BB, lda,
                       BP, ldq, vl, vu, il, iu, abstol, &m, D, Z, lda,
                       work, rwork, iwork + n, iwork, &iinfo);
                if (iinfo != 0) {
...
static void test_zdrvsg_case(void** state)
{
    zdrvsg_params_t* params = *state;
    run_zdrvsg_single(params);
}

#define MAX_TESTS (NNVAL * MAXTYP)

static zdrvsg_params_t g_params[MAX_TESTS];
...
static void build_test_array(void)
{
    ...
    for (size_t in = 0; in < NNVAL; in++) {
        ...
        for (INT jtype = 1; jtype <= MAXTYP; jtype++) {
            zdrvsg_params_t* p = &g_params[g_num_tests];
            p->n = n;
            p->jtype = jtype;
            snprintf(p->name, sizeof(p->name),
                     "zdrvsg_n%d_type%d", n, jtype);

            g_tests[g_num_tests].name = p->name;
            g_tests[g_num_tests].test_func = test_zdrvsg_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zdrvsg", g_tests, g_num_tests,
