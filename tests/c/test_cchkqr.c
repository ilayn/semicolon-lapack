/**
 * @file test_cchkqr.c
 * @brief Comprehensive test suite for QR factorization (CQR) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkqr.f to C using CMocka.
 * Tests CGEQRF, CUNGQR, and CUNMQR.
 *
 * Each (m, n, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkqr.f:
 *   TEST 1-2: QR factorization via cqrt01 (norm(R - Q'*A), norm(I - Q'*Q))
 *   TEST 3-6: CUNMQR tests via cqrt03 (applying Q from left/right with trans)
 *   TEST 7: Least squares solve via cgels and cget02
 *   TEST 8-9: CGEQRFP tests (positive diagonal R)
 *
...
#define NTYPES  8
#define NTESTS  9   /* Tests 1-2 (cqrt01), 3-6 (cqrt03), 7 (cgels), 8-9 (cqrt01p) */
#define THRESH  30.0f
...
typedef struct {
    ...
    c64* R;      /* Upper triangular R (NMAX x NMAX) */
    c64* C;      /* Workspace for cqrt03 (NMAX x NMAX) */
    c64* CC;     /* Another workspace for cqrt03 (NMAX x NMAX) */
...
static int group_setup(void** state)
{
    ...
    g_workspace->R = malloc(NMAX * NMAX * sizeof(c64));
...
static void run_zchkqr_single(INT m, INT n, INT imat, INT inb_idx)
{
    ...
    clatb4("CQR", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    for (INT ik = 0; ik < nk; ik++) {
        ...
        if (ik == 0) {
            /*
             * TEST 1-2: Test CGEQRF via cqrt01
             * Computes:
            ...
            cqrt01(m, n, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
            ...
            /*
             * TEST 8-9: Test CGEQRFP via cqrt01p
             * CGEQRFP produces QR factorization with non-negative diagonal R.
             * Test 8: same as tests 1-2 but using CGEQRFP
             * Test 9: check that diagonal of R is non-negative (via cgennd)
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 8 (CGEQRFP factorization)", m, n, imat, k);
            set_test_context(ctx);
            cqrt01p(m, n, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                    ws->WORK, lwork, ws->RWORK, &result[7]);

            assert_residual_below(result[7], THRESH);
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 8b (CGEQRFP orthogonality)", m, n, imat, k);
            set_test_context(ctx);
            assert_residual_below(result[8], THRESH);

            /* Test 9: Check that AF has non-negative diagonal */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 9 (CGEQRFP non-negative diagonal)", m, n, imat, k);
            set_test_context(ctx);
            if (!cgennd(m, n, ws->AF, lda)) {
                result[8] = 2 * THRESH;  /* Fail the test */
            }
            assert_residual_below(result[8], THRESH);
        } else if (m >= n) {
            /*
             * TEST 1-2: Test CUNGQR via cqrt02
             * Uses factorization from cqrt01
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 1-2 (CUNGQR)", m, n, imat, k);
            set_test_context(ctx);
            cqrt02(m, n, k, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                   ws->WORK, lwork, ws->RWORK, result);
        ...
        if (m >= k) {
            /*
             * TEST 3-6: Test CUNMQR via cqrt03
             * Tests Q from left/right with 'N'/'C' conjugate transpose options
             */
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 3-6 (CUNMQR)", m, n, imat, k);
            set_test_context(ctx);
            cqrt03(m, n, k, ws->AF, ws->C, ws->CC, ws->Q, lda, ws->TAU,
            ...
            /*
             * TEST 7: If M >= N and K == N and INB == 1, test CGELS least squares solve
            ...
            if (k == n && inb_idx == 0) {
                ...
                clarhs("CQR", "N", "F", "N", m, n, 0, 0, nrhs,
                       ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

                clacpy("F", m, nrhs, ws->B, lda, ws->X, lda);
                /* Use ws->C instead of ws->AF for CGELS - CGELS modifies its input
                 * and we need to preserve AF for subsequent cqrt02/cqrt03 tests */
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkqr", g_tests, g_num_tests,
