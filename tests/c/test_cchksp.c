/**
 * @file test_cchksp.c
 * @brief Comprehensive test suite for complex symmetric packed matrix (CSP) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchksp.f to C using CMocka.
 * Tests CSPTRF, CSPTRI, CSPTRS, CSPRFS, and CSPCON.
 *
 * Each (n, uplo, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksp.f:
 *   TEST 1: LDL'/UDU' factorization residual via cspt01
 *   TEST 2: Matrix inverse residual via cspt03
 *   TEST 3: Solution residual via cspt02
 *   TEST 4: Solution accuracy via cget04
 *   TEST 5: Refined solution accuracy via cget04 (after csprfs)
...
static void run_zchksp_single(INT n, INT iuplo, INT imat)
{
    ...
    if (imat != NTYPES) {
        ...
        izero = 0;
        clatsp(uplo_str, n, ws->A, rng_state);
    }

    cblas_ccopy(npp, ws->A, 1, ws->AFAC, 1);

    csptrf(uplo_str, n, ws->AFAC, ipiv, &info);
    ...
    cspt01(uplo_str, n, ws->A, ws->AFAC, ipiv, ws->WORK, lda, ws->RWORK, &result[0]);
    ...
    if (!trfcon) {
        ...
        csptri(uplo_str, n, ws->AINV, ipiv, ws->WORK, &info);
        assert_int_equal(info, 0);

        cspt03(uplo_str, n, ws->A, ws->AINV, ws->WORK, lda,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        clarhs("CSP", "N", uplo_str, " ", n, n, kl, ku, nrhs,
               ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

        clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        csptrs(uplo_str, n, nrhs, ws->AFAC, ipiv, ws->X, lda, &info);
        assert_int_equal(info, 0);

        clacpy("F", n, nrhs, ws->B, lda, ws->WORK, lda);
        cspt02(uplo_str, n, nrhs, ws->A, ws->X, lda,
        ...
        csprfs(uplo_str, n, nrhs, ws->A, ws->AFAC, ipiv,
...
test8:
    ...
    set_test_context(ctx);
    anorm = clansp("1", uplo_str, n, ws->A, ws->RWORK);
    cspcon(uplo_str, n, ws->AFAC, ipiv, anorm, &rcond, ws->WORK,
           &info);
    assert_int_equal(info, 0);
...
{
    build_test_array();
    return _cmocka_run_group_tests("zchksp", g_tests, g_num_tests,
