/**
 * @file test_cdrvpp.c
 * @brief ZDRVPP tests the driver routines CPPSV and CPPSVX.
 *
...
static int group_setup(void** state)
{
    ...
    g_workspace->ASAV = calloc(NPP_MAX, sizeof(c64));
...
static void run_zdrvpp_single(INT n, INT imat, INT iuplo, INT ifact, INT iequed)
{
    ...
    cblas_ccopy(npp, ws->A, 1, ws->ASAV, 1);
    ...
    if (zerot) {
        ...
        cblas_ccopy(npp, ws->ASAV, 1, ws->AFAC, 1);

        if (equil || iequed > 0) {
            cppequ(uplo, n, ws->AFAC, ws->S, &scond, &amax, &info);
            if (info == 0 && n > 0) {
                if (iequed > 0) {
                    scond = 0.0f;
                }
                claqhp(uplo, n, ws->AFAC, ws->S, scond, amax, &equed);
            }
        }
        ...
        f32 anrm = clanhp("1", uplo, n, ws->AFAC, ws->RWORK);

        cpptrf(uplo, n, ws->AFAC, &info);

        cblas_ccopy(npp, ws->AFAC, 1, ws->A, 1);
        cpptri(uplo, n, ws->A, &info);

        f32 ainvnm = clanhp("1", uplo, n, ws->A, ws->RWORK);
    ...
    cblas_ccopy(npp, ws->ASAV, 1, ws->A, 1);

    /* Form exact solution and set right hand side */
    rng_seed(rng_state, 1988 + n * 1000 + imat * 100 + iuplo * 10);
    char xtype = 'N';
    clarhs("CPP", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (nofact) {
        ...
        cppsv(uplo, n, NRHS, ws->AFAC, ws->X, lda, &info);

        if (info != izero) {
            fail_msg("CPPSV info=%d expected=%d", info, izero);
            return;
        } else if (info != 0) {
            return;
        }

        cppt01(uplo, n, ws->A, ws->AFAC, ws->RWORK, &result[0]);

        clacpy("Full", n, NRHS, ws->B, lda, ws->WORK, lda);
        cppt02(uplo, n, NRHS, ws->A, ws->X, lda,
        ...
        for (INT k = 0; k < nt; k++) {
            if (result[k] >= THRESH) {
                fail_msg("CPPSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    /* --- Test CPPSVX --- */
    if (!prefac && npp > 0) {
        claset("Full", npp, 1, CZERO, CZERO, ws->AFAC, npp);
    }
    claset("Full", n, NRHS, CZERO, CZERO, ws->X, lda);

    char equed_inout = equed;
    if (iequed > 0 && n > 0) {
        claqhp(uplo, n, ws->A, ws->S, scond, amax, &equed_inout);
    }
    ...
    f32 rcond;
    cppsvx(fact, uplo, n, NRHS, ws->A, ws->AFAC, &equed_inout, ws->S,
           ws->B, lda, ws->X, lda, &rcond,
           ws->RWORK, &ws->RWORK[NRHS], ws->WORK,
           &ws->RWORK[2 * NRHS], &info);

    if (info != izero) {
        fail_msg("CPPSVX info=%d expected=%d", info, izero);
        return;
    }
    ...
    if (info == 0) {
        ...
        cppt02(uplo, n, NRHS, ws->ASAV, ws->X, lda,
        ...
        cppt05(uplo, n, NRHS, ws->ASAV, ws->B, lda,
    ...
    if (info == 0) {
        for (INT k = k1 - 1; k < NTESTS; k++) {
            if (result[k] >= THRESH) {
                fail_msg("CPPSVX FACT=%s UPLO=%s EQUED=%c test %d: result=%e >= thresh=%e",
                         fact, uplo, equed_inout, k + 1, (double)result[k], (double)THRESH);
            }
        }
    } else {
        if (!prefac && result[0] >= THRESH) {
            fail_msg("CPPSVX FACT=%s UPLO=%s EQUED=%c test 1: result=%e >= thresh=%e",
                     fact, uplo, equed_inout, (double)result[0], (double)THRESH);
        }
        if (result[5] >= THRESH) {
            fail_msg("CPPSVX FACT=%s UPLO=%s EQUED=%c test 6: result=%e >= thresh=%e",
