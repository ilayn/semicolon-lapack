/**
 * @file cgbt05.c
 * @brief CGBT05 tests the error bounds from iterative refinement.
 *
 * Port of LAPACK TESTING/LIN/cgbt05.f
 */
        ...
        for (INT i = 0; i < n; i++) {
            ...
            if (notran) {
                ...
                for (INT j = j_start; j < j_end; j++) {
                    tmp += cabs1f(AB[(ku + i - j) + j * ldab]) * cabs1f(X[j + k * ldx]);
                }
            } else {
                INT j_start = (i - ku > 0) ? i - ku : 0;
                INT j_end = (i + kl + 1 < n) ? i + kl + 1 : n;
                for (INT j = j_start; j < j_end; j++) {
                    tmp += cabs1f(AB[(ku + j - i) + i * ldab]) * cabs1f(X[j + k * ldx]);
                }
            }
            if (i == 0) {
                axbi = tmp;
