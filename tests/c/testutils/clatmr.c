/**
 * @file clatmr.c
 * @brief CLATMR generates random matrices of various types for testing.
 *
 * Faithful port of LAPACK TESTING/MATGEN/clatmr.f
 */
    ...
    uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    clatm1(mode, cond, irsign, idist, d, mnmin, info, state);
    if (*info != 0) {
        *info = 1;
        return;
    }
    ...
    if (isym == 0) {
        for (i = 0; i < mnmin; i++) {
            d[i] = CMPLXF(crealf(d[i]), 0.0f);
        }
    }

    if (igrade == 1 || igrade == 3 || igrade == 4 ||
        igrade == 5 || igrade == 6) {
        clatm1(model, condl, 0, idist, dl, m, info, state);
        if (*info != 0) {
            *info = 3;
            return;
        }
    ...
    if (igrade == 2 || igrade == 3) {
        clatm1(moder, condr, 0, idist, dr, n, info, state);
        if (*info != 0) {
            *info = 4;
            return;
        }
    ...
    if (fulbnd) {
        if (ipack == 0) {
            ...
            for (j = 0; j < n; j++) {
                for (i = j - kuu; i <= j; i++) {
                    if (i < 0) {
                        ...
                        mxsub = max_int(isub, jsub);
                        if (mxsub == jsub && isym == 0) {
                            A[(mxsub - mnsub) + mnsub * lda] = conjf(ctemp);
                        } else {
            ...
            for (j = 0; j < n; j++) {
                for (i = j - kuu; i <= j; i++) {
                    ...
                    mxsub = max_int(isub, jsub);
                    if (mxsub == isub && isym == 0) {
                        A[(mnsub - mxsub + kuu) + mxsub * lda] = conjf(ctemp);
                    } else {
                    ...
                    for (i = j - kuu; i <= j; i++) {
                        ...
                        if (i >= 0 && mnsub != mxsub) {
                            if (mnsub == isub && isym == 0) {
                                A[(mxsub - mnsub + kuu) + mnsub * lda] = conjf(ctemp);
                            } else {
        ...
        if (ipack == 0) {
            if (isym == 0) {
                for (j = 0; j < n; j++) {
                    for (i = 0; i <= j; i++) {
                        A[i + j * lda] = clatm2(m, n, i, j, kl, ku, idist,
            ...
            for (j = 0; j < n; j++) {
                for (i = 0; i <= j; i++) {
                    if (isym == 0) {
                        A[j + i * lda] = conjf(clatm2(m, n, i, j, kl, ku,
                                                       idist, d, igrade, dl, dr,
                                                       ipvtng, iwork, sparse, state));
                    } else {
                        A[j + i * lda] = clatm2(m, n, i, j, kl, ku, idist,
                                                 d, igrade, dl, dr, ipvtng,
                                                 iwork, sparse, state);
                    }
                    if (i != j) {
            ...
            for (j = 0; j < n; j++) {
                for (i = 0; i <= j; i++) {
                    ...
                    A[isub + jsub * lda] = clatm2(m, n, i, j, kl, ku,
                                                   idist, d, igrade, dl, dr,
                                                   ipvtng, iwork, sparse, state);
                }
            }
                ...
                for (j = 0; j < n; j++) {
                    for (i = 0; i <= j; i++) {
                        ...
                        A[isub + jsub * lda] = clatm2(m, n, i, j, kl, ku,
                                                       ...
                                                       ipvtng, iwork, sparse, state);
                        if (isym == 0) {
                            A[isub + jsub * lda] = conjf(A[isub + jsub * lda]);
                        }
                    }
                }
            } else {
            ...
            for (j = 0; j < n; j++) {
                for (i = j - kuu; i <= j; i++) {
                    if (i < 0) {
                        ...
                        if (isym == 0) {
                            A[(j - i) + i * lda] = conjf(clatm2(m, n, i, j, kl,
                                                                 ku, idist, d, igrade, dl,
                                                                 dr, ipvtng, iwork, sparse, state));
                        } else {
                            A[(j - i) + i * lda] = clatm2(m, n, i, j, kl, ku,
                                                           idist, d, igrade, dl, dr,
                                                           ipvtng, iwork, sparse, state);
                        }
                    }
            ...
            for (j = 0; j < n; j++) {
                for (i = j - kuu; i <= j; i++) {
                    A[(i - j + kuu) + j * lda] = clatm2(m, n, i, j, kl, ku,
                                                         idist, d, igrade, dl, dr,
                                                         ipvtng, iwork, sparse, state);
                }
            }
            ...
            if (isym != 1) {
                for (j = 0; j < n; j++) {
                    for (i = j - kuu; i <= j; i++) {
                        ...
                        if (i >= 0 && i != j) {
                            if (isym == 0) {
                                A[(j - i + kuu) + i * lda] = conjf(A[(i - j + kuu) + j * lda]);
                            } else {
    ...
    if (anorm >= ZERO) {
        if (anorm > ZERO && onorm == ZERO) {
            *info = 5;
            ...
            if (ipack <= 2) {
                for (j = 0; j < n; j++) {
                    cblas_csscal(m, ONE / onorm, &A[j * lda], 1);
                    cblas_csscal(m, anorm, &A[j * lda], 1);
                }
            } else if (ipack == 3 || ipack == 4) {
                cblas_csscal(n * (n + 1) / 2, ONE / onorm, A, 1);
                cblas_csscal(n * (n + 1) / 2, anorm, A, 1);
            } else if (ipack >= 5) {
                for (j = 0; j < n; j++) {
                    cblas_csscal(kll + kuu + 1, ONE / onorm, &A[j * lda], 1);
                    cblas_csscal(kll + kuu + 1, anorm, &A[j * lda], 1);
                }
            }
        } else {
            if (ipack <= 2) {
                for (j = 0; j < n; j++) {
                    cblas_csscal(m, anorm / onorm, &A[j * lda], 1);
                }
            } else if (ipack == 3 || ipack == 4) {
                cblas_csscal(n * (n + 1) / 2, anorm / onorm, A, 1);
            } else if (ipack >= 5) {
                for (j = 0; j < n; j++) {
                    cblas_csscal(kll + kuu + 1, anorm / onorm, &A[j * lda], 1);
                }
            }
        }
    }
