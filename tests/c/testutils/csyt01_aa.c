/**
 * @file csyt01_aa.c
 * @brief CSYT01_AA reconstructs a complex symmetric indefinite matrix A from its
 *        block L*D*L' or U*D*U' factorization (Aasen's method) and computes the residual.
 *
 * Port of LAPACK TESTING/LIN/csyt01_aa.f to C.
 */
...
/**
 * CSYT01_AA reconstructs a complex symmetric indefinite matrix A from its
 ...
 *                        as computed by CSYTRF_AA.
 *                        Complex*16 array, dimension (ldafac, n).
 * @param[in]     ldafac  The leading dimension of AFAC. ldafac >= max(1, n).
 * @param[in]     ipiv    The pivot indices from CSYTRF_AA. Integer array, dimension (n).
 *                        0-based indexing.
    ...
    f32* resid)
{
    ...
    if (n > 1) {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            cblas_ctrmm(CblasColMajor, CblasLeft, CblasUpper, CblasTrans, CblasUnit,
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            ...
            cblas_ctrmm(CblasColMajor, CblasRight, CblasLower, CblasTrans, CblasUnit,
