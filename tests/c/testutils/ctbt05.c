/**
 * @file ctbt05.c
 * @brief CTBT05 tests the error bounds from iterative refinement for a
 *        triangular band system.
 *
 * Port of LAPACK TESTING/LIN/ctbt05.f to C.
 */
            ...
            f32* reslts)
{
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        xnorm = fmaxf(cabs1f(X[imax + j * ldx]), unfl);
        diff = ZERO;
        for (i = 0; i < n; i++) {
    ...
    for (k = 0; k < nrhs; k++) {
            ...
            if (upper) {
                if (!notran) {
                    ...
                    for (j = jstart; j < i + 1 - ifu; j++) {
                        tmp += cabs1f(AB[kd + j - i + i * ldab]) * cabs1f(X[j + k * ldx]);
                    }
                    if (unit) {
                        tmp += cabs1f(X[i + k * ldx]);
                    }
                } else {
                    ...
                    for (j = i + ifu; j <= jend; j++) {
                        tmp += cabs1f(AB[kd + i - j + j * ldab]) * cabs1f(X[j + k * ldx]);
                    }
                }
            } else {
                if (notran) {
                    ...
                    INT jstart = (i - kd > 0) ? i - kd : 0;
                    for (j = jstart; j < i + 1 - ifu; j++) {
                        tmp += cabs1f(AB[i - j + j * ldab]) * cabs1f(X[j + k * ldx]);
                    }
                    if (unit) {
                        tmp += cabs1f(X[i + k * ldx]);
                    }
                    ...
                    INT jend = (i + kd < n - 1) ? i + kd : n - 1;
                    for (j = i + ifu; j <= jend; j++) {
                        tmp += cabs1f(AB[j - i + i * ldab]) * cabs1f(X[j + k * ldx]);
                    }
                }
            }
            if (i == 0) {
        ...
        tmp = berr[k] / (nz * eps + nz * unfl / fmaxf(axbi, nz * unfl));
        if (k == 0) {
            reslts[1] = tmp;
        } else {
