/**
 * @file ctrt05.c
 * @brief CTRT05 tests the error bounds from iterative refinement for triangular systems.
 *
 * Port of LAPACK TESTING/LIN/ctrt05.f to C.
 */
            ...
            const f32* ferr, const f32* berr, f32* reslts)
{
    ...
    INT i, j, k, ifu, imax;
    f32 eps, unfl, ovfl, tmp, diff, xnorm, errbnd, axbi;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        xnorm = cabs1f(X[j * ldx + imax]);
        if (xnorm < unfl) xnorm = unfl;

        diff = ZERO;
        for (i = 0; i < n; i++) {
            f32 d = cabs1f(X[j * ldx + i] - XACT[j * ldxact + i]);
            if (d > diff) diff = d;
        }
    ...
    for (k = 0; k < nrhs; k++) {
        for (i = 0; i < n; i++) {
            tmp = cabs1f(B[k * ldb + i]);

            if (upper) {
                if (!notran) {
                    for (j = 0; j < i - ifu + 1; j++) {
                        tmp += cabs1f(A[i * lda + j]) * cabs1f(X[k * ldx + j]);
                    }
                    if (unit)
                        tmp += cabs1f(X[k * ldx + i]);
                } else {
                    if (unit)
                        tmp += cabs1f(X[k * ldx + i]);
                    for (j = i + ifu; j < n; j++) {
                        tmp += cabs1f(A[j * lda + i]) * cabs1f(X[k * ldx + j]);
                    }
                }
            } else {
                /* Lower triangular */
