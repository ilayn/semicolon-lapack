/**
 * @file cgerqs.c
 * @brief CGERQS computes a minimum-norm solution min || A*X - B || using the
 *        RQ factorization A = R*Q computed by CGERQF.
 */
 ...
 *                       as returned by CGERQF. Complex*16 array, dimension
 *                       (lda, n).
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    claset("F", n - m, nrhs, CZERO, CZERO, B, ldb);

    cunmrq("L", "C", n, nrhs, m, A, lda, tau, B, ldb, work, lwork, info);
}
