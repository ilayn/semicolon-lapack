/**
 * @file clarot.c
 * @brief CLAROT applies a (Givens) rotation to two adjacent rows or columns.
 *
 * Faithful port of LAPACK TESTING/MATGEN/clarot.f
 */
 ...
 *     to the output of ZROTG or to most versions of CROT, both C
    ...
    c64* xright)
{
    ...
    for (j = 0; j < nl - nt; j++) {
        ...
        A[iy + j * iinc] = -conjf(s) * A[ix + j * iinc] +
                            conjf(c) * A[iy + j * iinc];
        A[ix + j * iinc] = tempx;
    }
    ...
    for (j = 0; j < nt; j++) {
        tempx = c * xt[j] + s * yt[j];
        yt[j] = -conjf(s) * xt[j] + conjf(c) * yt[j];
        xt[j] = tempx;
    }
