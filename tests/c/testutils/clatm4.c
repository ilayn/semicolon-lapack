/**
 * @file clatm4.c
 * @brief CLATM4 generates basic square matrices for eigenvalue testing.
 */
            ...
            uint64_t state[static 4])
{
    ...
    if (itype != 0) {
        ...
        switch (abstype) {
            ...
            case 4:
                ...
                for (jd = kbeg - 1; jd < kend; jd++) {
                    A[jd + jd * lda] = CMPLXF((f32)(jd - nz1 + 1), 0.0f);
                }
                break;
            ...
            case 6:
                ...
                A[(kend - 1) + (kend - 1) * lda] = CMPLXF(rcond, 0.0f);
                break;
            ...
            case 7:
                ...
                if (klen > 1) {
                    ...
                    for (i = 1; i < klen; i++) {
                        A[(nz1 + i) + (nz1 + i) * lda] = CMPLXF(powf(alpha, (f32)i), 0.0f);
                    }
                }
                break;
            ...
            case 8:
                ...
                if (klen > 1) {
                    ...
                    for (i = 1; i < klen; i++) {
                        A[(nz1 + i) + (nz1 + i) * lda] = CMPLXF((f32)(klen - 1 - i) * alpha + rcond, 0.0f);
                    }
                }
                break;
            ...
            case 10:
                ...
                for (jd = kbeg - 1; jd < kend; jd++) {
                    A[jd + jd * lda] = clarnd_rng(idist, state);
                }
                break;
        }
        ...
        /* Scale by AMAGN */
        for (jd = kbeg - 1; jd < kend; jd++) {
            A[jd + jd * lda] = amagn * crealf(A[jd + jd * lda]);
        }
        for (jd = isdb - 1; jd < isde; jd++) {
            A[(jd + 1) + jd * lda] = amagn * crealf(A[(jd + 1) + jd * lda]);
        }

        /* If RSIGN, assign random signs to diagonal and subdiagonal */
        if (rsign) {
            for (jd = kbeg - 1; jd < kend; jd++) {
                if (crealf(A[jd + jd * lda]) != ZERO) {
                    ctemp = clarnd_rng(3, state);
                    ctemp = ctemp / cabsf(ctemp);
                    A[jd + jd * lda] = ctemp * crealf(A[jd + jd * lda]);
                }
            }
            for (jd = isdb - 1; jd < isde; jd++) {
                if (crealf(A[(jd + 1) + jd * lda]) != ZERO) {
                    ctemp = clarnd_rng(3, state);
                    ctemp = ctemp / cabsf(ctemp);
                    A[(jd + 1) + jd * lda] = ctemp * crealf(A[(jd + 1) + jd * lda]);
                }
            }
        }
    ...
    if (triang != ZERO) {
        for (jc = 1; jc < n; jc++) {
            for (jr = 0; jr < jc; jr++) {
                A[jr + jc * lda] = triang * clarnd_rng(idist, state);
            }
        }
    }
}
