/**
 * @file clatme.c
 * @brief CLATME generates random non-symmetric square matrices with
 *        specified eigenvalues for testing LAPACK programs.
 *
 * Faithful port of LAPACK TESTING/MATGEN/clatme.f
 ...
 * specified eigenvalues for testing LAPACK programs.
 *
 * CLATME operates by applying the following sequence of operations:
 *
 ...
 *
 * @param[in] mode
 *     Specifies how eigenvalues are computed (see clatm1).
 *
 * @param[in] cond
            ...
            uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    clatm1(mode, cond, irsign, idist, D, n, &iinfo, state);
    if (iinfo != 0) {
        *info = 1;
        return;
    }
    ...
    if (mode != 0 && mode != 6 && mode != -6) {
        ...
        cblas_cscal(n, &alpha, D, 1);
    }

    claset("F", n, n, CZERO, CZERO, A, lda);
    cblas_ccopy(n, D, 1, A, lda + 1);

    /* 3) If UPPER='T', set upper triangle of A to random numbers. */
    if (iupper != 0) {
        for (jc = 1; jc < n; jc++) {
            clarnv_rng(idist, jc, &A[0 + jc * lda], state);
        }
    }
    ...
    if (isim != 0) {
        ...
        /* Multiply by V and V' */
        clarge(n, A, lda, work, &iinfo, state);
        if (iinfo != 0) {
            *info = 4;
            return;
        }
        ...
        for (j = 0; j < n; j++) {
            cblas_csscal(n, DS[j], &A[j], lda);
            if (DS[j] != ZERO) {
                cblas_csscal(n, ONE / DS[j], &A[j * lda], 1);
            } else {
                *info = 5;
                return;
            }
    ...
    if (kl < n - 1) {
        ...
        for (jcr = kl; jcr < n - 1; jcr++) {
            ...
            cblas_ccopy(irows, &A[jcr + ic * lda], 1, work, 1);
            xnorms = work[0];
            clarfg(irows, &xnorms, &work[1], 1, &tau);
            tau = conjf(tau);
            work[0] = CONE;
            alpha = clarnd_rng(5, state);

            cblas_cgemv(CblasColMajor, CblasConjTrans, irows, icols, &CONE,
                        &A[jcr + (ic + 1) * lda], lda, work, 1, &CZERO,
                        &work[irows], 1);
            c64 neg_tau = -tau;
            cblas_cgerc(CblasColMajor, irows, icols, &neg_tau, work, 1,
                        &work[irows], 1, &A[jcr + (ic + 1) * lda], lda);

            cblas_cgemv(CblasColMajor, CblasNoTrans, n, irows, &CONE,
                        &A[0 + jcr * lda], lda, work, 1, &CZERO,
                        &work[irows], 1);
            c64 neg_conj_tau = -conjf(tau);
            cblas_cgerc(CblasColMajor, n, irows, &neg_conj_tau, &work[irows], 1,
                        work, 1, &A[0 + jcr * lda], lda);

            A[jcr + ic * lda] = xnorms;
            claset("F", irows - 1, 1, CZERO, CZERO, &A[jcr + 1 + ic * lda], lda);

            cblas_cscal(icols + 1, &alpha, &A[jcr + ic * lda], lda);
            c64 conj_alpha = conjf(alpha);
            cblas_cscal(n, &conj_alpha, &A[0 + jcr * lda], 1);
        }
    } else if (ku < n - 1) {
        ...
        for (jcr = ku; jcr < n - 1; jcr++) {
            ...
            cblas_ccopy(icols, &A[ir + jcr * lda], lda, work, 1);
            xnorms = work[0];
            clarfg(icols, &xnorms, &work[1], 1, &tau);
            tau = conjf(tau);
            work[0] = CONE;
            clacgv(icols - 1, &work[1], 1);
            alpha = clarnd_rng(5, state);

            cblas_cgemv(CblasColMajor, CblasNoTrans, irows, icols, &CONE,
                        &A[ir + 1 + jcr * lda], lda, work, 1, &CZERO,
                        &work[icols], 1);
            c64 neg_tau = -tau;
            cblas_cgerc(CblasColMajor, irows, icols, &neg_tau, &work[icols], 1,
                        work, 1, &A[ir + 1 + jcr * lda], lda);

            cblas_cgemv(CblasColMajor, CblasConjTrans, icols, n, &CONE,
                        &A[jcr], lda, work, 1, &CZERO,
                        &work[icols], 1);
            c64 neg_conj_tau = -conjf(tau);
            cblas_cgerc(CblasColMajor, icols, n, &neg_conj_tau, work, 1,
                        &work[icols], 1, &A[jcr], lda);

            A[ir + jcr * lda] = xnorms;
            claset("F", 1, icols - 1, CZERO, CZERO, &A[ir + (jcr + 1) * lda], lda);

            cblas_cscal(irows + 1, &alpha, &A[ir + jcr * lda], 1);
            c64 conj_alpha = conjf(alpha);
            cblas_cscal(n, &conj_alpha, &A[jcr], lda);
        }
    }
    ...
    if (anorm >= ZERO) {
        ...
        if (temp > ZERO) {
            ralpha = anorm / temp;
            for (j = 0; j < n; j++) {
                cblas_csscal(n, ralpha, &A[j * lda], 1);
            }
        }
    }
}
