/**
 * @file cpot05.c
 * @brief CPOT05 tests the error bounds from iterative refinement for
    ...
    for (INT k = 0; k < nrhs; k++) {
        ...
        for (INT i = 0; i < n; i++) {
            ...
            if (upper) {
                ...
                tmp += fabsf(crealf(A[i + i * lda])) * cabs1f(X[i + k * ldx]);
                for (INT j = i + 1; j < n; j++) {
        ...
        f32 np1 = (f32)(n + 1);
        f32 denom = np1 * eps + np1 * unfl / (axbi > np1 * unfl ? axbi : np1 * unfl);
        f32 tmp2 = berr[k] / denom;
        if (k == 0) {
            reslts[1] = tmp2;
        } else {
