/**
 * @file csyt01.c
 * @brief CSYT01 reconstructs a complex symmetric indefinite matrix A from its
 ...
 * @param[in]     AFAC    The factored form from CSYTRF.
 *                         Complex*16 array, dimension (ldafac, n).
 * @param[in]     ldafac  The leading dimension of AFAC. ldafac >= max(1, n).
 * @param[in]     ipiv    Pivot indices from CSYTRF. Integer array, dimension (n).
 *                         0-based indexing.
    ...
    f32* resid)
{
    ...
    clavsy(uplo, "T", "N", n, n, AFAC, ldafac, ipiv, C, ldc, &info);

    /* Call CLAVSY again to multiply by U (or L). */
    clavsy(uplo, "N", "U", n, n, AFAC, ldafac, ipiv, C, ldc, &info);
