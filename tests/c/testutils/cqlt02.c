/**
 * @file cqlt02.c
 * @brief CQLT02 tests CUNGQL for generating a partial Q matrix from QL.
 *
            ...
            f32* restrict result)
{
    ...
    cungql(m, n, k, Q, lda, &tau[minmn - k], work, lwork, &info);

    /* Copy L(m-n:m-1, n-k:n-1) */
    claset("F", n, k, CZERO, CZERO, &L[(m - n) + (n - k) * lda], lda);
    clacpy("L", k, k, &AF[(m - k) + (n - k) * lda], lda,
           &L[(m - k) + (n - k) * lda], lda);
    ...
    f32 anorm = clange("1", m, k, &A[0 + (n - k) * lda], lda, rwork);
    f32 resid = clange("1", n, k, &L[(m - n) + (n - k) * lda], lda, rwork);
    if (anorm > 0.0f) {
