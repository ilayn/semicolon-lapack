/**
 * @file ctpt03.c
 * @brief CTPT03 computes the residual for the solution to a scaled triangular
 *        system of equations when A is in packed format.
 *
 * Port of LAPACK TESTING/LIN/ctpt03.f to C.
 */
            ...
            c64* work, f32* resid)
{
    ...
    if (diag[0] == 'N' || diag[0] == 'n') {
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            jj = 0;
            for (j = 0; j < n; j++) {
                f32 t = tscal * cabsf(AP[jj]) + cnorm[j];
                if (t > tnorm) tnorm = t;
                jj += j + 2;
            }
        } else {
    ...
    *resid = ZERO;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        ix = (INT)cblas_icamax(n, &X[j * ldx], 1);
