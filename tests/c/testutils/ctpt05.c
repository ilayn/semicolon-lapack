/**
 * @file ctpt05.c
 * @brief CTPT05 tests the error bounds from iterative refinement for a
 *        triangular system when A is in packed format.
 *
 * Port of LAPACK TESTING/LIN/ctpt05.f to C.
 */
 ...
 * @param[in]     trans   = 'N': A * X = B; = 'T': A' * X = B; = 'C': A' * X = B.
    ...
    for (k = 0; k < nrhs; k++) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (upper) {
                ...
                if (!notran) {
                    for (j = 0; j < i + 1 - ifu; j++) {
                        tmp += cabs1f(AP[jc + j]) * cabs1f(X[j + k * ldx]);
                    }
                    if (unit) {
                    ...
                    for (j = i + ifu; j < n; j++) {
                        tmp += cabs1f(AP[jc]) * cabs1f(X[j + k * ldx]);
                        jc += j + 1;
                    }
                }
            } else {
                if (notran) {
                    ...
                    for (j = i + ifu; j < n; j++) {
                        tmp += cabs1f(AP[jc + j - i]) * cabs1f(X[j + k * ldx]);
                    }
                }
            }
            if (i == 0) {
