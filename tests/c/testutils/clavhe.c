/**
 * @file clavhe.c
 * @brief CLAVHE performs one of the matrix-vector operations
 *        x := A*x or x := A^H*x, where A is a factor from the
 *        block U*D*U' or L*D*L' factorization computed by CHETRF.
 */
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            ...
            while (k < n) {
                if (ipiv[k] >= 0) {
                    ...
                    if (nounit)
                        cblas_cscal(nrhs, &A[k + k * lda], &B[k], ldb);

                    /* Multiply by P(K) * inv(U(K)) if K > 0. */
                    if (k > 0) {
                        /* Apply the transformation. */
                        cblas_cgeru(CblasColMajor, k, nrhs, &CONE,
            ...
            while (k >= 0) {
                if (ipiv[k] >= 0) {
                    ...
                    if (k < n - 1) {
                        ...
                        /* Apply the transformation. */
                        cblas_cgeru(CblasColMajor, n - k - 1, nrhs, &CONE,
