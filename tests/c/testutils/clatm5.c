/**
 * @file clatm5.c
 * @brief CLATM5 generates matrices involved in the Generalized Sylvester equation.
 *
 * Faithful port of LAPACK TESTING/MATGEN/clatm5.f
 */
            ...
            const f32 alpha, INT qblcka, INT qblckb)
{
    ...
    const c64 CTWO = CMPLXF(2.0f, 0.0f);
    const c64 CHALF = CMPLXF(0.5f, 0.0f);
    const c64 CTWENTY = CMPLXF(20.0f, 0.0f);
    const c64 CNEG_ONE = CMPLXF(-1.0f, 0.0f);
    ...
    if (prtype == 1) {
        ...
        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                R[i + j * ldr] = (CHALF - sinf((f32)((i + 1) / (j + 1)))) * CTWENTY;
                L[i + j * ldl] = R[i + j * ldr];
            }
        }
        ...
        for (i = 0; i < m; i++) {
            for (j = 0; j < m; j++) {
                if (i <= j) {
                    A[i + j * lda] = (CHALF - sinf((f32)(i + 1))) * CTWO;
                    D[i + j * ldd] = (CHALF - sinf((f32)((i + 1) * (j + 1)))) * CTWO;
                } else {
                    A[i + j * lda] = CZERO;
                    D[i + j * ldd] = CZERO;
                }
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                if (i <= j) {
                    B[i + j * ldb] = (CHALF - sinf((f32)((i + 1) + (j + 1)))) * CTWO;
                    E[i + j * lde] = (CHALF - sinf((f32)(j + 1))) * CTWO;
                } else {
                    B[i + j * ldb] = CZERO;
                    E[i + j * lde] = CZERO;
                }
        ...
        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                R[i + j * ldr] = (CHALF - sinf((f32)((i + 1) * (j + 1)))) * CTWENTY;
                L[i + j * ldl] = (CHALF - sinf((f32)((i + 1) + (j + 1)))) * CTWENTY;
            }
        }
        ...
        for (i = 0; i < m; i++) {
            for (j = 0; j < m; j++) {
                A[i + j * lda] = (CHALF - sinf((f32)((i + 1) * (j + 1)))) * CTWENTY;
                D[i + j * ldd] = (CHALF - sinf((f32)((i + 1) + (j + 1)))) * CTWO;
            }
        }
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < n; j++) {
                B[i + j * ldb] = (CHALF - sinf((f32)((i + 1) + (j + 1)))) * CTWENTY;
                E[i + j * lde] = (CHALF - sinf((f32)((i + 1) * (j + 1)))) * CTWO;
            }
        }

        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                R[i + j * ldr] = (CHALF - sinf((f32)((j + 1) / (i + 1)))) * CTWENTY;
                L[i + j * ldl] = (CHALF - sinf((f32)((i + 1) * (j + 1)))) * CTWO;
            }
        }
        ...
        for (i = 0; i < m; i++) {
            for (j = 0; j < n; j++) {
                R[i + j * ldr] = (CHALF - sinf((f32)((i + 1) * (j + 1)))) * alpha / CTWENTY;
                L[i + j * ldl] = (CHALF - sinf((f32)((i + 1) + (j + 1)))) * alpha / CTWENTY;
            }
        }
