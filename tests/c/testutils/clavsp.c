/**
 * @file clavsp.c
 * @brief CLAVSP performs one of the matrix-vector operations
 *        x := A*x or x := A^T*x, where A is a factor from the
 *        symmetric factorization computed by CSPTRF stored in packed format.
 */
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            ...
            while (k < n) {
                if (ipiv[k] >= 0) {
                    ...
                    if (nounit)
                        cblas_cscal(nrhs, &A[kc + k], &B[k], ldb);
