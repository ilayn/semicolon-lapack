/**
 * @file cgbt01.c
 * @brief CGBT01 reconstructs a band matrix from its L*U factorization.
 *
 * Port of LAPACK TESTING/LIN/cgbt01.f
 */
 ...
 * @param[in] AFAC   The factored form of A from CGBTRF. Dimension (ldafac, n).
 * @param[in] ldafac The leading dimension of AFAC. ldafac >= max(1, 2*kl+ku+1).
 * @param[in] ipiv   The pivot indices from CGBTRF. Dimension (min(m,n)).
 * @param[out] work  Complex workspace array, dimension (2*kl+ku+1).
            ...
            f32* resid)
{
    ...
    *resid = ZERO;
    ...
    for (INT j = 0; j < n; j++) {
        ...
        if (i2_excl > i1) {
            f32 col_sum = cblas_scasum(i2_excl - i1, &A[i1 + j * lda], 1);
            if (col_sum > anorm) {
                anorm = col_sum;
            }
        }
    ...
    for (INT j = 0; j < n; j++) {
        ...
        INT ju = (kl + ku < j) ? kl + ku : j;
        INT jl = (kl < m - j - 1) ? kl : m - j - 1;
        INT lenj = ((m < j + 1) ? m : j + 1) - (j + 1) + ju + 1;

        if (lenj > 0) {
            /* Copy U column */
            cblas_ccopy(lenj, &AFAC[(kd - ju) + j * ldafac], 1, work, 1);
            ...
            INT i_start = (m - 2 < j) ? m - 2 : j;
            for (INT i = i_start; i >= j - ju; i--) {
                INT il = (kl < m - i - 1) ? kl : m - i - 1;
                if (il > 0) {
                    INT iw = i - j + ju;
                    c64 t = work[iw];
                    cblas_caxpy(il, &t, &AFAC[(kd + 1) + i * ldafac], 1, &work[iw + 1], 1);
            ...
            INT jua = (ju < ku) ? ju : ku;
            if (jua + jl + 1 > 0) {
                c64 neg_one = CMPLXF(-ONE, 0.0f);
                cblas_caxpy(jua + jl + 1, &neg_one,
                           &A[(ku - jua) + j * lda], 1,
                           &work[ju - jua], 1);
            }

            /* Compute the 1-norm of the column. */
            f32 col_resid = cblas_scasum(ju + jl + 1, work, 1);
            if (col_resid > *resid) {
                *resid = col_resid;
            }
        }
