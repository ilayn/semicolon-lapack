/**
 * @file cqrt17.c
 * @brief CQRT17 computes the ratio norm(R^H*op(A)) / (norm(A) * alpha * max(M,N,NRHS) * EPS).
 *
 * Faithful port of LAPACK TESTING/LIN/cqrt17.f
 *
              ...
              c64* work, const INT lwork)
{
    ...
    norma = clange("O", m, n, A, lda, rwork);
    smlnum = slamch("S") / slamch("P");
    iscl = 0;

    /* compute residual and scale it */
    clacpy("A", nrows, nrhs, B, ldb, C, ldb);
    cblas_cgemm(CblasColMajor,
                tpsd ? CblasConjTrans : CblasNoTrans,
                CblasNoTrans,
                nrows, nrhs, ncols, &CNEGONE, A, lda, X, ldx, &CONE, C, ldb);

    normrs = clange("M", nrows, nrhs, C, ldb, rwork);
    if (normrs > smlnum) {
        iscl = 1;
        clascl("G", 0, 0, normrs, ONE, nrows, nrhs, C, ldb, &info);
    }

    /* compute R^H * op(A) */
    cblas_cgemm(CblasColMajor, CblasConjTrans,
                tpsd ? CblasConjTrans : CblasNoTrans,
                nrhs, ncols, nrows, &CONE, C, ldb, A, lda, &CZERO, work, nrhs);

    /* compute and properly scale error */
    err = clange("O", nrhs, ncols, work, nrhs, rwork);
    if (norma != ZERO) {
        err = err / norma;
    }
    ...
    if (iresid == 1) {
        normb = clange("O", nrows, nrhs, B, ldb, rwork);
        if (normb != ZERO) {
            err = err / normb;
        }
    } else {
