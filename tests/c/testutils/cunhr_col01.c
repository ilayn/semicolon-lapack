/**
 * @file cunhr_col01.c
 * @brief CUNHR_COL01 tests CUNGTSQR and CUNHR_COL using CLATSQR, CGEMQRT.
 */
                 ...
                 const INT nb2, f32* restrict result)
{
    ...
    INT lwork2 = (INT)crealf(workquery);
    ...
    for (i = 0; i < n; i++) {
        if (crealf(DIAG[i]) == -1.0f && cimagf(DIAG[i]) == 0.0f) {
            cblas_cscal(n - i, &CNEGONE, &AF[i + i * m], m);
        }
    }
