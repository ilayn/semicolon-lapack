/**
 * @file csgt01.c
 * @brief CSGT01 checks a decomposition of the form A*Z = B*Z*D, A*B*Z = Z*D,
 *        or B*A*Z = Z*D.
 */
            ...
            const f32* D, c64* work, f32* rwork, f32* result)
{
    ...
    if (itype == 1) {
        ...
        cblas_chemm(CblasColMajor, CblasLeft, cblas_uplo,
                    n, m, &CONE, A, lda, Z, ldz, &CZERO, work, n);
        for (i = 0; i < m; i++) {
            cblas_csscal(n, D[i], &Z[i * ldz], 1);
        }
        cblas_chemm(CblasColMajor, CblasLeft, cblas_uplo,
                    n, m, &CONE, B, ldb, Z, ldz, &CMONE, work, n);

        result[0] = (clange("1", n, m, work, n, rwork) / anorm) /
                    (n * ulp);
        ...
        result[0] = (clange("1", n, m, Z, ldz, rwork) / anorm) /
                    (n * ulp);
