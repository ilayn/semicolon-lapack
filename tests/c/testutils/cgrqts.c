/**
 * @file cgrqts.c
 * @brief CGRQTS tests CGGRQF, which computes the GRQ factorization of an
 ...
 * @param[out]    AF      Copy of A, overwritten by CGGRQF.
 ...
 * @param[out]    BF      Copy of B, overwritten by CGGRQF.
    ...
    f32* result)
{
    ...
    if (m <= n) {
        if (m > 0 && m < n)
            clacpy("F", m, n - m, AF, lda, &Q[n - m], lda);
        if (m > 1)
            clacpy("L", m - 1, m - 1, &AF[1 + (n - m) * lda], lda,
                   &Q[(n - m) + 1 + (n - m) * lda], lda);
    } else {
        if (n > 1)
            clacpy("L", n - 1, n - 1, &AF[(m - n) + 1], lda,
                   &Q[1], lda);
    }
    cungrq(n, n, (m < n ? m : n), Q, lda, taua, work, lwork, &info);

    /* Generate the P-by-P matrix Z */

    claset("F", p, p, CROGUE, CROGUE, Z, ldb);
    if (p > 1)
        clacpy("L", p - 1, n, &BF[1], ldb, &Z[1], ldb);
    cungqr(p, p, (p < n ? p : n), Z, ldb, taub, work, lwork, &info);

    /* Copy R */

    claset("F", m, n, CZERO, CZERO, R, lda);
    if (m <= n) {
        clacpy("U", m, m, &AF[(n - m) * lda], lda, &R[(n - m) * lda], lda);
    } else {
        clacpy("F", m - n, n, AF, lda, R, lda);
        clacpy("U", n, n, &AF[m - n], lda, &R[m - n], lda);
    }

    /* Copy T */

    claset("F", p, n, CZERO, CZERO, T, ldb);
    clacpy("U", p, n, BF, ldb, T, ldb);
