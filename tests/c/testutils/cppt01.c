/**
 * @file cppt01.c
 * @brief CPPT01 reconstructs a Hermitian positive definite packed matrix A from its factorization.
 */
            ...
            f32* resid)
{
    ...
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        ...
        for (k = n; k >= 1; k--) {
            ...
            c64 dotc;
            cblas_cdotc_sub(k, &AFAC[kc], 1, &AFAC[kc], 1, &dotc);
            tr = crealf(dotc);
            AFAC[kc + k - 1] = CMPLXF(tr, 0.0f);

            /* Compute the rest of column K. */
            if (k > 1) {
                cblas_ctpmv(CblasColMajor, CblasUpper, CblasConjTrans, CblasNonUnit,
                            k - 1, AFAC, &AFAC[kc], 1);
                kc = kc - (k - 1);
            }
        }
        ...
        for (k = 0; k < n; k++) {
            ...
            AFAC[kc + k] = AFAC[kc + k] - crealf(A[kc + k]);
            kc = kc + k + 1;
        }
        ...
        for (k = n; k >= 1; k--) {
            ...
            if (k < n) {
                cblas_chpr(CblasColMajor, CblasLower, n - k, 1.0f,
                           &AFAC[kc + 1], 1, &AFAC[kc + n - k + 1]);
            }

            /* Scale column K by the diagonal element. */
            tc = AFAC[kc];
            cblas_cscal(n - k + 1, &tc, &AFAC[kc], 1);
