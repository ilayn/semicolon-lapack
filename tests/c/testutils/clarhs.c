/**
 * @file clarhs.c
 * @brief CLARHS generates right-hand sides for linear system tests.
 *
 * Port of LAPACK TESTING/LIN/clarhs.f
 */
            ...
            INT* info, uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    if (strcmp(c2, "GE") == 0 || strcmp(c2, "QR") == 0 ||
        ...
        cblas_chemm(CblasColMajor, CblasLeft, uploC,
                    n, nrhs, &CONE, A, lda, X, ldx, &CZERO, B, ldb);

    } else if (strcmp(c2, "SY") == 0) {

        cblas_csymm(CblasColMajor, CblasLeft, uploC,
                    n, nrhs, &CONE, A, lda, X, ldx, &CZERO, B, ldb);

    } else if (strcmp(c2, "GB") == 0) {

        for (j = 0; j < nrhs; j++) {
            cblas_cgbmv(CblasColMajor, cblas_trans, m, n, kl, ku,
        ...
        for (j = 0; j < nrhs; j++) {
            cblas_chbmv(CblasColMajor, uploC, n, kl,
                        &CONE, A, lda, &X[j * ldx], 1,
                        &CZERO, &B[j * ldb], 1);
        }
        ...
        for (j = 0; j < nrhs; j++) {
            cblas_chpmv(CblasColMajor, uploC, n,
                        &CONE, A, &X[j * ldx], 1,
                        &CZERO, &B[j * ldb], 1);
        }
        ...
        for (j = 0; j < nrhs; j++) {
            memcpy(&B[j * ldb], &X[j * ldx], n * sizeof(c64));
        }
        CBLAS_DIAG diagC = (ku == 2) ? CblasUnit : CblasNonUnit;
        cblas_ctrmm(CblasColMajor, CblasLeft, uploC, cblas_trans, diagC,
        ...
        CBLAS_DIAG diagC = (ku == 2) ? CblasUnit : CblasNonUnit;
        for (j = 0; j < nrhs; j++) {
            cblas_ctpmv(CblasColMajor, uploC, cblas_trans, diagC,
                        n, A, &B[j * ldb], 1);
        }
        ...
        CBLAS_DIAG diagC = (ku == 2) ? CblasUnit : CblasNonUnit;
        for (j = 0; j < nrhs; j++) {
            cblas_ctbmv(CblasColMajor, uploC, cblas_trans, diagC,
