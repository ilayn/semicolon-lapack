/**
 * @file clagge.c
 * @brief CLAGGE generates a complex general m by n matrix A, by pre- and post-
 *        multiplying a real diagonal matrix D with random unitary matrices.
 *
 * Faithful port of LAPACK TESTING/MATGEN/clagge.f
 */
    ...
    *info = 0;
    ...
    for (i = minmn - 1; i >= 0; i--) {
        if (i < m - 1) {
            ...
            cblas_cgemv(CblasColMajor, CblasConjTrans, len, n - i, &CONE,
                        &A[i + i * lda], lda, work, 1, &CZERO, &work[m], 1);
            cblas_cgerc(CblasColMajor, len, n - i, &neg_tau, work, 1,
        ...
        if (i < n - 1) {
            ...
            c64 neg_tau = CMPLXF(-tau, 0.0f);
            cblas_cgemv(CblasColMajor, CblasNoTrans, m - i, len, &CONE,
                        &A[i + i * lda], lda, work, 1, &CZERO, &work[n], 1);
            cblas_cgerc(CblasColMajor, m - i, len, &neg_tau, &work[n], 1,
    ...
    for (i = 0; i < maxiter; i++) {
        if (kl <= ku) {
            if (i < m - 1 - kl && i < n) {
                INT len = m - kl - i;
                wn = cblas_scnrm2(len, &A[kl + i + i * lda], 1);
                wa = (wn / cabsf(A[kl + i + i * lda])) * A[kl + i + i * lda];
                if (wn == 0.0f) {
                    tau = 0.0f;
                } else {
                    wb = A[kl + i + i * lda] + wa;
                    c64 scale = CONE / wb;
                    cblas_cscal(len - 1, &scale, &A[kl + i + 1 + i * lda], 1);
                    A[kl + i + i * lda] = CONE;
                    tau = crealf(wb / wa);
                }
                ...
                if (n - i - 1 > 0) {
                    c64 neg_tau = CMPLXF(-tau, 0.0f);
                    cblas_cgemv(CblasColMajor, CblasConjTrans, len, n - i - 1,
                                &CONE, &A[kl + i + (i + 1) * lda], lda,
                                &A[kl + i + i * lda], 1, &CZERO, work, 1);
                    cblas_cgerc(CblasColMajor, len, n - i - 1, &neg_tau,
            ...
            if (i < n - 1 - ku && i < m) {
                INT len = n - ku - i;
                wn = cblas_scnrm2(len, &A[i + (ku + i) * lda], lda);
                wa = (wn / cabsf(A[i + (ku + i) * lda])) * A[i + (ku + i) * lda];
                if (wn == 0.0f) {
                    tau = 0.0f;
                } else {
                    wb = A[i + (ku + i) * lda] + wa;
                    c64 scale = CONE / wb;
                    cblas_cscal(len - 1, &scale, &A[i + (ku + i + 1) * lda], lda);
                    A[i + (ku + i) * lda] = CONE;
                    tau = crealf(wb / wa);
                }

                clacgv(len, &A[i + (ku + i) * lda], lda);
                if (m - i - 1 > 0) {
                    c64 neg_tau = CMPLXF(-tau, 0.0f);
                    cblas_cgemv(CblasColMajor, CblasNoTrans, m - i - 1, len,
                                &CONE, &A[i + 1 + (ku + i) * lda], lda,
                                &A[i + (ku + i) * lda], lda, &CZERO, work, 1);
                    cblas_cgerc(CblasColMajor, m - i - 1, len, &neg_tau,
