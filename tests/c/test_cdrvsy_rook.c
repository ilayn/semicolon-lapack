/**
 * @file test_cdrvsy_rook.c
 * @brief ZDRVSY_ROOK tests the driver routine CSYSV_ROOK.
 *
...
static void run_zdrvsy_rook_single(INT n, INT imat, INT iuplo)
{
    ...
    if (zerot) {
        ...
        csytrf_rook(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK,
                    ws->lwork, &info);

        clacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        csytri_rook(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, &info);
    ...
    csysv_rook(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    /* Check error code from CSYSV_ROOK */
    if (info != k) {
        fail_msg("CSYSV_ROOK UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CSYSV_ROOK UPLO=%s test %d failed: result=%e >= thresh=%e",
