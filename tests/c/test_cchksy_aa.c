 ...
 * Tests CSYTRF_AA and CSYTRS_AA.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksy_aa.f:
 *   TEST 1: LDL^T factorization residual via csyt01_aa
 *   TEST 2: Solution residual via csyt02 (using csytrs_aa)
 *
...
typedef struct {
    ...
    c64* AINV;   /* Work array for factorization (NMAX x NMAX) */
...
static int group_setup(void** state)
{
     ...
     * Also need workspace for csytrs_aa: MAX(1, 3*N-2) = 148
...
static void run_zchksy_aa_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    csytrf_aa(uplo_str, n, ws->AFAC, lda, ws->IWORK, ws->AINV,
              lwork_trf, &info);
    ...
    /* Check error code from CSYTRF_AA */
    if (info != k_expected) {
        snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d CSYTRF_AA info=%d expected=%d",
    ...
    snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d nb=%d TEST 1 (factorization)",
             n, uplo, imat, nb);
    set_test_context(ctx);
    csyt01_aa(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 2: Solve and compute residual for A * X = B using csytrs_aa
         */
        ...
        csytrs_aa(uplo_str, n, nrhs, ws->AFAC, lda, ws->IWORK,
                  ws->X, lda, ws->WORK, lwork_trs, &info);

        if (info != 0) {
            if (izero < 0) {
                snprintf(ctx, sizeof(ctx),
                         "n=%d uplo=%c imat=%d nrhs=%d CSYTRS_AA info=%d",
