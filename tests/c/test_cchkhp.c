...
static void run_zchkhp_single(INT n, INT iuplo, INT imat)
{
    ...
    chptrf(uplo_str, n, ws->AFAC, ipiv, &info);
    ...
    chpt01(uplo_str, n, ws->A, ws->AFAC, ipiv, ws->C, lda, ws->RWORK, &result[0]);
    ...
    if (!trfcon) {
        ...
        chptri(uplo_str, n, ws->AINV, ipiv, ws->WORK, &info);
        assert_int_equal(info, 0);
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        clarhs("CHP", "N", uplo_str, " ", n, n, kl, ku, nrhs,
               ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

        clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        chptrs(uplo_str, n, nrhs, ws->AFAC, ipiv, ws->X, lda, &info);
        assert_int_equal(info, 0);
        ...
        chprfs(uplo_str, n, nrhs, ws->A, ws->AFAC, ipiv,
...
test8:
    ...
    chpcon(uplo_str, n, ws->AFAC, ipiv, anorm, &rcond, ws->WORK,
           &info);
    assert_int_equal(info, 0);
