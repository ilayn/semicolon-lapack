/**
 * @file test_cdrvhe_aa_2stage.c
 * @brief ZDRVHE_AA_2STAGE tests the driver routine CHESV_AA_2STAGE.
 *
...
static void run_zdrvhe_aa_2stage_single(INT n, INT imat, INT iuplo)
{
    ...
    chesv_aa_2stage(uplo, n, NRHS, ws->AFAC, lda,
                    ws->TB, ltb,
                    ws->IWORK, ws->IPIV2,
    ...
    /* Check error code from CHESV_AA_2STAGE */
    if (info != k) {
        fail_msg("CHESV_AA_2STAGE UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CHESV_AA_2STAGE UPLO=%s test %d failed: result=%e >= thresh=%e",
