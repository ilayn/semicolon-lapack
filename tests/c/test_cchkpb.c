/**
 * @file test_cchkpb.c
 * @brief Comprehensive test suite for Hermitian positive definite banded matrix (CPB) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkpb.f to C using CMocka.
 * Tests CPBTRF, CPBTRS, CPBRFS, and CPBCON.
 *
 * Test structure from zchkpb.f:
 *   TEST 1: Cholesky factorization residual via cpbt01
 *   TEST 2: Solution residual via cpbt02
 *   TEST 3: Solution accuracy via cget04
 *   TESTS 4-6: Iterative refinement via cpbrfs + cget04 + cpbt05
 ...
 */
typedef struct {
    c64* A;      /* Original band matrix ((NMAX+1) * NMAX) */
    c64* AFAC;   /* Factored band matrix ((NMAX+1) * NMAX) */
    c64* AINV;   /* Full inverse matrix (NMAX * NMAX) */
...
static int group_setup(void** state)
{
    ...
    g_workspace->A = malloc(max_ldab * NMAX * sizeof(c64));
    g_workspace->AFAC = malloc(max_ldab * NMAX * sizeof(c64));
    ...
    /* WORK needs NMAX * (3*NMAX + NSMAX + 30) for clatms full matrix generation */
    INT work_size = NMAX * (3 * NMAX + NSMAX + 30);
    g_workspace->WORK = malloc(work_size * sizeof(c64));
...
static void run_zchkpb_single(INT n, INT kd, INT iuplo, INT imat, INT inb)
{
    ...
    clatb4("CPB", imat, n, n, &type, &kl, &ku, &anorm_param, &mode, &cndnum, &dist);
    ...
    if (!zerot) {
        ...
        clatms(n, n, &dist, &type, ws->D, mode, cndnum, anorm_param,
    ...
    clacpy("F", kd + 1, n, ws->A, ldab, ws->AFAC, ldab);

    /* Compute the Cholesky factorization */
    cpbtrf(uplo_str, n, kd, ws->AFAC, ldab, &info);
    ...
    clacpy("F", kd + 1, n, ws->AFAC, ldab, ws->AINV, ldab);
    cpbt01(uplo_str, n, kd, ws->A, ldab, ws->AINV, ldab, ws->RWORK, &result[0]);
    ...
    claset("F", n, n, czero, cone, ws->AINV, lda);
    cpbtrs(uplo_str, n, kd, n, ws->AFAC, ldab, ws->AINV, lda, &info);

    anorm = clanhb("1", uplo_str, n, kd, ws->A, ldab, ws->RWORK);
    ainvnm = clange("1", n, n, ws->AINV, lda, ws->RWORK);
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        set_test_context(ctx);
        clarhs("CPB", "N", uplo_str, " ", n, n, kd, kd, nrhs,
               ws->A, ldab, ws->XACT, lda, ws->B, lda, &info, rng_state);

        clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        cpbtrs(uplo_str, n, kd, nrhs, ws->AFAC, ldab, ws->X, lda, &info);
        assert_int_equal(info, 0);

        clacpy("F", n, nrhs, ws->B, lda, ws->WORK, lda);
        cpbt02(uplo_str, n, kd, nrhs, ws->A, ldab, ws->X, lda,
    ...
    set_test_context(ctx);
    anorm = clanhb("1", uplo_str, n, kd, ws->A, ldab, ws->RWORK);
    cpbcon(uplo_str, n, kd, ws->AFAC, ldab, anorm, &rcond,
           ws->WORK, ws->RWORK, &info);
    assert_int_equal(info, 0);
