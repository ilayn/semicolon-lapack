/**
 * @file test_cchktz.c
 * @brief Comprehensive test suite for CTZRZF (trapezoidal RZ factorization).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchktz.f to C using CMocka.
 * Tests CTZRZF for various matrix types.
 *
 * Test structure from zchktz.f:
 *   TEST 1: norm(svd(R) - svd(A)) / (||svd(A)|| * eps * max(M,N)) via cqrt12
 *   TEST 2: norm(A - R*Q) / (||A|| * eps * max(M,N)) via crzt01
 *   TEST 3: norm(Q'*Q - I) / (eps * max(M,N)) via crzt02
 *
 ...
 */
typedef struct {
    c64* A;      /* Working matrix (NMAX x NMAX) */
    c64* COPYA;  /* Copy of original matrix (NMAX x NMAX) */
    f32* S;       /* Singular values (NMAX) */
    c64* TAU;    /* Scalar factors of elementary reflectors (NMAX) */
    c64* WORK;   /* General workspace */
    f32* RWORK;   /* Real workspace for cqrt12 */
} zchktz_workspace_t;
...
static void run_zchktz_single(INT m, INT n, INT imode)
{
    ...
    if (mode == 0) {
        /* Zero matrix */
        claset("F", m, n, CMPLXF(ZERO, ZERO), CMPLXF(ZERO, ZERO), ws->A, lda);
        ...
        cgeqr2(m, n, ws->A, lda, ws->WORK, &ws->WORK[mnmin], &info);

        /* Zero out below the diagonal */
        if (m > 1) {
            claset("L", m - 1, n, CMPLXF(ZERO, ZERO), CMPLXF(ZERO, ZERO), &ws->A[1], lda);
        }
    ...
    /* Save A */
    clacpy("A", m, n, ws->A, lda, ws->COPYA, lda);

    /* Call CTZRZF to reduce the upper trapezoidal matrix to upper triangular form */
    ctzrzf(m, n, ws->A, lda, ws->TAU, ws->WORK, lwork, &info);

    /* TEST 1: norm(svd(R) - svd(A)) / (||svd(A)|| * eps * max(M,N))
     * R is the m-by-m upper triangular factor from CTZRZF (stored in A(0:m-1, 0:m-1)) */
    snprintf(ctx, sizeof(ctx), "m=%d n=%d type=%d TEST 1 (SVD comparison)", m, n, imode);
    set_test_context(ctx);
    result[0] = cqrt12(m, m, ws->A, lda, ws->S, ws->WORK, lwork, ws->RWORK);
    assert_residual_below(result[0], THRESH);

    /* TEST 2: norm(A - R*Q) / (||A|| * eps * max(M,N)) */
    snprintf(ctx, sizeof(ctx), "m=%d n=%d type=%d TEST 2 (factorization)", m, n, imode);
    set_test_context(ctx);
    result[1] = crzt01(m, n, ws->COPYA, ws->A, lda, ws->TAU, ws->WORK, lwork);
    assert_residual_below(result[1], THRESH);

    /* TEST 3: norm(Q'*Q - I) / (eps * max(M,N)) */
    snprintf(ctx, sizeof(ctx), "m=%d n=%d type=%d TEST 3 (orthogonality)", m, n, imode);
    set_test_context(ctx);
    result[2] = crzt02(m, n, ws->A, lda, ws->TAU, ws->WORK, lwork);
...
static void build_test_array(void)
{
    ...
    for (INT im = 0; im < (INT)NM; im++) {
        ...
        for (INT in = 0; in < (INT)NN; in++) {
            ...
            /* CTZRZF only applies when M <= N (underdetermined systems) */
            if (m > n) {
                continue;
            }
...
{
    build_test_array();
    return _cmocka_run_group_tests("zchktz", g_tests, g_num_tests,
