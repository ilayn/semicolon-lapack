/**
 * @file test_cdrvhe_aa.c
 * @brief ZDRVHE_AA tests the driver routine CHESV_AA.
 *
...
static void run_zdrvhe_aa_single(INT n, INT imat, INT iuplo)
{
    ...
    chesv_aa(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    if (info != k) {
        fail_msg("CHESV_AA UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    chet01_aa(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CHESV_AA UPLO=%s test %d failed: result=%e >= thresh=%e",
