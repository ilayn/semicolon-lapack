/**
 * @file test_cchkqp3rk.c
 * @brief Comprehensive test suite for CGEQP3RK (QR with column pivoting, rank-revealing).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkqp3rk.f to C using CMocka.
 * Tests CGEQP3RK for various matrix types and block sizes.
 *
 * Test structure from zchkqp3rk.f:
 *   TEST 1: norm(svd(R) - svd(A)) / (||svd(A)|| * eps * max(M,N)) via cqrt12
 *           (only when KFACT == MINMN)
 *   TEST 2: norm(A*P - Q*R) / (||A|| * eps * max(M,N)) via cqpt01
 *   TEST 3: norm(Q'*Q - I) / (eps * M) via cqrt11
 *   TEST 4: Verify R diagonal is non-increasing in absolute value
 *   TEST 5: norm(Q**H * B - Q**H * B) / (M * eps)
 *
 ...
 */
typedef struct {
    c64* A;       /* Working matrix (NMAX * (NMAX + NSMAX)) - holds A and appended B */
    c64* COPYA;   /* Copy of original A (NMAX * NMAX) */
    c64* B;       /* RHS matrix (NMAX * NSMAX) */
    c64* COPYB;   /* Copy of original B (NMAX * NSMAX) */
    f32* S;        /* Singular values (NMAX) */
    c64* TAU;     /* Scalar factors of elementary reflectors (NMAX) */
    c64* WORK;    /* General workspace */
    f32* RWORK;    /* Real workspace (2*NMAX for cgeqp3rk + extra for cqrt12) */
    INT* IWORK;    /* Integer workspace for pivot indicators + pivot array + internal */
    INT lwork;
} zchkqp3rk_workspace_t;

static zchkqp3rk_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchkqp3rk_workspace_t));
    if (!g_workspace) return -1;

    /* Workspace size from zchkqp3rk.f:
    ...
    g_workspace->A = malloc(NMAX * (NMAX + NSMAX) * sizeof(c64));
    g_workspace->COPYA = malloc(NMAX * NMAX * sizeof(c64));
    g_workspace->B = malloc(NMAX * NSMAX * sizeof(c64));
    g_workspace->COPYB = malloc(NMAX * NSMAX * sizeof(c64));
    g_workspace->S = malloc(NMAX * sizeof(f32));
    g_workspace->TAU = malloc(NMAX * sizeof(c64));
    g_workspace->WORK = malloc(lwork * sizeof(c64));
    /* RWORK needs max(2*N for CGEQP3RK, 2*mn + 4*mn for CQRT12 D/E/sbdsqr) */
    g_workspace->RWORK = malloc(6 * NMAX * sizeof(f32));
 ...
 *
 * Matrix types 1-19 from zchkqp3rk.f:
 *   1: Zero matrix
 ...
 */
static INT generate_matrix(INT m, INT n, INT imat, c64* COPYA, INT lda,
                           f32* S, c64* WORK, uint64_t rng_state[static 4])
{
    const f32 ZERO = 0.0f;
    INT minmn = (m < n) ? m : n;
    INT info;
    ...
    if (imat == 1) {
        ...
        claset("F", m, n, CMPLXF(0.0f, 0.0f), CMPLXF(0.0f, 0.0f), COPYA, lda);
    ...
    if ((imat >= 2 && imat <= 4) || (imat >= 14 && imat <= 19)) {
        /* Matrices 2-4, 14-19: Generate via clatb4 + clatms */
        clatb4("CQK", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    if (minmn >= 2 && imat >= 5 && imat <= 13) {
        ...
        claset("F", m, nb_zero, CMPLXF(0.0f, 0.0f), CMPLXF(0.0f, 0.0f), COPYA, lda);

        /* Generate an M-by-NB_GEN matrix in COPYA starting at column NB_ZERO */
        clatb4("CQK", imat, m, nb_gen, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);

        char dist_str[2] = {dist, '\0'};
        char type_str[2] = {type, '\0'};
        INT ind_offset_gen = nb_zero * lda;
        clatms(m, nb_gen, dist_str,
               type_str, S, mode, cndnum, anorm,
        ...
        if (imat == 6 || imat == 7 || imat == 8 || imat == 10 || imat == 11) {
            ...
            for (INT j = 0; j < jb_zero - 1; j++) {
                cblas_cswap(m, &COPYA[(nb_zero + j) * lda], 1,
                            &COPYA[j * lda], 1);
            }
            ...
            for (INT j = 0; j < nb_gen; j++) {
                ...
                cblas_cswap(m, &COPYA[ind_out], 1, &COPYA[ind_in], 1);
            }
        }
...
/**
 * Run the full zchkqp3rk test battery for a single configuration.
 */
static void run_zchkqp3rk_single(INT m, INT n, INT nrhs, INT imat, INT inb, INT kmax)
{
    const f32 ZERO = 0.0f;
    zchkqp3rk_workspace_t* ws = g_workspace;
    ...
    /* Generate RHS matrix B (COPYB) with clatb4/clatms for IMAT=14 parameters */
    {
        char type, dist;
        INT kl, ku, mode;
        f32 anorm, cndnum;
        clatb4("CQK", 14, m, nrhs, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
        char dist_str[2] = {dist, '\0'};
        char type_str[2] = {type, '\0'};
        clatms(m, nrhs, dist_str,
     ...
     */
    clacpy("A", m, n, ws->COPYA, lda, ws->A, lda);
    clacpy("A", m, nrhs, ws->COPYB, lda, &ws->A[lda * n], lda);
    clacpy("A", m, nrhs, ws->COPYB, lda, ws->B, lda);
    for (INT i = 0; i < n; i++) {
        ws->IWORK[n + i] = ws->IWORK[i];
    }

    /* Workspace size for CGEQP3RK */
    ...
    /* Call CGEQP3RK */
    INT kfact;
    f32 maxc2nrmk, relmaxc2nrmk;
    cgeqp3rk(m, n, nrhs, kmax, abstol, reltol, ws->A, lda,
             &kfact, &maxc2nrmk, &relmaxc2nrmk, &ws->IWORK[n], ws->TAU,
             ws->WORK, lw, ws->RWORK, &ws->IWORK[2 * n], &info);

    if (info < 0) {
        snprintf(ctx, sizeof(ctx), "m=%d n=%d nrhs=%d kmax=%d type=%d nb=%d: CGEQP3RK failed (info=%d)",
    ...
    if (kfact == minmn) {
        result[0] = cqrt12(m, n, ws->A, lda, ws->S, ws->WORK, ws->lwork, ws->RWORK);
    ...
    /* TEST 2: norm(A*P - Q*R) / (||A|| * eps * max(M,N)) via cqpt01 */
    result[1] = cqpt01(m, n, kfact, ws->COPYA, ws->A, lda, ws->TAU,
    ...
    /* TEST 3: norm(Q'*Q - I) / (eps * M) via cqrt11 */
    result[2] = cqrt11(m, kfact, ws->A, lda, ws->TAU, ws->WORK, ws->lwork);
    ...
    if (ktest4 >= 2) {
        result[3] = ZERO;
        f32 r11 = cabsf(ws->A[0]);  /* |R(0,0)| */
        if (r11 > ZERO) {
            for (INT j = 0; j < kfact - 1; j++) {
                /* R(j,j) is at A[j + j*lda], R(j+1,j+1) is at A[(j+1) + (j+1)*lda] */
                f32 rjj = cabsf(ws->A[j + j * lda]);
                f32 rjj1 = cabsf(ws->A[(j + 1) + (j + 1) * lda]);
                f32 dtemp = (rjj - rjj1) / r11;
                if (dtemp < ZERO) {
                    result[3] = BIGNUM;
                    break;
                }
    ...
    /* TEST 5: Verify Q**H applied to RHS correctly
     *         Only when MINMN > 0 */
    if (minmn > 0) {
        /* B already contains COPYB; A[lda*n:] contains Q**H * COPYB from cgeqp3rk */
        /* Apply Q**H to B using cunmqr */
        INT lwork_mqr = (nrhs > 1) ? nrhs : 1;
        cunmqr("L", "C", m, nrhs, kfact, ws->A, lda, ws->TAU,
               ws->B, lda, ws->WORK, lwork_mqr, &info);

        if (info == 0) {
            /* Compute B := B - A[lda*n:] (where A[lda*n:] is Q**H * COPYB from cgeqp3rk) */
            const c64 NEGONE = CMPLXF(-1.0f, 0.0f);
            for (INT i = 0; i < nrhs; i++) {
                cblas_caxpy(m, &NEGONE, &ws->A[lda * n + i * lda], 1,
                            &ws->B[i * lda], 1);
            }

            /* result[4] = ||B|| / (M * eps) */
            f32 rdummy[1];
            f32 bnorm = clange("1", m, nrhs, ws->B, lda, rdummy);
            result[4] = fabsf(bnorm / ((f32)m * eps));

            snprintf(ctx, sizeof(ctx), "m=%d n=%d nrhs=%d kmax=%d type=%d nb=%d TEST 5 (Q**H * B)",
                     m, n, nrhs, kmax, imat, nb);
            set_test_context(ctx);
...
/**
 * CMocka test function - dispatches to run_zchkqp3rk_single based on prestate.
 */
static void test_zchkqp3rk_case(void** state)
{
    zchkqp3rk_params_t* params = *state;
    run_zchkqp3rk_single(params->m, params->n, params->nrhs, params->imat,
...
static zchkqp3rk_params_t g_params[MAX_TESTS];
                ...
                for (INT imat = 1; imat <= NTYPES; imat++) {
                    ...
                    for (INT inb = 0; inb < (INT)NNB; inb++) {
                        ...
                        for (INT ik = 0; ik < num_kmax; ik++) {
                            ...
                            zchkqp3rk_params_t* p = &g_params[g_num_tests];
                            p->m = m;
                            p->n = n;
                            p->nrhs = nrhs;
                            p->imat = imat;
                            ...
                            snprintf(p->name, sizeof(p->name),
                                     "zchkqp3rk_m%d_n%d_nrhs%d_type%d_nb%d_kmax%d",
                                     m, n, nrhs, imat, nb, kmax);

                            g_tests[g_num_tests].name = p->name;
                            g_tests[g_num_tests].test_func = test_zchkqp3rk_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zchkqp3rk", g_tests, g_num_tests,
