/**
 * @file test_cchkrq.c
 * @brief Comprehensive test suite for RQ factorization (CRQ) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkrq.f to C using CMocka.
 * Tests CGERQF, CUNGRQ, and CUNMRQ.
 *
 * Each (m, n, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkrq.f:
 *   TEST 1-2: RQ factorization via crqt01 (norm(R - A*Q'), norm(I - Q*Q'))
 *   TEST 3-6: CUNMRQ tests via crqt03 (applying Q from left/right with trans)
 *   TEST 7: Least squares solve via cgerqs and cget02
 *
...
static void run_zchkrq_single(INT m, INT n, INT imat, INT inb_idx)
{
    ...
    clatb4("CRQ", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    for (INT ik = 0; ik < nk; ik++) {
        ...
        if (ik == 0) {
            ...
            crqt01(m, n, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                   ws->WORK, lwork, ws->RWORK, result);

            assert_residual_below(result[0], THRESH);
            assert_residual_below(result[1], THRESH);
        } else if (m <= n) {
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 1-2 (CUNGRQ)", m, n, imat, k);
            set_test_context(ctx);
            crqt02(m, n, k, ws->A, ws->AF, ws->Q, ws->R, lda, ws->TAU,
                   ws->WORK, lwork, ws->RWORK, result);
        ...
        if (m >= k) {
            snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d TEST 3-6 (CUNMRQ)", m, n, imat, k);
            set_test_context(ctx);
            crqt03(m, n, k, ws->AF, ws->C, ws->CC, ws->Q, lda, ws->TAU,
            ...
            if (k == m && inb_idx == 0) {
                ...
                snprintf(ctx, sizeof(ctx), "m=%d n=%d imat=%d k=%d nrhs=%d TEST 7 (CGERQS)", m, n, imat, k, nrhs);
                set_test_context(ctx);

                clarhs("CRQ", "N", "F", "N", m, n, 0, 0, nrhs,
                       ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

                /* Fortran: CLACPY('Full', M, NRHS, B, LDA, X(N-M+1), LDA)
                 * X(N-M+1) is 1-based offset into flat array = 0-based offset n-m */
                clacpy("F", m, nrhs, ws->B, lda, ws->X + (n - m), lda);

                cgerqs(m, n, nrhs, ws->AF, lda, ws->TAU, ws->X,
...
{
    build_test_array();
    (void)_cmocka_run_group_tests("zchkrq", g_tests, g_num_tests,
