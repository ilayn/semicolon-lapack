 ...
 *
 * Tests the nonsymmetric eigenvalue (Schur form) problem driver CGEES.
 *
...
/**
 * Selection function for cgees.
 * Selects eigenvalues based on global selection criteria.
 */
static INT zslect(const c64* w)
{
    INT j;
    f32 eps = slamch("P");
    ...
    for (j = 0; j < g_seldim; j++) {
        f32 rmin = cabsf(*w - CMPLXF(g_selwr[j], g_selwi[j]));
        if (rmin < eps) {
            return g_selval[j];
        }
    }
...
static void run_zdrves_single(zdrves_params_t* params)
{
    ...
    INT sdim;
    cgees("V", sort, zslect, n, H, lda, &sdim, W, VS, ldvs,
          work, nnwork, rwork, bwork, &iinfo);

    if (iinfo != 0) {
        ws->result[rsub] = ulpinv;
        if (iinfo > 0 && iinfo <= n) {
            fprintf(stderr, "CGEES failed with info=%d (QR failed)\n", iinfo);
        }
        return;
    }
    ...
    f32 res[2];
    chst01(n, 0, n - 1, A, lda, H, lda, VS, ldvs, work, lwork_hst, rwork, res);
    ...
    cgees("N", sort, zslect, n, HT, lda, &sdim, WT, VS, ldvs,
          work, nnwork, rwork, bwork, &iinfo);
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zdrves", g_tests, g_num_tests,
