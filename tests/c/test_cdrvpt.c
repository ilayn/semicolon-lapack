/**
 * @file test_cdrvpt.c
 * @brief ZDRVPT tests the driver routines CPTSV and CPTSVX.
 *
...
typedef struct {
    f32* D;      /* Diagonal (N), real */
    f32* DF;     /* Factored diagonal (N), real */
    c64* E;     /* Off-diagonal (N-1), complex */
    c64* EF;    /* Factored off-diagonal (N-1), complex */
    c64* B;
    c64* X;
    c64* XACT;
    c64* WORK;
    f32* RWORK;
    c64* A;     /* Band storage for clatms (2 x N), complex */
} zdrvpt_workspace_t;
...
static int group_setup(void** state)
{
    ...
    g_workspace->E = calloc(2 * nmax, sizeof(c64));
    ...
    g_workspace->A = calloc(2 * nmax, sizeof(c64));
...
static void run_zdrvpt_single(INT n, INT imat, INT ifact)
{
    ...
    memset(ws->E, 0, 2 * NMAX * sizeof(c64));
    memset(ws->A, 0, 2 * NMAX * sizeof(c64));
    ...
    if (imat <= 6) {
        ...
        clatms(n, n, disttype, symtype, ws->RWORK, mode, cndnum,
               anorm, kl, ku, "B", ws->A, 2, ws->WORK, &info, rng_state);

        if (info != 0) {
            fail_msg("CLATMS info=%d for imat=%d", info, imat);
            return;
        }
        ...
        if (imat == 8) {
            izero = 1;
            D[0] = 0.0f;
            if (n > 1) E[0] = CMPLXF(0.0f, 0.0f);
        } else if (imat == 9) {
            izero = n;
            if (n > 1) E[n - 2] = CMPLXF(0.0f, 0.0f);
            D[n - 1] = 0.0f;
        } else if (imat == 10) {
            izero = (n + 1) / 2;
            if (izero > 1) {
                E[izero - 2] = CMPLXF(0.0f, 0.0f);
            }
            if (izero < n) {
                E[izero - 1] = CMPLXF(0.0f, 0.0f);
            }
            D[izero - 1] = 0.0f;
        } else {
            izero = 0;
    ...
    for (INT j = 0; j < NRHS; j++) {
        clarnv_rng(2, n, &ws->XACT[j * lda], rng_state);
    }

    /* Set the right hand side using CLAPTM: B = A * XACT */
    claptm("Lower", n, NRHS, 1.0f, D, E, ws->XACT, lda, 0.0f, ws->B, lda);
    ...
    if (zerot) {
        ...
        for (INT i = 0; i < n; i++) {
            ...
            cpttrs("Lower", n, 1, DF, EF, ws->X, lda, &info);
    ...
    if (ifact == 1) {
        ...
        cptsv(n, NRHS, DF, EF, ws->X, lda, &info);

        if (zerot) {
            if (info <= 0) {
                fail_msg("CPTSV: expected INFO > 0 for singular matrix, got %d", info);
                return;
            }
        } else if (info != izero) {
            fail_msg("CPTSV info=%d expected=%d", info, izero);
            return;
        }
        ...
        if (izero == 0 && info == 0) {
            ...
            cptt01(n, D, E, DF, EF, ws->WORK, &result[0]);

            /* TEST 2: Check residual of computed solution */
            clacpy("Full", n, NRHS, ws->B, lda, ws->WORK, lda);
            cptt02("Lower", n, NRHS, D, E, ws->X, lda, ws->WORK, lda, &result[1]);
        ...
        for (INT k = 0; k < nt; k++) {
            if (result[k] >= THRESH) {
                fail_msg("CPTSV test %d failed: result=%e >= thresh=%e",
    ...
    if (ifact == 0) {
         ...
         * Copy D/E to DF/EF and factor with CPTTRF. */
        cblas_scopy(n, D, 1, DF, 1);
        if (n > 1) cblas_ccopy(n - 1, E, 1, EF, 1);
        cpttrf(n, DF, EF, &info);
    } else {
        ...
        for (INT i = 0; i < n - 1; i++) EF[i] = CMPLXF(0.0f, 0.0f);
    }
    claset("Full", n, NRHS, CMPLXF(0.0f, 0.0f), CMPLXF(0.0f, 0.0f), ws->X, lda);

    f32 rcond;
    cptsvx(fact, n, NRHS, D, E, DF, EF, ws->B, lda, ws->X, lda, &rcond,
           ws->RWORK, &ws->RWORK[NRHS], ws->WORK, &ws->RWORK[2 * NRHS], &info);

    if (zerot) {
        if (info <= 0) {
            fail_msg("CPTSVX: expected INFO > 0 for singular matrix, got %d", info);
            return;
        }
    } else if (info != izero) {
        fail_msg("CPTSVX info=%d expected=%d", info, izero);
        return;
    }
    ...
    if (izero == 0) {
        ...
        cptt05(n, NRHS, D, E, ws->B, lda, ws->X, lda, ws->XACT, lda,
               ws->RWORK, &ws->RWORK[NRHS], &result[3]);
    } else {
        k1 = 6;
    }

    /* TEST 6: Compare RCOND from CPTSVX with computed value */
    result[5] = sget06(rcond, rcondc);

    /* Check results */
    for (INT k = k1 - 1; k < 5; k++) {
        if (result[k] >= THRESH) {
            fail_msg("CPTSVX FACT=%s test %d: result=%e >= thresh=%e",
                     fact, k + 1, (double)result[k], (double)THRESH);
        }
    }
    if (result[5] >= THRESH) {
        fail_msg("CPTSVX FACT=%s test 6: result=%e >= thresh=%e",
...
{
    build_test_array();
    (void)_cmocka_run_group_tests("zdrvpt", g_tests, g_num_tests,
