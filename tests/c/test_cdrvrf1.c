/**
 * @file test_cdrvrf1.c
 * @brief Test suite for CLANHF (RFP Hermitian matrix norm).
 *
 * Port of LAPACK's TESTING/LIN/zdrvrf1.f to C using CMocka.
 * Tests CLANHF by comparing it against CLANHE on the same matrix:
 *   result = |CLANHE(A) - CLANHF(ARF)| / (CLANHE(A) * eps)
 *
...
static void run_zdrvrf1_single(INT n, INT iit, INT iuplo, INT iform)
{
    ...
    f32 eps = slamch("P");
    f32 small_val = slamch("S");
    f32 large_val = 1.0f / small_val;
    ...
    if (info != 0) {
        snprintf(ctx, sizeof(ctx), "n=%d iit=%d uplo=%s form=%s CTRTTF info=%d",
    ...
    for (INT inorm = 0; inorm < 4; inorm++) {
        f32 normarf = clanhf(norms[inorm], cform, uplo, n, ARF, WORK);
        f32 norma = clanhe(norms[inorm], uplo, n, A, lda, WORK);
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("zdrvrf1", g_tests, g_num_tests, NULL, NULL);
    return 0;
}
