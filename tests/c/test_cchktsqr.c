/**
 * @file test_cchktsqr.c
 * @brief Comprehensive test suite for tall-skinny QR and short-wide LQ routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchktsqr.f to C using CMocka.
 * Tests CGEQR, CGEMQR (via tall-skinny QR) and CGELQ, CGEMLQ (via short-wide LQ).
 *
 * Each (mode, m, n, mb, nb) combination is registered as a separate CMocka test.
 * The actual test logic is in ctsqr01(), which generates random matrices, factors
 * them, and checks 6 residuals.
 *
 * Test structure from zchktsqr.f:
...
static zchktsqr_params_t g_params[MAX_TESTS];
static struct CMUnitTest g_tests[MAX_TESTS];
static INT g_num_tests = 0;

/**
 * CMocka test function - calls ctsqr01 for the given parameter combination.
 */
static void test_zchktsqr_case(void** state)
{
    zchktsqr_params_t* p = *state;
    f32 result[NTESTS];
    ...
    ctsqr01(tssw, p->m, p->n, mb, nb, result);
            ...
            for (INT jn = 0; jn < (INT)NN; jn++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    for (INT imb = 0; imb < (INT)NNB; imb++) {
                        ...
                        zchktsqr_params_t* p = &g_params[g_num_tests];
                        p->mode = mode;
                        p->m = m;
                        p->n = n;
                        p->imb = inb;
                        ...
                        g_tests[g_num_tests].test_func = test_zchktsqr_case;
