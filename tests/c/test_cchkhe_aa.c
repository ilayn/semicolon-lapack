 ...
 * Tests CHETRF_AA and CHETRS_AA.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkhe_aa.f:
 *   TEST 1: LDL^H factorization residual via chet01_aa
 *   TEST 2: Solution residual via cpot02 (using chetrs_aa)
 *
...
static int group_setup(void** state)
{
     ...
     * Also need workspace for chetrs_aa: MAX(1, 3*N-2) = 148
...
static void run_zchkhe_aa_single(INT n, INT iuplo, INT imat, INT inb)
{
     ...
     * Use MATPATH = "CHE" (Hermitian types) */
    ...
    chetrf_aa(uplo_str, n, ws->AFAC, lda, ws->IWORK, ws->AINV,
              lwork_trf, &info);
    ...
    /* Check error code from CHETRF_AA */
    if (info != k_expected) {
        snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d CHETRF_AA info=%d expected=%d",
    ...
    snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d nb=%d TEST 1 (factorization)",
             n, uplo, imat, nb);
    set_test_context(ctx);
    chet01_aa(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 2: Solve and compute residual for A * X = B using chetrs_aa
         */
        ...
        chetrs_aa(uplo_str, n, nrhs, ws->AFAC, lda, ws->IWORK,
                  ws->X, lda, ws->WORK, lwork_trs, &info);

        if (info != 0) {
            if (izero < 0) {
                snprintf(ctx, sizeof(ctx),
                         "n=%d uplo=%c imat=%d nrhs=%d CHETRS_AA info=%d",
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkhe_aa", g_tests, g_num_tests,
