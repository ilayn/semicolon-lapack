 ...
 *
 * Tests the nonsymmetric generalized eigenvalue expert driver CGGEVX.
 *
...
static int group_setup(void** state)
{
    ...
    g_ws->lscale = malloc(NMAX * sizeof(f32));
    g_ws->rscale = malloc(NMAX * sizeof(f32));
    g_ws->s      = malloc(NMAX * sizeof(f32));
    g_ws->dtru   = malloc(NMAX * sizeof(f32));
    g_ws->dif    = malloc(NMAX * sizeof(f32));
    g_ws->diftru = malloc(NMAX * sizeof(f32));
    ...
    for (INT iwb = 0; iwb < 5; iwb++) {
        for (INT iwx = 0; iwx < 5; iwx++) {
            for (INT iwy = 0; iwy < 5; iwy++) {
                ...
                clatm6(iptype, n, ws->A, lda, ws->B,
                       ws->VR, lda, ws->VL, lda,
                       WEIGHT[iwa], WEIGHT[iwb],
                       WEIGHT[iwx], WEIGHT[iwy],
                       ws->dtru, ws->diftru);

                clacpy("Full", n, n, ws->A, lda, ws->AI, lda);
                clacpy("Full", n, n, ws->B, lda, ws->BI, lda);

                INT ilo = 0, ihi = 0;
                f32 anorm = 0.0f, bnorm = 0.0f;
                INT linfo = 0;
                ...
                clacpy("Full", n, n, ws->AI, lda, ws->work, n);
                clacpy("Full", n, n, ws->BI, lda,
                       ws->work + n * n, n);
                f32 abnrm = clange("Fro", n, 2 * n,
                                    ws->work, n, ws->rwork);
                ...
                for (INT i = 0; i < n; i++) {
                    if (ws->s[i] == 0.0f) {
                        if (ws->dtru[i] > abnrm * ulp)
                            result[2] = ulpinv;
                    } else if (ws->dtru[i] == 0.0f) {
                        if (ws->s[i] > abnrm * ulp)
                            result[2] = ulpinv;
                    } else {
                        f32 ratio = fmaxf(fabsf(ws->dtru[i] / ws->s[i]),
                                         fabsf(ws->s[i] / ws->dtru[i]));
                        if (ratio > result[2])
                            result[2] = ratio;
                    }
                }
                ...
                if (ws->dif[0] == 0.0f) {
                    if (ws->diftru[0] > abnrm * ulp)
                        result[3] = ulpinv;
                } else if (ws->diftru[0] == 0.0f) {
                    if (ws->dif[0] > abnrm * ulp)
                        result[3] = ulpinv;
                } else if (ws->dif[n - 1] == 0.0f) {
                    if (ws->diftru[n - 1] > abnrm * ulp)
                        result[3] = ulpinv;
                } else if (ws->diftru[n - 1] == 0.0f) {
                    if (ws->dif[n - 1] > abnrm * ulp)
                        result[3] = ulpinv;
                } else {
                    f32 ratio1 = fmaxf(fabsf(ws->diftru[0] / ws->dif[0]),
                                      fabsf(ws->dif[0] / ws->diftru[0]));
                    f32 ratio2 = fmaxf(
                        fabsf(ws->diftru[n - 1] / ws->dif[n - 1]),
                        fabsf(ws->dif[n - 1] / ws->diftru[n - 1]));
...
static void test_precomputed(void** state)
{
    ...
    clacpy("Full", n, n, ws->AI, lda, ws->A, lda);
    clacpy("Full", n, n, ws->BI, lda, ws->B, lda);
