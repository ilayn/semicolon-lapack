/**
 * @file test_cdrvrf4.c
 * @brief Test suite for CHFRK (RFP Hermitian rank-k update).
 *
 * Port of LAPACK's TESTING/LIN/zdrvrf4.f to C using CMocka.
 * Tests CHFRK by comparing it against cblas_zherk on the same input:
                                ...
                                INT itrans, INT ialpha)
{
    ...
    f32 norma = clange("I", rows_a, cols_a, A, lda, D_WORK_ZLANGE);

    for (INT j = 0; j < n; j++)
        for (INT i = 0; i < n; i++) {
            C1[i + j * ldc] = clarnd_rng(4, rng_state);
            C2[i + j * ldc] = C1[i + j * ldc];
        }

    f32 normc = clange("I", n, n, C1, ldc, D_WORK_ZLANGE);
    ...
    cblas_cherk(CblasColMajor, uploC, transC, n, k,
    ...
    f32 result = clange("I", n, n, C1, ldc, D_WORK_ZLANGE);

    f32 denom = fabsf(alpha) * norma * norma + fabsf(beta) * normc;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zdrvrf4", g_tests, g_num_tests, NULL, NULL);
}
