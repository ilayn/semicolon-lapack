 ...
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchklqt.f to C using CMocka.
 * Tests CGELQT and ZUNMLQT.
 *
 * Test structure from zchklqt.f:
...
static void run_zchklqt_single(INT m, INT n, INT nb)
{
    f32 result[NTESTS];
    char ctx[128];
    INT minmn = (m < n) ? m : n;
...
static void test_zchklqt_case(void** state)
{
    zchklqt_params_t* params = *state;
    run_zchklqt_single(params->m, params->n, params->nb);
}

#define MAX_TESTS (NM * NN * NNB)

static zchklqt_params_t g_params[MAX_TESTS];
    ...
    for (INT im = 0; im < (INT)NM; im++) {
        ...
        for (INT in = 0; in < (INT)NN; in++) {
            ...
            for (INT inb = 0; inb < (INT)NNB; inb++) {
                ...
                zchklqt_params_t* p = &g_params[g_num_tests];
                p->m = m;
                p->n = n;
                p->nb = nb;
                snprintf(p->name, sizeof(p->name), "zchklqt_m%d_n%d_nb%d", m, n, nb);

                g_tests[g_num_tests].name = p->name;
                g_tests[g_num_tests].test_func = test_zchklqt_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchklqt", g_tests, g_num_tests, NULL, NULL);
}
