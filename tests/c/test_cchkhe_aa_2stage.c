 ...
 * Tests CHETRF_AA_2STAGE and CHETRS_AA_2STAGE.
 *
 ...
 *   TEST 2: Solution residual via cpot02 (using chetrs_aa_2stage)
 *
...
static void run_zchkhe_aa_2stage_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    chetrf_aa_2stage(uplo_str, n, ws->AFAC, lda,
                     ws->AINV, ltb,
                     ws->IWORK, ws->IWORK + n,
                     ws->WORK, lwork, &info);
    ...
    /* Check error code from CHETRF_AA_2STAGE */
    if (info != k_expected) {
        snprintf(ctx, sizeof(ctx),
                 "n=%d uplo=%c imat=%d CHETRF_AA_2STAGE info=%d expected=%d",
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 2: Solve and compute residual for A * X = B using chetrs_aa_2stage
         */
        snprintf(ctx, sizeof(ctx),
        ...
        chetrs_aa_2stage(uplo_str, n, nrhs, ws->AFAC, lda,
                         ws->AINV, (3 * nb + 1) * n,
                         ws->IWORK, ws->IWORK + n,
                         ws->X, lda, &info);
        ...
        if (info != 0) {
            if (izero < 0) {
                snprintf(ctx, sizeof(ctx),
                         "n=%d uplo=%c imat=%d nrhs=%d CHETRS_AA_2STAGE info=%d",
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkhe_aa_2stage", g_tests, g_num_tests,
