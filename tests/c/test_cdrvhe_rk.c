/**
 * @file test_cdrvhe_rk.c
 * @brief ZDRVHE_RK tests the driver routine CHESV_RK.
 *
...
static void run_zdrvhe_rk_single(INT n, INT imat, INT iuplo, INT ifact)
{
    ...
    if (zerot) {
        ...
        chetrf_rk(uplo, n, ws->AFAC, lda, ws->E, ws->IWORK, ws->WORK,
                  ws->lwork, &info);

        clacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        INT lwork_tri = (n + 1 + 1) * (1 + 3);
        chetri_3(uplo, n, ws->AINV, lda, ws->E, ws->IWORK, ws->WORK,
                 lwork_tri, &info);
    ...
    if (ifact == 1) {
        ...
        chesv_rk(uplo, n, NRHS, ws->AFAC, lda, ws->E, ws->IWORK,
                 ws->X, lda, ws->WORK, ws->lwork, &info);

        if (zerot) {
            if (info <= 0) {
                fail_msg("CHESV_RK: expected INFO > 0 for singular matrix, got %d", info);
                return;
            }
        } else if (info != 0) {
            fail_msg("CHESV_RK info=%d expected=0", info);
            return;
        } else {
            /* TEST 1: Reconstruct matrix from factors */
            chet01_3(uplo, n, ws->A, lda, ws->AFAC, lda, ws->E,
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("CHESV_RK UPLO=%s test %d failed: result=%e >= thresh=%e",
