/**
 * @file test_cchkhe.c
 * @brief Comprehensive test suite for Hermitian indefinite matrix (CHE) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkhe.f to C using CMocka.
 * Tests CHETRF, CHETRI2, CHETRS, CHETRS2, CHERFS, and CHECON.
 *
 * Each (n, uplo, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkhe.f:
 *   TEST 1: LDL^H factorization residual via chet01
 *   TEST 2: Matrix inverse residual via cpot03
 *   TEST 3: Solution residual via cpot02 (using chetrs)
 *   TEST 4: Solution residual via cpot02 (using chetrs2)
 *   TEST 5: Solution accuracy via cget04
 *   TEST 6: Refined solution accuracy via cget04 (after cherfs)
...
static int group_setup(void** state)
{
    ...
    INT lwork = NMAX * 64;  /* Generous workspace for chetrf */
...
static void run_zchkhe_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    chetrf(uplo_str, n, ws->AFAC, lda, ws->IPIV, ws->WORK, lwork, &info);
    ...
    if (inb == 0 && !trfcon) {
        ...
        chetri2(uplo_str, n, ws->AINV, lda, ws->IPIV, ws->WORK, lwork_tri2, &info);
        if (info == 0) {
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 3: Solve and compute residual for A * X = B using chetrs
         */
        ...
        chetrs(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV, ws->X, lda, &info);
        assert_int_equal(info, 0);
        ...
        /*
         * TEST 4: Solve using chetrs2 (blocked Hermitian solve)
         */
        snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d nrhs=%d TEST 4 (solve2)", n, uplo, imat, nrhs);
        set_test_context(ctx);
        clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        chetrs2(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV, ws->X, lda, ws->WORK, &info);
        assert_int_equal(info, 0);
        ...
        cherfs(uplo_str, n, nrhs, ws->A, lda, ws->AFAC, lda, ws->IPIV,
...
test9:
    ...
    checon(uplo_str, n, ws->AFAC, lda, ws->IPIV, anorm, &rcond,
           ws->WORK, &info);
    assert_int_equal(info, 0);
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkhe", g_tests, g_num_tests,
