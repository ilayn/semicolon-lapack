/**
 * @file test_cchkpo.c
 * @brief Comprehensive test suite for Hermitian positive definite matrix (CPO) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkpo.f to C using CMocka.
 * Tests CPOTRF, CPOTRI, CPOTRS, CPORFS, and CPOCON.
 *
 * Each (n, uplo, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkpo.f:
 *   TEST 1: Cholesky factorization residual via cpot01
 *   TEST 2: Matrix inverse residual via cpot03
 *   TEST 3: Solution residual via cpot02
 *   TEST 4: Solution accuracy via cget04
 *   TEST 5: Refined solution accuracy via cget04 (after cporfs)
 *   TEST 6-7: Error bounds via cpot05
...
static zchkpo_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchkpo_workspace_t));
...
/**
 * Run the full zchkpo test battery for a single (n, uplo, imat, inb) combination.
 *
 * Following LAPACK's zchkpo.f:
 *   - TESTs 1-2 (factorization, inverse) run for all NB values
 *   - TESTs 3-8 (solve, refinement) only run for inb=0 (first NB)
 */
static void run_zchkpo_single(INT n, INT iuplo, INT imat, INT inb)
{
    const c64 CZERO = CMPLXF(0.0f, 0.0f);
    zchkpo_workspace_t* ws = g_workspace;
    ...
    /* Generate Hermitian positive definite test matrix */
    ...
    cpotrf(uplo_str, n, ws->AFAC, lda, &info);
    ...
    cpot01(uplo_str, n, ws->A, lda, ws->AINV, lda, ws->RWORK, &result[0]);
    ...
    set_test_context(ctx);
    clacpy(uplo_str, n, n, ws->AFAC, lda, ws->AINV, lda);
    cpotri(uplo_str, n, ws->AINV, lda, &info);
    assert_int_equal(info, 0);
     ...
     * This matches LAPACK's zchkpo.f line 413: IF( INB.NE.1 ) GO TO 90
     */
    if (inb > 0) {
        goto test8;
    }
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        set_test_context(ctx);
        clarhs("CPO", "N", uplo_str, " ", n, n, kl, ku, nrhs,
               ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

        clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        cpotrs(uplo_str, n, nrhs, ws->AFAC, lda, ws->X, lda, &info);
        assert_int_equal(info, 0);
        ...
        cporfs(uplo_str, n, nrhs, ws->A, lda, ws->AFAC, lda,
...
test8:
    ...
    cpocon(uplo_str, n, ws->AFAC, lda, anorm, &rcond, ws->WORK,
           ws->RWORK, &info);
    assert_int_equal(info, 0);
...
/**
 * CMocka test function - dispatches to run_zchkpo_single based on prestate.
 */
static void test_zchkpo_case(void** state)
{
    zchkpo_params_t* params = *state;
    run_zchkpo_single(params->n, params->iuplo, params->imat, params->inb);
}
...
static zchkpo_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    /* Store parameters */
                    zchkpo_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->inb = inb;
                    snprintf(p->name, sizeof(p->name), "zchkpo_n%d_%c_type%d_nb%d_%d",
                             n, UPLOS[iuplo], imat, nb, inb);

                    /* Create CMocka test entry */
                    g_tests[g_num_tests].name = p->name;
                    g_tests[g_num_tests].test_func = test_zchkpo_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkpo", g_tests, g_num_tests,
