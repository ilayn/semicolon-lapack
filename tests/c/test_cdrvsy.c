/**
 * @file test_cdrvsy.c
 * @brief ZDRVSY tests the driver routines CSYSV and CSYSVX.
 *
...
static void run_zdrvsy_single(INT n, INT imat, INT iuplo, INT ifact)
{
    ...
    if (zerot) {
        ...
        csytrf(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK, ws->lwork, &info);

        clacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        INT lwork_tri = (n + 1 + 1) * (1 + 3);
        csytri2(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, lwork_tri, &info);
    ...
    if (ifact == 1) {
        ...
        csysv(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK, ws->X, lda,
              ws->WORK, ws->lwork, &info);

        if (zerot) {
            if (info <= 0) {
                fail_msg("CSYSV: expected INFO > 0 for singular matrix, got %d", info);
                return;
            }
        } else if (info != 0) {
            fail_msg("CSYSV info=%d expected=0", info);
            return;
        } else {
            /* TEST 1: Reconstruct matrix from factors */
            csyt01(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("CSYSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    f32 rcond;
    csysvx(fact, uplo, n, NRHS, ws->A, lda, ws->AFAC, lda,
    ...
    if (zerot) {
        if (info <= 0) {
            fail_msg("CSYSVX: expected INFO > 0 for singular matrix, got %d", info);
            return;
        }
    } else if (info != 0) {
        fail_msg("CSYSVX info=%d expected=0", info);
        return;
    }
    ...
    /* TEST 6: Compare RCOND from CSYSVX with computed value */
    result[5] = sget06(rcond, rcondc);

    /* Check results */
    for (INT i = k1 - 1; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fail_msg("CSYSVX FACT=%s UPLO=%s test %d: result=%e >= thresh=%e",
