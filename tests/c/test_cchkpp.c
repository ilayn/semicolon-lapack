/**
 * @file test_cchkpp.c
 * @brief Comprehensive test suite for Hermitian positive definite packed matrix (CPP) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkpp.f to C using CMocka.
 * Tests CPPTRF, CPPTRI, CPPTRS, CPPRFS, and CPPCON.
 *
 * Each (n, uplo, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkpp.f:
 *   TEST 1: Cholesky factorization residual via cppt01
 *   TEST 2: Matrix inverse residual via cppt03
 *   TEST 3: Solution residual via cppt02
 *   TEST 4: Solution accuracy via cget04
 *   TEST 5: Refined solution accuracy via cget04 (after cpprfs)
 *   TEST 6-7: Error bounds via cppt05
 ...
 */
typedef struct {
    c64* A;      /* Original packed matrix (NMAX*(NMAX+1)/2) */
    c64* AFAC;   /* Factored packed matrix (NMAX*(NMAX+1)/2) */
    c64* AINV;   /* Inverse packed matrix (NMAX*(NMAX+1)/2) */
...
static zchkpp_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchkpp_workspace_t));
    if (!g_workspace) return -1;

    /* Note: A needs full NMAX*NMAX storage for clatms even though final result
     * is packed. clatms generates in full format then packs. AFAC and AINV
    ...
    g_workspace->AFAC = malloc(npp * sizeof(c64));
    g_workspace->AINV = malloc(npp * sizeof(c64));
...
/**
 * Run the full zchkpp test battery for a single (n, uplo, imat) combination.
 * This is the core test logic, parameterized by the test case.
 */
static void run_zchkpp_single(INT n, INT iuplo, INT imat)
{
    const f32 ZERO = 0.0f;
    zchkpp_workspace_t* ws = g_workspace;
    ...
    clatb4("CPP", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);

    /* Generate Hermitian positive definite test matrix in packed form */
    ...
    cblas_ccopy(npp, ws->A, 1, ws->AFAC, 1);

    /* Compute the Cholesky factorization */
    cpptrf(uplo_str, n, ws->AFAC, &info);
    ...
    cblas_ccopy(npp, ws->AFAC, 1, ws->AINV, 1);
    cppt01(uplo_str, n, ws->A, ws->AINV, ws->RWORK, &result[0]);
    ...
    set_test_context(ctx);
    cblas_ccopy(npp, ws->AFAC, 1, ws->AINV, 1);
    cpptri(uplo_str, n, ws->AINV, &info);
    assert_int_equal(info, 0);

    cppt03(uplo_str, n, ws->A, ws->AINV, ws->WORK, lda,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        clarhs("CPP", "N", uplo_str, " ", n, n, kl, ku, nrhs,
               ws->A, lda, ws->XACT, lda, ws->B, lda, &info, rng_state);

        clacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        cpptrs(uplo_str, n, nrhs, ws->AFAC, ws->X, lda, &info);
        assert_int_equal(info, 0);

        clacpy("F", n, nrhs, ws->B, lda, ws->WORK, lda);
        cppt02(uplo_str, n, nrhs, ws->A, ws->X, lda,
    ...
    set_test_context(ctx);
    anorm = clanhp("1", uplo_str, n, ws->A, ws->RWORK);
    cppcon(uplo_str, n, ws->AFAC, anorm, &rcond, ws->WORK,
           ws->RWORK, &info);
    assert_int_equal(info, 0);
...
/**
 * CMocka test function - dispatches to run_zchkpp_single based on prestate.
 */
static void test_zchkpp_case(void** state)
{
    zchkpp_params_t* params = *state;
    run_zchkpp_single(params->n, params->iuplo, params->imat);
}
...
static zchkpp_params_t g_params[MAX_TESTS];
    ...
    for (INT in = 0; in < (INT)NN; in++) {
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                /* Store parameters */
                zchkpp_params_t* p = &g_params[g_num_tests];
                p->n = n;
                p->imat = imat;
                p->iuplo = iuplo;
                snprintf(p->name, sizeof(p->name), "zchkpp_n%d_%c_type%d",
                         n, UPLOS[iuplo], imat);

                /* Create CMocka test entry */
                g_tests[g_num_tests].name = p->name;
                g_tests[g_num_tests].test_func = test_zchkpp_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchkpp", g_tests, g_num_tests,
