 ...
 *   TEST 5: Solution residual via csyt02 (using csytrs_3)
...
typedef struct {
    ...
    c64* E;      /* Superdiagonal/subdiagonal of D (RK-specific, complex) */
...
static void run_zchksy_rk_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    csytrf_rk(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    csyt01_3(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        csytri_3(uplo_str, n, ws->AINV, lda, ws->E, ws->IPIV,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 5: Solve and compute residual for A * X = B using csytrs_3
         */
        ...
        csytrs_3(uplo_str, n, nrhs, ws->AFAC, lda, ws->E, ws->IPIV,
                 ws->X, lda, &info);
        assert_int_equal(info, 0);
...
test7:
    ...
    csycon_3(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV, anorm, &rcond,
             ws->WORK, &info);
    assert_int_equal(info, 0);
