 ...
 * Tests CSYTRF_AA_2STAGE and CSYTRS_AA_2STAGE.
 *
 ...
 *   TEST 2: Solution residual via csyt02 (using csytrs_aa_2stage)
 *
...
typedef struct {
    ...
    c64* AINV;   /* TB storage for band matrix factors */
...
static int group_setup(void** state)
{
    ...
    g_workspace->AINV = malloc(ltb_max * sizeof(c64));
...
static void run_zchksy_aa_2stage_single(INT n, INT iuplo, INT imat, INT inb)
{
     ...
     * Use MATPATH = "CSY" (complex symmetric types) */
    ...
    csytrf_aa_2stage(uplo_str, n, ws->AFAC, lda,
                     ws->AINV, ltb,
                     ws->IWORK, ws->IWORK + n,
                     ws->WORK, lwork, &info);
    ...
    /* Check error code from CSYTRF_AA_2STAGE */
    if (info != k_expected) {
        snprintf(ctx, sizeof(ctx),
                 "n=%d uplo=%c imat=%d CSYTRF_AA_2STAGE info=%d expected=%d",
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 2: Solve and compute residual for A * X = B using csytrs_aa_2stage
         */
        snprintf(ctx, sizeof(ctx),
        ...
        csytrs_aa_2stage(uplo_str, n, nrhs, ws->AFAC, lda,
                         ws->AINV, (3 * nb + 1) * n,
                         ws->IWORK, ws->IWORK + n,
                         ws->X, lda, &info);
        ...
        if (info != 0) {
            if (izero < 0) {
                snprintf(ctx, sizeof(ctx),
                         "n=%d uplo=%c imat=%d nrhs=%d CSYTRS_AA_2STAGE info=%d",
