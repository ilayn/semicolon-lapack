/**
 * @file test_cdrvrf3.c
 * @brief Test suite for CTFSM (RFP triangular solve).
 *
 * Port of LAPACK's TESTING/LIN/zdrvrf3.f to C using CMocka.
 * Tests CTFSM by comparing it against cblas_ztrsm on the same input:
                                ...
                                INT iside, INT itrans, INT idiag, INT ialpha)
{
    ...
    c64 alpha;
    if (ialpha == 1)
        alpha = CMPLXF(0.0f, 0.0f);
    else if (ialpha == 2)
        alpha = CMPLXF(1.0f, 0.0f);
    else
        alpha = clarnd_rng(4, rng_state);

    INT na = (iside == 0) ? m : n;

    c64 A[NMAX * NMAX];
    c64 ARF[NMAX * (NMAX + 1) / 2];
    c64 B1[NMAX * NMAX];
    c64 B2[NMAX * NMAX];
    c64 TAU[NMAX];
    c64 Z_WORK_ZGEQRF[NMAX];
    f32 D_WORK_ZLANGE[NMAX];

    for (INT j = 0; j < na; j++)
        for (INT i = 0; i < na; i++)
            A[i + j * lda] = clarnd_rng(4, rng_state);
    ...
    if (iuplo == 0) {
        ...
        if (idiag == 1) {
            for (INT i = 0; i < na; i++)
                for (INT j = 0; j <= i; j++)
                    A[i + j * lda] /= (2.0f * A[i + i * lda]);
        }
    }

    for (INT j = 0; j < na; j++)
        A[j + j * lda] *= clarnd_rng(5, rng_state);

    ctrttf(cform, uplo, na, A, lda, ARF, &info);

    for (INT j = 0; j < n; j++)
        for (INT i = 0; i < m; i++) {
            B1[i + j * lda] = clarnd_rng(4, rng_state);
    ...
    cblas_ctrsm(CblasColMajor, sideC, uploC, transC, diagC, m, n,
                &alpha, A, lda, B1, lda);
    ...
    f32 result = clange("I", m, n, B1, lda, D_WORK_ZLANGE);

    INT mn_max = (m > n) ? m : n;
    if (mn_max < 1) mn_max = 1;
    result = result / sqrtf(eps) / mn_max;

    snprintf(ctx, sizeof(ctx),
             "m=%d n=%d form=%s side=%s uplo=%s trans=%s diag=%s resid=%.6e",
             m, n, cform, side, uplo, trans, diag, (double)result);
