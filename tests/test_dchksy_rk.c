 ...
 *   TEST 5: Solution residual via dpot02 (using dsytrs_3)
...
/* Routines under test */
extern void dsytrf_rk(const char* uplo, const int n, double* A, const int lda,
                      double* E, int* ipiv, double* work, const int lwork,
                      int* info);
extern void dsytri_3(const char* uplo, const int n, double* A, const int lda,
                     const double* E, const int* ipiv, double* work,
                     const int lwork, int* info);
extern void dsytrs_3(const char* uplo, const int n, const int nrhs,
                     const double* A, const int lda, const double* E,
                     const int* ipiv, double* B, const int ldb, int* info);
extern void dsycon_3(const char* uplo, const int n, const double* A,
                     const int lda, const double* E, const int* ipiv,
                     const double anorm, double* rcond, double* work,
                     int* iwork, int* info);

/* Verification routines */
extern void dsyt01_3(const char* uplo, const int n, const double* A,
                     const int lda, double* AFAC, const int ldafac,
                     double* E, int* ipiv, double* C, const int ldc,
...
typedef struct {
    ...
    double* E;      /* Superdiagonal/subdiagonal of D (RK-specific) */
...
static int group_setup(void** state)
{
    ...
    g_workspace->E = malloc(NMAX * sizeof(double));
...
static void run_dchksy_rk_single(int n, int iuplo, int imat, int inb)
{
    ...
    dsytrf_rk(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    dsyt01_3(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        dsytri_3(uplo_str, n, ws->AINV, lda, ws->E, ws->IPIV,
    ...
    for (int irhs = 0; irhs < (int)NNS; irhs++) {
        ...
        /*
         * TEST 5: Solve and compute residual for A * X = B using dsytrs_3
         */
        ...
        dsytrs_3(uplo_str, n, nrhs, ws->AFAC, lda, ws->E, ws->IPIV,
                 ws->X, lda, &info);
        assert_int_equal(info, 0);
...
test7:
    ...
    dsycon_3(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV, anorm, &rcond,
