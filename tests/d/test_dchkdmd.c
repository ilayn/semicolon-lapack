/**
 * @file test_dchkdmd.c
 * @brief Test driver for the Dynamic Mode Decomposition routines DGEDMD and
 *        DGEDMDQ - port of LAPACK TESTING/EIG/dchkdmd.f90.
 *
 ...
 * produced by powering A. DGEDMD (and, for single-trajectory data, DGEDMDQ)
 ...
 *   - DGEDMD / DGEDMDQ singular values agree
 *   - F = Q*R                (QR factors from DGEDMDQ)
 *   - DGEDMDQ residuals match explicitly computed residuals
 */
...
typedef struct {
    f64 *A, *AC, *VA, *F, *F1, *F2, *X, *X0, *Y, *Y0, *Y1, *Z, *Z1, *S, *AU, *W;
    f64 *EIGA, *DA, *DL, *DR;
    f64 *REIG, *IEIG, *REIGQ, *IEIGQ, *REIGA, *IEIGA;
    f64 *RES, *RES1, *RESEX, *SINGVX, *SINGVQX;
    f64 *work;
    INT *iwork;
    INT *iwork_gen;
    uint64_t state[4];
...
static int group_setup(void** state)
{
    ...
    g_ws->A  = malloc(m2 * sizeof(f64));
    g_ws->AC = malloc(m2 * sizeof(f64));
    g_ws->VA = malloc(m2 * sizeof(f64));
    g_ws->F  = malloc(m2 * sizeof(f64));
    g_ws->F1 = malloc(m2 * sizeof(f64));
    g_ws->F2 = malloc(m2 * sizeof(f64));
    g_ws->X  = malloc(m2 * sizeof(f64));
    g_ws->X0 = malloc(m2 * sizeof(f64));
    g_ws->Y  = malloc(m2 * sizeof(f64));
    g_ws->Y0 = malloc(m2 * sizeof(f64));
    g_ws->Y1 = malloc(m2 * sizeof(f64));
    g_ws->Z  = malloc(m2 * sizeof(f64));
    g_ws->Z1 = malloc(m2 * sizeof(f64));
    g_ws->S  = malloc(m2 * sizeof(f64));
    g_ws->AU = malloc(m2 * sizeof(f64));
    g_ws->W  = malloc(m2 * sizeof(f64));
    g_ws->EIGA = malloc(MAXM * 2 * sizeof(f64));
    g_ws->DA = malloc(MAXM * sizeof(f64));
    g_ws->DL = malloc(MAXM * sizeof(f64));
    g_ws->DR = malloc(MAXM * sizeof(f64));
    g_ws->REIG  = malloc(MAXM * sizeof(f64));
    g_ws->IEIG  = malloc(MAXM * sizeof(f64));
    g_ws->REIGQ = malloc(MAXM * sizeof(f64));
    g_ws->IEIGQ = malloc(MAXM * sizeof(f64));
    g_ws->REIGA = malloc(MAXM * sizeof(f64));
    g_ws->IEIGA = malloc(MAXM * sizeof(f64));
    g_ws->RES     = malloc(MAXM * sizeof(f64));
    g_ws->RES1    = malloc(MAXM * sizeof(f64));
    g_ws->RESEX   = malloc(MAXM * sizeof(f64));
    g_ws->SINGVX  = malloc(MAXM * sizeof(f64));
    g_ws->SINGVQX = malloc(MAXM * sizeof(f64));
    g_ws->work  = malloc(WORKSZ * sizeof(f64));
...
static void run_case(INT M, INT N)
{
    ...
    const f64 eps = dlamch("P");
    const f64 tol = M * eps;
    f64 ab[4];
    INT info, i;

    /* Accumulated worst-case (adjusted) error measures over the whole battery */
    f64 e_zxw = ZERO, e_au = ZERO, e_rez = ZERO;
    f64 e_sv = ZERO, e_fqr = ZERO, e_rezq = ZERO;
    ...
    for (INT k_traj = 1; k_traj <= 2; k_traj++) {
        const f64 cond = 1.0e8, dmax = 1.0e2, condl = 1.0e2, condr = 1.0e2;

        for (INT mode = 1; mode <= 6; mode++) {
            /* Generate the M-by-M operator A. */
            dlatmr(M, M, "S", "N", w->DA, mode, cond, dmax, "F", "N", w->DL, 6,
            ...
            memcpy(w->AC, w->A, (size_t)lda * M * sizeof(f64));
            dgeev("N", "V", M, w->AC, M, w->REIGA, w->IEIGA, w->VA, M, w->VA, M,
                  w->work, 4 * M + 1, &info);
            f64 tmp = ZERO;
            for (i = 0; i < M; i++) {
                w->EIGA[i] = w->REIGA[i];
                w->EIGA[i + M] = w->IEIGA[i];
                f64 r = sqrt(w->REIGA[i] * w->REIGA[i] + w->IEIGA[i] * w->IEIGA[i]);
                if (r > tmp) tmp = r;
            }
            /* Scale A to have the desirable spectral radius. */
            dlascl("G", 0, 0, tmp, ONE, M, M, w->A, M, &info);
            dlascl("G", 0, 0, tmp, ONE, M, 2, w->EIGA, M, &info);

            const f64 anorm = dlange("F", N, N, w->A, M, NULL);

            if (k_traj == 2) {
                /* Data from two initial conditions. */
                dlarnv(2, iseed, M, &w->F1[0]);
                for (i = 0; i < M; i++) w->F1[i] = 1.0e-10 * w->F1[i];
                for (i = 0; i < N / 2; i++)
                    cblas_dgemv(CblasColMajor, CblasNoTrans, M, M, ONE, w->A, M,
                                &w->F1[i * ldf], 1, ZERO, &w->F1[(i + 1) * ldf], 1);
                for (INT jc = 0; jc < N / 2; jc++) {
                    memcpy(&w->X0[jc * ldx], &w->F1[jc * ldf], (size_t)M * sizeof(f64));
                    memcpy(&w->Y0[jc * ldy], &w->F1[(jc + 1) * ldf], (size_t)M * sizeof(f64));
                }
                dlarnv(2, iseed, M, &w->F1[0]);
                for (i = 0; i < N - N / 2; i++)
                    cblas_dgemv(CblasColMajor, CblasNoTrans, M, M, ONE, w->A, M,
                ...
                for (INT jc = 0; jc < N - N / 2; jc++) {
                    memcpy(&w->X0[(N / 2 + jc) * ldx], &w->F1[jc * ldf], (size_t)M * sizeof(f64));
                    memcpy(&w->Y0[(N / 2 + jc) * ldy], &w->F1[(jc + 1) * ldf], (size_t)M * sizeof(f64));
                }
            } else {
                /* Single trajectory. */
                dlarnv(2, iseed, M, &w->F[0]);
                for (i = 0; i < N; i++)
                    cblas_dgemv(CblasColMajor, CblasNoTrans, M, M, ONE, w->A, M,
                                &w->F[i * ldf], 1, ZERO, &w->F[(i + 1) * ldf], 1);
                for (INT jc = 0; jc < N; jc++) {
                    memcpy(&w->X0[jc * ldx], &w->F[jc * ldf], (size_t)M * sizeof(f64));
                    memcpy(&w->Y0[jc * ldy], &w->F[(jc + 1) * ldf], (size_t)M * sizeof(f64));
                }
            }
                        ...
                        for (INT inrnk = -1; inrnk >= -2; inrnk--) {
                            ...
                            for (INT iwhtsvd = 1; iwhtsvd <= 4; iwhtsvd++) {
                                ...
                                for (INT lwminopt = 1; lwminopt <= 2; lwminopt++) {
                                    ...
                                    for (INT jc = 0; jc < N; jc++) {
                                        memcpy(&w->X[jc * ldx], &w->X0[jc * ldx], (size_t)M * sizeof(f64));
                                        memcpy(&w->Y[jc * ldy], &w->Y0[jc * ldy], (size_t)M * sizeof(f64));
                                    }

                                    /* DGEDMD: workspace query */
                                    dgedmd(scale, jobz, resids, jobref, whtsvd, M, N,
                                    ...
                                    memcpy(w->SINGVX, w->work, (size_t)N * sizeof(f64));
                                    ...
                                    if (jobz[0] == 'V') {
                                        ...
                                        f64 tmp1 = ZERO;
                                        for (i = 0; i < k; i++) {
                                            cblas_daxpy(M, -ONE, &w->Z[i * ldz], 1, &w->Z1[i * ldz], 1);
                                            f64 nn = cblas_dnrm2(M, &w->Z1[i * ldz], 1);
                                            if (nn > tmp1) tmp1 = nn;
                                        }
                                    ...
                                    if (jobref[0] == 'R') {
                                        ...
                                        f64 tmp1 = ZERO;
                                        for (i = 0; i < k; i++) {
                                            cblas_daxpy(M, -ONE, &w->AU[i * ldau], 1, &w->Z1[i * ldz], 1);
                                            f64 v = cblas_dnrm2(M, &w->Z1[i * ldz], 1) *
                                        ...
                                        while (i < k) {
                                            if (w->IEIG[i] == ZERO) {
                                                cblas_daxpy(M, -w->REIG[i], &w->AU[i * ldau], 1, &w->Y1[i * M], 1);
                                                w->RESEX[i] = cblas_dnrm2(M, &w->Y1[i * M], 1) /
                                                              cblas_dnrm2(M, &w->AU[i * ldau], 1);
                                                i = i + 1;
                                            } else {
                                                ...
                                                cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                                            ...
                                                            ONE, &w->Y1[i * M], M);
                                                w->RESEX[i] = dlange("F", M, 2, &w->Y1[i * M], M, NULL) /
                                                              dlange("F", M, 2, &w->AU[i * ldau], M, NULL);
                                    ...
                                    if (resids[0] == 'R') {
                                        ...
                                        while (i < k) {
                                            if (w->IEIG[i] == ZERO) {
                                                cblas_daxpy(M, -w->REIG[i], &w->Z[i * ldz], 1, &w->Y1[i * M], 1);
                                                w->RES1[i] = cblas_dnrm2(M, &w->Y1[i * M], 1);
                                                i = i + 1;
                                            } else {
                                                ab[0] = w->REIG[i]; ab[1] = -w->IEIG[i];
                                                ab[2] = w->IEIG[i]; ab[3] = w->REIG[i];
                                                cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                                            ...
                                                            ONE, &w->Y1[i * M], M);
                                                w->RES1[i] = dlange("F", M, 2, &w->Y1[i * M], M, NULL);
                                                w->RES1[i + 1] = w->RES1[i];
                                                i = i + 2;
                                            }
                                        }
                                    ...
                                    /*==========  DGEDMDQ  ==========*/
                                    if (k_traj == 1) {
                                        memcpy(w->F1, w->F, (size_t)ldf * (N + 1) * sizeof(f64));

                                        dgedmdq(scale, jobz, resids, "Q", "R", jobref,
                                        ...
                                        for (i = 0; i < mnk; i++) {
                                            f64 v = fabs(w->SINGVX[i] - w->SINGVQX[i]) / w->SINGVX[0];
                                            if (v > tmp1) tmp1 = v;
                                        }
                                        if (tmp1 > e_sv) e_sv = tmp1;

                                        /* F = Q*R : ||F - F1*Y||_F / ||F||_F */
                                        memcpy(w->F2, w->F, (size_t)ldf * (N + 1) * sizeof(f64));
                                        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                                    M, N + 1, mn, -ONE, w->F1, ldf, w->Y, ldy,
                                                    ONE, w->F2, ldf);
                                        f64 fqr = dlange("F", M, N + 1, w->F2, ldf, NULL) /
                                                  dlange("F", M, N + 1, w->F, ldf, NULL);
                                        ...
                                        if (resids[0] == 'R') {
                                            ...
                                            while (i < kq) {
                                                if (w->IEIGQ[i] == ZERO) {
                                                    cblas_daxpy(M, -w->REIGQ[i], &w->Z[i * ldz], 1, &w->Y1[i * M], 1);
                                                    w->RES1[i] = cblas_dnrm2(M, &w->Y1[i * M], 1);
                                                    i = i + 1;
                                                } else {
...
static void test_dchkdmd_case(void** state)
{
...
int main(void)
{
    ...
    for (INT i = 0; i < NDIM; i++) {
        tests[i].name = "dchkdmd";
        tests[i].test_func = test_dchkdmd_case;
        tests[i].setup_func = NULL;
        tests[i].teardown_func = NULL;
        tests[i].initial_state = (void*)(intptr_t)i;
    }
    (void)_cmocka_run_group_tests("dchkdmd", tests, NDIM, group_setup, group_teardown);
    return 0;
}
