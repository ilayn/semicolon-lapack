 ...
 * Tests DGELS, DGELST, DGETSLS, DGELSY, DGELSS, and DGELSD.
 *
 ...
 *   - Tests 1-2: DGELS (full-rank, trans='N' and 'T')
 *   - Tests 3-4: DGELST (full-rank, trans='N' and 'T')
 *   - Tests 5-6: DGETSLS (full-rank, TSQR-based, trans='N' and 'T')
 *   - Tests 7-10: DGELSY (rank-revealing QR with pivoting)
 *   - Tests 11-14: DGELSS (SVD-based)
 *   - Tests 15-18: DGELSD (divide-conquer SVD)
 *
...
static void run_fullrank_tests(INT m, INT n, INT nrhs, INT iscale, INT nb, INT nx)
{
    ...
    for (INT itran = 1; itran <= 2; itran++) {
        ...
        dlacpy("F", nrows, nrhs, ws->B, ldb, ws->COPYB, ldb);

        if (m > 0 && n > 0) {
            dlacpy("F", m, n, ws->COPYA, lda, ws->A, lda);
            dlacpy("F", nrows, nrhs, ws->COPYB, ldb, ws->B, ldb);
        }
        dgels(trans, m, n, nrhs, ws->A, lda, ws->B, ldb, ws->WORK, ws->lwork, &info);

        if (nrows > 0 && nrhs > 0) {
            dlacpy("F", nrows, nrhs, ws->COPYB, ldb, ws->C, ldb);
        }
        dqrt16(trans, m, n, nrhs, ws->COPYA, lda, ws->B, ldb, ws->C, ldb,
               ws->WORK, &result[itran - 1]);

        snprintf(ctx, sizeof(ctx), "DGELS trans=%s m=%d n=%d nrhs=%d nb=%d scale=%d",
    ...
    for (INT itran = 1; itran <= 2; itran++) {
        ...
        dgelst(trans, m, n, nrhs, ws->A, lda, ws->B, ldb, ws->WORK, ws->lwork, &info);
        ...
        snprintf(ctx, sizeof(ctx), "DGELST trans=%s m=%d n=%d nrhs=%d nb=%d scale=%d",
                 trans, m, n, nrhs, nb, iscale);
        set_test_context(ctx);
    ...
    for (INT itran = 1; itran <= 2; itran++) {
        ...
        dgetsls(trans, m, n, nrhs, ws->A, lda, ws->B, ldb, ws->WORK, ws->lwork, &info);
        ...
        snprintf(ctx, sizeof(ctx), "DGETSLS trans=%s m=%d n=%d nrhs=%d nb=%d scale=%d",
                 trans, m, n, nrhs, nb, iscale);
        set_test_context(ctx);
...
static void run_rankdef_tests(INT m, INT n, INT nrhs, INT iscale, INT irank, INT nb, INT nx)
{
    ...
    dlacpy("F", m, n, ws->COPYA, lda, ws->A, lda);
    dlacpy("F", m, nrhs, ws->COPYB, ldb, ws->B, ldb);

    dgelsy(m, n, nrhs, ws->A, lda, ws->B, ldb, ws->JPVT, rcond, &crank,
           ws->WORK, ws->lwlsy, &info);

    result[6] = dqrt12(crank, crank, ws->A, lda, ws->COPYS, ws->WORK, ws->lwork);

    dlacpy("F", m, nrhs, ws->COPYB, ldb, ws->WORK, ldwork);
    dqrt16("N", m, n, nrhs, ws->COPYA, lda, ws->B, ldb, ws->WORK, ldwork,
           &ws->WORK[m * nrhs], &result[7]);

    if (m > crank) {
        result[8] = dqrt17("N", 1, m, n, nrhs, ws->COPYA, lda, ws->B, ldb,
                           ws->COPYB, ldb, ws->C, ws->WORK, ws->lwork);
    }

    if (n > crank) {
        result[9] = dqrt14("N", m, n, nrhs, ws->COPYA, lda, ws->B, ldb,
                           ws->WORK, ws->lwork);
    }

    snprintf(ctx, sizeof(ctx), "DGELSY m=%d n=%d nrhs=%d type=%d nb=%d", m, n, nrhs, itype, nb);
    set_test_context(ctx);
    ...
    dgelss(m, n, nrhs, ws->A, lda, ws->B, ldb, ws->S, rcond, &crank,
    ...
    if (m > crank) {
        result[12] = dqrt17("N", 1, m, n, nrhs, ws->COPYA, lda, ws->B, ldb,
                           ws->COPYB, ldb, ws->C, ws->WORK, ws->lwork);
    }

    if (n > crank) {
        result[13] = dqrt14("N", m, n, nrhs, ws->COPYA, lda, ws->B, ldb,
                           ws->WORK, ws->lwork);
    }

    snprintf(ctx, sizeof(ctx), "DGELSS m=%d n=%d nrhs=%d type=%d nb=%d", m, n, nrhs, itype, nb);
    set_test_context(ctx);
    ...
    dgelsd(m, n, nrhs, ws->A, lda, ws->B, ldb, ws->S, rcond, &crank,
    ...
    if (m > crank) {
        result[16] = dqrt17("N", 1, m, n, nrhs, ws->COPYA, lda, ws->B, ldb,
                           ws->COPYB, ldb, ws->C, ws->WORK, ws->lwork);
    }

    if (n > crank) {
        result[17] = dqrt14("N", m, n, nrhs, ws->COPYA, lda, ws->B, ldb,
                           ws->WORK, ws->lwork);
    }

    snprintf(ctx, sizeof(ctx), "DGELSD m=%d n=%d nrhs=%d type=%d nb=%d", m, n, nrhs, itype, nb);
    set_test_context(ctx);
...
static void run_ddrvls_single(ddrvls_params_t* p)
{
    ...
    /* Rank-deficient tests: DGELSY, DGELSS, DGELSD (run for all irank values) */
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("ddrvls", g_tests, g_num_tests,
