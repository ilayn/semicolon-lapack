 ...
 * standard 1-stage reduction DSYTRD. Tests 1 and 2 remain to verify
 ...
 *   DSYTRD_2STAGE       - 2-stage symmetric to tridiagonal reduction
 ...
 *   Tests  1-2 : Reduction quality (DSYTRD/DORGTR upper)
 *   Tests  3-4 : 2-stage eigenvalue comparison (DSYTRD_2STAGE upper/lower)
 *   Tests  5-8 : Packed reduction quality (DSPTRD/DOPGTR)
...
static int group_setup(void** state)
{
    ...
    /* Ensure workspace is large enough for dsytrd_2stage.
...
static void test_dchkst2stg_case(void** state)
{
     ...
     * Compute D1 from 1-stage DSYTRD(U) as reference, then compare
     * with DSYTRD_2STAGE upper (D2) and lower (D3).
    ...
    ntest = 3;
    dsteqr("N", n, D1, work, NULL, ldu, NULL, &iinfo);
    if (iinfo != 0) {
    ...
    dlacpy("U", n, n, A, lda, V, ldu);
    dsytrd_2stage("N", "U", n, V, ldu, SD, SE, TAU,
    ...
    dsytrd_2stage("N", "L", n, V, ldu, SD, SE, TAU,
                  work, lh, work + lh, lw, &iinfo);

    cblas_dcopy(n, SD, 1, D3, 1);
    if (n > 0) cblas_dcopy(n - 1, SE, 1, work, 1);

    dsteqr("N", n, D3, work, NULL, ldu, NULL, &iinfo);
    if (iinfo != 0) {
        print_message("DSTEQR(N) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        if (iinfo < 0) {
            ws->result[3] = ulpinv;
