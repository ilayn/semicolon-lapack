 ...
 *   TEST 5: Solution residual via zpot02 (using zhetrs_3)
...
typedef struct {
    ...
    c128* E;      /* Superdiagonal/subdiagonal of D (RK-specific) */
...
static void run_zchkhe_rk_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    zhetrf_rk(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    zhet01_3(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        zhetri_3(uplo_str, n, ws->AINV, lda, ws->E, ws->IPIV,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 5: Solve and compute residual for A * X = B using zhetrs_3
         */
        ...
        zhetrs_3(uplo_str, n, nrhs, ws->AFAC, lda, ws->E, ws->IPIV,
                 ws->X, lda, &info);
        assert_int_equal(info, 0);
...
test7:
    ...
    zhecon_3(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV, anorm, &rcond,
             ws->WORK, &info);
    assert_int_equal(info, 0);
