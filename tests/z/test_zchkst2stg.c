 ...
 * standard 1-stage reduction ZHETRD. Tests 1 and 2 remain to verify
 ...
 *   ZHETRD_2STAGE       - 2-stage Hermitian to tridiagonal reduction
 ...
 *   Tests  1-2 : Reduction quality (ZHETRD/ZUNGTR upper)
 *   Tests  3-4 : 2-stage eigenvalue comparison (ZHETRD_2STAGE upper/lower)
 *   Tests  5-8 : Packed reduction quality (ZHPTRD/ZUPGTR)
...
static int group_setup(void** state)
{
    ...
    /* Ensure workspace is large enough for zhetrd_2stage */
    {
        INT kd_2stg = ilaenv2stage(1, "ZHETRD_2STAGE", "N", nmax, -1, -1, -1);
        INT ib_2stg = ilaenv2stage(2, "ZHETRD_2STAGE", "N", nmax, kd_2stg, -1, -1);
        INT lhmin = ilaenv2stage(3, "ZHETRD_2STAGE", "N", nmax, kd_2stg, ib_2stg, -1);
        INT lwmin = ilaenv2stage(4, "ZHETRD_2STAGE", "N", nmax, kd_2stg, ib_2stg, -1);
...
static void test_zchkst2stg_case(void** state)
{
     ...
     * Compute D1 from 1-stage ZHETRD(U) as reference, then compare
     * with ZHETRD_2STAGE upper (D2) and lower (D3).
    ...
    ntest = 3;
    zsteqr("N", n, D1, rwork, work, ldu, rwork + n, &iinfo);
    if (iinfo != 0) {
    ...
    dlaset("Full", n, 1, 0.0, 0.0, SD, n);
    dlaset("Full", n, 1, 0.0, 0.0, SE, n);
    zlacpy("U", n, n, A, lda, V, ldu);
    zhetrd_2stage("N", "U", n, V, ldu, SD, SE, TAU,
    ...
    zhetrd_2stage("N", "L", n, V, ldu, SD, SE, TAU,
                  work, lh, work + lh, lw, &iinfo);

    cblas_dcopy(n, SD, 1, D3, 1);
    if (n > 0) cblas_dcopy(n - 1, SE, 1, rwork, 1);

    zsteqr("N", n, D3, rwork, work, ldu, rwork + n, &iinfo);
    if (iinfo != 0) {
        fprintf(stderr, "ZSTEQR(N) failed: info=%d n=%d jtype=%d\n", iinfo, n, jtype);
        if (iinfo < 0) {
            ws->result[3] = ulpinv;
    ...
    for (size_t in = 0; in < NNVAL; in++) {
        ...
        for (INT jtype = 1; jtype <= MAXTYP; jtype++) {
            /* Skip type 9 — matches LAPACK's sep.in which tests types
               ...
               cond=ulpinv) produces tridiagonals where ZSTEMR's MRRR
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("zchkst2stg", g_tests, g_num_tests,
