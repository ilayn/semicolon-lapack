/**
 * @file zqrt14.c
 * @brief ZQRT14 checks whether X is in the row space of A or A'.
 *
 * Faithful port of LAPACK TESTING/LIN/zqrt14.f
 *
...
/**
 * ZQRT14 checks whether X is in the row space of A or A'.
 *
 * @param[in] trans
              ...
              c128* work, const INT lwork)
{
    ...
    /* Copy and scale A */
    zlacpy("A", m, n, A, lda, work, ldwork);
    anrm = zlange("M", m, n, work, ldwork, rwork);
    if (anrm != ZERO) {
        zlascl("G", 0, 0, anrm, ONE, m, n, work, ldwork, &info);
    }

    /* Copy X or X' into the right place and scale it */
    if (tpsd) {
        /* Copy X into columns n:n+nrhs-1 of work */
        zlacpy("A", m, nrhs, X, ldx, &work[n * ldwork], ldwork);
        xnrm = zlange("M", m, nrhs, &work[n * ldwork], ldwork, rwork);
        if (xnrm != ZERO) {
            zlascl("G", 0, 0, xnrm, ONE, m, nrhs, &work[n * ldwork],
                   ldwork, &info);
        }
        ...
        for (i = 0; i < n; i++) {
            for (j = 0; j < nrhs; j++) {
                work[m + j + i * ldwork] = conj(X[i + j * ldx]);
            }
        }

        xnrm = zlange("M", nrhs, n, &work[m], ldwork, rwork);
        if (xnrm != ZERO) {
            zlascl("G", 0, 0, xnrm, ONE, nrhs, n, &work[m], ldwork, &info);
        }

        /* Compute LQ factorization of work */
        zgelq2(ldwork, n, work, ldwork,
               &work[ldwork * n],
               &work[ldwork * (n + 1)],
               &info);
