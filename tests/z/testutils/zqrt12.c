/**
 * @file zqrt12.c
 * @brief ZQRT12 computes || svd(R) - s || / (||s|| * eps * max(M,N)).
 *
 * Port of LAPACK TESTING/LIN/zqrt12.f to C.
 */
...
/**
 * ZQRT12 computes the singular values of the upper trapezoid
              ...
              f64* rwork)
{
    ...
    if (anrm != ZERO) {
         ...
         * work(m*n+2*mn:)  = workspace for zgebd2 (complex)
         */
        zgebd2(m, n, work, m,
               &rwork[0], &rwork[mn],
        ...
        if (iscl == 1) {
            if (anrm > bignum) {
                dlascl("G", 0, 0, bignum, anrm, mn, 1, &rwork[0], mn, &info);
            }
            if (anrm < smlnum) {
                dlascl("G", 0, 0, smlnum, anrm, mn, 1, &rwork[0], mn, &info);
            }
        }
    } else {
        for (i = 0; i < mn; i++) {
    ...
    cblas_daxpy(mn, -ONE, S, 1, &rwork[0], 1);

    /* Return || diff || / (eps * max(M,N)) / ||S|| */
    f64 result = cblas_dasum(mn, &rwork[0], 1) /
