/**
 * @file zsyt01_3.c
 * @brief ZSYT01_3 reconstructs a complex symmetric indefinite matrix A from its
 *        block L*D*L' or U*D*U' factorization (ZSYTRF_RK/ZSYTRF_BK) and computes the residual.
 *
 * Port of LAPACK TESTING/LIN/zsyt01_3.f to C.
 */
 ...
 * @param[in,out] ipiv    The pivot indices from ZSYTRF_RK (or ZSYTRF_BK).
    ...
    f64* resid)
{
    ...
    zlavsy_rook(uplo, "T", "N", n, n, AFAC, ldafac, ipiv, C, ldc, &info);

    /* 4) Call ZLAVSY_ROOK again to multiply by U (or L). */
    zlavsy_rook(uplo, "N", "U", n, n, AFAC, ldafac, ipiv, C, ldc, &info);
