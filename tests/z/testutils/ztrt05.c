/**
 * @file ztrt05.c
 * @brief ZTRT05 tests the error bounds from iterative refinement for triangular systems.
 *
 * Port of LAPACK TESTING/LIN/ztrt05.f to C.
 */
            ...
            const f64* ferr, const f64* berr, f64* reslts)
{
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        xnorm = cabs1(X[j * ldx + imax]);
        if (xnorm < unfl) xnorm = unfl;

        diff = ZERO;
        for (i = 0; i < n; i++) {
            f64 d = cabs1(X[j * ldx + i] - XACT[j * ldxact + i]);
            if (d > diff) diff = d;
        }
    ...
    for (k = 0; k < nrhs; k++) {
        for (i = 0; i < n; i++) {
            ...
            if (upper) {
                if (!notran) {
                    for (j = 0; j < i - ifu + 1; j++) {
                        tmp += cabs1(A[i * lda + j]) * cabs1(X[k * ldx + j]);
                    }
                    if (unit)
                        tmp += cabs1(X[k * ldx + i]);
                } else {
                    if (unit)
                        tmp += cabs1(X[k * ldx + i]);
                    for (j = i + ifu; j < n; j++) {
                        tmp += cabs1(A[j * lda + i]) * cabs1(X[k * ldx + j]);
                    }
                }
            } else {
                /* Lower triangular */
