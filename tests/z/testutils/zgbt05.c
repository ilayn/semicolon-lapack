/**
 * @file zgbt05.c
 * @brief ZGBT05 tests the error bounds from iterative refinement.
 *
 * Port of LAPACK TESTING/LIN/zgbt05.f
 */
        ...
        for (INT i = 0; i < n; i++) {
            ...
            if (notran) {
                ...
                for (INT j = j_start; j < j_end; j++) {
                    tmp += cabs1(AB[(ku + i - j) + j * ldab]) * cabs1(X[j + k * ldx]);
                }
            } else {
                INT j_start = (i - ku > 0) ? i - ku : 0;
                INT j_end = (i + kl + 1 < n) ? i + kl + 1 : n;
                for (INT j = j_start; j < j_end; j++) {
                    tmp += cabs1(AB[(ku + j - i) + i * ldab]) * cabs1(X[j + k * ldx]);
                }
            }
            if (i == 0) {
                axbi = tmp;
