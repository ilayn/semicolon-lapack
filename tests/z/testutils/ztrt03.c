/**
 * @file ztrt03.c
 * @brief ZTRT03 computes the residual for a scaled triangular system.
 *
 * Port of LAPACK TESTING/LIN/ztrt03.f to C.
 */
            ...
            c128* work, f64* resid)
{
    ...
    if (diag[0] == 'N' || diag[0] == 'n') {
        for (j = 0; j < n; j++) {
            f64 val = tscal * cabs(A[j * lda + j]) + cnorm[j];
