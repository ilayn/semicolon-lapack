/**
 * @file zppt05.c
 * @brief ZPPT05 tests the error bounds from iterative refinement for
 *        Hermitian packed matrices.
 *
 * Port of LAPACK TESTING/LIN/zppt05.f
 */
    ...
    for (k = 0; k < nrhs; k++) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (upper) {
                jc = (i * (i + 1)) / 2;
                for (j = 0; j < i; j++) {
                    tmp = tmp + cabs1(AP[jc + j]) * cabs1(X[j + k * ldx]);
                }
                tmp = tmp + fabs(creal(AP[jc + i])) * cabs1(X[i + k * ldx]);
                jc = jc + i + i + 1;
                for (j = i + 1; j < n; j++) {
