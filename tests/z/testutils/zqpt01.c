/**
 * @file zqpt01.c
 * @brief ZQPT01 tests the QR-factorization with pivoting of a matrix A.
 *
 * Port of LAPACK TESTING/LIN/zqpt01.f to C.
 */
              ...
              c128* work, const INT lwork)
{
    ...
    for (j = 0; j < k; j++) {
        ...
        for (i = j + 1; i < m; i++) {
            work[j * m + i] = CMPLX(0.0, 0.0);
        }
    }

    /* Copy columns (k:n-1) from AF into work */
    for (j = k; j < n; j++) {
        cblas_zcopy(m, &AF[j * lda], 1, &work[j * m], 1);
    }

    /* Form Q*R using zunmqr */
    zunmqr("L", "N", m, n, k, AF, lda, tau, work, m, &work[m * n], lwork - m * n, &info);

    /* Compute Q*R - A*P: for each column j, subtract A(:, jpvt[j]) */
    for (j = 0; j < n; j++) {
        cblas_zaxpy(m, &CNEGONE, &A[jpvt[j] * lda], 1, &work[j * m], 1);
    }
