/**
 * @file zpot06.c
 * @brief ZPOT06 computes the residual for a solution of a Hermitian positive definite system.
 */
            ...
            c128* B, const INT ldb, f64* rwork, f64* resid)
{
    ...
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        ...
        for (INT i = 0; i < n; i++) {
            f64 row_sum = 0.0;
            for (INT j = 0; j < i; j++) {
                row_sum += cabs1(A[j + i * lda]);
            }
            for (INT j = i; j < n; j++) {
                row_sum += cabs1(A[i + j * lda]);
            }
    ...
    cblas_zhemm(CblasColMajor, CblasLeft, uplo_cblas,
