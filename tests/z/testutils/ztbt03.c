/**
 * @file ztbt03.c
 * @brief ZTBT03 computes the residual for the solution to a scaled triangular
 *        system of equations when A is a triangular band matrix.
 *
 * Port of LAPACK TESTING/LIN/ztbt03.f to C.
 */
            ...
            c128* work, f64* resid)
{
    ...
    if (diag[0] == 'N' || diag[0] == 'n') {
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                f64 t = tscal * cabs(AB[kd + j * ldab]) + cnorm[j];
                if (t > tnorm) tnorm = t;
            }
        } else {
            for (j = 0; j < n; j++) {
                f64 t = tscal * cabs(AB[j * ldab]) + cnorm[j];
                if (t > tnorm) tnorm = t;
            }
        }
    } else {
    ...
    *resid = ZERO;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        cblas_ztbmv(CblasColMajor, cblas_uplo, cblas_trans, cblas_diag,
                    n, kd, AB, ldab, work, 1);
        c128 cscal = CMPLX(-scale * xscal, 0.0);
        cblas_zaxpy(n, &cscal, &B[j * ldb], 1, work, 1);
        ix = cblas_izamax(n, work, 1);
        err = tscal * cabs(work[ix]);
        ix = cblas_izamax(n, &X[j * ldx], 1);
