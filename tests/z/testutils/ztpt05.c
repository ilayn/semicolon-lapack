/**
 * @file ztpt05.c
 * @brief ZTPT05 tests the error bounds from iterative refinement for a
 *        triangular system when A is in packed format.
 *
 * Port of LAPACK TESTING/LIN/ztpt05.f to C.
 */
    ...
    for (k = 0; k < nrhs; k++) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (upper) {
                ...
                if (!notran) {
                    for (j = 0; j < i + 1 - ifu; j++) {
                        tmp += cabs1(AP[jc + j]) * cabs1(X[j + k * ldx]);
                    }
                    if (unit) {
                        tmp += cabs1(X[i + k * ldx]);
                    }
                    ...
                    for (j = i + ifu; j < n; j++) {
                        tmp += cabs1(AP[jc]) * cabs1(X[j + k * ldx]);
                        jc += j + 1;
                    }
                }
            } else {
                if (notran) {
                    ...
                    for (j = i + ifu; j < n; j++) {
                        tmp += cabs1(AP[jc + j - i]) * cabs1(X[j + k * ldx]);
                    }
                }
            }
            if (i == 0) {
