/**
 * @file zlqt03.c
 * @brief ZLQT03 tests ZUNMLQ, which computes Q*C, Q'*C, C*Q or C*Q'.
 *
 * Compares the results of ZUNMLQ with the results of forming Q explicitly
 * via ZUNGLQ and then performing ZGEMM.
 *
...
static f64 zlqt03_drand(void) {
    uint64_t x = zlqt03_next();
    return ((f64)(x >> 11)) * 0x1.0p-53 * 2.0 - 1.0;
}

/**
 * @param[in]     m       Number of rows or columns of C (see ZUNMLQ docs).
 * @param[in]     n       Order of Q. n >= 0.
 * @param[in]     k       Number of reflectors. n >= k >= 0.
 * @param[in]     AF      LQ factorization from ZGELQF, dimension (lda, n).
            ...
            f64* restrict result)
{
    ...
    for (INT iside = 0; iside < 2; iside++) {
        ...
        for (INT j = 0; j < nc; j++) {
            for (INT i = 0; i < mc; i++) {
                C[i + j * lda] = CMPLX(zlqt03_drand(), zlqt03_drand());
            }
        }
