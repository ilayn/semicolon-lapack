/**
 * @file zpot03.c
 * @brief ZPOT03 computes the residual for a Hermitian matrix times its inverse.
 */
...
/**
 * ZPOT03 computes the residual for a Hermitian matrix times its inverse:
    ...
    f64* resid)
{
    ...
    f64 ainvnm = zlanhe("1", uplo, n, AINV, ldainv, rwork);
    ...
    *rcond = (ONE / anorm) / ainvnm;
    ...
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        for (INT j = 0; j < n; j++) {
            for (INT i = 0; i < j; i++) {
                AINV[j + i * ldainv] = conj(AINV[i + j * ldainv]);
            }
        }
    } else {
        for (INT j = 0; j < n; j++) {
