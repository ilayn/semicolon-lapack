/**
 * @file zlatm2.c
 * @brief ZLATM2 returns the (I,J) entry of a random matrix.
 *
 * Faithful port of LAPACK TESTING/MATGEN/zlatm2.f
 */
 ...
 *
 * Use of ZLATM2 differs from ZLATM3 in the order in which the random
    ...
    uint64_t state[static 4])
{
    ...
    if (igrade == 1) {
        ...
        ctemp = ctemp * dl[isub] * conj(dl[jsub]);
    } else if (igrade == 6) {
        ctemp = ctemp * dl[isub] * dl[jsub];
    }
