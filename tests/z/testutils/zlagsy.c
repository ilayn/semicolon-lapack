/**
 * @file zlagsy.c
 * @brief ZLAGSY generates a complex symmetric matrix A, by pre- and post-
 *        multiplying a real diagonal matrix D with a random unitary matrix.
 *
 * Faithful port of LAPACK TESTING/MATGEN/zlagsy.f
 */
    ...
    uint64_t state[static 4])
{
    ...
    *info = 0;
    ...
    for (i = n - 2; i >= 0; i--) {
        ...
        zsymv("L", len, tau_c, &A[i + i * lda], lda, work, 1, CZERO, &work[n], 1);
        zlacgv(len, work, 1);

        /* Compute v := y - 1/2 * tau * ( u, y ) * u */
        c128 dot;
        cblas_zdotc_sub(len, work, 1, &work[n], 1, &dot);
    ...
    for (i = 0; i < n - 1 - k; i++) {
        ...
        /* Compute y := tau * A * conjg(u) */
        zlacgv(len, &A[k + i + i * lda], 1);
        c128 tau_c = CMPLX(tau, 0.0);
        zsymv("L", len, tau_c, &A[k + i + (k + i) * lda], lda,
              &A[k + i + i * lda], 1, CZERO, work, 1);
        zlacgv(len, &A[k + i + i * lda], 1);

        /* Compute v := y - 1/2 * tau * ( u, y ) * u */
        c128 dot;
        cblas_zdotc_sub(len, &A[k + i + i * lda], 1, work, 1, &dot);
        c128 alpha = CMPLX(-0.5 * tau, 0.0) * dot;
