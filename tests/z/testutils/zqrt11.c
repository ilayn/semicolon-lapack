/**
 * @file zqrt11.c
 * @brief ZQRT11 computes the test ratio || Q'*Q - I || / (eps * m).
 *
 * Port of LAPACK TESTING/LIN/zqrt11.f to C.
 */
              ...
              const c128* tau, c128* work, const INT lwork)
{
    ...
    zlaset("F", m, m, CZERO, CONE, work, m);

    /* Form Q by applying transformations from the left with 'N' */
    zunm2r("L", "N", m, m, k, A, lda, tau, work, m, &work[m * m], &info);

    /* Form Q'*Q by applying transformations from the left with 'C' */
    zunm2r("L", "C", m, m, k, A, lda, tau, work, m, &work[m * m], &info);
    ...
    /* Return || Q'*Q - I || / (eps * m) */
    return zlange("1", m, m, work, m, rdummy) / ((f64)m * dlamch("E"));
}
