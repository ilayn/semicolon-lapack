/**
 * @file zgbt01.c
 * @brief ZGBT01 reconstructs a band matrix from its L*U factorization.
 *
 * Port of LAPACK TESTING/LIN/zgbt01.f
 */
 ...
 * @param[in] AFAC   The factored form of A from ZGBTRF. Dimension (ldafac, n).
 * @param[in] ldafac The leading dimension of AFAC. ldafac >= max(1, 2*kl+ku+1).
 * @param[in] ipiv   The pivot indices from ZGBTRF. Dimension (min(m,n)).
            ...
            f64* resid)
{
    ...
    *resid = ZERO;
    ...
    for (INT j = 0; j < n; j++) {
        ...
        if (i2_excl > i1) {
            f64 col_sum = cblas_dzasum(i2_excl - i1, &A[i1 + j * lda], 1);
            if (col_sum > anorm) {
                anorm = col_sum;
            }
        }
    ...
    for (INT j = 0; j < n; j++) {
        ...
        if (lenj > 0) {
            /* Copy U column */
            cblas_zcopy(lenj, &AFAC[(kd - ju) + j * ldafac], 1, work, 1);
            ...
            for (INT i = i_start; i >= j - ju; i--) {
                ...
                if (il > 0) {
                    INT iw = i - j + ju;
                    c128 t = work[iw];
                    cblas_zaxpy(il, &t, &AFAC[(kd + 1) + i * ldafac], 1, &work[iw + 1], 1);
            ...
            INT jua = (ju < ku) ? ju : ku;
            if (jua + jl + 1 > 0) {
                c128 neg_one = CMPLX(-ONE, 0.0);
                cblas_zaxpy(jua + jl + 1, &neg_one,
                           &A[(ku - jua) + j * lda], 1,
                           &work[ju - jua], 1);
            }

            /* Compute the 1-norm of the column. */
            f64 col_resid = cblas_dzasum(ju + jl + 1, work, 1);
            if (col_resid > *resid) {
                *resid = col_resid;
            }
        }
