...
static void run_zdrvev_single(zdrvev_params_t* params)
{
    ...
    if (iinfo != 0) {
        ws->result[0] = ulpinv;
        print_message("ZGEEV(V,V) failed with info=%d\n", iinfo);
        assert_info_success(iinfo);
        return;
    }
    ...
    if (iinfo != 0) {
        ws->result[4] = ulpinv;
        print_message("ZGEEV(N,N) failed with info=%d\n", iinfo);
    } else {
    ...
    if (iinfo != 0) {
        ws->result[5] = ulpinv;
        print_message("ZGEEV(N,V) failed with info=%d\n", iinfo);
    } else {
        /* Test 5 again */
        for (INT j = 0; j < n; j++) {
            if (W[j] != W1[j]) {
    ...
    if (iinfo != 0) {
        ws->result[6] = ulpinv;
        print_message("ZGEEV(V,N) failed with info=%d\n", iinfo);
    } else {
        /* Test 5 again */
        for (INT j = 0; j < n; j++) {
            if (W[j] != W1[j]) {
