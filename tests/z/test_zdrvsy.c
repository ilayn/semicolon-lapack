/**
 * @file test_zdrvsy.c
 * @brief ZDRVSY tests the driver routines ZSYSV and ZSYSVX.
 *
 * Port of LAPACK TESTING/LIN/zdrvsy.f to C with CMocka parameterization.
 */
...
static zdrvsy_workspace_t* g_workspace = NULL;

static int group_setup(void** state)
{
    (void)state;
    g_workspace = malloc(sizeof(zdrvsy_workspace_t));
    ...
    g_workspace->AINV = calloc(nmax * nmax, sizeof(c128));
...
static void run_zdrvsy_single(INT n, INT imat, INT iuplo, INT ifact)
{
    static const char* UPLOS[] = {"U", "L"};
    static const char* FACTS[] = {"F", "N"};

    zdrvsy_workspace_t* ws = g_workspace;
    ...
    /* Generate test matrix with ZLATMS */
    ...
    if (zerot) {
        ...
        f64 anrm = zlansy("1", uplo, n, ws->A, lda, ws->RWORK);

        zlacpy(uplo, n, n, ws->A, lda, ws->AFAC, lda);
        zsytrf(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK, ws->lwork, &info);

        zlacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        INT lwork_tri = (n + 1 + 1) * (1 + 3);
        zsytri2(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, lwork_tri, &info);
        f64 ainvnm = zlansy("1", uplo, n, ws->AINV, lda, ws->RWORK);
    ...
    char xtype = 'N';
    zlarhs("ZSY", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (ifact == 1) {
        ...
        zsysv(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK, ws->X, lda,
              ws->WORK, ws->lwork, &info);

        if (zerot) {
            if (info <= 0) {
                fail_msg("ZSYSV: expected INFO > 0 for singular matrix, got %d", info);
                return;
            }
        } else if (info != 0) {
            fail_msg("ZSYSV info=%d expected=0", info);
            return;
        } else {
            /* TEST 1: Reconstruct matrix from factors */
            zsyt01(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
                   ws->AINV, lda, ws->RWORK, &result[0]);

            /* TEST 2: Compute residual of computed solution */
            zlacpy("Full", n, NRHS, ws->B, lda, ws->WORK, lda);
            zsyt02(uplo, n, NRHS, ws->A, lda, ws->X, lda,
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("ZSYSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    /* --- Test ZSYSVX --- */
    if (ifact == 1) {
        zlaset(uplo, n, n, CMPLX(0.0, 0.0), CMPLX(0.0, 0.0), ws->AFAC, lda);
    }
    zlaset("Full", n, NRHS, CMPLX(0.0, 0.0), CMPLX(0.0, 0.0), ws->X, lda);

    f64 rcond;
    zsysvx(fact, uplo, n, NRHS, ws->A, lda, ws->AFAC, lda,
    ...
    if (zerot) {
        if (info <= 0) {
            fail_msg("ZSYSVX: expected INFO > 0 for singular matrix, got %d", info);
            return;
        }
    } else if (info != 0) {
        fail_msg("ZSYSVX info=%d expected=0", info);
        return;
    }
    ...
    if (info == 0) {
        ...
        zpot05(uplo, n, NRHS, ws->A, lda, ws->B, lda,
    ...
    /* TEST 6: Compare RCOND from ZSYSVX with computed value */
    result[5] = dget06(rcond, rcondc);

    /* Check results */
    for (INT i = k1 - 1; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZSYSVX FACT=%s UPLO=%s test %d: result=%e >= thresh=%e",
                     fact, uplo, i + 1, result[i], THRESH);
        }
    }
}

static void test_zdrvsy_case(void** state)
{
    zdrvsy_params_t* p = *state;
    run_zdrvsy_single(p->n, p->imat, p->iuplo, p->ifact);
}

#define MAX_TESTS 3000

static zdrvsy_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < 2; iuplo++) {
                for (INT ifact = 0; ifact < NFACT; ifact++) {
                    ...
                    zdrvsy_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->ifact = ifact;
                    ...
                    g_tests[g_num_tests].test_func = test_zdrvsy_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvsy", g_tests, g_num_tests,
