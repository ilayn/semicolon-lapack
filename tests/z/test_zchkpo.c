/**
 * @file test_zchkpo.c
 * @brief Comprehensive test suite for Hermitian positive definite matrix (ZPO) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkpo.f to C using CMocka.
 * Tests ZPOTRF, ZPOTRI, ZPOTRS, ZPORFS, and ZPOCON.
 *
 * Each (n, uplo, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkpo.f:
 *   TEST 1: Cholesky factorization residual via zpot01
 *   TEST 2: Matrix inverse residual via zpot03
 *   TEST 3: Solution residual via zpot02
 *   TEST 4: Solution accuracy via zget04
 *   TEST 5: Refined solution accuracy via zget04 (after zporfs)
 *   TEST 6-7: Error bounds via zpot05
...
static void run_zchkpo_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    zpotrf(uplo_str, n, ws->AFAC, lda, &info);
    ...
    zpot01(uplo_str, n, ws->A, lda, ws->AINV, lda, ws->RWORK, &result[0]);
    ...
    set_test_context(ctx);
    zlacpy(uplo_str, n, n, ws->AFAC, lda, ws->AINV, lda);
    zpotri(uplo_str, n, ws->AINV, lda, &info);
    assert_int_equal(info, 0);
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        zpotrs(uplo_str, n, nrhs, ws->AFAC, lda, ws->X, lda, &info);
        assert_int_equal(info, 0);
        ...
        zporfs(uplo_str, n, nrhs, ws->A, lda, ws->AFAC, lda,
...
test8:
    ...
    zpocon(uplo_str, n, ws->AFAC, lda, anorm, &rcond, ws->WORK,
           ws->RWORK, &info);
    assert_int_equal(info, 0);
