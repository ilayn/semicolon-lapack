/**
 * @file test_zdrvhe_rk.c
 * @brief ZDRVHE_RK tests the driver routine ZHESV_RK.
 *
 * Port of LAPACK TESTING/LIN/zdrvhe_rk.f to C with CMocka parameterization.
 */
...
typedef struct {
    ...
    INT* IWORK;
    INT lwork;
} zdrvhe_rk_workspace_t;

static zdrvhe_rk_workspace_t* g_workspace = NULL;

static int group_setup(void** state)
{
    (void)state;
    g_workspace = malloc(sizeof(zdrvhe_rk_workspace_t));
    ...
    g_workspace->E = calloc(nmax, sizeof(c128));
...
static void run_zdrvhe_rk_single(INT n, INT imat, INT iuplo, INT ifact)
{
    static const char* UPLOS[] = {"U", "L"};
    static const char* FACTS[] = {"F", "N"};

    zdrvhe_rk_workspace_t* ws = g_workspace;
    ...
    if (zerot) {
        ...
        f64 anrm = zlanhe("1", uplo, n, ws->A, lda, ws->RWORK);

        zlacpy(uplo, n, n, ws->A, lda, ws->AFAC, lda);
        zhetrf_rk(uplo, n, ws->AFAC, lda, ws->E, ws->IWORK, ws->WORK,
                  ws->lwork, &info);

        zlacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        INT lwork_tri = (n + 1 + 1) * (1 + 3);
        zhetri_3(uplo, n, ws->AINV, lda, ws->E, ws->IWORK, ws->WORK,
                 lwork_tri, &info);
        f64 ainvnm = zlanhe("1", uplo, n, ws->AINV, lda, ws->RWORK);
    ...
    char xtype = 'N';
    zlarhs("ZHE", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (ifact == 1) {
        ...
        zhesv_rk(uplo, n, NRHS, ws->AFAC, lda, ws->E, ws->IWORK,
                 ws->X, lda, ws->WORK, ws->lwork, &info);

        if (zerot) {
            if (info <= 0) {
                fail_msg("ZHESV_RK: expected INFO > 0 for singular matrix, got %d", info);
                return;
            }
        } else if (info != 0) {
            fail_msg("ZHESV_RK info=%d expected=0", info);
            return;
        } else {
            /* TEST 1: Reconstruct matrix from factors */
            zhet01_3(uplo, n, ws->A, lda, ws->AFAC, lda, ws->E,
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("ZHESV_RK UPLO=%s test %d failed: result=%e >= thresh=%e",
...
static void test_zdrvhe_rk_case(void** state)
{
    zdrvhe_rk_params_t* p = *state;
    run_zdrvhe_rk_single(p->n, p->imat, p->iuplo, p->ifact);
}

#define MAX_TESTS 3000

static zdrvhe_rk_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < 2; iuplo++) {
                for (INT ifact = 0; ifact < NFACT; ifact++) {
                    ...
                    zdrvhe_rk_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->ifact = ifact;
                    ...
                    g_tests[g_num_tests].test_func = test_zdrvhe_rk_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvhe_rk", g_tests, g_num_tests,
