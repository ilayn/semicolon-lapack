/**
 * @file test_zdrvhe_rook.c
 * @brief ZDRVHE_ROOK tests the driver routine ZHESV_ROOK.
 *
...
static void run_zdrvhe_rook_single(INT n, INT imat, INT iuplo)
{
    ...
    if (zerot) {
        ...
        zhetrf_rook(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK,
                    ws->lwork, &info);

        zlacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        zhetri_rook(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, &info);
    ...
    zhesv_rook(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    /* Check error code from ZHESV_ROOK */
    if (info != k) {
        fail_msg("ZHESV_ROOK UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    zhet01_rook(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZHESV_ROOK UPLO=%s test %d failed: result=%e >= thresh=%e",
...
{
    build_test_array();
    (void)_cmocka_run_group_tests("zdrvhe_rook", g_tests, g_num_tests,
