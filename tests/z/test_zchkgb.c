/**
 * @file test_zchkgb.c
 * @brief Comprehensive test suite for complex general band matrix (ZGB) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkgb.f to C using CMocka.
 * Tests ZGBTRF, ZGBTRS, ZGBRFS, and ZGBCON.
 *
 * Each (m, n, kl, ku, imat) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchkgb.f:
 *   TEST 1: LU factorization residual via zgbt01
 *   TEST 2: Solution residual via zgbt02
 *   TEST 3: Solution accuracy via zget04
 *   TEST 4: Refined solution accuracy via zget04 (after zgbrfs)
 *   TEST 5-6: Error bounds via zgbt05
...
static int group_setup(void** state)
{
    ...
    g_workspace->A = malloc(LA * sizeof(c128));
    g_workspace->AFAC = malloc(LAFAC * sizeof(c128));
    ...
    g_workspace->WORK = malloc(3 * NMAX * NMAX * sizeof(c128));
...
static void run_zchkgb_single(INT m, INT n, INT kl, INT ku, INT imat, INT inb)
{
    ...
    zlatb4("ZGB", imat, m, n, &type, &kl_gen, &ku_gen, &anorm_gen, &mode, &cndnum, &dist);

    zlatms(m, n, &dist, &type, ws->D, mode, cndnum, anorm_gen,
    ...
    zgbtrf(m, n, kl, ku, ws->AFAC, ldafac, ws->IPIV, &info);
     ...
     */
    zgbt01(m, n, kl, ku, ws->A, lda, ws->AFAC, ldafac,
    ...
    anormo = zlangb("O", n, kl, ku, ws->A, lda, ws->RWORK);
    anormi = zlangb("I", n, kl, ku, ws->A, lda, ws->RWORK);

    if (info == 0) {
        INT ldb_inv = (n > 1) ? n : 1;
        zlaset("F", n, n, CMPLX(ZERO, ZERO), CMPLX(ONE, ZERO), ws->WORK, ldb_inv);

        zgbtrs("N", n, kl, ku, n, ws->AFAC, ldafac, ws->IPIV, ws->WORK, ldb_inv, &info);

        f64 ainvnm = zlange("O", n, n, ws->WORK, ldb_inv, ws->RWORK);
        ...
        ainvnm = zlange("I", n, n, ws->WORK, ldb_inv, ws->RWORK);
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        for (INT itran = 0; itran < (INT)NTRAN; itran++) {
            ...
            zlarhs("ZGB", &xtype, " ", trans, n, n, kl, ku, nrhs,
                   ws->A, lda, ws->XACT, ldb, ws->B, ldb, &info, rng_state);
            xtype = 'C';

            zlacpy("F", n, nrhs, ws->B, ldb, ws->X, ldb);

            /*
             * TEST 2: Solve and compute residual.
             */
            zgbtrs(trans, n, kl, ku, nrhs, ws->AFAC, ldafac, ws->IPIV,
                   ws->X, ldb, &info);
            assert_int_equal(info, 0);

            zlacpy("F", n, nrhs, ws->B, ldb, ws->WORK, ldb);
            zgbt02(trans, m, n, kl, ku, nrhs, ws->A, lda, ws->X, ldb,
             ...
             */
            zgbrfs(trans, n, kl, ku, nrhs, ws->A, lda, ws->AFAC, ldafac,
                   ws->IPIV, ws->B, ldb, ws->X, ldb,
                   ws->FERR, ws->BERR, ws->WORK, ws->RWORK, &info);
            assert_int_equal(info, 0);

            zget04(n, nrhs, ws->X, ldb, ws->XACT, ldb, rcondc, &result[3]);

            f64 reslts[2];
            zgbt05(trans, n, kl, ku, nrhs, ws->A, lda, ws->B, ldb,
...
test7:
    ...
    for (INT itran = 0; itran < 2; itran++) {
        ...
        zgbcon(norm, n, kl, ku, ws->AFAC, ldafac, ws->IPIV,
