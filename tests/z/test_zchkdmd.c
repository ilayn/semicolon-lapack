/**
 * @file test_zchkdmd.c
 * @brief Test driver for the Dynamic Mode Decomposition routines ZGEDMD and
 *        ZGEDMDQ - port of LAPACK TESTING/EIG/zchkdmd.f90.
 */
...
typedef struct {
    c128 *ZA, *ZAC, *ZF, *ZF0, *ZF1, *ZX, *ZX0, *ZY, *ZY0, *ZY1, *ZZ, *ZZ1;
    c128 *ZS, *ZAU, *ZW, *ZEIGS, *ZEIGSA, *ZDA, *ZDL, *ZDR;
...
static int group_setup(void** state)
{
    ...
    g_ws->ZA  = xmalloc(m2 * sizeof(c128));
    g_ws->ZAC = xmalloc(m2 * sizeof(c128));
    g_ws->ZF  = xmalloc(m2 * sizeof(c128));
    g_ws->ZF0 = xmalloc(m2 * sizeof(c128));
    g_ws->ZF1 = xmalloc(m2 * sizeof(c128));
    g_ws->ZX  = xmalloc(m2 * sizeof(c128));
    g_ws->ZX0 = xmalloc(m2 * sizeof(c128));
    g_ws->ZY  = xmalloc(m2 * sizeof(c128));
    g_ws->ZY0 = xmalloc(m2 * sizeof(c128));
    g_ws->ZY1 = xmalloc(m2 * sizeof(c128));
    g_ws->ZZ  = xmalloc(m2 * sizeof(c128));
    g_ws->ZZ1 = xmalloc(m2 * sizeof(c128));
    g_ws->ZS  = xmalloc(m2 * sizeof(c128));
    g_ws->ZAU = xmalloc(m2 * sizeof(c128));
    g_ws->ZW  = xmalloc(m2 * sizeof(c128));
    g_ws->ZEIGS  = xmalloc(MAXM * sizeof(c128));
    g_ws->ZEIGSA = xmalloc(MAXM * sizeof(c128));
    g_ws->ZDA = xmalloc(MAXM * sizeof(c128));
    g_ws->ZDL = xmalloc(MAXM * sizeof(c128));
    g_ws->ZDR = xmalloc(MAXM * sizeof(c128));
    g_ws->RES     = xmalloc(MAXM * sizeof(f64));
    g_ws->RES1    = xmalloc(MAXM * sizeof(f64));
    g_ws->RESEX   = xmalloc(MAXM * sizeof(f64));
    g_ws->SINGVX  = xmalloc(MAXM * sizeof(f64));
    g_ws->SINGVQX = xmalloc(MAXM * sizeof(f64));
    g_ws->zwork = xmalloc(WORKSZ * sizeof(c128));
    g_ws->rwork = xmalloc(WORKSZ * sizeof(f64));
...
static void run_case(INT M, INT N)
{
    ...
    const c128 CZERO = CMPLX(0.0, 0.0), CONE = CMPLX(1.0, 0.0);
    const INT lda = M, ldf = M, ldx = M, ldy = M, ldw = N, ldz = M, ldau = M, lds = N;
    const f64 eps = dlamch("P");
    const f64 tol = M * eps;
    INT info, i;

    f64 e_zxw = ZERO, e_au = ZERO, e_rez = ZERO, e_fqr = ZERO, e_rezq = ZERO;

    rng_seed(w->state, 0x9E3779B97F4A7C15ULL);
    INT iseed[4] = {4, 3, 2, 1};

    for (INT k_traj = 1; k_traj <= 2; k_traj++) {
        const f64 cond = 1.0e4, condl = 1.0e1, condr = 1.0e1;
        const c128 zmax = CMPLX(1.0e1, 1.0e1);

        for (INT mode = 1; mode <= 6; mode++) {
            zlatmr(M, M, "N", "N", w->ZDA, mode, cond, zmax, "F", "N", w->ZDL, 6,
            ...
            memcpy(w->ZAC, w->ZA, (size_t)lda * M * sizeof(c128));
            c128 zdum[4];
            zgeev("N", "N", M, w->ZAC, lda, w->ZEIGSA, zdum, 2, zdum, 2,
                  w->zwork, WORKSZ, w->rwork, &info);
            f64 tmp = cabs(w->ZEIGSA[cblas_izamax(M, w->ZEIGSA, 1)]);
            zlascl("G", 0, 0, tmp, ONE, M, M, w->ZA, lda, &info);
            zlascl("G", 0, 0, tmp, ONE, M, 1, w->ZEIGSA, M, &info);
            const f64 anorm = zlange("F", M, M, w->ZA, lda, NULL);

            if (k_traj == 2) {
                zlarnv(2, iseed, M, &w->ZF[0]);
                for (i = 0; i < N / 2; i++)
                    cblas_zgemv(CblasColMajor, CblasNoTrans, M, M, &CONE, w->ZA, lda,
                                &w->ZF[i * ldf], 1, &CZERO, &w->ZF[(i + 1) * ldf], 1);
                for (INT jc = 0; jc < N / 2; jc++) {
                    memcpy(&w->ZX0[jc * ldx], &w->ZF[jc * ldf], (size_t)M * sizeof(c128));
                    memcpy(&w->ZY0[jc * ldy], &w->ZF[(jc + 1) * ldf], (size_t)M * sizeof(c128));
                }
                zlarnv(2, iseed, M, &w->ZF[0]);
                for (i = 0; i < N - N / 2; i++)
                    cblas_zgemv(CblasColMajor, CblasNoTrans, M, M, &CONE, w->ZA, lda,
                ...
                for (INT jc = 0; jc < N - N / 2; jc++) {
                    memcpy(&w->ZX0[(N / 2 + jc) * ldx], &w->ZF[jc * ldf], (size_t)M * sizeof(c128));
                    memcpy(&w->ZY0[(N / 2 + jc) * ldy], &w->ZF[(jc + 1) * ldf], (size_t)M * sizeof(c128));
                }
            } else {
                zlarnv(2, iseed, M, &w->ZF[0]);
                for (i = 0; i < N; i++)
                    cblas_zgemv(CblasColMajor, CblasNoTrans, M, M, &CONE, w->ZA, M,
                                &w->ZF[i * ldf], 1, &CZERO, &w->ZF[(i + 1) * ldf], 1);
                memcpy(w->ZF0, w->ZF, (size_t)ldf * (N + 1) * sizeof(c128));
                for (INT jc = 0; jc < N; jc++) {
                    memcpy(&w->ZX0[jc * ldx], &w->ZF0[jc * ldf], (size_t)M * sizeof(c128));
                    memcpy(&w->ZY0[jc * ldy], &w->ZF0[(jc + 1) * ldf], (size_t)M * sizeof(c128));
                }
            }
                        ...
                        for (INT inrnk = -1; inrnk >= -2; inrnk--) {
                            ...
                            for (INT iwhtsvd = 1; iwhtsvd <= 3; iwhtsvd++) {
                                ...
                                for (INT lwminopt = 1; lwminopt <= 2; lwminopt++) {
                                    ...
                                    for (INT jc = 0; jc < N; jc++) {
                                        memcpy(&w->ZX[jc * ldx], &w->ZX0[jc * ldx], (size_t)M * sizeof(c128));
                                        memcpy(&w->ZY[jc * ldy], &w->ZY0[jc * ldy], (size_t)M * sizeof(c128));
                                    }

                                    /* ZGEDMD: workspace query */
                                    zgedmd(scale, jobz, resids, jobref, whtsvd, M, N,
                                           w->ZX, ldx, w->ZY, ldy, nrnk, tol, &k,
                                           w->ZEIGS, w->ZZ, ldz, w->RES, w->ZAU, ldau,
                                           w->ZW, ldw, w->ZS, lds, zdummy, -1, wdummy, -1,
                                           idummy, -1, &info);
                                    INT lzwork = (INT)creal(zdummy[lwminopt - 1]);
                                    ...
                                    if (jobz[0] == 'V') {
                                        ...
                                        for (i = 0; i < k; i++) {
                                            c128 neg = -CONE;
                                            cblas_zaxpy(M, &neg, &w->ZZ[i * ldz], 1, &w->ZZ1[i * ldz], 1);
                                            f64 nn = cblas_dznrm2(M, &w->ZZ1[i * ldz], 1);
                                            if (nn > tmp1) tmp1 = nn;
                                        }
                                    ...
                                    if (jobref[0] == 'R') {
                                        ...
                                        for (i = 0; i < k; i++) {
                                            c128 neg = -CONE;
                                            cblas_zaxpy(M, &neg, &w->ZAU[i * ldau], 1, &w->ZZ1[i * ldz], 1);
                                            f64 v = cblas_dznrm2(M, &w->ZZ1[i * ldz], 1) *
                                        ...
                                        for (i = 0; i < k; i++) {
                                            c128 neg = -w->ZEIGS[i];
                                            cblas_zaxpy(M, &neg, &w->ZAU[i * ldau], 1, &w->ZY1[i * ldy], 1);
                                            w->RESEX[i] = cblas_dznrm2(M, &w->ZY1[i * ldy], 1) /
                                                          cblas_dznrm2(M, &w->ZAU[i * ldau], 1);
                                        }
                                    }
                                    ...
                                    if (resids[0] == 'R') {
                                        ...
                                        for (i = 0; i < k; i++) {
                                            c128 neg = -w->ZEIGS[i];
                                            cblas_zaxpy(M, &neg, &w->ZZ[i * ldz], 1, &w->ZY1[i * ldy], 1);
                                            w->RES1[i] = cblas_dznrm2(M, &w->ZY1[i * ldy], 1);
                                        }
                                        f64 tmp1 = ZERO;
                                        for (i = 0; i < k; i++) {
                                    ...
                                    /*==========  ZGEDMDQ  ==========*/
                                    if (k_traj == 1) {
                                        memcpy(w->ZF, w->ZF0, (size_t)ldf * (N + 1) * sizeof(c128));

                                        zgedmdq(scale, jobz, resids, "Q", "R", jobref,
                                        ...
                                        lzwork = (INT)creal(zdummy[lwminopt - 1]);
                                        lrwork = (INT)wdummy[0];
                                        liwork = idummy[0];
                                        assert_true(lzwork <= WORKSZ && lrwork <= WORKSZ && liwork <= IWORKSZ);
                                        ...
                                        memcpy(w->ZF1, w->ZF0, (size_t)ldf * (N + 1) * sizeof(c128));
                                        const INT mn = (M < N + 1) ? M : N + 1;
                                        c128 negone = -CONE;
                                        cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                                    M, N + 1, mn, &negone, w->ZF, ldf, w->ZY, ldy,
                                                    &CONE, w->ZF1, ldf);
                                        f64 fqr = zlange("F", M, N + 1, w->ZF1, ldf, NULL) /
                                                  zlange("F", M, N + 1, w->ZF0, ldf, NULL);
...
static void test_zchkdmd_case(void** state)
{
...
int main(void)
{
    ...
    for (INT i = 0; i < NDIM; i++) {
        tests[i].name = "zchkdmd";
        tests[i].test_func = test_zchkdmd_case;
        tests[i].setup_func = NULL;
        tests[i].teardown_func = NULL;
        tests[i].initial_state = (void*)(intptr_t)i;
    }
    (void)_cmocka_run_group_tests("zchkdmd", tests, NDIM, group_setup, group_teardown);
    return 0;
}
