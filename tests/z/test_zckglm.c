 ...
 *
 * Tests ZGGGLM - the generalized linear model solver:
...
static int group_setup(void** state)
{
    ...
    g_ws->U     = malloc(NMAX * sizeof(c128));
...
static void test_zckglm_case(void** state)
{
    ...
    zlatms(n, p, distb_str, type_str, g_ws->rwork, modeb, cndnmb,
           bnorm, klb, kub, "N", g_ws->B, ldb, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    zlarnv_rng(2, n, g_ws->D, rng_state);

    f64 resid;
    zglmts(n, m, p, g_ws->A, g_ws->AF, lda, g_ws->B, g_ws->BF, ldb,
           g_ws->D, g_ws->DF, g_ws->X, g_ws->U,
...
int main(void)
{
    ...
    (void)_cmocka_run_group_tests("zckglm", g_tests, g_num_tests,
