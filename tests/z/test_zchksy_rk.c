/**
 * @file test_zchksy_rk.c
 * @brief Comprehensive test suite for complex symmetric indefinite RK (ZSK) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchksy_rk.f to C using CMocka.
 * Tests ZSYTRF_RK, ZSYTRI_3, ZSYTRS_3, and ZSYCON_3.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksy_rk.f:
 *   TEST 1: LDL^T factorization residual via zsyt01_3
 *   TEST 2: Matrix inverse residual via zsyt03
 *   TEST 3: Largest element in U or L (growth factor bound)
 *   TEST 4: Largest 2-Norm of 2-by-2 diagonal blocks (condition bound)
 *   TEST 5: Solution residual via zsyt02 (using zsytrs_3)
 *   TEST 6: Solution accuracy via zget04
...
typedef struct {
    ...
    c128* E;      /* Superdiagonal/subdiagonal of D (RK-specific, complex) */
    INT* IPIV;    /* Pivot indices */
    INT* IWORK;   /* Integer workspace */
} zchksy_rk_workspace_t;

static zchksy_rk_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchksy_rk_workspace_t));
    ...
    g_workspace->E = malloc(NMAX * sizeof(c128));
...
/**
 * Run the full zchksy_rk test battery for a single (n, uplo, imat, inb) combination.
 *
 * Following LAPACK's zchksy_rk.f:
 ...
 */
static void run_zchksy_rk_single(INT n, INT iuplo, INT imat, INT inb)
{
    const f64 ZERO = 0.0;
    const f64 ONE = 1.0;
    const f64 EIGHT = 8.0;
    const f64 SEVTEN = 17.0;
    zchksy_rk_workspace_t* ws = g_workspace;
    ...
    zlatb4("ZSY", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    zsytrf_rk(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    zsyt01_3(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->E, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        zsytri_3(uplo_str, n, ws->AINV, lda, ws->E, ws->IPIV,
                 ws->WORK, lwork_tri3, &info);
        if (info == 0) {
            zsyt03(uplo_str, n, ws->A, lda, ws->AINV, lda, ws->WORK, lda,
     ...
     */
    {
        ...
        if (iuplo == 0) {
            ...
            while (k >= 1) {
                if (ws->IPIV[k] >= 0) {
                    dtemp = zlange("M", k, 1, &ws->AFAC[k * lda], lda, ws->RWORK);
                } else {
                    dtemp = zlange("M", k - 1, 2, &ws->AFAC[(k - 1) * lda], lda, ws->RWORK);
                    k--;
                }
            ...
            while (k < n - 1) {
                if (ws->IPIV[k] >= 0) {
                    dtemp = zlange("M", n - k - 1, 1, &ws->AFAC[k * lda + k + 1], lda, ws->RWORK);
                } else {
                    dtemp = zlange("M", n - k - 2, 2, &ws->AFAC[k * lda + k + 2], lda, ws->RWORK);
                    k++;
                }
                dtemp = dtemp - const_val + THRESH;
                if (dtemp > result[2])
...
test7:
    ...
    set_test_context(ctx);
    anorm = zlansy("1", uplo_str, n, ws->A, lda, ws->RWORK);
    zsycon_3(uplo_str, n, ws->AFAC, lda, ws->E, ws->IPIV, anorm, &rcond,
             ws->WORK, &info);
    assert_int_equal(info, 0);
...
/**
 * CMocka test function - dispatches to run_zchksy_rk_single based on prestate.
 */
static void test_zchksy_rk_case(void** state)
{
    zchksy_rk_params_t* params = *state;
    run_zchksy_rk_single(params->n, params->iuplo, params->imat, params->inb);
}
...
static zchksy_rk_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    /* Store parameters */
                    zchksy_rk_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->inb = inb;
                    snprintf(p->name, sizeof(p->name), "zchksy_rk_n%d_%c_type%d_nb%d_%d",
                             n, UPLOS[iuplo], imat, nb, inb);

                    /* Create CMocka test entry */
                    g_tests[g_num_tests].name = p->name;
                    g_tests[g_num_tests].test_func = test_zchksy_rk_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchksy_rk", g_tests, g_num_tests,
