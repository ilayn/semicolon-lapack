/**
 * @file test_zdrvpp.c
 * @brief ZDRVPP tests the driver routines ZPPSV and ZPPSVX.
 *
 * Port of LAPACK TESTING/LIN/zdrvpp.f to C with CMocka parameterization.
 */
...
static zdrvpp_workspace_t* g_workspace = NULL;

static int group_setup(void** state)
{
    (void)state;
    g_workspace = malloc(sizeof(zdrvpp_workspace_t));
    ...
    g_workspace->AFAC = calloc(NPP_MAX, sizeof(c128));
    g_workspace->ASAV = calloc(NPP_MAX, sizeof(c128));
...
static void run_zdrvpp_single(INT n, INT imat, INT iuplo, INT ifact, INT iequed)
{
    static const char* UPLOS[] = {"U", "L"};
    static const char* FACTS[] = {"F", "N", "E"};
    static const char* EQUEDS[] = {"N", "Y"};

    zdrvpp_workspace_t* ws = g_workspace;
    ...
    INT kl, ku, mode;
    f64 anorm, cndnum;
    zlatb4("ZPP", imat, n, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    cblas_zcopy(npp, ws->A, 1, ws->ASAV, 1);
    ...
    if (zerot) {
        ...
        cblas_zcopy(npp, ws->ASAV, 1, ws->AFAC, 1);

        if (equil || iequed > 0) {
            zppequ(uplo, n, ws->AFAC, ws->S, &scond, &amax, &info);
            if (info == 0 && n > 0) {
                if (iequed > 0) {
                    scond = 0.0;
                }
                zlaqhp(uplo, n, ws->AFAC, ws->S, scond, amax, &equed);
            }
        }
        ...
        f64 anrm = zlanhp("1", uplo, n, ws->AFAC, ws->RWORK);

        zpptrf(uplo, n, ws->AFAC, &info);

        cblas_zcopy(npp, ws->AFAC, 1, ws->A, 1);
        zpptri(uplo, n, ws->A, &info);

        f64 ainvnm = zlanhp("1", uplo, n, ws->A, ws->RWORK);
    ...
    cblas_zcopy(npp, ws->ASAV, 1, ws->A, 1);

    /* Form exact solution and set right hand side */
    rng_seed(rng_state, 1988 + n * 1000 + imat * 100 + iuplo * 10);
    char xtype = 'N';
    zlarhs("ZPP", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (nofact) {
        ...
        zppsv(uplo, n, NRHS, ws->AFAC, ws->X, lda, &info);

        if (info != izero) {
            fail_msg("ZPPSV info=%d expected=%d", info, izero);
            return;
        } else if (info != 0) {
            return;
        }

        zppt01(uplo, n, ws->A, ws->AFAC, ws->RWORK, &result[0]);

        zlacpy("Full", n, NRHS, ws->B, lda, ws->WORK, lda);
        zppt02(uplo, n, NRHS, ws->A, ws->X, lda,
        ...
        for (INT k = 0; k < nt; k++) {
            if (result[k] >= THRESH) {
                fail_msg("ZPPSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    /* --- Test ZPPSVX --- */
    if (!prefac && npp > 0) {
        zlaset("Full", npp, 1, CZERO, CZERO, ws->AFAC, npp);
    }
    zlaset("Full", n, NRHS, CZERO, CZERO, ws->X, lda);

    char equed_inout = equed;
    if (iequed > 0 && n > 0) {
        zlaqhp(uplo, n, ws->A, ws->S, scond, amax, &equed_inout);
    }
    ...
    f64 rcond;
    zppsvx(fact, uplo, n, NRHS, ws->A, ws->AFAC, &equed_inout, ws->S,
           ws->B, lda, ws->X, lda, &rcond,
           ws->RWORK, &ws->RWORK[NRHS], ws->WORK,
           &ws->RWORK[2 * NRHS], &info);

    if (info != izero) {
        fail_msg("ZPPSVX info=%d expected=%d", info, izero);
        return;
    }
    ...
    if (info == 0) {
        ...
        zppt02(uplo, n, NRHS, ws->ASAV, ws->X, lda,
        ...
        zppt05(uplo, n, NRHS, ws->ASAV, ws->B, lda,
    ...
    if (info == 0) {
        for (INT k = k1 - 1; k < NTESTS; k++) {
            if (result[k] >= THRESH) {
                fail_msg("ZPPSVX FACT=%s UPLO=%s EQUED=%c test %d: result=%e >= thresh=%e",
                         fact, uplo, equed_inout, k + 1, result[k], THRESH);
            }
        }
    } else {
        if (!prefac && result[0] >= THRESH) {
            fail_msg("ZPPSVX FACT=%s UPLO=%s EQUED=%c test 1: result=%e >= thresh=%e",
                     fact, uplo, equed_inout, result[0], THRESH);
        }
        if (result[5] >= THRESH) {
            fail_msg("ZPPSVX FACT=%s UPLO=%s EQUED=%c test 6: result=%e >= thresh=%e",
                     fact, uplo, equed_inout, result[5], THRESH);
        }
    }
}

static void test_zdrvpp_case(void** state)
{
    zdrvpp_params_t* p = *state;
    run_zdrvpp_single(p->n, p->imat, p->iuplo, p->ifact, p->iequed);
}

#define MAX_TESTS 3000

static zdrvpp_params_t g_params[MAX_TESTS];
            ...
            for (INT iuplo = 0; iuplo < 2; iuplo++) {
                for (INT iequed = 0; iequed < 2; iequed++) {
                    ...
                    for (INT ifact = 0; ifact < nfact; ifact++) {
                        ...
                        zdrvpp_params_t* p = &g_params[g_num_tests];
                        p->n = n;
                        p->imat = imat;
                        p->iuplo = iuplo;
                        p->ifact = ifact;
                        ...
                        g_tests[g_num_tests].test_func = test_zdrvpp_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvpp", g_tests, g_num_tests,
