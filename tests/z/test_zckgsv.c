/**
 * @file test_zckgsv.c
 * @brief GSV test driver - port of LAPACK TESTING/EIG/zckgsv.f
 *
 * Tests ZGGSVD3 - the GSVD for M-by-N matrix A and P-by-N matrix B:
...
static zckgsv_workspace_t* g_ws = NULL;

static int group_setup(void** state)
{
    (void)state;

    g_ws = malloc(sizeof(zckgsv_workspace_t));
    if (!g_ws) return -1;

    INT lwork = NMAX * NMAX;

    g_ws->A     = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->AF    = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->B     = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->BF    = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->U     = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->V     = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->Q     = malloc(NMAX * NMAX * sizeof(c128));
    g_ws->R     = malloc(NMAX * NMAX * sizeof(c128));
...
static void test_zckgsv_case(void** state)
{
    zckgsv_params_t* params = *state;
    ...
    dlatb9("GSV", imat, m, p, n, &type, &kla, &kua, &klb, &kub,
    ...
    zgsvts3(m, p, n, g_ws->A, g_ws->AF, lda, g_ws->B, g_ws->BF, ldb,
    ...
    for (INT i = 0; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fprintf(stderr, "  GSV: M=%d P=%d N=%d type=%d test=%d ratio=%g\n",
...
static zckgsv_params_t g_params[MAX_TESTS];
...
static void build_test_array(void)
{
    ...
    for (INT im = 0; im < NM; im++) {
        for (INT imat = 1; imat <= NTYPES; imat++) {
            zckgsv_params_t* par = &g_params[g_num_tests];
            par->im = im;
            par->imat = imat;
            ...
            g_tests[g_num_tests].test_func = test_zckgsv_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zckgsv", g_tests, g_num_tests,
