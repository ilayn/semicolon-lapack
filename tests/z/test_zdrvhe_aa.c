/**
 * @file test_zdrvhe_aa.c
 * @brief ZDRVHE_AA tests the driver routine ZHESV_AA.
 *
 * Port of LAPACK TESTING/LIN/zdrvhe_aa.f to C with CMocka parameterization.
 */
...
typedef struct {
    ...
    INT* IWORK;
    INT lwork;
} zdrvhe_aa_workspace_t;

static zdrvhe_aa_workspace_t* g_workspace = NULL;

static int group_setup(void** state)
{
    (void)state;
    g_workspace = malloc(sizeof(zdrvhe_aa_workspace_t));
...
static void run_zdrvhe_aa_single(INT n, INT imat, INT iuplo)
{
    static const char* UPLOS[] = {"U", "L"};

    zdrvhe_aa_workspace_t* ws = g_workspace;
    ...
    zhesv_aa(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    if (info != k) {
        fail_msg("ZHESV_AA UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    zhet01_aa(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZHESV_AA UPLO=%s test %d failed: result=%e >= thresh=%e",
                     uplo, i + 1, result[i], THRESH);
        }
    }
}

static void test_zdrvhe_aa_case(void** state)
{
    zdrvhe_aa_params_t* p = *state;
    run_zdrvhe_aa_single(p->n, p->imat, p->iuplo);
}

#define MAX_TESTS 1500

static zdrvhe_aa_params_t g_params[MAX_TESTS];
    ...
    for (INT in = 0; in < (INT)NN; in++) {
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < 2; iuplo++) {
                zdrvhe_aa_params_t* p = &g_params[g_num_tests];
                p->n = n;
                p->imat = imat;
                p->iuplo = iuplo;
                ...
                g_tests[g_num_tests].test_func = test_zdrvhe_aa_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvhe_aa", g_tests, g_num_tests,
