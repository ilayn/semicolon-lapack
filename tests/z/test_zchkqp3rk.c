/**
 * @file test_zchkqp3rk.c
 * @brief Comprehensive test suite for ZGEQP3RK (QR with column pivoting, rank-revealing).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchkqp3rk.f to C using CMocka.
 * Tests ZGEQP3RK for various matrix types and block sizes.
 *
 ...
 */
typedef struct {
    c128* A;       /* Working matrix (NMAX * (NMAX + NSMAX)) - holds A and appended B */
    c128* COPYA;   /* Copy of original A (NMAX * NMAX) */
    c128* B;       /* RHS matrix (NMAX * NSMAX) */
    c128* COPYB;   /* Copy of original B (NMAX * NSMAX) */
    f64* S;        /* Singular values (NMAX) */
    c128* TAU;     /* Scalar factors of elementary reflectors (NMAX) */
    c128* WORK;    /* General workspace */
    f64* RWORK;    /* Real workspace (2*NMAX for zgeqp3rk + extra for zqrt12) */
 ...
 */
static INT generate_matrix(INT m, INT n, INT imat, c128* COPYA, INT lda,
                           f64* S, c128* WORK, uint64_t rng_state[static 4])
{
    const f64 ZERO = 0.0;
    INT minmn = (m < n) ? m : n;
    INT info;
    ...
    if (imat == 1) {
        ...
        zlaset("F", m, n, CMPLX(0.0, 0.0), CMPLX(0.0, 0.0), COPYA, lda);
    ...
    if ((imat >= 2 && imat <= 4) || (imat >= 14 && imat <= 19)) {
        /* Matrices 2-4, 14-19: Generate via zlatb4 + zlatms */
        zlatb4("ZQK", imat, m, n, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
    ...
    if (minmn >= 2 && imat >= 5 && imat <= 13) {
        ...
        zlaset("F", m, nb_zero, CMPLX(0.0, 0.0), CMPLX(0.0, 0.0), COPYA, lda);

        /* Generate an M-by-NB_GEN matrix in COPYA starting at column NB_ZERO */
        zlatb4("ZQK", imat, m, nb_gen, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);

        char dist_str[2] = {dist, '\0'};
        char type_str[2] = {type, '\0'};
        INT ind_offset_gen = nb_zero * lda;
        zlatms(m, nb_gen, dist_str,
               type_str, S, mode, cndnum, anorm,
        ...
        if (imat == 6 || imat == 7 || imat == 8 || imat == 10 || imat == 11) {
            ...
            for (INT j = 0; j < jb_zero - 1; j++) {
                cblas_zswap(m, &COPYA[(nb_zero + j) * lda], 1,
                            &COPYA[j * lda], 1);
            }
            ...
            for (INT j = 0; j < nb_gen; j++) {
                ...
                cblas_zswap(m, &COPYA[ind_out], 1, &COPYA[ind_in], 1);
            }
        }
...
static void run_zchkqp3rk_single(INT m, INT n, INT nrhs, INT imat, INT inb, INT kmax)
{
    ...
    /* Generate RHS matrix B (COPYB) with zlatb4/zlatms for IMAT=14 parameters */
    {
        char type, dist;
        INT kl, ku, mode;
        f64 anorm, cndnum;
        zlatb4("ZQK", 14, m, nrhs, &type, &kl, &ku, &anorm, &mode, &cndnum, &dist);
        char dist_str[2] = {dist, '\0'};
        char type_str[2] = {type, '\0'};
        zlatms(m, nrhs, dist_str,
    ...
    zlacpy("A", m, nrhs, ws->COPYB, lda, &ws->A[lda * n], lda);
    zlacpy("A", m, nrhs, ws->COPYB, lda, ws->B, lda);
    for (INT i = 0; i < n; i++) {
        ws->IWORK[n + i] = ws->IWORK[i];
    }

    /* Workspace size for ZGEQP3RK */
    ...
    zgeqp3rk(m, n, nrhs, kmax, abstol, reltol, ws->A, lda,
             &kfact, &maxc2nrmk, &relmaxc2nrmk, &ws->IWORK[n], ws->TAU,
             ws->WORK, lw, ws->RWORK, &ws->IWORK[2 * n], &info);

    if (info < 0) {
        snprintf(ctx, sizeof(ctx), "m=%d n=%d nrhs=%d kmax=%d type=%d nb=%d: ZGEQP3RK failed (info=%d)",
    ...
    if (kfact == minmn) {
        result[0] = zqrt12(m, n, ws->A, lda, ws->S, ws->WORK, ws->lwork, ws->RWORK);
    ...
    /* TEST 2: norm(A*P - Q*R) / (||A|| * eps * max(M,N)) via zqpt01 */
    result[1] = zqpt01(m, n, kfact, ws->COPYA, ws->A, lda, ws->TAU,
    ...
    /* TEST 3: norm(Q'*Q - I) / (eps * M) via zqrt11 */
    result[2] = zqrt11(m, kfact, ws->A, lda, ws->TAU, ws->WORK, ws->lwork);
    ...
    if (ktest4 >= 2) {
        result[3] = ZERO;
        f64 r11 = cabs(ws->A[0]);  /* |R(0,0)| */
        if (r11 > ZERO) {
            for (INT j = 0; j < kfact - 1; j++) {
                /* R(j,j) is at A[j + j*lda], R(j+1,j+1) is at A[(j+1) + (j+1)*lda] */
                f64 rjj = cabs(ws->A[j + j * lda]);
                f64 rjj1 = cabs(ws->A[(j + 1) + (j + 1) * lda]);
                f64 dtemp = (rjj - rjj1) / r11;
                if (dtemp < ZERO) {
                    result[3] = BIGNUM;
                    break;
                }
     ...
     *         Only when MINMN > 0 */
    if (minmn > 0) {
        /* B already contains COPYB; A[lda*n:] contains Q**H * COPYB from zgeqp3rk */
        /* Apply Q**H to B using zunmqr */
        INT lwork_mqr = (nrhs > 1) ? nrhs : 1;
        zunmqr("L", "C", m, nrhs, kfact, ws->A, lda, ws->TAU,
               ws->B, lda, ws->WORK, lwork_mqr, &info);

        if (info == 0) {
            /* Compute B := B - A[lda*n:] (where A[lda*n:] is Q**H * COPYB from zgeqp3rk) */
            const c128 NEGONE = CMPLX(-1.0, 0.0);
            for (INT i = 0; i < nrhs; i++) {
                cblas_zaxpy(m, &NEGONE, &ws->A[lda * n + i * lda], 1,
                            &ws->B[i * lda], 1);
            }

            /* result[4] = ||B|| / (M * eps) */
            f64 rdummy[1];
            f64 bnorm = zlange("1", m, nrhs, ws->B, lda, rdummy);
