/**
 * @file test_zchksy_aa.c
 * @brief Comprehensive test suite for complex symmetric indefinite (Aasen) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchksy_aa.f to C using CMocka.
 * Tests ZSYTRF_AA and ZSYTRS_AA.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksy_aa.f:
 *   TEST 1: LDL^T factorization residual via zsyt01_aa
 *   TEST 2: Solution residual via zsyt02 (using zsytrs_aa)
 *
...
} zchksy_aa_workspace_t;

static zchksy_aa_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchksy_aa_workspace_t));
    if (!g_workspace) return -1;

    /* LWORK = MAX(1, N*NB + N) where max NB=20, N=50 => 50*20+50 = 1050
     * Also need workspace for zsytrs_aa: MAX(1, 3*N-2) = 148
...
/**
 * Run the full zchksy_aa test battery for a single (n, uplo, imat, inb) combination.
 *
 * Following LAPACK's zchksy_aa.f:
 *   - TEST 1 (factorization residual) runs for all NB values
 *   - TEST 2 (solve) runs for all NB values, for each NRHS
 */
static void run_zchksy_aa_single(INT n, INT iuplo, INT imat, INT inb)
{
    const f64 ZERO = 0.0;
    zchksy_aa_workspace_t* ws = g_workspace;
    ...
    zsytrf_aa(uplo_str, n, ws->AFAC, lda, ws->IWORK, ws->AINV,
              lwork_trf, &info);

    /* Adjust the expected value of INFO to account for pivoting.
     * In zchksy_aa.f, this logic is commented out: K = 0 unconditionally */
    INT k_expected = 0;

    /* Check error code from ZSYTRF_AA */
    if (info != k_expected) {
        snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d ZSYTRF_AA info=%d expected=%d",
    ...
    snprintf(ctx, sizeof(ctx), "n=%d uplo=%c imat=%d nb=%d TEST 1 (factorization)",
             n, uplo, imat, nb);
    set_test_context(ctx);
    zsyt01_aa(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 2: Solve and compute residual for A * X = B using zsytrs_aa
         */
        ...
        zsytrs_aa(uplo_str, n, nrhs, ws->AFAC, lda, ws->IWORK,
                  ws->X, lda, ws->WORK, lwork_trs, &info);

        if (info != 0) {
            if (izero < 0) {
                snprintf(ctx, sizeof(ctx),
                         "n=%d uplo=%c imat=%d nrhs=%d ZSYTRS_AA info=%d",
...
/**
 * CMocka test function - dispatches to run_zchksy_aa_single based on prestate.
 */
static void test_zchksy_aa_case(void** state)
{
    zchksy_aa_params_t* params = *state;
    run_zchksy_aa_single(params->n, params->iuplo, params->imat, params->inb);
}
...
static zchksy_aa_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    /* Store parameters */
                    zchksy_aa_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->inb = inb;
                    snprintf(p->name, sizeof(p->name),
                             "zchksy_aa_n%d_%c_type%d_nb%d_%d",
                             n, UPLOS[iuplo], imat, nb, inb);

                    /* Create CMocka test entry */
                    g_tests[g_num_tests].name = p->name;
                    g_tests[g_num_tests].test_func = test_zchksy_aa_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchksy_aa", g_tests, g_num_tests,
