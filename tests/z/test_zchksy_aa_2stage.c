/**
 * @file test_zchksy_aa_2stage.c
 * @brief Comprehensive test suite for complex symmetric indefinite (Aasen 2-stage) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchksy_aa_2stage.f to C using CMocka.
 * Tests ZSYTRF_AA_2STAGE and ZSYTRS_AA_2STAGE.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksy_aa_2stage.f:
 *   TEST 1: Factorization residual (commented out in Fortran source, NT=0)
 *   TEST 2: Solution residual via zsyt02 (using zsytrs_aa_2stage)
 *
...
} zchksy_aa_2stage_workspace_t;

static zchksy_aa_2stage_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchksy_aa_2stage_workspace_t));
...
/**
 * Run the full zchksy_aa_2stage test battery for a single (n, uplo, imat, inb)
 * combination.
 *
 * Following LAPACK's zchksy_aa_2stage.f:
 *   - TEST 1 (factorization residual) is commented out (NT=0)
 *   - TEST 2 (solve) runs for all NB values, for each NRHS, when INFO==0
 */
static void run_zchksy_aa_2stage_single(INT n, INT iuplo, INT imat, INT inb)
{
    const f64 ZERO = 0.0;
    zchksy_aa_2stage_workspace_t* ws = g_workspace;
     ...
     * Use MATPATH = "ZSY" (complex symmetric types) */
    ...
    zsytrf_aa_2stage(uplo_str, n, ws->AFAC, lda,
                     ws->AINV, ltb,
                     ws->IWORK, ws->IWORK + n,
                     ws->WORK, lwork, &info);

    /* Adjust the expected value of INFO to account for pivoting.
     * Unlike zchksy_aa.f where this is commented out, in
     * zchksy_aa_2stage.f the pivot tracing logic IS active. */
    INT k_expected;
    if (izero >= 0) {
    ...
    /* Check error code from ZSYTRF_AA_2STAGE */
    if (info != k_expected) {
        snprintf(ctx, sizeof(ctx),
                 "n=%d uplo=%c imat=%d ZSYTRF_AA_2STAGE info=%d expected=%d",
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 2: Solve and compute residual for A * X = B using zsytrs_aa_2stage
         */
        snprintf(ctx, sizeof(ctx),
        ...
        zsytrs_aa_2stage(uplo_str, n, nrhs, ws->AFAC, lda,
                         ws->AINV, (3 * nb + 1) * n,
                         ws->IWORK, ws->IWORK + n,
                         ws->X, lda, &info);
        ...
        if (info != 0) {
            if (izero < 0) {
                snprintf(ctx, sizeof(ctx),
                         "n=%d uplo=%c imat=%d nrhs=%d ZSYTRS_AA_2STAGE info=%d",
...
/**
 * CMocka test function - dispatches to run_zchksy_aa_2stage_single based on prestate.
 */
static void test_zchksy_aa_2stage_case(void** state)
{
    zchksy_aa_2stage_params_t* params = *state;
    run_zchksy_aa_2stage_single(params->n, params->iuplo, params->imat, params->inb);
}
...
static zchksy_aa_2stage_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    /* Store parameters */
                    zchksy_aa_2stage_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->inb = inb;
                    snprintf(p->name, sizeof(p->name),
                             "zchksy_aa_2stage_n%d_%c_type%d_nb%d_%d",
                             n, UPLOS[iuplo], imat, nb, inb);

                    /* Create CMocka test entry */
                    g_tests[g_num_tests].name = p->name;
                    g_tests[g_num_tests].test_func = test_zchksy_aa_2stage_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchksy_aa_2stage", g_tests, g_num_tests,
