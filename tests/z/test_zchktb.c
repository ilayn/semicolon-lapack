/**
 * @file test_zchktb.c
 * @brief Comprehensive test suite for triangular banded matrix routines (ZTB).
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchktb.f to C using CMocka.
 * Tests ZTBTRS, ZTBRFS, ZTBCON, and ZLATBS.
 *
 ...
 *     TEST 1: ZTBTRS - | b - op(A)*x | / (|op(A)| * |x| * eps)
 *     TEST 2: Compare solution to exact (ZGET04)
 *     TESTS 3-5: ZTBRFS iterative refinement + error bounds
 *     TEST 6: ZTBCON condition number estimate
 *
 *   IMAT 10-17: Pathological matrices for ZLATBS
 *     TEST 7: ZLATBS with NORMIN='N'
 *     TEST 8: ZLATBS with NORMIN='Y'
 *
 ...
 */
typedef struct {
    c128* AB;     /* Banded matrix: (NMAX+1) * NMAX */
    c128* AINV;   /* Full inverse: NMAX * NMAX */
    c128* B;      /* Right-hand side: NMAX * NSMAX */
    c128* X;      /* Solution: NMAX * NSMAX */
    c128* XACT;   /* Exact solution: NMAX * NSMAX */
    c128* WORK;   /* General workspace: NMAX * max(3, NSMAX) */
    f64* RWORK;   /* Real workspace: NMAX + 2*NSMAX */
} zchktb_workspace_t;
...
static int group_setup(void** state)
{
    ...
    g_workspace->AB = malloc(((NMAX + 1) * NMAX + 1) * sizeof(c128));
...
static void test_standard(void** state)
{
    ...
    zlattb(imat, uplo, "N", &diag, n, kd, ws->AB, ldab,
     ...
     */
    zlaset("F", n, n, CMPLX(ZERO, ZERO), CMPLX(ONE, ZERO), ws->AINV, lda);
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        ...
        for (INT j = 0; j < n; j++) {
            cblas_ztbsv(CblasColMajor, CblasLower, CblasNoTrans,
                        (diag == 'N' || diag == 'n') ? CblasNonUnit : CblasUnit,
    ...
    anorm = zlantb("1", uplo, diag_str, n, kd, ws->AB, ldab, ws->RWORK);
    ainvnm = zlantr("1", uplo, diag_str, n, n, ws->AINV, lda, ws->RWORK);
    ...
    anorm = zlantb("I", uplo, diag_str, n, kd, ws->AB, ldab, ws->RWORK);
    ainvnm = zlantr("I", uplo, diag_str, n, n, ws->AINV, lda, ws->RWORK);
    if (anorm <= ZERO || ainvnm <= ZERO) {
        rcondi = ONE;
    } else {
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        for (INT itran = 0; itran < 3; itran++) {
            ...
            zlarhs("ZTB", xtype_str, uplo, trans, n, n, kd, idiag, nrhs,
                   ws->AB, ldab, ws->XACT, lda, ws->B, lda, &info, rng_state);
            xtype_str[0] = 'C';

            zlacpy("F", n, nrhs, ws->B, lda, ws->X, lda);

            ztbtrs(uplo, trans, diag_str, n, kd, nrhs, ws->AB, ldab,
                   ws->X, lda, &info);
            assert_info_success(info);

            ztbt02(uplo, trans, diag_str, n, kd, nrhs, ws->AB, ldab,
            ...
            zget04(n, nrhs, ws->X, lda, ws->XACT, lda, rcondc, &result[1]);
            ...
            ztbrfs(uplo, trans, diag_str, n, kd, nrhs, ws->AB, ldab,
                   ws->B, lda, ws->X, lda,
            ...
            ztbt05(uplo, trans, diag_str, n, kd, nrhs, ws->AB, ldab,
    ...
    for (INT itran = 0; itran < 2; itran++) {
        ...
        ztbcon(norm, uplo, diag_str, n, kd, ws->AB, ldab,
               &rcond, ws->WORK, ws->RWORK, &info);
        assert_info_success(info);

        ztbt06(rcond, rcondc, uplo, diag_str, n, kd, ws->AB, ldab,
               ws->RWORK, &result[5]);
...
static void test_latbs(void** state)
{
    ...
    for (INT itran = 0; itran < 3; itran++) {
        ...
        /* Generate triangular banded test matrix */
        zlattb(imat, uplo, trans, &diag, n, kd, ws->AB, ldab,
               ws->XACT, ws->WORK, ws->RWORK, &info, rng_state);
        char diag_str[2] = {diag, '\0'};
        ...
        zlatbs(uplo, trans, diag_str, "N", n, kd, ws->AB, ldab,
               ws->B, &scale, ws->RWORK, &info);

        ztbt03(uplo, trans, diag_str, n, kd, 1, ws->AB, ldab,
        ...
        cblas_zcopy(n, ws->XACT, 1, ws->B, 1);
        zlatbs(uplo, trans, diag_str, "Y", n, kd, ws->AB, ldab,
               ws->B, &scale, ws->RWORK, &info);
...
static void build_test_array(void)
{
    ...
    for (INT in = 0; in < (INT)NN; in++) {
        ...
        for (INT ik = 0; ik < nk; ik++) {
            ...
            /* Pathological tests (ZLATBS): IMAT NTYPE1+1 to NTYPES */
