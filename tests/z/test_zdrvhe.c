/**
 * @file test_zdrvhe.c
 * @brief ZDRVHE tests the driver routines ZHESV and ZHESVX.
 *
...
static void run_zdrvhe_single(INT n, INT imat, INT iuplo, INT ifact)
{
    ...
    if (zerot) {
        ...
        zhetrf(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK, ws->lwork, &info);

        zlacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        INT lwork_tri = (n + 1 + 1) * (1 + 3);
        zhetri2(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, lwork_tri, &info);
    ...
    if (ifact == 1) {
        ...
        zhesv(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK, ws->X, lda,
              ws->WORK, ws->lwork, &info);

        if (zerot) {
            if (info <= 0) {
                fail_msg("ZHESV: expected INFO > 0 for singular matrix, got %d", info);
                return;
            }
        } else if (info != 0) {
            fail_msg("ZHESV info=%d expected=0", info);
            return;
        } else {
            /* TEST 1: Reconstruct matrix from factors */
            zhet01(uplo, n, ws->A, lda, ws->AFAC, lda, ws->IWORK,
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("ZHESV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    f64 rcond;
    zhesvx(fact, uplo, n, NRHS, ws->A, lda, ws->AFAC, lda,
    ...
    if (zerot) {
        if (info <= 0) {
            fail_msg("ZHESVX: expected INFO > 0 for singular matrix, got %d", info);
            return;
        }
    } else if (info != 0) {
        fail_msg("ZHESVX info=%d expected=0", info);
        return;
    }
    ...
    /* TEST 6: Compare RCOND from ZHESVX with computed value */
    result[5] = dget06(rcond, rcondc);

    /* Check results */
    for (INT i = k1 - 1; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZHESVX FACT=%s UPLO=%s test %d: result=%e >= thresh=%e",
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvhe", g_tests, g_num_tests,
