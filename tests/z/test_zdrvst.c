/**
 * @file test_zdrvst.c
 * @brief ZDRVST checks the Hermitian eigenvalue problem drivers.
 *
 * Port of LAPACK TESTING/EIG/zdrvst.f
 */
...
static zdrvst_workspace_t* g_ws = NULL;
...
static int group_setup(void** state)
{
    ...
    g_ws = malloc(sizeof(zdrvst_workspace_t));
    if (!g_ws) return -1;
    ...
    /* LWEDC: MAX(2*N+N*N, 2*N*N) (zdrvst.f line 478) */
    INT lwedc1 = 2 * nmax + n2;
    INT lwedc2 = 2 * n2;
    g_ws->lwork = (lwedc1 > lwedc2) ? lwedc1 : lwedc2;
    /* LRWEDC: 1 + 4*N + 2*N*LGN + 3*N**2 (zdrvst.f line 479) */
    g_ws->lrwork = 1 + 4 * nmax + 2 * nmax * lgn + 3 * n2;
    /* LIWEDC: 3 + 5*N (zdrvst.f line 480) */
...
static void run_zdrvst_single(zdrvst_params_t* params)
{
    INT n = params->n;
    INT jtype = params->jtype;

    zdrvst_workspace_t* ws = g_ws;
    INT nmax = ws->nmax;
    INT lda = nmax;
    INT ldu = nmax;
    ...
    for (INT iuplo = 0; iuplo <= 1; iuplo++) {
        ...
        if (iinfo != 0) {
            print_message("ZHEEVD(V,%s) returned info=%d\n", uplo, iinfo);
        ...
        ntest = ntest + 2;
        zheevd("N", uplo, n, A, ldu, D3, work, lwedc,
               rwork, lrwedc, iwork, liwedc, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVD(N,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L130;
        }
...
L130:
        ...
        ntest = ntest + 2;
        zheevx("N", "A", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m2, WA2, Z, ldu, work, ws->lwork, rwork,
               iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVX(N,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L150;
        }
...
L150:
        ...
        /* ZHEEVX('V','I') — zdrvst.f line 789 */
        zheevx("V", "I", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m2, WA2, Z, ldu, work, ws->lwork, rwork,
               iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVX(V,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L160;
        ...
        zheevx("N", "I", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m3, WA3, Z, ldu, work, ws->lwork, rwork,
               iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVX(N,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L160;
        }
...
L160:
        ...
        /* ZHEEVX('V','V') — zdrvst.f line 845 */
        zheevx("V", "V", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m2, WA2, Z, ldu, work, ws->lwork, rwork,
               iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVX(V,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L170;
        ...
        zheevx("N", "V", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m3, WA3, Z, ldu, work, ws->lwork, rwork,
               iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVX(N,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L170;
        }
...
L170:
        ...
        zhpevd("V", uplo, n, work, D1, Z, ldu,
               ...
               liwedc, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVD(V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L270;
        ...
        zhpevd("N", uplo, n, work, D3, Z, ldu,
               ...
               liwedc, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVD(N,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L270;
        }
...
L270:
        ...
        zhpevx("V", "A", uplo, n, work, vl, vu, il, iu,
               ...
               iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVX(V,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L370;
        ...
        zhpevx("N", "A", uplo, n, work, vl, vu, il, iu,
               ...
               iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVX(N,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L370;
        }
...
L370:
        ...
        /* ZHPEVX('V','I') — zdrvst.f line 1132 */
        zhpevx("V", "I", uplo, n, work, vl, vu, il, iu,
               abstol, &m2, WA2, Z, ldu, V, rwork, iwork,
               iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVX(V,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L460;
        ...
        zhpevx("N", "I", uplo, n, work, vl, vu, il, iu,
               ...
               iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVX(N,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L460;
        }
...
L460:
        ...
        /* ZHPEVX('V','V') — zdrvst.f line 1221 */
        zhpevx("V", "V", uplo, n, work, vl, vu, il, iu,
               abstol, &m2, WA2, Z, ldu, V, rwork, iwork,
               iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVX(V,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L550;
        ...
        zhpevx("N", "V", uplo, n, work, vl, vu, il, iu,
               ...
               iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEVX(N,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L550;
        }
...
L550:
        ...
        if (iinfo != 0) {
            print_message("ZHBEVD(V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L650;
        ...
        ntest = ntest + 2;
        zhbevd("N", uplo, n, kd, V, ldu, D3, Z, ldu, work,
               lwedc, rwork, lrwedc, iwork, liwedc, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEVD(N,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L650;
        }
...
L650:
        ...
        if (iinfo != 0) {
            print_message("ZHBEVX(V,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L750;
        ...
        zhbevx("N", "A", uplo, n, kd, V, ldu, U, ldu, vl,
               vu, il, iu, abstol, &m2, WA2, Z, ldu, work,
               rwork, iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEVX(N,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L750;
        }
...
L750:
        ...
        /* ZHBEVX('V','I') — zdrvst.f line 1488 */
        zhbevx("V", "I", uplo, n, kd, V, ldu, U, ldu, vl,
               vu, il, iu, abstol, &m2, WA2, Z, ldu, work,
               rwork, iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEVX(V,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L840;
        ...
        zhbevx("N", "I", uplo, n, kd, V, ldu, U, ldu, vl,
               vu, il, iu, abstol, &m3, WA3, Z, ldu, work,
               rwork, iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEVX(N,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L840;
        }
...
L840:
        ...
        /* ZHBEVX('V','V') — zdrvst.f line 1570 */
        zhbevx("V", "V", uplo, n, kd, V, ldu, U, ldu, vl,
               vu, il, iu, abstol, &m2, WA2, Z, ldu, work,
               rwork, iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEVX(V,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L930;
        ...
        zhbevx("N", "V", uplo, n, kd, V, ldu, U, ldu, vl,
               vu, il, iu, abstol, &m3, WA3, Z, ldu, work,
               rwork, iwork, iwork + 5 * n, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEVX(N,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L930;
        }
...
L930:
        /* ZHEEV — zdrvst.f line 1643 */
        zlacpy(" ", n, n, A, lda, V, ldu);

        ntest = ntest + 1;
        zheev("V", uplo, n, A, ldu, D1, work, ws->lwork, rwork, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEV(V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L950;
        ...
        ntest = ntest + 2;
        zheev("N", uplo, n, A, ldu, D3, work, ws->lwork, rwork, &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEV(N,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L950;
        }
...
L950:
        ...
        if (iinfo != 0) {
            print_message("ZHPEV(V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L1050;
        ...
        zhpev("N", uplo, n, work, D3, Z, ldu,
              work + indwrk, rwork, &iinfo);
        if (iinfo != 0) {
            print_message("ZHPEV(N,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L1050;
        }
...
L1050:
        ...
        zhbev("V", uplo, n, kd, V, ldu, D1, Z, ldu, work,
              rwork, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEV(V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L1140;
        ...
        ntest = ntest + 2;
        zhbev("N", uplo, n, kd, V, ldu, D3, Z, ldu, work,
              rwork, &iinfo);
        if (iinfo != 0) {
            print_message("ZHBEV(N,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L1140;
        }
...
L1140:
        ...
        zheevr("V", "A", uplo, n, A, ldu, vl, vu, il, iu,
               ...
               &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVR(V,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L1170;
        ...
        ntest = ntest + 2;
        zheevr("N", "A", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m2, WA2, Z, ldu, iwork, work, ws->lwork,
               rwork, ws->lrwork, iwork + 2 * n, ws->liwork - 2 * n,
               &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVR(N,A,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L1170;
        }
...
L1170:
        ...
        zheevr("V", "I", uplo, n, A, ldu, vl, vu, il, iu,
               ...
               &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVR(V,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L1180;
        ...
        ntest = ntest + 2;
        zlacpy(" ", n, n, V, ldu, A, lda);
        zheevr("N", "I", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m3, WA3, Z, ldu, iwork, work, ws->lwork,
               rwork, ws->lrwork, iwork + 2 * n, ws->liwork - 2 * n,
               &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVR(N,I,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L1180;
        }
...
L1180:
        ...
        zheevr("V", "V", uplo, n, A, ldu, vl, vu, il, iu,
               ...
               &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVR(V,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            ws->result[ntest] = ulpinv;
            ws->result[ntest + 1] = ulpinv;
            goto L1190;
        ...
        ntest = ntest + 2;
        zlacpy(" ", n, n, V, ldu, A, lda);
        zheevr("N", "V", uplo, n, A, ldu, vl, vu, il, iu,
               abstol, &m3, WA3, Z, ldu, iwork, work, ws->lwork,
               rwork, ws->lrwork, iwork + 2 * n, ws->liwork - 2 * n,
               &iinfo);
        if (iinfo != 0) {
            print_message("ZHEEVR(N,V,%s) returned info=%d\n", uplo, iinfo);
            ws->result[ntest - 1] = ulpinv;
            goto L1190;
        }
...
static void test_zdrvst_case(void** state)
{
    zdrvst_params_t* params = *state;
    run_zdrvst_single(params);
}

#define MAX_TESTS (NNVAL * MAXTYP)

static zdrvst_params_t g_params[MAX_TESTS];
...
static void build_test_array(void)
{
    ...
    for (size_t in = 0; in < NNVAL; in++) {
        ...
        for (INT jtype = 1; jtype <= MAXTYP; jtype++) {
            zdrvst_params_t* p = &g_params[g_num_tests];
            p->n = n;
            p->jtype = jtype;
            snprintf(p->name, sizeof(p->name),
                     "zdrvst_n%d_type%d", n, jtype);

            g_tests[g_num_tests].name = p->name;
            g_tests[g_num_tests].test_func = test_zdrvst_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zdrvst", g_tests, g_num_tests,
