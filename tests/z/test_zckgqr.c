 ...
 *
 * Tests ZGGQRF and ZGGRQF - the generalized QR and RQ factorizations:
 *   ZGGQRF: A = Q*R, B = Q*T*Z  (N-by-M matrix A, N-by-P matrix B)
 *   ZGGRQF: A = R*Q, B = Z*T*Q  (M-by-N matrix A, P-by-N matrix B)
 *
 * Each (im, ip, in, imat) combination is registered as a separate CMocka test.
 * For each combination, both ZGGRQF (via zgrqts) and ZGGQRF (via zgqrts) are tested.
 *
...
static int group_setup(void** state)
{
    ...
    g_ws->A     = malloc(lda * NMAX * sizeof(c128));
    g_ws->AF    = malloc(lda * NMAX * sizeof(c128));
    g_ws->AQ    = malloc(lda * NMAX * sizeof(c128));
    g_ws->AR    = malloc(lda * NMAX * sizeof(c128));
    g_ws->taua  = malloc(NMAX * sizeof(c128));
    g_ws->B     = malloc(ldb * NMAX * sizeof(c128));
    g_ws->BF    = malloc(ldb * NMAX * sizeof(c128));
    g_ws->BZ    = malloc(ldb * NMAX * sizeof(c128));
    g_ws->BT    = malloc(ldb * NMAX * sizeof(c128));
    g_ws->BWK   = malloc(ldb * NMAX * sizeof(c128));
    g_ws->taub  = malloc(NMAX * sizeof(c128));
    g_ws->work  = malloc(lwork * sizeof(c128));
...
static void test_zckgqr_case(void** state)
{
    ...
    zlatms(m, n, dista_str, type_str, g_ws->rwork, modea, cndnma,
           anorm, kla, kua, "N", g_ws->A, lda, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    char distb_str[2] = {distb, '\0'};
    zlatms(p, n, distb_str, type_str, g_ws->rwork, modeb, cndnmb,
           bnorm, klb, kub, "N", g_ws->B, ldb, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    zgrqts(m, p, n, g_ws->A, g_ws->AF, g_ws->AQ, g_ws->AR, lda,
    ...
    for (INT i = 0; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fprintf(stderr, "  GRQ: M=%d P=%d N=%d type=%d test=%d ratio=%g\n",
    ...
    dista_str[0] = dista;
    type_str[0] = type;
    zlatms(n, m, dista_str, type_str, g_ws->rwork, modea, cndnma,
           anorm, kla, kua, "N", g_ws->A, lda, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    distb_str[0] = distb;
    zlatms(n, p, distb_str, type_str, g_ws->rwork, modea, cndnma,
           bnorm, klb, kub, "N", g_ws->B, ldb, g_ws->work, &iinfo,
           rng_state);
    assert_int_equal(iinfo, 0);

    zgqrts(n, m, p, g_ws->A, g_ws->AF, g_ws->AQ, g_ws->AR, lda,
           g_ws->taua, g_ws->B, g_ws->BF, g_ws->BZ, g_ws->BT,
           g_ws->BWK, ldb, g_ws->taub, g_ws->work, lwork,
           g_ws->rwork, result);
    ...
    for (INT i = 0; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fprintf(stderr, "  GQR: N=%d M=%d P=%d type=%d test=%d ratio=%g\n",
