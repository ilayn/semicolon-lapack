/**
 * @file test_zdrvsy_rook.c
 * @brief ZDRVSY_ROOK tests the driver routine ZSYSV_ROOK.
 *
 * Port of LAPACK TESTING/LIN/zdrvsy_rook.f to C with CMocka parameterization.
 */
...
typedef struct {
    ...
    INT* IWORK;
    INT lwork;
} zdrvsy_rook_workspace_t;

static zdrvsy_rook_workspace_t* g_workspace = NULL;

static int group_setup(void** state)
{
    (void)state;
    g_workspace = malloc(sizeof(zdrvsy_rook_workspace_t));
...
static void run_zdrvsy_rook_single(INT n, INT imat, INT iuplo)
{
    static const char* UPLOS[] = {"U", "L"};

    zdrvsy_rook_workspace_t* ws = g_workspace;
    ...
    if (zerot) {
        ...
        zsytrf_rook(uplo, n, ws->AFAC, lda, ws->IWORK, ws->WORK,
                    ws->lwork, &info);

        zlacpy(uplo, n, n, ws->AFAC, lda, ws->AINV, lda);
        zsytri_rook(uplo, n, ws->AINV, lda, ws->IWORK, ws->WORK, &info);
    ...
    zsysv_rook(uplo, n, NRHS, ws->AFAC, lda, ws->IWORK,
    ...
    /* Check error code from ZSYSV_ROOK */
    if (info != k) {
        fail_msg("ZSYSV_ROOK UPLO=%s info=%d expected=%d (n=%d imat=%d)",
                 uplo, info, k, n, imat);
        return;
    } else if (info != 0) {
        return;
    ...
    for (INT i = 0; i < nt; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZSYSV_ROOK UPLO=%s test %d failed: result=%e >= thresh=%e",
                     uplo, i + 1, result[i], THRESH);
        }
    }
}

static void test_zdrvsy_rook_case(void** state)
{
    zdrvsy_rook_params_t* p = *state;
    run_zdrvsy_rook_single(p->n, p->imat, p->iuplo);
}

#define MAX_TESTS 1500

static zdrvsy_rook_params_t g_params[MAX_TESTS];
    ...
    for (INT in = 0; in < (INT)NN; in++) {
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < 2; iuplo++) {
                zdrvsy_rook_params_t* p = &g_params[g_num_tests];
                p->n = n;
                p->imat = imat;
                p->iuplo = iuplo;
                ...
                g_tests[g_num_tests].test_func = test_zdrvsy_rook_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvsy_rook", g_tests, g_num_tests,
