 ...
 *   TEST 5: Solution residual via zpot02 (using zhetrs_rook)
...
static void run_zchkhe_rook_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    zhetrf_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV,
    ...
    zhet01_rook(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        zhetri_rook(uplo_str, n, ws->AINV, lda, ws->IPIV, ws->WORK, &info);
        if (info == 0) {
    ...
    {
        f64 dtemp;
        f64 const_val = ((alpha * alpha - ONE) / (alpha * alpha - ONEHALF))
                        / (ONE - alpha);
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 5: Solve and compute residual for A * X = B using zhetrs_rook
         */
        ...
        zhetrs_rook(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV,
                    ws->X, lda, &info);
        assert_int_equal(info, 0);
...
test7:
    ...
    zhecon_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV, anorm, &rcond,
                ws->WORK, &info);
    assert_int_equal(info, 0);
