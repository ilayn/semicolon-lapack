 ...
 *   Type 1: Random unitary matrix via ZLAROR
...
static void zlacsg(const INT m, const INT p, const INT q,
                   const f64* theta, c128* X, const INT ldx,
                   c128* work, uint64_t rng[static 4])
{
    INT r = MIN4(p, m - p, q, m - q);
    INT info;
    const c128 CZERO = CMPLX(0.0, 0.0);

    zlaset("Full", m, m, CZERO, CZERO, X, ldx);

    for (INT i = 0; i < MIN(p, q) - r; i++)
        X[i + i * ldx] = CMPLX(1.0, 0.0);
    for (INT i = 0; i < r; i++)
        X[(MIN(p, q) - r + i) + (MIN(p, q) - r + i) * ldx] = CMPLX(cos(theta[i]), 0.0);

    for (INT i = 0; i < MIN(p, m - q) - r; i++)
        X[(p - 1 - i) + (m - 1 - i) * ldx] = CMPLX(-1.0, 0.0);
    for (INT i = 0; i < r; i++) {
        INT k = MIN(p, m - q) - r;
        X[(p - k - 1 - i) + (m - k - 1 - i) * ldx] = CMPLX(-sin(theta[r - 1 - i]), 0.0);
    }

    for (INT i = 0; i < MIN(m - p, q) - r; i++)
        X[(m - 1 - i) + (q - 1 - i) * ldx] = CMPLX(1.0, 0.0);
    for (INT i = 0; i < r; i++) {
        INT k2 = MIN(m - p, q) - r;
        X[(m - k2 - 1 - i) + (q - k2 - 1 - i) * ldx] = CMPLX(sin(theta[r - 1 - i]), 0.0);
    }

    for (INT i = 0; i < MIN(m - p, m - q) - r; i++)
        X[(p + i) + (q + i) * ldx] = CMPLX(1.0, 0.0);
    for (INT i = 0; i < r; i++) {
        INT k3 = MIN(m - p, m - q) - r;
        X[(p + k3 + i) + (q + k3 + i) * ldx] = CMPLX(cos(theta[i]), 0.0);
    }

    zlaror("Left", "No init", p, m, X, ldx, work, &info, rng);
    zlaror("Left", "No init", m - p, m, &X[p], ldx, work, &info, rng);
    zlaror("Right", "No init", m, q, X, ldx, work, &info, rng);
    zlaror("Right", "No init", m, m - q, &X[q * ldx], ldx, work, &info, rng);
}
...
static int group_setup(void** state)
{
    ...
    g_ws->X     = malloc((size_t)ldx * mmax * sizeof(c128));
    g_ws->XF    = malloc((size_t)ldx * mmax * sizeof(c128));
    g_ws->U1    = malloc((size_t)ldx * mmax * sizeof(c128));
    g_ws->U2    = malloc((size_t)ldx * mmax * sizeof(c128));
    g_ws->V1T   = malloc((size_t)ldx * mmax * sizeof(c128));
    g_ws->V2T   = malloc((size_t)ldx * mmax * sizeof(c128));
    g_ws->theta = malloc((size_t)mmax * sizeof(f64));
    g_ws->iwork = malloc((size_t)mmax * sizeof(INT));
    g_ws->rwork = malloc((size_t)(17 * (mmax + 2)) * sizeof(f64));
...
static void run_zckcsd_single(zckcsd_params_t* params)
{
    ...
    if (imat == 1) {
        zlaror("Left", "I", m, m, X, ldx, work, &iinfo, rng);
        if (m != 0 && iinfo != 0) {
            snprintf(ctx, sizeof(ctx),
                     "ZLAROR in ZCKCSD: M=%d, INFO=%d", m, iinfo);
        ...
        zlaset("Full", m, m, CZERO, CONE, X, ldx);
        for (INT i = 0; i < m; i++) {
            INT j = (INT)(rng_uniform(rng) * m);
            if (j != i) {
                cblas_zdrot(m, &X[i * ldx], 1, &X[j * ldx], 1, 0.0, 1.0);
            }
        }
    }
    ...
    zcsdts(m, p, q, X, XF, ldx, U1, ldx, U2, ldx,
