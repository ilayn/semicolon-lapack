/**
 * @file test_zchksy.c
 * @brief Comprehensive test suite for complex symmetric indefinite matrix (ZSY) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchksy.f to C using CMocka.
 * Tests ZSYTRF, ZSYTRI2, ZSYTRS, ZSYTRS2, ZSYRFS, and ZSYCON.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksy.f:
 *   TEST 1: LDL^T factorization residual via zsyt01
 *   TEST 2: Matrix inverse residual via zsyt03
 *   TEST 3: Solution residual via zsyt02 (using zsytrs)
 *   TEST 4: Solution residual via zsyt02 (using zsytrs2)
 *   TEST 5: Solution accuracy via zget04
 *   TEST 6: Refined solution accuracy via zget04 (after zsyrfs)
...
static void run_zchksy_single(INT n, INT iuplo, INT imat, INT inb)
{
    ...
    if (imat != NTYPES) {
        ...
        kl = n - 1;
        ku = n - 1;
        zlatsy(uplo_str, n, ws->A, lda, rng_state);
    }

    zlacpy(uplo_str, n, n, ws->A, lda, ws->AFAC, lda);

    zsytrf(uplo_str, n, ws->AFAC, lda, ws->IPIV, ws->WORK, lwork, &info);
    ...
    zsyt01(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        zsytri2(uplo_str, n, ws->AINV, lda, ws->IPIV, ws->WORK, lwork_tri2, &info);
        if (info == 0) {
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        zsytrs(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV, ws->X, lda, &info);
        assert_int_equal(info, 0);
        ...
        zlacpy("F", n, nrhs, ws->B, lda, ws->X, lda);
        zsytrs2(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV, ws->X, lda, ws->WORK, &info);
        assert_int_equal(info, 0);
        ...
        zsyrfs(uplo_str, n, nrhs, ws->A, lda, ws->AFAC, lda, ws->IPIV,
...
test9:
    ...
    zsycon(uplo_str, n, ws->AFAC, lda, ws->IPIV, anorm, &rcond,
           ws->WORK, &info);
    assert_int_equal(info, 0);
...
{
    build_test_array();
    return _cmocka_run_group_tests("zchksy", g_tests, g_num_tests,
