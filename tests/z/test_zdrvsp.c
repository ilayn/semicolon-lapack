/**
 * @file test_zdrvsp.c
 * @brief ZDRVSP tests the driver routines ZSPSV and ZSPSVX.
 *
...
static void run_zdrvsp_single(INT n, INT imat, INT iuplo, INT ifact)
{
    ...
    if (zerot) {
        ...
        f64 anrm = zlansp("1", uplo, n, ws->A, ws->RWORK);

        memcpy(ws->AFAC, ws->A, npp * sizeof(c128));
        zsptrf(uplo, n, ws->AFAC, ws->IWORK, &info);

        memcpy(ws->AINV, ws->AFAC, npp * sizeof(c128));
        zsptri(uplo, n, ws->AINV, ws->IWORK, ws->WORK, &info);
        f64 ainvnm = zlansp("1", uplo, n, ws->AINV, ws->RWORK);
    ...
    char xtype = 'N';
    zlarhs("ZSP", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (ifact == 1) {
        ...
        zspsv(uplo, n, NRHS, ws->AFAC, ws->IWORK, ws->X, lda, &info);
        ...
        if (info != k) {
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("ZSPSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    f64 rcond;
    zspsvx(fact, uplo, n, NRHS, ws->A, ws->AFAC, ws->IWORK,
    ...
    /* Check error code from ZSPSVX */
    if (info != k) {
        fail_msg("ZSPSVX FACT=%s UPLO=%s info=%d expected=%d (n=%d imat=%d)",
    ...
    /* TEST 6: Compare RCOND from ZSPSVX with computed value */
    result[5] = dget06(rcond, rcondc);

    /* Check results */
    for (INT i = k1 - 1; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZSPSVX FACT=%s UPLO=%s test %d: result=%e >= thresh=%e",
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvsp", g_tests, g_num_tests,
