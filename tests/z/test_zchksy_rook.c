/**
 * @file test_zchksy_rook.c
 * @brief Comprehensive test suite for complex symmetric indefinite ROOK (ZSR) routines.
 *
 * This is a faithful port of LAPACK's TESTING/LIN/zchksy_rook.f to C using CMocka.
 * Tests ZSYTRF_ROOK, ZSYTRI_ROOK, ZSYTRS_ROOK, and ZSYCON_ROOK.
 *
 * Each (n, uplo, imat, inb) combination is registered as a separate CMocka test,
 * providing pytest-like parameterized test behavior with clear failure isolation.
 *
 * Test structure from zchksy_rook.f:
 *   TEST 1: LDL^T factorization residual via zsyt01_rook
 ...
 *   TEST 5: Solution residual via zsyt02 (using zsytrs_rook)
...
} zchksy_rook_workspace_t;

static zchksy_rook_workspace_t* g_workspace = NULL;
...
{
    (void)state;
    g_workspace = malloc(sizeof(zchksy_rook_workspace_t));
...
/**
 * Run the full zchksy_rook test battery for a single (n, uplo, imat, inb) combination.
 *
 * Following LAPACK's zchksy_rook.f:
 ...
 */
static void run_zchksy_rook_single(INT n, INT iuplo, INT imat, INT inb)
{
    const f64 ZERO = 0.0;
    const f64 ONE = 1.0;
    const f64 EIGHT = 8.0;
    const f64 SEVTEN = 17.0;
    zchksy_rook_workspace_t* ws = g_workspace;
    ...
    zsytrf_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV,
    ...
    zsyt01_rook(uplo_str, n, ws->A, lda, ws->AFAC, lda, ws->IPIV,
    ...
    if (inb == 0 && !trfcon) {
        ...
        zsytri_rook(uplo_str, n, ws->AINV, lda, ws->IPIV, ws->WORK, &info);
        if (info == 0) {
    ...
    for (INT irhs = 0; irhs < (INT)NNS; irhs++) {
        ...
        /*
         * TEST 5: Solve and compute residual for A * X = B using zsytrs_rook
         */
        ...
        zsytrs_rook(uplo_str, n, nrhs, ws->AFAC, lda, ws->IPIV,
                    ws->X, lda, &info);
        assert_int_equal(info, 0);
...
test7:
    ...
    zsycon_rook(uplo_str, n, ws->AFAC, lda, ws->IPIV, anorm, &rcond,
                ws->WORK, &info);
    assert_int_equal(info, 0);
...
/**
 * CMocka test function - dispatches to run_zchksy_rook_single based on prestate.
 */
static void test_zchksy_rook_case(void** state)
{
    zchksy_rook_params_t* params = *state;
    run_zchksy_rook_single(params->n, params->iuplo, params->imat, params->inb);
}
...
static zchksy_rook_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < (INT)NUPLO; iuplo++) {
                ...
                for (INT inb = 0; inb < (INT)NNB; inb++) {
                    ...
                    /* Store parameters */
                    zchksy_rook_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->inb = inb;
                    snprintf(p->name, sizeof(p->name), "zchksy_rook_n%d_%c_type%d_nb%d_%d",
                             n, UPLOS[iuplo], imat, nb, inb);

                    /* Create CMocka test entry */
                    g_tests[g_num_tests].name = p->name;
                    g_tests[g_num_tests].test_func = test_zchksy_rook_case;
...
int main(void)
{
    ...
    return _cmocka_run_group_tests("zchksy_rook", g_tests, g_num_tests,
