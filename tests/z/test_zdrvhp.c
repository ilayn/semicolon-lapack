/**
 * @file test_zdrvhp.c
 * @brief ZDRVHP tests the driver routines ZHPSV and ZHPSVX.
 *
 * Port of LAPACK TESTING/LIN/zdrvhp.f to C with CMocka parameterization.
 */
...
static zdrvhp_workspace_t* g_workspace = NULL;

static int group_setup(void** state)
{
    (void)state;
    g_workspace = malloc(sizeof(zdrvhp_workspace_t));
...
static void run_zdrvhp_single(INT n, INT imat, INT iuplo, INT ifact)
{
    static const char* UPLOS[] = {"U", "L"};
    static const char* FACTS[] = {"F", "N"};

    zdrvhp_workspace_t* ws = g_workspace;
    ...
    if (zerot) {
        ...
        f64 anrm = zlanhp("1", uplo, n, ws->A, ws->RWORK);

        memcpy(ws->AFAC, ws->A, npp * sizeof(c128));
        zhptrf(uplo, n, ws->AFAC, ws->IWORK, &info);

        memcpy(ws->AINV, ws->AFAC, npp * sizeof(c128));
        zhptri(uplo, n, ws->AINV, ws->IWORK, ws->WORK, &info);
        f64 ainvnm = zlanhp("1", uplo, n, ws->AINV, ws->RWORK);
    ...
    char xtype = 'N';
    zlarhs("ZHP", &xtype, uplo, " ", n, n, kl, ku,
    ...
    if (ifact == 1) {
        ...
        zhpsv(uplo, n, NRHS, ws->AFAC, ws->IWORK, ws->X, lda, &info);
        ...
        if (info != k) {
            ...
            for (INT i = 0; i < nt; i++) {
                if (result[i] >= THRESH) {
                    fail_msg("ZHPSV UPLO=%s test %d failed: result=%e >= thresh=%e",
    ...
    /* --- Test ZHPSVX --- */
    if (ifact == 1 && npp > 0) {
        zlaset("Full", npp, 1, CMPLX(0.0, 0.0), CMPLX(0.0, 0.0),
               ws->AFAC, npp);
    }
    zlaset("Full", n, NRHS, CMPLX(0.0, 0.0), CMPLX(0.0, 0.0),
           ws->X, lda);

    f64 rcond;
    zhpsvx(fact, uplo, n, NRHS, ws->A, ws->AFAC, ws->IWORK,
    ...
    /* Check error code from ZHPSVX */
    if (info != k) {
        fail_msg("ZHPSVX FACT=%s UPLO=%s info=%d expected=%d (n=%d imat=%d)",
    ...
    if (info == 0) {
        ...
        zppt05(uplo, n, NRHS, ws->A, ws->B, lda,
    ...
    /* TEST 6: Compare RCOND from ZHPSVX with computed value */
    result[5] = dget06(rcond, rcondc);

    /* Check results */
    for (INT i = k1 - 1; i < NTESTS; i++) {
        if (result[i] >= THRESH) {
            fail_msg("ZHPSVX FACT=%s UPLO=%s test %d: result=%e >= thresh=%e",
                     fact, uplo, i + 1, result[i], THRESH);
        }
    }
}

static void test_zdrvhp_case(void** state)
{
    zdrvhp_params_t* p = *state;
    run_zdrvhp_single(p->n, p->imat, p->iuplo, p->ifact);
}

#define MAX_TESTS 3000

static zdrvhp_params_t g_params[MAX_TESTS];
        ...
        for (INT imat = 1; imat <= nimat; imat++) {
            ...
            for (INT iuplo = 0; iuplo < 2; iuplo++) {
                for (INT ifact = 0; ifact < NFACT; ifact++) {
                    ...
                    zdrvhp_params_t* p = &g_params[g_num_tests];
                    p->n = n;
                    p->imat = imat;
                    p->iuplo = iuplo;
                    p->ifact = ifact;
                    ...
                    g_tests[g_num_tests].test_func = test_zdrvhp_case;
...
{
    build_test_array();
    return _cmocka_run_group_tests("zdrvhp", g_tests, g_num_tests,
