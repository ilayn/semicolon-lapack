 ...
 *
 * Tests the nonsymmetric generalized eigenvalue (Schur form) expert driver ZGGESX.
 *
...
typedef struct {
    ...
    c128* A;      /* original A */
    c128* B;      /* original B */
    c128* AI;     /* working copy, modified by zggesx */
    c128* BI;     /* working copy */
    c128* Q;      /* left Schur vectors */
    c128* Z;      /* right Schur vectors */
    c128* alpha;
    c128* beta;
    c128* C;      /* Kronecker matrix for DIF computation */
    f64*  S;      /* singular values of C */
    c128* work;
    f64*  rwork;
    INT*  iwork;
    INT*  bwork;
...
static int group_setup(void** state)
{
    ...
    g_ws->C   = malloc(g_ws->ldc * g_ws->ldc * sizeof(c128));
...
static void test_builtin(void** state)
{
    ...
    f64 ulp    = dlamch("P");
    f64 ulpinv = 1.0 / ulp;
    f64 smlnum = dlamch("S") / ulp;
    f64 thrsh2 = 10.0 * THRESH;

    static f64 weight = 0.0;
    if (weight == 0.0) weight = sqrt(ulp);
    weight = 1.0 / weight;

    for (INT i = 0; i < 10; i++)
        ws->result[i] = 0.0;

    zlctsx_reset(m, n, mplusn);

    const c128 czero = CMPLX(0.0, 0.0);

    zlaset("Full", mplusn, mplusn, czero, czero, ws->AI, lda);
    zlaset("Full", mplusn, mplusn, czero, czero, ws->BI, lda);
    ...
    zlacpy("Full", mplusn, mplusn, ws->AI, lda, ws->A, lda);
    zlacpy("Full", mplusn, mplusn, ws->BI, lda, ws->B, lda);

    INT mm = 0;
    f64 pl[2] = {0.0, 0.0};
    f64 difest[2] = {0.0, 0.0};
    INT linfo = 0;

    zggesx("V", "V", "S", zlctsx, sense, mplusn,
    ...
    zlacpy("Full", mplusn, mplusn, ws->AI, mplusn, ws->work, mplusn);
    zlacpy("Full", mplusn, mplusn, ws->BI, mplusn,
           ws->work + mplusn * mplusn, mplusn);
    abnrm = zlange("Fro", mplusn, 2 * mplusn, ws->work, mplusn, ws->rwork);

    zget51(1, mplusn, ws->A, lda, ws->AI, lda, ws->Q, lda, ws->Z, lda,
           ws->work, ws->rwork, &ws->result[0]);
    zget51(1, mplusn, ws->B, lda, ws->BI, lda, ws->Q, lda, ws->Z, lda,
           ws->work, ws->rwork, &ws->result[1]);
    zget51(3, mplusn, ws->B, lda, ws->BI, lda, ws->Q, lda, ws->Q, lda,
           ws->work, ws->rwork, &ws->result[2]);
    zget51(3, mplusn, ws->B, lda, ws->BI, lda, ws->Z, lda, ws->Z, lda,
    ...
    for (INT j = 0; j < mplusn; j++) {
        f64 temp2 = (abs1(ws->alpha[j] - ws->AI[j + j * lda]) /
                     fmax(smlnum, fmax(abs1(ws->alpha[j]),
                                       abs1(ws->AI[j + j * lda]))) +
                     abs1(ws->beta[j] - ws->BI[j + j * lda]) /
                     fmax(smlnum, fmax(abs1(ws->beta[j]),
                                       abs1(ws->BI[j + j * lda])))) / ulp;

        if (j < mplusn - 1) {
            if (ws->AI[(j + 1) + j * lda] != 0.0)
                ws->result[4] = ulpinv;
        }
        if (j > 0) {
            if (ws->AI[j + (j - 1) * lda] != 0.0)
                ws->result[4] = ulpinv;
        }
    ...
    if (ifunc >= 2 && mn2 <= NCMAX * NCMAX) {
        ...
        INT svd_info = 0;
        zgesvd("N", "N", mn2, mn2, ws->C, ws->ldc, ws->S,
