/**
 * @file csptrf.c
 * @brief CSPTRF computes the factorization of a complex symmetric matrix stored
            ...
            if (fmaxf(absakk, colmax) == ZERO) {
                ...
                if (absakk >= alpha * colmax) {
                    ...
                    if (absakk >= alpha * colmax * (colmax / rowmax)) {
                        kp = k;
                    } else if (cabs1f(AP[kpc + imax]) >= alpha * rowmax) {
                        kp = imax;
                    } else {
                        kp = imax;
                        kstep = 2;
                ...
                if (kstep == 1) {
                    if (k > 0) {
                        r1 = CONE / AP[kc + k];
                        c64 neg_r1 = -r1;
                        cspr(uplo, k, neg_r1, &AP[kc], 1, AP);
                        cblas_cscal(k, &r1, &AP[kc], 1);
                    }
                } else {
                    if (k > 1) {
                        d12 = AP[kc + k - 1];
        ...
        while (k < n) {
            ...
            if (fmaxf(absakk, colmax) == ZERO) {
                ...
                if (absakk >= alpha * colmax) {
                    ...
                    if (absakk >= alpha * colmax * (colmax / rowmax)) {
                        kp = k;
                    } else if (cabs1f(AP[kpc]) >= alpha * rowmax) {
                        kp = imax;
                    } else {
                        kp = imax;
                        kstep = 2;
                ...
                if (kstep == 1) {
                    if (k < n - 1) {
                        r1 = CONE / AP[kc];
                        c64 neg_r1 = -r1;
                        cspr(uplo, n - k - 1, neg_r1,
                                   &AP[kc + 1], 1, &AP[kc + n - k]);
                        cblas_cscal(n - k - 1, &r1, &AP[kc + 1], 1);
                    }
                } else {
                    if (k < n - 2) {
                        d21 = AP[kc + 1];
