 ...
 * CSYTRF_RK (or ZSYTRF_BK) is the same and is not converted.
 *
 * If parameter WAY = 'R':
 ...
 * @param[in,out] E     Complex array of dimension `n`.
 *
 *                      1) If `way='C'`:
 *
 ...
 *                      2) by `csytrf_rk` (or `zsytrf_bk`), if `way='R'`.
 *
        ...
        if (convert) {
            ...
            while (i < n) {
                if (ipiv[i] >= 0) {
                    ...
                    if (i < n - 1) {
                        if (ip2 != i - 1) {
                            cblas_cswap(n - i - 1, &A[ip2 + (i + 1) * lda], lda,
                                        &A[i - 1 + (i + 1) * lda], lda);
                        }
                        if (ip != i) {
                            cblas_cswap(n - i - 1, &A[ip + (i + 1) * lda], lda,
