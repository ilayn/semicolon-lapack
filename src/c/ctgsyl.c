 ...
 * @param[in]     A       The upper triangular matrix A.
 *                        Array of dimension (lda, m).
 * @param[in]     lda     The leading dimension of the array A. lda >= max(1, m).
 * @param[in]     B       The upper triangular matrix B.
    ...
    if ((mb <= 1 && nb <= 1) || (mb >= m && nb >= n)) {
        ...
        for (iround = 1; iround <= isolve; iround++) {
            ...
            *scale = ONE;
            ...
            dsum = ONE;
            pq = m * n;
            ctgsy2(trans, ifunc, m, n, A, lda, B, ldb, C, ldc, D,
            ...
            if (isolve == 2 && iround == 1) {
                ...
                clacpy("F", m, n, F, ldf, &work[m * n], m);
                claset("F", m, n, CZERO, CZERO, C, ldc);
                claset("F", m, n, CZERO, CZERO, F, ldf);
            } else if (isolve == 2 && iround == 2) {
                clacpy("F", m, n, work, m, C, ldc);
                clacpy("F", m, n, &work[m * n], m, F, ldf);
                *scale = scale2;
            }
        }
            ...
            for (j = p + 2; j <= q; j++) {
                ...
                for (i = p; i >= 1; i--) {
                    ...
                    if (scaloc != ONE) {
                        ...
                        for (k = 1; k <= js - 1; k++) {
                            cblas_cscal(m, &scaloc_cmplx,
                                        &C[0 + (k - 1) * ldc], 1);
                            cblas_cscal(m, &scaloc_cmplx,
                                        &F[0 + (k - 1) * ldf], 1);
                        }
                        for (k = js; k <= je; k++) {
                            cblas_cscal(is - 1, &scaloc_cmplx,
                                        &C[0 + (k - 1) * ldc], 1);
                            cblas_cscal(is - 1, &scaloc_cmplx,
                                        &F[0 + (k - 1) * ldf], 1);
                        }
                        for (k = js; k <= je; k++) {
                            cblas_cscal(m - ie, &scaloc_cmplx,
                                        &C[ie + (k - 1) * ldc], 1);
                            cblas_cscal(m - ie, &scaloc_cmplx,
                                        &F[ie + (k - 1) * ldf], 1);
                        }
                    ...
                    if (i > 1) {
                        const c64 NEG_ONE = CMPLXF(-ONE, ZERO);
                        const c64 CONE = CMPLXF(ONE, ZERO);
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    is - 1, nb, mb, &NEG_ONE,
                                    &A[0 + (is - 1) * lda], lda,
                                    &C[(is - 1) + (js - 1) * ldc], ldc,
                                    &CONE, &C[0 + (js - 1) * ldc], ldc);
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    is - 1, nb, mb, &NEG_ONE,
                                    &D[0 + (is - 1) * ldd], ldd,
                                    &C[(is - 1) + (js - 1) * ldc], ldc,
                    ...
                    if (j < q) {
                        ...
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    &F[(is - 1) + (js - 1) * ldf], ldf,
                                    &B[(js - 1) + je * ldb], ldb,
                                    &CONE, &C[(is - 1) + je * ldc], ldc);
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    mb, n - je, nb, &CONE,
                                    &F[(is - 1) + (js - 1) * ldf], ldf,
                                    &E[(js - 1) + je * lde], lde,
                                    &CONE, &F[(is - 1) + je * ldf], ldf);
                    }
                }
            }
            if (dscale != ZERO) {
