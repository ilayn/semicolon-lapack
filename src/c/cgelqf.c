/**
 * @file cgelqf.c
 * @brief CGELQF computes an LQ factorization of a general M-by-N matrix
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k - nx; i += nb) {
            ...
            cgelq2(ib, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    ...
    if (i < k) {
        cgelq2(m - i, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    }
