/**
 * @file cgelsy.c
 * @brief CGELSY computes the minimum-norm solution to a complex linear least
            ...
            int* info)
{
    ...
    *info = 0;
    ...
    cgeqp3(m, n, A, lda, jpvt, work, &work[mn], lwork - mn, rwork, &iinfo);
    wsize = (f32)(mn) + crealf(work[mn]);

    /* Determine RANK using incremental condition estimation */
    work[ismin] = CONE;
    work[ismax] = CONE;
    smax = cabsf(A[0]);  /* |A(0,0)| = |R(1,1)| */
    smin = smax;
    ...
    while (*rank < mn) {
        ...
        claic1(IMIN, *rank, &work[ismin], smin,
               &A[0 + i * lda], A[i + i * lda],
               &sminpr, &s1, &c1);
        claic1(IMAX, *rank, &work[ismax], smax,
               &A[0 + i * lda], A[i + i * lda],
               &smaxpr, &s2, &c2);
    ...
    cblas_ctrsm(CblasColMajor, CblasLeft, CblasUpper, CblasNoTrans,
                CblasNonUnit, *rank, nrhs, &CONE, A, lda, B, ldb);
    ...
    /* B(0:n-1, 0:nrhs-1) := Z**H * B(0:n-1, 0:nrhs-1)
     * sub-workspace in work[2*mn..] */
    if (*rank < n) {
        cunmrz("L", "C", n, nrhs, *rank, n - *rank, A, lda,
    ...
    for (int j = 0; j < nrhs; j++) {
        ...
        cblas_ccopy(n, work, 1, &B[j * ldb], 1);
    }

    /* Undo scaling */
    if (iascl == 1) {
        clascl("G", 0, 0, anrm, smlnum, n, nrhs, B, ldb, &iinfo);
        clascl("U", 0, 0, smlnum, anrm, *rank, *rank, A, lda, &iinfo);
    } else if (iascl == 2) {
        clascl("G", 0, 0, anrm, bignum, n, nrhs, B, ldb, &iinfo);
        clascl("U", 0, 0, bignum, anrm, *rank, *rank, A, lda, &iinfo);
    }
    if (ibscl == 1) {
