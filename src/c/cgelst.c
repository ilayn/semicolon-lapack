...
/**
 * CGELST solves overdetermined or underdetermined complex linear systems
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (mn == 0 || nrhs == 0) {
        ...
        claset("F", max_mn, nrhs, CZERO, CZERO, B, ldb);
        work[0] = (c64)lwopt;
        return;
    }
    ...
    if (m >= n) {
        ...
        cgeqrt(m, n, nb, A, lda, &work[0], nb, &work[mn * nb], info);

        if (!tpsd) {

            cgemqrt("L", "C", m, nrhs, n, nb, A, lda,
        ...
        cgelqt(m, n, nb, A, lda, &work[0], nb, &work[mn * nb], info);
