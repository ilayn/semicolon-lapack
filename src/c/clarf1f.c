/**
 * @file clarf1f.c
 * @brief CLARF1F applies an elementary reflector to a general rectangular
 ...
 */
void clarf1f(const char* side, const INT m, const INT n,
             ...
             c64* restrict work)
{
    ...
    if (applyleft) {
        ...
        if (lastv == 1) {
            ...
            cblas_cscal(lastc, &one_minus_tau, C, ldc);
        } else {
            /*
             ...
             */
            for (i = 0; i < lastc; i++) {
                C[i * ldc] = C[i * ldc] - tau * conjf(work[i]);
            }
             ...
             */
            neg_tau = -tau;
            cblas_cgerc(CblasColMajor, lastv - 1, lastc,
        ...
        if (lastv == 1) {
            ...
            one_minus_tau = ONE - tau;
            cblas_cscal(lastc, &one_minus_tau, C, 1);
        } else {
            /*
             ...
             */
            cblas_caxpy(lastc, &ONE, C, 1, work, 1);

            /*
             * C(0:lastc-1, 0) -= tau * w
             */
            neg_tau = -tau;
            cblas_caxpy(lastc, &neg_tau, work, 1, C, 1);

            /*
             * C(0:lastc-1, 1:lastv-1) -= tau * w * v(incv:)**H
             */
            cblas_cgerc(CblasColMajor, lastc, lastv - 1,
