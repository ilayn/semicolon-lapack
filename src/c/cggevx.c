            ...
            int* info)
{
    ...
    *bbnrm = clange("1", n, n, B, ldb, rwork);
    ...
    itau = 0;
    iwrk = itau + irows;
    cgeqrf(irows, icols, &B[(*ilo - 1) + (*ilo - 1) * ldb], ldb, &work[itau],
           &work[iwrk], lwork - iwrk, &ierr);

    /* Apply the unitary transformation to A */
    cunmqr("L", "C", irows, icols, irows, &B[(*ilo - 1) + (*ilo - 1) * ldb], ldb,
    ...
    if (ilvl) {
        ...
        if (irows > 1) {
            clacpy("L", irows - 1, irows - 1, &B[*ilo + (*ilo - 1) * ldb], ldb,
                   &VL[*ilo + (*ilo - 1) * ldvl], ldvl);
        }
        cungqr(irows, irows, irows, &VL[(*ilo - 1) + (*ilo - 1) * ldvl], ldvl,
    ...
    if (ilv || !wantsn) {
        ...
        cgghrd("N", "N", irows, 1, irows, &A[(*ilo - 1) + (*ilo - 1) * lda], lda,
