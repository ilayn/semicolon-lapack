/**
 * @file chesvx.c
 * @brief CHESVX computes the solution to a complex system of linear equations
 ...
 * @param[in,out] AF    If fact = 'F', contains the factored form from CHETRF.
 *                      If fact = 'N', on exit contains the factored form.
 *                      Single complex array, dimension (ldaf, n).
 * @param[in]     ldaf  The leading dimension of the array AF. ldaf >= max(1, n).
 * @param[in,out] ipiv  If fact = 'F', contains the pivot indices from CHETRF.
 ...
 *                      Single complex array, dimension (ldx, nrhs).
 ...
 *                      Single precision array, dimension (nrhs).
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    cherfs(uplo, n, nrhs, A, lda, AF, ldaf, ipiv, B, ldb, X, ldx,
