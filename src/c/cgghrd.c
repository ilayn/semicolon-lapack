/**
 * @file cgghrd.c
 * @brief CGGHRD reduces a pair of complex matrices (A,B) to generalized upper Hessenberg form.
 */
    ...
    *info = 0;
    ...
    for (jcol = ilo; jcol <= ihi - 2; jcol++) {
        for (jrow = ihi; jrow >= jcol + 2; jrow--) {
            ...
            clartg(ctemp, A[jrow + jcol * lda], &c, &s,
                   &A[jrow - 1 + jcol * lda]);
            A[jrow + jcol * lda] = CZERO;
            crot(n - jcol - 1, &A[jrow - 1 + (jcol + 1) * lda], lda,
                 &A[jrow + (jcol + 1) * lda], lda, c, s);
            crot(n + 2 - jrow - 1, &B[jrow - 1 + (jrow - 1) * ldb], ldb,
                 &B[jrow + (jrow - 1) * ldb], ldb, c, s);
            if (ilq)
                crot(n, &Q[(jrow - 1) * ldq], 1, &Q[jrow * ldq], 1, c,
                     conjf(s));

            ctemp = B[jrow + jrow * ldb];
            clartg(ctemp, B[jrow + (jrow - 1) * ldb], &c, &s,
                   &B[jrow + jrow * ldb]);
            B[jrow + (jrow - 1) * ldb] = CZERO;
            crot(ihi + 1, &A[jrow * lda], 1, &A[(jrow - 1) * lda], 1, c, s);
            crot(jrow, &B[jrow * ldb], 1, &B[(jrow - 1) * ldb], 1, c, s);
            if (ilz)
                crot(n, &Z[jrow * ldz], 1, &Z[(jrow - 1) * ldz], 1, c, s);
        }
    }
}
