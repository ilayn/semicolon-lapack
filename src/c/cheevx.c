/**
 * @file cheevx.c
 * @brief CHEEVX computes selected eigenvalues and, optionally, eigenvectors of a
 ...
 * @param[out]    W       Single precision array, dimension (N). On normal exit,
 ...
 * @param[out]    rwork   Single precision workspace array, dimension (7*N).
    ...
    *m = 0;
    ...
    if (iscale == 1) {
        if (lower) {
            ...
            for (j = 0; j < n; j++) {
                cblas_csscal(j + 1, sigma, &A[0 + j * lda], 1);
            }
        }
        if (abstol > 0) {
            abstll = abstol * sigma;
    ...
    chetrd(uplo, n, A, lda, &rwork[indd], &rwork[inde], &work[indtau],
    ...
    if ((alleig || test) && abstol <= ZERO) {
        ...
        if (!wantz) {
            ...
            cungtr(uplo, n, Z, ldz, &work[indtau],
                   &work[indwrk], llwork, &iinfo);
            cblas_scopy(n - 1, &rwork[inde], 1, &rwork[indee], 1);
    ...
    if (wantz) {
        for (j = 0; j < *m - 1; j++) {
            ...
            if (i >= 0) {
                ...
                cblas_cswap(n, &Z[0 + i * ldz], 1, &Z[0 + j * ldz], 1);
                if (*info != 0) {
                    itmp1 = ifail[i];
                    ifail[i] = ifail[j];
                    ifail[j] = itmp1;
