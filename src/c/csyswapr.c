/**
 * @file csyswapr.c
 * @brief CSYSWAPR applies an elementary permutation on the rows and columns of a symmetric matrix.
 */
 ...
 * @param[in,out] A     Complex array of dimension `(lda,*)`.
    ...
    const INT i2)
{
    ...
    if (upper) {
        ...
        if (i2 - i1 - 1 > 0) {
            cblas_cswap(i2 - i1 - 1, &A[i1 + (i1 + 1) * lda], lda, &A[i1 + 1 + i2 * lda], 1);
        }

        if (i2 < n - 1) {
            cblas_cswap(n - i2 - 1, &A[i1 + (i2 + 1) * lda], lda, &A[i2 + (i2 + 1) * lda], lda);
        }
        ...
        if (i2 - i1 - 1 > 0) {
            cblas_cswap(i2 - i1 - 1, &A[i1 + 1 + i1 * lda], 1, &A[i2 + (i1 + 1) * lda], lda);
        }

        if (i2 < n - 1) {
            cblas_cswap(n - i2 - 1, &A[i2 + 1 + i1 * lda], 1, &A[i2 + 1 + i2 * lda], 1);
        }
    }
}
