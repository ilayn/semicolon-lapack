    ...
    *m = 0;
    ...
    if (*m == n || *m == 0) {
        ...
        if (wantd) {
            ...
            dsum = ONE;
            for (i = 0; i < n; i++) {
                classq(n, &A[0 + i * lda], 1, &dscale, &dsum);
                classq(n, &B[0 + i * ldb], 1, &dscale, &dsum);
            }
    ...
    if (wantp) {
        ...
        n2 = n - *m;
        i = n1;
        clacpy("F", n1, n2, &A[0 + i * lda], lda, work, n1);
        clacpy("F", n1, n2, &B[0 + i * ldb], ldb, &work[n1 * n2], n1);
        ijb = 0;
        ctgsyl("N", ijb, n1, n2, A, lda, &A[i + i * lda], lda, work, n1,
        ...
        *pl = rdscal * sqrtf(dsum);
        ...
        rdscal = ZERO;
        dsum = ONE;
        classq(n1 * n2, &work[n1 * n2], 1, &rdscal, &dsum);
    ...
    if (wantd) {
        ...
        if (wantd1) {
            ...
            ctgsyl("N", ijb, n1, n2, A, lda, &A[i + i * lda], lda,
                   work, n1, B, ldb, &B[i + i * ldb], ldb, &work[n1 * n2],
                   n1, &dscale, &dif[0], &work[n1 * n2 * 2],
                   lwork - 2 * n1 * n2, iwork, &ierr);

            /* Frobenius norm-based Difl estimate. */
            ctgsyl("N", ijb, n2, n1, &A[i + i * lda], lda, A, lda,
...
L40:
            clacn2(mn2, &work[mn2], work, &dif[0], &kase, isave);
            if (kase != 0) {
                if (kase == 1) {
                    ...
                    ctgsyl("C", ijb, n1, n2, A, lda, &A[i + i * lda], lda,
                           work, n1, B, ldb, &B[i + i * ldb], ldb,
                           &work[n1 * n2], n1, &dscale, &dif[0],
                           &work[2 * n1 * n2], lwork - 2 * n1 * n2,
                           iwork, &ierr);
...
L50:
            clacn2(mn2, &work[mn2], work, &dif[1], &kase, isave);
            if (kase != 0) {
                if (kase == 1) {
                    /* Solve generalized Sylvester equation */
                    ctgsyl("N", ijb, n2, n1, &A[i + i * lda], lda, A, lda,
                    ...
                    /* Solve the transposed variant. */
                    ctgsyl("C", ijb, n2, n1, &A[i + i * lda], lda, A, lda,
    ...
    for (k = 0; k < n; k++) {
        dscale = cabsf(B[k + k * ldb]);
        if (dscale > safmin) {
            temp1 = conjf(B[k + k * ldb] / dscale);
            temp2 = B[k + k * ldb] / dscale;
            B[k + k * ldb] = CMPLXF(dscale, 0.0f);
            cblas_cscal(n - k - 1, &temp1, &B[k + (k + 1) * ldb], ldb);
            cblas_cscal(n - k, &temp1, &A[k + k * lda], lda);
            if (wantq)
                cblas_cscal(n, &temp2, &Q[0 + k * ldq], 1);
        } else {
            B[k + k * ldb] = CMPLXF(ZERO, ZERO);
        }
