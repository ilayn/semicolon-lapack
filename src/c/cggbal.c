    ...
    if (!(job[0] == 'S' || job[0] == 's')) {
        ...
        for (;;) {
            ...
            for (i = l - 1; i >= 0; i--) {
                ...
                lscale[m - 1] = (f32)(i + 1);
                if (i != m - 1) {
                    cblas_cswap(n - k + 1, &A[i + (k - 1) * lda], lda,
                                &A[m - 1 + (k - 1) * lda], lda);
                    cblas_cswap(n - k + 1, &B[i + (k - 1) * ldb], ldb,
                                &B[m - 1 + (k - 1) * ldb], ldb);
                }
                rscale[m - 1] = (f32)(j + 1);
                if (j != m - 1) {
                    cblas_cswap(l, &A[j * lda], 1, &A[(m - 1) * lda], 1);
                    cblas_cswap(l, &B[j * ldb], 1, &B[(m - 1) * ldb], 1);
                }
                l--;
                found = 1;
                break;
...
scaling:
    ...
    *ihi = l;
    ...
    for (i = k - 1; i < l; i++) {
        irab = cblas_icamax(n - k + 1, &A[i + (k - 1) * lda], lda);
        rab = cabsf(A[i + (irab + k - 1) * lda]);
        irab = cblas_icamax(n - k + 1, &B[i + (k - 1) * ldb], ldb);
        rab = (rab > cabsf(B[i + (irab + k - 1) * ldb])) ?
              rab : cabsf(B[i + (irab + k - 1) * ldb]);
        ...
        icab = cblas_icamax(l, &A[i * lda], 1);
        cab = cabsf(A[icab + i * lda]);
        icab = cblas_icamax(l, &B[i * ldb], 1);
        cab = (cab > cabsf(B[icab + i * ldb])) ?
              cab : cabsf(B[icab + i * ldb]);
    ...
    for (i = k - 1; i < l; i++) {
        cblas_csscal(n - k + 1, lscale[i], &A[i + (k - 1) * lda], lda);
        cblas_csscal(n - k + 1, lscale[i], &B[i + (k - 1) * ldb], ldb);
    }

    /* Column scaling of matrices A and B */

    for (j = k - 1; j < l; j++) {
        cblas_csscal(l, rscale[j], &A[j * lda], 1);
        cblas_csscal(l, rscale[j], &B[j * ldb], 1);
    }
}
