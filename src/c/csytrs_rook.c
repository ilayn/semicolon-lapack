/**
 * @file csytrs_rook.c
 * @brief CSYTRS_ROOK solves a system of linear equations A*X = B with a complex symmetric matrix A using the factorization computed by CSYTRF_ROOK.
 */
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                cblas_cgeru(CblasColMajor, k, nrhs, &NEG_CONE, &A[0 + k * lda], 1,
                            &B[k], ldb, &B[0], ldb);

                c64 s = CONE / A[k + k * lda];
                cblas_cscal(nrhs, &s, &B[k], ldb);
                k = k - 1;
                ...
                if (k > 1) {
                    cblas_cgeru(CblasColMajor, k - 1, nrhs, &NEG_CONE, &A[0 + k * lda], 1,
                                &B[k], ldb, &B[0], ldb);
                    cblas_cgeru(CblasColMajor, k - 1, nrhs, &NEG_CONE, &A[0 + (k - 1) * lda], 1,
                                &B[k - 1], ldb, &B[0], ldb);
                }
        ...
        while (k < n) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k > 0) {
                    cblas_cgemv(CblasColMajor, CblasTrans, k, nrhs, &NEG_CONE, B,
        ...
        while (k < n) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    cblas_cgeru(CblasColMajor, n - k - 1, nrhs, &NEG_CONE, &A[k + 1 + k * lda], 1,
                                &B[k], ldb, &B[k + 1], ldb);
                }
                ...
                if (k < n - 2) {
                    cblas_cgeru(CblasColMajor, n - k - 2, nrhs, &NEG_CONE, &A[k + 2 + k * lda], 1,
                                &B[k], ldb, &B[k + 2], ldb);
                    cblas_cgeru(CblasColMajor, n - k - 2, nrhs, &NEG_CONE, &A[k + 2 + (k + 1) * lda], 1,
                                &B[k + 1], ldb, &B[k + 2], ldb);
                }
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    cblas_cgemv(CblasColMajor, CblasTrans, n - k - 1, nrhs, &NEG_CONE,
