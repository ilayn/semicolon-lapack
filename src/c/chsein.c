/**
 * @file chsein.c
 * @brief CHSEIN uses inverse iteration to find specified right and/or left
 ...
 * @param[in]     eigsrc  'Q': eigenvalues were found using CHSEQR, so matrix
 ...
 * @param[in]     H       Upper Hessenberg matrix. Single complex array,
 *                        dimension (ldh, n).
 *                        If a NaN is detected in H, the routine will return
 *                        with info=-6.
 * @param[in]     ldh     The leading dimension of H (ldh >= max(1,n)).
 * @param[in,out] W       Single complex array, dimension (n).
 ...
 * @param[in,out] VL      Left eigenvectors. Single complex array,
 *                        dimension (ldvl, mm).
 * @param[in]     ldvl    Leading dimension of VL (ldvl >= 1; ldvl >= n if
 *                        computing left eigenvectors).
 * @param[in,out] VR      Right eigenvectors. Single complex array,
 *                        dimension (ldvr, mm).
 ...
 * @param[out]    work    Single complex workspace array, dimension (n*n).
    ...
    for (k = 0; k < n; k++) {
        if (select[k]) {
            ...
            if (kl != kln) {
                ...
                hnorm = clanhs("I", kr - kl + 1, &H[kl + kl * ldh], ldh,
                               rwork);
                if (sisnan(hnorm)) {
                    *info = -6;
                    return;
        ...
        L60:
            for (i = k - 1; i >= kl; i--) {
                if (select[i] && cabs1f(W[i] - wk) < eps3) {
                    wk = wk + eps3;
                    goto L60;
                }
            }
            ...
            if (leftv) {
                ...
                claein(0, noinit, n - kl, &H[kl + kl * ldh], ldh,
            ...
            if (rightv) {
                ...
                claein(1, noinit, kr + 1, H, ldh, wk, &VR[ks * ldvr],
