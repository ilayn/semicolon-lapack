/**
 * @file claed8.c
 * @brief CLAED8 merges eigenvalues and deflates the secular equation.
 ...
 * @param[in,out] Z       Single precision array, dimension (N).
 ...
 *                        process.
 * @param[out]    dlambda Single precision array, dimension (N).
 *                        A copy of the first K eigenvalues which will be used by
 *                        SLAED3 to form the secular equation.
 * @param[out]    Q2      Complex array, dimension (LDQ2, N).
 ...
 *                        leading dimension 2.
 * @param[out]    givnum  Single precision array, dimension (2 * N).
    ...
    *rho = fabsf(TWO * (*rho));
    ...
    if ((*rho) * fabsf(Z[imax]) <= tol) {
        *K = 0;
        for (j = 0; j < n; j++) {
            perm[j] = indxq[indx[j]];
            cblas_ccopy(qsiz, &Q[0 + perm[j] * ldq], 1,
                        &Q2[0 + j * ldq2], 1);
        }
        clacpy("A", qsiz, n, Q2, ldq2, Q, ldq);
        return;
    }
    ...
    if ((*rho) * fabsf(Z[j]) <= tol) {
        ...
        if (fabsf(t * c * s) <= tol) {
            ...
            *givptr = *givptr + 1;
            ...
            cblas_csrot(qsiz, &Q[0 + indxq[indx[jlam]] * ldq], 1,
...
L110:
    ...
    if (*K < n) {
        ...
        clacpy("A", qsiz, n - *K, &Q2[0 + (*K) * ldq2], ldq2,
               &Q[0 + (*K) * ldq], ldq);
    }
