/**
 * @file chetri2x.c
 * @brief CHETRI2X computes the inverse of a complex hermitian indefinite matrix
 ...
 * @param[out]    work  Single complex array, dimension (n+nb+1, nb+3).
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                work[k + invd * ldw] = CMPLXF(1.0f / crealf(A[k + k * lda]), 0.0f);
                work[k + (invd + 1) * ldw] = ZERO;
                k = k + 1;
            } else {
                t = CMPLXF(cabsf(work[(k + 1) + 0 * ldw]), 0.0f);
                ak = CMPLXF(crealf(A[k + k * lda]), 0.0f) / t;
                akp1 = CMPLXF(crealf(A[(k + 1) + (k + 1) * lda]), 0.0f) / t;
                akkp1 = work[(k + 1) + 0 * ldw] / t;
                d = t * (ak * akp1 - ONE);
                work[k + invd * ldw] = akp1 / d;
                work[(k + 1) + (invd + 1) * ldw] = ak / d;
                work[k + (invd + 1) * ldw] = -akkp1 / d;
                work[(k + 1) + invd * ldw] = conjf(work[k + (invd + 1) * ldw]);
                k = k + 2;
            }
        }
        ...
        while (k >= 0) {
            if (ipiv[k] >= 0) {
                ...
                t = CMPLXF(cabsf(work[(k - 1) + 0 * ldw]), 0.0f);
                ak = CMPLXF(crealf(A[(k - 1) + (k - 1) * lda]), 0.0f) / t;
                akp1 = CMPLXF(crealf(A[k + k * lda]), 0.0f) / t;
                akkp1 = work[(k - 1) + 0 * ldw] / t;
                d = t * (ak * akp1 - ONE);
                work[(k - 1) + invd * ldw] = akp1 / d;
                work[k + invd * ldw] = ak / d;
                work[k + (invd + 1) * ldw] = -akkp1 / d;
                work[(k - 1) + (invd + 1) * ldw] = conjf(work[k + (invd + 1) * ldw]);
                k = k - 2;
            }
        }
