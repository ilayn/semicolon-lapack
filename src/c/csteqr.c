/**
 * @file csteqr.c
 * @brief CSTEQR computes all eigenvalues and, optionally, eigenvectors of a
 ...
 * @param[in,out] Z     Single complex array, dimension (ldz, n).
 ...
 * @param[out]    work  Single precision array, dimension (max(1,2*n-2)).
    ...
    if (icompz == 0) {
        ...
        for (INT ii = 1; ii < n; ii++) {
            ...
            if (k != i) {
                D[k] = D[i];
                D[i] = p;
                cblas_cswap(n, &Z[0 + i * ldz], 1, &Z[0 + k * ldz], 1);
            }
        }
    }
}
