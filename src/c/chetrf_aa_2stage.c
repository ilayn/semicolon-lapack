/**
 * @file chetrf_aa_2stage.c
 * @brief CHETRF_AA_2STAGE computes the factorization of a complex hermitian matrix using Aasen's 2-stage algorithm.
 */

#include <complex.h>
#include "semicolon_cblas.h"
#include "semicolon_lapack_complex_single.h"
#include "lapack_tuning.h"

/**
 * CHETRF_AA_2STAGE computes the factorization of a complex Hermitian
 * matrix A using the Aasen's algorithm. The form of the factorization is
 * @rst
 * .. code-block:: text
 *
 *     A = U**H*T*U  or  A = L*T*L**H
 * @endrst
 * where U (or L) is a product of permutation and unit upper (lower)
 * triangular matrices, and T is a Hermitian band matrix with the
 * bandwidth of `nb` (`nb` is internally selected and stored in `TB[0]`,
 * and T is LU factorized with partial pivoting).
 *
 * This is the blocked version of the algorithm, calling Level 3 BLAS.
 *
 * @param[in]     uplo
 *                      - `'U'`: Upper triangle of A is stored
 *                      - `'L'`: Lower triangle of A is stored
 * @param[in]     n     The order of the matrix A. `n>=0`.
 * @param[in,out] A     Array of dimension `(lda,n)`.
 *                      On entry, the Hermitian matrix A. If `uplo='U'`, the
 *                      leading `n`-by-`n` upper triangular part of A
 *                      contains the upper triangular part of the matrix A,
 *                      and the strictly lower triangular part of A is not
 *                      referenced. If `uplo='L'`, the leading `n`-by-`n`
 *                      lower triangular part of A contains the lower
 *                      triangular part of the matrix A, and the strictly
 *                      upper triangular part of A is not referenced.
 *                      On exit, L is stored below (or above) the
 *                      subdiagonal blocks, when `uplo='L'` (or `'U'`).
 * @param[in]     lda   The leading dimension of the array A. `lda>=max(1,n)`.
 * @param[out]    TB    Array of dimension `max(1,ltb)`.
 *                      On exit, details of the LU factorization of the band
 *                      matrix.
 * @param[in]     ltb   The size of the array TB. `ltb>=max(1,4*n)`,
 *                      internally used to select `nb` such that
 *                      `ltb>=(3*nb+1)*n`.
 *                      If `ltb=-1`, then a workspace query is assumed; the
 *                      routine only calculates the optimal size of `ltb`,
 *                      returns this value as the first entry of TB, and no
 *                      error message related to `ltb` is issued.
 * @param[out]    ipiv  Array of dimension `n`.
 *                      On exit, it contains the details of the
 *                      interchanges, i.e., the row and column `k` of A were
 *                      interchanged with the row and column `ipiv[k]`.
 * @param[out]    ipiv2 Array of dimension `n`.
 *                      On exit, it contains the details of the
 *                      interchanges, i.e., the row and column `k` of T were
 *                      interchanged with the row and column `ipiv2[k]`.
 * @param[out]    work  Array of dimension `max(1,lwork)`.
 * @param[in]     lwork The size of `work`. `lwork>=max(1,n)`, internally
 *                      used to select `nb` such that `lwork>=n*nb`.
 *                      If `lwork=-1`, then a workspace query is assumed; the
 *                      routine only calculates the optimal size of the
 *                      `work` array, returns this value as the first entry
 *                      of the `work` array, and no error message related to
 *                      `lwork` is issued.
 * @param[out]    info
 *                         - `info=0`: successful exit
 *                         - `info<0`: if `info=-i`, the i-th argument had an
 *                           illegal value
 *                         - `info>0`: if `info=i`, band LU factorization
 *                           failed on the i-th column
 */
void chetrf_aa_2stage(
    const char* uplo,
    const INT n,
    c64* restrict A,
    const INT lda,
    c64* restrict TB,
    const INT ltb,
    INT* restrict ipiv,
    INT* restrict ipiv2,
    c64* restrict work,
    const INT lwork,
    INT* info)
{
    const c64 ZERO = CMPLXF(0.0f, 0.0f);
    const c64 ONE  = CMPLXF(1.0f, 0.0f);
    const c64 NEG_ONE = CMPLXF(-1.0f, 0.0f);

    INT upper, tquery, wquery;
    INT i, j, k, i1, i2, td;
    INT ldtb, nb, kb, jb, nt, iinfo;
    c64 piv;

    *info = 0;
    upper = (uplo[0] == 'U' || uplo[0] == 'u');
    wquery = (lwork == -1);
    tquery = (ltb == -1);

    if (!upper && !(uplo[0] == 'L' || uplo[0] == 'l')) {
        *info = -1;
    } else if (n < 0) {
        *info = -2;
    } else if (lda < (1 > n ? 1 : n)) {
        *info = -4;
    } else if (ltb < (1 > 4 * n ? 1 : 4 * n) && !tquery) {
        *info = -6;
    } else if (lwork < (1 > n ? 1 : n) && !wquery) {
        *info = -10;
    }

    if (*info != 0) {
        xerbla("CHETRF_AA_2STAGE", -(*info));
        return;
    }

    nb = lapack_get_nb("HETRF");

    if (*info == 0) {
        if (tquery) {
            TB[0] = CMPLXF((f32)((1 > (3 * nb + 1) * n) ? 1 : (3 * nb + 1) * n), 0.0f);
        }
        if (wquery) {
            work[0] = CMPLXF((f32)((1 > n * nb) ? 1 : n * nb), 0.0f);
        }
    }
    if (tquery || wquery) {
        return;
    }

    if (n == 0) {
        return;
    }

    ldtb = ltb / n;
    if (ldtb < 3 * nb + 1) {
        nb = (ldtb - 1) / 3;
    }
    if (lwork < nb * n) {
        nb = lwork / n;
    }

    nt = (n + nb - 1) / nb;
    td = 2 * nb;
    kb = (nb < n) ? nb : n;

    for (j = 0; j < kb; j++) {
        ipiv[j] = j;
    }

    TB[0] = (c64)nb;

    if (upper) {

        for (j = 0; j < nt; j++) {

            kb = (nb < n - j * nb) ? nb : n - j * nb;
            for (i = 1; i <= j - 1; i++) {
                if (i == 1) {
                    if (i == j - 1) {
                        jb = nb + kb;
                    } else {
                        jb = 2 * nb;
                    }
                    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                nb, kb, jb,
                                &ONE, &TB[td + (i * nb) * ldtb], ldtb - 1,
                                &A[(i - 1) * nb + j * nb * lda], lda,
                                &ZERO, &work[i * nb], n);
                } else {
                    if (i == j - 1) {
                        jb = 2 * nb + kb;
                    } else {
                        jb = 3 * nb;
                    }
                    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                nb, kb, jb,
                                &ONE, &TB[td + nb + ((i - 1) * nb) * ldtb], ldtb - 1,
                                &A[(i - 2) * nb + j * nb * lda], lda,
                                &ZERO, &work[i * nb], n);
                }
            }

            clacpy("Upper", kb, kb, &A[j * nb + j * nb * lda], lda,
                   &TB[td + (j * nb) * ldtb], ldtb - 1);
            if (j > 1) {
                cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans,
                            kb, kb, (j - 1) * nb,
                            &NEG_ONE, &A[0 + j * nb * lda], lda,
                            &work[nb], n,
                            &ONE, &TB[td + (j * nb) * ldtb], ldtb - 1);
                cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans,
                            kb, nb, kb,
                            &ONE, &A[(j - 1) * nb + j * nb * lda], lda,
                            &TB[td + nb + ((j - 1) * nb) * ldtb], ldtb - 1,
                            &ZERO, &work[0], n);
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            kb, kb, nb,
                            &NEG_ONE, &work[0], n,
                            &A[(j - 2) * nb + j * nb * lda], lda,
                            &ONE, &TB[td + (j * nb) * ldtb], ldtb - 1);
            }
            if (j > 0) {
                chegst(1, "Upper", kb,
                       &TB[td + (j * nb) * ldtb], ldtb - 1,
                       &A[(j - 1) * nb + j * nb * lda], lda, &iinfo);
            }

            for (i = 0; i < kb; i++) {
                TB[td + (j * nb + i) * ldtb] = crealf(TB[td + (j * nb + i) * ldtb]);
                for (k = i + 1; k < kb; k++) {
                    TB[td + (k - i) + (j * nb + i) * ldtb] =
                        conjf(TB[td - (k - i) + (j * nb + k) * ldtb]);
                }
            }

            if (j < nt - 1) {
                if (j > 0) {

                    if (j == 1) {
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    kb, kb, kb,
                                    &ONE, &TB[td + (j * nb) * ldtb], ldtb - 1,
                                    &A[(j - 1) * nb + j * nb * lda], lda,
                                    &ZERO, &work[j * nb], n);
                    } else {
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    kb, kb, nb + kb,
                                    &ONE, &TB[td + nb + ((j - 1) * nb) * ldtb], ldtb - 1,
                                    &A[(j - 2) * nb + j * nb * lda], lda,
                                    &ZERO, &work[j * nb], n);
                    }

                    cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans,
                                nb, n - (j + 1) * nb, j * nb,
                                &NEG_ONE, &work[nb], n,
                                &A[0 + (j + 1) * nb * lda], lda,
                                &ONE, &A[j * nb + (j + 1) * nb * lda], lda);
                }

                for (k = 0; k < nb; k++) {
                    cblas_ccopy(n - (j + 1) * nb,
                                &A[j * nb + k + (j + 1) * nb * lda], lda,
                                &work[0 + k * n], 1);
                }

                cgetrf(n - (j + 1) * nb, nb,
                       work, n,
                       &ipiv[(j + 1) * nb], &iinfo);

                for (k = 0; k < nb; k++) {
                    INT count = n - (k + 1) - (j + 1) * nb;
                    if (count > 0) {
                        cblas_ccopy(count,
                                    &work[(k + 1) + k * n], 1,
                                    &A[j * nb + k + ((j + 1) * nb + k + 1) * lda], lda);
                    }
                    clacgv(k + 1, &work[k * n], 1);
                }

                kb = (nb < n - (j + 1) * nb) ? nb : n - (j + 1) * nb;
                claset("Full", kb, nb, ZERO, ZERO,
                       &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                clacpy("Upper", kb, nb,
                       work, n,
                       &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                if (j > 0) {
                    cblas_ctrsm(CblasColMajor, CblasRight, CblasUpper,
                                CblasNoTrans, CblasUnit, kb, nb, &ONE,
                                &A[(j - 1) * nb + j * nb * lda], lda,
                                &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                }

                for (k = 0; k < nb; k++) {
                    for (i = 0; i < kb; i++) {
                        TB[td - nb + k - i + (j * nb + nb + i) * ldtb] =
                            conjf(TB[td + nb + i - k + (j * nb + k) * ldtb]);
                    }
                }
                claset("Lower", kb, nb, ZERO, ONE,
                       &A[j * nb + (j + 1) * nb * lda], lda);

                for (k = 0; k < kb; k++) {
                    ipiv[(j + 1) * nb + k] = ipiv[(j + 1) * nb + k] + (j + 1) * nb;

                    i1 = (j + 1) * nb + k;
                    i2 = ipiv[(j + 1) * nb + k];
                    if (i1 != i2) {
                        cblas_cswap(k, &A[(j + 1) * nb + i1 * lda], 1,
                                    &A[(j + 1) * nb + i2 * lda], 1);
                        if (i2 > i1 + 1) {
                            cblas_cswap(i2 - i1 - 1, &A[i1 + (i1 + 1) * lda], lda,
                                        &A[i1 + 1 + i2 * lda], 1);
                            clacgv(i2 - i1 - 1, &A[i1 + 1 + i2 * lda], 1);
                        }
                        clacgv(i2 - i1, &A[i1 + (i1 + 1) * lda], lda);
                        if (i2 < n - 1) {
                            cblas_cswap(n - i2 - 1, &A[i1 + (i2 + 1) * lda], lda,
                                        &A[i2 + (i2 + 1) * lda], lda);
                        }
                        piv = A[i1 + i1 * lda];
                        A[i1 + i1 * lda] = A[i2 + i2 * lda];
                        A[i2 + i2 * lda] = piv;
                        if (j > 0) {
                            cblas_cswap(j * nb, &A[0 + i1 * lda], 1,
                                        &A[0 + i2 * lda], 1);
                        }
                    }
                }
            }
        }

    } else {

        for (j = 0; j < nt; j++) {

            kb = (nb < n - j * nb) ? nb : n - j * nb;
            for (i = 1; i <= j - 1; i++) {
                if (i == 1) {
                    if (i == j - 1) {
                        jb = nb + kb;
                    } else {
                        jb = 2 * nb;
                    }
                    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasConjTrans,
                                nb, kb, jb,
                                &ONE, &TB[td + (i * nb) * ldtb], ldtb - 1,
                                &A[j * nb + (i - 1) * nb * lda], lda,
                                &ZERO, &work[i * nb], n);
                } else {
                    if (i == j - 1) {
                        jb = 2 * nb + kb;
                    } else {
                        jb = 3 * nb;
                    }
                    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasConjTrans,
                                nb, kb, jb,
                                &ONE, &TB[td + nb + ((i - 1) * nb) * ldtb], ldtb - 1,
                                &A[j * nb + (i - 2) * nb * lda], lda,
                                &ZERO, &work[i * nb], n);
                }
            }

            clacpy("Lower", kb, kb, &A[j * nb + j * nb * lda], lda,
                   &TB[td + (j * nb) * ldtb], ldtb - 1);
            if (j > 1) {
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            kb, kb, (j - 1) * nb,
                            &NEG_ONE, &A[j * nb + 0 * lda], lda,
                            &work[nb], n,
                            &ONE, &TB[td + (j * nb) * ldtb], ldtb - 1);
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            kb, nb, kb,
                            &ONE, &A[j * nb + (j - 1) * nb * lda], lda,
                            &TB[td + nb + ((j - 1) * nb) * ldtb], ldtb - 1,
                            &ZERO, &work[0], n);
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasConjTrans,
                            kb, kb, nb,
                            &NEG_ONE, &work[0], n,
                            &A[j * nb + (j - 2) * nb * lda], lda,
                            &ONE, &TB[td + (j * nb) * ldtb], ldtb - 1);
            }
            if (j > 0) {
                chegst(1, "Lower", kb,
                       &TB[td + (j * nb) * ldtb], ldtb - 1,
                       &A[j * nb + (j - 1) * nb * lda], lda, &iinfo);
            }

            for (i = 0; i < kb; i++) {
                TB[td + (j * nb + i) * ldtb] = crealf(TB[td + (j * nb + i) * ldtb]);
                for (k = i + 1; k < kb; k++) {
                    TB[td - (k - i) + (j * nb + k) * ldtb] =
                        conjf(TB[td + (k - i) + (j * nb + i) * ldtb]);
                }
            }

            if (j < nt - 1) {
                if (j > 0) {

                    if (j == 1) {
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasConjTrans,
                                    kb, kb, kb,
                                    &ONE, &TB[td + (j * nb) * ldtb], ldtb - 1,
                                    &A[j * nb + (j - 1) * nb * lda], lda,
                                    &ZERO, &work[j * nb], n);
                    } else {
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasConjTrans,
                                    kb, kb, nb + kb,
                                    &ONE, &TB[td + nb + ((j - 1) * nb) * ldtb], ldtb - 1,
                                    &A[j * nb + (j - 2) * nb * lda], lda,
                                    &ZERO, &work[j * nb], n);
                    }

                    cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                n - (j + 1) * nb, nb, j * nb,
                                &NEG_ONE, &A[(j + 1) * nb + 0 * lda], lda,
                                &work[nb], n,
                                &ONE, &A[(j + 1) * nb + j * nb * lda], lda);
                }

                cgetrf(n - (j + 1) * nb, nb,
                       &A[(j + 1) * nb + j * nb * lda], lda,
                       &ipiv[(j + 1) * nb], &iinfo);

                kb = (nb < n - (j + 1) * nb) ? nb : n - (j + 1) * nb;
                claset("Full", kb, nb, ZERO, ZERO,
                       &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                clacpy("Upper", kb, nb,
                       &A[(j + 1) * nb + j * nb * lda], lda,
                       &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                if (j > 0) {
                    cblas_ctrsm(CblasColMajor, CblasRight, CblasLower,
                                CblasConjTrans, CblasUnit, kb, nb, &ONE,
                                &A[j * nb + (j - 1) * nb * lda], lda,
                                &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                }

                for (k = 0; k < nb; k++) {
                    for (i = 0; i < kb; i++) {
                        TB[td - nb + k - i + (j * nb + nb + i) * ldtb] =
                            conjf(TB[td + nb + i - k + (j * nb + k) * ldtb]);
                    }
                }
                claset("Upper", kb, nb, ZERO, ONE,
                       &A[(j + 1) * nb + j * nb * lda], lda);

                for (k = 0; k < kb; k++) {
                    ipiv[(j + 1) * nb + k] = ipiv[(j + 1) * nb + k] + (j + 1) * nb;

                    i1 = (j + 1) * nb + k;
                    i2 = ipiv[(j + 1) * nb + k];
                    if (i1 != i2) {
                        cblas_cswap(k, &A[i1 + (j + 1) * nb * lda], lda,
                                    &A[i2 + (j + 1) * nb * lda], lda);
                        if (i2 > i1 + 1) {
                            cblas_cswap(i2 - i1 - 1, &A[i1 + 1 + i1 * lda], 1,
                                        &A[i2 + (i1 + 1) * lda], lda);
                            clacgv(i2 - i1 - 1, &A[i2 + (i1 + 1) * lda], lda);
                        }
                        clacgv(i2 - i1, &A[i1 + 1 + i1 * lda], 1);
                        if (i2 < n - 1) {
                            cblas_cswap(n - i2 - 1, &A[i2 + 1 + i1 * lda], 1,
                                        &A[i2 + 1 + i2 * lda], 1);
                        }
                        piv = A[i1 + i1 * lda];
                        A[i1 + i1 * lda] = A[i2 + i2 * lda];
                        A[i2 + i2 * lda] = piv;
                        if (j > 0) {
                            cblas_cswap(j * nb, &A[i1 + 0 * lda], lda,
                                        &A[i2 + 0 * lda], lda);
                        }
                    }
                }
            }
        }
    }

    cgbtrf(n, n, nb, nb, TB, ldtb, ipiv2, info);
}
