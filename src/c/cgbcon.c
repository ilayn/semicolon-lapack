        ...
        if (kase == kase1) {
            ...
            if (lnoti) {
                for (j = 0; j < n - 1; j++) {
                    ...
                    c64 neg_t = -t;
                    cblas_caxpy(lm, &neg_t, &AB[kd + 1 + j * ldab], 1, &work[j + 1], 1);
                }
            }

            /* Multiply by inv(U) */
            clatbs("U", "N", "N", &normin, n, kl + ku, AB, ldab, work, &scale,
                   rwork, &linfo);
        } else {
            /* Multiply by inv(U**H) */
            clatbs("U", "C", "N", &normin, n, kl + ku, AB, ldab, work, &scale,
                   rwork, &linfo);
            ...
            if (lnoti) {
                for (j = n - 2; j >= 0; j--) {
                    ...
                    c64 dotc;
                    cblas_cdotc_sub(lm, &AB[kd + 1 + j * ldab], 1, &work[j + 1], 1, &dotc);
                    work[j] -= dotc;
                    jp = ipiv[j];
                    if (jp != j) {
                        t = work[jp];
