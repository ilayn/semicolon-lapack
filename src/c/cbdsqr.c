/**
 * @file cbdsqr.c
 * @brief CBDSQR computes the SVD of a real bidiagonal matrix using the
 ...
 * @param[out]    rwork Single precision array, dimension (lrwork).
 ...
 */
void cbdsqr(const char* uplo, const INT n, const INT ncvt, const INT nru,
            ...
            f32* restrict rwork, INT* info)
{
    ...
    *info = 0;
    ...
    if (lower) {
        ...
        if (nru > 0) {
            clasr("R", "V", "F", nru, n, rwork, &rwork[nm1], U, ldu);
        }
        if (ncc > 0) {
            clasr("L", "V", "F", n, ncc, rwork, &rwork[nm1], C, ldc);
        }
    }
...
L90:
    ...
    if (ll == m - 2) {
        ...
        if (ncvt > 0) {
            cblas_csrot(ncvt, &VT[m - 2], ldvt, &VT[m - 1], ldvt,
                       cosr, sinr);
        }
        if (nru > 0) {
            cblas_csrot(nru, &U[(m - 2) * ldu], 1, &U[(m - 1) * ldu], 1, cosl, sinl);
        }
        if (ncc > 0) {
            cblas_csrot(ncc, &C[m - 2], ldc, &C[m - 1], ldc, cosl, sinl);
        }
        m = m - 2;
        goto L60;
    }
    ...
    if (shift == ZERO) {
        if (idir == 1) {
            ...
            if (ncvt > 0) {
                clasr("L", "V", "F", m - ll, ncvt, rwork,
                      &rwork[nm1], &VT[ll], ldvt);
            }
            if (nru > 0) {
                clasr("R", "V", "F", nru, m - ll,
                      &rwork[nm12], &rwork[nm13], &U[ll * ldu], ldu);
            }
            if (ncc > 0) {
                clasr("L", "V", "F", m - ll, ncc,
            ...
            /* Update singular vectors */
            if (ncvt > 0) {
                clasr("L", "V", "B", m - ll, ncvt,
                      &rwork[nm12], &rwork[nm13], &VT[ll], ldvt);
            }
            if (nru > 0) {
                clasr("R", "V", "B", nru, m - ll, rwork,
                      &rwork[nm1], &U[ll * ldu], ldu);
            }
            if (ncc > 0) {
                clasr("L", "V", "B", m - ll, ncc, rwork,
                      &rwork[nm1], &C[ll], ldc);
            }
...
L160:
    for (i = 0; i < n; i++) {
        ...
        if (D[i] < ZERO) {
            ...
            if (ncvt > 0) {
                cblas_csscal(ncvt, NEGONE, &VT[i], ldvt);
            }
        }
    }
    ...
    for (i = 0; i < n - 1; i++) {
        ...
        if (isub != n - 1 - i) {
            ...
            D[n - 1 - i] = smin;
            if (ncvt > 0) {
                cblas_cswap(ncvt, &VT[isub], ldvt, &VT[n - 1 - i], ldvt);
            }
            if (nru > 0) {
                cblas_cswap(nru, &U[isub * ldu], 1, &U[(n - 1 - i) * ldu], 1);
            }
            if (ncc > 0) {
                cblas_cswap(ncc, &C[isub], ldc, &C[n - 1 - i], ldc);
            }
        }
    }
    return;
