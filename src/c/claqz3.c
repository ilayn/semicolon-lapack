    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (lwork == -1) {
        ...
        work[0] = CMPLXF((f32)(n * nblock_desired), 0.0f);
        return;
    ...
    claset("F", ns + 1, ns + 1, CZERO, CONE, QC, ldqc);
    claset("F", ns, ns, CZERO, CONE, ZC, ldzc);

    for (i = 0; i < ns; i++) {
        /* Introduce the shift */
        scale = sqrtf(cabsf(alpha[i])) * sqrtf(cabsf(beta[i]));
        ...
        if (cabsf(temp2) > safmax || cabsf(temp3) > safmax) {
            temp2 = CONE;
            temp3 = CZERO;
        }

        clartg(temp2, temp3, &c, &s, &temp);
        crot(ns, &A[ilo + ilo * lda], lda, &A[(ilo + 1) + ilo * lda], lda, c, s);
        crot(ns, &B[ilo + ilo * ldb], ldb, &B[(ilo + 1) + ilo * ldb], ldb, c, s);
        crot(ns + 1, &QC[0], 1, &QC[ldqc], 1, c, conjf(s));

        /* Chase the shift down */
        for (j = 0; j < ns - 1 - i; j++) {
            claqz1(1, 1, j, 0, ns - 1, ihi - ilo,
    ...
    if (swidth > 0) {
        cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, sheight, swidth, sheight,
                    &CONE, QC, ldqc, &A[ilo + (ilo + ns) * lda], lda, &CZERO, work, sheight);
        clacpy("A", sheight, swidth, work, sheight, &A[ilo + (ilo + ns) * lda], lda);
        cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, sheight, swidth, sheight,
                    &CONE, QC, ldqc, &B[ilo + (ilo + ns) * ldb], ldb, &CZERO, work, sheight);
        clacpy("A", sheight, swidth, work, sheight, &B[ilo + (ilo + ns) * ldb], ldb);
    }
    if (ilq) {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, sheight, sheight,
                    &CONE, &Q[0 + ilo * ldq], ldq, QC, ldqc, &CZERO, work, n);
        clacpy("A", n, sheight, work, n, &Q[0 + ilo * ldq], ldq);
    }
    ...
    swidth = ns;
    if (sheight > 0) {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                    &CONE, &A[istartm + ilo * lda], lda, ZC, ldzc, &CZERO, work, sheight);
        clacpy("A", sheight, swidth, work, sheight, &A[istartm + ilo * lda], lda);
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                    &CONE, &B[istartm + ilo * ldb], ldb, ZC, ldzc, &CZERO, work, sheight);
        clacpy("A", sheight, swidth, work, sheight, &B[istartm + ilo * ldb], ldb);
    }
    if (ilz) {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, swidth, swidth,
                    &CONE, &Z[0 + ilo * ldz], ldz, ZC, ldzc, &CZERO, work, n);
        clacpy("A", n, swidth, work, n, &Z[0 + ilo * ldz], ldz);
    }
    ...
    while (k < ihi - ns) {
        ...
        claset("F", ns + np, ns + np, CZERO, CONE, QC, ldqc);
        claset("F", ns + np, ns + np, CZERO, CONE, ZC, ldzc);

        /* Near the diagonal shift chase */
        for (i = ns - 1; i >= 0; i--) {
            for (j = 0; j <= np - 1; j++) {
                claqz1(1, 1, k + i + j, istartb, istopb, ihi,
        ...
        if (swidth > 0) {
            ...
            clacpy("A", sheight, swidth, work, sheight, &A[(k + 1) + (k + ns + np) * lda], lda);
            cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, sheight, swidth, sheight,
                        &CONE, QC, ldqc, &B[(k + 1) + (k + ns + np) * ldb], ldb, &CZERO, work, sheight);
            clacpy("A", sheight, swidth, work, sheight, &B[(k + 1) + (k + ns + np) * ldb], ldb);
        }
        if (ilq) {
            cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, nblock, nblock,
                        &CONE, &Q[0 + (k + 1) * ldq], ldq, QC, ldqc, &CZERO, work, n);
            clacpy("A", n, nblock, work, n, &Q[0 + (k + 1) * ldq], ldq);
        }
        ...
        if (sheight > 0) {
            ...
            clacpy("A", sheight, swidth, work, sheight, &A[istartm + k * lda], lda);
            cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                        &CONE, &B[istartm + k * ldb], ldb, ZC, ldzc, &CZERO, work, sheight);
            clacpy("A", sheight, swidth, work, sheight, &B[istartm + k * ldb], ldb);
        }
        if (ilz) {
            cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, nblock, nblock,
                        &CONE, &Z[0 + k * ldz], ldz, ZC, ldzc, &CZERO, work, n);
            clacpy("A", n, nblock, work, n, &Z[0 + k * ldz], ldz);
        }
    ...
    claset("F", ns, ns, CZERO, CONE, QC, ldqc);
    claset("F", ns + 1, ns + 1, CZERO, CONE, ZC, ldzc);
    ...
    for (i = 0; i < ns; i++) {
        ...
        for (ishift = ihi - i - 1; ishift <= ihi - 1; ishift++) {
            claqz1(1, 1, ishift, istartb, istopb, ihi,
    ...
    if (swidth > 0) {
        ...
        clacpy("A", sheight, swidth, work, sheight, &A[(ihi - ns + 1) + (ihi + 1) * lda], lda);
        cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, sheight, swidth, sheight,
                    &CONE, QC, ldqc, &B[(ihi - ns + 1) + (ihi + 1) * ldb], ldb, &CZERO, work, sheight);
        clacpy("A", sheight, swidth, work, sheight, &B[(ihi - ns + 1) + (ihi + 1) * ldb], ldb);
    }
    if (ilq) {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, ns, ns,
                    &CONE, &Q[0 + (ihi - ns + 1) * ldq], ldq, QC, ldqc, &CZERO, work, n);
        clacpy("A", n, ns, work, n, &Q[0 + (ihi - ns + 1) * ldq], ldq);
    }
    ...
    if (sheight > 0) {
        ...
        clacpy("A", sheight, swidth, work, sheight, &A[istartm + (ihi - ns) * lda], lda);
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                    &CONE, &B[istartm + (ihi - ns) * ldb], ldb, ZC, ldzc, &CZERO, work, sheight);
        clacpy("A", sheight, swidth, work, sheight, &B[istartm + (ihi - ns) * ldb], ldb);
    }
    if (ilz) {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, ns + 1, ns + 1,
                    &CONE, &Z[0 + (ihi - ns) * ldz], ldz, ZC, ldzc, &CZERO, work, n);
        clacpy("A", n, ns + 1, work, n, &Z[0 + (ihi - ns) * ldz], ldz);
    }
}
