            ...
            c64* T, const int ldt, c64* Y, const int ldy)
{
    ...
    for (i = 0; i < nb; i++) {
        if (i > 0) {
            ...
            clacgv(i, &A[k + i - 1], lda);
            cblas_cgemv(CblasColMajor, CblasNoTrans, n - k, i, &NEG_ONE,
            ...
            /* Apply I - V * T**H * V**H to this column (call it b) from the
               ...
               w := V1**H * b1 */
            cblas_ccopy(i, &A[k + i * lda], 1, &T[(nb - 1) * ldt], 1);
            cblas_ctrmv(CblasColMajor, CblasLower, CblasConjTrans, CblasUnit,
                        i, &A[k], lda, &T[(nb - 1) * ldt], 1);

            /* w := w + V2**H * b2 */
            cblas_cgemv(CblasColMajor, CblasConjTrans, n - k - i, i,
            ...
            /* b2 := b2 - V2*w */
            cblas_cgemv(CblasColMajor, CblasNoTrans, n - k - i, i, &NEG_ONE,
            ...
            cblas_ctrmv(CblasColMajor, CblasLower, CblasNoTrans, CblasUnit,
                        i, &A[k], lda, &T[(nb - 1) * ldt], 1);
            cblas_caxpy(i, &NEG_ONE, &T[(nb - 1) * ldt], 1, &A[k + i * lda], 1);
        ...
        clarfg(len, &A[k + i + i * lda], &A[start + i * lda], 1, &tau[i]);
        ei = A[k + i + i * lda];
        A[k + i + i * lda] = ONE;

        /* Compute Y(k:n-1, i) */
        cblas_cgemv(CblasColMajor, CblasNoTrans, n - k, len,
        ...
        cblas_cscal(n - k, &tau[i], &Y[k + i * ldy], 1);
    ...
    clacpy("All", k, nb, &A[lda], lda, Y, ldy);
    cblas_ctrmm(CblasColMajor, CblasRight, CblasLower, CblasNoTrans, CblasUnit,
                k, nb, &ONE, &A[k], lda, Y, ldy);
    if (n > k + nb) {
        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, k, nb, n - k - nb,
