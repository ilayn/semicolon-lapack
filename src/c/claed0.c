/**
 * @file claed0.c
 * @brief CLAED0 used by CSTEDC. Computes all eigenvalues and corresponding
 ...
 *                        LDQS >= max(1,N).
 * @param[out]    rwork  Single precision array,
            ...
            f32* rwork, INT* iwork, INT* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i <= spm1; i++) {
        ...
        clacrm(qsiz, matsiz, &Q[submat * ldq], ldq, &rwork[ll],
    ...
    while (subpbs > 1) {
        ...
        for (i = 0; i <= spm2; i += 2) {
             ...
             */
            claed7(matsiz, msd2, qsiz, tlvls, curlvl, curprb,
    ...
    for (i = 0; i < n; i++) {
        j = iwork[indxq + i];
        rwork[i] = D[j];
        cblas_ccopy(qsiz, &qstore[j * ldqs], 1, &Q[i * ldq], 1);
    }
    cblas_scopy(n, rwork, 1, D, 1);
