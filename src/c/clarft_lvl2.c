/**
 * @file clarft_lvl2.c
 * @brief CLARFT_LVL2 Level 2 BLAS version for terminating case of CLARFT.
 */
        ...
        for (i = 1; i <= k; i++) {
            ...
            if (tau[i - 1] == ZERO) {
                ...
                if (storev[0] == 'C' || storev[0] == 'c') {
                    ...
                    for (j = 1; j <= i - 1; j++) {
                        T[(j - 1) + (i - 1) * ldt] = -tau[i - 1] * conjf(V[(i - 1) + (j - 1) * ldv]);
                    }
                    j = (lastv < prevlastv) ? lastv : prevlastv;

                    c64 neg_tau_i = -tau[i - 1];
                    cblas_cgemv(CblasColMajor, CblasConjTrans, j - i, i - 1,
            ...
            if (tau[i - 1] == ZERO) {
                ...
                if (i < k) {
                    if (storev[0] == 'C' || storev[0] == 'c') {
                        ...
                        for (j = i + 1; j <= k; j++) {
                            T[(j - 1) + (i - 1) * ldt] = -tau[i - 1] * conjf(V[(n - k + i - 1) + (j - 1) * ldv]);
                        }
                        j = (lastv > prevlastv) ? lastv : prevlastv;
