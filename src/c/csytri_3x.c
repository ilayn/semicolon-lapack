 ...
 * matrix A using the factorization computed by CSYTRF_RK or ZSYTRF_BK:
 *
 ...
 *                      and factors U or L as computed by `csytrf_rk` or
 *                      `csytrf_bk`:
 *
 ...
 *                      of D as determined by `csytrf_rk` or `csytrf_bk`.
 * @param[out]    work  Complex array of dimension `(n+nb+1,nb+3)`.
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (cut < n) {
            ...
            cblas_ctrmm(CblasColMajor, CblasLeft, CblasLower, CblasTrans, CblasUnit,
                        nnb, nnb, &ONE, &A[cut + cut * lda], lda, &work[u11 + 0 * ldwork], ldwork);
