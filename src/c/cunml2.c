/**
 * @file cunml2.c
 * @brief CUNML2 multiplies a general matrix by the unitary matrix from a LQ
 ...
 *                        as returned by CGELQF in the first k rows of its
 *                        array argument A.
 ...
 *                        elementary reflector H(i), as returned by CGELQF.
 ...
 *                             value.
 */
void cunml2(const char* side, const char* trans,
            ...
            INT* info)
{
    ...
    *info = 0;
    ...
    for (i = i1; i != i2; i += i3) {
        ...
        if (i < nq - 1) {
            clacgv(nq - i - 1, &A[i + (i + 1) * lda], lda);
        }
        clarf1f(side, mi, ni, &A[i + i * lda], lda, taui,
