/**
 * @file clarzb.c
 * @brief CLARZB applies a block reflector or its conjugate-transpose to a
            ...
            c64* restrict work, const INT ldwork)
{
    ...
    if (side[0] == 'L' || side[0] == 'l') {
         ...
         */
        for (j = 0; j < k; j++) {
            cblas_ccopy(n, &C[j + 0 * ldc], ldc, &work[0 + j * ldwork], 1);
        }
         ...
         */
        if (l > 0) {
            cblas_cgemm(CblasColMajor, CblasTrans, CblasConjTrans,
                        n, k, l, &ONE,
         ...
         */
        if (l > 0) {
            cblas_cgemm(CblasColMajor, CblasTrans, CblasTrans,
                        l, n, k, &NEG_ONE, V, ldv,
         ...
         */
        for (j = 0; j < k; j++) {
            cblas_ccopy(m, &C[0 + j * ldc], 1, &work[0 + j * ldwork], 1);
        }
