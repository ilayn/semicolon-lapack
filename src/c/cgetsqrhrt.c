/**
 * @file cgetsqrhrt.c
 * @brief CGETSQRHRT computes a NB2-sized column blocked QR-factorization of a complex M-by-N matrix A using TSQR and Householder reconstruction.
 */
 ...
 *
 * @param[out] T
 *          Single complex array, dimension (ldt, n).
    ...
    INT* info)
{
    ...
    *info = 0;
            ...
            *info = -9;
            ...
            if (lwork < n * n + 1 && !lquery) {
                *info = -11;
                ...
                if (m - n > 0) {
                    num_all_row_blocks = (INT)ceilf((f32)(m - n) / (f32)(mb1 - n));
                } else {
    ...
    clatsqr(m, n, mb1, nb1local, A, lda, work, ldwt, &work[lwt], lw1, &iinfo);

    for (j = 0; j < n; j++) {
        cblas_ccopy(j + 1, &A[0 + j * lda], 1, &work[lwt + n * j], 1);
    }

    cungtsqr_row(m, n, mb1, nb1local, A, lda, work, ldwt, &work[lwt + n * n], lw2, &iinfo);

    cunhr_col(m, n, nb2local, A, lda, T, ldt, &work[lwt + n * n], &iinfo);

    for (i = 0; i < n; i++) {
        if (work[lwt + n * n + i] == -CONE) {
            for (j = i; j < n; j++) {
                A[i + j * lda] = -CONE * work[lwt + n * j + i];
            }
        } else {
            cblas_ccopy(n - i, &work[lwt + n * i + i], n, &A[i + i * lda], lda);
        }
    }
