/**
 * @file claed7.c
 * @brief CLAED7 computes the updated eigensystem of a diagonal matrix after
 ...
 * @param[in,out] qstore  Single precision array, dimension (N**2+1).
 ...
 * @param[in]     givnum  Single precision array, dimension (2 * N lg N).
 *                        Each number indicates the S value to be used in the
 *                        corresponding Givens rotation.
 * @param[out]    work    Complex array, dimension (QSIZ*N).
 * @param[out]    rwork   Single precision array, dimension (3*N+2*QSIZ*N).
            ...
            f32* rwork, INT* iwork, INT* info)
{
    ...
    *info = 0;
     ...
     */
    rho_local = rho;
    claed8(&k, n, qsiz, Q, ldq, D, &rho_local, cutpnt, &rwork[iz],
     ...
     */
    if (k != 0) {
        slaed9(k, 0, k, n, D, &rwork[iq], k, rho_local,
               &rwork[idlmda], &rwork[iw],
               &qstore[qptr[curr]], k, info);
        clacrm(qsiz, k, work, qsiz, &qstore[qptr[curr]], k,
               Q, ldq, &rwork[iq]);
