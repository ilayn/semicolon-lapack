/**
 * @file chpsvx.c
 * @brief CHPSVX computes the solution to a complex system of linear equations A * X = B for Hermitian packed matrices (expert driver).
 */
 ...
 * @param[in,out] AFP    If fact = 'F', contains the factored form from CHPTRF.
 *                       If fact = 'N', on exit contains the factorization.
 *                       Array of dimension (n*(n+1)/2).
 * @param[in,out] ipiv   If fact = 'F', contains the pivot details from CHPTRF.
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    chprfs(uplo, n, nrhs, AP, AFP, ipiv, B, ldb, X, ldx, ferr,
