            ...
            c64* work, const int lwork, int* info)
{
    ...
    *info = 0;
    ...
    if (wants) {
         ...
         */
        clacpy("F", n1, n2, &T[n1 * ldt], ldt, work, n1);
        ctrsyl("N", "N", -1, n1, n2, T, ldt, &T[n1 + n1 * ldt],
        ...
        if (rnorm == ZERO) {
            ...
            *s = scale / (sqrtf(scale * scale / rnorm + rnorm) *
                 sqrtf(rnorm));
        }
    }
    ...
    if (wantsp) {
        ...
        kase = 0;
        for (;;) {
            clacn2(nn, &work[nn], work, &est, &kase, isave);
            if (kase == 0)
                break;
            if (kase == 1) {
                /* Solve T11*R - R*T22 = scale*X. */
                ctrsyl("N", "N", -1, n1, n2, T, ldt,
                       &T[n1 + n1 * ldt], ldt, work, n1, &scale,
                       &ierr);
            } else {
                /* Solve T11**H*R - R*T22**H = scale*X. */
                ctrsyl("C", "C", -1, n1, n2, T, ldt,
                       &T[n1 + n1 * ldt], ldt, work, n1, &scale,
                       &ierr);
            }
        }
