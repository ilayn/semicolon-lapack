/**
 * @file clanhb.c
 * @brief CLANHB returns the value of the 1-norm, Frobenius norm, infinity norm, or max element of a Hermitian band matrix.
 */
    ...
    if (n == 0) {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                ...
                sum = fabsf(crealf(AB[k + j * ldab]));
                if (value < sum || sisnan(sum)) value = sum;
            }
        } else {
            for (j = 0; j < n; j++) {
                sum = fabsf(crealf(AB[j * ldab]));
                if (value < sum || sisnan(sum)) value = sum;
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                ...
                for (i = istart; i < j; i++) {
                    absa = cabsf(AB[l + i + j * ldab]);
                    sum = sum + absa;
                    work[i] = work[i] + absa;
                }
                work[j] = sum + fabsf(crealf(AB[k + j * ldab]));
            }
            for (i = 0; i < n; i++) {
                sum = work[i];
                if (value < sum || sisnan(sum)) value = sum;
            ...
            for (j = 0; j < n; j++) {
                sum = work[j] + fabsf(crealf(AB[j * ldab]));
                l = -j;
                INT iend = (n < j + k + 1) ? n : j + k + 1;
                for (i = j + 1; i < iend; i++) {
                    absa = cabsf(AB[l + i + j * ldab]);
        ...
        if (k > 0) {
            if (uplo[0] == 'U' || uplo[0] == 'u') {
                for (j = 1; j < n; j++) {
                    ...
                    classq(len, &AB[start + j * ldab], 1, &scale, &sum);
                }
                l = k;
            } else {
                for (j = 0; j < n - 1; j++) {
                    INT len = (n - j - 1 < k) ? n - j - 1 : k;
                    classq(len, &AB[1 + j * ldab], 1, &scale, &sum);
                }
                l = 0;
            }
            sum = 2 * sum;
        ...
        for (j = 0; j < n; j++) {
            if (crealf(AB[l + j * ldab]) != ZERO) {
                absa = fabsf(crealf(AB[l + j * ldab]));
                if (scale < absa) {
