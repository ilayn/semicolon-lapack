/**
 * @file cung2r.c
 * @brief CUNG2R generates all or part of the unitary matrix Q from
 *        a QR factorization determined by CGEQRF (unblocked algorithm).
 */
 ...
 *                      i = 0,1,...,k-1, as returned by CGEQRF in the first
            ...
            INT* info)
{
    ...
    *info = 0;
    ...
    for (i = k - 1; i >= 0; i--) {
        ...
        if (i < n - 1) {
            clarf1f("L", m - i, n - i - 1, &A[i + i * lda], 1, tau[i],
                    &A[i + (i + 1) * lda], lda, work);
        }
        if (i < m - 1) {
            c64 neg_tau = -tau[i];
            cblas_cscal(m - i - 1, &neg_tau, &A[(i + 1) + i * lda], 1);
        }
        A[i + i * lda] = ONE - tau[i];
