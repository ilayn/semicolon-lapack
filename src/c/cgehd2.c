/**
 * @file cgehd2.c
 * @brief CGEHD2 reduces a general square matrix to upper Hessenberg form
 ...
 *                       are normally set by a previous call to CGEBAL;
            ...
            c64* work, INT* info)
{
    ...
    *info = 0;
    ...
    for (i = ilo; i <= ihi - 1; i++) {
        ...
        clarfg(len, &A[(i + 1) + i * lda], &A[start + i * lda], 1, &tau[i]);

        /* Apply H(i) to A(0:ihi, i+1:ihi) from the right */
        clarf1f("Right", ihi + 1, len, &A[(i + 1) + i * lda], 1, tau[i],
                &A[(i + 1) * lda], lda, work);

        /* Apply H(i)**H to A(i+1:ihi, i+1:n-1) from the left */
        c64 conjtau = conjf(tau[i]);
        clarf1f("Left", len, n - i - 1, &A[(i + 1) + i * lda], 1, conjtau,
