 ...
 *                   CGGBAL; otherwise they should be set to 1 and N
 *                   respectively.
    ...
    int* info)
{
    ...
    f32 c;
    c64 c1, c2, ctemp, s, s1, s2, temp, temp1, temp2, temp3;
    ...
    *info = 0;
    ...
    if (initq)
        claset("A", n, n, CZERO, CONE, Q, ldq);
    if (initz)
        claset("A", n, n, CZERO, CONE, Z, ldz);

    if (n > 1)
        claset("L", n - 1, n - 1, CZERO, CZERO, &B[1], ldb);
    ...
    if (nb < nbmin || nb >= nh) {
        ...
        for (jcol = ilo - 1; jcol <= ihi - 3; jcol += nb) {
            ...
            claset("A", nblst, nblst, CZERO, CONE, work, nblst);
            pw = nblst * nblst;
            for (i = 0; i < n2nb; i++) {
                claset("A", 2 * nnb, 2 * nnb, CZERO, CONE, &work[pw], 2 * nnb);
                pw = pw + 4 * nnb * nnb;
            }
            ...
            for (j = jcol; j <= jcol + nnb - 1; j++) {
                for (i = ihi - 1; i >= j + 2; i--) {
                    temp = A[(i - 1) + j * lda];
                    clartg(temp, A[i + j * lda], &c, &s, &A[(i - 1) + j * lda]);
                    A[i + j * lda] = CMPLXF(c, 0.0f);
                    B[i + j * ldb] = s;
                }
                ...
                for (i = ihi - 1; i >= jrow; i--) {
                    ...
                    for (jj = ppw; jj <= ppw + len - 1; jj++) {
                        ...
                        work[jj] = conjf(s) * temp + ctemp * work[jj];
                    }
                    len = len + 1;
                    ppw = ppw - nblst - 1;
                }
                ...
                for (jj = n - 1; jj >= j + 1; jj--) {
                    for (i = (jj + 1 < ihi - 1 ? jj + 1 : ihi - 1); i >= j + 2; i--) {
                        ...
                        s = B[i + j * ldb];
                        temp = B[i + jj * ldb];
                        B[i + jj * ldb] = ctemp * temp - conjf(s) * B[(i - 1) + jj * ldb];
                        B[(i - 1) + jj * ldb] = s * temp + ctemp * B[(i - 1) + jj * ldb];
                    }

                    if (jj < ihi - 1) {
                        temp = B[(jj + 1) + (jj + 1) * ldb];
                        clartg(temp, B[(jj + 1) + jj * ldb], &c, &s, &B[(jj + 1) + (jj + 1) * ldb]);
                        B[(jj + 1) + jj * ldb] = CZERO;
                        crot(jj - top, &B[top + (jj + 1) * ldb], 1,
                             &B[top + jj * ldb], 1, c, s);
                        A[(jj + 1) + j * lda] = CMPLXF(c, 0.0f);
                        B[(jj + 1) + j * ldb] = -conjf(s);
                    }
                }
                ...
                for (i = ihi - j - 4; i >= jj + 1; i -= 3) {
                    ...
                    for (k = top; k <= ihi - 1; k++) {
                        ...
                        temp2 = A[k + (j + i + 2) * lda];
                        temp3 = A[k + (j + i + 3) * lda];
                        A[k + (j + i + 3) * lda] = c2 * temp3 + conjf(s2) * temp2;
                        temp2 = -s2 * temp3 + c2 * temp2;
                        A[k + (j + i + 2) * lda] = c1 * temp2 + conjf(s1) * temp1;
                        temp1 = -s1 * temp2 + c1 * temp1;
                        A[k + (j + i + 1) * lda] = ctemp * temp1 + conjf(s) * temp;
                ...
                if (jj > 0) {
                    for (i = jj; i >= 1; i--) {
                        c = crealf(A[(j + 1 + i) + j * lda]);
                        crot(ihi - top, &A[top + (j + i + 1) * lda], 1,
                             &A[top + (j + i) * lda], 1, c,
                             -conjf(B[(j + 1 + i) + j * ldb]));
                    }
                }
                ...
                if (j < jcol + nnb - 1) {
                    ...
                    jrow = ihi - nblst;
                    cblas_cgemv(CblasColMajor, CblasConjTrans, nblst, len, &CONE, work,
                    ...
                    cblas_ctrmv(CblasColMajor, CblasLower, CblasConjTrans, CblasNonUnit,
                                ...
                                &work[pw + len], 1);
                    cblas_cgemv(CblasColMajor, CblasConjTrans, len, nblst - len, &CONE,
                    ...
                    for (jrow = j0; jrow >= jcol + 1; jrow -= nnb) {
                        ...
                        cblas_ctrmv(CblasColMajor, CblasLower, CblasConjTrans, CblasNonUnit,
                                    ...
                                    2 * nnb, &work[pw + len], 1);
                        cblas_cgemv(CblasColMajor, CblasConjTrans, nnb, len, &CONE,
                                    &work[ppwo], 2 * nnb, &A[jrow + (j + 1) * lda], 1,
                                    &CONE, &work[pw], 1);
                        cblas_cgemv(CblasColMajor, CblasConjTrans, len, nnb, &CONE,
            ...
            cola = n - jcol - nnb;
            j = ihi - nblst;
            cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans, nblst, cola, nblst,
                        &CONE, work, nblst, &A[j + (jcol + nnb) * lda], lda,
                        &CZERO, &work[pw], nblst);
            clacpy("A", nblst, cola, &work[pw], nblst, &A[j + (jcol + nnb) * lda], lda);
            ppwo = nblst * nblst;
            j0 = j - nnb;
            for (j = j0; j >= jcol + 1; j -= nnb) {
                if (blk22) {
                    cunm22("L", "C", 2 * nnb, cola, nnb, nnb,
                    ...
                    clacpy("A", 2 * nnb, cola, &work[pw], 2 * nnb,
            ...
            if (wantq) {
                ...
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nh, nblst, nblst,
                            &CONE, &Q[topq + j * ldq], ldq, work, nblst,
                            &CZERO, &work[pw], nh);
                clacpy("A", nh, nblst, &work[pw], nh, &Q[topq + j * ldq], ldq);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (initq) {
                    ...
                    if (blk22) {
                        ...
                        clacpy("A", nh, 2 * nnb, &work[pw], nh, &Q[topq + j * ldq], ldq);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
            }
            ...
            if (wantz || top > 0) {
                ...
                for (j = jcol; j <= jcol + nnb - 1; j++) {
                    ...
                    for (i = ihi - 1; i >= jrow; i--) {
                        ...
                        for (jj = ppw; jj <= ppw + len - 1; jj++) {
                            temp = work[jj + nblst];
                            work[jj + nblst] = ctemp * temp - conjf(s) * work[jj];
                    ...
                    for (jrow = j0; jrow >= j + 2; jrow -= nnb) {
                        ...
                        for (i = jrow + nnb - 1; i >= jrow; i--) {
                            ...
                            for (jj = ppw; jj <= ppw + len - 1; jj++) {
                                temp = work[jj + 2 * nnb];
                                work[jj + 2 * nnb] = ctemp * temp - conjf(s) * work[jj];
                                work[jj] = s * temp + ctemp * work[jj];
                            }
                            len = len + 1;
                            ppw = ppw - 2 * nnb - 1;
                ...
                claset("L", ihi - jcol - 2, nnb, CZERO, CZERO, &A[(jcol + 2) + jcol * lda], lda);
                claset("L", ihi - jcol - 2, nnb, CZERO, CZERO, &B[(jcol + 2) + jcol * ldb], ldb);
            }

            if (top > 0) {
                j = ihi - nblst;
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, top, nblst, nblst,
                            &CONE, &A[j * lda], lda, work, nblst, &CZERO, &work[pw], top);
                clacpy("A", top, nblst, &work[pw], top, &A[j * lda], lda);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (blk22) {
                        cunm22("R", "N", top, 2 * nnb, nnb, nnb,
                        ...
                        clacpy("A", top, 2 * nnb, &work[pw], top, &A[j * lda], lda);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
                ...
                clacpy("A", top, nblst, &work[pw], top, &B[j * ldb], ldb);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (blk22) {
                        ...
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    &work[ppwo], 2 * nnb, &CZERO, &work[pw], top);
                        clacpy("A", top, 2 * nnb, &work[pw], top, &B[j * ldb], ldb);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
            }
            ...
            if (wantz) {
                ...
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nh, nblst, nblst,
                            ...
                            &CZERO, &work[pw], nh);
                clacpy("A", nh, nblst, &work[pw], nh, &Z[topq + j * ldz], ldz);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (initq) {
                    ...
                    if (blk22) {
                        ...
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    &work[ppwo], 2 * nnb, &CZERO, &work[pw], nh);
                        clacpy("A", nh, 2 * nnb, &work[pw], nh, &Z[topq + j * ldz], ldz);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
            }
    ...
    if (jcol < ihi - 1) {
        char cq2[2] = {compq2, '\0'};
        char cz2[2] = {compz2, '\0'};
        cgghrd(cq2, cz2, n, jcol + 1, ihi, A, lda, B, ldb, Q, ldq, Z, ldz, &ierr);
    }
