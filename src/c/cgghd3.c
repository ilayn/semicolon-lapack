 ...
 *                        set by a previous call to CGGBAL; otherwise they should be set
 *                        to 0 and n-1 respectively.
 *                        0 <= ilo <= ihi <= n-1, if n > 0; ilo=0 and ihi=-1, if n=0.
 * @param[in,out] A       Complex array of dimension (lda, n). On entry, the N-by-N general
 ...
 * @param[in,out] B       Complex array of dimension (ldb, n). On entry, the upper triangular
 ...
 * @param[out]    work    Complex array of dimension (max(1,lwork)). On exit, if info = 0,
    ...
    INT* info)
{
    ...
    f32 c;
    c64 ctemp, s, c1, c2, s1, s2, temp, temp1, temp2, temp3;
    ...
    *info = 0;
    ...
    if (nb < nbmin || nb >= nh) {
        ...
        for (jcol = ilo; jcol <= ihi - 2; jcol += nb) {
            ...
            for (j = jcol; j < jcol + nnb; j++) {
                ...
                for (jj = n - 1; jj >= j + 1; jj--) {
                    ...
                    if (jj < ihi) {
                        ...
                        clartg(temp, B[jj + 1 + jj * ldb], &c, &s,
                               &B[jj + 1 + (jj + 1) * ldb]);
                        B[jj + 1 + jj * ldb] = CZERO;
                        crot(jj + 1 - top, &B[top + (jj + 1) * ldb], 1,
                             &B[top + jj * ldb], 1, c, s);
                ...
                if (j < jcol + nnb - 1) {
                    ...
                    jrow = ihi - nblst + 1;
                    cblas_cgemv(CblasColMajor, CblasConjTrans, nblst, len, &CONE,
                    ...
                    cblas_cgemv(CblasColMajor, CblasConjTrans, len, nblst - len,
                                &CONE,
                    ...
                    for (jrow = j0; jrow >= jcol + 1; jrow -= nnb) {
                        ...
                        cblas_ctrmv(CblasColMajor, CblasUpper, CblasConjTrans,
                                    CblasNonUnit, len,
                                    &work[ppwo + nnb], 2 * nnb,
                                    &work[pw], 1);
                        cblas_ctrmv(CblasColMajor, CblasLower, CblasConjTrans,
            ...
            cblas_cgemm(CblasColMajor, CblasConjTrans, CblasNoTrans,
                        nblst, cola, nblst, &CONE, work, nblst,
                        &A[j + (jcol + nnb) * lda], lda, &CZERO,
                        &work[pw], nblst);
            clacpy("A", nblst, cola, &work[pw], nblst,
            ...
            if (wantz || top > 0) {
                ...
                pw = nblst * nblst;
                for (i = 0; i < n2nb; i++) {
                    claset("A", 2 * nnb, 2 * nnb, CZERO, CONE,
                           &work[pw], 2 * nnb);
                    pw += 4 * nnb * nnb;
                }
                ...
                claset("L", ihi - jcol - 1, nnb, CZERO, CZERO,
            ...
            if (top > 0) {
                j = ihi - nblst + 1;
                cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            top, nblst, nblst, &CONE,
                            &A[j * lda], lda, work, nblst, &CZERO,
                            &work[pw], top);
                clacpy("A", top, nblst, &work[pw], top,
                       &A[j * lda], lda);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (blk22) {
                        ...
                        cblas_cgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    top, 2 * nnb, 2 * nnb, &CONE,
                                    &A[j * lda], lda,
                                    &work[ppwo], 2 * nnb, &CZERO,
                                    &work[pw], top);
                        clacpy("A", top, 2 * nnb, &work[pw], top,
                               &A[j * lda], lda);
                    }
                    ppwo += 4 * nnb * nnb;
                }
...

    {
        ...
        if (jcol < ihi)
            cgghrd(compq2, compz2, n, jcol, ihi, A, lda, B, ldb,
                   Q, ldq, Z, ldz, &ierr);
    }
