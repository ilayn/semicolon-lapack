/**
 * @file cunmqr.c
 * @brief CUNMQR multiplies a general matrix by the unitary matrix from
 ...
 *                       the elementary reflector H(i), as returned by CGEQRF.
 *                       Dimension (lda, k).
 * @param[in]     lda    Leading dimension of A.
 *                       If SIDE = "L", lda >= max(1, m);
 *                       if SIDE = "R", lda >= max(1, n).
 * @param[in]     tau    Array of dimension (k). TAU(i) is the scalar factor
 *                       of H(i), as returned by CGEQRF.
 ...
 */
void cunmqr(const char* side, const char* trans,
    ...
    *info = 0;
    ...
    if (nb < nbmin || nb >= k) {
        ...
        for (i = i1; i3 > 0 ? i <= i2 : i >= i2; i += i3) {
            ...
            clarfb(side, trans, "F", "C", mi, ni, ib,
                   &A[i + i * lda], lda,
                   &work[iwt], LDT,
                   &C[ic + jc * ldc], ldc,
                   work, ldwork);
