/**
 * @file clarrv.c
 * @brief CLARRV computes the eigenvectors of the tridiagonal matrix
 ...
 * @param[in,out] L       Single precision array, dimension (n).
 ...
 * @param[in,out] W       Single precision array, dimension (n).
 ...
 * @param[in]     gers    Single precision array, dimension (2*n).
 *                        The n Gerschgorin intervals (gers[2*i], gers[2*i+1]).
 * @param[out]    Z       Complex f32 array, dimension (ldz, max(1,m)).
 ...
 * @param[out]    work    Single precision array, dimension (12*n).
 ...
 */
void clarrv(const int n, const f32 vl, const f32 vu,
            ...
            c64* Z, const int ldz, int* isuppz,
            f32* work, int* iwork, int* info)
{
    ...
    int done, i, ibegin, idone, iend, ii, iindc1,
        ...
        indld, indlld, indin1, indin2, indwrk,
        isupmn, isupmx, iter,
    ...
    *info = 0;
    ...
    claset("Full", n, zusedw, CZERO, CZERO, &Z[zusedl * ldz], ldz);
    ...
    for (jblk = 0; jblk <= iblock[m - 1]; jblk++) {
        ...
        if (ibegin == iend) {
            ...
            Z[ibegin + wbegin * ldz] = CMPLXF(ONE, 0.0f);
        ...
        while (idone < im) {
            ...
            for (i = 0; i < oldncl; i++) {
                ...
                if (ndepth > 0) {
                    ...
                    for (k = 0; k < in - 1; k++) {
                        D[ibegin + k] = crealf(Z[ibegin + k + j * ldz]);
                        L[ibegin + k] = crealf(Z[ibegin + k + (j + 1) * ldz]);
                    }
                    D[iend] = crealf(Z[iend + j * ldz]);
                    sigma = crealf(Z[iend + (j + 1) * ldz]);

                    /* Set the corresponding entries in Z to zero */
                    claset("Full", in, 2, CZERO, CZERO, &Z[ibegin + j * ldz], ldz);
                }
                ...
                for (j = oldfst; j <= oldlst; j++) {
                    ...
                    if (newsiz > 1) {
                         ...
                         * workspace */
                        for (k = 0; k < in - 1; k++) {
                            Z[ibegin + k + newftt * ldz] = CMPLXF(work[indin1 + k], 0.0f);
                            Z[ibegin + k + (newftt + 1) * ldz] = CMPLXF(work[indin2 + k], 0.0f);
                        }
                        Z[iend + newftt * ldz] = CMPLXF(work[indin1 + in - 1], 0.0f);
                        if (iinfo == 0) {
                            /* a new RRR for the cluster was found by SLARRF
                             * update shift and store it */
                            ssigma = sigma + tau;
                            Z[iend + (newftt + 1) * ldz] = CMPLXF(ssigma, 0.0f);
...
label_120:
                        ...
                        clar1v(in, 0, in - 1, lambda, &D[ibegin],
                        ...
                        cblas_csscal(zto - zfrom + 1, nrminv,
