...
/**
 * @file cgeqrt.c
 * @brief CGEQRT computes a blocked QR factorization of a general M-by-N matrix
 ...
 * @param[out]    T     Complex*16 array, dimension (ldt, min(m,n)).
 *                      The upper triangular block reflectors stored in compact form
 *                      as a sequence of upper triangular blocks.
 * @param[in]     ldt   The leading dimension of the array T. ldt >= nb.
 * @param[out]    work  Complex*16 array, dimension (nb*n).
 * @param[out]    info
 *                         - = 0: successful exit
            ...
            int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < k; i += nb) {
        ...
        cgeqrt3(m - i, ib, &A[i + i * lda], lda, &T[0 + i * ldt], ldt, &iinfo);

        if (i + ib < n) {
            /* Update by applying H^H to A(i:m-1, i+ib:n-1) from the left */
            clarfb("L", "C", "F", "C",
                   m - i, n - i - ib, ib,
                   &A[i + i * lda], lda,
                   &T[0 + i * ldt], ldt,
