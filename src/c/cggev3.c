/**
 * @file cggev3.c
 * @brief CGGEV3 computes the eigenvalues and, optionally, the left and/or
            ...
            f32* rwork, int* info)
{
    ...
    *info = 0;
    ...
    claqz0(chtemp, jobvl, jobvr, n, ilo, ihi, A, lda, B, ldb,
    ...
    if (ilv) {
        ...
        if (ilvl) {
            ...
            for (jc = 0; jc < n; jc++) {
                ...
                for (jr = 0; jr < n; jr++) {
                    x = VL[jr + jc * ldvl];
                    temp = temp > (fabsf(crealf(x)) + fabsf(cimagf(x))) ?
                           temp : (fabsf(crealf(x)) + fabsf(cimagf(x)));
                }
                if (temp < smlnum)
                    continue;
                temp = ONE / temp;
