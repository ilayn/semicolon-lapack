 ...
 * @param[out] S      Single precision array, dimension (mm).
 *                    If job = 'E' or 'B', the reciprocal condition numbers
 *                    of the selected eigenvalues.
 *                    If job = 'V', S is not referenced.
 * @param[out] sep    Single precision array, dimension (mm).
...
void ctrsna(const char* job, const char* howmny, const int* select,
            const int n, const c64* T, const int ldt,
            const c64* VL, const int ldvl,
            const c64* VR, const int ldvr,
            f32* S, f32* sep, const int mm, int* m,
            c64* work, const int ldwork,
            f32* rwork, int* info)
{
    const f32 ZERO = 0.0f;
    const f32 ONE = 1.0f;
    ...
    f32 eps, est, lnrm, rnrm, scale, smlnum, xnorm;
    c64 prod;
    char normin;
    int isave[3];
    c64 dummy[1];
    ...
    *info = 0;
    ...
    for (k = 0; k < n; k++) {
        ...
        if (wants) {
             ...
             * eigenvalue. */
            cblas_cdotc_sub(n, &VR[ks * ldvr], 1, &VL[ks * ldvl], 1, &prod);
            rnrm = cblas_scnrm2(n, &VR[ks * ldvr], 1);
            lnrm = cblas_scnrm2(n, &VL[ks * ldvl], 1);
            S[ks] = cabsf(prod) / (rnrm * lnrm);
        }
        ...
        if (wantsp) {
            ...
            clacpy("F", n, n, T, ldt, work, ldwork);
            ctrexc("N", n, work, ldwork, dummy, 1, k, 0, &ierr);
            ...
            normin = 'N';
            for (;;) {
                clacn2(n - 1, &work[n * ldwork], work, &est, &kase, isave);
                ...
                if (kase == 1) {
                    /* Solve C**H*x = scale*b */
                    clatrs("U", "C", "N", &normin, n - 1,
                           &work[1 + 1 * ldwork], ldwork,
                           work, &scale, rwork, &ierr);
                } else {
                    /* Solve C*x = scale*b */
                    clatrs("U", "N", "N", &normin, n - 1,
                           &work[1 + 1 * ldwork], ldwork,
                           work, &scale, rwork, &ierr);
                }
                normin = 'Y';
