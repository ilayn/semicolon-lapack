/**
 * @file cgerfs.c
 * @brief CGERFS improves the computed solution and provides error bounds.
 */
 ...
 *                       as computed by cgetrf. Complex array of dimension (ldaf, n).
 * @param[in]     ldaf   The leading dimension of the array AF (ldaf >= max(1,n)).
 * @param[in]     ipiv   The pivot indices from cgetrf. Array of dimension n.
 * @param[in]     B      The right hand side matrix B. Complex array of dimension (ldb, nrhs).
 * @param[in]     ldb    The leading dimension of the array B (ldb >= max(1,n)).
 * @param[in,out] X      On entry, the solution matrix X, as computed by cgetrs.
    ...
    *info = 0;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        while (1) {
            ...
            cgetrs(trans, n, 1, AF, ldaf, ipiv, work, n, &linfo);
        ...
        while (1) {
            ...
            if (kase == 1) {
                cgetrs(&transt, n, 1, AF, ldaf, ipiv, work, n, &linfo);
                for (i = 0; i < n; i++) {
                ...
                cgetrs(&transn, n, 1, AF, ldaf, ipiv, work, n, &linfo);
            }
        }
