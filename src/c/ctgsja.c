 ...
 * @param[in]     k       Subblock dimension from CGGSVP3.
 * @param[in]     l       Subblock dimension from CGGSVP3.
            ...
            c64* restrict work, INT* ncycle, INT* info)
{
    ...
    *info = 0;
    ...
    if (initu) {
        claset("F", m, m, CZERO, CONE, U, ldu);
    }
    if (initv) {
        claset("F", p, p, CZERO, CONE, V, ldv);
    }
    if (initq) {
    ...
    for (kcycle = 0; kcycle < MAXIT; kcycle++) {
        ...
        for (i = 0; i < l - 1; i++) {
            for (j = i + 1; j < l; j++) {
                ...
                a3 = ZERO;
                if (k + i < m) {
                    a1 = crealf(A[(k + i) + (n - l + i) * lda]);
                }
                if (k + j < m) {
                    a3 = crealf(A[(k + j) + (n - l + j) * lda]);
                }

                b1 = crealf(B[i + (n - l + i) * ldb]);
                b3 = crealf(B[j + (n - l + j) * ldb]);
                ...
                clags2(upper, a1, a2, a3, b1, b2, b3,
                       &csu, &snu, &csv, &snv, &csq, &snq);

                if (k + j < m) {
                    crot(l, &A[(k + j) + (n - l) * lda], lda,
                         &A[(k + i) + (n - l) * lda], lda, csu, conjf(snu));
                }

                crot(l, &B[j + (n - l) * ldb], ldb,
                     &B[i + (n - l) * ldb], ldb, csv, conjf(snv));

                minval = k + l;
                if (m < minval) minval = m;
                crot(minval, &A[(n - l + j) * lda], 1,
                     &A[(n - l + i) * lda], 1, csq, snq);

                crot(l, &B[(n - l + j) * ldb], 1,
                ...
                if (k + i < m) {
                    A[(k + i) + (n - l + i) * lda] = crealf(A[(k + i) + (n - l + i) * lda]);
                }
                if (k + j < m) {
                    A[(k + j) + (n - l + j) * lda] = crealf(A[(k + j) + (n - l + j) * lda]);
                }
                B[i + (n - l + i) * ldb] = crealf(B[i + (n - l + i) * ldb]);
                B[j + (n - l + j) * ldb] = crealf(B[j + (n - l + j) * ldb]);
        ...
        if (!upper) {
            ...
            for (i = 0; i < minval; i++) {
                cblas_ccopy(l - i, &A[(k + i) + (n - l + i) * lda], lda, work, 1);
                cblas_ccopy(l - i, &B[i + (n - l + i) * ldb], ldb, &work[l], 1);
                clapll(l - i, work, 1, &work[l], 1, &ssmin);
...
converged:
    ...
    for (i = 0; i < minval; i++) {
        ...
        if (gamma <= HUGENUM && gamma >= -HUGENUM) {
            if (gamma < ZERO) {
                cblas_csscal(l - i, -ONE, &B[i + (n - l + i) * ldb], ldb);
                if (wantv) {
                    cblas_csscal(p, -ONE, &V[i * ldv], 1);
                }
            }
            ...
            if (alpha[k + i] >= beta[k + i]) {
                cblas_csscal(l - i, ONE / alpha[k + i],
                             &A[(k + i) + (n - l + i) * lda], lda);
            } else {
                cblas_csscal(l - i, ONE / beta[k + i],
                             &B[i + (n - l + i) * ldb], ldb);
                cblas_ccopy(l - i, &B[i + (n - l + i) * ldb], ldb,
                            &A[(k + i) + (n - l + i) * lda], lda);
            }
        } else {
            alpha[k + i] = ZERO;
