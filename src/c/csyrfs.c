/**
 * @file csyrfs.c
 * @brief CSYRFS improves the computed solution to a system of linear
 ...
 *                      as determined by CSYTRF. Integer array, dimension (n).
 * @param[in]     B     The right hand side matrix B.
 *                      Complex array, dimension (ldb, nrhs).
 * @param[in]     ldb   The leading dimension of the array B. ldb >= max(1, n).
 * @param[in,out] X     On entry, the solution matrix X, as computed by CSYTRS.
    ...
    *info = 0;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        while (1) {
            ...
            csymv(uplo, n, neg_one, A, lda, &X[j * ldx], 1, ONE, work, 1);
            ...
            if (upper) {
                for (k = 0; k < n; k++) {
                    ...
                    xk = cabs1f(X[k + j * ldx]);
                    for (i = 0; i < k; i++) {
                        rwork[i] = rwork[i] + cabs1f(A[i + k * lda]) * xk;
                        s = s + cabs1f(A[i + k * lda]) * cabs1f(X[i + j * ldx]);
                    }
                    rwork[k] = rwork[k] + cabs1f(A[k + k * lda]) * xk + s;
                }
            } else {
                for (k = 0; k < n; k++) {
                    s = ZERO;
                    xk = cabs1f(X[k + j * ldx]);
                    rwork[k] = rwork[k] + cabs1f(A[k + k * lda]) * xk;
                    for (i = k + 1; i < n; i++) {
                        rwork[i] = rwork[i] + cabs1f(A[i + k * lda]) * xk;
                        s = s + cabs1f(A[i + k * lda]) * cabs1f(X[i + j * ldx]);
                    }
            ...
            for (i = 0; i < n; i++) {
                if (rwork[i] > safe2) {
                    s = (s > cabs1f(work[i]) / rwork[i])
                        ? s : cabs1f(work[i]) / rwork[i];
                } else {
            ...
            csytrs(uplo, n, 1, AF, ldaf, ipiv, work, n, &linfo);
