/**
 * @file cstedc.c
 * @brief CSTEDC computes all eigenvalues and, optionally, eigenvectors of a
    ...
    *info = 0;
    ...
    if (n <= SMLSIZ) {
        ...
        if (icompz == 2) {
            ...
            ll = n * n;
            sstedc("I", n, D, E, rwork, n,
                   &rwork[ll], lrwork - ll, iwork, liwork, info);
            for (j = 0; j < n; j++) {
                for (i = 0; i < n; i++) {
                    Z[i + j * ldz] = CMPLXF(rwork[j * n + i], 0.0f);
                }
            }
            goto L70;
        }
    ...
    L30:
        ...
        L40:
            ...
            if (m > SMLSIZ) {
                ...
                claed0(n, m, &D[start], &E[start], &Z[start * ldz],
                ...
                ssteqr("I", m, &D[start], &E[start], rwork, m,
                        &rwork[m * m], info);
                clacrm(n, m, &Z[start * ldz], ldz, rwork, m, work,
                        n, &rwork[m * m]);
                clacpy("A", n, m, work, n, &Z[start * ldz], ldz);
                if (*info > 0) {
        ...
        for (ii = 1; ii < n; ii++) {
            ...
            if (k != i) {
                D[k] = D[i];
                D[i] = p;
                cblas_cswap(n, &Z[i * ldz], 1, &Z[k * ldz], 1);
            }
        }
    }
