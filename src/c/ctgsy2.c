/**
 * @file ctgsy2.c
 * @brief CTGSY2 solves the generalized Sylvester equation (unblocked algorithm).
 */
        ...
        for (j = 0; j < n; j++) {
            for (i = m - 1; i >= 0; i--) {
                ...
                if (ijob == 0) {
                    ...
                    if (scaloc != ONE) {
                        c64 scaloc_cmplx = CMPLXF(scaloc, ZERO);
                        for (k = 0; k < n; k++) {
                            cblas_cscal(m, &scaloc_cmplx, &C[0 + k * ldc], 1);
                            cblas_cscal(m, &scaloc_cmplx, &F[0 + k * ldf], 1);
                        }
                        *scale = (*scale) * scaloc;
                    }
                } else {
                ...
                if (i > 0) {
                    alpha = -rhs[0];
                    cblas_caxpy(i, &alpha, &A[0 + i * lda], 1, &C[0 + j * ldc], 1);
                    cblas_caxpy(i, &alpha, &D[0 + i * ldd], 1, &F[0 + j * ldf], 1);
                }
                if (j < n - 1) {
                    cblas_caxpy(n - j - 1, &rhs[1], &B[j + (j + 1) * ldb], ldb,
                                &C[i + (j + 1) * ldc], ldc);
                    cblas_caxpy(n - j - 1, &rhs[1], &E[j + (j + 1) * lde], lde,
                                &F[i + (j + 1) * ldf], ldf);
                }
        ...
        *scale = ONE;
        ...
        for (i = 0; i < m; i++) {
            for (j = n - 1; j >= 0; j--) {
                ...
                z[0 + 0 * LDZ] = conjf(A[i + i * lda]);
                z[1 + 0 * LDZ] = -conjf(B[j + j * ldb]);
                z[0 + 1 * LDZ] = conjf(D[i + i * ldd]);
                z[1 + 1 * LDZ] = -conjf(E[j + j * lde]);
                ...
                for (k = 0; k < j; k++) {
                    F[i + k * ldf] = F[i + k * ldf]
                        + rhs[0] * conjf(B[k + j * ldb])
                        + rhs[1] * conjf(E[k + j * lde]);
                }
                for (k = i + 1; k < m; k++) {
                    C[k + j * ldc] = C[k + j * ldc]
                        - conjf(A[i + k * lda]) * rhs[0]
                        - conjf(D[i + k * ldd]) * rhs[1];
                }
