    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    smin = 0.0f;  // Will be set on first iteration
    ...
    for (i = 0; i < n - 1; i++) {
        ...
        for (jp = i; jp < n; jp++) {
            for (ip = i; ip < n; ip++) {
                if (cabsf(A[ip + jp * lda]) >= xmax) {
                    xmax = cabsf(A[ip + jp * lda]);
                    ipv = ip;
                    jpv = jp;
                }
            }
        ...
        // Swap rows
        if (ipv != i) {
            cblas_cswap(n, &A[ipv], lda, &A[i], lda);
        }
        ipiv[i] = ipv;

        // Swap columns
        if (jpv != i) {
            cblas_cswap(n, &A[jpv * lda], 1, &A[i * lda], 1);
        }
        jpiv[i] = jpv;

        // Check for singularity
        if (cabsf(A[i + i * lda]) < smin) {
            *info = i + 1;
            A[i + i * lda] = CMPLXF(smin, 0.0f);
        }
        ...
        cblas_cgeru(CblasColMajor, n - i - 1, n - i - 1, &NEG_ONE,
    ...
    // Check last pivot
    if (cabsf(A[n - 1 + (n - 1) * lda]) < smin) {
        *info = n;
        A[n - 1 + (n - 1) * lda] = CMPLXF(smin, 0.0f);
    }
