/**
 * @file cgeqrf.c
 * @brief CGEQRF computes a QR factorization of a general M-by-N matrix
 ...
 */
void cgeqrf(const INT m, const INT n,
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k - nx; i += nb) {
            ...
            cgeqr2(m - i, ib, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    ...
    if (i < k) {
        cgeqr2(m - i, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    }

    work[0] = CMPLXF((f32)iws, 0.0f);
}
