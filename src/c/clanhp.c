    ...
    f32* restrict work)
{
    ...
    if (n == 0) {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 1; j < n; j++) {
                // Fortran: CALL CLASSQ( J-1, AP( K ), 1, SCALE, SUM )
                classq(j, &AP[k], 1, &scale, &sum);
                k = k + (j + 1);
            }
        } else {
            for (j = 0; j < n - 1; j++) {
                // Fortran: CALL CLASSQ( N-J, AP( K ), 1, SCALE, SUM )
