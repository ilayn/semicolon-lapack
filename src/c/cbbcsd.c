/**
 * @file cbbcsd.c
 * @brief CBBCSD computes the CS decomposition of a unitary matrix in bidiagonal-block form.
 */
    ...
    int* info)
{
    ...
    const c64 negonecomplex = CMPLXF(-1.0f, 0.0f);
    ...
    *info = 0;
    ...
    while (imax > 1) {
        ...
        if (B11D[imin - 1] * B11D[imin - 1] + b11bulge * b11bulge > thresh * thresh) {
            slartgp(b11bulge, B11D[imin - 1], &rwork[iu1sn + imin - 1],
        ...
        if (B21D[imin - 1] * B21D[imin - 1] + b21bulge * b21bulge > thresh * thresh) {
            slartgp(b21bulge, B21D[imin - 1], &rwork[iu2sn + imin - 1],
        ...
        for (i = imin; i < imax - 1; i++) {
            ...
            if (!restart11 && !restart21) {
                slartgp(x2, x1, &rwork[iv1tsn + i], &rwork[iv1tcs + i], &r);
            } else if (!restart11 && restart21) {
                slartgp(b11bulge, B11E[i - 1], &rwork[iv1tsn + i], &rwork[iv1tcs + i], &r);
            } else if (restart11 && !restart21) {
                slartgp(b21bulge, B21E[i - 1], &rwork[iv1tsn + i], &rwork[iv1tcs + i], &r);
            } else if (mu <= nu) {
                slartgs(B11D[i], B11E[i], mu, &rwork[iv1tcs + i], &rwork[iv1tsn + i]);
            } else {
                slartgs(B21D[i], B21E[i], nu, &rwork[iv1tcs + i], &rwork[iv1tsn + i]);
            }
            rwork[iv1tcs + i] = -rwork[iv1tcs + i];
            rwork[iv1tsn + i] = -rwork[iv1tsn + i];
            if (!restart12 && !restart22) {
                slartgp(y2, y1, &rwork[iv2tsn + i - 1], &rwork[iv2tcs + i - 1], &r);
            } else if (!restart12 && restart22) {
                slartgp(b12bulge, B12D[i - 1], &rwork[iv2tsn + i - 1], &rwork[iv2tcs + i - 1], &r);
            } else if (restart12 && !restart22) {
                slartgp(b22bulge, B22D[i - 1], &rwork[iv2tsn + i - 1], &rwork[iv2tcs + i - 1], &r);
            } else if (nu < mu) {
                slartgs(B12E[i - 1], B12D[i], nu, &rwork[iv2tcs + i - 1], &rwork[iv2tsn + i - 1]);
            } else {
                slartgs(B22E[i - 1], B22D[i], mu, &rwork[iv2tcs + i - 1], &rwork[iv2tsn + i - 1]);
            }
            ...
            if (!restart11 && !restart12) {
                slartgp(x2, x1, &rwork[iu1sn + i], &rwork[iu1cs + i], &r);
            } else if (!restart11 && restart12) {
                slartgp(b11bulge, B11D[i], &rwork[iu1sn + i], &rwork[iu1cs + i], &r);
            } else if (restart11 && !restart12) {
                slartgp(b12bulge, B12E[i - 1], &rwork[iu1sn + i], &rwork[iu1cs + i], &r);
            } else if (mu <= nu) {
                slartgs(B11E[i], B11D[i + 1], mu, &rwork[iu1cs + i], &rwork[iu1sn + i]);
            } else {
                slartgs(B12D[i], B12E[i], nu, &rwork[iu1cs + i], &rwork[iu1sn + i]);
            }
            if (!restart21 && !restart22) {
                slartgp(y2, y1, &rwork[iu2sn + i], &rwork[iu2cs + i], &r);
            } else if (!restart21 && restart22) {
                slartgp(b21bulge, B21D[i], &rwork[iu2sn + i], &rwork[iu2cs + i], &r);
            } else if (restart21 && !restart22) {
                slartgp(b22bulge, B22E[i - 1], &rwork[iu2sn + i], &rwork[iu2cs + i], &r);
            } else if (nu < mu) {
                slartgs(B21E[i], B21E[i + 1], nu, &rwork[iu2cs + i], &rwork[iu2sn + i]);
            } else {
                slartgs(B22D[i], B22E[i], mu, &rwork[iu2cs + i], &rwork[iu2sn + i]);
            }
        ...
        if (!restart12 && !restart22) {
            slartgp(y2, y1, &rwork[iv2tsn + imax - 2], &rwork[iv2tcs + imax - 2], &r);
        } else if (!restart12 && restart22) {
            slartgp(b12bulge, B12D[imax - 2], &rwork[iv2tsn + imax - 2],
                    &rwork[iv2tcs + imax - 2], &r);
        } else if (restart12 && !restart22) {
            slartgp(b22bulge, B22D[imax - 2], &rwork[iv2tsn + imax - 2],
                    &rwork[iv2tcs + imax - 2], &r);
        } else if (nu < mu) {
        ...
        if (wantu1) {
            if (colmajor) {
                clasr("R", "V", "F", p, imax - imin + 1,
                      &rwork[iu1cs + imin - 1], &rwork[iu1sn + imin - 1],
                      &U1[0 + (imin - 1) * ldu1], ldu1);
            } else {
                clasr("L", "V", "F", imax - imin + 1, p,
                      &rwork[iu1cs + imin - 1], &rwork[iu1sn + imin - 1],
        ...
        if (wantu2) {
            if (colmajor) {
                clasr("R", "V", "F", m - p, imax - imin + 1,
                      &rwork[iu2cs + imin - 1], &rwork[iu2sn + imin - 1],
                      &U2[0 + (imin - 1) * ldu2], ldu2);
            } else {
                clasr("L", "V", "F", imax - imin + 1, m - p,
                      &rwork[iu2cs + imin - 1], &rwork[iu2sn + imin - 1],
        ...
        if (wantv1t) {
            if (colmajor) {
                clasr("L", "V", "F", imax - imin + 1, q,
                      &rwork[iv1tcs + imin - 1], &rwork[iv1tsn + imin - 1],
                      &V1T[(imin - 1) + 0 * ldv1t], ldv1t);
            } else {
                clasr("R", "V", "F", q, imax - imin + 1,
                      &rwork[iv1tcs + imin - 1], &rwork[iv1tsn + imin - 1],
        ...
        if (B11E[imax - 2] + B21E[imax - 2] > 0) {
            ...
            if (wantv1t) {
                if (colmajor) {
                    cblas_cscal(q, &negonecomplex, &V1T[(imax - 1) + 0 * ldv1t], ldv1t);
                } else {
                    cblas_cscal(q, &negonecomplex, &V1T[0 + (imax - 1) * ldv1t], 1);
                }
            }
        }
        ...
        if (B11D[imax - 1] + B12E[imax - 2] < 0) {
            ...
            if (wantu1) {
                if (colmajor) {
                    cblas_cscal(p, &negonecomplex, &U1[0 + (imax - 1) * ldu1], 1);
                } else {
                    cblas_cscal(p, &negonecomplex, &U1[(imax - 1) + 0 * ldu1], ldu1);
                }
            }
        }
        if (B21D[imax - 1] + B22E[imax - 2] > 0) {
            ...
            if (wantu2) {
                if (colmajor) {
                    cblas_cscal(m - p, &negonecomplex, &U2[0 + (imax - 1) * ldu2], 1);
                } else {
                    cblas_cscal(m - p, &negonecomplex, &U2[(imax - 1) + 0 * ldu2], ldu2);
                }
            }
        }
        ...
        if (B12D[imax - 1] + B22D[imax - 1] < 0) {
            if (wantv2t) {
                if (colmajor) {
                    cblas_cscal(m - q, &negonecomplex, &V2T[(imax - 1) + 0 * ldv2t], ldv2t);
                } else {
                    cblas_cscal(m - q, &negonecomplex, &V2T[0 + (imax - 1) * ldv2t], 1);
                }
            }
        }
    ...
    for (i = 0; i < q; i++) {
        ...
        if (mini != i) {
            ...
            if (colmajor) {
                if (wantu1)
                    cblas_cswap(p, &U1[0 + i * ldu1], 1, &U1[0 + mini * ldu1], 1);
                if (wantu2)
                    cblas_cswap(m - p, &U2[0 + i * ldu2], 1, &U2[0 + mini * ldu2], 1);
                if (wantv1t)
                    cblas_cswap(q, &V1T[i + 0 * ldv1t], ldv1t, &V1T[mini + 0 * ldv1t], ldv1t);
                if (wantv2t)
                    cblas_cswap(m - q, &V2T[i + 0 * ldv2t], ldv2t, &V2T[mini + 0 * ldv2t], ldv2t);
            } else {
                if (wantu1)
                    cblas_cswap(p, &U1[i + 0 * ldu1], ldu1, &U1[mini + 0 * ldu1], ldu1);
                if (wantu2)
                    cblas_cswap(m - p, &U2[i + 0 * ldu2], ldu2, &U2[mini + 0 * ldu2], ldu2);
                if (wantv1t)
                    cblas_cswap(q, &V1T[0 + i * ldv1t], 1, &V1T[0 + mini * ldv1t], 1);
                if (wantv2t)
                    cblas_cswap(m - q, &V2T[0 + i * ldv2t], 1, &V2T[0 + mini * ldv2t], 1);
            }
        }
