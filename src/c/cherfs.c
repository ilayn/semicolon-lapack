/**
 * @file cherfs.c
 * @brief CHERFS improves the computed solution to a system of linear
    ...
    *info = 0;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        while (1) {
            ...
            cblas_chemv(CblasColMajor, upper ? CblasUpper : CblasLower,
            ...
            if (upper) {
                for (k = 0; k < n; k++) {
                    ...
                    rwork[k] = rwork[k] + fabsf(crealf(A[k + k * lda])) * xk + s;
                }
            } else {
                for (k = 0; k < n; k++) {
                    s = ZERO;
                    xk = cabs1f(X[k + j * ldx]);
                    rwork[k] = rwork[k] + fabsf(crealf(A[k + k * lda])) * xk;
                    for (i = k + 1; i < n; i++) {
                        rwork[i] = rwork[i] + cabs1f(A[i + k * lda]) * xk;
                        s = s + cabs1f(A[i + k * lda]) * cabs1f(X[i + j * ldx]);
                    }
            ...
            chetrs(uplo, n, 1, AF, ldaf, ipiv, work, n, &linfo);
