/**
 * @file cptts2.c
 * @brief CPTTS2 solves a tridiagonal system of the form AX=B using the
 *        L*D*L**H factorization computed by CPTTRF.
 */
    ...
    const INT ldb)
{
    ...
    if (iuplo == 1) {
        ...
        if (nrhs <= 2) {
            ...
            for (i = 1; i < n; i++) {
                B[i + j * ldb] = B[i + j * ldb] - B[(i - 1) + j * ldb] * conjf(E[i - 1]);
            }
        ...
        if (nrhs <= 2) {
            ...
            for (i = n - 2; i >= 0; i--) {
                B[i + j * ldb] = B[i + j * ldb] - B[(i + 1) + j * ldb] * conjf(E[i]);
            }
            if (j < nrhs - 1) {
                j = j + 1;
                goto L80;
            ...
            for (j = 0; j < nrhs; j++) {
                ...
                B[(n - 1) + j * ldb] = B[(n - 1) + j * ldb] / D[n - 1];
                for (i = n - 2; i >= 0; i--) {
                    B[i + j * ldb] = B[i + j * ldb] / D[i] - B[(i + 1) + j * ldb] * conjf(E[i]);
                }
            }
        }
    }
