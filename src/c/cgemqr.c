/**
 * @file cgemqr.c
 * @brief CGEMQR overwrites matrix C with Q*C, Q^H*C, C*Q, or C*Q^H
 *        where Q is defined by a QR factorization from CGEQR.
 */
 ...
 * @param[out]    work   Single complex workspace array, dimension (max(1, lwork)).
 ...
 */
void cgemqr(const char* side, const char* trans,
            ...
            INT* info)
{
    ...
    /* Read block sizes from T array (stored by CGEQR) */
    mb = (INT)crealf(T[1]);
    nb = (INT)crealf(T[2]);
    ...
    *info = 0;
    ...
    /* Choose between CGEMQRT (standard) and CLAMTSQR (tall-skinny) */
    if ((left && m <= k) || (right && n <= k)
        || (mb <= k) || (mb >= (m > n ? (m > k ? m : k) : (n > k ? n : k)))) {
        /* Use standard blocked algorithm */
        cgemqrt(side, trans, m, n, k, nb,
                A, lda, &T[5], nb, C, ldc, work, info);
    } else {
        /* Use tall-skinny algorithm */
        clamtsqr(side, trans, m, n, k, mb, nb,
