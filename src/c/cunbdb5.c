/**
 * @file cunbdb5.c
 * @brief CUNBDB5 orthogonalizes the column vector X with respect to the
 *        columns of Q.
 */
             ...
             INT* info)
{
    ...
    *info = 0;
    ...
    if (norm > n * eps) {
         ...
         *     orthogonalization. */
        c64 inv_norm = ONE / norm;
        cblas_cscal(m1, &inv_norm, X1, incx1);
        cblas_cscal(m2, &inv_norm, X2, incx2);
        cunbdb6(m1, m2, n, X1, incx1, X2, incx2, Q1, ldq1, Q2,
                ldq2, work, lwork, &childinfo);

        /* If the projection is nonzero, then return */
        if (cblas_scnrm2(m1, X1, incx1) != REALZERO
            || cblas_scnrm2(m2, X2, incx2) != REALZERO) {
            return;
        }
    }
