/**
 * @file cuncsd2by1.c
 * @brief CUNCSD2BY1 computes the CS decomposition of an M-by-Q matrix with orthonormal columns partitioned into a 2-by-1 block structure.
 */
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (r == 0) {
        ...
        if (q == 0) {
            if (wantu1 && p > 0)
                claset("A", p, p, zero_c, one, U1, ldu1);
            if (wantu2 && m - p > 0)
                claset("A", m - p, m - p, zero_c, one, U2, ldu2);
            return;
        }

        if (p == 0 && m == q) {
            if (wantu2)
                clacpy("A", m - p, q, X21, ldx21, U2, ldu2);
            if (wantv1t)
                claset("A", q, q, zero_c, one, V1T, ldv1t);
            return;
        }

        if (p == m && m == q) {
            if (wantu1)
                clacpy("A", p, q, X11, ldx11, U1, ldu1);
            if (wantv1t)
                claset("A", q, q, zero_c, one, V1T, ldv1t);
            return;
        }
    ...
    if (r == q) {
        ...
        if (wantv1t && q > 0) {
            ...
            clacpy("U", q - 1, q - 1, &X21[1 * ldx21], ldx21, &V1T[1 + 1 * ldv1t], ldv1t);
        ...
        if (wantu1 && p > 0) {
            ...
            clacpy("L", p - 1, p - 1, &X11[1], ldx11, &U1[1 + 1 * ldu1], ldu1);
            cungqr(p - 1, p - 1, p - 1, &U1[1 + 1 * ldu1], ldu1, &work[itaup1 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantu2 && m - p > 0) {
            clacpy("L", m - p, q, X21, ldx21, U2, ldu2);
        ...
        if (wantv1t && q > 0) {
            ...
            cunglq(q, q, r, V1T, ldv1t, &work[itauq1 - 1],
        ...
        if (wantu2 && m - p > 0) {
            ...
            clacpy("L", m - p - 1, m - p - 1, &X21[1], ldx21, &U2[1 + 1 * ldu2], ldu2);
            cungqr(m - p - 1, m - p - 1, m - p - 1, &U2[1 + 1 * ldu2], ldu2,
        ...
        cbbcsd("N", jobv1t, jobu2, jobu1, "T", m, m - q, m - p, theta,
        ...
        if (q > r) {
            ...
            if (wantu1) {
                clapmt(0, p, q, U1, ldu1, iwork);
            }
            if (wantv1t) {
                clapmr(0, q, q, V1T, ldv1t, iwork);
            }
        }
        ...
        if (wantu2 && m - p > 0) {
            cblas_ccopy(m - p, &work[iorbdb + p - 1], 1, U2, 1);
        }
        if (wantu1 && p > 0) {
            cblas_ccopy(p, &work[iorbdb - 1], 1, U1, 1);
            for (j = 1; j < p; j++) {
                U1[j * ldu1] = zero_c;
            }
            clacpy("L", p - 1, m - q - 1, &X11[1], ldx11, &U1[1 + 1 * ldu1], ldu1);
            cungqr(p, p, m - q, U1, ldu1, &work[itaup1 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantu2 && m - p > 0) {
            for (j = 1; j < m - p; j++) {
            ...
            clacpy("L", m - p - 1, m - q - 1, &X21[1], ldx21, &U2[1 + 1 * ldu2], ldu2);
            cungqr(m - p, m - p, m - q, U2, ldu2, &work[itaup2 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantv1t && q > 0) {
            clacpy("U", m - q, q, X21, ldx21, V1T, ldv1t);
            clacpy("U", p - (m - q), q - (m - q), &X11[(m - q) + (m - q) * ldx11], ldx11,
                   &V1T[(m - q) + (m - q) * ldv1t], ldv1t);
            clacpy("U", -p + q, q - p, &X21[(m - q) + p * ldx21], ldx21,
                   &V1T[p + p * ldv1t], ldv1t);
            cunglq(q, q, q, V1T, ldv1t, &work[itauq1 - 1],
                   &work[iorglq - 1], lorglq, &childinfo);
        }

        cbbcsd(jobu2, jobu1, "N", jobv1t, "N", m, m - p, m - q, theta,
        ...
        if (p > r) {
            ...
            if (wantu1) {
                clapmt(0, p, p, U1, ldu1, iwork);
            }
            if (wantv1t) {
                clapmr(0, p, q, V1T, ldv1t, iwork);
            }
        }
    }
}
