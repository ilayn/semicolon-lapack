/**
 * @file clals0.c
 * @brief CLALS0 applies back multiplying factors in solving the least squares
            ...
            f32* restrict rwork, INT* info)
{
    ...
    *info = 0;
    ...
    if (icompq == 0) {
        ...
        for (i = 1; i <= givptr; i++) {
            cblas_csrot(nrhs, &B[givcol[i - 1 + 1 * ldgcol]], ldb,
        ...
        cblas_ccopy(nrhs, &B[nlp1 - 1], ldb, &BX[0], ldbx);
        for (i = 2; i <= n; i++) {
            cblas_ccopy(nrhs, &B[perm[i - 1]], ldb, &BX[i - 1], ldbx);
        }
        ...
        if (k == 1) {
            cblas_ccopy(nrhs, BX, ldbx, B, ldb);
            if (Z[0] < 0.0f) {
                cblas_csscal(nrhs, -1.0f, B, ldb);
            }
        } else {
            for (j = 1; j <= k; j++) {
                diflj = difl[j - 1];
                ...
                for (jcol = 1; jcol <= nrhs; jcol++) {
                    for (jrow = 1; jrow <= k; jrow++) {
                        rwork[i] = crealf(BX[(jrow - 1) + (jcol - 1) * ldbx]);
                        i++;
                    }
                }
                cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, 1.0f,
                            &rwork[k + nrhs * 2], k, rwork, 1, 0.0f, &rwork[k], 1);
                i = k + nrhs * 2;
                for (jcol = 1; jcol <= nrhs; jcol++) {
                    for (jrow = 1; jrow <= k; jrow++) {
                        rwork[i] = cimagf(BX[(jrow - 1) + (jcol - 1) * ldbx]);
                        i++;
                    }
                }
                cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, 1.0f,
                            &rwork[k + nrhs * 2], k, rwork, 1, 0.0f, &rwork[k + nrhs], 1);
                for (jcol = 1; jcol <= nrhs; jcol++) {
                    B[(j - 1) + (jcol - 1) * ldb] = CMPLXF(rwork[k + jcol - 1],
                                                           rwork[k + nrhs + jcol - 1]);
                }
                clascl("G", 0, 0, temp, 1.0f, 1, nrhs, &B[j - 1], ldb, info);
            }
        }
        ...
        if (k < (m > n ? m : n)) {
            clacpy("A", n - k, nrhs, &BX[k], ldbx, &B[k], ldb);
        }
    } else {
        ...
        if (k == 1) {
            cblas_ccopy(nrhs, B, ldb, BX, ldbx);
        } else {
            for (j = 1; j <= k; j++) {
                dsigj = poles[j - 1 + 1 * ldgnum];
                if (Z[j - 1] == 0.0f) {
                ...
                for (jcol = 1; jcol <= nrhs; jcol++) {
                    for (jrow = 1; jrow <= k; jrow++) {
                        rwork[i] = crealf(B[(jrow - 1) + (jcol - 1) * ldb]);
                        i++;
                    }
                }
                cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, 1.0f,
                ...
                for (jcol = 1; jcol <= nrhs; jcol++) {
                    for (jrow = 1; jrow <= k; jrow++) {
                        rwork[i] = cimagf(B[(jrow - 1) + (jcol - 1) * ldb]);
                        i++;
                    }
                }
                cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, 1.0f,
                            &rwork[k + nrhs * 2], k, rwork, 1, 0.0f, &rwork[k + nrhs], 1);
                for (jcol = 1; jcol <= nrhs; jcol++) {
                    BX[(j - 1) + (jcol - 1) * ldbx] = CMPLXF(rwork[k + jcol - 1],
                                                              rwork[k + nrhs + jcol - 1]);
                }
            }
        }
        ...
        if (sqre == 1) {
            cblas_ccopy(nrhs, &B[m - 1], ldb, &BX[m - 1], ldbx);
            cblas_csrot(nrhs, &BX[0], ldbx, &BX[m - 1], ldbx, c, s);
        }
        if (k < (m > n ? m : n)) {
            clacpy("A", n - k, nrhs, &B[k], ldb, &BX[k], ldbx);
        }

        /* Step (3R): permute rows of B. */

        cblas_ccopy(nrhs, &BX[0], ldbx, &B[nlp1 - 1], ldb);
        if (sqre == 1) {
            cblas_ccopy(nrhs, &BX[m - 1], ldbx, &B[m - 1], ldb);
        }
        for (i = 2; i <= n; i++) {
            cblas_ccopy(nrhs, &BX[i - 1], ldbx, &B[perm[i - 1]], ldb);
        }
