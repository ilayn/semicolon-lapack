/**
 * @file chptrs.c
 * @brief CHPTRS solves a system of linear equations with a Hermitian matrix
 ...
 *                       to obtain the factor U or L as computed by CHPTRF,
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                akm1k = AP[kc + k - 1];
                akm1 = AP[kc - 1] / akm1k;
                ak = AP[kc + k] / conjf(akm1k);
                denom = akm1 * ak - ONE;
                for (j = 0; j < nrhs; j++) {
                    bkm1 = B[(k - 1) + j * ldb] / akm1k;
                    bk = B[k + j * ldb] / conjf(akm1k);
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                ...
                if (k > 0) {
                    clacgv(nrhs, &B[k], ldb);
                    cblas_cgemv(CblasColMajor, CblasConjTrans, k, nrhs, &NEG_ONE, B,
                                ldb, &AP[kc], 1, &ONE, &B[k], ldb);
                    clacgv(nrhs, &B[k], ldb);
                }
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                ...
                /* Multiply by the inverse of the diagonal block. */
                akm1k = AP[kc + 1];
                akm1 = AP[kc] / conjf(akm1k);
                ak = AP[kc + n - k] / akm1k;
                denom = akm1 * ak - ONE;
                for (j = 0; j < nrhs; j++) {
                    bkm1 = B[k + j * ldb] / conjf(akm1k);
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    clacgv(nrhs, &B[k], ldb);
                    cblas_cgemv(CblasColMajor, CblasConjTrans, n - k - 1, nrhs, &NEG_ONE,
                                &B[k + 1], ldb, &AP[kc + 1], 1, &ONE, &B[k], ldb);
                    clacgv(nrhs, &B[k], ldb);
                }
                ...
                if (k < n - 1) {
                    ...
                    clacgv(nrhs, &B[k - 1], ldb);
                    cblas_cgemv(CblasColMajor, CblasConjTrans, n - k - 1, nrhs, &NEG_ONE,
                                &B[k + 1], ldb, &AP[kc - (n - k) + 1], 1, &ONE, &B[k - 1], ldb);
                    clacgv(nrhs, &B[k - 1], ldb);
                }
