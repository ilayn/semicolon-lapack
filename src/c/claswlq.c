/**
 * @file claswlq.c
 * @brief CLASWLQ computes a blocked Short-Wide LQ factorization of a
...
/**
 * CLASWLQ computes a blocked Short-Wide LQ factorization of
 ...
 * @param[out]    T      Single complex array, dimension
 ...
 */
void claswlq(const INT m, const INT n, const INT mb, const INT nb,
             ...
             INT* info)
{
    ...
    *info = 0;
    ...
    if (ii < n) {
        ctplqt(m, kk, 0, mb, &A[0 + 0 * lda], lda, &A[0 + ii * lda],
               lda, &T[0 + ctr * m * ldt], ldt, work, info);
    }
