/**
 * @file clamtsqr.c
 * @brief CLAMTSQR overwrites the general complex M-by-N matrix C with
 ...
 *                       The blocked elementary reflectors as returned by CLATSQR.
    ...
    *info = 0;
    ...
    if (left && notran) {
        ...
        if (kk > 0) {
            ...
            ctpmqrt("L", "N", kk, n, k, 0, nb, &A[ii], lda,
        ...
        for (i = ii - (mb - k); i >= mb; i -= (mb - k)) {
             ...
             */
            ctr = ctr - 1;
            ctpmqrt("L", "N", mb - k, n, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[i], ldc, work, info);
        }
        ...
        for (i = mb; i <= ii - mb + k; i += (mb - k)) {
             ...
             */
            ctpmqrt("L", "C", mb - k, n, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[i], ldc, work, info);
            ctr = ctr + 1;
        }
        ...
        if (ii < m) {
             ...
             */
            ctpmqrt("L", "C", kk, n, k, 0, nb, &A[ii], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[ii], ldc, work, info);
        }
        ...
        if (kk > 0) {
            ii = n - kk;  /* 0-based */
            ctpmqrt("R", "C", m, kk, k, 0, nb, &A[ii], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
        ...
        for (i = ii - (mb - k); i >= mb; i -= (mb - k)) {
             ...
             */
            ctr = ctr - 1;
            ctpmqrt("R", "C", m, mb - k, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
        ...
        for (i = mb; i <= ii - mb + k; i += (mb - k)) {
             ...
             * Multiply Q to the current block of C
             */
            ctpmqrt("R", "N", m, mb - k, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[0 + i * ldc], ldc, work, info);
            ctr = ctr + 1;
        }
        ...
        if (ii < n) {
             ...
             * Multiply Q to the last block of C
             */
            ctpmqrt("R", "N", m, kk, k, 0, nb, &A[ii], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[0 + ii * ldc], ldc, work, info);
        }
    }
