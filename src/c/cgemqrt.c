/**
 * @file cgemqrt.c
 * @brief CGEMQRT overwrites the general complex M-by-N matrix C with the product
 ...
 *                       in CGEQRT.
 * @param[in]     V      Single complex array, dimension (ldv, k).
 *                       The i-th column must contain the vector which defines
 *                       the elementary reflector H(i), for i = 0,1,...,k-1,
 *                       as returned by CGEQRT in the first k columns of its
 *                       array argument A.
 ...
 *                       as returned by CGEQRT, stored as an nb-by-k matrix.
 ...
 */
void cgemqrt(const char* side, const char* trans,
