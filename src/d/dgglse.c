/**
 * @file dgglse.c
 * @brief DGGLSE solves the linear equality-constrained least squares (LSE) problem.
 */
 ...
 *          DGEQRF, DGERQF, DORMQR and DORMRQ.
 *
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    dggrqf(p, m, n, B, ldb, work, A, lda, &work[p],
           &work[p + mn], lwork - p - mn, info);
    lopt = (INT)work[p + mn];

    dormqr("L", "T", m, 1, mn, A, lda, &work[p],
           C, (1 > m ? 1 : m), &work[p + mn], lwork - p - mn, info);
    lopt = (lopt > (INT)work[p + mn]) ? lopt : (INT)work[p + mn];

    if (p > 0) {
        dtrtrs("U", "N", "N", p, 1, &B[0 + (n - p) * ldb], ldb, D, p, info);
        ...
        cblas_dcopy(p, D, 1, &X[n - p], 1);

        cblas_dgemv(CblasColMajor, CblasNoTrans, n - p, p, -one,
                    &A[0 + (n - p) * lda], lda, D, 1, one, C, 1);
    }

    if (n > p) {
        dtrtrs("U", "N", "N", n - p, 1, A, lda, C, n - p, info);
        ...
        cblas_dcopy(n - p, C, 1, X, 1);
    }

    if (m < n) {
        nr = m + p - n;
        if (nr > 0) {
            cblas_dgemv(CblasColMajor, CblasNoTrans, nr, n - m, -one,
                        &A[(n - p) + m * lda], lda, &D[nr], 1, one, &C[n - p], 1);
        }
    } else {
        nr = p;
    }
    if (nr > 0) {
        ...
        cblas_daxpy(nr, -one, D, 1, &C[n - p], 1);
    }
