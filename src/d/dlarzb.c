/**
 * @file dlarzb.c
 * @brief DLARZB applies a block reflector or its transpose to a general
 *        distributed matrix.
 */
            ...
            f64* restrict work, const INT ldwork)
{
    ...
    if (side[0] == 'L' || side[0] == 'l') {
         ...
         */
        for (j = 0; j < k; j++) {
            cblas_dcopy(n, &C[j + 0 * ldc], ldc, &work[0 + j * ldwork], 1);
        }
         ...
         */
        for (j = 0; j < k; j++) {
            cblas_dcopy(m, &C[0 + j * ldc], 1, &work[0 + j * ldwork], 1);
        }
