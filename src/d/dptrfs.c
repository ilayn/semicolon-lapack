/**
 * @file dptrfs.c
 * @brief DPTRFS improves the computed solution to a system of linear equations
...
refine:
        if (n == 1) {
            ...
            dx = D[0] * X[0 + j * ldx];
            work[n] = bi - dx;
            work[0] = fabs(bi) + fabs(dx);
        } else {
            bi = B[0 + j * ldb];
            dx = D[0] * X[0 + j * ldx];
            ex = E[0] * X[1 + j * ldx];
            work[n] = bi - dx - ex;
            work[0] = fabs(bi) + fabs(dx) + fabs(ex);
            for (i = 1; i < n - 1; i++) {
                bi = B[i + j * ldb];
                ...
                work[i] = fabs(bi) + fabs(cx) + fabs(dx) + fabs(ex);
            }
            bi = B[(n - 1) + j * ldb];
            cx = E[n - 2] * X[(n - 2) + j * ldx];
            dx = D[n - 1] * X[(n - 1) + j * ldx];
            work[n + n - 1] = bi - cx - dx;
            work[n - 1] = fabs(bi) + fabs(cx) + fabs(dx);
        }
        ...
        if (berr[j] > eps && TWO * berr[j] <= lstres && count <= ITMAX) {
            ...
            dpttrs(n, 1, DF, EF, &work[n], n, &info_local);
            cblas_daxpy(n, ONE, &work[n], 1, &X[0 + j * ldx], 1);
            lstres = berr[j];
            count = count + 1;
            goto refine;
        }
        ...
        work[0] = ONE;
        for (i = 1; i < n; i++) {
            work[i] = ONE + work[i - 1] * fabs(EF[i - 1]);
        }
        ...
        for (i = n - 2; i >= 0; i--) {
            work[i] = work[i] / DF[i] + work[i + 1] * fabs(EF[i]);
        }

        /* Compute norm(inv(A)) = max(x(i)), 0<=i<n. */

        ix = cblas_idamax(n, work, 1);
        ferr[j] = ferr[j] * fabs(work[ix]);
