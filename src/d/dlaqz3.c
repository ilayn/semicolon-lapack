/**
 * @file dlaqz3.c
 * @brief DLAQZ3 performs aggressive early deflation (AED).
 */
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    ifst = 0;
    ilst = jw - 1;
    dtgexc(1, 1, jw, A, lda, B, ldb, QC, ldqc, ZC, ldzc,
           &ifst, &ilst, work, -1, &dtgexc_info);
    lworkreq = (int)work[0];
    dlaqz0("S", "V", "V", jw, 0, jw - 1, &A[kwtop + kwtop * lda], lda,
    ...
    dlacpy("A", jw, jw, &A[kwtop + kwtop * lda], lda, work, jw);
    dlacpy("A", jw, jw, &B[kwtop + kwtop * ldb], ldb, &work[jw * jw], jw);

    /* Transform window to real Schur form */
    dlaset("F", jw, jw, ZERO, ONE, QC, ldqc);
    dlaset("F", jw, jw, ZERO, ONE, ZC, ldzc);
    dlaqz0("S", "V", "V", jw, 0, jw - 1, &A[kwtop + kwtop * lda], lda,
           &B[kwtop + kwtop * ldb], ldb, alphar, alphai, beta, QC, ldqc,
    ...
    if (qz_small_info != 0) {
        ...
        *nd = 0;
        *ns = jw - qz_small_info;
        dlacpy("A", jw, jw, work, jw, &A[kwtop + kwtop * lda], lda);
        dlacpy("A", jw, jw, &work[jw * jw], jw, &B[kwtop + kwtop * ldb], ldb);
        return;
    }
    ...
    if (kwtop == ilo || s == ZERO) {
        ...
        while (k <= jw - 1) {
            ...
            if (bulge) {
                ...
                temp = fabs(A[kwbot + kwbot * lda]) +
                       sqrt(fabs(A[kwbot + (kwbot - 1) * lda])) *
                       sqrt(fabs(A[(kwbot - 1) + kwbot * lda]));
                if (temp == ZERO) {
                    temp = fabs(s);
                }
                {
                    ...
                    if (maxval <= thresh) {
                        ...
                        ifst = kwbot - kwtop;
                        ilst = k2;
                        dtgexc(1, 1, jw, &A[kwtop + kwtop * lda], lda,
    ...
    *ns = jw - *nd;
    ...
    while (k <= ihi) {
        ...
        if (bulge) {
            ...
            dlag2(&A[k + k * lda], lda, &B[k + k * ldb], ldb, safmin,
    ...
    if (kwtop != ilo && s != ZERO) {
        ...
        for (k = kwbot - 1; k >= kwtop; k--) {
            dlartg(A[k + (kwtop - 1) * lda], A[(k + 1) + (kwtop - 1) * lda],
                   &c1, &s1, &temp);
            A[k + (kwtop - 1) * lda] = temp;
            A[(k + 1) + (kwtop - 1) * lda] = ZERO;
            k2 = (kwtop > k - 1) ? kwtop : k - 1;
            cblas_drot(ihi - k2 + 1, &A[k + k2 * lda], lda, &A[(k + 1) + k2 * lda], lda, c1, s1);
            cblas_drot(ihi - (k - 1) + 1, &B[k + (k - 1) * ldb], ldb,
                       &B[(k + 1) + (k - 1) * ldb], ldb, c1, s1);
            cblas_drot(jw, &QC[0 + (k - kwtop) * ldqc], 1,
        ...
        while (k >= kwtop) {
            if ((k >= kwtop + 1) && A[(k + 1) + (k - 1) * lda] != ZERO) {
                ...
                for (k2 = k; k2 <= kwbot - 2; k2++) {
                    /* Move shift down */
                    dlartg(B[(k2 + 1) + (k2 + 1) * ldb], B[(k2 + 1) + k2 * ldb],
                           &c1, &s1, &temp);
                    B[(k2 + 1) + (k2 + 1) * ldb] = temp;
                    B[(k2 + 1) + k2 * ldb] = ZERO;
                    cblas_drot(k2 + 2 - istartm + 1, &A[istartm + (k2 + 1) * lda], 1,
                               &A[istartm + k2 * lda], 1, c1, s1);
                    cblas_drot(k2 - istartm + 1, &B[istartm + (k2 + 1) * ldb], 1,
                               &B[istartm + k2 * ldb], 1, c1, s1);
                    cblas_drot(jw, &ZC[0 + (k2 + 1 - kwtop) * ldzc], 1,
                               &ZC[0 + (k2 - kwtop) * ldzc], 1, c1, s1);

                    dlartg(A[(k2 + 1) + k2 * lda], A[(k2 + 2) + k2 * lda],
                           &c1, &s1, &temp);
                    A[(k2 + 1) + k2 * lda] = temp;
                    A[(k2 + 2) + k2 * lda] = ZERO;
                    cblas_drot(istopm - k2, &A[(k2 + 1) + (k2 + 1) * lda], lda,
                               &A[(k2 + 2) + (k2 + 1) * lda], lda, c1, s1);
                    cblas_drot(istopm - k2, &B[(k2 + 1) + (k2 + 1) * ldb], ldb,
                               &B[(k2 + 2) + (k2 + 1) * ldb], ldb, c1, s1);
                    cblas_drot(jw, &QC[0 + (k2 + 1 - kwtop) * ldqc], 1,
                               &QC[0 + (k2 + 2 - kwtop) * ldqc], 1, c1, s1);
                }

                /* Remove the shift */
                dlartg(B[kwbot + kwbot * ldb], B[kwbot + (kwbot - 1) * ldb],
                       &c1, &s1, &temp);
                B[kwbot + kwbot * ldb] = temp;
                B[kwbot + (kwbot - 1) * ldb] = ZERO;
                cblas_drot(kwbot - istartm, &B[istartm + kwbot * ldb], 1,
                           &B[istartm + (kwbot - 1) * ldb], 1, c1, s1);
                cblas_drot(kwbot - istartm + 1, &A[istartm + kwbot * lda], 1,
                           &A[istartm + (kwbot - 1) * lda], 1, c1, s1);
                cblas_drot(jw, &ZC[0 + (kwbot - kwtop) * ldzc], 1,
    ...
    if (istopm - ihi > 0) {
        cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, jw, istopm - ihi, jw,
                    ONE, QC, ldqc, &A[kwtop + (ihi + 1) * lda], lda, ZERO, work, jw);
        dlacpy("A", jw, istopm - ihi, work, jw, &A[kwtop + (ihi + 1) * lda], lda);
        cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, jw, istopm - ihi, jw,
                    ONE, QC, ldqc, &B[kwtop + (ihi + 1) * ldb], ldb, ZERO, work, jw);
        dlacpy("A", jw, istopm - ihi, work, jw, &B[kwtop + (ihi + 1) * ldb], ldb);
    }
    if (ilq) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, jw, jw,
                    ONE, &Q[0 + kwtop * ldq], ldq, QC, ldqc, ZERO, work, n);
        dlacpy("A", n, jw, work, n, &Q[0 + kwtop * ldq], ldq);
    }

    if (kwtop - istartm > 0) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, kwtop - istartm, jw, jw,
                    ONE, &A[istartm + kwtop * lda], lda, ZC, ldzc, ZERO, work, kwtop - istartm);
        dlacpy("A", kwtop - istartm, jw, work, kwtop - istartm, &A[istartm + kwtop * lda], lda);
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, kwtop - istartm, jw, jw,
                    ONE, &B[istartm + kwtop * ldb], ldb, ZC, ldzc, ZERO, work, kwtop - istartm);
        dlacpy("A", kwtop - istartm, jw, work, kwtop - istartm, &B[istartm + kwtop * ldb], ldb);
    }
    if (ilz) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, jw, jw,
                    ONE, &Z[0 + kwtop * ldz], ldz, ZC, ldzc, ZERO, work, n);
        dlacpy("A", n, jw, work, n, &Z[0 + kwtop * ldz], ldz);
    }
}
