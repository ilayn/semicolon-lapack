/**
 * @file dorbdb4.c
 * @brief DORBDB4 simultaneously bidiagonalizes the blocks of a tall and skinny matrix with orthonormal columns.
 */
    ...
    *info = 0;
    ...
    for (i = 0; i < m - q; i++) {
        ...
        if (i == 0) {
            ...
            dorbdb5(p, m - p, q, &phantom[0], 1, &phantom[p], 1,
                    X11, ldx11, X21, ldx21, &work[iorbdb5], lorbdb5, &childinfo);
            cblas_dscal(p, negone, &phantom[0], 1);
            dlarfgp(p, &phantom[0], &phantom[1], 1, &taup1[0]);
            dlarfgp(m - p, &phantom[p], &phantom[p + 1], 1, &taup2[0]);
            theta[i] = atan2(phantom[0], phantom[p]);
            c = cos(theta[i]);
            s = sin(theta[i]);
            dlarf1f("L", p, q, &phantom[0], 1, taup1[0], X11, ldx11, &work[ilarf]);
            dlarf1f("L", m - p, q, &phantom[p], 1, taup2[0], X21, ldx21, &work[ilarf]);
        } else {
            dorbdb5(p - i, m - p - i, q - i,
                    &X11[i + (i - 1) * ldx11], 1, &X21[i + (i - 1) * ldx21], 1,
                    &X11[i + i * ldx11], ldx11, &X21[i + i * ldx21], ldx21,
                    &work[iorbdb5], lorbdb5, &childinfo);
            cblas_dscal(p - i, negone, &X11[i + (i - 1) * ldx11], 1);
            dlarfgp(p - i, &X11[i + (i - 1) * ldx11], &X11[(i + 1) + (i - 1) * ldx11], 1,
                    &taup1[i]);
            dlarfgp(m - p - i, &X21[i + (i - 1) * ldx21], &X21[(i + 1) + (i - 1) * ldx21], 1,
                    &taup2[i]);
            theta[i] = atan2(X11[i + (i - 1) * ldx11], X21[i + (i - 1) * ldx21]);
            c = cos(theta[i]);
            s = sin(theta[i]);
            dlarf1f("L", p - i, q - i, &X11[i + (i - 1) * ldx11], 1, taup1[i],
                    &X11[i + i * ldx11], ldx11, &work[ilarf]);
            dlarf1f("L", m - p - i, q - i, &X21[i + (i - 1) * ldx21], 1, taup2[i],
                    &X21[i + i * ldx21], ldx21, &work[ilarf]);
        }

        cblas_drot(q - i, &X11[i + i * ldx11], ldx11, &X21[i + i * ldx21], ldx21, s, -c);
        dlarfgp(q - i, &X21[i + i * ldx21], &X21[i + (i + 1) * ldx21], ldx21, &tauq1[i]);
        c = X21[i + i * ldx21];
        dlarf1f("R", p - i - 1, q - i, &X21[i + i * ldx21], ldx21, tauq1[i],
                &X11[(i + 1) + i * ldx11], ldx11, &work[ilarf]);
        dlarf1f("R", m - p - i - 1, q - i, &X21[i + i * ldx21], ldx21, tauq1[i],
                &X21[(i + 1) + i * ldx21], ldx21, &work[ilarf]);
        if (i < m - q - 1) {
            double nrm1 = cblas_dnrm2(p - i - 1, &X11[(i + 1) + i * ldx11], 1);
            double nrm2 = cblas_dnrm2(m - p - i - 1, &X21[(i + 1) + i * ldx21], 1);
    ...
    for (i = m - q; i < p; i++) {
        dlarfgp(q - i, &X11[i + i * ldx11], &X11[i + (i + 1) * ldx11], ldx11, &tauq1[i]);
        dlarf1f("R", p - i - 1, q - i, &X11[i + i * ldx11], ldx11, tauq1[i],
                &X11[(i + 1) + i * ldx11], ldx11, &work[ilarf]);
        dlarf1f("R", q - p, q - i, &X11[i + i * ldx11], ldx11, tauq1[i],
                &X21[(m - q) + i * ldx21], ldx21, &work[ilarf]);
    }

    for (i = p; i < q; i++) {
        dlarfgp(q - i, &X21[(m - q + i - p) + i * ldx21],
                &X21[(m - q + i - p) + (i + 1) * ldx21], ldx21, &tauq1[i]);
        dlarf1f("R", q - i - 1, q - i, &X21[(m - q + i - p) + i * ldx21], ldx21, tauq1[i],
