            ...
            if (ip == 0) {
                ...
                for (j = ki - 1; j >= 0; j--) {
                    ...
                    if (j1 == j2) {
                        ...
                        dlaln2(0, 1, 1, smin, 1.0, &T[j + j * ldt],
                               ldt, 1.0, 1.0, &work[j + n], n, wr,
                               0.0, x, 2, &scale, &xnorm, &ierr);
                        ...
                        /* Scale if necessary */
                        if (scale != 1.0)
                            cblas_dscal(ki + 1, scale, &work[n], 1);
                        work[j + n] = x[0];

                        /* Update right-hand side */
                        cblas_daxpy(j, -x[0], &T[j * ldt], 1, &work[n], 1);

                    } else {
                        /* 2-by-2 diagonal block */
                        dlaln2(0, 2, 1, smin, 1.0,
                               &T[(j - 1) + (j - 1) * ldt], ldt, 1.0, 1.0,
                               &work[(j - 1) + n], n, wr, 0.0, x, 2,
                               &scale, &xnorm, &ierr);
                        ...
                        /* Update right-hand side */
                        cblas_daxpy(j - 1, -x[0], &T[(j - 1) * ldt], 1, &work[n], 1);
                        cblas_daxpy(j - 1, -x[1], &T[j * ldt], 1, &work[n], 1);
                    }
                }

                /* Copy the vector x or Q*x to VR and normalize. */
                if (!over) {
                    cblas_dcopy(ki + 1, &work[n], 1, &VR[is * ldvr], 1);

                    ii = cblas_idamax(ki + 1, &VR[is * ldvr], 1);
                    remax = 1.0 / fabs(VR[ii + is * ldvr]);
                    ...
                    if (ki > 0)
                        cblas_dgemv(CblasColMajor, CblasNoTrans, n, ki, 1.0, VR, ldvr,
                                    &work[n], 1, work[ki + n], &VR[ki * ldvr], 1);

                    ii = cblas_idamax(n, &VR[ki * ldvr], 1);
                    remax = 1.0 / fabs(VR[ii + ki * ldvr]);
                ...
                for (j = ki - 2; j >= 0; j--) {
                    ...
                    if (j1 == j2) {
                        /* 1-by-1 diagonal block */
                        dlaln2(0, 1, 2, smin, 1.0, &T[j + j * ldt],
                               ldt, 1.0, 1.0, &work[j + n], n, wr, wi,
                        ...
                        if (scale != 1.0) {
                            cblas_dscal(ki + 1, scale, &work[n], 1);
                            cblas_dscal(ki + 1, scale, &work[n2], 1);
                        }
                        work[j + n] = x[0];
                        work[j + n2] = x[2];
                        ...
                        cblas_daxpy(j, -x[0], &T[j * ldt], 1, &work[n], 1);
                        cblas_daxpy(j, -x[2], &T[j * ldt], 1, &work[n2], 1);

                    } else {
                        /* 2-by-2 diagonal block */
                        dlaln2(0, 2, 2, smin, 1.0,
                               &T[(j - 1) + (j - 1) * ldt], ldt, 1.0, 1.0,
                        ...
                        cblas_daxpy(j - 1, -x[0], &T[(j - 1) * ldt], 1, &work[n], 1);
                        cblas_daxpy(j - 1, -x[1], &T[j * ldt], 1, &work[n], 1);
                        cblas_daxpy(j - 1, -x[2], &T[(j - 1) * ldt], 1, &work[n2], 1);
                        cblas_daxpy(j - 1, -x[3], &T[j * ldt], 1, &work[n2], 1);
                    }
                }

                /* Copy the vector x or Q*x to VR and normalize. */
                if (!over) {
                    cblas_dcopy(ki + 1, &work[n], 1, &VR[(is - 1) * ldvr], 1);
                    cblas_dcopy(ki + 1, &work[n2], 1, &VR[is * ldvr], 1);
                    ...
                    remax = 1.0 / emax;
                    cblas_dscal(ki + 1, remax, &VR[(is - 1) * ldvr], 1);
                    cblas_dscal(ki + 1, remax, &VR[is * ldvr], 1);
                    ...
                    if (ki > 1) {
                        cblas_dgemv(CblasColMajor, CblasNoTrans, n, ki - 1, 1.0, VR, ldvr,
                        ...
                        cblas_dscal(n, work[(ki - 1) + n], &VR[(ki - 1) * ldvr], 1);
                        cblas_dscal(n, work[ki + n2], &VR[ki * ldvr], 1);
                    }
            ...
            if (ip == 0) {
                ...
                for (j = ki + 1; j < n; j++) {
                    ...
                    if (j1 == j2) {
                        ...
                        if (work[j] > vcrit) {
                            rec = 1.0 / vmax;
                            cblas_dscal(n - ki, rec, &work[ki + n], 1);
                            vmax = 1.0;
                            vcrit = bignum;
                        }

                        work[j + n] -= cblas_ddot(j - ki - 1, &T[(ki + 1) + j * ldt], 1,
                                                  &work[(ki + 1) + n], 1);
                        ...
                        /* Scale if necessary */
                        if (scale != 1.0)
                            cblas_dscal(n - ki, scale, &work[ki + n], 1);
                        work[j + n] = x[0];
                        vmax = fabs(work[j + n]);
                        vcrit = bignum / vmax;
                        ...
                        work[(j + 1) + n] -= cblas_ddot(j - ki - 1, &T[(ki + 1) + (j + 1) * ldt], 1,
                                                        &work[(ki + 1) + n], 1);

                        /* Solve
                         *   [T(J,J)-WR   T(J,J+1)     ]**T * X = SCALE*( WORK1 )
                         *   [T(J+1,J)    T(J+1,J+1)-WR]                ( WORK2 ) */
                        dlaln2(1, 2, 1, smin, 1.0, &T[j + j * ldt],
                               ldt, 1.0, 1.0, &work[j + n], n, wr,
                               0.0, x, 2, &scale, &xnorm, &ierr);
                        ...
                        vmax = fabs(work[j + n]);
                        if (fabs(work[(j + 1) + n]) > vmax) vmax = fabs(work[(j + 1) + n]);
                        vcrit = bignum / vmax;
                    }
                }
                ...
                if (!over) {
                    cblas_dcopy(n - ki, &work[ki + n], 1, &VL[ki + is * ldvl], 1);

                    ii = cblas_idamax(n - ki, &VL[ki + is * ldvl], 1) + ki;
                    remax = 1.0 / fabs(VL[ii + is * ldvl]);
                    ...
                    if (ki < n - 1)
                        cblas_dgemv(CblasColMajor, CblasNoTrans, n, n - ki - 1, 1.0,
                                    &VL[(ki + 1) * ldvl], ldvl,
                                    &work[(ki + 1) + n], 1, work[ki + n],
                                    &VL[ki * ldvl], 1);

                    ii = cblas_idamax(n, &VL[ki * ldvl], 1);
                    remax = 1.0 / fabs(VL[ii + ki * ldvl]);
                ...
                for (j = ki + 2; j < n; j++) {
                    ...
                    if (j1 == j2) {
                        ...
                        if (work[j] > vcrit) {
                            rec = 1.0 / vmax;
                            cblas_dscal(n - ki, rec, &work[ki + n], 1);
                            cblas_dscal(n - ki, rec, &work[ki + n2], 1);
                            vmax = 1.0;
                            vcrit = bignum;
                        }

                        work[j + n] -= cblas_ddot(j - ki - 2, &T[(ki + 2) + j * ldt], 1,
                                                  &work[(ki + 2) + n], 1);
                        work[j + n2] -= cblas_ddot(j - ki - 2, &T[(ki + 2) + j * ldt], 1,
                                                   &work[(ki + 2) + n2], 1);
                        ...
                        if (scale != 1.0) {
                            cblas_dscal(n - ki, scale, &work[ki + n], 1);
                            cblas_dscal(n - ki, scale, &work[ki + n2], 1);
                        }
                        work[j + n] = x[0];
                        work[j + n2] = x[2];
                        vmax = fabs(work[j + n]);
                        if (fabs(work[j + n2]) > vmax) vmax = fabs(work[j + n2]);
                        vcrit = bignum / vmax;
                        ...
                        work[(j + 1) + n] -= cblas_ddot(j - ki - 2, &T[(ki + 2) + (j + 1) * ldt], 1,
                                                        &work[(ki + 2) + n], 1);

                        work[(j + 1) + n2] -= cblas_ddot(j - ki - 2, &T[(ki + 2) + (j + 1) * ldt], 1,
                                                         &work[(ki + 2) + n2], 1);

                        /* Solve 2-by-2 complex linear equation
                         *   ([T(j,j)   T(j,j+1)  ]**T-(wr-i*wi)*I)*X = SCALE*B
                         *   ([T(j+1,j) T(j+1,j+1)]               ) */
                        dlaln2(1, 2, 2, smin, 1.0, &T[j + j * ldt],
                               ldt, 1.0, 1.0, &work[j + n], n, wr,
                               -wi, x, 2, &scale, &xnorm, &ierr);
                ...
                if (!over) {
                    cblas_dcopy(n - ki, &work[ki + n], 1, &VL[ki + is * ldvl], 1);
                    cblas_dcopy(n - ki, &work[ki + n2], 1, &VL[ki + (is + 1) * ldvl], 1);
                    ...
                    if (ki < n - 2) {
                        cblas_dgemv(CblasColMajor, CblasNoTrans, n, n - ki - 2, 1.0,
                                    &VL[(ki + 2) * ldvl], ldvl,
                        ...
                        cblas_dscal(n, work[ki + n], &VL[ki * ldvl], 1);
                        cblas_dscal(n, work[(ki + 1) + n2], &VL[(ki + 1) * ldvl], 1);
                    }
