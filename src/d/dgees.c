           ...
           double* work, const int lwork, int* bwork, int* info)
{
    ...
    *info = 0;
    ...
    if (wantvs) {
        ...
        dorghr(n, ilo, ihi, VS, ldvs, &work[itau], &work[iwrk], lwork - iwrk, &ierr);
    }
    ...
    *sdim = 0;
    ...
    iwrk = itau;
    dhseqr("S", jobvs, n, ilo, ihi, A, lda, wr, wi, VS, ldvs,
    ...
    if (wantst && *info == 0) {
        ...
        dtrsen("N", jobvs, bwork, n, A, lda, VS, ldvs, wr, wi,
    ...
    if (scalea) {
        ...
        if (cscale == smlnum) {
             ...
             */
            if (ieval > 0) {
                i1 = ieval;  /* Already 0-based adjustment from dhseqr */
                i2 = ihi - 2;
                dlascl("G", 0, 0, cscale, anrm, ilo - 1, 1, wi,
            ...
            for (i = i1; i <= i2; i++) {
                ...
                if (wi[i] == ZERO) {
                    ...
                    if (A[(i + 1) + i * lda] == ZERO) {
                        ...
                        wi[i + 1] = ZERO;
                        if (i > 0)
                            cblas_dswap(i, &A[i * lda], 1, &A[(i + 1) * lda], 1);
                        if (n > i + 2)
                            cblas_dswap(n - i - 2, &A[i + (i + 2) * lda], lda,
                                        &A[(i + 1) + (i + 2) * lda], lda);
                        if (wantvs)
                            cblas_dswap(n, &VS[i * ldvs], 1, &VS[(i + 1) * ldvs], 1);
        ...
        dlascl("G", 0, 0, cscale, anrm, n - ieval, 1, &wi[ieval],
