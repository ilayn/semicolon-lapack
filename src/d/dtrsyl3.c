             ...
             int* info)
{
     ...
     * From ilaenv.f lines 477-485: for DTRSYL (real precision),
    ...
    *scale = ONE;
    ...
    for (k = 0; k < nba; k++) {
        ...
        for (l = k; l < nba; l++) {
            ...
            l2 = iwork[l + 1];
            if (notrna) {
                swork[k + (awrk + l) * ldswork] = dlange("I", k2 - k1, l2 - l1,
                                                          &A[k1 + l1 * lda], lda, NULL);
            } else {
                swork[l + (awrk + k) * ldswork] = dlange("1", k2 - k1, l2 - l1,
                                                          &A[k1 + l1 * lda], lda, NULL);
            }
        }
    }
    ...
    for (k = 0; k < nbb; k++) {
        ...
        for (l = k; l < nbb; l++) {
            ...
            l2 = iwork[pc + l + 1];
            if (notrnb) {
                swork[k + (bwrk + l) * ldswork] = dlange("I", k2 - k1, l2 - l1,
                                                          &B[k1 + l1 * ldb], ldb, NULL);
            } else {
                swork[l + (bwrk + k) * ldswork] = dlange("1", k2 - k1, l2 - l1,
                                                          &B[k1 + l1 * ldb], ldb, NULL);
            }
        }
    }
    ...
    if (notrna && notrnb) {
        ...
        for (k = nba - 1; k >= 0; k--) {
            ...
            for (l = 0; l < nbb; l++) {
                ...
                dtrsyl(trana, tranb, isgn, k2 - k1, l2 - l1,
                       &A[k1 + k1 * lda], lda,
                       &B[l1 + l1 * ldb], ldb,
                ...
                *info = (*info > iinfo) ? *info : iinfo;
                ...
                xnrm = dlange("I", k2 - k1, l2 - l1, &C[k1 + l1 * ldc], ldc, NULL);
                ...
                for (i = k - 1; i >= 0; i--) {
                     ...
                     */
                    cnrm = dlange("I", i2 - i1, l2 - l1, &C[i1 + l1 * ldc],
                                  ldc, NULL);
                    ...
                    if (scaloc * scamin == ZERO) {
                        ...
                        scamin = scamin / ldexp(1.0, ilogb(scaloc));
                        scaloc = scaloc / ldexp(1.0, ilogb(scaloc));
                    }
                    cnrm = cnrm * scaloc;
                    xnrm = xnrm * scaloc;
                    ...
                    if (scal != ONE) {
                        for (jj = l1; jj < l2; jj++) {
                            cblas_dscal(k2 - k1, scal, &C[k1 + jj * ldc], 1);
                        }
                    }
                    ...
                    if (scal != ONE) {
                        for (ll = l1; ll < l2; ll++) {
                            cblas_dscal(i2 - i1, scal, &C[i1 + ll * ldc], 1);
                        }
                    }
                ...
                for (j = l + 1; j < nbb; j++) {
                     ...
                     * simulating consistent scaling.
                     */
                    cnrm = dlange("I", k2 - k1, j2 - j1, &C[k1 + j1 * ldc],
                                  ldc, NULL);
                    ...
                    if (scal != ONE) {
                        for (ll = l1; ll < l2; ll++) {
                            cblas_dscal(k2 - k1, scal, &C[k1 + ll * ldc], 1);
                        }
                    }
    ...
    *scale = swork[0];
    ...
    if (buf != ONE && buf > ZERO) {
        ...
        buf = buf * scaloc;
        dlascl("G", -1, -1, ONE, scaloc, m, n, C, ldc, &iinfo);
    }
