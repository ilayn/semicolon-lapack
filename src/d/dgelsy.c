/**
 * @file dgelsy.c
 * @brief DGELSY computes the minimum-norm solution to a real linear least
            ...
            int *info)
{
    ...
    *info = 0;
    ...
    bnrm = dlange("M", m, nrhs, B, ldb, NULL);
    ibscl = 0;
    ...
    while (*rank < mn) {
        ...
        dlaic1(IMIN, *rank, &work[ismin], smin,
               &A[0 + i * lda], A[i + i * lda],
               &sminpr, &s1, &c1);
        dlaic1(IMAX, *rank, &work[ismax], smax,
               &A[0 + i * lda], A[i + i * lda],
               &smaxpr, &s2, &c2);
    ...
    if (*rank < n) {
        ...
        dtzrzf(*rank, n, A, lda, &work[mn], &work[2 * mn],
               lwork - 2 * mn, &iinfo);
    }
     ...
     * sub-workspace in work[2*mn..] */
    if (*rank < n) {
        dormrz("L", "T", n, nrhs, *rank, n - *rank, A, lda,
    ...
    if (iascl == 1) {
        ...
        dlascl("U", 0, 0, smlnum, anrm, *rank, *rank, A, lda, &iinfo);
    } else if (iascl == 2) {
        dlascl("G", 0, 0, anrm, bignum, n, nrhs, B, ldb, &iinfo);
        dlascl("U", 0, 0, bignum, anrm, *rank, *rank, A, lda, &iinfo);
    }
    if (ibscl == 1) {
