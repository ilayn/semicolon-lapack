/**
 * @file dlaqp2.c
 * @brief DLAQP2 computes a QR factorization with column pivoting of a matrix
            ...
            double * const restrict work)
{
    ...
    for (int i = 0; i < mn; i++) {
        ...
        if (pvt != i) {
            cblas_dswap(m, &A[0 + pvt * lda], 1, &A[0 + i * lda], 1);
            int itemp = jpvt[pvt];
            jpvt[pvt] = jpvt[i];
            jpvt[i] = itemp;
            vn1[pvt] = vn1[i];
        ...
        if (offpi < m - 1) {
            dlarfg(m - offpi, &A[offpi + i * lda],
                   &A[offpi + 1 + i * lda], 1, &tau[i]);
        } else {
            dlarfg(1, &A[m - 1 + i * lda],
                   &A[m - 1 + i * lda], 1, &tau[i]);
        }

        if (i < n - 1) {
            /* Apply H(i)**T to A(offpi:m-1, i+1:n-1) from the left. */
            dlarf1f("L", m - offpi, n - 1 - i,
        ...
        for (int j = i + 1; j < n; j++) {
            if (vn1[j] != 0.0) {
                 ...
                 */
                double temp = 1.0 - pow(fabs(A[offpi + j * lda]) / vn1[j], 2);
                temp = temp > 0.0 ? temp : 0.0;
                double temp2 = temp * pow(vn1[j] / vn2[j], 2);
                if (temp2 <= tol3z) {
                    if (offpi < m - 1) {
                        vn1[j] = cblas_dnrm2(m - offpi - 1,
                                             &A[offpi + 1 + j * lda], 1);
                        vn2[j] = vn1[j];
                    } else {
                        vn1[j] = 0.0;
