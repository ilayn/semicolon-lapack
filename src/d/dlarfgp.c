/**
 * @file dlarfgp.c
 * @brief DLARFGP generates an elementary reflector (Householder matrix)
 ...
 */
void dlarfgp(const int n, double *alpha, double * const restrict x,
             const int incx, double *tau)
{
    const double TWO = 2.0;
    const double ONE = 1.0;
    double xnorm, beta, bignum, eps, savealpha, smlnum;
    int j, knt;
    ...
    if (xnorm <= eps * fabs(*alpha)) {
        ...
        *alpha += beta;
        ...
        if (fabs(*tau) <= smlnum) {
             ...
             */
            cblas_dscal(n - 1, ONE / *alpha, x, incx);
        }
