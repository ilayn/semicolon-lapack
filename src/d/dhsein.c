/**
 * @file dhsein.c
 * @brief DHSEIN uses inverse iteration to find specified right and/or left
 ...
 * @param[in]     eigsrc  'Q': eigenvalues were found using DHSEQR, so matrix
    ...
    for (k = 0; k < n; k++) {
        if (select[k]) {
            ...
            if (kl != kln) {
                ...
                hnorm = dlanhs("I", kr - kl + 1, &H[kl + kl * ldh], ldh, work);
                if (disnan(hnorm)) {
                    *info = -6;
                    return;
                } else if (hnorm > ZERO) {
        ...
        L60:
            for (i = k - 1; i >= kl; i--) {
                if (select[i] && fabs(wr[i] - wkr) + fabs(wi[i] - wki) < eps3) {
                    wkr = wkr + eps3;
                    goto L60;
                }
            }
            ...
            if (leftv) {
                ...
                dlaein(0, noinit_flag, n - kl, &H[kl + kl * ldh], ldh,
            ...
            if (rightv) {
                ...
                dlaein(1, noinit_flag, kr + 1, H, ldh, wkr, wki,
