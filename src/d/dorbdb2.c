    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < p; i++) {
        ...
        if (i > 0) {
            cblas_drot(q - i, &X11[i + i * ldx11], ldx11,
        ...
        dorbdb5(p - i - 1, m - p - i, q - i - 1,
                &X11[(i + 1) + i * ldx11], 1, &X21[i + i * ldx21], 1,
                &X11[(i + 1) + (i + 1) * ldx11], ldx11,
                &X21[i + (i + 1) * ldx21], ldx21,
                &work[iorbdb5], lorbdb5, &childinfo);
        cblas_dscal(p - i - 1, negone, &X11[(i + 1) + i * ldx11], 1);
        dlarfgp(m - p - i, &X21[i + i * ldx21], &X21[(i + 1) + i * ldx21], 1, &taup2[i]);
        if (i < p - 1) {
            dlarfgp(p - i - 1, &X11[(i + 1) + i * ldx11], &X11[(i + 2) + i * ldx11], 1, &taup1[i]);
            phi[i] = atan2(X11[(i + 1) + i * ldx11], X21[i + i * ldx21]);
            c = cos(phi[i]);
            s = sin(phi[i]);
            dlarf1f("L", p - i - 1, q - i - 1, &X11[(i + 1) + i * ldx11], 1, taup1[i],
