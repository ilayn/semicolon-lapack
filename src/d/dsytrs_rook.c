/**
 * @file dsytrs_rook.c
 * @brief DSYTRS_ROOK solves a system of linear equations A*X = B with a real symmetric matrix A using the factorization computed by DSYTRF_ROOK.
 */
 ...
 *                      of D as determined by `dsytrf_rook`.
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                cblas_dger(CblasColMajor, k, nrhs, -1.0, &A[0 + k * lda], 1,
                           &B[k], ldb, &B[0], ldb);
                ...
                if (k > 1) {
                    cblas_dger(CblasColMajor, k - 1, nrhs, -1.0, &A[0 + k * lda], 1,
                               &B[k], ldb, &B[0], ldb);
                    cblas_dger(CblasColMajor, k - 1, nrhs, -1.0, &A[0 + (k - 1) * lda], 1,
                               &B[k - 1], ldb, &B[0], ldb);
                }
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k > 0) {
                    cblas_dgemv(CblasColMajor, CblasTrans, k, nrhs, -1.0, B,
                                ldb, &A[0 + k * lda], 1, 1.0, &B[k], ldb);
                    cblas_dgemv(CblasColMajor, CblasTrans, k, nrhs, -1.0, B,
                                ldb, &A[0 + (k + 1) * lda], 1, 1.0, &B[k + 1], ldb);
                }
        ...
        while (k < n) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    cblas_dger(CblasColMajor, n - k - 1, nrhs, -1.0, &A[k + 1 + k * lda], 1,
                               &B[k], ldb, &B[k + 1], ldb);
                }
                ...
                if (k < n - 2) {
                    cblas_dger(CblasColMajor, n - k - 2, nrhs, -1.0, &A[k + 2 + k * lda], 1,
                               &B[k], ldb, &B[k + 2], ldb);
                    cblas_dger(CblasColMajor, n - k - 2, nrhs, -1.0, &A[k + 2 + (k + 1) * lda], 1,
                               &B[k + 1], ldb, &B[k + 2], ldb);
                }
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    cblas_dgemv(CblasColMajor, CblasTrans, n - k - 1, nrhs, -1.0,
                                &B[k + 1], ldb, &A[k + 1 + k * lda], 1, 1.0, &B[k], ldb);
                    cblas_dgemv(CblasColMajor, CblasTrans, n - k - 1, nrhs, -1.0,
                                &B[k + 1], ldb, &A[k + 1 + (k - 1) * lda], 1, 1.0, &B[k - 1], ldb);
                }
