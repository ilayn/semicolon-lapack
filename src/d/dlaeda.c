/**
 * @file dlaeda.c
 * @brief DLAEDA computes the Z vector determining the rank-one modification
            ...
            f64* ztemp, INT* info)
{
    ...
    *info = 0;
    ...
    cblas_dcopy(bsiz1, &Q[qptr[curr] + bsiz1 - 1], bsiz1,
                &Z[mid - bsiz1], 1);
    cblas_dcopy(bsiz2, &Q[qptr[curr + 1]], bsiz2, &Z[mid], 1);
    ...
    for (k = 1; k <= curlvl - 1; k++) {
        ...
        cblas_dcopy(psiz1 - bsiz1, &ztemp[bsiz1], 1,
                    &Z[zptr1 + bsiz1], 1);
        if (bsiz2 > 0) {
            cblas_dgemv(CblasColMajor, CblasTrans, bsiz2, bsiz2, 1.0,
                        &Q[qptr[curr + 1]], bsiz2, &ztemp[psiz1], 1, 0.0,
                        &Z[mid], 1);
        }
        cblas_dcopy(psiz2 - bsiz2, &ztemp[psiz1 + bsiz2], 1,
                    &Z[mid + bsiz2], 1);
