            ...
            for (i = p - 1; i >= 0; i--) {
                ...
                if ((mb == 1) && (nb == 1)) {
                    ...
                    if (ijob == 0) {
                        ...
                        if (scaloc != ONE) {
                            for (k = 0; k < n; k++) {
                                cblas_dscal(m, scaloc, &C[0 + k * ldc], 1);
                                cblas_dscal(m, scaloc, &F[0 + k * ldf], 1);
                            }
                            *scale = (*scale) * scaloc;
                        }
                    } else {
                    ...
                    if (i > 0) {
                        alpha = -rhs[0];
                        cblas_daxpy(is, alpha, &A[0 + is * lda], 1, &C[0 + js * ldc], 1);
                        cblas_daxpy(is, alpha, &D[0 + is * ldd], 1, &F[0 + js * ldf], 1);
                    }
                    if (j < q - 1) {
                        cblas_daxpy(n - je - 1, rhs[1], &B[js + (je + 1) * ldb], ldb,
                                    &C[is + (je + 1) * ldc], ldc);
                        cblas_daxpy(n - je - 1, rhs[1], &E[js + (je + 1) * lde], lde,
                                    &F[is + (je + 1) * ldf], ldf);
                    }
                    ...
                    /* Substitute R(I, J) and L(I, J) into remaining equation. */
                    if (i > 0) {
                        cblas_dger(CblasColMajor, is, nb, -ONE, &A[0 + is * lda], 1,
                                   &rhs[0], 1, &C[0 + js * ldc], ldc);
                        cblas_dger(CblasColMajor, is, nb, -ONE, &D[0 + is * ldd], 1,
                                   &rhs[0], 1, &F[0 + js * ldf], ldf);
                    }
                    if (j < q - 1) {
                        cblas_daxpy(n - je - 1, rhs[2], &B[js + (je + 1) * ldb], ldb,
                                    &C[is + (je + 1) * ldc], ldc);
                        cblas_daxpy(n - je - 1, rhs[2], &E[js + (je + 1) * lde], lde,
                                    &F[is + (je + 1) * ldf], ldf);
                        cblas_daxpy(n - je - 1, rhs[3], &B[jsp1 + (je + 1) * ldb], ldb,
                                    &C[is + (je + 1) * ldc], ldc);
                        cblas_daxpy(n - je - 1, rhs[3], &E[jsp1 + (je + 1) * lde], lde,
                                    &F[is + (je + 1) * ldf], ldf);
                    }
                    ...
                    /* Substitute R(I, J) and L(I, J) into remaining equation. */
                    if (i > 0) {
                        cblas_dgemv(CblasColMajor, CblasNoTrans, is, mb, -ONE,
                    ...
                    if (j < q - 1) {
                        cblas_dger(CblasColMajor, mb, n - je - 1, ONE, &rhs[2], 1,
                    ...
                    ii = mb * nb;
                    for (jj = 0; jj < nb; jj++) {
                        cblas_dcopy(mb, &C[is + (js + jj) * ldc], 1, &rhs[k], 1);
                        cblas_dcopy(mb, &F[is + (js + jj) * ldf], 1, &rhs[ii], 1);
                        k = k + mb;
                        ii = ii + mb;
                    }
                    ...
                    ii = mb * nb;
                    for (jj = 0; jj < nb; jj++) {
                        cblas_dcopy(mb, &rhs[k], 1, &C[is + (js + jj) * ldc], 1);
                        cblas_dcopy(mb, &rhs[ii], 1, &F[is + (js + jj) * ldf], 1);
                        k = k + mb;
                        ii = ii + mb;
                    }
        ...
        for (i = 0; i < p; i++) {
            ...
            for (j = q - 1; j >= p + 1; j--) {
                ...
                if ((mb == 1) && (nb == 1)) {
                    ...
                    if (j > p + 1) {
                        alpha = rhs[0];
                        cblas_daxpy(js, alpha, &B[0 + js * ldb], 1, &F[is + 0 * ldf], ldf);
                        alpha = rhs[1];
                        cblas_daxpy(js, alpha, &E[0 + js * lde], 1, &F[is + 0 * ldf], ldf);
                    }
                    if (i < p - 1) {
                        alpha = -rhs[0];
                        cblas_daxpy(m - ie - 1, alpha, &A[is + (ie + 1) * lda], lda,
                                    &C[(ie + 1) + js * ldc], 1);
                        alpha = -rhs[1];
                        cblas_daxpy(m - ie - 1, alpha, &D[is + (ie + 1) * ldd], ldd,
                                    &C[(ie + 1) + js * ldc], 1);
                    }
                    ...
                    /* Substitute R(I, J) and L(I, J) into remaining equation. */
                    if (j > p + 1) {
                        cblas_daxpy(js, rhs[0], &B[0 + js * ldb], 1, &F[is + 0 * ldf], ldf);
                        cblas_daxpy(js, rhs[1], &B[0 + jsp1 * ldb], 1, &F[is + 0 * ldf], ldf);
                        cblas_daxpy(js, rhs[2], &E[0 + js * lde], 1, &F[is + 0 * ldf], ldf);
                        cblas_daxpy(js, rhs[3], &E[0 + jsp1 * lde], 1, &F[is + 0 * ldf], ldf);
                    }
                    if (i < p - 1) {
                        cblas_dger(CblasColMajor, m - ie - 1, nb, -ONE, &A[is + (ie + 1) * lda], lda,
                                   &rhs[0], 1, &C[(ie + 1) + js * ldc], ldc);
                        cblas_dger(CblasColMajor, m - ie - 1, nb, -ONE, &D[is + (ie + 1) * ldd], ldd,
                    ...
                    /* Substitute R(I, J) and L(I, J) into remaining equation. */
                    if (j > p + 1) {
                        cblas_dger(CblasColMajor, mb, js, ONE, &rhs[0], 1,
                                   &B[0 + js * ldb], 1, &F[is + 0 * ldf], ldf);
                        cblas_dger(CblasColMajor, mb, js, ONE, &rhs[2], 1,
                                   &E[0 + js * lde], 1, &F[is + 0 * ldf], ldf);
                    }
                    if (i < p - 1) {
                        cblas_dgemv(CblasColMajor, CblasTrans, mb, m - ie - 1, -ONE,
