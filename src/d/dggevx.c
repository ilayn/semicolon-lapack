            ...
            int* info)
{
    ...
    *bbnrm = dlange("1", n, n, B, ldb, work);
    ...
    itau = 0;
    iwrk = itau + irows;
    dgeqrf(irows, icols, &B[(*ilo - 1) + (*ilo - 1) * ldb], ldb, &work[itau],
           &work[iwrk], lwork - iwrk, &ierr);

    dormqr("L", "T", irows, icols, irows, &B[(*ilo - 1) + (*ilo - 1) * ldb], ldb,
    ...
    if (ilvl) {
        ...
        if (irows > 1) {
            dlacpy("L", irows - 1, irows - 1, &B[*ilo + (*ilo - 1) * ldb], ldb,
                   &VL[*ilo + (*ilo - 1) * ldvl], ldvl);
        }
        dorgqr(irows, irows, irows, &VL[(*ilo - 1) + (*ilo - 1) * ldvl], ldvl,
    ...
    if (ilv || !wantsn) {
        ...
        dgghrd("N", "N", irows, 1, irows, &A[(*ilo - 1) + (*ilo - 1) * lda], lda,
