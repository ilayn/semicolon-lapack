/**
 * @file dtpqrt2.c
 * @brief DTPQRT2 computes a QR factorization of a real "triangular-pentagonal"
             ...
             f64* restrict T, const int ldt, int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        dlarfg(p + 1, &A[i + i * lda], &B[i * ldb], 1, &T[i]);

        if (i < n - 1) {
            for (j = 0; j < n - i - 1; j++) {
                T[j + (n - 1) * ldt] = A[i + (i + 1 + j) * lda];
            }
            cblas_dgemv(CblasColMajor, CblasTrans, p, n - i - 1, ONE,
            ...
            cblas_dger(CblasColMajor, p, n - i - 1, alpha, &B[i * ldb], 1,
    ...
    for (i = 1; i < n; i++) {
        ...
        if (i - p > 0) {
            cblas_dgemv(CblasColMajor, CblasTrans, l, i - p, alpha,
                        &B[mp + (np - 1) * ldb], ldb, &B[mp + i * ldb], 1, ZERO,
                        &T[(np - 1) + i * ldt], 1);
        }

        if (m - l > 0) {
            cblas_dgemv(CblasColMajor, CblasTrans, m - l, i, alpha,
