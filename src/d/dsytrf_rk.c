    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (k + 1 > nb) {
                ...
                dlasyf_rk(uplo, k + 1, nb, &kb, A, lda, E, ipiv, work, ldwork, &iinfo);

            } else {

                dsytf2_rk(uplo, k + 1, A, lda, E, ipiv, &iinfo);
                kb = k + 1;
            }
        ...
        while (k < n) {
            ...
            if (k <= n - 1 - nb) {
                ...
                dlasyf_rk(uplo, n - k, nb, &kb, &A[k + k * lda], lda, &E[k],
                          &ipiv[k], work, ldwork, &iinfo);

            } else {

                dsytf2_rk(uplo, n - k, &A[k + k * lda], lda, &E[k], &ipiv[k], &iinfo);
                kb = n - k;
            }
