/**
 * @file dsytrf_rook.c
 * @brief DSYTRF_ROOK computes the factorization of a real symmetric matrix using the bounded Bunch-Kaufman ("rook") diagonal pivoting method.
 */
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (k + 1 > nb) {
                ...
                dlasyf_rook(uplo, k + 1, nb, &kb, A, lda, ipiv, work, ldwork, &iinfo);

            } else {

                dsytf2_rook(uplo, k + 1, A, lda, ipiv, &iinfo);
                kb = k + 1;
            }
        ...
        while (k < n) {
            ...
            if (k <= n - 1 - nb) {
                ...
                dlasyf_rook(uplo, n - k, nb, &kb, &A[k + k * lda], lda,
                            &ipiv[k], work, ldwork, &iinfo);

            } else {

                dsytf2_rook(uplo, n - k, &A[k + k * lda], lda, &ipiv[k], &iinfo);
                kb = n - k;
            }
