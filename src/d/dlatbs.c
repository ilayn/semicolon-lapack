    ...
    if (normin_n) {
        ...
        if (upper) {
            ...
            for (j = 0; j < n; j++) {
                ...
                jlen = (kd < j) ? kd : j;
                if (jlen > 0) {
                    cnorm[j] = cblas_dasum(jlen, &AB[kd - jlen + j * ldab], 1);
                } else {
                    cnorm[j] = ZERO;
                }
            }
            ...
            for (j = 0; j < n; j++) {
                ...
                if (jlen > 0) {
                    cnorm[j] = cblas_dasum(jlen, &AB[1 + j * ldab], 1);
                } else {
                    cnorm[j] = ZERO;
                }
            }
            ...
            for (j = jfirst; jinc > 0 ? j <= jlast : j >= jlast; j += jinc) {
                ...
                if (upper) {
                    if (j > 0) {
                        ...
                        jlen = (kd < j) ? kd : j;
                        cblas_daxpy(jlen, -X[j] * tscal, &AB[kd - jlen + j * ldab], 1, &X[j - jlen], 1);
                    ...
                    if (j < n - 1) {
                        ...
                        jlen = (kd < n - 1 - j) ? kd : n - 1 - j;
                        if (jlen > 0) {
                            cblas_daxpy(jlen, -X[j] * tscal, &AB[1 + j * ldab], 1, &X[j + 1], 1);
                        }
            ...
            for (j = jfirst; jinc > 0 ? j <= jlast : j >= jlast; j += jinc) {
                ...
                if (uscal == ONE) {
                    if (upper) {
                        jlen = (kd < j) ? kd : j;
                        if (jlen > 0) {
                            sumj = cblas_ddot(jlen, &AB[kd - jlen + j * ldab], 1, &X[j - jlen], 1);
                        }
                    } else {
                        jlen = (kd < n - 1 - j) ? kd : n - 1 - j;
                        if (jlen > 0) {
                            sumj = cblas_ddot(jlen, &AB[1 + j * ldab], 1, &X[j + 1], 1);
                        }
                    }
                } else {
                    if (upper) {
