/**
 * @file dlarre.c
 * @brief DLARRE sets small off-diagonal elements to zero, finds base
    ...
    *pivmin = safmin * (ONE > emax * emax ? ONE : emax * emax);
    ...
    if ((irange == ALLRNG) && (!forceb)) {
        ...
        *vu = gu;
        ...
        dlarrd(range, "B", n, *vl, *vu, il, iu, gers,
    ...
    for (jblk = 0; jblk < *nsplit; jblk++) {
        ...
        if (((irange == ALLRNG) && (!forceb)) || usedqd) {
            ...
            dlarrk(in, 1, gl, gu, &D[ibegin],
                   &E2[ibegin], *pivmin, rtl, &tmp, &tmp1, &iinfo);
            if (iinfo != 0) {
                *info = -1;
                return;
            }
            isleft = gl > (tmp - tmp1 - HNDRD * eps * fabs(tmp - tmp1)) ?
                     gl : (tmp - tmp1 - HNDRD * eps * fabs(tmp - tmp1));

            dlarrk(in, in, gl, gu, &D[ibegin],
                   &E2[ibegin], *pivmin, rtl, &tmp, &tmp1, &iinfo);
            if (iinfo != 0) {
                *info = -1;
                return;
            }
            isrght = gu < (tmp + tmp1 + HNDRD * eps * fabs(tmp + tmp1)) ?
                     gu : (tmp + tmp1 + HNDRD * eps * fabs(tmp + tmp1));
            /* Improve the estimate of the spectral diameter */
            spdiam = isrght - isleft;
        } else {
            /* Case of bisection */
            /* Find approximations to the wanted extremal eigenvalues */
            tmp = W[wbegin] - werr[wbegin] - HNDRD * eps * fabs(W[wbegin] - werr[wbegin]);
            isleft = gl > tmp ? gl : tmp;
            tmp = W[wend] + werr[wend] + HNDRD * eps * fabs(W[wend] + werr[wend]);
        ...
        if (usedqd) {
            ...
            tau = tau > TWO * eps * fabs(sigma) ? tau : TWO * eps * fabs(sigma);
        } else {
            if (mb > 1) {
        ...
        if (!usedqd) {
            ...
            dlarrb(in, &D[ibegin], &work[ibegin],
