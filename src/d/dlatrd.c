/**
 * @file dlatrd.c
 * @brief DLATRD reduces NB rows and columns of a real symmetric matrix to
 *        tridiagonal form.
 */
...
void dlatrd(const char* uplo, const int n, const int nb,
            ...
            double* const restrict E, double* const restrict tau,
            double* const restrict W, const int ldw)
{
    ...
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        ...
        for (ii = n; ii >= n - nb + 1; ii--) {
            ...
            if (ii > 1) {
                ...
                cblas_dscal(ii - 1, tau[ii - 2], &W[(iw - 1) * ldw], 1);
                alpha = -HALF * tau[ii - 2] *
                        cblas_ddot(ii - 1,
                                   &W[(iw - 1) * ldw], 1,
                                   &A[(ii - 1) * lda], 1);
                cblas_daxpy(ii - 1, alpha,
                            &A[(ii - 1) * lda], 1,
                            &W[(iw - 1) * ldw], 1);
            }
        }
        ...
        for (ii = 1; ii <= nb; ii++) {
            ...
            if (ii < n) {
                ...
                cblas_dscal(n - ii, tau[ii - 1], &W[ii + (ii - 1) * ldw], 1);
                alpha = -HALF * tau[ii - 1] *
                        cblas_ddot(n - ii,
                                   &W[ii + (ii - 1) * ldw], 1,
                                   &A[ii + (ii - 1) * lda], 1);
                cblas_daxpy(n - ii, alpha,
                            &A[ii + (ii - 1) * lda], 1,
                            &W[ii + (ii - 1) * ldw], 1);
            }
        }
