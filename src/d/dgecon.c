 ...
 *                      as computed by dgetrf. Array of dimension (lda, n).
    ...
    int *info)
{
    ...
    *rcond = ZERO;
    ...
    while (1) {
        ...
        if (kase == kase1) {
            // Multiply by inv(L)
            dlatrs("L", "N", "U", &normin, n, A, lda, work, &sl, &work[2 * n], &linfo);

            // Multiply by inv(U)
            dlatrs("U", "N", "N", &normin, n, A, lda, work, &su, &work[3 * n], &linfo);
        } else {
            // Multiply by inv(U**T)
            dlatrs("U", "T", "N", &normin, n, A, lda, work, &su, &work[3 * n], &linfo);

            // Multiply by inv(L**T)
            dlatrs("L", "T", "U", &normin, n, A, lda, work, &sl, &work[2 * n], &linfo);
        }
