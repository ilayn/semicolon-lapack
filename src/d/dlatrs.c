    ...
    INT* info)
{
    ...
    *scale = ONE;
    ...
    if (normin_n) {
        ...
        if (upper) {
            for (j = 0; j < n; j++) {
                cnorm[j] = cblas_dasum(j, &A[j * lda], 1);
            }
        } else {
            for (j = 0; j < n - 1; j++) {
                cnorm[j] = cblas_dasum(n - j - 1, &A[j + 1 + j * lda], 1);
            }
    ...
    if ((grow * tscal) > smlnum) {
        ...
        if (notran) {
            ...
            for (j = jfirst; jinc > 0 ? j <= jlast : j >= jlast; j += jinc) {
                ...
                if (upper) {
                    if (j > 0) {
                        cblas_daxpy(j, -X[j] * tscal, &A[j * lda], 1, X, 1);
                        i = cblas_idamax(j, X, 1);
                        xmax = fabs(X[i]);
                    }
                } else {
                    if (j < n - 1) {
                        cblas_daxpy(n - j - 1, -X[j] * tscal, &A[j + 1 + j * lda], 1, &X[j + 1], 1);
            ...
            for (j = jfirst; jinc > 0 ? j <= jlast : j >= jlast; j += jinc) {
                ...
                if (uscal == ONE) {
                    if (upper) {
                        sumj = cblas_ddot(j, &A[j * lda], 1, X, 1);
                    } else if (j < n - 1) {
                        sumj = cblas_ddot(n - j - 1, &A[j + 1 + j * lda], 1, &X[j + 1], 1);
                    }
                } else {
                    if (upper) {
                        for (i = 0; i < j; i++) {
