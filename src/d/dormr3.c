/**
 * @file dormr3.c
 * @brief DORMR3 multiplies a general matrix by the orthogonal matrix from
 ...
 *                       the elementary reflector H(i), as returned by DTZRZF
    ...
    *info = 0;
    ...
    for (int i = i_start; i != i_end; i += i_step) {
        if (left) {
            ...
            mi = m - i;
            /* dlarz("L", mi, ni, l, &A[i + (m-l)*lda], lda, tau[i],
             *        &C[i + 0*ldc], ldc, work) */
            dlarz("L", mi, ni, l,
                  &A[i + (m - l) * lda], lda,
                  tau[i],
            ...
            ni = n - i;
            /* dlarz("R", mi, ni, l, &A[i + (n-l)*lda], lda, tau[i],
             *        &C[0 + i*ldc], ldc, work) */
            dlarz("R", mi, ni, l,
                  &A[i + (n - l) * lda], lda,
                  tau[i],
