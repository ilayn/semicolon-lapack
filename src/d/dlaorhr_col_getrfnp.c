    ...
    *info = 0;
    ...
    if (nb <= 1 || nb >= minmn) {
        ...
        for (j = 0; j < minmn; j += nb) {
            ...
            dlaorhr_col_getrfnp2(m - j, jb, &A[j + j * lda], lda, &D[j], &iinfo);

            if (j + jb < n) {

                cblas_dtrsm(CblasColMajor, CblasLeft, CblasLower, CblasNoTrans, CblasUnit,
                            jb, n - j - jb, 1.0, &A[j + j * lda], lda, &A[j + (j + jb) * lda], lda);

                if (j + jb < m) {

                    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                m - j - jb, n - j - jb, jb, -1.0, &A[(j + jb) + j * lda], lda,
                                &A[j + (j + jb) * lda], lda, 1.0, &A[(j + jb) + (j + jb) * lda], lda);
                }
            }
        }
    }
