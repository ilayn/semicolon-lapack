/**
 * @file dlansb.c
 * @brief DLANSB returns the value of the 1-norm, Frobenius norm, infinity norm, or max element of a symmetric band matrix.
 */
    ...
    if (n == 0) {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                ...
                for (i = istart; i < j; i++) {
                    absa = fabs(AB[l + i + j * ldab]);
                    sum = sum + absa;
                    work[i] = work[i] + absa;
                }
                work[j] = sum + fabs(AB[k + j * ldab]);
            }
            for (i = 0; i < n; i++) {
                sum = work[i];
                if (value < sum || disnan(sum)) value = sum;
        ...
        if (k > 0) {
            if (uplo[0] == 'U' || uplo[0] == 'u') {
                for (j = 1; j < n; j++) {
                    ...
                    dlassq(len, &AB[start + j * ldab], 1, &scale, &sum);
                }
                l = k;
            } else {
                for (j = 0; j < n - 1; j++) {
                    int len = (n - j - 1 < k) ? n - j - 1 : k;
                    dlassq(len, &AB[1 + j * ldab], 1, &scale, &sum);
                }
                l = 0;
            }
            sum = 2 * sum;
