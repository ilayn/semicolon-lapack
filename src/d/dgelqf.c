/**
 * @file dgelqf.c
 * @brief DGELQF computes an LQ factorization of a general M-by-N matrix
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k - nx; i += nb) {
            ...
            dgelq2(ib, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    ...
    if (i < k) {
        dgelq2(m - i, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    }
