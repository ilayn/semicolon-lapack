    ...
    const int ldwork)
{
    ...
    if (n > k) {
        ...
        for (j = 0; j < n - k; j++) {
            cblas_dcopy(k, &A[0 + (k + j) * lda], 1, &work[0 + j * ldwork], 1);
        }

        if (lnotident) {

            cblas_dtrmm(CblasColMajor, CblasLeft, CblasLower, CblasTrans, CblasUnit,
                        k, n - k, 1.0, A, lda, work, ldwork);
        }

        if (m > 0) {
            cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                        k, n - k, m, 1.0, B, ldb, &B[0 + k * ldb], ldb, 1.0, work, ldwork);
        }

        cblas_dtrmm(CblasColMajor, CblasLeft, CblasUpper, CblasNoTrans, CblasNonUnit,
                    k, n - k, 1.0, T, ldt, work, ldwork);

        if (m > 0) {
            cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                        m, n - k, k, -1.0, B, ldb, work, ldwork, 1.0, &B[0 + k * ldb], ldb);
        }
    ...
    for (j = 0; j < k; j++) {
        cblas_dcopy(j + 1, &A[0 + j * lda], 1, &work[0 + j * ldwork], 1);
    }
    ...
    if (lnotident) {
        ...
        cblas_dtrmm(CblasColMajor, CblasLeft, CblasLower, CblasTrans, CblasUnit,
                    k, k, 1.0, A, lda, work, ldwork);
    }

    cblas_dtrmm(CblasColMajor, CblasLeft, CblasUpper, CblasNoTrans, CblasNonUnit,
                k, k, 1.0, T, ldt, work, ldwork);

    if (m > 0) {
        cblas_dtrmm(CblasColMajor, CblasRight, CblasUpper, CblasNoTrans, CblasNonUnit,
                    m, k, -1.0, work, ldwork, B, ldb);
    }
