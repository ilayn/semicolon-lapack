/**
 * @file dlaed8.c
 * @brief DLAED8 merges eigenvalues and deflates the secular equation.
    ...
    if ((*rho) * fabs(Z[imax]) <= tol) {
        *K = 0;
        if (icompq == 0) {
            ...
            for (j = 0; j < n; j++) {
                perm[j] = indxq[indx[j]];
                cblas_dcopy(qsiz, &Q[0 + perm[j] * ldq], 1,
                            &Q2[0 + j * ldq2], 1);
            }
    ...
    if ((*rho) * fabs(Z[j]) <= tol) {
        ...
        if (fabs(t * c * s) <= tol) {
            ...
            *givptr = *givptr + 1;
            ...
            if (icompq == 1) {
                cblas_drot(qsiz, &Q[0 + indxq[indx[jlam]] * ldq], 1,
...
L110:
    ...
    if (*K < n) {
        if (icompq == 0) {
            ...
            cblas_dcopy(n - *K, &dlambda[*K], 1, &D[*K], 1);
            dlacpy("A", qsiz, n - *K, &Q2[0 + (*K) * ldq2], ldq2,
                   &Q[0 + (*K) * ldq], ldq);
        }
    }
