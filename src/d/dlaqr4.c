/**
 * @file dlaqr4.c
 * @brief DLAQR4 computes eigenvalues of a Hessenberg matrix using small-bulge
...
/**
 * Compute IPARMQ-style tuning parameters for DLAQR4.
...
{
    /* For DLAQR4: use NS-based threshold */
    const int kacmin = 14;
    const int k22min = 14;
    int kacc = 0;
    if (ns >= kacmin) kacc = 1;
 ...
 * @param[out] info
 *                         - = 0: successful exit
 *                         - > 0: if info = i, DLAQR4 failed to compute all of the
 *                           eigenvalues. Elements ilo:info contain those
 *                           eigenvalues which have been successfully computed.
 */
SEMICOLON_API void dlaqr4(const int wantt, const int wantz, const int n,
                          ...
                          f64* work, const int lwork, int* info)
{
    ...
    /* Local array for dummy Z in DLAHQR call */
    f64 zdum[1];
    ...
    *info = 0;
    ...
    if (n <= ntiny) {
        ...
        *info = 0;
        ...
        dlaqr2(wantt, wantz, n, ilo, ihi, nwr + 1, H, ldh, iloz,
               ihiz, Z, ldz, &ls, &ld, wr, wi, H, ldh, n, H, ldh,
               n, H, ldh, work, -1);

        /* Optimal workspace = MAX(3*NSR/2, DLAQR2 workspace) */
        lwkopt = 3 * nsr / 2;
        ...
        for (it = 1; it <= itmax; it++) {
            ...
            if ((ld == 0) || ((100 * ld <= nw * nibble) &&
                 ...
                 * shifts provided by DLAQR2 above or from the eigenvalues
                ...
                if ((ndfl % kexsh) == 0) {
                    ...
                    /* Got NS/2 or fewer shifts? Use DLAHQR on a trailing
