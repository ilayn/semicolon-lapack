/**
 * @file dlaed6.c
 * @brief DLAED6 computes one Newton step in solution of the secular equation.
 */
 ...
 *
 * @param[in]     kniter Refer to DLAED4 for its significance.
            ...
            const double finit, double* tau, int* info)
{
    ...
    *tau = ZERO;
    ...
    eps = dlamch("E");
    base = dlamch("B");
    small1 = pow(base, (int)(log(dlamch("S")) / log(base) / THREE));
    sminv1 = ONE / small1;
    ...
    for (niter = niter + 1; niter <= MAXIT; niter++) {
        ...
        *tau = *tau + eta;
        ...
        f = finit + (*tau) * fc;
        erretm = EIGHT * (fabs(finit) + fabs(*tau) * erretm) +
                 fabs(*tau) * df;
        if ((fabs(f) <= FOUR * eps * erretm) ||
            ((ubd - lbd) <= FOUR * eps * fabs(*tau))) {
            goto done;
        }
        if (f <= ZERO) {
            lbd = *tau;
