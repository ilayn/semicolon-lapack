/**
 * @file dlaed0.c
 * @brief DLAED0 computes all eigenvalues and corresponding eigenvectors of an
    ...
    *info = 0;
        ...
        if (icompq == 2) {
            ...
            if (icompq == 1) {
                cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            qsiz, matsiz, matsiz, 1.0,
                            &Q[submat * ldq], ldq,
                            &work[iq + iwork[iqptr + curr]], matsiz,
                            0.0, &qstore[submat * ldqs], ldqs);
            }
    ...
    while (subpbs > 1) {
        ...
        for (i = 0; i <= spm2; i += 2) {
             ...
             */
            if (icompq == 2) {
                dlaed1(matsiz, &D[submat], &Q[submat + submat * ldq],
                       ldq, &iwork[indxq + submat],
                       E[submat + msd2 - 1], msd2, work,
                       &iwork[subpbs], info);
            } else {
                dlaed7(icompq, matsiz, qsiz, tlvls, curlvl,
                       curprb,
    ...
    if (icompq == 1) {
        for (i = 0; i < n; i++) {
            ...
            work[i] = D[j];
            cblas_dcopy(qsiz, &qstore[j * ldqs], 1, &Q[i * ldq], 1);
        }
        ...
        for (i = 0; i < n; i++) {
            j = iwork[indxq + i];  /* Already 0-based */
            work[i] = D[j];
            cblas_dcopy(n, &Q[j * ldq], 1, &work[n + (int64_t)n * i], 1);
        }
        cblas_dcopy(n, work, 1, D, 1);
