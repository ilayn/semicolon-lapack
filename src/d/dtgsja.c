/**
 * @file dtgsja.c
 * @brief DTGSJA computes the GSVD of two upper triangular matrices.
 */
 ...
 * @param[in]     k       Subblock dimension from DGGSVP3.
 * @param[in]     l       Subblock dimension from DGGSVP3.
    ...
    for (kcycle = 0; kcycle < MAXIT; kcycle++) {
        ...
        for (i = 0; i < l - 1; i++) {
            for (j = i + 1; j < l; j++) {
                ...
                if (k + j < m) {
                    cblas_drot(l, &A[(k + j) + (n - l) * lda], lda,
                               &A[(k + i) + (n - l) * lda], lda, csu, snu);
                }

                cblas_drot(l, &B[j + (n - l) * ldb], ldb,
                           &B[i + (n - l) * ldb], ldb, csv, snv);

                minval = k + l;
                if (m < minval) minval = m;
                cblas_drot(minval, &A[(n - l + j) * lda], 1,
                           &A[(n - l + i) * lda], 1, csq, snq);

                cblas_drot(l, &B[(n - l + j) * ldb], 1,
                ...
                if (wantu && k + j < m) {
                    cblas_drot(m, &U[(k + j) * ldu], 1,
                               &U[(k + i) * ldu], 1, csu, snu);
                }

                if (wantv) {
                    cblas_drot(p, &V[j * ldv], 1,
                               &V[i * ldv], 1, csv, snv);
                }

                if (wantq) {
                    cblas_drot(n, &Q[(n - l + j) * ldq], 1,
        ...
        if (!upper) {
            ...
            for (i = 0; i < minval; i++) {
                cblas_dcopy(l - i, &A[(k + i) + (n - l + i) * lda], lda, work, 1);
                cblas_dcopy(l - i, &B[i + (n - l + i) * ldb], ldb, &work[l], 1);
                dlapll(l - i, work, 1, &work[l], 1, &ssmin);
...
converged:
    ...
    for (i = 0; i < minval; i++) {
        ...
        if (gamma <= HUGENUM && gamma >= -HUGENUM) {
            if (gamma < ZERO) {
                cblas_dscal(l - i, -ONE, &B[i + (n - l + i) * ldb], ldb);
                if (wantv) {
                    cblas_dscal(p, -ONE, &V[i * ldv], 1);
                }
            }
            ...
            if (alpha[k + i] >= beta[k + i]) {
                cblas_dscal(l - i, ONE / alpha[k + i],
                            &A[(k + i) + (n - l + i) * lda], lda);
            } else {
                cblas_dscal(l - i, ONE / beta[k + i],
                            &B[i + (n - l + i) * ldb], ldb);
                cblas_dcopy(l - i, &B[i + (n - l + i) * ldb], ldb,
                            &A[(k + i) + (n - l + i) * lda], lda);
            }
        } else {
            alpha[k + i] = ZERO;
