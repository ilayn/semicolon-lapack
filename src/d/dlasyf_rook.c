/**
 * @file dlasyf_rook.c
 * @brief DLASYF_ROOK computes a partial factorization of a real symmetric matrix using the bounded Bunch-Kaufman ("rook") diagonal pivoting method.
 */
    ...
    *info = 0;
    ...
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        ...
        while (1) {
            ...
            if (k < n - 1) {
                cblas_dgemv(CblasColMajor, CblasNoTrans, k + 1, n - k - 1, -1.0,
                            &A[0 + (k + 1) * lda], lda, &W[k + (kw + 1) * ldw], ldw,
                            1.0, &W[0 + kw * ldw], 1);
            }

            absakk = fabs(W[k + kw * ldw]);
            ...
            if (fmax(absakk, colmax) == 0.0) {
                ...
                if (!(absakk < alpha * colmax)) {
                    ...
                    while (!done) {
                        ...
                        cblas_dcopy(imax + 1, &A[0 + imax * lda], 1, &W[0 + (kw - 1) * ldw], 1);
                        ...
                        if (imax > 0) {
                            itemp = cblas_idamax(imax, &W[0 + (kw - 1) * ldw], 1);
                            dtemp = fabs(W[itemp + (kw - 1) * ldw]);
                            if (dtemp > rowmax) {
                                rowmax = dtemp;
                                jmax = itemp;
                            }
                        ...
                        if (!(fabs(W[imax + (kw - 1) * ldw]) < alpha * rowmax)) {

                            kp = imax;

                            cblas_dcopy(k + 1, &W[0 + (kw - 1) * ldw], 1, &W[0 + kw * ldw], 1);
                ...
                if ((kstep == 2) && (p != k)) {
                    ...
                    cblas_dcopy(k - p, &A[p + 1 + k * lda], 1, &A[p + (p + 1) * lda], lda);
                    cblas_dcopy(p + 1, &A[0 + k * lda], 1, &A[0 + p * lda], 1);

                    cblas_dswap(n - k, &A[k + k * lda], lda, &A[p + k * lda], lda);
                    cblas_dswap(n - kk, &W[k + kkw * ldw], ldw, &W[p + kkw * ldw], ldw);
                }

                if (kp != kk) {

                    A[kp + k * lda] = A[kk + k * lda];
                    cblas_dcopy(k - 1 - kp, &A[kp + 1 + kk * lda], 1, &A[kp + (kp + 1) * lda], lda);
                    cblas_dcopy(kp + 1, &A[0 + kk * lda], 1, &A[0 + kp * lda], 1);

                    cblas_dswap(n - kk, &A[kk + kk * lda], lda, &A[kp + kk * lda], lda);
                    cblas_dswap(n - kk, &W[kk + kkw * ldw], ldw, &W[kp + kkw * ldw], ldw);
                }
        ...
        while (j < n) {
            ...
            j = j + 1;
            if (jp2 != jj && j < n) {
                cblas_dswap(n - j, &A[jp2 + j * lda], lda, &A[jj + j * lda], lda);
            }
            jj = j - 1;
            if (jp1 != jj && kstep == 2 && j < n) {
                cblas_dswap(n - j, &A[jp1 + j * lda], lda, &A[jj + j * lda], lda);
            }
        }
        ...
        *kb = n - 1 - k;
        ...
        while (1) {
            ...
            if (k > 0) {
                cblas_dgemv(CblasColMajor, CblasNoTrans, n - k, k, -1.0,
                            &A[k + 0 * lda], lda, &W[k + 0 * ldw], ldw,
                            1.0, &W[k + k * ldw], 1);
            }

            absakk = fabs(W[k + k * ldw]);
            ...
            if (fmax(absakk, colmax) == 0.0) {
                ...
                if (!(absakk < alpha * colmax)) {
                    ...
                    while (!done) {
                        ...
                        cblas_dcopy(imax - k, &A[imax + k * lda], lda, &W[k + (k + 1) * ldw], 1);
                        cblas_dcopy(n - imax, &A[imax + imax * lda], 1, &W[imax + (k + 1) * ldw], 1);
                        if (k > 0) {
                            cblas_dgemv(CblasColMajor, CblasNoTrans, n - k, k, -1.0,
                        ...
                        if (imax != k) {
                            jmax = k + cblas_idamax(imax - k, &W[k + (k + 1) * ldw], 1);
                            rowmax = fabs(W[jmax + (k + 1) * ldw]);
                        } else {
                            rowmax = 0.0;
                        }

                        if (imax < n - 1) {
                            itemp = imax + 1 + cblas_idamax(n - imax - 1, &W[imax + 1 + (k + 1) * ldw], 1);
                            dtemp = fabs(W[itemp + (k + 1) * ldw]);
                            if (dtemp > rowmax) {
                                rowmax = dtemp;
                                jmax = itemp;
                            }
                        ...
                        if (!(fabs(W[imax + (k + 1) * ldw]) < alpha * rowmax)) {

                            kp = imax;

                            cblas_dcopy(n - k, &W[k + (k + 1) * ldw], 1, &W[k + k * ldw], 1);
                ...
                if ((kstep == 2) && (p != k)) {
                    ...
                    cblas_dcopy(p - k, &A[k + k * lda], 1, &A[p + k * lda], lda);
                    cblas_dcopy(n - p, &A[p + k * lda], 1, &A[p + p * lda], 1);

                    cblas_dswap(k + 1, &A[k + 0 * lda], lda, &A[p + 0 * lda], lda);
                    cblas_dswap(kk + 1, &W[k + 0 * ldw], ldw, &W[p + 0 * ldw], ldw);
                }

                if (kp != kk) {

                    A[kp + k * lda] = A[kk + k * lda];
                    cblas_dcopy(kp - k - 1, &A[k + 1 + kk * lda], 1, &A[kp + (k + 1) * lda], lda);
                    cblas_dcopy(n - kp, &A[kp + kk * lda], 1, &A[kp + kp * lda], 1);

                    cblas_dswap(kk + 1, &A[kk + 0 * lda], lda, &A[kp + 0 * lda], lda);
                    cblas_dswap(kk + 1, &W[kk + 0 * ldw], ldw, &W[kp + 0 * ldw], ldw);
                }
        ...
        while (j >= 0) {
            ...
            j = j - 1;
            if (jp2 != jj && j >= 0) {
                cblas_dswap(j + 1, &A[jp2 + 0 * lda], lda, &A[jj + 0 * lda], lda);
            }
            jj = j + 1;
            if (jp1 != jj && kstep == 2 && j >= 0) {
                cblas_dswap(j + 1, &A[jp1 + 0 * lda], lda, &A[jj + 0 * lda], lda);
            }
        }
