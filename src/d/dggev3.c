/**
 * @file dggev3.c
 * @brief DGGEV3 computes the eigenvalues and, optionally, the left and/or
            ...
            double* const restrict work, const int lwork, int* info)
{
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (ilvl) {
            dorgqr(n, n, n, VL, ldvl, NULL, work, -1, &ierr);
            lwkopt = lwkopt > (3 * n + (int)work[0]) ? lwkopt : (3 * n + (int)work[0]);
        }
        if (ilv) {
            dgghd3(jobvl, jobvr, n, 0, n - 1, A, lda, B, ldb, VL, ldvl,
                   VR, ldvr, work, -1, &ierr);
            lwkopt = lwkopt > (3 * n + (int)work[0]) ? lwkopt : (3 * n + (int)work[0]);
            dlaqz0("S", jobvl, jobvr, n, 0, n - 1, A, lda, B, ldb,
                   alphar, alphai, beta, VL, ldvl, VR, ldvr,
                   work, -1, 0, &ierr);
            lwkopt = lwkopt > (2 * n + (int)work[0]) ? lwkopt : (2 * n + (int)work[0]);
        } else {
            dgghd3("N", "N", n, 0, n - 1, A, lda, B, ldb, VL, ldvl,
                   VR, ldvr, work, -1, &ierr);
            lwkopt = lwkopt > (3 * n + (int)work[0]) ? lwkopt : (3 * n + (int)work[0]);
            dlaqz0("E", jobvl, jobvr, n, 0, n - 1, A, lda, B, ldb,
                   alphar, alphai, beta, VL, ldvl, VR, ldvr,
                   work, -1, 0, &ierr);
            lwkopt = lwkopt > (2 * n + (int)work[0]) ? lwkopt : (2 * n + (int)work[0]);
        }
    ...
    if (ilv) {
        ...
        dgghd3(jobvl, jobvr, n, ilo, ihi, A, lda, B, ldb, VL,
               ldvl, VR, ldvr, &work[iwrk], lwork - iwrk, &ierr);
    } else {
        dgghd3("N", "N", irows, 0, irows - 1, &A[ilo + ilo * lda], lda,
    ...
    dlaqz0(chtemp, jobvl, jobvr, n, ilo, ihi, A, lda, B, ldb,
           alphar, alphai, beta, VL, ldvl, VR, ldvr,
