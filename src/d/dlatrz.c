...
/**
 * @file dlatrz.c
 * @brief DLATRZ factors an upper trapezoidal matrix by means of orthogonal
 *        transformations.
 */
            ...
            double * const restrict work)
{
    ...
    for (i = m - 1; i >= 0; i--) {
         ...
         * [ A(i,i) A(i,n-l:n-1) ]. */
        dlarfg(l + 1, &A[i + i * lda], &A[i + (n - l) * lda], lda, &tau[i]);

        /* Apply H(i) to A(0:i-1, i:n-1) from the right. */
        if (i > 0) {
            dlarz("R", i, n - i, l, &A[i + (n - l) * lda], lda, tau[i],
                  &A[i * lda], lda, work);
        }
    }
}
