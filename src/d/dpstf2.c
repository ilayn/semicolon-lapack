/**
 * @file dpstf2.c
 * @brief DPSTF2 computes the Cholesky factorization with complete pivoting
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (INT j = 0; j < n && jstop < 0; j++) {
            ...
            if (j < n - 1) {
                if (j > 0) {
                    cblas_dgemv(CblasColMajor, CblasTrans, j, n - j - 1,
                                -ONE, &A[(j + 1) * lda], lda,
                                &A[j * lda], 1,
                                ONE, &A[j + (j + 1) * lda], lda);
                }
        ...
        for (INT j = 0; j < n && jstop < 0; j++) {
            ...
            if (j < n - 1) {
                if (j > 0) {
                    cblas_dgemv(CblasColMajor, CblasNoTrans, n - j - 1, j,
                                -ONE, &A[j + 1], lda,
                                &A[j], lda,
                                ONE, &A[(j + 1) + j * lda], 1);
                }
