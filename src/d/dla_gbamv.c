/**
 * @file dla_gbamv.c
 * @brief DLA_GBAMV computes a matrix-vector product using a general banded
 ...
 * @param[in]     X      Double precision array, dimension at least
...
void dla_gbamv(const INT trans, const INT m, const INT n,
               ...
               const f64 alpha, const f64* restrict AB,
               const INT ldab, const f64* restrict X,
        ...
        if (trans == CblasNoTrans) {
            for (i = 0; i < leny; i++) {
                ...
                if (alpha != 0.0) {
                    ...
                    for (j = jmin; j <= jmax; j++) {
                        temp = fabs(AB[kd + i - j + j * ldab]);
                        symb_zero = symb_zero &&
                            (X[j] == 0.0 || temp == 0.0);

                        Y[iy] = Y[iy] + alpha * fabs(X[j]) * temp;
                    }
                }
            ...
            for (i = 0; i < leny; i++) {
                ...
                if (alpha != 0.0) {
                    ...
                    for (j = jmin; j <= jmax; j++) {
                        ...
                        Y[iy] = Y[iy] + alpha * fabs(X[jx]) * temp;
                        jx = jx + incx;
                    }
                }
