...
/**
 * @file dgtcon.c
 * @brief DGTCON estimates the reciprocal of the condition number of a
    ...
    *rcond = ZERO;
    ...
    for (;;) {
        ...
        if (kase == kase1) {
            ...
            dgttrs("N", n, 1, DL, D, DU, DU2, ipiv, work, ldb, info);
        } else {
            /* Multiply by inv(L**T)*inv(U**T) */
            dgttrs("T", n, 1, DL, D, DU, DU2, ipiv, work, ldb, info);
        }
    }
