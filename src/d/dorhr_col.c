    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    for (jb = 0; jb < n; jb += nb) {
        ...
        jbtemp1 = jb;
        for (j = jb; j < jb + jnb; j++) {
            cblas_dcopy(j - jbtemp1 + 1, &A[jb + j * lda], 1, &T[0 + j * ldt], 1);
        }

        for (j = jb; j < jb + jnb; j++) {
            if (D[j] == 1.0) {
                cblas_dscal(j - jbtemp1 + 1, -1.0, &T[0 + j * ldt], 1);
            }
        }
        ...
        cblas_dtrsm(CblasColMajor, CblasRight, CblasLower, CblasTrans, CblasUnit,
                    jnb, jnb, 1.0, &A[jb + jb * lda], lda, &T[0 + jb * ldt], ldt);
