/**
 * @file dlamswlq.c
 * @brief DLAMSWLQ overwrites the general real M-by-N matrix C with
 ...
 * elementary reflectors computed by short wide LQ factorization (DLASWLQ)
 *
 ...
 *                       blocked elementary reflector H(i), as returned by DLASWLQ.
              ...
              int* info)
{
    ...
    *info = 0;
    ...
    if (left && tran) {
        ...
        dgemlqt("L", "T", nb, n, k, mb, &A[0 + 0 * lda], lda, T, ldt,
        ...
        dgemlqt("L", "N", nb, n, k, mb, &A[0 + 0 * lda], lda, T, ldt,
                &C[0 + 0 * ldc], ldc, work, info);

        for (i = nb; i <= ii - nb + k; i += (nb - k)) {
            dtpmlqt("L", "N", nb - k, n, k, 0, mb,
                    &A[0 + i * lda], lda,
                    &T[0 + ctr * k * ldt], ldt,
                    &C[0 + 0 * ldc], ldc,
                    &C[i + 0 * ldc], ldc, work, info);
        ...
        dgemlqt("R", "N", m, nb, k, mb, &A[0 + 0 * lda], lda, T, ldt,
                &C[0 + 0 * ldc], ldc, work, info);
        ...
        dgemlqt("R", "T", m, nb, k, mb, &A[0 + 0 * lda], lda, T, ldt,
                &C[0 + 0 * ldc], ldc, work, info);

        for (i = nb; i <= ii - nb + k; i += (nb - k)) {
            dtpmlqt("R", "T", m, nb - k, k, 0, mb,
                    &A[0 + i * lda], lda,
                    &T[0 + ctr * k * ldt], ldt,
                    &C[0 + 0 * ldc], ldc,
                    &C[0 + i * ldc], ldc, work, info);
