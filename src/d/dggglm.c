/**
 * @file dggglm.c
 * @brief DGGGLM solves a general Gauss-Markov linear model (GLM) problem.
 */
 ...
 *
 * @param[in,out] B
 *          Double precision array, dimension (ldb, p).
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    if (n > m) {
        ...
        cblas_dcopy(n - m, &D[m], 1, &Y[m + p - n], 1);
    }
    ...
    cblas_dgemv(CblasColMajor, CblasNoTrans, m, n - m, -one,
                &B[0 + (m + p - n) * ldb], ldb, &Y[m + p - n], 1, one, D, 1);

    if (m > 0) {
        dtrtrs("U", "N", "N", m, 1, A, lda, D, m, info);
