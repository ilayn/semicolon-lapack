/**
 * @file dsytrs_3.c
 * @brief DSYTRS_3 solves a system of linear equations A*X = B with a real symmetric matrix using the factorization computed by DSYTRF_RK or DSYTRF_BK.
 */
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (i >= 0) {
            if (ipiv[i] >= 0) {
                cblas_dscal(nrhs, ONE / A[i + i * lda], &B[i + 0 * ldb], ldb);
            } else if (i > 0) {
                akm1k = E[i];
