/**
 * @file dsytrf_aa.c
 * @brief DSYTRF_AA computes the factorization of a real symmetric matrix using Aasen's algorithm.
 */
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        cblas_dcopy(n, &A[0 + 0 * lda], lda, &work[0], 1);
        ...
        while (j < n) {
            ...
            if (j < n) {
                ...
                if (j1 > 0 || jb > 1) {
                    ...
                    alpha = A[(j - 1) + j * lda];
                    A[(j - 1) + j * lda] = 1.0;
                    cblas_dcopy(n - j, &A[(j - 2) + j * lda], lda,
                                &work[(j - j1) + jb * n], 1);
                    cblas_dscal(n - j, alpha, &work[(j - j1) + jb * n], 1);
                    ...
                    for (j2 = j; j2 < n; j2 += nb) {
                        ...
                        j3 = j2;
                        for (mj = nj - 1; mj >= 1; mj--) {
                            cblas_dgemv(CblasColMajor, CblasNoTrans, mj, jb + 1,
                                        -1.0, &work[(j3 - j1) + (k1 + 1) * n], n,
                                        &A[(j1 - k2) + j3 * lda], 1,
                                        1.0, &A[j3 + j3 * lda], lda);
                            j3 = j3 + 1;
                        }
                ...
                cblas_dcopy(n - j, &A[j + j * lda], lda, &work[0], 1);
            }
        }

    } else {

        cblas_dcopy(n, &A[0 + 0 * lda], 1, &work[0], 1);
        ...
        while (j < n) {
            ...
            if (j < n) {
                ...
                cblas_dcopy(n - j, &A[j + j * lda], 1, &work[0], 1);
            }
        }
    }
