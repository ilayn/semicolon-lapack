/**
 * @file dlaed1.c
 * @brief DLAED1 computes the updated eigensystem of a diagonal matrix after
 ...
 */
void dlaed1(const int n, f64* D, f64* Q, const int ldq,
            int* indxq, const f64 rho, const int cutpnt,
            f64* work, int* iwork, int* info)
{
    ...
    *info = 0;
     ...
     */
    cblas_dcopy(cutpnt, &Q[(cutpnt - 1)], ldq, &work[iz], 1);
    ...
    cblas_dcopy(n - cutpnt, &Q[zpp1 + zpp1 * ldq], ldq, &work[iz + cutpnt], 1);

    /* Deflate eigenvalues. */
    n1 = cutpnt;
    rho_local = rho;
    dlaed2(&k, n, n1, D, Q, ldq, indxq, &rho_local, &work[iz],
    ...
    if (k != 0) {
        ...
        dlaed3(k, n, n1, D, Q, ldq, rho_local, &work[idlmda],
