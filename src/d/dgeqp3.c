/**
 * @file dgeqp3.c
 * @brief DGEQP3 computes a QR factorization with column pivoting of a
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (minmn == 0) {
            ...
            iws = 3 * n + 1;
            /* ILAENV(1, 'DGEQRF', ' ', M, N, -1, -1) -> NB for GEQRF */
    ...
    for (j = 0; j < n; j++) {
        if (jpvt[j] != 0) {
            if (j != nfxd) {
                cblas_dswap(m, &A[j * lda], 1, &A[nfxd * lda], 1);
                jpvt[j] = jpvt[nfxd];
    ...
    if (nfxd < minmn) {
        ...
        /* ILAENV(1, 'DGEQRF', ' ', SM, SN, -1, -1) -> NB for GEQRF */
        nb = lapack_get_nb("GEQRF");
        nbmin = 2;
        nx = 0;
        ...
        if ((nb > 1) && (nb < sminmn)) {
            ...
            /* ILAENV(3, 'DGEQRF', ' ', SM, SN, -1, -1) -> NX for GEQRF */
            ...
            if (nx < sminmn) {
                ...
                if (lwork < minws) {
                     ...
                     */
                    nb = (lwork - 2 * sn) / (sn + 1);
                    /* ILAENV(2, 'DGEQRF', ' ', SM, SN, -1, -1) -> NBMIN for GEQRF */
         ...
         *   work[2n..]      = used by dlaqps (auxv + F matrix)
         *
         ...
         */
        for (j = nfxd; j < n; j++) {
            work[j] = cblas_dnrm2(sm, &A[nfxd + j * lda], 1);
            work[n + j] = work[j];
        }
        ...
        if ((nb >= nbmin) && (nb < sminmn) && (nx < sminmn)) {
            ...
            while (j < topbmn) {
                 ...
                 *
                 * dlaqps arguments (translated from Fortran 1-based):
                 *   M      = m
         ...
         *
         * dlaqp2 arguments (translated from Fortran 1-based):
         *   M      = m
