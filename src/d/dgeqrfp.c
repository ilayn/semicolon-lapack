/**
 * @file dgeqrfp.c
 * @brief DGEQRFP computes a QR factorization of a general M-by-N matrix
 ...
 *            ( 0 )
 *
 * where Q is a M-by-M orthogonal matrix, R is an upper-triangular N-by-N
 ...
 *                       are nonnegative. The elements below the diagonal, with
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k - nx; i += nb) {
             ...
             * A(i:m-1, i:i+ib-1) */
            dgeqr2p(m - i, ib, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    ...
    if (i < k) {
        dgeqr2p(m - i, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    }
