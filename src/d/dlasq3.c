/** @file dlasq3.c
 * @brief DLASQ3 checks for deflation, computes shift and calls dqds.
 */
...
compute_shift:
        ...
        if (*dmin <= ZERO || *n0 < n0in) {
            if (CBIAS * Z[(4 * i0 + *pp - 3) - 1] < Z[(4 * (*n0) + *pp - 3) - 1]) {
                ...
                *dmin2 = fmin(*dmin2, Z[(4 * (*n0) + *pp - 1) - 1]);
                Z[(4 * (*n0) + *pp - 1) - 1] = fmin(fmin(Z[(4 * (*n0) + *pp - 1) - 1],
                                                         Z[(4 * i0 + *pp - 1) - 1]),
                                                    Z[(4 * i0 + *pp + 3) - 1]);
                Z[(4 * (*n0) - *pp) - 1] = fmin(fmin(Z[(4 * (*n0) - *pp) - 1],
                                                     Z[(4 * i0 - *pp) - 1]),
                                                Z[(4 * i0 - *pp + 4) - 1]);
                qmax_local = fmax(fmax(qmax_local, Z[(4 * i0 + *pp - 3) - 1]),
                                  Z[(4 * i0 + *pp + 1) - 1]);
                *dmin = -ZERO;
            }
        }
