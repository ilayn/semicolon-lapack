/**
 * @file dbdsqr.c
 * @brief DBDSQR computes the SVD of a real bidiagonal matrix using the
            ...
            double* const restrict work, int* info)
{
    ...
    *info = 0;
    ...
    if (lower) {
        ...
        if (nru > 0) {
            dlasr("R", "V", "F", nru, n, work, &work[nm1], U, ldu);
        }
        if (ncc > 0) {
            dlasr("L", "V", "F", n, ncc, work, &work[nm1], C, ldc);
        }
    }
...
L90:
    ...
    if (ll == m - 2) {
        ...
        if (ncvt > 0) {
            cblas_drot(ncvt, &VT[m - 2], ldvt, &VT[m - 1], ldvt,
                       cosr, sinr);
        }
        if (nru > 0) {
            cblas_drot(nru, &U[(m - 2) * ldu], 1, &U[(m - 1) * ldu], 1, cosl, sinl);
        }
        if (ncc > 0) {
            cblas_drot(ncc, &C[m - 2], ldc, &C[m - 1], ldc, cosl, sinl);
        }
        m = m - 2;
        goto L60;
    }
    ...
    if (shift == ZERO) {
        if (idir == 1) {
            ...
            if (ncvt > 0) {
                dlasr("L", "V", "F", m - ll, ncvt, work,
                      &work[nm1], &VT[ll], ldvt);
            }
            if (nru > 0) {
                dlasr("R", "V", "F", nru, m - ll,
                      &work[nm12], &work[nm13], &U[ll * ldu], ldu);
            }
            if (ncc > 0) {
                dlasr("L", "V", "F", m - ll, ncc,
            ...
            /* Update singular vectors */
            if (ncvt > 0) {
                dlasr("L", "V", "B", m - ll, ncvt,
                      &work[nm12], &work[nm13], &VT[ll], ldvt);
            }
            if (nru > 0) {
                dlasr("R", "V", "B", nru, m - ll, work,
                      &work[nm1], &U[ll * ldu], ldu);
            }
            if (ncc > 0) {
                dlasr("L", "V", "B", m - ll, ncc, work,
                      &work[nm1], &C[ll], ldc);
            }
...
L160:
    for (i = 0; i < n; i++) {
        ...
        if (D[i] < ZERO) {
            ...
            if (ncvt > 0) {
                cblas_dscal(ncvt, NEGONE, &VT[i], ldvt);
            }
        }
    }
    ...
    for (i = 0; i < n - 1; i++) {
        ...
        if (isub != n - 1 - i) {
            ...
            D[n - 1 - i] = smin;
            if (ncvt > 0) {
                cblas_dswap(ncvt, &VT[isub], ldvt, &VT[n - 1 - i], ldvt);
            }
            if (nru > 0) {
                cblas_dswap(nru, &U[isub * ldu], 1, &U[(n - 1 - i) * ldu], 1);
            }
            if (ncc > 0) {
                cblas_dswap(ncc, &C[isub], ldc, &C[n - 1 - i], ldc);
            }
        }
    }
    return;
