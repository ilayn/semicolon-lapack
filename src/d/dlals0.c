            ...
            f64* restrict work, INT* info)
{
    ...
    *info = 0;
    ...
    if (icompq == 0) {
        for (i = 0; i < givptr; i++) {
            cblas_drot(nrhs, &B[givcol[i + 1 * ldgcol]], ldb,
        ...
        for (i = 1; i < n; i++) {
            cblas_dcopy(nrhs, &B[perm[i]], ldb, &BX[i], ldbx);
        }
        ...
        if (k == 1) {
            ...
            for (j = 0; j < k; j++) {
                ...
                cblas_dgemv(CblasColMajor, CblasTrans, k, nrhs, 1.0, BX, ldbx,
                            work, 1, 0.0, &B[j], ldb);
                dlascl("G", 0, 0, temp, 1.0, 1, nrhs, &B[j], ldb, info);
            }
        }
        ...
        for (i = 1; i < n; i++) {
            cblas_dcopy(nrhs, &BX[i], ldbx, &B[perm[i]], ldb);
        }
