/**
 * @file dstevr.c
 * @brief DSTEVR computes selected eigenvalues and, optionally, eigenvectors
            ...
            int* iwork, const int liwork, int* info)
{
    ...
    double bignum, eps, rmax, rmin, safmin, sigma, smlnum,
           tmp1, tnrm, vll, vuu;
    ...
    *m = 0;
    ...
    if ((alleig || test) && ieee == 1) {
        ...
        if (!wantz) {
            ...
            cblas_dcopy(n, D, 1, &work[n], 1);
            if (abstol <= TWO * n * eps) {
                tryrac = 1;
            } else {
                tryrac = 0;
            }
            dstemr(jobz, "A", n, &work[n], work, vl, vu, il,
...

    {
        ...
        /* dstebz uses 1-based il/iu; convert from 0-based */
        dstebz(range, order, n, vll, vuu, il + 1, iu + 1, abstol, D, E,
               m, &nsplit, W, &iwork[indibl], &iwork[indisp], work,
               &iwork[indiwo], info);
    }

    if (wantz) {
        dstein(n, D, E, *m, W, &iwork[indibl],
               &iwork[indisp],
