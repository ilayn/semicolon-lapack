/**
 * @file dsyswapr.c
 * @brief DSYSWAPR applies an elementary permutation on the rows and columns of a symmetric matrix.
 */
    ...
    const int i2)
{
    ...
    if (upper) {
        ...
        if (i1 > 0) {
            cblas_dswap(i1, &A[0 + i1 * lda], 1, &A[0 + i2 * lda], 1);
        }
        ...
        if (i2 - i1 - 1 > 0) {
            cblas_dswap(i2 - i1 - 1, &A[i1 + (i1 + 1) * lda], lda, &A[i1 + 1 + i2 * lda], 1);
        }

        if (i2 < n - 1) {
            cblas_dswap(n - i2 - 1, &A[i1 + (i2 + 1) * lda], lda, &A[i2 + (i2 + 1) * lda], lda);
        }

    } else {

        if (i1 > 0) {
            cblas_dswap(i1, &A[i1 + 0 * lda], lda, &A[i2 + 0 * lda], lda);
        }
        ...
        if (i2 - i1 - 1 > 0) {
            cblas_dswap(i2 - i1 - 1, &A[i1 + 1 + i1 * lda], 1, &A[i2 + (i1 + 1) * lda], lda);
        }

        if (i2 < n - 1) {
            cblas_dswap(n - i2 - 1, &A[i2 + 1 + i1 * lda], 1, &A[i2 + 1 + i2 * lda], 1);
        }
    }
}
