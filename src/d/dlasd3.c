/**
 * @file dlasd3.c
 * @brief DLASD3 finds all square roots of the roots of the secular equation,
            ...
            double* const restrict Z, int* info)
{
    ...
    *info = 0;
    ...
    if (k == 1) {
        D[0] = fabs(Z[0]);
        cblas_dcopy(m, VT2, ldvt2, VT, ldvt);
        if (Z[0] > 0.0) {
            cblas_dcopy(n, U2, 1, U, 1);
        } else {
            for (i = 0; i < n; i++) {
                U[i] = -U2[i];
            }
    ...
    cblas_dcopy(k, Z, 1, Q, 1);

    rho = cblas_dnrm2(k, Z, 1);
    dlascl("G", 0, 0, rho, 1.0, k, 1, Z, k, info);
    rho = rho * rho;
    ...
    for (i = 0; i < k; i++) {
        ...
        Z[i] = copysign(sqrt(fabs(Z[i])), Q[i]);
    }
    ...
    if (k == 2) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    n, k, k, 1.0, U2, ldu2, Q, ldq, 0.0, U, ldu);
        goto L100;
    }
    if (CTOT[0] > 0) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    nl, k, CTOT[0], 1.0, &U2[1 * ldu2], ldu2,
                    &Q[1], ldq, 0.0, U, ldu);
        if (CTOT[2] > 0) {
            ktemp = 1 + CTOT[0] + CTOT[1];
            cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                        nl, k, CTOT[2], 1.0, &U2[ktemp * ldu2], ldu2,
                        &Q[ktemp], ldq, 1.0, U, ldu);
        }
    } else if (CTOT[2] > 0) {
        ktemp = 1 + CTOT[0] + CTOT[1];
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    nl, k, CTOT[2], 1.0, &U2[ktemp * ldu2], ldu2,
                    &Q[ktemp], ldq, 0.0, U, ldu);
    } else {
        dlacpy("F", nl, k, U2, ldu2, U, ldu);
    }
    cblas_dcopy(k, Q, ldq, &U[nl], ldu);
    ktemp = 1 + CTOT[0];
    ctemp = CTOT[1] + CTOT[2];
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                nr, k, ctemp, 1.0, &U2[nl + 1 + ktemp * ldu2], ldu2,
                &Q[ktemp], ldq, 0.0, &U[nl + 1], ldu);

L100:
    for (i = 0; i < k; i++) {
        temp = cblas_dnrm2(k, &VT[i * ldvt], 1);
    ...
    if (k == 2) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    k, m, k, 1.0, Q, ldq, VT2, ldvt2, 0.0, VT, ldvt);
        return;
    }
    ktemp = 1 + CTOT[0];
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                k, nl + 1, ktemp, 1.0, Q, ldq, VT2, ldvt2, 0.0, VT, ldvt);
    ktemp = 1 + CTOT[0] + CTOT[1];
    if (ktemp <= ldvt2) {
        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                    k, nl + 1, CTOT[2], 1.0, &Q[ktemp * ldq], ldq,
                    &VT2[ktemp], ldvt2, 1.0, VT, ldvt);
    }
    ...
    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                k, nrp1, ctemp, 1.0, &Q[ktemp * ldq], ldq,
                &VT2[ktemp + (nl + 1) * ldvt2], ldvt2, 0.0, &VT[(nl + 1) * ldvt], ldvt);
}
