/**
 * @file dsytf2.c
 * @brief DSYTF2 computes the factorization of a real symmetric indefinite
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (fmax(absakk, colmax) == 0.0 || isnan(absakk)) {
                ...
                if (absakk >= ALPHA_BK * colmax) {
                    ...
                    f64 rowmax = 0.0;
                    if (imax + 1 <= k) {
                        int jmax = imax + 1 + cblas_idamax(k - imax, &A[imax + (imax + 1) * lda], lda);
                        rowmax = fabs(A[imax + jmax * lda]);
                    }
                    if (imax > 0) {
                        int jmax = cblas_idamax(imax, &A[0 + imax * lda], 1);
                        rowmax = fmax(rowmax, fabs(A[jmax + imax * lda]));
                    }

                    if (absakk >= ALPHA_BK * colmax * (colmax / rowmax)) {
                        /* No interchange, use 1-by-1 pivot block */
                        kp = k;
                    } else if (fabs(A[imax + imax * lda]) >= ALPHA_BK * rowmax) {
                ...
                if (kp != kk) {
                    ...
                    if (kk - kp - 1 > 0) {
                        cblas_dswap(kk - kp - 1, &A[(kp + 1) + kk * lda], 1,
                                    &A[kp + (kp + 1) * lda], lda);
                    }
                    f64 t = A[kk + kk * lda];
                ...
                if (kstep == 1) {
                    ...
                    if (k > 0) {
                        ...
                        cblas_dsyr(CblasColMajor, CblasUpper, k, -r1,
                                   &A[0 + k * lda], 1, A, lda);
                    ...
                    if (k > 1) {
                        f64 d12 = A[(k - 1) + k * lda];
                        f64 d22 = A[(k - 1) + (k - 1) * lda] / d12;
                        f64 d11 = A[k + k * lda] / d12;
                        f64 t = 1.0 / (d11 * d22 - 1.0);
                        d12 = t / d12;

                        for (int j = k - 2; j >= 0; j--) {
                            f64 wkm1 = d12 * (d11 * A[j + (k - 1) * lda] - A[j + k * lda]);
                            f64 wk = d12 * (d22 * A[j + k * lda] - A[j + (k - 1) * lda]);
                            for (int i = j; i >= 0; i--) {
        ...
        while (k < n) {
            ...
            if (fmax(absakk, colmax) == 0.0 || isnan(absakk)) {
                ...
                if (absakk >= ALPHA_BK * colmax) {
                    ...
                    f64 rowmax = 0.0;
                    if (imax > k) {
                        int jmax = k + cblas_idamax(imax - k, &A[imax + k * lda], lda);
                        rowmax = fabs(A[imax + jmax * lda]);
                    }
                    if (imax < n - 1) {
                        int jmax = imax + 1 + cblas_idamax(n - imax - 1, &A[(imax + 1) + imax * lda], 1);
                        rowmax = fmax(rowmax, fabs(A[jmax + imax * lda]));
                    }
                ...
                if (kp != kk) {
                    ...
                    if (kp < n - 1) {
                        cblas_dswap(n - kp - 1, &A[(kp + 1) + kk * lda], 1,
                                    &A[(kp + 1) + kp * lda], 1);
                    }
                    if (kp - kk - 1 > 0) {
                        cblas_dswap(kp - kk - 1, &A[(kk + 1) + kk * lda], 1,
                                    &A[kp + (kk + 1) * lda], lda);
                    }
                    f64 t = A[kk + kk * lda];
                    A[kk + kk * lda] = A[kp + kp * lda];
                ...
                if (kstep == 1) {
                    ...
                    if (k < n - 1) {
                        f64 d11 = 1.0 / A[k + k * lda];
                        cblas_dsyr(CblasColMajor, CblasLower, n - k - 1, -d11,
                    ...
                    if (k < n - 2) {
                        f64 d21 = A[(k + 1) + k * lda];
                        f64 d11 = A[(k + 1) + (k + 1) * lda] / d21;
                        f64 d22 = A[k + k * lda] / d21;
                        f64 t = 1.0 / (d11 * d22 - 1.0);
                        d21 = t / d21;

                        for (int j = k + 2; j < n; j++) {
                            f64 wk = d21 * (d11 * A[j + k * lda] - A[j + (k + 1) * lda]);
                            f64 wkp1 = d21 * (d22 * A[j + (k + 1) * lda] - A[j + k * lda]);
                            for (int i = j; i < n; i++) {
                                A[i + j * lda] -= A[i + k * lda] * wk +
                                                  A[i + (k + 1) * lda] * wkp1;
                            }
