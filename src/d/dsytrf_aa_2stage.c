/**
 * @file dsytrf_aa_2stage.c
 * @brief DSYTRF_AA_2STAGE computes the factorization of a real symmetric matrix using Aasen's 2-stage algorithm.
 */
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j = 0; j < nt; j++) {
            ...
            for (i = 1; i <= j - 1; i++) {
                if (i == 1) {
                    ...
                    cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                nb, kb, jb,
                                1.0, &TB[td + nb + ((i - 1) * nb) * ldtb], ldtb - 1,
                                &A[(i - 2) * nb + j * nb * lda], lda,
                                0.0, &work[i * nb], n);
                }
            }

            dlacpy("Upper", kb, kb, &A[j * nb + j * nb * lda], lda,
                   &TB[td + (j * nb) * ldtb], ldtb - 1);
            if (j > 1) {
                cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                            kb, kb, (j - 1) * nb,
                            -1.0, &A[0 + j * nb * lda], lda,
                            &work[nb], n,
                            1.0, &TB[td + (j * nb) * ldtb], ldtb - 1);
                cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                            kb, nb, kb,
                            1.0, &A[(j - 1) * nb + j * nb * lda], lda,
            ...
            if (j < nt - 1) {
                if (j > 0) {
                    ...
                    if (j == 1) {
                        ...
                        cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    kb, kb, nb + kb,
                                    1.0, &TB[td + nb + ((j - 1) * nb) * ldtb], ldtb - 1,
                                    &A[(j - 2) * nb + j * nb * lda], lda,
                                    0.0, &work[j * nb], n);
                    }
                    ...
                    cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                                ...
                                -1.0, &work[nb], n,
                                &A[0 + (j + 1) * nb * lda], lda,
                                1.0, &A[j * nb + (j + 1) * nb * lda], lda);
                }

                for (k = 0; k < nb; k++) {
                    cblas_dcopy(n - (j + 1) * nb,
                ...
                if (j > 0) {
                    cblas_dtrsm(CblasColMajor, CblasRight, CblasUpper,
                                CblasNoTrans, CblasUnit, kb, nb, 1.0,
                                &A[(j - 1) * nb + j * nb * lda], lda,
                                &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                }
                ...
                for (k = 0; k < kb; k++) {
                    ...
                    if (i1 != i2) {
                        cblas_dswap(k, &A[(j + 1) * nb + i1 * lda], 1,
                                    &A[(j + 1) * nb + i2 * lda], 1);
                        if (i2 > i1 + 1) {
                            cblas_dswap(i2 - i1 - 1, &A[i1 + (i1 + 1) * lda], lda,
                                        &A[i1 + 1 + i2 * lda], 1);
                        }
                        if (i2 < n - 1) {
                            cblas_dswap(n - i2 - 1, &A[i1 + (i2 + 1) * lda], lda,
                                        &A[i2 + (i2 + 1) * lda], lda);
                        }
                        piv = A[i1 + i1 * lda];
        ...
        for (j = 0; j < nt; j++) {
            ...
            dlacpy("Lower", kb, kb, &A[j * nb + j * nb * lda], lda,
                   &TB[td + (j * nb) * ldtb], ldtb - 1);
            if (j > 1) {
                cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            kb, kb, (j - 1) * nb,
                            -1.0, &A[j * nb + 0 * lda], lda,
                            &work[nb], n,
                            1.0, &TB[td + (j * nb) * ldtb], ldtb - 1);
                cblas_dgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            kb, nb, kb,
            ...
            if (j < nt - 1) {
                ...
                if (j > 0) {
                    cblas_dtrsm(CblasColMajor, CblasRight, CblasLower,
                                CblasTrans, CblasUnit, kb, nb, 1.0,
                                &A[j * nb + (j - 1) * nb * lda], lda,
                                &TB[td + nb + (j * nb) * ldtb], ldtb - 1);
                }
                ...
                for (k = 0; k < kb; k++) {
                    ...
                    if (i1 != i2) {
                        cblas_dswap(k, &A[i1 + (j + 1) * nb * lda], lda,
                                    &A[i2 + (j + 1) * nb * lda], lda);
                        if (i2 > i1 + 1) {
                            cblas_dswap(i2 - i1 - 1, &A[i1 + 1 + i1 * lda], 1,
                                        &A[i2 + (i1 + 1) * lda], lda);
                        }
                        if (i2 < n - 1) {
                            cblas_dswap(n - i2 - 1, &A[i2 + 1 + i1 * lda], 1,
                                        &A[i2 + 1 + i2 * lda], 1);
                        }
                        piv = A[i1 + i1 * lda];
                        A[i1 + i1 * lda] = A[i2 + i2 * lda];
                        A[i2 + i2 * lda] = piv;
                        if (j > 0) {
                            cblas_dswap(j * nb, &A[i1 + 0 * lda], lda,
                                        &A[i2 + 0 * lda], lda);
                        }
                    }
                }
