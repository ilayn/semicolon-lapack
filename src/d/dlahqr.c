    ...
    *info = 0;
        ...
        for (its = 0; its <= itmax; its++) {
            ...
            for (k = i; k >= l + 1; k--) {
                if (fabs(H[k + (k - 1) * ldh]) <= smlnum)
                    break;
                tst = fabs(H[(k - 1) + (k - 1) * ldh]) + fabs(H[k + k * ldh]);
                if (tst == zero) {
                    if (k - 2 >= ilo)
                        tst = tst + fabs(H[(k - 1) + (k - 2) * ldh]);
                    if (k + 1 <= ihi)
                        tst = tst + fabs(H[(k + 1) + k * ldh]);
                }
                /* Conservative small subdiagonal deflation criterion
                 * (Ahues & Tisseur, LAWN 122, 1997) */
                if (fabs(H[k + (k - 1) * ldh]) <= ulp * tst) {
                    ab = fabs(H[k + (k - 1) * ldh]) > fabs(H[(k - 1) + k * ldh]) ?
                         fabs(H[k + (k - 1) * ldh]) : fabs(H[(k - 1) + k * ldh]);
                    ba = fabs(H[k + (k - 1) * ldh]) < fabs(H[(k - 1) + k * ldh]) ?
                         fabs(H[k + (k - 1) * ldh]) : fabs(H[(k - 1) + k * ldh]);
                    aa = fabs(H[k + k * ldh]) > fabs(H[(k - 1) + (k - 1) * ldh] - H[k + k * ldh]) ?
                         fabs(H[k + k * ldh]) : fabs(H[(k - 1) + (k - 1) * ldh] - H[k + k * ldh]);
                    bb = fabs(H[k + k * ldh]) < fabs(H[(k - 1) + (k - 1) * ldh] - H[k + k * ldh]) ?
                         fabs(H[k + k * ldh]) : fabs(H[(k - 1) + (k - 1) * ldh] - H[k + k * ldh]);
                    s = aa + ab;
            ...
            for (k = m; k <= i - 1; k++) {
                ...
                nr = 3 < (i - k + 1) ? 3 : (i - k + 1);
                if (k > m)
                    cblas_dcopy(nr, &H[k + (k - 1) * ldh], 1, v, 1);
    ...
    converged:
        if (l == i) {
            ...
            dlanv2(&H[(i - 1) + (i - 1) * ldh], &H[(i - 1) + i * ldh],
            ...
            if (wantt) {
                ...
                if (i2 > i)
                    cblas_drot(i2 - i, &H[(i - 1) + (i + 1) * ldh], ldh,
                               &H[i + (i + 1) * ldh], ldh, cs, sn);
                cblas_drot(i - i1 - 1, &H[i1 + (i - 1) * ldh], 1,
                           &H[i1 + i * ldh], 1, cs, sn);
            }
            if (wantz) {
                /* Apply the transformation to Z */
                cblas_drot(nz, &Z[iloz + (i - 1) * ldz], 1,
