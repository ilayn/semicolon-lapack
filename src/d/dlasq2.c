/**
 * @file dlasq2.c
 * @brief DLASQ2 computes all eigenvalues of a symmetric positive definite tridiagonal matrix.
 */
...
void dlasq2(const INT n, f64* restrict Z, INT* info)
{
    ...
    *info = 0;
    ...
    for (k = 1; k <= 2 * (n - 1); k += 2) {
        ...
        d = d + Z[k - 1];
        e = e + Z[k];
        qmax = fmax(qmax, Z[k - 1]);
        emin = fmin(emin, Z[k]);
        zmax = fmax(fmax(qmax, zmax), Z[k]);
    }
    if (Z[2 * n - 2] < ZERO) {
        *info = -(200 + 2 * n - 1);
        xerbla("DLASQ2", 2);
    ...
    for (iwhila = 1; iwhila <= n + 1; iwhila++) {
        ...
        for (i4 = 4 * (n0 + 1); i4 >= 8; i4 -= 4) {
            ...
            if (qmin >= FOUR * emax) {
                qmin = fmin(qmin, Z[i4 - 4]);
                emax = fmax(emax, Z[i4 - 6]);
            }
            qmax = fmax(qmax, Z[i4 - 8] + Z[i4 - 6]);
            emin = fmin(emin, Z[i4 - 6]);
        }
        i4 = 4;
...
L100:
        ...
        dmin = -fmax(ZERO, qmin - TWO * sqrt(qmin) * sqrt(emax));

        /*
         * Now I0:N0 is unreduced.
         * PP = 0 for ping, PP = 1 for pong.
         * PP = 2 indicates that flipping was applied to the Z array and
         *        that the tests for deflation upon entry in DLASQ3
