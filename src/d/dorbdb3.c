/**
 * @file dorbdb3.c
 * @brief DORBDB3 simultaneously bidiagonalizes the blocks of a tall and skinny matrix with orthonormal columns.
 */
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < m - p; i++) {
        ...
        if (i < m - p - 1) {
            dlarfgp(m - p - i - 1, &X21[(i + 1) + i * ldx21], &X21[(i + 2) + i * ldx21], 1,
                    &taup2[i]);
            phi[i] = atan2(X21[(i + 1) + i * ldx21], X11[i + i * ldx11]);
            c = cos(phi[i]);
            s = sin(phi[i]);
            dlarf1f("L", m - p - i - 1, q - i - 1, &X21[(i + 1) + i * ldx21], 1, taup2[i],
