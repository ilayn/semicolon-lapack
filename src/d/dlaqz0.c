/**
 * @file dlaqz0.c
 * @brief DLAQZ0 computes the eigenvalues of a matrix pair (H,T) using multishift QZ.
 */
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    nmin = iparmq(12, "DLAQZ0", jbcmpz, n, ilo + 1, ihi + 1, lwork);
    nwr = iparmq(13, "DLAQZ0", jbcmpz, n, ilo + 1, ihi + 1, lwork);
    nwr = (2 > nwr) ? 2 : nwr;
    {
        INT tmp = ihi - ilo + 1;
        INT tmp2 = (n - 1) / 3;
    ...
    nibble = iparmq(14, "DLAQZ0", jbcmpz, n, ilo + 1, ihi + 1, lwork);

    nsr = iparmq(15, "DLAQZ0", jbcmpz, n, ilo + 1, ihi + 1, lwork);
    {
        INT tmp = (n + 6) / 9;
        INT tmp2 = ihi - ilo;
        nsr = (nsr < tmp) ? nsr : tmp;
    ...
    if (n < nmin || rec >= 2) {
        dhgeqz(wants, wantq, wantz, n, ilo, ihi, A, lda, B, ldb,
    ...
    /* Initialize Q and Z */
    if (iwantq == 3) dlaset("F", n, n, ZERO, ONE, Q, ldq);
    if (iwantz == 3) dlaset("F", n, n, ZERO, ONE, Z, ldz);
    ...
    for (iiter = 0; iiter < maxit; iiter++) {
        ...
        /* Check deflations at the end */
        {
            ...
            if (fabs(A[(istop - 1) + (istop - 2) * lda]) <= thresh) {
                ...
                tmp = fabs(A[istop + istop * lda]) + fabs(A[(istop - 1) + (istop - 1) * lda]);
                thresh = (smlnum > ulp * tmp) ? smlnum : ulp * tmp;
                if (fabs(A[istop + (istop - 1) * lda]) <= thresh) {
        ...
        /* Check deflations at the start */
        {
            ...
            if (fabs(A[(istart + 2) + (istart + 1) * lda]) <= thresh) {
                ...
                tmp = fabs(A[istart + istart * lda]) + fabs(A[(istart + 1) + (istart + 1) * lda]);
                thresh = (smlnum > ulp * tmp) ? smlnum : ulp * tmp;
                if (fabs(A[(istart + 1) + istart * lda]) <= thresh) {
        ...
        k = istop;
        while (k >= istart2) {
            if (fabs(B[k + k * ldb]) < btol) {
                /* A diagonal element of B is negligible, move it to the top and deflate it */
                for (k2 = k; k2 >= istart2 + 1; k2--) {
                    dlartg(B[(k2 - 1) + k2 * ldb], B[(k2 - 1) + (k2 - 1) * ldb],
                           &c1, &s1, &temp);
                    B[(k2 - 1) + k2 * ldb] = temp;
                    B[(k2 - 1) + (k2 - 1) * ldb] = ZERO;

                    cblas_drot(k2 - 1 - istartm, &B[istartm + k2 * ldb], 1,
                               &B[istartm + (k2 - 1) * ldb], 1, c1, s1);
                    {
                        INT cnt = ((k2 + 1 < istop) ? k2 + 1 : istop) - istartm + 1;
                        cblas_drot(cnt, &A[istartm + k2 * lda], 1,
                                   &A[istartm + (k2 - 1) * lda], 1, c1, s1);
                    }
                    if (ilz) {
                        cblas_drot(n, &Z[0 + k2 * ldz], 1, &Z[0 + (k2 - 1) * ldz], 1, c1, s1);
                    }

                    if (k2 < istop) {
                        dlartg(A[k2 + (k2 - 1) * lda], A[(k2 + 1) + (k2 - 1) * lda],
                               &c1, &s1, &temp);
                        A[k2 + (k2 - 1) * lda] = temp;
                        A[(k2 + 1) + (k2 - 1) * lda] = ZERO;

                        cblas_drot(istopm - k2 + 1, &A[k2 + k2 * lda], lda,
                                   &A[(k2 + 1) + k2 * lda], lda, c1, s1);
                        cblas_drot(istopm - k2 + 1, &B[k2 + k2 * ldb], ldb,
                                   &B[(k2 + 1) + k2 * ldb], ldb, c1, s1);
                        if (ilq) {
                            cblas_drot(n, &Q[0 + k2 * ldq], 1, &Q[0 + (k2 + 1) * ldq], 1, c1, s1);
                        }
                    }
                }

                if (istart2 < istop) {
                    dlartg(A[istart2 + istart2 * lda], A[(istart2 + 1) + istart2 * lda],
                           &c1, &s1, &temp);
                    A[istart2 + istart2 * lda] = temp;
                    A[(istart2 + 1) + istart2 * lda] = ZERO;

                    cblas_drot(istopm - (istart2 + 1) + 1, &A[istart2 + (istart2 + 1) * lda], lda,
                               &A[(istart2 + 1) + (istart2 + 1) * lda], lda, c1, s1);
                    cblas_drot(istopm - (istart2 + 1) + 1, &B[istart2 + (istart2 + 1) * ldb], ldb,
                               &B[(istart2 + 1) + (istart2 + 1) * ldb], ldb, c1, s1);
                    if (ilq) {
                        cblas_drot(n, &Q[0 + istart2 * ldq], 1, &Q[0 + (istart2 + 1) * ldq], 1, c1, s1);
                    }
                }
        ...
        /* Time for AED */
        dlaqz3(ilschur, ilq, ilz, n, istart2, istop, nw, A, lda, B, ldb,
               Q, ldq, Z, ldz, &n_undeflated, &n_deflated, alphar, alphai, beta,
        ...
        /* Time for a QZ sweep */
        dlaqz4(ilschur, ilq, ilz, n, istart2, istop, ns, nblock,
