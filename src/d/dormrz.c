/**
 * @file dormrz.c
 * @brief DORMRZ multiplies a general matrix by the orthogonal matrix from
 ...
 *                       as returned by DTZRZF in the last k rows of its
 *                       array argument A.
 ...
 *                       elementary reflector H(i), as returned by DTZRZF.
    ...
    *info = 0;
    ...
    if (nb < nbmin || nb >= k) {
        ...
        for (i = i1; i3 > 0 ? i <= i2 : i >= i2; i += i3) {
            ...
            /* Apply H or H^T */
            dlarzb(side, &transt, "B", "R", mi, ni, ib, l,
                   &A[i + ja * lda], lda,
                   &work[iwt], ldt_val,
                   &C[ic + jc * ldc], ldc,
                   work, ldwork);
