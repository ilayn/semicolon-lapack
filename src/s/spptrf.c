    ...
    int* info)
{
    ...
    *info = 0;
    ...
    // spptrf.f lines 171-172: Quick return if possible
    if (n == 0) {
        return;
    }
    ...
    if (upper) {
        ...
        for (int j = 0; j < n; j++) {  // spptrf.f line 203: DO 20 J = 1, N
            ...
            // spptrf.f lines 218-223: Compute elements J+1:N of column J and update
            // the trailing submatrix.
            if (j < n - 1) {
                // spptrf.f line 219: CALL DSCAL( N-J, ONE / AJJ, AP( JJ+1 ), 1 )
                cblas_sscal(n - j - 1, ONE / ajj, &AP[jj + 1], 1);
                // spptrf.f lines 220-221: CALL DSPR( 'Lower', N-J, -ONE, AP( JJ+1 ), 1, AP( JJ+N-J+1 ) )
                ...
                cblas_sspr(CblasColMajor, CblasLower, n - j - 1, -ONE,
                           &AP[jj + 1], 1, &AP[jj + n - j]);
                // spptrf.f line 222: JJ = JJ + N - J + 1
