            ...
            float * const restrict T, const int ldt)
{
     ...
     * ILAENV(3, 'SLARFT', ...) has no special case in ilaenv.f,
    ...
    if (qr) {
        ...
        slarft(direct, storev, n, l, V, ldv, tau, T, ldt);

        /* Compute T_{2,2} recursively (remaining k-l reflectors) */
        slarft(direct, storev, n - l, k - l,
               &V[(l) + (l) * ldv], ldv,
               &tau[l], &T[(l) + (l) * ldt], ldt);
        ...
        /* T_{1,2} = V_{1,2} (copy) */
        slacpy("A", l, k - l, &V[0 + (l) * ldv], ldv, &T[0 + (l) * ldt], ldt);
        ...
        slarft(direct, storev, n - l, k - l, V, ldv, tau, T, ldt);

        /* Compute T_{2,2} recursively (last l reflectors) */
        slarft(direct, storev, n, l,
               &V[0 + (k - l) * ldv], ldv,
        ...
        /* T_{2,1} = V_{2,2} (copy) */
        slacpy("A", l, k - l, &V[(k - l) + (n - k) * ldv], ldv,
               &T[(k - l) + 0 * ldt], ldt);
