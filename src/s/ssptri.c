/**
 * @file ssptri.c
 * @brief SSPTRI computes the inverse of a real symmetric indefinite matrix in packed storage.
 */
    ...
    if (upper) {
        ...
        while (k < n) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k > 0) {
                    cblas_scopy(k, &AP[kc], 1, work, 1);
                    cblas_sspmv(CblasColMajor, CblasUpper, k, -ONE, AP, work, 1, ZERO, &AP[kc], 1);
                    AP[kc + k] = AP[kc + k] - cblas_sdot(k, work, 1, &AP[kc], 1);
                }
                kstep = 1;
            } else {
                /*
                ...
                if (k > 0) {
                    ...
                    cblas_sspmv(CblasColMajor, CblasUpper, k, -ONE, AP, work, 1, ZERO, &AP[kc], 1);
                    AP[kc + k] = AP[kc + k] - cblas_sdot(k, work, 1, &AP[kc], 1);
                    AP[kcnext + k] = AP[kcnext + k] - cblas_sdot(k, &AP[kc], 1, &AP[kcnext], 1);
                    cblas_scopy(k, &AP[kcnext], 1, work, 1);
                    cblas_sspmv(CblasColMajor, CblasUpper, k, -ONE, AP, work, 1, ZERO, &AP[kcnext], 1);
                    AP[kcnext + k + 1] = AP[kcnext + k + 1] - cblas_sdot(k, work, 1, &AP[kcnext], 1);
                }
                kstep = 2;
                kcnext = kcnext + k + 2;
            }
            ...
            if (kp != k) {
                 ...
                 */
                kpc = kp * (kp + 1) / 2;
                cblas_sswap(kp, &AP[kc], 1, &AP[kpc], 1);
                kx = kpc + kp;
                for (j = kp + 1; j < k; j++) {
                    kx = kx + j;
                    temp = AP[kc + j];
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    cblas_scopy(n - k - 1, &AP[kc + 1], 1, work, 1);
                    cblas_sspmv(CblasColMajor, CblasLower, n - k - 1, -ONE, &AP[kc + n - k], work, 1, ZERO, &AP[kc + 1], 1);
                    AP[kc] = AP[kc] - cblas_sdot(n - k - 1, work, 1, &AP[kc + 1], 1);
                }
                kstep = 1;
            } else {
                /*
                ...
                if (k < n - 1) {
                    ...
                    cblas_sspmv(CblasColMajor, CblasLower, n - k - 1, -ONE, &AP[kc + n - k], work, 1, ZERO, &AP[kc + 1], 1);
                    AP[kc] = AP[kc] - cblas_sdot(n - k - 1, work, 1, &AP[kc + 1], 1);
                    AP[kcnext + 1] = AP[kcnext + 1] - cblas_sdot(n - k - 1, &AP[kc + 1], 1, &AP[kcnext + 2], 1);
                    cblas_scopy(n - k - 1, &AP[kcnext + 2], 1, work, 1);
                    cblas_sspmv(CblasColMajor, CblasLower, n - k - 1, -ONE, &AP[kc + n - k], work, 1, ZERO, &AP[kcnext + 2], 1);
                    AP[kcnext] = AP[kcnext] - cblas_sdot(n - k - 1, work, 1, &AP[kcnext + 2], 1);
                }
                kstep = 2;
                kcnext = kcnext - (n - k + 2);
            }
            ...
            if (kp != k) {
                ...
                if (kp < n - 1)
                    cblas_sswap(n - kp - 1, &AP[kc + kp - k + 1], 1, &AP[kpc + 1], 1);
                kx = kc + kp - k;
                for (j = k + 1; j < kp; j++) {
                    kx = kx + n - j;
                    temp = AP[kc + j - k];
