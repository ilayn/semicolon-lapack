/**
 * @file slasd2.c
 * @brief SLASD2 merges the two sets of singular values together into a single
...
void slasd2(const int nl, const int nr, const int sqre, int* k,
            ...
            float* const restrict DSIGMA,
            float* const restrict U2, const int ldu2,
            float* const restrict VT2, const int ldvt2,
            ...
            int* const restrict COLTYP, int* info)
{
    ...
    float c, eps, hlftol, s, tau, tol, z1;
    ...
    while (1) {
        ...
        if (fabsf(Z[j]) <= tol) {
            ...
            if (fabsf(D[j] - D[jprev]) <= tol) {
                ...
                cblas_srot(n, &U[idxjp * ldu], 1, &U[idxj * ldu], 1, c, s);
                cblas_srot(m, &VT[idxjp], ldvt, &VT[idxj], ldvt, c, s);
...
L120:
    ...
    for (j = 1; j < n; j++) {
        ...
        cblas_scopy(n, &U[idxj * ldu], 1, &U2[j * ldu2], 1);
        cblas_scopy(m, &VT[idxj], ldvt, &VT2[j], ldvt2);
    }
    ...
    cblas_scopy(*k - 1, &U2[1], 1, &Z[1], 1);

    slaset("A", n, 1, 0.0f, 0.0f, U2, ldu2);
    U2[nl] = 1.0f;
    ...
    if (m > n) {
        ...
        cblas_scopy(m, &VT[nl], ldvt, VT2, ldvt2);
    }
    if (m > n) {
        cblas_scopy(m, &VT[m - 1], ldvt, &VT2[m - 1], ldvt2);
    }

    if (n > *k) {
        cblas_scopy(n - *k, &DSIGMA[*k], 1, &D[*k], 1);
        slacpy("A", n, n - *k, &U2[*k * ldu2], ldu2, &U[*k * ldu], ldu);
        slacpy("A", n - *k, m, &VT2[*k], ldvt2, &VT[*k], ldvt);
    }
