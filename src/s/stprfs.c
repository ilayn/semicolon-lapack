    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        cblas_stpmv(CblasColMajor, cblas_uplo, cblas_trans, cblas_diag,
                    n, AP, &work[n], 1);
            ...
            if (upper) {
                ...
                if (nounit) {
                    for (k = 0; k < n; k++) {
                        xk = fabsf(X[k + j * ldx]);
                        for (i = 0; i <= k; i++) {
                            work[i] = work[i] + fabsf(AP[kc + i]) * xk;
                        }
                        kc = kc + k + 1;
                    }
                } else {
                ...
                if (nounit) {
                    for (k = 0; k < n; k++) {
                        xk = fabsf(X[k + j * ldx]);
                        for (i = k; i < n; i++) {
                            work[i] = work[i] + fabsf(AP[kc + i - k]) * xk;
                        }
                        kc = kc + n - k;
                    }
                } else {
            ...
            if (upper) {
                ...
                if (nounit) {
                    for (k = 0; k < n; k++) {
                        s = ZERO;
                        for (i = 0; i <= k; i++) {
                            s = s + fabsf(AP[kc + i]) * fabsf(X[i + j * ldx]);
                        }
                        work[k] = work[k] + s;
                        kc = kc + k + 1;
                    }
                ...
                if (nounit) {
                    for (k = 0; k < n; k++) {
                        s = ZERO;
                        for (i = k; i < n; i++) {
                            s = s + fabsf(AP[kc + i - k]) * fabsf(X[i + j * ldx]);
                        }
                        work[k] = work[k] + s;
                        kc = kc + n - k;
                    }
        ...
        do {
            ...
            if (kase != 0) {
                if (kase == 1) {
                    ...
                    cblas_stpsv(CblasColMajor, cblas_uplo, cblas_transt, cblas_diag,
                                n, AP, &work[n], 1);
                    for (i = 0; i < n; i++) {
                    ...
                    cblas_stpsv(CblasColMajor, cblas_uplo, cblas_trans, cblas_diag,
                                n, AP, &work[n], 1);
                }
            }
        } while (kase != 0);
