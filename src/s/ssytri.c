    ...
    if (upper) {
        ...
        while (k < n) {
            ...
            if (ipiv[k] >= 0) {
                ...
                float t = fabsf(A[k + (k + 1) * lda]);
                float ak = A[k + k * lda] / t;
                float akp1 = A[(k + 1) + (k + 1) * lda] / t;
                float akkp1 = A[k + (k + 1) * lda] / t;
                float d = t * (ak * akp1 - 1.0f);
                A[k + k * lda] = akp1 / d;
                A[(k + 1) + (k + 1) * lda] = ak / d;
                A[k + (k + 1) * lda] = -akkp1 / d;
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                 ...
                 * 0-based: t = |A[k + (k-1)*lda]| */
                float t = fabsf(A[k + (k - 1) * lda]);
                float ak = A[(k - 1) + (k - 1) * lda] / t;
                float akp1 = A[k + k * lda] / t;
                float akkp1 = A[k + (k - 1) * lda] / t;
                float d = t * (ak * akp1 - 1.0f);
