/**
 * @file sormr2.c
 * @brief SORMR2 multiplies a general matrix by the orthogonal matrix from
 ...
 *                       the elementary reflector H(i), as returned by SGERQF
            ...
            int* info)
{
    ...
    *info = 0;
    ...
    for (i = i1; i3 > 0 ? i <= i2 : i >= i2; i += i3) {
         ...
         * at position A(i, nq-k+i). Use slarf1l which handles
         * the implicit last-element = 1 convention. */
        slarf1l(side, mi, ni, &A[i + 0 * lda], lda, tau[i],
                C, ldc, work);
    }
}
