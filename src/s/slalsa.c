/**
 * @file slalsa.c
 * @brief SLALSA computes the SVD of the coefficient matrix in compact form.
 *        Used by sgelsd.
 */
    ...
    *info = 0;
    ...
    for (i = ndb1; i <= nd; i++) {
        ...
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                    nl, nrhs, nl, 1.0f, &U[nlf], ldu, &B[nlf], ldb,
                    0.0f, &BX[nlf], ldbx);
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                    nr, nrhs, nr, 1.0f, &U[nrf], ldu, &B[nrf], ldb,
                    0.0f, &BX[nrf], ldbx);
    }

    for (i = 1; i <= nd; i++) {
        ic = iwork[inode + i - 1];
        cblas_scopy(nrhs, &B[ic], ldb, &BX[ic], ldbx);
    }
...
L50:
    ...
    for (i = ndb1; i <= nd; i++) {
        ...
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                    nlp1, nrhs, nlp1, 1.0f, &VT[nlf], ldu, &B[nlf], ldb,
                    0.0f, &BX[nlf], ldbx);
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                    nrp1, nrhs, nrp1, 1.0f, &VT[nrf], ldu, &B[nrf], ldb,
                    0.0f, &BX[nrf], ldbx);
    }
