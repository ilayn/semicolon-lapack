/**
 * @file ssytd2.c
 * @brief SSYTD2 reduces a symmetric matrix to real symmetric tridiagonal
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (i = n - 2; i >= 0; i--) {
            ...
            slarfg(i + 1, &A[i + (i + 1) * lda], &A[(i + 1) * lda], 1, &taui);
            E[i] = A[i + (i + 1) * lda];
            ...
            if (taui != ZERO) {
                ...
                cblas_ssymv(CblasColMajor, cblas_uplo, i + 1, taui,
                            A, lda, &A[(i + 1) * lda], 1,
                            ZERO, tau, 1);

                /* Compute w := x - 1/2 * tau * (x**T * v) * v */
                alpha = -HALF * taui * cblas_sdot(i + 1, tau, 1,
                                                  &A[(i + 1) * lda], 1);
                cblas_saxpy(i + 1, alpha, &A[(i + 1) * lda], 1, tau, 1);

                /* Apply the transformation as a rank-2 update:
                 *   A := A - v * w**T - w * v**T */
                cblas_ssyr2(CblasColMajor, cblas_uplo, i + 1, -ONE,
        ...
        for (i = 0; i <= n - 2; i++) {
            ...
            slarfg(ni, &A[(i + 1) + i * lda], &A[x_start + i * lda], 1, &taui);
            E[i] = A[(i + 1) + i * lda];
            ...
            if (taui != ZERO) {
                ...
                cblas_ssymv(CblasColMajor, cblas_uplo, ni, taui,
                            &A[(i + 1) + (i + 1) * lda], lda,
                            &A[(i + 1) + i * lda], 1,
                            ZERO, &tau[i], 1);

                /* Compute w := x - 1/2 * tau * (x**T * v) * v */
                alpha = -HALF * taui * cblas_sdot(ni, &tau[i], 1,
                                                  &A[(i + 1) + i * lda], 1);
                cblas_saxpy(ni, alpha, &A[(i + 1) + i * lda], 1, &tau[i], 1);

                /* Apply the transformation as a rank-2 update:
                 *   A := A - v * w**T - w * v**T */
                cblas_ssyr2(CblasColMajor, cblas_uplo, ni, -ONE,
