/**
 * @file sla_gbrpvgrw.c
 * @brief SLA_GBRPVGRW computes the reciprocal pivot growth factor
 ...
 * @param[in] AFB    Single precision array, dimension (ldafb, n).
    ...
    const INT ldafb)
{
    ...
    for (j = 0; j < ncols; j++) {
        ...
        for (i = (j - ku > 0 ? j - ku : 0); i < (j + kl + 1 < n ? j + kl + 1 : n); i++) {
            amax = fmaxf(fabsf(AB[kd + i - j + j * ldab]), amax);
        }
        for (i = (j - ku > 0 ? j - ku : 0); i <= j; i++) {
            umax = fmaxf(fabsf(AFB[kd + i - j + j * ldafb]), umax);
        }
        if (umax != 0.0f) {
