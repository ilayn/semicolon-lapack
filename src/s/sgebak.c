/**
 * @file sgebak.c
 * @brief SGEBAK forms the right or left eigenvectors of a real general matrix
 ...
 * @param[in]     ilo    The integers ILO and IHI determined by SGEBAL.
 ...
 *                       by SGEBAL. Permutation indices are 0-based.
 * @param[in]     m      The number of columns of the matrix V. M >= 0.
 * @param[in,out] V      Double precision array, dimension (LDV,M).
    ...
    if (job[0] == 'P' || job[0] == 'p' || job[0] == 'B' || job[0] == 'b') {
        if (rightv) {
            for (ii = 0; ii < n; ii++) {
                ...
                k = (INT)scale[i];
                if (k == i) continue;
                cblas_sswap(m, &V[i + 0 * ldv], ldv, &V[k + 0 * ldv], ldv);
            }
        }
