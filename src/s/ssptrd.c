/**
 * @file ssptrd.c
 * @brief SSPTRD reduces a real symmetric matrix stored in packed form to
 ...
 */
void ssptrd(const char* uplo, const int n, f32* restrict AP,
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (i = n - 2; i >= 0; i--) {
            ...
            slarfg(i + 1, &AP[i1 + i], &AP[i1], 1, &taui);
            E[i] = AP[i1 + i];
            ...
            if (taui != ZERO) {
                ...
                cblas_sspmv(CblasColMajor, CblasUpper, i + 1, taui, AP,
                            &AP[i1], 1, ZERO, tau, 1);

                /* Compute  w := y - 1/2 * tau * (y**T *v) * v */

                alpha = -HALF * taui * cblas_sdot(i + 1, tau, 1, &AP[i1], 1);
                cblas_saxpy(i + 1, alpha, &AP[i1], 1, tau, 1);

                /* Apply the transformation as a rank-2 update:
                      A := A - v * w**T - w * v**T */

                cblas_sspr2(CblasColMajor, CblasUpper, i + 1, -ONE,
                            &AP[i1], 1, tau, 1, AP);
        ...
        for (i = 0; i < n - 1; i++) {
            ...
            slarfg(n - i - 1, &AP[ii + 1], &AP[ii + 2], 1, &taui);
            E[i] = AP[ii + 1];
            ...
            if (taui != ZERO) {
                ...
                cblas_sspmv(CblasColMajor, CblasLower, n - i - 1, taui,
                            &AP[i1i1], &AP[ii + 1], 1, ZERO, &tau[i], 1);

                /* Compute  w := y - 1/2 * tau * (y**T *v) * v */

                alpha = -HALF * taui * cblas_sdot(n - i - 1, &tau[i], 1,
                                                   &AP[ii + 1], 1);
                cblas_saxpy(n - i - 1, alpha, &AP[ii + 1], 1, &tau[i], 1);

                /* Apply the transformation as a rank-2 update:
                      A := A - v * w**T - w * v**T */

                cblas_sspr2(CblasColMajor, CblasLower, n - i - 1, -ONE,
