    ...
    const INT ldz)
{
    ...
    if (k + 2 == ihi) {
        ...
        slartg(h[0 + 0 * 2], h[1 + 0 * 2], &c1, &s1, &temp);
        h[1 + 0 * 2] = ZERO;
        h[0 + 0 * 2] = temp;
        cblas_srot(2, &h[0 + 1 * 2], 2, &h[1 + 1 * 2], 2, c1, s1);

        slartg(h[1 + 2 * 2], h[1 + 1 * 2], &c1, &s1, &temp);
        cblas_srot(1, &h[0 + 2 * 2], 1, &h[0 + 1 * 2], 1, c1, s1);
        slartg(h[0 + 1 * 2], h[0 + 0 * 2], &c2, &s2, &temp);

        /* DROT(IHI-ISTARTM+1, B(ISTARTM,IHI), 1, B(ISTARTM,IHI-1), 1, C1, S1) */
        /* Count: ihi - istartm + 1 (size, not adjusted) */
        cblas_srot(ihi - istartm + 1, &B[istartm + ihi * ldb], 1,
                   &B[istartm + (ihi - 1) * ldb], 1, c1, s1);
        cblas_srot(ihi - istartm + 1, &B[istartm + (ihi - 1) * ldb], 1,
                   &B[istartm + (ihi - 2) * ldb], 1, c2, s2);
        B[(ihi - 1) + (ihi - 2) * ldb] = ZERO;
        B[ihi + (ihi - 2) * ldb] = ZERO;
        cblas_srot(ihi - istartm + 1, &A[istartm + ihi * lda], 1,
                   &A[istartm + (ihi - 1) * lda], 1, c1, s1);
        cblas_srot(ihi - istartm + 1, &A[istartm + (ihi - 1) * lda], 1,
                   &A[istartm + (ihi - 2) * lda], 1, c2, s2);
        if (ilz) {
            /* Z(1, IHI-ZSTART+1) in Fortran → Z[0 + (ihi-zstart)*ldz] in C */
            cblas_srot(nz, &Z[0 + (ihi - zstart) * ldz], 1,
                       &Z[0 + (ihi - 1 - zstart) * ldz], 1, c1, s1);
            cblas_srot(nz, &Z[0 + (ihi - 1 - zstart) * ldz], 1,
                       &Z[0 + (ihi - 2 - zstart) * ldz], 1, c2, s2);
        }

        /* SLARTG(A(IHI-1,IHI-2), A(IHI,IHI-2), ...) */
        slartg(A[(ihi - 1) + (ihi - 2) * lda], A[ihi + (ihi - 2) * lda],
               &c1, &s1, &temp);
        A[(ihi - 1) + (ihi - 2) * lda] = temp;
        A[ihi + (ihi - 2) * lda] = ZERO;
        /* DROT(ISTOPM-IHI+2, A(IHI-1,IHI-1), LDA, A(IHI,IHI-1), LDA, C1, S1) */
        cblas_srot(istopm - ihi + 2, &A[(ihi - 1) + (ihi - 1) * lda], lda,
                   &A[ihi + (ihi - 1) * lda], lda, c1, s1);
        cblas_srot(istopm - ihi + 2, &B[(ihi - 1) + (ihi - 1) * ldb], ldb,
                   &B[ihi + (ihi - 1) * ldb], ldb, c1, s1);
        if (ilq) {
            /* Q(1, IHI-1-QSTART+1) in Fortran → Q[0 + (ihi-1-qstart)*ldq] in C */
            cblas_srot(nq, &Q[0 + (ihi - 1 - qstart) * ldq], 1,
                       &Q[0 + (ihi - qstart) * ldq], 1, c1, s1);
        }

        /* SLARTG(B(IHI,IHI), B(IHI,IHI-1), ...) */
        slartg(B[ihi + ihi * ldb], B[ihi + (ihi - 1) * ldb],
               &c1, &s1, &temp);
        B[ihi + ihi * ldb] = temp;
        B[ihi + (ihi - 1) * ldb] = ZERO;
        /* DROT(IHI-ISTARTM, ...) - note: no +1 here */
        cblas_srot(ihi - istartm, &B[istartm + ihi * ldb], 1,
                   &B[istartm + (ihi - 1) * ldb], 1, c1, s1);
        cblas_srot(ihi - istartm + 1, &A[istartm + ihi * lda], 1,
                   &A[istartm + (ihi - 1) * lda], 1, c1, s1);
        if (ilz) {
        ...
        cblas_srot(k + 3 - istartm + 1, &A[istartm + (k + 2) * lda], 1,
                   &A[istartm + (k + 1) * lda], 1, c1, s1);
        cblas_srot(k + 3 - istartm + 1, &A[istartm + (k + 1) * lda], 1,
                   &A[istartm + k * lda], 1, c2, s2);
        cblas_srot(k + 2 - istartm + 1, &B[istartm + (k + 2) * ldb], 1,
                   &B[istartm + (k + 1) * ldb], 1, c1, s1);
        cblas_srot(k + 2 - istartm + 1, &B[istartm + (k + 1) * ldb], 1,
                   &B[istartm + k * ldb], 1, c2, s2);
        if (ilz) {
            /* Z(1, K+2-ZSTART+1) in Fortran → Z[0 + (k+2-zstart)*ldz] in C */
            cblas_srot(nz, &Z[0 + (k + 2 - zstart) * ldz], 1,
                       &Z[0 + (k + 1 - zstart) * ldz], 1, c1, s1);
            cblas_srot(nz, &Z[0 + (k + 1 - zstart) * ldz], 1,
        ...
        /* Calculate Q1 and Q2 */
        /* SLARTG(A(K+2,K), A(K+3,K), ...) */
        slartg(A[(k + 2) + k * lda], A[(k + 3) + k * lda], &c1, &s1, &temp);
        A[(k + 2) + k * lda] = temp;
        A[(k + 3) + k * lda] = ZERO;
        slartg(A[(k + 1) + k * lda], A[(k + 2) + k * lda], &c2, &s2, &temp);
        A[(k + 1) + k * lda] = temp;
        A[(k + 2) + k * lda] = ZERO;

        /* Apply transformations from the left */
        /* DROT(ISTOPM-K, A(K+2,K+1), LDA, A(K+3,K+1), LDA, C1, S1) */
        cblas_srot(istopm - k, &A[(k + 2) + (k + 1) * lda], lda,
                   &A[(k + 3) + (k + 1) * lda], lda, c1, s1);
        cblas_srot(istopm - k, &A[(k + 1) + (k + 1) * lda], lda,
                   &A[(k + 2) + (k + 1) * lda], lda, c2, s2);

        cblas_srot(istopm - k, &B[(k + 2) + (k + 1) * ldb], ldb,
                   &B[(k + 3) + (k + 1) * ldb], ldb, c1, s1);
        cblas_srot(istopm - k, &B[(k + 1) + (k + 1) * ldb], ldb,
                   &B[(k + 2) + (k + 1) * ldb], ldb, c2, s2);
        if (ilq) {
            /* Q(1, K+2-QSTART+1) in Fortran → Q[0 + (k+2-qstart)*ldq] in C */
            cblas_srot(nq, &Q[0 + (k + 2 - qstart) * ldq], 1,
                       &Q[0 + (k + 3 - qstart) * ldq], 1, c1, s1);
            cblas_srot(nq, &Q[0 + (k + 1 - qstart) * ldq], 1,
