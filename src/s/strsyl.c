/**
 * @file strsyl.c
 * @brief STRSYL solves the real Sylvester matrix equation.
 */
    ...
    if (notrna && notrnb) {
        ...
        for (l = 0; l < n; l++) {
            ...
            for (k = m - 1; k >= 0; k--) {
                ...
                if (l1 == l2 && k1 == k2) {
                    ...
                    suml = cblas_sdot(m - k1 - 1, &A[k1 + minval * lda], lda,
                                      &C[minval + l1 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k1], ldc, &B[l1 * ldb], 1);
                    ...
                    if (scaloc != ONE) {
                        for (j = 0; j < n; j++)
                            cblas_sscal(m, scaloc, &C[j * ldc], 1);
                        *scale = *scale * scaloc;
                    }
                    C[k1 + l1 * ldc] = x[0];
                    ...
                    suml = cblas_sdot(m - k2 - 1, &A[k1 + minval * lda], lda,
                                      &C[minval + l1 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k1], ldc, &B[l1 * ldb], 1);
                    vec[0] = C[k1 + l1 * ldc] - (suml + sgn * sumr);

                    suml = cblas_sdot(m - k2 - 1, &A[k2 + minval * lda], lda,
                                      &C[minval + l1 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k2], ldc, &B[l1 * ldb], 1);
                    vec[1] = C[k2 + l1 * ldc] - (suml + sgn * sumr);

                    slaln2(0, 2, 1, smin, ONE, &A[k1 + k1 * lda],
                    ...
                    suml = cblas_sdot(m - k1 - 1, &A[k1 + minval * lda], lda,
                                      &C[minval + l2 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k1], ldc, &B[l2 * ldb], 1);
                    vec[1] = sgn * (C[k1 + l2 * ldc] - (suml + sgn * sumr));

                    slaln2(1, 2, 1, smin, ONE, &B[l1 + l1 * ldb],
                    ...
                    suml = cblas_sdot(m - k2 - 1, &A[k2 + minval * lda], lda,
                                      &C[minval + l2 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k2], ldc, &B[l2 * ldb], 1);
        ...
        for (l = 0; l < n; l++) {
            ...
            for (k = 0; k < m; k++) {
                ...
                if (l1 == l2 && k1 == k2) {
                    suml = cblas_sdot(k1, &A[k1 * lda], 1, &C[l1 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k1], ldc, &B[l1 * ldb], 1);
                    vec[0] = C[k1 + l1 * ldc] - (suml + sgn * sumr);
                    scaloc = ONE;
                    ...
                    suml = cblas_sdot(k1, &A[k2 * lda], 1, &C[l1 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k2], ldc, &B[l1 * ldb], 1);
                    vec[1] = C[k2 + l1 * ldc] - (suml + sgn * sumr);

                    slaln2(1, 2, 1, smin, ONE, &A[k1 + k1 * lda],
                           lda, ONE, ONE, vec, 2, -sgn * B[l1 + l1 * ldb],
                           ZERO, x, 2, &scaloc, &xnorm, &ierr);
                    if (ierr != 0)
                        *info = 1;
                    ...
                    suml = cblas_sdot(k1, &A[k1 * lda], 1, &C[l2 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k1], ldc, &B[l2 * ldb], 1);
                    vec[1] = sgn * (C[k1 + l2 * ldc] - (suml + sgn * sumr));
                    ...
                    suml = cblas_sdot(k1, &A[k2 * lda], 1, &C[l2 * ldc], 1);
                    sumr = cblas_sdot(l1, &C[k2], ldc, &B[l2 * ldb], 1);
                    vec[3] = C[k2 + l2 * ldc] - (suml + sgn * sumr);
        ...
        for (l = n - 1; l >= 0; l--) {
            ...
            for (k = 0; k < m; k++) {
                ...
                if (l1 == l2 && k1 == k2) {
                    ...
                    suml = cblas_sdot(k1, &A[k1 * lda], 1, &C[l1 * ldc], 1);
                    sumr = cblas_sdot(n - l1 - 1, &C[k1 + minval * ldc], ldc,
                                      &B[l1 + minval * ldb], ldb);
                    vec[0] = C[k1 + l1 * ldc] - (suml + sgn * sumr);
                    scaloc = ONE;
                    ...
                    suml = cblas_sdot(k1, &A[k1 * lda], 1, &C[l1 * ldc], 1);
                    sumr = cblas_sdot(n - l2 - 1, &C[k1 + minval * ldc], ldc,
                                      &B[l1 + minval * ldb], ldb);
                    vec[0] = C[k1 + l1 * ldc] - (suml + sgn * sumr);

                    suml = cblas_sdot(k1, &A[k2 * lda], 1, &C[l1 * ldc], 1);
                    sumr = cblas_sdot(n - l2 - 1, &C[k2 + minval * ldc], ldc,
                                      &B[l1 + minval * ldb], ldb);
                    vec[1] = C[k2 + l1 * ldc] - (suml + sgn * sumr);
                    ...
                    slaln2(0, 2, 1, smin, ONE, &B[l1 + l1 * ldb],
                           ldb, ONE, ONE, vec, 2, -sgn * A[k1 + k1 * lda],
                           ZERO, x, 2, &scaloc, &xnorm, &ierr);
                    if (ierr != 0)
                        *info = 1;
