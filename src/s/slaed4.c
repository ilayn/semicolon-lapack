...
/**
 * SLAED4 computes the I-th updated eigenvalue of a symmetric
    ...
    if (i == n - 1) {
        ...
        if (w <= ZERO) {
            ...
            if (c <= temp) {
                ...
                b = Z[n - 1] * Z[n - 1] * del;
                if (a < ZERO) {
                    tau = TWO * b / (sqrtf(a * a + FOUR * b * c) - a);
                } else {
                    tau = (a + sqrtf(a * a + FOUR * b * c)) / (TWO * c);
                }
            }
        ...
        if (w <= ZERO) {
            dltlb = fmaxf(dltlb, tau);
        } else {
            dltub = fminf(dltub, tau);
        }
        ...
        *dlam = D[i] + tau;
        ...
        if (!swtch3) {
            ...
            zz[1] = Z[ii] * Z[ii];
            slaed6(niter, orgati, c, &delta[iim1], zz, w, &eta, info);
            if (*info != 0)
                return;
        }
        ...
        erretm = EIGHT * (phi - psi) + erretm + TWO * rhoinv +
                 THREE * fabsf(temp) + fabsf(tau + eta) * dw;
