/**
 * @file strevc3.c
 * @brief STREVC3 computes eigenvectors of a real upper quasi-triangular matrix.
 *
 * This is the Level 3 BLAS version with blocked back-transformation.
 * Faithful port from LAPACK SRC/strevc3.f
 */
                           ...
                           f32* work, const int lwork, int* info)
{
    ...
    *info = 0;
    ...
    /* Workspace query - use NB=64 as a reasonable default for STREVC */
    nb = 64;
    maxwrk = n + 2 * n * nb;
            ...
            if (ip == 0) {
                ...
                for (j = ki - 1; j >= 0; j--) {
                    ...
                    if (j1 == j2) {
                        ...
                        slaln2(0, 1, 1, smin, one, &T[j + j * ldt], ldt,
                        ...
                        /* Scale if necessary */
                        if (scale != one) {
                            cblas_sscal(ki + 1, scale, &work[iv * n], 1);
                        }
                        work[j + iv * n] = x[0];

                        /* Update right-hand side */
                        cblas_saxpy(j, -x[0], &T[j * ldt], 1, &work[iv * n], 1);

                    } else {
                        /* 2-by-2 diagonal block */
                        slaln2(0, 2, 1, smin, one, &T[(j - 1) + (j - 1) * ldt], ldt,
                        ...
                        /* Update right-hand side */
                        cblas_saxpy(j - 1, -x[0], &T[(j - 1) * ldt], 1, &work[iv * n], 1);
                        cblas_saxpy(j - 1, -x[1], &T[j * ldt], 1, &work[iv * n], 1);
                    }
                }
                ...
                if (!over) {
                    ...
                    cblas_scopy(ki + 1, &work[iv * n], 1, &VR[is * ldvr], 1);

                    ii = cblas_isamax(ki + 1, &VR[is * ldvr], 1);
                    remax = one / fabsf(VR[ii + is * ldvr]);
                ...
                for (j = ki - 2; j >= 0; j--) {
                    ...
                    if (j1 == j2) {
                        /* 1-by-1 diagonal block */
                        slaln2(0, 1, 2, smin, one, &T[j + j * ldt], ldt,
                        ...
                        /* Scale if necessary */
                        if (scale != one) {
                            cblas_sscal(ki + 1, scale, &work[(iv - 1) * n], 1);
                            cblas_sscal(ki + 1, scale, &work[iv * n], 1);
                        }
                        work[j + (iv - 1) * n] = x[0];
                        work[j + iv * n] = x[2];
                        ...
                        cblas_saxpy(j, -x[0], &T[j * ldt], 1, &work[(iv - 1) * n], 1);
                        cblas_saxpy(j, -x[2], &T[j * ldt], 1, &work[iv * n], 1);

                    } else {
                        /* 2-by-2 diagonal block */
                        slaln2(0, 2, 2, smin, one, &T[(j - 1) + (j - 1) * ldt], ldt,
                        ...
                        /* Update the right-hand side */
                        cblas_saxpy(j - 1, -x[0], &T[(j - 1) * ldt], 1, &work[(iv - 1) * n], 1);
                        cblas_saxpy(j - 1, -x[1], &T[j * ldt], 1, &work[(iv - 1) * n], 1);
                        cblas_saxpy(j - 1, -x[2], &T[(j - 1) * ldt], 1, &work[iv * n], 1);
                        cblas_saxpy(j - 1, -x[3], &T[j * ldt], 1, &work[iv * n], 1);
                    }
                }
                ...
                if (!over) {
                    /* No back-transform: copy x to VR and normalize */
                    cblas_scopy(ki, &work[(iv - 1) * n], 1, &VR[(is - 1) * ldvr], 1);
                    cblas_scopy(ki, &work[iv * n], 1, &VR[is * ldvr], 1);
                    ...
                    remax = one / emax;
                    cblas_sscal(ki, remax, &VR[(is - 1) * ldvr], 1);
                    cblas_sscal(ki, remax, &VR[is * ldvr], 1);
                    ...
                    /* Version 1: back-transform each vector with GEMV, Q*x */
                    if (ki > 1) {
                        cblas_sgemv(CblasColMajor, CblasNoTrans, n, ki - 1,
                        ...
                        cblas_sscal(n, work[(ki - 1) + (iv - 1) * n], &VR[(ki - 1) * ldvr], 1);
                        cblas_sscal(n, work[ki + iv * n], &VR[ki * ldvr], 1);
                    }
            ...
            if (nb > 1) {
                ...
                if ((iv <= 1) || (ki2 == 0)) {
                    ...
                    for (k = iv; k < nb; k++) {
                        if (iscomplex[k] == 0) {
                            /* Real eigenvector */
                            ii = cblas_isamax(n, &work[(nb + k) * n], 1);
                            remax = one / fabsf(work[ii + (nb + k) * n]);
                        ...
                        cblas_sscal(n, remax, &work[(nb + k) * n], 1);
                    }
                    slacpy("F", n, nb - iv, &work[(nb + iv) * n], n,
                           &VR[ki2 * ldvr], ldvr);
                    iv = nb - 1;
                } else {
                    iv = iv - 1;
            ...
            if (ip == 0) {
                ...
                for (j = ki + 1; j < n; j++) {
                    ...
                    if (j1 == j2) {
                        ...
                        if (work[j] > vcrit) {
                            rec = one / vmax;
                            cblas_sscal(n - ki, rec, &work[ki + iv * n], 1);
                            vmax = one;
                            vcrit = bignum;
                        }

                        work[j + iv * n] = work[j + iv * n] -
                            cblas_sdot(j - ki - 1, &T[(ki + 1) + j * ldt], 1,
                                       &work[(ki + 1) + iv * n], 1);
                        ...
                        /* Scale if necessary */
                        if (scale != one) {
                            cblas_sscal(n - ki, scale, &work[ki + iv * n], 1);
                        }
                        work[j + iv * n] = x[0];
                        vmax = fabsf(work[j + iv * n]);
                        if (vmax > one) vmax = fabsf(work[j + iv * n]);
                        vcrit = bignum / vmax;
                        ...
                        work[(j + 1) + iv * n] = work[(j + 1) + iv * n] -
                            cblas_sdot(j - ki - 1, &T[(ki + 1) + (j + 1) * ldt], 1,
                                       &work[(ki + 1) + iv * n], 1);

                        /* Solve
                         * [T(j,j)-wr    T(j,j+1)  ]^T * X = scale * (work1)
                         * [T(j+1,j)   T(j+1,j+1)-wr]              (work2) */
                        slaln2(1, 2, 1, smin, one, &T[j + j * ldt], ldt,
                               one, one, &work[j + iv * n], n, wr, zero,
                               x, 2, &scale, &xnorm, &ierr);
                ...
                if (!over) {
                    ...
                    cblas_scopy(n - ki, &work[ki + iv * n], 1, &VL[ki + is * ldvl], 1);

                    ii = cblas_isamax(n - ki, &VL[ki + is * ldvl], 1) + ki;
                    remax = one / fabsf(VL[ii + is * ldvl]);
                    ...
                    /* Version 1: back-transform each vector with GEMV, Q*x */
                    if (ki < n - 1) {
                        cblas_sgemv(CblasColMajor, CblasNoTrans, n, n - ki - 1,
                ...
                for (j = ki + 2; j < n; j++) {
                    ...
                    if (j1 == j2) {
                        ...
                        if (work[j] > vcrit) {
                            rec = one / vmax;
                            cblas_sscal(n - ki, rec, &work[ki + iv * n], 1);
                            cblas_sscal(n - ki, rec, &work[ki + (iv + 1) * n], 1);
                            vmax = one;
                            vcrit = bignum;
                        }

                        work[j + iv * n] = work[j + iv * n] -
                            cblas_sdot(j - ki - 2, &T[(ki + 2) + j * ldt], 1,
                                       &work[(ki + 2) + iv * n], 1);
                        ...
                        if (scale != one) {
                            cblas_sscal(n - ki, scale, &work[ki + iv * n], 1);
                            cblas_sscal(n - ki, scale, &work[ki + (iv + 1) * n], 1);
                        }
                        work[j + iv * n] = x[0];
                        work[j + (iv + 1) * n] = x[2];
                        vmax = fabsf(work[j + iv * n]);
                        if (fabsf(work[j + (iv + 1) * n]) > vmax)
                            vmax = fabsf(work[j + (iv + 1) * n]);
                        vcrit = bignum / vmax;
                        ...
                        work[j + (iv + 1) * n] = work[j + (iv + 1) * n] -
                            cblas_sdot(j - ki - 2, &T[(ki + 2) + j * ldt], 1,
                                       &work[(ki + 2) + (iv + 1) * n], 1);
                        work[(j + 1) + iv * n] = work[(j + 1) + iv * n] -
                            cblas_sdot(j - ki - 2, &T[(ki + 2) + (j + 1) * ldt], 1,
                                       &work[(ki + 2) + iv * n], 1);
                         ...
                         * [(T(j+1,j) T(j+1,j+1))]                     */
                        slaln2(1, 2, 2, smin, one, &T[j + j * ldt], ldt,
                               one, one, &work[j + iv * n], n, wr, -wi,
                               x, 2, &scale, &xnorm, &ierr);
                ...
                if (!over) {
                    /* No back-transform: copy x to VL and normalize */
                    cblas_scopy(n - ki, &work[ki + iv * n], 1, &VL[ki + is * ldvl], 1);
                    cblas_scopy(n - ki, &work[ki + (iv + 1) * n], 1, &VL[ki + (is + 1) * ldvl], 1);
                    ...
                    /* Version 1: back-transform each vector with GEMV, Q*x */
                    if (ki < n - 2) {
                        cblas_sgemv(CblasColMajor, CblasNoTrans, n, n - ki - 2,
                        ...
                        cblas_sscal(n, work[ki + iv * n], &VL[ki * ldvl], 1);
                        cblas_sscal(n, work[(ki + 1) + (iv + 1) * n], &VL[(ki + 1) * ldvl], 1);
                    }
