/**
 * @file sorbdb1.c
 * @brief SORBDB1 simultaneously bidiagonalizes the blocks of a tall and skinny matrix with orthonormal columns.
 */
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < q; i++) {
        ...
        slarfgp(p - i, &X11[i + i * ldx11], &X11[(i + 1) + i * ldx11], 1, &taup1[i]);
        slarfgp(m - p - i, &X21[i + i * ldx21], &X21[(i + 1) + i * ldx21], 1, &taup2[i]);
        theta[i] = atan2f(X21[i + i * ldx21], X11[i + i * ldx11]);
        c = cosf(theta[i]);
        s = sinf(theta[i]);
        ...
        if (i < q - 1) {
            cblas_srot(q - i - 1, &X11[i + (i + 1) * ldx11], ldx11,
                       &X21[i + (i + 1) * ldx21], ldx21, c, s);
            slarfgp(q - i - 1, &X21[i + (i + 1) * ldx21], &X21[i + (i + 2) * ldx21],
                    ldx21, &tauq1[i]);
            s = X21[i + (i + 1) * ldx21];
            slarf1f("R", p - i - 1, q - i - 1, &X21[i + (i + 1) * ldx21], ldx21,
                    tauq1[i], &X11[(i + 1) + (i + 1) * ldx11], ldx11, &work[ilarf]);
            slarf1f("R", m - p - i - 1, q - i - 1, &X21[i + (i + 1) * ldx21], ldx21,
                    tauq1[i], &X21[(i + 1) + (i + 1) * ldx21], ldx21, &work[ilarf]);
            float nrm1 = cblas_snrm2(p - i - 1, &X11[(i + 1) + (i + 1) * ldx11], 1);
            float nrm2 = cblas_snrm2(m - p - i - 1, &X21[(i + 1) + (i + 1) * ldx21], 1);
            c = sqrtf(nrm1 * nrm1 + nrm2 * nrm2);
            phi[i] = atan2f(s, c);
            sorbdb5(p - i - 1, m - p - i - 1, q - i - 2,
                    &X11[(i + 1) + (i + 1) * ldx11], 1,
                    &X21[(i + 1) + (i + 1) * ldx21], 1,
