/**
 * @file sorbdb3.c
 * @brief SORBDB3 simultaneously bidiagonalizes the blocks of a tall and skinny matrix with orthonormal columns.
 */
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < m - p; i++) {
        ...
        if (i > 0) {
            cblas_srot(q - i, &X11[(i - 1) + i * ldx11], ldx11,
                       &X21[i + i * ldx21], ldx11, c, s);
        }

        slarfgp(q - i, &X21[i + i * ldx21], &X21[i + (i + 1) * ldx21], ldx21, &tauq1[i]);
        s = X21[i + i * ldx21];
        slarf1f("R", p - i, q - i, &X21[i + i * ldx21], ldx21, tauq1[i],
        ...
        if (i < m - p - 1) {
            slarfgp(m - p - i - 1, &X21[(i + 1) + i * ldx21], &X21[(i + 2) + i * ldx21], 1,
                    &taup2[i]);
            phi[i] = atan2f(X21[(i + 1) + i * ldx21], X11[i + i * ldx11]);
            c = cosf(phi[i]);
            s = sinf(phi[i]);
            slarf1f("L", m - p - i - 1, q - i - 1, &X21[(i + 1) + i * ldx21], 1, taup2[i],
