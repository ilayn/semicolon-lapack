/**
 * @file slaqr5.c
 * @brief SLAQR5 performs a single small-bulge multi-shift QR sweep.
 */
                          ...
                          const INT nh, f32* WH, const INT ldwh)
{
    ...
    for (incol = ktop - 2 * nbmps + 1; incol <= kbot - 2; incol += 2 * nbmps) {
        ...
        ndcol = incol + kdu;
        if (accum)
            slaset("ALL", kdu, kdu, zero, one, U, ldu);
        ...
        for (krcol = incol; krcol <= (incol + 2 * nbmps - 1 < kbot - 2 ?
            ...
            if (bmp22) {
                k = krcol + 2 * (m22 - 1);
                if (k == ktop - 1) {
                    slaqr1(2, &H[(k + 1) + (k + 1) * ldh], ldh,
                           sr[2 * m22 - 2], si[2 * m22 - 2],
                           sr[2 * m22 - 1], si[2 * m22 - 1],
                           &V[(m22 - 1) * ldv]);
                    beta = V[(m22 - 1) * ldv];
                    slarfg(2, &beta, &V[1 + (m22 - 1) * ldv], 1,
                           &V[(m22 - 1) * ldv]);
                } else {
                    beta = H[(k + 1) + k * ldh];
                ...
                if (k >= ktop) {
                    if (H[(k + 1) + k * ldh] != zero) {
                        tst1 = fabsf(H[k + k * ldh]) + fabsf(H[(k + 1) + (k + 1) * ldh]);
                        if (tst1 == zero) {
                            if (k >= ktop + 1)
                                tst1 = tst1 + fabsf(H[k + (k - 1) * ldh]);
                            if (k >= ktop + 2)
                                tst1 = tst1 + fabsf(H[k + (k - 2) * ldh]);
                            if (k >= ktop + 3)
                                tst1 = tst1 + fabsf(H[k + (k - 3) * ldh]);
                            if (k <= kbot - 2)
                                tst1 = tst1 + fabsf(H[(k + 2) + (k + 1) * ldh]);
                            if (k <= kbot - 3)
                                tst1 = tst1 + fabsf(H[(k + 3) + (k + 1) * ldh]);
                            if (k <= kbot - 4)
                                tst1 = tst1 + fabsf(H[(k + 4) + (k + 1) * ldh]);
                        }
                        if (fabsf(H[(k + 1) + k * ldh]) <=
                            (smlnum > ulp * tst1 ? smlnum : ulp * tst1)) {
                            h12 = fabsf(H[(k + 1) + k * ldh]) > fabsf(H[k + (k + 1) * ldh]) ?
                                  fabsf(H[(k + 1) + k * ldh]) : fabsf(H[k + (k + 1) * ldh]);
                            h21 = fabsf(H[(k + 1) + k * ldh]) < fabsf(H[k + (k + 1) * ldh]) ?
                                  fabsf(H[(k + 1) + k * ldh]) : fabsf(H[k + (k + 1) * ldh]);
                            h11 = fabsf(H[(k + 1) + (k + 1) * ldh]) >
                                  fabsf(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]) ?
                                  fabsf(H[(k + 1) + (k + 1) * ldh]) :
                                  fabsf(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]);
                            h22 = fabsf(H[(k + 1) + (k + 1) * ldh]) <
                                  fabsf(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]) ?
                                  fabsf(H[(k + 1) + (k + 1) * ldh]) :
                                  fabsf(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]);
                            scl = h11 + h12;
            ...
            for (m = mbot; m >= mtop; m--) {
                k = krcol + 2 * (m - 1);
                if (k == ktop - 1) {
                    slaqr1(3, &H[ktop + ktop * ldh], ldh,
                           sr[2 * m - 2], si[2 * m - 2],
                           sr[2 * m - 1], si[2 * m - 1],
                           &V[(m - 1) * ldv]);
                    alpha = V[(m - 1) * ldv];
                    slarfg(3, &alpha, &V[1 + (m - 1) * ldv], 1,
                           &V[(m - 1) * ldv]);
                } else {
                    ...
                    if (H[(k + 3) + k * ldh] != zero ||
                        ...
                        slaqr1(3, &H[(k + 1) + (k + 1) * ldh], ldh,
                               sr[2 * m - 2], si[2 * m - 2],
                               sr[2 * m - 1], si[2 * m - 1], vt);
                        alpha = vt[0];
                        slarfg(3, &alpha, &vt[1], 1, &vt[0]);
                        t1 = vt[0];
                        t2 = t1 * vt[1];
                        t3 = t1 * vt[2];
                        refsum = H[(k + 1) + k * ldh] + vt[1] * H[(k + 2) + k * ldh];

                        if (fabsf(H[(k + 2) + k * ldh] - refsum * t2) +
                            fabsf(refsum * t3) > ulp * (fabsf(H[k + k * ldh]) +
                            fabsf(H[(k + 1) + (k + 1) * ldh]) +
                            fabsf(H[(k + 2) + (k + 2) * ldh]))) {
        ...
        if (accum) {
            ...
            for (jcol = (ndcol < kbot ? ndcol : kbot) + 1; jcol <= jbot; jcol += nh) {
                ...
                slacpy("ALL", nu, jlen, WH, ldwh, &H[(incol + k1) + jcol * ldh], ldh);
            }
            ...
            for (jrow = jtop; jrow <= (ktop > incol ? ktop : incol) - 1; jrow += nv) {
                ...
                slacpy("ALL", jlen, nu, WV, ldwv, &H[jrow + (incol + k1) * ldh], ldh);
            }
            ...
            if (wantz) {
                for (jrow = iloz; jrow <= ihiz; jrow += nv) {
                    ...
                    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                               ...
                               &U[(k1 - 1) + (k1 - 1) * ldu], ldu, zero, WV, ldwv);
                    slacpy("ALL", jlen, nu, WV, ldwv, &Z[jrow + (incol + k1) * ldz], ldz);
                }
            }
        }
    }
