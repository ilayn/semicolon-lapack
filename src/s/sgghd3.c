    ...
    int* info)
{
    ...
    *info = 0;
    ...
    if (initq)
        slaset("A", n, n, ZERO, ONE, Q, ldq);
    if (initz)
        slaset("A", n, n, ZERO, ONE, Z, ldz);

    if (n > 1)
        slaset("L", n - 1, n - 1, ZERO, ZERO, &B[1], ldb);
    ...
    if (nb < nbmin || nb >= nh) {
        ...
        for (jcol = ilo - 1; jcol <= ihi - 3; jcol += nb) {
            ...
            slaset("A", nblst, nblst, ZERO, ONE, work, nblst);
            pw = nblst * nblst;
            for (i = 0; i < n2nb; i++) {
                slaset("A", 2 * nnb, 2 * nnb, ZERO, ONE, &work[pw], 2 * nnb);
                pw = pw + 4 * nnb * nnb;
            }
            ...
            for (j = jcol; j <= jcol + nnb - 1; j++) {
                ...
                for (i = ihi - 1; i >= j + 2; i--) {
                    temp = A[(i - 1) + j * lda];
                    slartg(temp, A[i + j * lda], &c, &s, &A[(i - 1) + j * lda]);
                    A[i + j * lda] = c;
                    B[i + j * ldb] = s;
                }
                ...
                for (jj = n - 1; jj >= j + 1; jj--) {
                    ...
                    if (jj < ihi - 1) {
                        ...
                        slartg(temp, B[(jj + 1) + jj * ldb], &c, &s, &B[(jj + 1) + (jj + 1) * ldb]);
                        B[(jj + 1) + jj * ldb] = ZERO;
                        cblas_srot(jj - top, &B[top + (jj + 1) * ldb], 1,
                                   &B[top + jj * ldb], 1, c, s);
                        A[(jj + 1) + j * lda] = c;
                        B[(jj + 1) + j * ldb] = -s;
                    }
                ...
                if (jj > 0) {
                    for (i = jj; i >= 1; i--) {
                        cblas_srot(ihi - top, &A[top + (j + i + 1) * lda], 1,
                                   &A[top + (j + i) * lda], 1,
                ...
                if (j < jcol + nnb - 1) {
                    ...
                    jrow = ihi - nblst;
                    cblas_sgemv(CblasColMajor, CblasTrans, nblst, len, ONE, work,
                    ...
                    cblas_strmv(CblasColMajor, CblasLower, CblasTrans, CblasNonUnit,
                                ...
                                &work[pw + len], 1);
                    cblas_sgemv(CblasColMajor, CblasTrans, len, nblst - len, ONE,
                    ...
                    for (jrow = j0; jrow >= jcol + 1; jrow -= nnb) {
                        ...
                        cblas_strmv(CblasColMajor, CblasLower, CblasTrans, CblasNonUnit,
                                    ...
                                    2 * nnb, &work[pw + len], 1);
                        cblas_sgemv(CblasColMajor, CblasTrans, nnb, len, ONE,
                                    &work[ppwo], 2 * nnb, &A[jrow + (j + 1) * lda], 1,
                                    ONE, &work[pw], 1);
                        cblas_sgemv(CblasColMajor, CblasTrans, len, nnb, ONE,
            ...
            cola = n - jcol - nnb;
            j = ihi - nblst;
            cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, nblst, cola, nblst,
                        ONE, work, nblst, &A[j + (jcol + nnb) * lda], lda,
                        ZERO, &work[pw], nblst);
            slacpy("A", nblst, cola, &work[pw], nblst, &A[j + (jcol + nnb) * lda], lda);
            ppwo = nblst * nblst;
            j0 = j - nnb;
            for (j = j0; j >= jcol + 1; j -= nnb) {
                if (blk22) {
                    sorm22("L", "T", 2 * nnb, cola, nnb, nnb,
                    ...
                    slacpy("A", 2 * nnb, cola, &work[pw], 2 * nnb,
            ...
            if (wantq) {
                ...
                cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nh, nblst, nblst,
                            ONE, &Q[topq + j * ldq], ldq, work, nblst,
                            ZERO, &work[pw], nh);
                slacpy("A", nh, nblst, &work[pw], nh, &Q[topq + j * ldq], ldq);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (initq) {
                    ...
                    if (blk22) {
                        ...
                        slacpy("A", nh, 2 * nnb, &work[pw], nh, &Q[topq + j * ldq], ldq);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
            }
            ...
            if (wantz || top > 0) {
                ...
                slaset("L", ihi - jcol - 2, nnb, ZERO, ZERO, &A[(jcol + 2) + jcol * lda], lda);
                slaset("L", ihi - jcol - 2, nnb, ZERO, ZERO, &B[(jcol + 2) + jcol * ldb], ldb);
            }

            /* Apply accumulated orthogonal matrices to A and B (Fortran lines 766-832) */
            if (top > 0) {
                j = ihi - nblst;
                cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, top, nblst, nblst,
                            ONE, &A[j * lda], lda, work, nblst, ZERO, &work[pw], top);
                slacpy("A", top, nblst, &work[pw], top, &A[j * lda], lda);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (blk22) {
                        sorm22("R", "N", top, 2 * nnb, nnb, nnb,
                        ...
                        slacpy("A", top, 2 * nnb, &work[pw], top, &A[j * lda], lda);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
                ...
                slacpy("A", top, nblst, &work[pw], top, &B[j * ldb], ldb);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (blk22) {
                        ...
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    &work[ppwo], 2 * nnb, ZERO, &work[pw], top);
                        slacpy("A", top, 2 * nnb, &work[pw], top, &B[j * ldb], ldb);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
            }
            ...
            if (wantz) {
                ...
                cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, nh, nblst, nblst,
                            ...
                            ZERO, &work[pw], nh);
                slacpy("A", nh, nblst, &work[pw], nh, &Z[topq + j * ldz], ldz);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (initq) {
                    ...
                    if (blk22) {
                        ...
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    &work[ppwo], 2 * nnb, ZERO, &work[pw], nh);
                        slacpy("A", nh, 2 * nnb, &work[pw], nh, &Z[topq + j * ldz], ldz);
                    }
                    ppwo = ppwo + 4 * nnb * nnb;
                }
            }
