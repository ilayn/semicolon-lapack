    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (nb < nbmin || nb >= nh) {
        ...
        for (jcol = ilo; jcol <= ihi - 2; jcol += nb) {
            ...
            for (j = jcol; j < jcol + nnb; j++) {
                ...
                for (jj = n - 1; jj >= j + 1; jj--) {
                    ...
                    if (jj < ihi) {
                        ...
                        slartg(temp, B[jj + 1 + jj * ldb], &c, &s,
                               &B[jj + 1 + (jj + 1) * ldb]);
                        B[jj + 1 + jj * ldb] = ZERO;
                        cblas_srot(jj + 1 - top, &B[top + (jj + 1) * ldb], 1,
                                   &B[top + jj * ldb], 1, c, s);
                        A[jj + 1 + j * lda] = c;
                        B[jj + 1 + j * ldb] = -s;
                    }
                ...
                if (j < jcol + nnb - 1) {
                    ...
                    jrow = ihi - nblst + 1;
                    cblas_sgemv(CblasColMajor, CblasTrans, nblst, len, ONE,
                    ...
                    cblas_strmv(CblasColMajor, CblasLower, CblasTrans,
                                CblasNonUnit, nblst - len,
                                &work[len * nblst], nblst, &work[pw + len], 1);
                    cblas_sgemv(CblasColMajor, CblasTrans, len, nblst - len,
                                ONE,
                    ...
                    for (jrow = j0; jrow >= jcol + 1; jrow -= nnb) {
                        ...
                        cblas_strmv(CblasColMajor, CblasUpper, CblasTrans,
                                    CblasNonUnit, len,
                                    &work[ppwo + nnb], 2 * nnb,
                                    &work[pw], 1);
                        cblas_strmv(CblasColMajor, CblasLower, CblasTrans,
            ...
            cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
                        ...
                        &work[pw], nblst);
            slacpy("A", nblst, cola, &work[pw], nblst,
            ...
            if (wantz || top > 0) {
                ...
                slaset("L", ihi - jcol - 1, nnb, ZERO, ZERO,
            ...
            if (top > 0) {
                ...
                cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                            ...
                            &work[pw], top);
                slacpy("A", top, nblst, &work[pw], top,
                       &A[j * lda], lda);
                ppwo = nblst * nblst;
                j0 = j - nnb;
                for (j = j0; j >= jcol + 1; j -= nnb) {
                    if (blk22) {
                        ...
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    &work[ppwo], 2 * nnb, ZERO,
                                    &work[pw], top);
                        slacpy("A", top, 2 * nnb, &work[pw], top,
                               &A[j * lda], lda);
                    }
                    ppwo += 4 * nnb * nnb;
                }
...

    {
        ...
        if (jcol < ihi)
            sgghrd(compq2, compz2, n, jcol, ihi, A, lda, B, ldb,
                   Q, ldq, Z, ldz, &ierr);
    }
