    ...
    int* info)
{
    ...
    *info = 0;
    ...
    anorm = slanhs("F", in, &H[(ilo - 1) + (ilo - 1) * ldh], ldh, work);
    bnorm = slanhs("F", in, &T[(ilo - 1) + (ilo - 1) * ldt], ldt, work);
    ...
    for (jiter = 1; jiter <= maxit; jiter++) {
        ...
        if (ilast == ilo - 1) {
            ...
            temp = fabsf(H[ilast + (ilast - 1) * ldh]);
            temp2 = fabsf(H[ilast + ilast * ldh]) + fabsf(H[(ilast - 1) + (ilast - 1) * ldh]);
        ...
        for (j = ilast - 1; j >= ilo - 1; j--) {
            ...
            if (j == ilo - 1) {
                ...
                temp2 = fabsf(H[j + j * ldh]) + fabsf(H[(j - 1) + (j - 1) * ldh]);
                if (temp <= (safmin > ulp * temp2 ? safmin : ulp * temp2)) {
                    H[j + (j - 1) * ldh] = ZERO;
                    ilazro = 1;
                } else {
            ...
            if (fabsf(T[j + j * ldt]) < btol) {
                ...
                if (!ilazro) {
                    ...
                    if (temp * (ascale * fabsf(H[(j + 1) + j * ldh])) <= temp2 * (ascale * atol))
                        ilazr2 = 1;
                }
                ...
                if (ilazro || ilazr2) {
                    for (jch = j; jch < ilast; jch++) {
                        temp = H[jch + jch * ldh];
                        slartg(temp, H[(jch + 1) + jch * ldh], &c, &s, &H[jch + jch * ldh]);
                        ...
                        if (ilq)
                            cblas_srot(n, &Q[jch * ldq], 1, &Q[(jch + 1) * ldq], 1, c, s);
                        if (ilazr2)
                    ...
                    for (jch = j; jch < ilast; jch++) {
                        ...
                        slartg(temp, T[(jch + 1) + (jch + 1) * ldt], &c, &s, &T[jch + (jch + 1) * ldt]);
                        T[(jch + 1) + (jch + 1) * ldt] = ZERO;
                        if (jch < ilastm - 1)
                            cblas_srot(ilastm - jch - 1, &T[jch + (jch + 2) * ldt], ldt,
                                       &T[(jch + 1) + (jch + 2) * ldt], ldt, c, s);
                        cblas_srot(ilastm - jch + 2, &H[jch + (jch - 1) * ldh], ldh,
                                   &H[(jch + 1) + (jch - 1) * ldh], ldh, c, s);
                        if (ilq)
                            cblas_srot(n, &Q[jch * ldq], 1, &Q[(jch + 1) * ldq], 1, c, s);
                        temp = H[(jch + 1) + jch * ldh];
                        slartg(temp, H[(jch + 1) + (jch - 1) * ldh], &c, &s, &H[(jch + 1) + jch * ldh]);
                        H[(jch + 1) + (jch - 1) * ldh] = ZERO;
                        cblas_srot(jch + 1 - ifrstm, &H[ifrstm + jch * ldh], 1,
                                   &H[ifrstm + (jch - 1) * ldh], 1, c, s);
                        cblas_srot(jch - ifrstm, &T[ifrstm + jch * ldt], 1,
                                   &T[ifrstm + (jch - 1) * ldt], 1, c, s);
                        if (ilz)
                            cblas_srot(n, &Z[jch * ldz], 1, &Z[(jch - 1) * ldz], 1, c, s);
                    }
                    goto L70;
                }
            } else if (ilazro) {
...
L70:
        ...
        slartg(temp, H[ilast + (ilast - 1) * ldh], &c, &s, &H[ilast + ilast * ldh]);
        H[ilast + (ilast - 1) * ldh] = ZERO;
        ...
        if (ilz)
            cblas_srot(n, &Z[ilast * ldz], 1, &Z[(ilast - 1) * ldz], 1, c, s);
...
L110:
        ...
        if (fabsf(wr) > temp)
            scale = (scale < temp / fabsf(wr)) ? scale : temp / fabsf(wr);
        s1 = scale * s1;
        wr = scale * wr;
        ...
        for (j = ilast - 1; j >= ifirst + 1; j--) {
            ...
            if (fabsf((ascale * H[(j + 1) + j * ldh]) * temp) <= (ascale * atol) * temp2)
                goto L130;
        }
...
L200:
        if (ifirst + 1 == ilast) {
            ...
            cblas_srot(ilastm + 1 - ifirst, &H[(ilast - 1) + (ilast - 1) * ldh], ldh,
                       &H[ilast + (ilast - 1) * ldh], ldh, cl, sl);
            cblas_srot(ilast + 1 - ifrstm, &H[ifrstm + (ilast - 1) * ldh], 1,
                       &H[ifrstm + ilast * ldh], 1, cr, sr);

            if (ilast < ilastm)
                cblas_srot(ilastm - ilast, &T[(ilast - 1) + (ilast + 1) * ldt], ldt,
                           &T[ilast + (ilast + 1) * ldt], ldt, cl, sl);
            if (ifrstm < ilast - 1)
                cblas_srot(ifirst - ifrstm, &T[ifrstm + (ilast - 1) * ldt], 1,
                           &T[ifrstm + ilast * ldt], 1, cr, sr);

            if (ilq)
                cblas_srot(n, &Q[(ilast - 1) * ldq], 1, &Q[ilast * ldq], 1, cl, sl);
            if (ilz)
                cblas_srot(n, &Z[(ilast - 1) * ldz], 1, &Z[ilast * ldz], 1, cr, sr);
