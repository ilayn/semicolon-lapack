/**
 * @file slalsd.c
 * @brief SLALSD uses the singular value decomposition of A to solve
...
void slalsd(const char* uplo, const int smlsiz, const int n, const int nrhs,
            ...
            float* const restrict B, const int ldb, const float rcond,
            int* rank, float* const restrict work, int* const restrict iwork,
            int* info)
{
    ...
    float cs, eps, orgnrm, r, rcnd, sn, tol;
    ...
    *rank = 0;
    ...
    if (n == 0) {
        ...
        if (D[0] == ZERO) {
            ...
            *rank = 1;
            slascl("G", 0, 0, D[0], ONE, 1, nrhs, B, ldb, info);
            D[0] = fabsf(D[0]);
        }
        return;
    }
    ...
    if (uplo[0] == 'L' || uplo[0] == 'l') {
        ...
        for (i = 0; i < n - 1; i++) {
            ...
            D[i + 1] = cs * D[i + 1];
            if (nrhs == 1) {
                cblas_srot(1, &B[i], 1, &B[i + 1], 1, cs, sn);
            } else {
                work[i * 2] = cs;
                work[i * 2 + 1] = sn;
            }
        ...
        if (nrhs > 1) {
            ...
            for (i = 0; i < nrhs; i++) {
                ...
                for (j = 0; j < n - 1; j++) {
                    cs = work[j * 2];
                    sn = work[j * 2 + 1];
                    cblas_srot(1, &B[j + i * ldb], 1, &B[j + 1 + i * ldb], 1, cs, sn);
                }
            }
        }
    }
    ...
    if (n <= smlsiz) {
        ...
        slasdq("U", 0, n, n, 0, nrhs, D, E, work, n, NULL, n, B, ldb, &work[nwork], info);
        if (*info != 0) {
            return;
        }
        ...
        for (i = 0; i < n; i++) {
            if (D[i] <= tol) {
                slaset("A", 1, nrhs, ZERO, ZERO, &B[i], ldb);
            } else {
                slascl("G", 0, 0, D[i], ONE, 1, nrhs, &B[i], ldb, info);
                *rank = *rank + 1;
            }
        }
    ...
    nlvl = (int)(logf((float)n / (float)(smlsiz + 1)) / logf(TWO)) + 1;
    ...
    for (i = 0; i < nm1; i++) {
        if (fabsf(E[i]) < eps || i == nm1 - 1) {
            ...
            if (i < nm1 - 1) {
                ...
                cblas_scopy(nrhs, &B[n - 1], ldb, &work[bx + n - 1], n);
            }
            st1 = st;
            if (nsize == 1) {
                /* This is a 1-by-1 subproblem and is not solved explicitly. */
                cblas_scopy(nrhs, &B[st], ldb, &work[bx + st1], n);
            } else if (nsize <= smlsiz) {
                /* This is a small subproblem and is solved by SLASDQ. */
                slaset("A", nsize, nsize, ZERO, ONE, &work[vt_idx + st1], n);
                /* NRU=0: U not referenced */
                slasdq("U", 0, nsize, nsize, 0, nrhs, &D[st], &E[st],
                       &work[vt_idx + st1], n, NULL, n,
                       &B[st], ldb, &work[nwork], info);
                if (*info != 0) {
                    return;
                }
                slacpy("A", nsize, nrhs, &B[st], ldb, &work[bx + st1], n);
            } else {
                ...
                bxst = bx + st1;
                slalsa(icmpq2, smlsiz, nsize, nrhs, &B[st], ldb,
    ...
    for (i = 0; i < n; i++) {
         ...
         */
        if (fabsf(D[i]) <= tol) {
            slaset("A", 1, nrhs, ZERO, ZERO, &work[bx + i], n);
        } else {
            *rank = *rank + 1;
            slascl("G", 0, 0, D[i], ONE, 1, nrhs, &work[bx + i], n, info);
        }
        D[i] = fabsf(D[i]);
    }
    ...
    for (i = 0; i < nsub; i++) {
        ...
        bxst = bx + st1;
        if (nsize == 1) {
            cblas_scopy(nrhs, &work[bxst], n, &B[st], ldb);
        } else if (nsize <= smlsiz) {
            cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
