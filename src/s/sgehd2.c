/**
 * @file sgehd2.c
 * @brief SGEHD2 reduces a general square matrix to upper Hessenberg form
            ...
            f32* work, int* info)
{
    ...
    *info = 0;
    ...
    for (i = ilo; i <= ihi - 1; i++) {
        ...
        slarfg(len, &A[(i + 1) + i * lda], &A[start + i * lda], 1, &tau[i]);

        /* Apply H(i) to A(0:ihi, i+1:ihi) from the right */
        slarf1f("Right", ihi + 1, len, &A[(i + 1) + i * lda], 1, tau[i],
                &A[(i + 1) * lda], lda, work);

        /* Apply H(i) to A(i+1:ihi, i+1:n-1) from the left */
        slarf1f("Left", len, n - i - 1, &A[(i + 1) + i * lda], 1, tau[i],
