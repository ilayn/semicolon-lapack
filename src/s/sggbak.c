/**
 * @file sggbak.c
 * @brief SGGBAK forms the right or left eigenvectors of a real generalized eigenvalue problem.
 */
 ...
 * @param[in]     ihi     The integers ILO and IHI determined by SGGBAL.
 * @param[in]     lscale  Array of dimension (n). Details of permutations and/or
 *                        scaling factors applied to left side, as returned by SGGBAL.
 * @param[in]     rscale  Array of dimension (n). Details of permutations and/or
 *                        scaling factors applied to right side, as returned by SGGBAL.
...
void sggbak(
    ...
    const int ilo,
    const int ihi,
    const float* const restrict lscale,
    const float* const restrict rscale,
    const int m,
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    if (job[0] == 'S' || job[0] == 's' || job[0] == 'B' || job[0] == 'b') {
        if (rightv) {
            for (i = ilo - 1; i < ihi; i++) {
                cblas_sscal(m, rscale[i], &V[i], ldv);
            }
        }

        if (leftv) {
            for (i = ilo - 1; i < ihi; i++) {
                cblas_sscal(m, lscale[i], &V[i], ldv);
            }
        }
    }
    ...
    if (job[0] == 'P' || job[0] == 'p' || job[0] == 'B' || job[0] == 'b') {
        if (rightv) {
            ...
            for (i = ilo - 2; i >= 0; i--) {
                ...
                if (k == i + 1)
                    continue;
                cblas_sswap(m, &V[i], ldv, &V[k - 1], ldv);
            }
