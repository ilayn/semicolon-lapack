/**
 * @file sgeqrf.c
 * @brief SGEQRF computes a QR factorization of a general M-by-N matrix
    ...
    *info = 0;
    ...
    if (nb >= nbmin && nb < k && nx < k) {
        ...
        for (i = 0; i < k - nx; i += nb) {
            ...
            sgeqr2(m - i, ib, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    ...
    if (i < k) {
        sgeqr2(m - i, n - i, &A[i + i * lda], lda, &tau[i], work, &iinfo);
    }
