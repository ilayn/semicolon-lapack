/**
 * @file sgelsd.c
 * @brief SGELSD computes the minimum norm solution to a real linear least
            ...
            int* const restrict iwork, int* info)
{
    ...
    float anrm, bignum, bnrm, eps, sfmin, smlnum;
    int iinfo;
    int nb;
    ...
    *info = 0;
    ...
    nlvl = (int)(logf((float)minmn / (float)(smlsiz + 1)) / logf(TWO)) + 1;
    if (nlvl < 0) nlvl = 0;
    ...
    if (m >= n) {
        ...
        if (m >= mnthr) {
            ...
            /* Compute A=Q*R */
            sgeqrf(m, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &iinfo);
        ...
        slalsd("U", smlsiz, n, nrhs, S, &work[ie], B, ldb, rcond, rank,
        ...
        sormbr("P", "L", "N", n, nrhs, n, A, lda, &work[itaup],
               B, ldb, &work[nwork], lwork - nwork, &iinfo);
        ...
        /* Compute A=L*Q */
        sgelqf(m, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &iinfo);
        il = nwork;
        ...
        /* Solve the bidiagonal least squares problem */
        slalsd("U", smlsiz, m, nrhs, S, &work[ie], B, ldb, rcond, rank,
               &work[nwork], iwork, info);
        if (*info != 0) {
            goto cleanup;
        }
        ...
        sormbr("P", "L", "N", m, nrhs, m, &work[il], ldwork, &work[itaup],
               B, ldb, &work[nwork], lwork - nwork, &iinfo);
        ...
        /* Solve the bidiagonal least squares problem */
        slalsd("L", smlsiz, m, nrhs, S, &work[ie], B, ldb, rcond, rank,
               &work[nwork], iwork, info);
        if (*info != 0) {
            goto cleanup;
        }
        ...
        sormbr("P", "L", "N", n, nrhs, m, A, lda, &work[itaup],
               B, ldb, &work[nwork], lwork - nwork, &iinfo);
    }
