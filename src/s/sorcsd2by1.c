/**
 * @file sorcsd2by1.c
 * @brief SORCSD2BY1 computes the CS decomposition of an M-by-Q matrix with orthonormal columns partitioned into a 2-by-1 block structure.
 */
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (r == q) {
            ...
            lorbdb = (INT)work[0];
            if (wantu1 && p > 0) {
                sorgqr(p - 1, p - 1, p - 1, &U1[1 + 1 * ldu1], ldu1, dum1, work, -1, &childinfo);
            ...
            if (wantv1t && q > 0) {
                sorglq(q, q, r, V1T, ldv1t, dum1, work, -1, &childinfo);
                lorglqmin = lorglqmin > q ? lorglqmin : q;
                lorglqopt = lorglqopt > (INT)work[0] ? lorglqopt : (INT)work[0];
            }
            sbbcsd(jobv1t, "N", jobu1, jobu2, "T", m, q, p, theta,
            ...
            if (wantu2 && m - p > 0) {
                sorgqr(m - p - 1, m - p - 1, m - p - 1, &U2[1 + 1 * ldu2], ldu2,
            ...
            sbbcsd("N", jobv1t, jobu2, jobu1, "T", m, m - q, m - p,
            ...
            lorbdb = m + (INT)work[0];
            if (wantu1 && p > 0) {
                sorgqr(p, p, m - q, U1, ldu1, dum1, work, -1, &childinfo);
                lorgqrmin = lorgqrmin > p ? lorgqrmin : p;
                lorgqropt = lorgqropt > (INT)work[0] ? lorgqropt : (INT)work[0];
            }
            if (wantu2 && m - p > 0) {
                sorgqr(m - p, m - p, m - q, U2, ldu2, dum1, work, -1, &childinfo);
                lorgqrmin = lorgqrmin > (m - p) ? lorgqrmin : (m - p);
                lorgqropt = lorgqropt > (INT)work[0] ? lorgqropt : (INT)work[0];
            }
            if (wantv1t && q > 0) {
                sorglq(q, q, q, V1T, ldv1t, dum1, work, -1, &childinfo);
                lorglqmin = lorglqmin > q ? lorglqmin : q;
                lorglqopt = lorglqopt > (INT)work[0] ? lorglqopt : (INT)work[0];
            }
            sbbcsd(jobu2, jobu1, "N", jobv1t, "N", m, m - p, m - q,
    ...
    if (r == 0) {
        ...
        if (q == 0) {
            if (wantu1 && p > 0)
                slaset("A", p, p, zero, one, U1, ldu1);
            if (wantu2 && m - p > 0)
                slaset("A", m - p, m - p, zero, one, U2, ldu2);
            return;
        }

        if (p == 0 && m == q) {
            if (wantu2)
                slacpy("A", m - p, q, X21, ldx21, U2, ldu2);
            if (wantv1t)
                slaset("A", q, q, zero, one, V1T, ldv1t);
            return;
        }

        if (p == m && m == q) {
            if (wantu1)
                slacpy("A", p, q, X11, ldx11, U1, ldu1);
            if (wantv1t)
                slaset("A", q, q, zero, one, V1T, ldv1t);
            return;
        }
    ...
    if (r == q) {
        ...
        if (wantu1 && p > 0) {
            ...
            sorgqr(p, p, q, U1, ldu1, &work[itaup1 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantu2 && m - p > 0) {
            slacpy("L", m - p, q, X21, ldx21, U2, ldu2);
            sorgqr(m - p, m - p, q, U2, ldu2, &work[itaup2 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantv1t && q > 0) {
            V1T[0] = one;
            ...
            slacpy("U", q - 1, q - 1, &X21[1 * ldx21], ldx21, &V1T[1 + 1 * ldv1t], ldv1t);
            sorglq(q - 1, q - 1, q - 1, &V1T[1 + 1 * ldv1t], ldv1t,
        ...
        if (wantu1 && p > 0) {
            ...
            slacpy("L", p - 1, p - 1, &X11[1], ldx11, &U1[1 + 1 * ldu1], ldu1);
            sorgqr(p - 1, p - 1, p - 1, &U1[1 + 1 * ldu1], ldu1, &work[itaup1 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantu2 && m - p > 0) {
            slacpy("L", m - p, q, X21, ldx21, U2, ldu2);
        ...
        if (wantv1t && q > 0) {
            ...
            sorglq(q, q, r, V1T, ldv1t, &work[itauq1 - 1],
        ...
        if (wantu2 && m - p > 0) {
            ...
            slacpy("L", m - p - 1, m - p - 1, &X21[1], ldx21, &U2[1 + 1 * ldu2], ldu2);
            sorgqr(m - p - 1, m - p - 1, m - p - 1, &U2[1 + 1 * ldu2], ldu2,
        ...
        sbbcsd("N", jobv1t, jobu2, jobu1, "T", m, m - q, m - p, theta,
        ...
        if (q > r) {
            ...
            if (wantu1) {
                slapmt(0, p, q, U1, ldu1, iwork);
            }
            if (wantv1t) {
                slapmr(0, q, q, V1T, ldv1t, iwork);
            }
        }
        ...
        if (wantu2 && m - p > 0) {
            cblas_scopy(m - p, &work[iorbdb + p - 1], 1, U2, 1);
        }
        if (wantu1 && p > 0) {
            cblas_scopy(p, &work[iorbdb - 1], 1, U1, 1);
            for (j = 1; j < p; j++) {
                U1[j * ldu1] = zero;
            }
            slacpy("L", p - 1, m - q - 1, &X11[1], ldx11, &U1[1 + 1 * ldu1], ldu1);
            sorgqr(p, p, m - q, U1, ldu1, &work[itaup1 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantu2 && m - p > 0) {
            for (j = 1; j < m - p; j++) {
            ...
            slacpy("L", m - p - 1, m - q - 1, &X21[1], ldx21, &U2[1 + 1 * ldu2], ldu2);
            sorgqr(m - p, m - p, m - q, U2, ldu2, &work[itaup2 - 1],
                   &work[iorgqr - 1], lorgqr, &childinfo);
        }
        if (wantv1t && q > 0) {
            slacpy("U", m - q, q, X21, ldx21, V1T, ldv1t);
            slacpy("U", p - (m - q), q - (m - q), &X11[(m - q) + (m - q) * ldx11], ldx11,
                   &V1T[(m - q) + (m - q) * ldv1t], ldv1t);
            slacpy("U", -p + q, q - p, &X21[(m - q) + p * ldx21], ldx21,
                   &V1T[p + p * ldv1t], ldv1t);
            sorglq(q, q, q, V1T, ldv1t, &work[itauq1 - 1],
                   &work[iorglq - 1], lorglq, &childinfo);
        }

        sbbcsd(jobu2, jobu1, "N", jobv1t, "N", m, m - p, m - q, theta,
        ...
        if (p > r) {
            ...
            if (wantu1) {
                slapmt(0, p, p, U1, ldu1, iwork);
            }
            if (wantv1t) {
                slapmr(0, p, q, V1T, ldv1t, iwork);
            }
        }
    }
}
