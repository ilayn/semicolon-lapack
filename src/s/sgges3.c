/**
 * @file sgges3.c
 * @brief SGGES3 computes the eigenvalues, the Schur form, and, optionally,
            ...
            int* const restrict bwork, int* info)
{
    ...
    *info = 0;
    ...
    /* Initialize VSR */
    if (ilvsr)
        slaset("Full", n, n, ZERO, ONE, VSR, ldvsr);

    /* Reduce to generalized Hessenberg form */
    sgghd3(jobvsl, jobvsr, n, ilo, ihi, A, lda, B, ldb, VSL,
           ldvsl, VSR, ldvsr, &work[iwrk], lwork - iwrk, &ierr);

    /* Perform QZ algorithm, computing Schur vectors if desired */
    iwrk = itau;
    slaqz0("S", jobvsl, jobvsr, n, ilo, ihi, A, lda, B, ldb,
           alphar, alphai, beta, VSL, ldvsl, VSR, ldvsr,
        ...
        for (i = 0; i < n; i++) {
            if (alphai[i] != ZERO) {
                if ((alphar[i] / safmax) > (anrmto / anrm) ||
                    ...
                    work[0] = fabsf(A[i + i * lda] / alphar[i]);
                    ...
                    work[0] = fabsf(A[i + (i + 1) * lda] / alphai[i]);
                    beta[i] = beta[i] * work[0];
                    alphar[i] = alphar[i] * work[0];
                    alphai[i] = alphai[i] * work[0];
                }
