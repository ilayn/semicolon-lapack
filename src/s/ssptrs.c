/**
 * @file ssptrs.c
 * @brief SSPTRS solves a system of linear equations with a symmetric matrix
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k > 0)
                    cblas_sger(CblasColMajor, k, nrhs, -ONE, &AP[kc], 1, &B[k], ldb, B, ldb);

                /* Multiply by the inverse of the diagonal block. */
                cblas_sscal(nrhs, ONE / AP[kc + k], &B[k], ldb);
                k = k - 1;
            } else {
                ...
                if (k > 1) {
                    cblas_sger(CblasColMajor, k - 1, nrhs, -ONE, &AP[kc], 1, &B[k], ldb, B, ldb);
                    cblas_sger(CblasColMajor, k - 1, nrhs, -ONE, &AP[kc - k], 1, &B[k - 1], ldb, B, ldb);
                }
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                 ...
                 * stored in column K of A. */
                if (k > 0)
                    cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, -ONE, B, ldb, &AP[kc], 1, ONE, &B[k], ldb);
                ...
                if (k > 0) {
                    cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, -ONE, B, ldb, &AP[kc], 1, ONE, &B[k], ldb);
                    cblas_sgemv(CblasColMajor, CblasTrans, k, nrhs, -ONE, B, ldb, &AP[kc + k + 1], 1, ONE, &B[k + 1], ldb);
                }
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                 ...
                 * stored in column K of A. */
                if (k < n - 1)
                    cblas_sger(CblasColMajor, n - k - 1, nrhs, -ONE, &AP[kc + 1], 1, &B[k], ldb, &B[k + 1], ldb);

                /* Multiply by the inverse of the diagonal block. */
                cblas_sscal(nrhs, ONE / AP[kc], &B[k], ldb);
                kc = kc + n - k;
                k = k + 1;
            } else {
                /* 2 x 2 diagonal block
                 ...
                 * stored in columns K and K+1 of A. */
                if (k < n - 2) {
                    cblas_sger(CblasColMajor, n - k - 2, nrhs, -ONE, &AP[kc + 2], 1, &B[k], ldb, &B[k + 2], ldb);
                    cblas_sger(CblasColMajor, n - k - 2, nrhs, -ONE, &AP[kc + n - k + 1], 1, &B[k + 1], ldb, &B[k + 2], ldb);
                }
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                 ...
                 * stored in column K of A. */
                if (k < n - 1)
                    cblas_sgemv(CblasColMajor, CblasTrans, n - k - 1, nrhs, -ONE, &B[k + 1], ldb, &AP[kc + 1], 1, ONE, &B[k], ldb);
                ...
                if (k < n - 1) {
                    cblas_sgemv(CblasColMajor, CblasTrans, n - k - 1, nrhs, -ONE, &B[k + 1], ldb, &AP[kc + 1], 1, ONE, &B[k], ldb);
                    cblas_sgemv(CblasColMajor, CblasTrans, n - k - 1, nrhs, -ONE, &B[k + 1], ldb, &AP[kc - (n - k) + 1], 1, ONE, &B[k - 1], ldb);
                }
