/**
 * @file slaein.c
 * @brief SLAEIN computes a specified right or left eigenvector of an upper
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    if (wi == ZERO) {
        ...
        if (rightv) {
            ...
            for (its = 0; its < n; its++) {
                ...
                slatrs("U", "N", "N", &normin, n, B, ldb, vr, &scale, work, &ierr);
                normin = 'Y';

                /* Test for sufficient growth in the norm of v */
                vnorm = cblas_sasum(n, vr, 1);
        ...
        L120:
            ...
            i = cblas_isamax(n, vr, 1);
            cblas_sscal(n, ONE / fabsf(vr[i]), vr, 1);
            ...
            for (its = 0; its < n; its++) {
                ...
                slatrs("U", "T", "N", &normin, n, B, ldb, vr, &scale, work, &ierr);
                normin = 'Y';
        ...
        if (rightv) {
            ...
            for (i = 0; i < n - 1; i++) {
                absbii = slapy2(B[i + i * ldb], B[i + 1 + i * ldb]);
                ei = H[i + 1 + i * ldh];
                if (absbii < fabsf(ei)) {
                    /* Interchange rows and eliminate */
                    xr = B[i + i * ldb] / ei;
                ...
                work[i] = cblas_sasum(n - i - 1, &B[i + (i + 1) * ldb], ldb) +
                          cblas_sasum(n - i - 1, &B[i + 2 + i * ldb], 1);
            }
            ...
            for (j = n - 1; j >= 1; j--) {
                ej = H[j + (j - 1) * ldh];
                absbjj = slapy2(B[j + j * ldb], B[j + 1 + j * ldb]);
                if (absbjj < fabsf(ej)) {
                    /* Interchange columns and eliminate */
                    xr = B[j + j * ldb] / ej;
                    xi = B[j + 1 + j * ldb] / ej;
                ...
                work[j] = cblas_sasum(j, &B[j * ldb], 1) +
                          cblas_sasum(j, &B[j + 1], ldb);
            }
        ...
        for (its = 0; its < n; its++) {
            ...
            for (i = i1; (i3 > 0) ? (i <= i2) : (i >= i2); i += i3) {
                ...
                w = fabsf(B[i + i * ldb]) + fabsf(B[i + 1 + i * ldb]);
                if (w > smlnum) {
                    if (w < ONE) {
                        w1 = fabsf(xr) + fabsf(xi);
                        if (w1 > w * bignum) {
                            rec = ONE / w1;
                            cblas_sscal(n, rec, vr, 1);
                            cblas_sscal(n, rec, vi, 1);
            ...
            vnorm = cblas_sasum(n, vr, 1) + cblas_sasum(n, vi, 1);
            if (vnorm >= growto * scale) {
                goto L280;
            }
    ...
    L280:
        ...
        cblas_sscal(n, ONE / vnorm, vr, 1);
        cblas_sscal(n, ONE / vnorm, vi, 1);
    }
}
