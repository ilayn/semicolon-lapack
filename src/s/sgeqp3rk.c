/**
 * @file sgeqp3rk.c
 * @brief SGEQP3RK computes a truncated Householder QR factorization with column pivoting of a real m-by-n matrix using Level 3 BLAS.
 */
...
/**
 * SGEQP3RK computes a truncated (rank K) or full rank Householder QR
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    for (j = 0; j < n; j++) {
        work[j] = cblas_snrm2(m, &A[0 + j * lda], 1);
        work[n + j] = work[j];
    }

    kp1 = cblas_isamax(n, &work[0], 1);
    maxc2nrm = work[kp1];
    ...
    if (j <= jmax) {
        ...
        *K = jmax;
        ...
        if (*K < minmn) {
            jmaxc2nrm = *K + cblas_isamax(n - *K, &work[*K], 1);
