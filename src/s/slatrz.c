...
/**
 * @file slatrz.c
 * @brief SLATRZ factors an upper trapezoidal matrix by means of orthogonal
 *        transformations.
 */
 ...
 * by means of orthogonal transformations. Z is an (M+L)-by-(M+L)
 * orthogonal matrix and, R and A1 are M-by-M upper triangular matrices.
 *
 ...
 *                      TAU, represent the orthogonal matrix Z as a product of
            ...
            f32* restrict work)
{
    ...
    for (i = m - 1; i >= 0; i--) {
         ...
         * [ A(i,i) A(i,n-l:n-1) ]. */
        slarfg(l + 1, &A[i + i * lda], &A[i + (n - l) * lda], lda, &tau[i]);

        /* Apply H(i) to A(0:i-1, i:n-1) from the right. */
        if (i > 0) {
            slarz("R", i, n - i, l, &A[i + (n - l) * lda], lda, tau[i],
                  &A[i * lda], lda, work);
        }
    }
}
