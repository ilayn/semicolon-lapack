/**
 * @file sgbtrs.c
 * @brief SGBTRS solves a system of linear equations with a general band
 ...
 *                        as computed by SGBTRF. U is stored as an upper triangular
    ...
    if (notran) {
        ...
        if (lnoti) {
            for (j = n - 2; j >= 0; j--) {
                ...
                cblas_sgemv(CblasColMajor, CblasTrans, lm, nrhs, NEG_ONE,
                            &B[j + 1], ldb,
                            &AB[kd + 1 + j * ldab], 1,
                            ONE, &B[j], ldb);
                l = ipiv[j];
