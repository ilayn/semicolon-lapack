/**
 * @file sgglse.c
 * @brief SGGLSE solves the linear equality-constrained least squares (LSE) problem.
 */
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    if (p > 0) {
        ...
        cblas_scopy(p, D, 1, &X[n - p], 1);

        cblas_sgemv(CblasColMajor, CblasNoTrans, n - p, p, -one,
                    &A[0 + (n - p) * lda], lda, D, 1, one, C, 1);
    }

    if (n > p) {
        strtrs("U", "N", "N", n - p, 1, A, lda, C, n - p, info);
        ...
        cblas_scopy(n - p, C, 1, X, 1);
    }

    if (m < n) {
        nr = m + p - n;
        if (nr > 0) {
            cblas_sgemv(CblasColMajor, CblasNoTrans, nr, n - m, -one,
    ...
    if (nr > 0) {
        ...
        cblas_saxpy(nr, -one, D, 1, &C[n - p], 1);
    }
