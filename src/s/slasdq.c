/**
 * @file slasdq.c
 * @brief SLASDQ computes the SVD of a real bidiagonal matrix with diagonal d
 *        and off-diagonal e.
 */
            ...
            float* const restrict work, int* info)
{
    ...
    *info = 0;
    ...
    if (iuplo == 1 && sqre1 == 1) {
        ...
        if (ncvt > 0) {
            slasr("L", "V", "F", np1, ncvt, &work[0], &work[np1 - 1], VT, ldvt);
        }
    }
    ...
    if (iuplo == 2) {
        ...
        if (nru > 0) {
            if (sqre1 == 0) {
                slasr("R", "V", "F", nru, n, &work[0], &work[np1 - 1], U, ldu);
            } else {
                slasr("R", "V", "F", nru, np1, &work[0], &work[np1 - 1], U, ldu);
            }
        }
        if (ncc > 0) {
            if (sqre1 == 0) {
                slasr("L", "V", "F", n, ncc, &work[0], &work[np1 - 1], C, ldc);
            } else {
                slasr("L", "V", "F", np1, ncc, &work[0], &work[np1 - 1], C, ldc);
            }
        }
    }
    ...
    for (i = 0; i < n; i++) {
        ...
        if (isub != i) {
            ...
            D[i] = smin;
            if (ncvt > 0) {
                cblas_sswap(ncvt, &VT[isub + 0 * ldvt], ldvt, &VT[i + 0 * ldvt], ldvt);
            }
            if (nru > 0) {
                cblas_sswap(nru, &U[0 + isub * ldu], 1, &U[0 + i * ldu], 1);
            }
            if (ncc > 0) {
                cblas_sswap(ncc, &C[isub + 0 * ldc], ldc, &C[i + 0 * ldc], ldc);
            }
        }
    }
}
