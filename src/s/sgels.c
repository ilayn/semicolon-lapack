/**
 * @file sgels.c
 * @brief SGELS solves overdetermined or underdetermined real linear systems
           ...
           int *info)
{
    ...
    *info = 0;
    ...
    if (bnrm > 0.0f && bnrm < smlnum) {
        /* Scale matrix norm up to SMLNUM */
        slascl("G", 0, 0, bnrm, smlnum, brow, nrhs, B, ldb, &iinfo);
        ibscl = 1;
    } else if (bnrm > bignum) {
        /* Scale matrix norm down to BIGNUM */
        slascl("G", 0, 0, bnrm, bignum, brow, nrhs, B, ldb, &iinfo);
        ibscl = 2;
    }
    ...
    if (m >= n) {
        ...
        sgeqrf(m, n, A, lda, work, &work[mn], lwork - mn, &iinfo);

        if (!tpsd) {
            /* Least-Squares Problem min || A * X - B ||
             *
             * B(0:m-1, 0:nrhs-1) := Q^T * B(0:m-1, 0:nrhs-1) */
            sormqr("L", "T", m, nrhs, n, A, lda,
        ...
        /* work[0..mn-1] = tau, work[mn..] = sub-workspace */
        sgelqf(m, n, A, lda, work, &work[mn], lwork - mn, &iinfo);
    ...
    /* Undo scaling */
    if (iascl == 1) {
        slascl("G", 0, 0, anrm, smlnum, scllen, nrhs, B, ldb, &iinfo);
    } else if (iascl == 2) {
        slascl("G", 0, 0, anrm, bignum, scllen, nrhs, B, ldb, &iinfo);
    }
    if (ibscl == 1) {
        slascl("G", 0, 0, smlnum, bnrm, scllen, nrhs, B, ldb, &iinfo);
    } else if (ibscl == 2) {
