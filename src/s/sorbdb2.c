 ...
 *
 * @param[out] taup1
 *          Double precision array, dimension (p-1).
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < p; i++) {
        ...
        if (i > 0) {
            cblas_srot(q - i, &X11[i + i * ldx11], ldx11,
        ...
        slarf1f("R", m - p - i, q - i, &X11[i + i * ldx11], ldx11, tauq1[i],
                &X21[i + i * ldx21], ldx21, &work[ilarf]);
        float nrm1 = cblas_snrm2(p - i - 1, &X11[(i + 1) + i * ldx11], 1);
        float nrm2 = cblas_snrm2(m - p - i, &X21[i + i * ldx21], 1);
        s = sqrtf(nrm1 * nrm1 + nrm2 * nrm2);
        theta[i] = atan2f(s, c);

        sorbdb5(p - i - 1, m - p - i, q - i - 1,
                &X11[(i + 1) + i * ldx11], 1, &X21[i + i * ldx21], 1,
                &X11[(i + 1) + (i + 1) * ldx11], ldx11,
                &X21[i + (i + 1) * ldx21], ldx21,
                &work[iorbdb5], lorbdb5, &childinfo);
        cblas_sscal(p - i - 1, negone, &X11[(i + 1) + i * ldx11], 1);
        slarfgp(m - p - i, &X21[i + i * ldx21], &X21[(i + 1) + i * ldx21], 1, &taup2[i]);
        if (i < p - 1) {
            slarfgp(p - i - 1, &X11[(i + 1) + i * ldx11], &X11[(i + 2) + i * ldx11], 1, &taup1[i]);
            phi[i] = atan2f(X11[(i + 1) + i * ldx11], X21[i + i * ldx21]);
            c = cosf(phi[i]);
            s = sinf(phi[i]);
            slarf1f("L", p - i - 1, q - i - 1, &X11[(i + 1) + i * ldx11], 1, taup1[i],
