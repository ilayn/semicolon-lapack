    ...
    INT* info)
{
    ...
    if (!(job[0] == 'S' || job[0] == 's')) {
        ...
        for (;;) {
            ...
            for (i = l; i >= 0; i--) {
                ...
                lscale[m] = (f32)i;
                if (i != m) {
                    cblas_sswap(n - k, &A[i + k * lda], lda, &A[m + k * lda], lda);
                    cblas_sswap(n - k, &B[i + k * ldb], ldb, &B[m + k * ldb], ldb);
                }
                ...
                rscale[m] = (f32)j;
                if (j != m) {
                    cblas_sswap(l + 1, &A[j * lda], 1, &A[m * lda], 1);
                    cblas_sswap(l + 1, &B[j * ldb], 1, &B[m * ldb], 1);
                }
                l--;
                found = 1;
                break;
    ...
    *ihi = l;
    ...
    for (i = k; i <= l; i++) {
        ...
        icab = cblas_isamax(l + 1, &A[i * lda], 1);
    ...
    for (i = k; i <= l; i++) {
        cblas_sscal(n - k, lscale[i], &A[i + k * lda], lda);
        cblas_sscal(n - k, lscale[i], &B[i + k * ldb], ldb);
    }

    /* Column scaling of matrices A and B */

    for (j = k; j <= l; j++) {
        cblas_sscal(l + 1, rscale[j], &A[j * lda], 1);
        cblas_sscal(l + 1, rscale[j], &B[j * ldb], 1);
    }
}
