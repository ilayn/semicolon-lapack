        ...
        if (kase == kase1) {
            ...
            if (lnoti) {
                for (j = 0; j < n - 1; j++) {
                    ...
                    cblas_saxpy(lm, -t, &AB[kd + 1 + j * ldab], 1, &work[j + 1], 1);
                }
            }

            /* Multiply by inv(U) */
            slatbs("U", "N", "N", &normin, n, kl + ku, AB, ldab, work, &scale,
                   &work[2 * n], &linfo);
        } else {
            /* Multiply by inv(U**T) */
            slatbs("U", "T", "N", &normin, n, kl + ku, AB, ldab, work, &scale,
                   &work[2 * n], &linfo);
            ...
            if (lnoti) {
                for (j = n - 2; j >= 0; j--) {
                    ...
                    work[j] -= cblas_sdot(lm, &AB[kd + 1 + j * ldab], 1, &work[j + 1], 1);
                    jp = ipiv[j];
                    if (jp != j) {
                        t = work[jp];
                        work[jp] = work[j];
