/**
 * @file stgsen.c
 * @brief STGSEN reorders the generalized real Schur decomposition.
 */
    ...
    *m = 0;
    ...
    if (*m == n || *m == 0) {
        ...
        if (wantd) {
            ...
            dsum = ONE;
            for (i = 0; i < n; i++) {
                slassq(n, &A[0 + i * lda], 1, &dscale, &dsum);
                slassq(n, &B[0 + i * ldb], 1, &dscale, &dsum);
            }
            dif[0] = dscale * sqrtf(dsum);
            dif[1] = dif[0];
        }
        goto L60;
    }
    ...
    for (k = 0; k < n; k++) {
        if (pair) {
            ...
            if (swap) {
                ...
                kk = k;
                if (k != ks) {
                    stgexc(wantq, wantz, n, A, lda, B, ldb, Q, ldq, Z, ldz,
    ...
    if (wantp) {
        ...
        i = n1;
        ijb = 0;
        slacpy("F", n1, n2, &A[0 + i * lda], lda, work, n1);
        slacpy("F", n1, n2, &B[0 + i * ldb], ldb, &work[n1 * n2], n1);
        stgsyl("N", ijb, n1, n2, A, lda, &A[i + i * lda], lda, work, n1,
        ...
        *pl = rdscal * sqrtf(dsum);
        ...
        rdscal = ZERO;
        dsum = ONE;
        slassq(n1 * n2, &work[n1 * n2], 1, &rdscal, &dsum);
    ...
    if (wantd) {
        ...
        if (wantd1) {
            ...
            stgsyl("N", ijb, n2, n1, &A[i + i * lda], lda, A, lda, work, n2,
            ...
            /* Compute 1-norm-based estimates of Difu and Difl using
               reversed communication with SLACN2. */
            kase = 0;
            n1 = *m;
            n2 = n - *m;
            i = n1;
...
L40:
            slacn2(mn2, &work[mn2], work, iwork, &dif[0], &kase, isave);
            if (kase != 0) {
                if (kase == 1) {
                    /* Solve generalized Sylvester equation. */
                    stgsyl("N", ijb, n1, n2, A, lda, &A[i + i * lda], lda,
                    ...
                    stgsyl("T", ijb, n1, n2, A, lda, &A[i + i * lda], lda,
                           work, n1, B, ldb, &B[i + i * ldb], ldb,
                           &work[n1 * n2], n1, &dscale, &dif[0],
                           &work[2 * n1 * n2], lwork - 2 * n1 * n2,
                           iwork, &ierr);
...
L50:
            slacn2(mn2, &work[mn2], work, iwork, &dif[1], &kase, isave);
            if (kase != 0) {
                if (kase == 1) {
                    /* Solve generalized Sylvester equation. */
                    stgsyl("N", ijb, n2, n1, &A[i + i * lda], lda, A, lda,
                    ...
                    /* Solve the transposed variant. */
                    stgsyl("T", ijb, n2, n1, &A[i + i * lda], lda, A, lda,
                           work, n2, &B[i + i * ldb], ldb, B, ldb,
                           &work[n1 * n2], n2, &dscale, &dif[1],
                           &work[2 * n1 * n2], lwork - 2 * n1 * n2,
                           iwork, &ierr);
    ...
    for (k = 0; k < n; k++) {
        if (pair) {
            ...
            if (pair) {
                ...
                slag2(work, 2, &work[4], 2, smlnum * eps, &beta[k],
