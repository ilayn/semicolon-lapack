            ...
            int* iwork, const int liwork, int* info)
{
    ...
    *info = 0;
    ...
    for (k = 0; k < n; k++) {
        if (pair) {
            ...
            if (swap) {
                 ...
                 * Note: strexc uses 1-based indices for ifst and ilst. */
                ierr = 0;
                kk = k + 1;  /* Convert to 1-based */
                if (k + 1 != ks) {
                    strexc(compq, n, T, ldt, Q, ldq, &kk, &ks, work, &ierr);
                }
                if (ierr == 1 || ierr == 2) {
    ...
    if (wants) {
         ...
         */
        slacpy("F", n1, n2, &T[n1 * ldt], ldt, work, n1);
        strsyl("N", "N", -1, n1, n2, T, ldt, &T[n1 + n1 * ldt],
         ...
         * of eigenvalues.
         */
        rnorm = slange("F", n1, n2, work, n1, NULL);
        if (rnorm == ZERO) {
            *s = ONE;
        } else {
    ...
    if (wantsp) {
        ...
        kase = 0;
        for (;;) {
            slacn2(nn, &work[nn], work, iwork, &est, &kase, isave);
            if (kase == 0)
                break;
            if (kase == 1) {
                /* Solve  T11*R - R*T22 = scale*X. */
                strsyl("N", "N", -1, n1, n2, T, ldt,
                       &T[n1 + n1 * ldt], ldt, work, n1, &scale, &ierr);
            } else {
                /* Solve T11**T*R - R*T22**T = scale*X. */
                strsyl("T", "T", -1, n1, n2, T, ldt,
                       &T[n1 + n1 * ldt], ldt, work, n1, &scale, &ierr);
            }
        }
