/**
 * @file sla_lin_berr.c
 * @brief SLA_LIN_BERR computes a component-wise relative backward error.
 */
 ...
 * @param[in]     RES     Single precision array, dimension (n, nrhs).
    ...
    f32* restrict BERR)
{
    ...
    for (j = 0; j < nrhs; j++) {
        ...
        for (i = 0; i < n; i++) {
            if (AYB[i + j * n] != 0.0f) {
                tmp = (safe1 + fabsf(RES[i + j * n])) / AYB[i + j * n];
                if (BERR[j] < tmp) {
                    BERR[j] = tmp;
                }
            }
