/**
 * @file stplqt2.c
 * @brief STPLQT2 computes a LQ factorization of a real "triangular-pentagonal"
...
/**
 * STPLQT2 computes a LQ factorization of a real "triangular-pentagonal"
             ...
             f32* const restrict T, const int ldt, int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < m; i++) {
        ...
        slarfg(p + 1, &A[i + i * lda], &B[i], ldb, &T[i * ldt]);

        if (i < m - 1) {
            for (j = 0; j < m - i - 1; j++) {
                T[(m - 1) + j * ldt] = A[(i + 1 + j) + i * lda];
            }
            cblas_sgemv(CblasColMajor, CblasNoTrans, m - i - 1, p, ONE,
            ...
            cblas_sger(CblasColMajor, m - i - 1, p, alpha, &T[m - 1], ldt,
                       &B[i], ldb, &B[i + 1], ldb);
        }
    }
    ...
    for (i = 1; i < m; i++) {
        ...
        if (i - p > 0) {
            cblas_sgemv(CblasColMajor, CblasNoTrans, i - p, l, alpha,
                        &B[(mp - 1) + (np - 1) * ldb], ldb, &B[i + (np - 1) * ldb], ldb, ZERO,
                        &T[i + (mp - 1) * ldt], ldt);
        }

        if (n - l > 0) {
            cblas_sgemv(CblasColMajor, CblasNoTrans, i, n - l, alpha,
