/**
 * @file slasd0.c
 * @brief SLASD0 computes the singular values of a real upper bidiagonal
 ...
 */
void slasd0(const int n, const int sqre, float* const restrict D,
            float* const restrict E, float* const restrict U, const int ldu,
            float* const restrict VT, const int ldvt, const int smlsiz,
            int* const restrict IWORK, float* const restrict work, int* info)
{
    ...
    *info = 0;
    ...
    if (n <= smlsiz) {
        slasdq("U", sqre, n, m, n, 0, D, E, VT, ldvt, U, ldu, NULL, 1, work, info);
        return;
    }
    ...
    for (lvl = nlvl; lvl >= 1; lvl--) {
        ...
        for (i = lf; i <= ll; i++) {
            ...
            slasd1(nl, nr, sqrei, &D[nlf], &alpha, &beta,
                   &U[nlf + nlf * ldu], ldu,
                   &VT[nlf + nlf * ldvt], ldvt,
