/**
 * @file sgbtrf.c
 * @brief SGBTRF computes the LU factorization of a general band matrix
    ...
    if (nb <= 1 || nb > kl) {
        ...
        for (j = 0; j < minmn; j += nb) {
            ...
            for (jj = j; jj < j + jb; jj++) {
                ...
                jp = cblas_isamax(km + 1, &AB[kv + jj * ldab], 1);
                ...
                if (AB[kv + jp + jj * ldab] != ZERO) {
                    ...
                    if (jp != 0) {
                        ...
                        if (jp + jj < j + kl) {
                            ...
                            cblas_sswap(jb, &AB[kv + jj - j + j * ldab], ldab - 1,
                                        &AB[kv + jp + jj - j + j * ldab], ldab - 1);
                        } else {
                            /* The interchange affects columns j to jj-1 of A31
                               which are stored in the work array work31 */
                            cblas_sswap(jj - j, &AB[kv + jj - j + j * ldab], ldab - 1,
                                        &work31[jp + jj - j - kl + 0 * LDWORK], LDWORK);
                            cblas_sswap(j + jb - jj, &AB[kv + jj * ldab], ldab - 1,
                                        &AB[kv + jp + jj * ldab], ldab - 1);
                        }
                    }

                    /* Compute multipliers */
                    cblas_sscal(km, ONE / AB[kv + jj * ldab], &AB[kv + 1 + jj * ldab], 1);
                    ...
                    if (jm > jj) {
                        cblas_sger(CblasColMajor, km, jm - jj, NEG_ONE,
                                   &AB[kv + 1 + jj * ldab], 1,
                ...
                if (nw > 0) {
                    cblas_scopy(nw, &AB[kv + kl - jj + j + jj * ldab], 1,
            ...
            if (j + jb < n) {
                ...
                /* Use slaswp to apply the row interchanges to A12, A22, and A32 */
                slaswp(j2, &AB[kv - jb + (j + jb) * ldab], ldab - 1, 0, jb - 1, &ipiv[j], 1);
            ...
            for (jj = j + jb - 1; jj >= j; jj--) {
                ...
                if (nw > 0) {
                    cblas_scopy(nw, &work31[0 + (jj - j) * LDWORK], 1,
