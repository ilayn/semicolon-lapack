/**
 * @file sgbtf2.c
 * @brief SGBTF2 computes the LU factorization of a general band matrix
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    for (j = 0; j < minmn; j++) {
         ...
         */
        jp = cblas_isamax(km + 1, &AB[kv + j * ldab], 1);
        ...
        if (AB[kv + jp + j * ldab] != ZERO) {
            ...
            if (jp != 0) {
                cblas_sswap(ju - j + 1, &AB[kv + jp + j * ldab], ldab - 1,
                            &AB[kv + j * ldab], ldab - 1);
            }
            ...
            if (km > 0) {
                ...
                cblas_sscal(km, ONE / AB[kv + j * ldab], &AB[kv + 1 + j * ldab], 1);
                ...
                if (ju > j) {
                    cblas_sger(CblasColMajor, km, ju - j, -ONE,
