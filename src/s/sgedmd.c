/**
 * @file sgedmd.c
 * @brief SGEDMD computes the Dynamic Mode Decomposition (DMD) for a pair of
...
/**
 * SGEDMD computes the Dynamic Mode Decomposition (DMD) for a pair of data
 * snapshot matrices. For the input matrices X and Y such that Y = A*X with an
 * unaccessible matrix A, SGEDMD computes a certain number of Ritz pairs of A
 ...
 * SGEDMD returns the residuals of the computed Ritz pairs, the information
 ...
 * @param[in,out] Y       Single precision M-by-N array. On entry the data
 ...
 */
void sgedmd(const char* jobs, const char* jobz, const char* jobr,
            const char* jobf, const INT whtsvd, const INT m, const INT n,
            f32* restrict X, const INT ldx, f32* restrict Y, const INT ldy,
            const INT nrnk, const f32 tol, INT* k, f32* restrict reig,
            f32* restrict imeig, f32* restrict Z, const INT ldz,
            f32* restrict res, f32* restrict B, const INT ldb,
            f32* restrict W, const INT ldw, f32* restrict S, const INT lds,
            f32* restrict work, const INT lwork, INT* restrict iwork,
    ...
    *info  = 0;
    ...
    if (*info == 0) {
        ...
        case 1:
            ...
            if (lquery) {
                sgesvd("O", "S", m, n, X, ldx, work, B, ldb, W, ldw,
                       rdummy, -1, &info1);
        ...
        case 2:
            ...
            iminwr = 8 * imin2(m, n);
            if (lquery) {
                sgesdd("O", m, n, X, ldx, work, B, ldb, W, ldw,
            ...
            break;
        case 3:
            sgesvdq("H", "P", "N", "R", "R", m, n, X, ldx, work, Z, ldz,
        ...
        if (lquery) {
            sgeev("N", jobzl, n, S, lds, reig, imeig, W, ldw, W, ldw,
                  rdummy, -1, &info1);
    ...
    if (sccolx) {
        ...
        *k = 0;
        for (i = 0; i < n; i++) {
            ...
            if ((scale != ZERO) && (ssum != ZERO)) {
                ...
                if (scale >= tbig) {
                    ...
                    slascl("G", 0, 0, scale, ONE / rootsc, m, 1, &X[i * ldx], m, &info2);
                    work[i] = -scale * (rootsc / (f32)m);
                } else {
                    /* X(:,i) will be scaled to unit 2-norm */
                    work[i] = scale * rootsc;
                    slascl("G", 0, 0, work[i], ONE, m, 1, &X[i * ldx], m, &info2);
                }
            } else {
                work[i] = ZERO;
                *k = *k + 1;
        ...
        for (i = 0; i < n; i++) {
            ...
            if (work[i] > ZERO) {
                cblas_sscal(m, ONE / work[i], &Y[i * ldy], 1);
            } else if (work[i] < ZERO) {
                slascl("G", 0, 0, -work[i], ONE / (f32)m, m, 1, &Y[i * ldy], m, &info2);
            } else if (Y[cblas_isamax(m, &Y[i * ldy], 1) + i * ldy] != ZERO) {
                /* X(:,i) is zero vector. For consistency, Y(:,i) should also
                 * be zero. */
                badxy = 1;
                if (jobs[0] == 'C' || jobs[0] == 'c')
                    cblas_sscal(m, ZERO, &Y[i * ldy], 1);
            }
        }
    }
    ...
    if (sccoly) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (scale != ZERO && (ssum != ZERO)) {
                ...
                if (scale >= tbig) {
                    ...
                    slascl("G", 0, 0, scale, ONE / rootsc, m, 1, &Y[i * ldy], m, &info2);
                    work[i] = -scale * (rootsc / (f32)m);
                } else {
                    /* Y(:,i) will be scaled to unit 2-norm */
                    work[i] = scale * rootsc;
                    slascl("G", 0, 0, work[i], ONE, m, 1, &Y[i * ldy], m, &info2);
                }
            } else {
                work[i] = ZERO;
            }
        ...
        for (i = 0; i < n; i++) {
            ...
            if (work[i] > ZERO) {
                cblas_sscal(m, ONE / work[i], &X[i * ldx], 1);
            } else if (work[i] < ZERO) {
                slascl("G", 0, 0, -work[i], ONE / (f32)m, m, 1, &X[i * ldx], m, &info2);
            } else if (X[cblas_isamax(m, &X[i * ldx], 1) + i * ldx] != ZERO) {
                /* Y(:,i) is zero vector. */
                badxy = 1;
            }
        }
    ...
    case 2:
        ...
        break;
    case 3:
        sgesvdq("H", "P", "N", "R", "R", m, n, X, ldx, work, Z, ldz, W, ldw,
                &numrnk, iwork, liwork, &work[n + imax2(2, m)],
                lwork - n - imax2(2, m), &work[n], imax2(2, m), &info1);
        slacpy("A", m, numrnk, Z, ldz, X, ldx);
        t_or_n = 'T';
        break;
    case 4:
        sgejsv("F", "U", jsvopt, "N", "N", "P", m, n, X, ldx, work, Z, ldz,
               W, ldw, &work[n], lwork - n, iwork, &info1);
        slacpy("A", m, n, Z, ldz, X, ldx);
        t_or_n = 'N';
        xscl1 = work[n];
        xscl2 = work[n + 1];
        if (xscl1 != xscl2) {
            /* Exceptional situation: SGEJSV returned the SVD in scaled form. */
            slascl("G", 0, 0, xscl1, xscl2, m, n, Y, ldy, &info2);
        }
        break;
    default:
        t_or_n = 'T';
    ...
    if (t_or_n == 'N') {
        for (i = 0; i < *k; i++) {
            cblas_sscal(n, ONE / work[i], &W[i * ldw], 1);
        }
    } else {
        for (i = 0; i < *k; i++) {
            work[n + i] = ONE / work[i];
    ...
    if (wntref) {
        ...
        if (wntres || wntex) {
            if (t_or_n == 'N') {
                slacpy("A", n, *k, W, ldw, Z, ldz);
            } else {
                slacpy("A", *k, n, W, ldw, Z, ldz);
            }
        }
    }
    ...
    sgeev("N", jobzl, *k, S, lds, reig, imeig, W, ldw, W, ldw,
          &work[n], lwork - n, &info1);

    if (info1 > 0) {
        /* SGEEV failed to compute the eigenvalues and eigenvectors. */
        *info = 3;
        return;
    }
    ...
    if (wntvec || wntex) {
        if (wntres) {
            if (wntref) {
                ...
                slacpy("A", m, *k, Z, ldz, Y, ldy);
                if (wntex) slacpy("A", m, *k, Z, ldz, B, ldb);
            }
        } else if (wntex) {
            cblas_sgemm(CblasColMajor,
                        (t_or_n == 'T') ? CblasTrans : CblasNoTrans,
        ...
        if (wntres) {
            ...
            while (i < *k) {
                if (imeig[i] == ZERO) {
                    ...
                    cblas_saxpy(m, -reig[i], &Z[i * ldz], 1, &Y[i * ldy], 1);
                    res[i] = cblas_snrm2(m, &Y[i * ldy], 1);
                    i = i + 1;
                } else {
                    ...
                    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                ...
                                &Y[i * ldy], ldy);
                    res[i] = slange("F", m, 2, &Y[i * ldy], ldy, &work[n]);
                    res[i + 1] = res[i];
                    i = i + 2;
                }
            }
