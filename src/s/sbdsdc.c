/**
 * @file sbdsdc.c
 * @brief SBDSDC computes the singular value decomposition (SVD) of a real
            ...
            f32* restrict work, INT* restrict IWORK, INT* info)
{
    ...
    *info = 0;
    ...
    /* smlsiz = 25 from ilaenv(9, 'SBDSDC', ...) */
    if (n == 1) {
        if (icompq == 1) {
    ...
    if (icompq == 1) {
        cblas_scopy(n, D, 1, Q, 1);
        cblas_scopy(n - 1, E, 1, &Q[n], 1);
    }
    ...
    if (icompq == 0) {
        ...
        slasdq("U", 0, n, 0, 0, 0, D, E, NULL, 1, NULL, 1, NULL, 1, work, info);
        goto L40;
    }
    ...
    if (n <= SMLSIZ) {
        if (icompq == 2) {
            ...
            slasdq("U", 0, n, n, n, 0, D, E, VT, ldvt, U, ldu, NULL, 1,
                   &work[wstart], info);
        } else if (icompq == 1) {
            iu = 0;           /* 0-based */
            ivt = iu + n;
             ...
             */
            slaset("A", n, n, ZERO, ONE, &Q[iu + qstart * n], n);
            slaset("A", n, n, ZERO, ONE, &Q[ivt + qstart * n], n);
            slasdq("U", 0, n, n, n, 0, D, E,
                   &Q[ivt + qstart * n], n,
                   &Q[iu + qstart * n], n,
                   NULL, 1,
                   &work[wstart], info);
    ...
    for (i = 0; i < nm1; i++) {
        if (fabsf(E[i]) < eps || i == nm1 - 1) {
            ...
            if (icompq == 2) {
                ...
                /* Call slasda for compact form. */
                slasda(icompq, SMLSIZ, nsize, sqre, &D[start], &E[start],
...
L40:
    ...
    if (iuplo == 2 && icompq == 2) {
        slasr("L", "V", "B", n, n, work, &work[n - 1], U, ldu);
    }
}
