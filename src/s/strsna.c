    ...
    for (k = 0; k < n; k++) {
        ...
        if (wants) {
            ...
            if (!pair) {
                ...
                /* Real eigenvalue. */
                prod = cblas_sdot(n, &VR[ks * ldvr], 1, &VL[ks * ldvl], 1);
                rnrm = cblas_snrm2(n, &VR[ks * ldvr], 1);
                lnrm = cblas_snrm2(n, &VL[ks * ldvl], 1);
                S[ks] = fabsf(prod) / (rnrm * lnrm);

            } else {

                /* Complex eigenvalue. */
                prod1 = cblas_sdot(n, &VR[ks * ldvr], 1, &VL[ks * ldvl], 1);
                prod1 = prod1 + cblas_sdot(n, &VR[(ks + 1) * ldvr], 1,
                                           &VL[(ks + 1) * ldvl], 1);
                prod2 = cblas_sdot(n, &VL[ks * ldvl], 1, &VR[(ks + 1) * ldvr], 1);
                prod2 = prod2 - cblas_sdot(n, &VL[(ks + 1) * ldvl], 1,
                                           &VR[ks * ldvr], 1);
                rnrm = slapy2(cblas_snrm2(n, &VR[ks * ldvr], 1),
                              cblas_snrm2(n, &VR[(ks + 1) * ldvr], 1));
                lnrm = slapy2(cblas_snrm2(n, &VL[ks * ldvl], 1),
                              cblas_snrm2(n, &VL[(ks + 1) * ldvl], 1));
        ...
        if (wantsp) {
            ...
            if (ierr == 1 || ierr == 2) {
                ...
                if (work[1] == ZERO) {
                    ...
                    mu = sqrtf(fabsf(work[1 * ldwork])) * sqrtf(fabsf(work[1]));
                    delta = slapy2(mu, work[1]);
                    cs = mu / delta;
                    sn = -work[1] / delta;
                ...
                kase = 0;
                for (;;) {
                    slacn2(nn, &work[(n + 1) * ldwork], &work[(n + 3) * ldwork],
                    ...
                    if (kase == 1) {
                        ...
                        if (n2 == 1) {
                            ...
                            slaqtr(0, 1, n - 1, &work[1 + ldwork],
                                   ldwork, dummy, dumm, &scale,
                                   &work[(n + 3) * ldwork], &work[(n + 5) * ldwork], &ierr);
                        } else {
                            /* Complex eigenvalue: solve
                             * C*(p+iq) = scale*(c+id) in real arithmetic. */
                            slaqtr(0, 0, n - 1, &work[1 + ldwork],
                                   ldwork, &work[n * ldwork], mu, &scale,
                                   &work[(n + 3) * ldwork], &work[(n + 5) * ldwork], &ierr);
                        }
                    }
