/**
 * @file sgemlq.c
 * @brief SGEMLQ overwrites matrix C with Q*C, Q^T*C, C*Q, or C*Q^T
 *        where Q is defined by an LQ factorization from SGELQ.
 */
 ...
 * elementary reflectors computed by short wide LQ factorization (SGELQ)
 *
            ...
            INT* info)
{
    ...
    /* Read block sizes from T array (stored by SGELQ) */
    mb = (INT)T[1];
    nb = (INT)T[2];
