/**
 * @file ssytrs2.c
 * @brief SSYTRS2 solves a system of linear equations A*X = B with a real
 *        symmetric matrix A using the factorization computed by SSYTRF
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (i >= 0) {
            if (ipiv[i] >= 0) {
                cblas_sscal(nrhs, ONE / A[i + i * lda], &B[i], ldb);
            } else if (i > 0) {
                if (ipiv[i - 1] == ipiv[i]) {
                    akm1k = work[i];
