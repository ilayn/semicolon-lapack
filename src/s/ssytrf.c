/**
 * @file ssytrf.c
 * @brief SSYTRF computes the factorization of a real symmetric matrix
    ...
    if (upper) {
        ...
        while (k < n) {
            if (k <= n - 1 - nb) {
                 ...
                 * (since iinfo is already 1-based from slasyf, and k is the
                 * 0-based offset, the global 1-based index = iinfo + k). */
                if (*info == 0 && iinfo > 0) {
                    *info = iinfo + k;
                }

                /* Adjust IPIV: slasyf returns local indices for the submatrix
