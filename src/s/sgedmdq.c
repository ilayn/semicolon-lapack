/**
 * @file sgedmdq.c
 * @brief SGEDMDQ computes the Dynamic Mode Decomposition (DMD) for a pair of
...
/**
 * SGEDMDQ computes the Dynamic Mode Decomposition (DMD) for a pair of data
 * snapshot matrices, using a QR factorization based compression of the data.
 * For the input matrix that contains a sequence of snapshots, SGEDMDQ computes
 ...
 *                        contains the scalar factors from sgeqrf, and
 ...
 *                              converge.
 *                        = 4 : data inconsistency warning (see sgedmd).
 */
void sgedmdq(const char* jobs, const char* jobz, const char* jobr,
             const char* jobq, const char* jobt, const char* jobf,
             const INT whtsvd, const INT m, const INT n, f32* restrict F,
             const INT ldf, f32* restrict X, const INT ldx, f32* restrict Y,
             const INT ldy, const INT nrnk, const f32 tol, INT* k,
             f32* restrict reig, f32* restrict imeig, f32* restrict Z,
             const INT ldz, f32* restrict res, f32* restrict B, const INT ldb,
             f32* restrict V, const INT ldv, f32* restrict S, const INT lds,
             ...
             const INT liwork, INT* info)
{
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        mlwqr = imax2(1, n);  /* Minimal workspace length for SGEQRF. */
        mlwork = minmn + mlwqr;
        if (lquery) {
            sgeqrf(m, n, F, ldf, work, rdummy, -1, &info1);
            olwqr = (INT)rdummy[0];
            olwork = imin2(m, n) + olwqr;
        }
        sgedmd(jobs, jobvl, jobr, jobf, whtsvd, minmn, n - 1, X, ldx, Y, ldy,
        ...
        if (wntvec || wntvcf) {
            ...
            if (lquery) {
                sormqr("L", "N", m, n, minmn, F, ldf, NULL, Z, ldz, work, -1, &info1);
                olwmqr = (INT)work[0];
        ...
        if (wantq) {
            ...
            if (lquery) {
                sorgqr(m, minmn, minmn, F, ldf, NULL, work, -1, &info1);
                olwgqr = (INT)work[0];
    ...
    sgeqrf(m, n, F, ldf, work, &work[minmn], lwork - minmn, &info1);

    /* Define X and Y as the snapshots representations in the orthogonal basis
     * computed in the QR factorization. */
    slaset("L", minmn, n - 1, ZERO, ZERO, X, ldx);
    slacpy("U", minmn, n - 1, F, ldf, X, ldx);
    slacpy("A", minmn, n - 1, &F[ldf], ldf, Y, ldy);
    if (m >= 3) {
        slaset("L", minmn - 2, n - 2, ZERO, ZERO, &Y[2], ldy);
    }

    /* Compute the DMD of the projected snapshot pairs (X,Y) */
    sgedmd(jobs, jobvl, jobr, jobf, whtsvd, minmn, n - 1, X, ldx, Y, ldy, nrnk,
           tol, k, reig, imeig, Z, ldz, res, B, ldb, V, ldv, S, lds,
           &work[minmn], lwork - minmn, iwork, liwork, &info1);
    if (info1 == 2 || info1 == 3) {
        /* Return with error code. See SGEDMD for details. */
        *info = info1;
        return;
    } else {
        *info = info1;
    ...
    if (wntvec) {
        ...
        if (m > minmn)
            slaset("A", m - minmn, *k, ZERO, ZERO, &Z[minmn], ldz);
        sormqr("L", "N", m, *k, minmn, F, ldf, work, Z, ldz,
               &work[minmn + n - 1], lwork - (minmn + n - 1), &info1);
    } else if (wntvcf) {
        /* Return the Ritz vectors (eigenvectors) in factored form Z*V. */
        slacpy("A", n, *k, X, ldx, Z, ldz);
        if (m > n)
            slaset("A", m - n, *k, ZERO, ZERO, &Z[n], ldz);
        sormqr("L", "N", m, *k, minmn, F, ldf, work, Z, ldz,
               &work[minmn + n - 1], lwork - (minmn + n - 1), &info1);
    }
    ...
    if (wnttrf) {
        slaset("A", minmn, n, ZERO, ZERO, Y, ldy);
        slacpy("U", minmn, n, F, ldf, Y, ldy);
    }

    /* The orthonormal/orthogonal factor Q in the initial QR factorization is
     * optionally returned in the array F. */
    if (wantq) {
        sorgqr(m, minmn, minmn, F, ldf, work, &work[minmn + n - 1],
