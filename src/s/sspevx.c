/**
 * @file sspevx.c
 * @brief SSPEVX computes selected eigenvalues and, optionally, eigenvectors
...
void sspevx(const char* jobz, const char* range, const char* uplo,
            const int n, f32* const restrict AP,
            const f32 vl, const f32 vu, const int il, const int iu,
            const f32 abstol, int* m, f32* const restrict W,
    ...
    *m = 0;
    ...
    if (wantz) {
...

        {
            const char* side = "L";
            const char* trans = "N";
            sopmtr(side, uplo, trans, n, *m, AP, &work[indtau], Z, ldz,
                   &work[indwrk], &iinfo);
        }
    }
