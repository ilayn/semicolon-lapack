/**
 * @file slasy2.c
 * @brief SLASY2 solves the Sylvester matrix equation where the matrices are
 *        of order 1 or 2.
 */
            ...
            f32* xnorm, int* info)
{
    ...
    *info = 0;
    ...
    if (k == 2 || k == 3) {
        ...
        *scale = ONE;
        if ((TWO * smlnum) * fabsf(btmp[1]) > fabsf(u22) ||
            (TWO * smlnum) * fabsf(btmp[0]) > fabsf(u11)) {
            *scale = HALF / (fabsf(btmp[0]) > fabsf(btmp[1]) ? fabsf(btmp[0]) : fabsf(btmp[1]));
            btmp[0] = btmp[0] * (*scale);
            btmp[1] = btmp[1] * (*scale);
        }
        x2[1] = btmp[1] / u22;
        ...
        if (n1 == 1) {
            ...
            *xnorm = fabsf(X[0]) + fabsf(X[ldx]);
            ...
            X[1] = x2[1];
            *xnorm = fabsf(X[0]) > fabsf(X[1]) ? fabsf(X[0]) : fabsf(X[1]);
        }
        return;
    }
    ...
    for (i = 0; i < 3; i++) {
        ...
        for (ip = i; ip < 4; ip++) {
            for (jp = i; jp < 4; jp++) {
                if (fabsf(t16[ip + 4*jp]) >= xmax) {
                    xmax = fabsf(t16[ip + 4*jp]);
                    ipsv = ip;
                    jpsv = jp;
                }
            }
        ...
        if (ipsv != i) {
            ...
            cblas_sswap(4, &t16[ipsv], 4, &t16[i], 4);
            temp = btmp[i];
            btmp[i] = btmp[ipsv];
            btmp[ipsv] = temp;
        }
        if (jpsv != i) {
            ...
            cblas_sswap(4, &t16[4*jpsv], 1, &t16[4*i], 1);
        }
        jpiv[i] = jpsv;
        if (fabsf(t16[i + 4*i]) < smin) {
            *info = 1;
            t16[i + 4*i] = smin;
        }
        for (j = i + 1; j < 4; j++) {
    ...
    if (fabsf(t16[3 + 4*3]) < smin) {
        *info = 1;
        t16[3 + 4*3] = smin;
    }

    *scale = ONE;
    if ((EIGHT * smlnum) * fabsf(btmp[0]) > fabsf(t16[0 + 4*0]) ||
        (EIGHT * smlnum) * fabsf(btmp[1]) > fabsf(t16[1 + 4*1]) ||
        (EIGHT * smlnum) * fabsf(btmp[2]) > fabsf(t16[2 + 4*2]) ||
        (EIGHT * smlnum) * fabsf(btmp[3]) > fabsf(t16[3 + 4*3])) {
        xmax = fabsf(btmp[0]);
        if (fabsf(btmp[1]) > xmax) xmax = fabsf(btmp[1]);
        if (fabsf(btmp[2]) > xmax) xmax = fabsf(btmp[2]);
        if (fabsf(btmp[3]) > xmax) xmax = fabsf(btmp[3]);
    ...
    X[ldx] = tmp[2];
    X[1 + ldx] = tmp[3];
    *xnorm = fabsf(tmp[0]) + fabsf(tmp[2]);
    temp = fabsf(tmp[1]) + fabsf(tmp[3]);
