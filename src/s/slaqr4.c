/**
 * @file slaqr4.c
 * @brief SLAQR4 computes eigenvalues of a Hessenberg matrix using small-bulge
 ...
 * @param[out] info
 *                         - = 0: successful exit
 *                         - > 0: if info = i, SLAQR4 failed to compute all of the
    ...
    *info = 0;
    ...
    if (n <= ntiny) {
        ...
        *info = 0;
        ...
        slaqr2(wantt, wantz, n, ilo, ihi, nwr + 1, H, ldh, iloz,
        ...
        for (it = 1; it <= itmax; it++) {
            ...
            if ((ld == 0) || ((100 * ld <= nw * nibble) &&
                 ...
                 * shifts provided by SLAQR2 above or from the eigenvalues
                ...
                if ((ndfl % kexsh) == 0) {
                    ...
                    /* Got NS/2 or fewer shifts? Use SLAHQR on a trailing
                     * principal submatrix to get more. (Since NS <= NSMAX
                     * <= (N-3)/6, there is enough space below the subdiagonal
