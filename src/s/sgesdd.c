/**
 * @file sgesdd.c
 * @brief SGESDD computes the singular value decomposition (SVD) of a real
            ...
            int* const restrict IWORK, int* info)
{
    ...
    float dum[1];   /* For slange work and sbdsdc Q when not used */
    float dum1[1];  /* For workspace query results */
    int idum[1];     /* For sbdsdc IQ when not used */
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (m >= n && minmn > 0) {
            ...
            sgebrd(m, n, NULL, m, NULL, NULL, NULL, NULL, dum1, -1, &ierr);
            lwork_dgebrd_mn = (int)dum1[0];

            sgebrd(n, n, NULL, n, NULL, NULL, NULL, NULL, dum1, -1, &ierr);
            lwork_dgebrd_nn = (int)dum1[0];

            sgeqrf(m, n, NULL, m, NULL, dum1, -1, &ierr);
            lwork_dgeqrf_mn = (int)dum1[0];

            sorgbr("Q", n, n, n, NULL, n, NULL, dum1, -1, &ierr);

            sorgqr(m, m, n, NULL, m, NULL, dum1, -1, &ierr);
            lwork_dorgqr_mm = (int)dum1[0];

            sorgqr(m, n, n, NULL, m, NULL, dum1, -1, &ierr);
            lwork_dorgqr_mn = (int)dum1[0];

            sormbr("P", "R", "T", n, n, n, NULL, n, NULL, NULL, n, dum1, -1, &ierr);
            lwork_dormbr_prt_nn = (int)dum1[0];

            sormbr("Q", "L", "N", n, n, n, NULL, n, NULL, NULL, n, dum1, -1, &ierr);
            lwork_dormbr_qln_nn = (int)dum1[0];

            sormbr("Q", "L", "N", m, n, n, NULL, m, NULL, NULL, m, dum1, -1, &ierr);
            lwork_dormbr_qln_mn = (int)dum1[0];

            sormbr("Q", "L", "N", m, m, n, NULL, m, NULL, NULL, m, dum1, -1, &ierr);
            ...
            sgebrd(m, m, NULL, m, NULL, NULL, NULL, NULL, dum1, -1, &ierr);
            lwork_dgebrd_mm = (int)dum1[0];

            sgelqf(m, n, NULL, m, NULL, dum1, -1, &ierr);
            lwork_dgelqf_mn = (int)dum1[0];

            sorglq(n, n, m, NULL, n, NULL, dum1, -1, &ierr);
            lwork_dorglq_nn = (int)dum1[0];

            sorglq(m, n, m, NULL, m, NULL, dum1, -1, &ierr);
            lwork_dorglq_mn = (int)dum1[0];

            sorgbr("P", m, m, m, NULL, n, NULL, dum1, -1, &ierr);

            sormbr("P", "R", "T", m, m, m, NULL, m, NULL, NULL, m, dum1, -1, &ierr);
            lwork_dormbr_prt_mm = (int)dum1[0];

            sormbr("P", "R", "T", m, n, m, NULL, m, NULL, NULL, m, dum1, -1, &ierr);
            lwork_dormbr_prt_mn = (int)dum1[0];

            sormbr("P", "R", "T", n, n, m, NULL, n, NULL, NULL, n, dum1, -1, &ierr);
            lwork_dormbr_prt_nn = (int)dum1[0];

            sormbr("Q", "L", "N", m, m, m, NULL, m, NULL, NULL, m, dum1, -1, &ierr);
            lwork_dormbr_qln_mm = (int)dum1[0];
    ...
    if (m >= n) {
        ...
        if (m >= mnthr) {
            if (wntqn) {
                ...
                sgeqrf(m, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &ierr);
                ...
                sbdsdc("U", "N", n, S, &work[ie], NULL, 1, NULL, 1, NULL, NULL,
                ...
                sorgqr(m, n, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &ierr);
                ...
                sbdsdc("U", "I", n, S, &work[ie], &work[iu], n, VT, ldvt, dum, idum,
                       &work[nwork], IWORK, info);

                sormbr("Q", "L", "N", n, n, n, &work[ir], ldwrkr, &work[itauq],
                       &work[iu], n, &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", n, n, n, &work[ir], ldwrkr, &work[itaup],
                ...
                for (i = 0; i < m; i += ldwrkr) {
                    ...
                    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                ...
                                ZERO, &work[ir], ldwrkr);
                    slacpy("F", chunk, n, &work[ir], ldwrkr, &A[i], lda);
                }
            } else if (wntqs) {
                /* Path 3 (M >> N, JOBZ='S') */
                ir = 0;
                ...
                sbdsdc("U", "I", n, S, &work[ie], U, ldu, VT, ldvt, dum, idum,
                       &work[nwork], IWORK, info);
                ...
                sorgqr(m, m, n, U, ldu, &work[itau], &work[nwork], lwork - nwork, &ierr);
                ...
                sormbr("Q", "L", "N", n, n, n, A, lda, &work[itauq], &work[iu],
                       ldwrku, &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", n, n, n, A, lda, &work[itaup], VT, ldvt,
                       &work[nwork], lwork - nwork, &ierr);
            ...
            if (wntqn) {
                ...
                sbdsdc("U", "I", n, S, &work[ie], &work[iu], ldwrku, VT, ldvt,
                ...
                if (lwork >= m * n + 3 * n + bdspac) {
                    sormbr("Q", "L", "N", m, n, n, A, lda, &work[itauq],
                           &work[iu], ldwrku, &work[nwork], lwork - nwork, &ierr);
                    slacpy("F", m, n, &work[iu], ldwrku, A, lda);
                } else {
                    sorgbr("Q", m, n, n, A, lda, &work[itauq], &work[nwork],
                           lwork - nwork, &ierr);
                    for (i = 0; i < m; i += ldwrkr) {
                        chunk = ((m - i) < ldwrkr) ? (m - i) : ldwrkr;
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                ...
                sormbr("Q", "L", "N", m, n, n, A, lda, &work[itauq], U, ldu,
                       &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", n, n, n, A, lda, &work[itaup], VT, ldvt,
                       &work[nwork], lwork - nwork, &ierr);
            } else if (wntqa) {
                ...
                if (m > n) {
                    slaset("F", m - n, m - n, ZERO, ONE, &U[n + n * ldu], ldu);
                }

                sormbr("Q", "L", "N", m, m, n, A, lda, &work[itauq], U, ldu,
                       &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", n, n, m, A, lda, &work[itaup], VT, ldvt,
                       &work[nwork], lwork - nwork, &ierr);
            }
        }
    } else {
        ...
        if (n >= mnthr) {
            if (wntqn) {
                ...
                sgelqf(m, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &ierr);
                ...
                sbdsdc("U", "N", m, S, &work[ie], NULL, 1, NULL, 1, NULL, NULL,
                       &work[nwork], IWORK, info);
            } else if (wntqo) {
                /* Path 2t (N >> M, JOBZ='O') */
                ivt = 0;
                ...
                sorglq(m, n, m, A, lda, &work[itau], &work[nwork], lwork - nwork, &ierr);
                ...
                sgebrd(m, m, &work[il], ldwrkl, S, &work[ie], &work[itauq],
                       &work[itaup], &work[nwork], lwork - nwork, &ierr);

                sbdsdc("U", "I", m, S, &work[ie], U, ldu, &work[ivt], m, dum, idum,
                       &work[nwork], IWORK, info);

                sormbr("Q", "L", "N", m, m, m, &work[il], ldwrkl, &work[itauq],
                       U, ldu, &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", m, m, m, &work[il], ldwrkl, &work[itaup],
                ...
                for (i = 0; i < n; i += chunk) {
                    ...
                    cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                ...
                                ZERO, &work[il], ldwrkl);
                    slacpy("F", m, blk, &work[il], ldwrkl, &A[i * lda], lda);
                }
            } else if (wntqs) {
                /* Path 3t (N >> M, JOBZ='S') */
                il = 0;
                ...
                sbdsdc("U", "I", m, S, &work[ie], U, ldu, VT, ldvt, dum, idum,
                       &work[nwork], IWORK, info);
                ...
                sorglq(n, n, m, VT, ldvt, &work[itau], &work[nwork], lwork - nwork, &ierr);
                ...
                sbdsdc("U", "I", m, S, &work[ie], U, ldu, &work[ivt], ldwkvt,
                       dum, idum, &work[nwork], IWORK, info);

                sormbr("Q", "L", "N", m, m, m, A, lda, &work[itauq], U, ldu,
                       &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", m, m, m, A, lda, &work[itaup], &work[ivt],
            ...
            if (wntqn) {
                sbdsdc("L", "N", m, S, &work[ie], NULL, 1, NULL, 1, NULL, NULL,
                       &work[nwork], IWORK, info);
            } else if (wntqo) {
                ldwkvt = m;
                ivt = nwork;
                ...
                sbdsdc("L", "I", m, S, &work[ie], U, ldu, &work[ivt], ldwkvt,
                       dum, idum, &work[nwork], IWORK, info);
                ...
                if (lwork >= m * n + 3 * m + bdspac) {
                    sormbr("P", "R", "T", m, n, m, A, lda, &work[itaup],
                           &work[ivt], ldwkvt, &work[nwork], lwork - nwork, &ierr);
                    slacpy("F", m, n, &work[ivt], ldwkvt, A, lda);
                } else {
                    sorgbr("P", m, n, m, A, lda, &work[itaup], &work[nwork],
                           lwork - nwork, &ierr);
                    for (i = 0; i < n; i += chunk) {
                        blk = ((n - i) < chunk) ? (n - i) : chunk;
                        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                    ...
                                    ZERO, &work[il], m);
                        slacpy("F", m, blk, &work[il], m, &A[i * lda], lda);
                    }
                }
            } else if (wntqs) {
                slaset("F", m, n, ZERO, ZERO, VT, ldvt);
                sbdsdc("L", "I", m, S, &work[ie], U, ldu, VT, ldvt, dum, idum,
                       &work[nwork], IWORK, info);

                sormbr("Q", "L", "N", m, m, n, A, lda, &work[itauq], U, ldu,
                       &work[nwork], lwork - nwork, &ierr);
                sormbr("P", "R", "T", m, n, m, A, lda, &work[itaup], VT, ldvt,
                       &work[nwork], lwork - nwork, &ierr);
            } else if (wntqa) {
                ...
                if (n > m) {
                    slaset("F", n - m, n - m, ZERO, ONE, &VT[m + m * ldvt], ldvt);
                }
