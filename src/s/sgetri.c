 ...
 *                      The pivot indices from sgetrf; row i was interchanged
 *                      with row `ipiv[i]`.
    ...
    *info = 0;
    ...
    if (nb < nbmin || nb >= n) {
        ...
        for (j = n - 1; j >= 0; j--) {
            ...
            if (j < n - 1) {
                cblas_sgemv(CblasColMajor, CblasNoTrans, n, n - j - 1, NEG_ONE,
        ...
        for (j = nn; j >= 0; j -= nb) {
            ...
            cblas_strsm(CblasColMajor, CblasRight, CblasLower, CblasNoTrans, CblasUnit,
