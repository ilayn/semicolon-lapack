/**
 * @file slasyf.c
 * @brief SLASYF computes a partial factorization of a real symmetric matrix
 ...
 *                      leading `n`-by-`n` upper triangular part contains
 *                      the upper triangular part. If `uplo='L'`, the
 *                      leading `n`-by-`n` lower triangular part contains
 ...
 *                      are set.
 *
 *                      - If `ipiv[k]>=0`, rows and columns `k` and
 *                        `ipiv[k]` were interchanged, `D(k,k)` is a 1-by-1
 *                        diagonal block.
 *                      - If `ipiv[k]<0`, rows and columns `k-1` and
 ...
 *                         - `info>0`: if `info=k+1`, `D(k,k)` is exactly zero.
    ...
    if (uplo[0] == 'U' || uplo[0] == 'u') {
        ...
        while (1) {
            ...
            if (fmaxf(absakk, colmax) == 0.0f) {
                ...
                if (absakk >= ALPHA_BK * colmax) {
                    ...
                    cblas_scopy(imax + 1, &A[0 + imax * lda], 1,
                                &W[0 + (kw - 1) * ldw], 1);
                    ...
                    INT jmax = (imax + 1) + cblas_isamax(k - imax, &W[(imax + 1) + (kw - 1) * ldw], 1);
                    f32 rowmax = fabsf(W[jmax + (kw - 1) * ldw]);

                    if (imax > 0) {
                        jmax = cblas_isamax(imax, &W[0 + (kw - 1) * ldw], 1);
                        rowmax = fmaxf(rowmax, fabsf(W[jmax + (kw - 1) * ldw]));
                    }

                    if (absakk >= ALPHA_BK * colmax * (colmax / rowmax)) {
                        /* No interchange, use 1-by-1 pivot block */
                        kp = k;
                    } else if (fabsf(W[imax + (kw - 1) * ldw]) >= ALPHA_BK * rowmax) {
                        /* Interchange rows and columns K and IMAX,
                         * use 1-by-1 pivot block.
                         * Copy column KW-1 of W to column KW of W. */
                        kp = imax;
                        cblas_scopy(k + 1, &W[0 + (kw - 1) * ldw], 1,
                                    &W[0 + kw * ldw], 1);
                    } else {
                ...
                if (kp != kk) {
                    ...
                    if (kk - kp - 1 > 0) {
                        cblas_scopy(kk - kp - 1, &A[(kp + 1) + kk * lda], 1,
                                    &A[kp + (kp + 1) * lda], lda);
                    }

                    if (kp > 0) {
                        cblas_scopy(kp, &A[0 + kk * lda], 1,
                                    &A[0 + kp * lda], 1);
                    }
                    ...
                    if (k < n - 1) {
                        cblas_sswap(n - 1 - k, &A[kk + (k + 1) * lda], lda,
                                    &A[kp + (k + 1) * lda], lda);
                    }

                    cblas_sswap(nb - kkw, &W[kk + kkw * ldw], ldw,
                                &W[kp + kkw * ldw], ldw);
                }
                ...
                if (kstep == 1) {
                    ...
                    if (k > 1) {
                         ...
                         * = D21 * ( ( D11 ) (  -1 ) )
                         *         ( (  -1 ) ( D22 ) ) */
                        f32 d21 = W[(k - 1) + kw * ldw];
                        f32 d11 = W[k + kw * ldw] / d21;
                        f32 d22 = W[(k - 1) + (kw - 1) * ldw] / d21;
         ...
         * in columns k+1:n looping backwards from k+1 to n */
        {
            ...
            do {
                ...
                j++;
                if (jp != jj && j <= n - 1) {
                    cblas_sswap(n - j, &A[jp + j * lda], lda,
                                &A[jj + j * lda], lda);
                }
            } while (j <= n - 2);
        }
        ...
        *kb = n - 1 - k;
        ...
        while (1) {
            ...
            if (fmaxf(absakk, colmax) == 0.0f) {
                ...
                if (absakk >= ALPHA_BK * colmax) {
                    ...
                    cblas_scopy(imax - k, &A[imax + k * lda], lda,
                                &W[k + (k + 1) * ldw], 1);

                    cblas_scopy(n - imax, &A[imax + imax * lda], 1,
                                &W[imax + (k + 1) * ldw], 1);
                    ...
                    /* JMAX is the column-index of the largest off-diagonal
                     * element in row IMAX, and ROWMAX is its absolute value */
                    INT jmax = k + cblas_isamax(imax - k, &W[k + (k + 1) * ldw], 1);
                    f32 rowmax = fabsf(W[jmax + (k + 1) * ldw]);

                    if (imax < n - 1) {
                        jmax = (imax + 1) + cblas_isamax(n - imax - 1, &W[(imax + 1) + (k + 1) * ldw], 1);
                        rowmax = fmaxf(rowmax, fabsf(W[jmax + (k + 1) * ldw]));
                    }

                    if (absakk >= ALPHA_BK * colmax * (colmax / rowmax)) {
                        /* No interchange, use 1-by-1 pivot block */
                        kp = k;
                    } else if (fabsf(W[imax + (k + 1) * ldw]) >= ALPHA_BK * rowmax) {
                        /* Interchange rows and columns K and IMAX,
                         * use 1-by-1 pivot block.
                         * Copy column K+1 of W to column K of W. */
                        kp = imax;
                        cblas_scopy(n - k, &W[k + (k + 1) * ldw], 1,
                                    &W[k + k * ldw], 1);
                    } else {
                        /* Interchange rows and columns K+1 and IMAX,
                         * use 2-by-2 pivot block */
                ...
                if (kp != kk) {
                    ...
                    if (kp - kk - 1 > 0) {
                        cblas_scopy(kp - kk - 1, &A[(kk + 1) + kk * lda], 1,
                                    &A[kp + (kk + 1) * lda], lda);
                    }

                    if (kp < n - 1) {
                        cblas_scopy(n - kp - 1, &A[(kp + 1) + kk * lda], 1,
                                    &A[(kp + 1) + kp * lda], 1);
                    }
                    ...
                    if (k > 0) {
                        cblas_sswap(k, &A[kk + 0 * lda], lda,
                                    &A[kp + 0 * lda], lda);
                    }

                    cblas_sswap(kk + 1, &W[kk + 0 * ldw], ldw,
                                &W[kp + 0 * ldw], ldw);
                }
                ...
                if (kstep == 1) {
                    ...
                    if (k < n - 2) {
                         ...
                         *         ( (  -1 ) ( D22 ) ) */
                        f32 d21 = W[(k + 1) + k * ldw];
                        f32 d11 = W[(k + 1) + (k + 1) * ldw] / d21;
                        f32 d22 = W[k + k * ldw] / d21;
                        f32 t = 1.0f / (d11 * d22 - 1.0f);
                        d21 = t / d21;
         ...
         * of rows in columns 1:k-1 looping backwards from k-1 to 1 */
        {
            ...
            do {
                ...
                j--;
                if (jp != jj && j >= 0) {
                    cblas_sswap(j + 1, &A[jp + 0 * lda], lda,
                                &A[jj + 0 * lda], lda);
                }
            } while (j >= 1);
        }
