/**
 * @file slaqps.c
 * @brief SLAQPS computes a step of QR factorization with column pivoting
 ...
 * @param[out]    tau     Double precision array, dimension (kb).
 ...
 * @param[in,out] auxv    Double precision array, dimension (nb).
 *                        Auxiliary vector.
 * @param[in,out] F       Double precision array, dimension (ldf, nb).
 *                        Matrix F**T = L*Y**T*A.
            ...
            f32* restrict F, const int ldf)
{
    ...
    while (k < nb && lsticc == 0) {
        ...
        if (pvt != k - 1) {
            cblas_sswap(m, &A[pvt * lda], 1, &A[(k - 1) * lda], 1);
            cblas_sswap(k - 1, &F[pvt], ldf, &F[k - 1], ldf);
            itemp = jpvt[pvt];
            jpvt[pvt] = jpvt[k - 1];
            jpvt[k - 1] = itemp;
            vn1[pvt] = vn1[k - 1];
         ...
         */
        if (rk < m) {
            slarfg(m - rk + 1, &A[(rk - 1) + (k - 1) * lda],
                   &A[rk + (k - 1) * lda], 1, &tau[k - 1]);
        } else {
            slarfg(1, &A[(rk - 1) + (k - 1) * lda],
         ...
         * F(K+1:N, K) := tau(K)*A(RK:M, K+1:N)**T * A(RK:M, K).
         */
        if (k < n) {
        ...
        if (rk < lastrk) {
            for (j = k + 1; j <= n; j++) {
                if (vn1[j - 1] != 0.0f) {
                     ...
                     */
                    temp = fabsf(A[(rk - 1) + (j - 1) * lda]) / vn1[j - 1];
    ...
    *kb = k;
    ...
    while (lsticc > 0) {
        ...
        vn1[lsticc - 1] = cblas_snrm2(m - rk, &A[rk + (lsticc - 1) * lda], 1);
