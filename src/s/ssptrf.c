/**
 * @file ssptrf.c
 * @brief SSPTRF computes the factorization of a real symmetric matrix stored
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            absakk = fabsf(AP[kc + k]);

            if (k > 0) {
                imax = cblas_isamax(k, &AP[kc], 1);
                colmax = fabsf(AP[kc + imax]);
            } else {
                colmax = ZERO;
            }
            ...
            if (fmaxf(absakk, colmax) == ZERO) {
                ...
                if (absakk >= alpha * colmax) {
                    ...
                    kpc = imax * (imax + 1) / 2;
                    if (imax > 0) {
                        jmax = cblas_isamax(imax, &AP[kpc], 1);
                        rowmax = fmaxf(rowmax, fabsf(AP[kpc + jmax]));
                    }

                    if (absakk >= alpha * colmax * (colmax / rowmax)) {
                        kp = k;
                    } else if (fabsf(AP[kpc + imax]) >= alpha * rowmax) {
                        kp = imax;
                    } else {
                        kp = imax;
                        kstep = 2;
                ...
                if (kp != kk) {
                    if (kp > 0)
                        cblas_sswap(kp, &AP[knc], 1, &AP[kpc], 1);
                ...
                if (kstep == 1) {
                    if (k > 0) {
                        r1 = ONE / AP[kc + k];
                        cblas_sspr(CblasColMajor, CblasUpper, k, -r1, &AP[kc], 1, AP);
                        cblas_sscal(k, r1, &AP[kc], 1);
                    }
                } else {
                    if (k > 1) {
                        d12 = AP[kc + k - 1];
        ...
        while (k < n) {
            ...
            if (k < n - 1) {
                imax = k + 1 + cblas_isamax(n - k - 1, &AP[kc + 1], 1);
                colmax = fabsf(AP[kc + imax - k]);
            } else {
                colmax = ZERO;
            }
            ...
            if (fmaxf(absakk, colmax) == ZERO) {
                ...
                if (absakk >= alpha * colmax) {
                    ...
                    if (imax < n - 1) {
                        jmax = imax + 1 + cblas_isamax(n - imax - 1, &AP[kpc + 1], 1);
                        rowmax = fmaxf(rowmax, fabsf(AP[kpc + jmax - imax]));
                    }

                    if (absakk >= alpha * colmax * (colmax / rowmax)) {
                        kp = k;
                    } else if (fabsf(AP[kpc]) >= alpha * rowmax) {
                        kp = imax;
                    } else {
                        kp = imax;
                        kstep = 2;
                ...
                if (kp != kk) {
                    if (kp < n - 1)
                        cblas_sswap(n - kp - 1, &AP[knc + kp - kk + 1], 1, &AP[kpc + 1], 1);
                ...
                if (kstep == 1) {
                    if (k < n - 1) {
                        r1 = ONE / AP[kc];
                        cblas_sspr(CblasColMajor, CblasLower, n - k - 1, -r1,
                                   &AP[kc + 1], 1, &AP[kc + n - k]);
                        cblas_sscal(n - k - 1, r1, &AP[kc + 1], 1);
                    }
                } else {
                    if (k < n - 2) {
                        d21 = AP[kc + 1];
