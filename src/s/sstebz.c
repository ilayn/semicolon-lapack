            ...
            int* info)
{
    ...
    *m = 0;
     ...
     * SSTEBZ has no special case in ILAENV, so NB defaults to 1,
    ...
    *nsplit = 1;
    work[n - 1] = 0.0f;  /* Fortran: WORK(N) = ZERO */
    pivmin = 1.0f;
    ...
    for (j = 1; j < n; j++) {
        ...
        tmp1 = E[j - 1] * E[j - 1];
        if (fabsf(D[j] * D[j - 1]) * ulp * ulp + safemn > tmp1) {
    ...
    if (irange == 3) {
        ...
        tnorm = fabsf(gl) > fabsf(gu) ? fabsf(gl) : fabsf(gu);
        gl = gl - FUDGE * tnorm * ulp * n - FUDGE * 2.0f * pivmin;
        ...
        /*
         * Set up the initial intervals for SLAEBZ IJOB=3 call.
        ...
        /*
         * SLAEBZ(3, ITMAX, N, 2, 2, NB, ATOLI, RTOLI, PIVMIN,
         ...
         *
         * AB layout after slaebz (mmax=2, column-major):
        ...
        for (j = 1; j < n - 1; j++) {
            ...
            tmp1 = fabsf(D[j]) + fabsf(E[j - 1]) + fabsf(E[j]);
    ...
    *info = 0;
    ...
    for (jb = 0; jb < *nsplit; jb++) {
        ...
        if (in == 1) {
            ...
            bnorm = fabsf(gl) > fabsf(gu) ? fabsf(gl) : fabsf(gu);
            ...
            /*
             * Set Up Initial Interval for SLAEBZ IJOB=1 call.
             *
            ...
            /*
             * SLAEBZ(1, 0, IN, IN, 1, NB, ATOLI, RTOLI, PIVMIN,
            ...
            /*
             * Compute Eigenvalues via SLAEBZ IJOB=2 call.
             *
             ...
             * But slaebz uses 0-based internally: AB[j + 0*mmax] = work[n + j]
             ...
             *
             * Actually, our slaebz uses 0-based indexing throughout, so:
             ...
             */
            for (j = 0; j < iout; j++) {
                tmp1 = 0.5f * (work[n + j] + work[n + in + j]);
