/**
 * @file slaqr0.c
 * @brief SLAQR0 computes eigenvalues of a Hessenberg matrix using small-bulge
...
/**
 * Compute IPARMQ-style tuning parameters for SLAQR0.
...
{
    /* For SLAQR0: use NS-based threshold */
    const int kacmin = 14;
    const int k22min = 14;
    int kacc = 0;
    if (ns >= kacmin) kacc = 1;
 ...
 *
 * SLAQR0 is identical to SLAQR4 except that it calls SLAQR3
 * instead of SLAQR2.
 *
 ...
 */
SEMICOLON_API void slaqr0(const int wantt, const int wantz, const int n,
                          ...
                          float* work, const int lwork, int* info)
{
    ...
    /* Local array for dummy Z in SLAHQR/SLAQR4 call */
    float zdum[1];
    ...
    *info = 0;
    ...
    if (n <= ntiny) {
        ...
        *info = 0;
        ...
        slaqr3(wantt, wantz, n, ilo, ihi, nwr + 1, H, ldh, iloz,
               ihiz, Z, ldz, &ls, &ld, wr, wi, H, ldh, n, H, ldh,
               n, H, ldh, work, -1);

        /* Optimal workspace = MAX(3*NSR/2, SLAQR3 workspace) */
        lwkopt = 3 * nsr / 2;
        ...
        /* NMIN = SLAHQR/SLAQR0 crossover point */
        nmin = iparmq_nmin();
        ...
        for (it = 1; it <= itmax; it++) {
            ...
            if (nw < nwmax) {
                if (nw >= nh - 1) {
                    ...
                    kwtop = kbot - nw + 1;
                    if (fabsf(H[kwtop + (kwtop - 1) * ldh]) >
                        fabsf(H[(kwtop - 1) + (kwtop - 2) * ldh]))
                        nw = nw + 1;
                }
            }
            if (ndfl < kexnw) {
            ...
            /* Aggressive early deflation - uses SLAQR3 (not SLAQR2) */
            ...
            if ((ld == 0) || ((100 * ld <= nw * nibble) &&
                 ...
                 * shifts provided by SLAQR3 above or from the eigenvalues
                 * of a trailing principal submatrix. */

                if ((ndfl % kexsh) == 0) {
                    ks = kbot - ns + 1;
                    for (i = kbot; i >= (ks + 1 > ktop + 2 ? ks + 1 : ktop + 2); i -= 2) {
                        ss = fabsf(H[i + (i - 1) * ldh]) + fabsf(H[(i - 1) + (i - 2) * ldh]);
                        aa = wilk1 * ss + H[i + i * ldh];
                        bb = ss;
                        cc = wilk2 * ss;
                        dd = aa;
                        slanv2(&aa, &bb, &cc, &dd, &wr[i - 1], &wi[i - 1],
                               &wr[i], &wi[i], &cs, &sn);
                    }
                    if (ks == ktop) {
                    ...
                    /* Got NS/2 or fewer shifts? Use SLAQR4 or SLAHQR on a
                    ...
                    if (kbot - ks + 1 <= ns / 2) {
                        ...
                        slacpy("A", ns, ns, &H[ks + ks * ldh], ldh,
                               &H[kt], ldh);
                        if (ns > nmin) {
                            /* Use SLAQR4 for larger submatrices */
                            slaqr4(0, 0, ns, 0, ns - 1, &H[kt], ldh, &wr[ks],
                                   &wi[ks], 0, 0, zdum, 1, work, lwork, &inf);
                        } else {
                            /* Use SLAHQR for smaller submatrices */
                            slahqr(0, 0, ns, 0, ns - 1, &H[kt], ldh, &wr[ks],
                    ...
                    if (kbot - ks + 1 > ns) {
                        ...
                        for (k = kbot; k >= ks + 1; k--) {
                            ...
                            sorted = 1;
                            for (i = ks; i <= k - 1; i++) {
                                if (fabsf(wr[i]) + fabsf(wi[i]) <
                                    fabsf(wr[i + 1]) + fabsf(wi[i + 1])) {
                                    sorted = 0;
                ...
                if (kbot - ks + 1 == 2) {
                    if (wi[kbot] == zero) {
                        if (fabsf(wr[kbot] - H[kbot + kbot * ldh]) <
                            fabsf(wr[kbot - 1] - H[kbot + kbot * ldh])) {
                            wr[kbot - 1] = wr[kbot];
                        } else {
                            wr[kbot] = wr[kbot - 1];
                        }
                ...
                /* Small-bulge multi-shift QR sweep */
                slaqr5(wantt, wantz, kacc22, n, ktop, kbot, ns,
