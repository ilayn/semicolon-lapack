    ...
    int* info)
{
    ...
    *info = 0;
    ...
    /* The following block introduces the shifts and chases
       ...
       of size (ns+1) x ns. */
    slaset("F", ns + 1, ns + 1, ZERO, ONE, QC, ldqc);
    slaset("F", ns, ns, ZERO, ONE, ZC, ldzc);

    for (i = 0; i <= ns - 1; i += 2) {
        /* Introduce the shift */
        slaqz1(&A[ilo + ilo * lda], lda, &B[ilo + ilo * ldb], ldb,
               sr[i], sr[i + 1], si[i], ss[i], ss[i + 1], v);

        temp = v[1];
        slartg(temp, v[2], &c1, &s1, &v[1]);
        slartg(v[0], v[1], &c2, &s2, &temp);

        cblas_srot(ns, &A[(ilo + 1) + ilo * lda], lda, &A[(ilo + 2) + ilo * lda], lda, c1, s1);
        cblas_srot(ns, &A[ilo + ilo * lda], lda, &A[(ilo + 1) + ilo * lda], lda, c2, s2);
        cblas_srot(ns, &B[(ilo + 1) + ilo * ldb], ldb, &B[(ilo + 2) + ilo * ldb], ldb, c1, s1);
        cblas_srot(ns, &B[ilo + ilo * ldb], ldb, &B[(ilo + 1) + ilo * ldb], ldb, c2, s2);
        cblas_srot(ns + 1, &QC[0 + 1 * ldqc], 1, &QC[0 + 2 * ldqc], 1, c1, s1);
        cblas_srot(ns + 1, &QC[0 + 0 * ldqc], 1, &QC[0 + 1 * ldqc], 1, c2, s2);

        /* Chase the shift down */
        for (j = 0; j <= ns - 2 - i; j++) {
            /* Fortran: SLAQZ2(.TRUE., .TRUE., J, 1, NS, IHI-ILO+1, A(ILO,ILO), ...) */
            /* j is 0-based in C, maps to Fortran J which is 1-based */
            /* In Fortran: J goes 1 to NS-1-I, in C: j goes 0 to NS-2-I */
            /* The k parameter to slaqz2 in Fortran is J (1-based), in C it's j (0-based) */
            slaqz2(1, 1, j, 0, ns - 1, ihi - ilo,
    ...
    if (swidth > 0) {
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, sheight, swidth, sheight,
                    ONE, QC, ldqc, &A[ilo + (ilo + ns) * lda], lda, ZERO, work, sheight);
        slacpy("A", sheight, swidth, work, sheight, &A[ilo + (ilo + ns) * lda], lda);
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, sheight, swidth, sheight,
                    ONE, QC, ldqc, &B[ilo + (ilo + ns) * ldb], ldb, ZERO, work, sheight);
        slacpy("A", sheight, swidth, work, sheight, &B[ilo + (ilo + ns) * ldb], ldb);
    }
    if (ilq) {
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, sheight, sheight,
                    ONE, &Q[0 + ilo * ldq], ldq, QC, ldqc, ZERO, work, n);
        slacpy("A", n, sheight, work, n, &Q[0 + ilo * ldq], ldq);
    }
    ...
    swidth = ns;
    if (sheight > 0) {
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                    ONE, &A[istartm + ilo * lda], lda, ZC, ldzc, ZERO, work, sheight);
        slacpy("A", sheight, swidth, work, sheight, &A[istartm + ilo * lda], lda);
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                    ONE, &B[istartm + ilo * ldb], ldb, ZC, ldzc, ZERO, work, sheight);
        slacpy("A", sheight, swidth, work, sheight, &B[istartm + ilo * ldb], ldb);
    }
    if (ilz) {
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, swidth, swidth,
                    ONE, &Z[0 + ilo * ldz], ldz, ZC, ldzc, ZERO, work, n);
        slacpy("A", n, swidth, work, n, &Z[0 + ilo * ldz], ldz);
    }
    ...
    while (k < ihi - ns) {
        ...
        slaset("F", ns + np, ns + np, ZERO, ONE, QC, ldqc);
        slaset("F", ns + np, ns + np, ZERO, ONE, ZC, ldzc);
        ...
        for (i = ns - 1; i >= 0; i -= 2) {
            for (j = 0; j <= np - 1; j++) {
                /* Move down the block with index k+i+j-1, updating
                   ...
                   Fortran: SLAQZ2(.TRUE., .TRUE., K+I+J-1, ISTARTB, ISTOPB, IHI, ...)
                   ...
                   But wait, I need to be more careful about what slaqz2 expects.
                   slaqz2's k parameter is the bulge position (0-based in C).
                   So we pass k + i + j directly (all 0-based). */
                slaqz2(1, 1, k + i + j, istartb, istopb, ihi,
        ...
        if (swidth > 0) {
            ...
            slacpy("A", sheight, swidth, work, sheight, &A[(k + 1) + (k + ns + np) * lda], lda);
            cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, sheight, swidth, sheight,
                        ONE, QC, ldqc, &B[(k + 1) + (k + ns + np) * ldb], ldb, ZERO, work, sheight);
            slacpy("A", sheight, swidth, work, sheight, &B[(k + 1) + (k + ns + np) * ldb], ldb);
        }
        if (ilq) {
            cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, nblock, nblock,
                        ONE, &Q[0 + (k + 1) * ldq], ldq, QC, ldqc, ZERO, work, n);
            slacpy("A", n, nblock, work, n, &Q[0 + (k + 1) * ldq], ldq);
        }
        ...
        if (sheight > 0) {
            ...
            slacpy("A", sheight, swidth, work, sheight, &A[istartm + k * lda], lda);
            cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                        ONE, &B[istartm + k * ldb], ldb, ZC, ldzc, ZERO, work, sheight);
            slacpy("A", sheight, swidth, work, sheight, &B[istartm + k * ldb], ldb);
        }
        if (ilz) {
            cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, nblock, nblock,
                        ONE, &Z[0 + k * ldz], ldz, ZC, ldzc, ZERO, work, n);
            slacpy("A", n, nblock, work, n, &Z[0 + k * ldz], ldz);
        }
    ...
    slaset("F", ns, ns, ZERO, ONE, QC, ldqc);
    slaset("F", ns + 1, ns + 1, ZERO, ONE, ZC, ldzc);
    ...
    for (i = 0; i <= ns - 1; i += 2) {
        ...
        for (ishift = ihi - i - 2; ishift <= ihi - 2; ishift++) {
            /* Fortran: DO ISHIFT = IHI-I-1, IHI-2 (1-based IHI, I goes 1,3,5,...)
               ...
               But ISHIFT is the k parameter to slaqz2, which should be 0-based.
               Fortran ISHIFT is 1-based position, C needs 0-based.
               So C ishift should go from ihi-i-2 to ihi-2. */
            slaqz2(1, 1, ishift, istartb, istopb, ihi,
    ...
    if (swidth > 0) {
        ...
        slacpy("A", sheight, swidth, work, sheight, &A[(ihi - ns + 1) + (ihi + 1) * lda], lda);
        cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, sheight, swidth, sheight,
                    ONE, QC, ldqc, &B[(ihi - ns + 1) + (ihi + 1) * ldb], ldb, ZERO, work, sheight);
        slacpy("A", sheight, swidth, work, sheight, &B[(ihi - ns + 1) + (ihi + 1) * ldb], ldb);
    }
    if (ilq) {
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, ns, ns,
                    ONE, &Q[0 + (ihi - ns + 1) * ldq], ldq, QC, ldqc, ZERO, work, n);
        slacpy("A", n, ns, work, n, &Q[0 + (ihi - ns + 1) * ldq], ldq);
    }
    ...
    if (sheight > 0) {
        ...
        slacpy("A", sheight, swidth, work, sheight, &A[istartm + (ihi - ns) * lda], lda);
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, sheight, swidth, swidth,
                    ONE, &B[istartm + (ihi - ns) * ldb], ldb, ZC, ldzc, ZERO, work, sheight);
        slacpy("A", sheight, swidth, work, sheight, &B[istartm + (ihi - ns) * ldb], ldb);
    }
    if (ilz) {
        cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, n, ns + 1, ns + 1,
                    ONE, &Z[0 + (ihi - ns) * ldz], ldz, ZC, ldzc, ZERO, work, n);
        slacpy("A", n, ns + 1, work, n, &Z[0 + (ihi - ns) * ldz], ldz);
    }
}
