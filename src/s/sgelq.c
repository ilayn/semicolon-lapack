/**
 * @file sgelq.c
 * @brief SGELQ computes an LQ factorization of a real M-by-N matrix A.
 */
           ...
           int* info)
{
    ...
    *info = 0;
    ...
    if ((n <= m) || (nb <= m) || (nb >= n)) {
        sgelqt(m, n, mb, A, lda, &T[5], mb, work, info);
    } else {
        slaswlq(m, n, mb, nb, A, lda, &T[5], mb, work, lwork, info);
    }
