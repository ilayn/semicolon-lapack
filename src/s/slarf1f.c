/**
 * @file slarf1f.c
 * @brief SLARF1F applies an elementary reflector to a general rectangular
 ...
 */
void slarf1f(const char* side, const int m, const int n,
             ...
             f32 * const restrict work)
{
    ...
    if (applyleft) {
        ...
        if (lastv == 1) {
            ...
            cblas_sscal(lastc, ONE - tau, C, ldc);
        } else {
            ...
            cblas_saxpy(lastc, ONE, C, ldc, work, 1);

            /* C(0, 0:lastc-1) -= tau * w */
            cblas_saxpy(lastc, -tau, work, 1, C, ldc);

            /* C(1:lastv-1, 0:lastc-1) -= tau * v(incv:) * w^T */
            cblas_sger(CblasColMajor, lastv - 1, lastc,
        ...
        if (lastv == 1) {
            ...
            cblas_saxpy(lastc, ONE, C, 1, work, 1);

            /* C(0:lastc-1, 0) -= tau * w */
            cblas_saxpy(lastc, -tau, work, 1, C, 1);

            /* C(0:lastc-1, 1:lastv-1) -= tau * w * v(incv:)^T */
            cblas_sger(CblasColMajor, lastc, lastv - 1,
