/**
 * @file slaruv.c
 * @brief SLARUV returns a vector of n random real numbers from a uniform (0,1) distribution.
 */
 ...
 * distribution (n <= 128).
 *
 * This is an auxiliary routine called by SLARNV and ZLARNV.
 *
 ...
 * @param[in]     n      The number of random numbers to be generated. n <= 128.
...
#define LV  128
/* IPW2 = 2^12 = 4096, the modulus for each 12-bit digit */
#define IPW2 4096
/* R = 1.0 / IPW2 */
#define R (1.0f / 4096.0f)

/**
 * Multiplier table: MM[i][j] for i=0..127, j=0..3.
 * Each row contains the 4 x 12-bit digits of the i-th power of the
 * multiplier 33952834046453 modulo 2^48.
 *
...
void slaruv(int* restrict iseed, const int n, f32* restrict X)
{
    ...
    for (i = 0; i < nlv; i++) {
        ...
        for (;;) {
            ...
            it1 = it2 / IPW2;
            it2 = it2 - IPW2 * it1;
            it1 = it1 + i1 * MM[i][3] + i2 * MM[i][2] + i3 * MM[i][1] +
                  i4 * MM[i][0];
            it1 = it1 % IPW2;
