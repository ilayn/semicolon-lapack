/**
 * @file zlangt.c
 * @brief ZLANGT returns the value of the 1-norm, Frobenius norm, infinity-norm,
 ...
 * @param[in] n     The order of the matrix A. n >= 0. When n = 0, ZLANGT is
 *                  set to zero.
    ...
    const double complex* const restrict DU)
{
    ...
    if (n <= 0) {
        ...
        if (n == 1) {
            ...
            anorm = cabs(D[0]) + cabs(DL[0]);
            /* Last column: DU[n-2] + D[n-1] */
            temp = cabs(D[n - 1]) + cabs(DU[n - 2]);
            if (anorm < temp || isnan(temp)) {
                anorm = temp;
            }
            /* Middle columns: DU[i-1] + D[i] + DL[i] */
            for (i = 1; i < n - 1; i++) {
                temp = cabs(D[i]) + cabs(DL[i]) + cabs(DU[i - 1]);
                if (anorm < temp || isnan(temp)) {
                    anorm = temp;
                }
            }
        ...
        if (n == 1) {
            ...
            /* First row: D[0] + DU[0] */
            anorm = cabs(D[0]) + cabs(DU[0]);
            /* Last row: DL[n-2] + D[n-1] */
            temp = cabs(D[n - 1]) + cabs(DL[n - 2]);
            if (anorm < temp || isnan(temp)) {
                anorm = temp;
            }
            /* Middle rows: DL[i-1] + D[i] + DU[i] */
            for (i = 1; i < n - 1; i++) {
                temp = cabs(D[i]) + cabs(DU[i]) + cabs(DL[i - 1]);
                if (anorm < temp || isnan(temp)) {
                    anorm = temp;
                }
            }
