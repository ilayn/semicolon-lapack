/**
 * @file zhetrd.c
 * @brief ZHETRD reduces a complex Hermitian matrix to real symmetric
    ...
    if (upper) {
        ...
        for (i = n - nb; i >= kk; i -= nb) {
            /* Reduce columns i:i+nb-1 to tridiagonal form and form the
               ...
               the matrix */
            zlatrd(uplo, i + nb, nb, A, lda, E, tau, work, ldwork);
        ...
        for (i = 0; i < n - nx; i += nb) {
            /* Reduce columns i:i+nb-1 to tridiagonal form and form the
               matrix W which is needed to update the unreduced part of
               the matrix */
            zlatrd(uplo, n - i, nb, &A[i + i * lda], lda, &E[i],
                   &tau[i], work, ldwork);
            ...
            /* Copy subdiagonal elements back into A, and diagonal
               elements into D */
            for (j = i; j < i + nb; j++) {
                A[(j + 1) + j * lda] = CMPLX(E[j], 0.0);
                D[j] = creal(A[j + j * lda]);
            }
        }

        /* Use unblocked code to reduce the last or only block */
        zhetd2(uplo, n - i, &A[i + i * lda], lda, &D[i], &E[i],
               &tau[i], &iinfo);
    }
