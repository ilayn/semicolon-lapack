/**
 * @file zung2r.c
 * @brief ZUNG2R generates all or part of the unitary matrix Q from
 ...
 *                      i = 0,1,...,k-1, as returned by ZGEQRF in the first
            ...
            int* info)
{
    ...
    *info = 0;
    ...
    for (i = k - 1; i >= 0; i--) {
        ...
        if (i < n - 1) {
            zlarf1f("L", m - i, n - i - 1, &A[i + i * lda], 1, tau[i],
                    &A[i + (i + 1) * lda], lda, work);
        }
        if (i < m - 1) {
            c128 neg_tau = -tau[i];
            cblas_zscal(m - i - 1, &neg_tau, &A[(i + 1) + i * lda], 1);
        }
        A[i + i * lda] = ONE - tau[i];
