 ...
 * @param[out]    AB      Double complex array, dimension (LDAB,N).
 ...
 * @param[out]    tau     Double complex array, dimension (N-KD).
    ...
    *info = 0;
    ...
    if (n <= kd + 1) {
        if (upper) {
            for (i = 0; i < n; i++) {
                ...
                cblas_zcopy(lk, &A[(i - lk + 1) + i * lda], 1,
                            &AB[(kd + 1 - lk) + i * ldab], 1);
            }
        } else {
            for (i = 0; i < n; i++) {
                lk = (kd + 1 < n - i) ? (kd + 1) : (n - i);
                cblas_zcopy(lk, &A[i + i * lda], 1, &AB[0 + i * ldab], 1);
            }
        }
    ...
    if (upper) {
        for (i = 0; i < n - kd; i += kd) {
            ...
            zgelqf(kd, pn, &A[i + (i + kd) * lda], lda,
                   &tau[i], &work[s2pos], ls2, &iinfo);

            for (j = i; j < i + pk; j++) {
                lk = ((kd < n - j - 1) ? kd : (n - j - 1)) + 1;
                cblas_zcopy(lk, &A[j + j * lda], lda,
                            &AB[kd + j * ldab], ldab - 1);
            }

            zlaset("Lower", pk, pk, zero, one,
                   &A[i + (i + kd) * lda], lda);

            zlarft("Forward", "Rowwise", pn, pk,
        ...
        for (i = 0; i < n - kd; i += kd) {
            ...
            zgeqrf(pn, kd, &A[(i + kd) + i * lda], lda,
                   &tau[i], &work[s2pos], ls2, &iinfo);

            for (j = i; j < i + pk; j++) {
                lk = ((kd < n - j - 1) ? kd : (n - j - 1)) + 1;
                cblas_zcopy(lk, &A[j + j * lda], 1, &AB[0 + j * ldab], 1);
            }

            zlaset("Upper", pk, pk, zero, one,
                   &A[(i + kd) + i * lda], lda);

            zlarft("Forward", "Columnwise", pn, pk,
