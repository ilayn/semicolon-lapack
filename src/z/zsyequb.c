/**
 * @file zsyequb.c
 * @brief ZSYEQUB computes row and column scalings intended to equilibrate
    ...
    int* info)
{
    ...
    *amax = ZERO;
    ...
    for (int iter = 0; iter < MAX_ITER; iter++) {
        ...
        for (int i = 0; i < n; i++) {
            double t = cabs1(A[i + i * lda]);
            double si = S[i];
            double c2 = (n - 1) * t;
            double c1 = (n - 2) * (creal(work[i]) - t * si);
            double c0 = -(t * si) * si + 2 * creal(work[i]) * si - n * avg;
