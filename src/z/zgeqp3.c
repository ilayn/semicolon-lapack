/**
 * @file zgeqp3.c
 * @brief ZGEQP3 computes a QR factorization with column pivoting of a
 ...
 * @param[out]    tau   Double complex array, dimension (min(m,n)).
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (minmn == 0) {
            ...
            iws = n + 1;
            /* ILAENV(1, 'ZGEQRF', ' ', M, N, -1, -1) -> NB for GEQRF */
    ...
    for (j = 0; j < n; j++) {
        if (jpvt[j] != 0) {
            if (j != nfxd) {
                cblas_zswap(m, &A[j * lda], 1, &A[nfxd * lda], 1);
                jpvt[j] = jpvt[nfxd];
    ...
    if (nfxd > 0) {
        ...
        zgeqrf(m, na, A, lda, tau, work, lwork, &iinfo);
    ...
    if (nfxd < minmn) {
        ...
        /* ILAENV(1, 'ZGEQRF', ' ', SM, SN, -1, -1) -> NB for GEQRF */
        nb = lapack_get_nb("GEQRF");
        nbmin = 2;
        nx = 0;
        ...
        if ((nb > 1) && (nb < sminmn)) {
            ...
            /* ILAENV(3, 'ZGEQRF', ' ', SM, SN, -1, -1) -> NX for GEQRF */
            ...
            if (nx < sminmn) {
                ...
                if (lwork < minws) {
                     ...
                     */
                    nb = lwork / (sn + 1);
                    /* ILAENV(2, 'ZGEQRF', ' ', SM, SN, -1, -1) -> NBMIN for GEQRF */
         ...
         *   work[0..]        = used by zlaqps (auxv + F matrix)
         *
         ...
         */
        for (j = nfxd; j < n; j++) {
            rwork[j] = cblas_dznrm2(sm, &A[nfxd + j * lda], 1);
            rwork[n + j] = rwork[j];
        }
        ...
        if ((nb >= nbmin) && (nb < sminmn) && (nx < sminmn)) {
            ...
            while (j < topbmn) {
                 ...
                 *
                 * zlaqps arguments (translated from Fortran 1-based):
                 *   M      = m
         ...
         *
         * zlaqp2 arguments (translated from Fortran 1-based):
         *   M      = m
