/**
 * @file zgedmd.c
 * @brief ZGEDMD computes the Dynamic Mode Decomposition (DMD) for a pair of
...
/**
 * ZGEDMD computes the Dynamic Mode Decomposition (DMD) for a pair of data
 * snapshot matrices. For the input matrices X and Y such that Y = A*X with an
 * unaccessible matrix A, ZGEDMD computes a certain number of Ritz pairs of A
 ...
 * ZGEDMD returns the residuals of the computed Ritz pairs, the information
 ...
 *                        1 :: zgesvd, 2 :: zgesdd, 3 :: zgesvdq, 4 :: zgejsv.
 ...
 */
void zgedmd(const char* jobs, const char* jobz, const char* jobr,
            const char* jobf, const INT whtsvd, const INT m, const INT n,
            c128* restrict X, const INT ldx, c128* restrict Y, const INT ldy,
            const INT nrnk, const f64 tol, INT* k, c128* restrict eigs,
            c128* restrict Z, const INT ldz, f64* restrict res,
            c128* restrict B, const INT ldb, c128* restrict W, const INT ldw,
            c128* restrict S, const INT lds, c128* restrict zwork,
            const INT lzwork, f64* restrict rwork, const INT lrwork,
    ...
    *info  = 0;
    ...
    if (*info == 0) {
        ...
        case 1:
            ...
            if (lquery) {
                zgesvd("O", "S", m, n, X, ldx, rwork, B, ldb, W, ldw,
                       zwork, -1, rdummy, &info1);
                lwrsvd = (INT)creal(zwork[0]);
        ...
        case 2:
            ...
            if (lquery) {
                zgesdd("O", m, n, X, ldx, rwork, B, ldb, W, ldw,
                       zwork, -1, rdummy, iwork, &info1);
                lwrsdd = imax2(mwrsdd, (INT)creal(zwork[0]));
        ...
        if (lquery) {
            zgeev("N", jobzl, n, S, lds, eigs, W, ldw, W, ldw, zwork, -1,
                  rwork, &info1);
            lwrkev = (INT)creal(zwork[0]);
    ...
    if (sccolx) {
        ...
        *k = 0;
        for (i = 0; i < n; i++) {
            ...
            if ((scale != ZERO) && (ssum != ZERO)) {
                ...
                if (scale >= tbig) {
                    ...
                    zlascl("G", 0, 0, scale, ONE / rootsc, m, 1, &X[i * ldx], ldx, &info2);
                    rwork[i] = -scale * (rootsc / (f64)m);
                } else {
                    /* X(:,i) will be scaled to unit 2-norm */
                    rwork[i] = scale * rootsc;
                    zlascl("G", 0, 0, rwork[i], ONE, m, 1, &X[i * ldx], ldx, &info2);
                }
            } else {
                rwork[i] = ZERO;
                *k = *k + 1;
        ...
        for (i = 0; i < n; i++) {
            ...
            if (rwork[i] > ZERO) {
                cblas_zdscal(m, ONE / rwork[i], &Y[i * ldy], 1);
            } else if (rwork[i] < ZERO) {
                zlascl("G", 0, 0, -rwork[i], ONE / (f64)m, m, 1, &Y[i * ldy], ldy, &info2);
            } else if (cabs(Y[cblas_izamax(m, &Y[i * ldy], 1) + i * ldy]) != ZERO) {
                /* X(:,i) is zero vector. For consistency, Y(:,i) should also
                 * be zero. */
                badxy = 1;
                if (jobs[0] == 'C' || jobs[0] == 'c')
                    cblas_zdscal(m, ZERO, &Y[i * ldy], 1);
            }
        }
    }
    ...
    if (sccoly) {
        ...
        for (i = 0; i < n; i++) {
            ...
            if (scale != ZERO && (ssum != ZERO)) {
                ...
                if (scale >= tbig) {
                    ...
                    zlascl("G", 0, 0, scale, ONE / rootsc, m, 1, &Y[i * ldy], ldy, &info2);
                    rwork[i] = -scale * (rootsc / (f64)m);
                } else {
                    /* Y(:,i) will be scaled to unit 2-norm */
                    rwork[i] = scale * rootsc;
                    zlascl("G", 0, 0, rwork[i], ONE, m, 1, &Y[i * ldy], ldy, &info2);
                }
            } else {
                rwork[i] = ZERO;
            }
        ...
        for (i = 0; i < n; i++) {
            ...
            if (rwork[i] > ZERO) {
                cblas_zdscal(m, ONE / rwork[i], &X[i * ldx], 1);
            } else if (rwork[i] < ZERO) {
                zlascl("G", 0, 0, -rwork[i], ONE / (f64)m, m, 1, &X[i * ldx], ldx, &info2);
            } else if (cabs(X[cblas_izamax(m, &X[i * ldx], 1) + i * ldx]) != ZERO) {
                /* Y(:,i) is zero vector. */
                badxy = 1;
            }
        }
    ...
    case 4:
        ...
        xscl2 = rwork[n + 1];
        if (xscl1 != xscl2) {
            /* Exceptional situation: ZGEJSV returned the SVD in scaled form. */
    ...
    if (t_or_n == 'N') {
        for (i = 0; i < *k; i++) {
            cblas_zdscal(n, ONE / rwork[i], &W[i * ldw], 1);
        }
    } else {
        for (i = 0; i < *k; i++) {
    ...
    if (wntref) {
        ...
        if (wntres || wntex) {
            if (t_or_n == 'N') {
                zlacpy("A", n, *k, W, ldw, Z, ldz);
            } else {
                zlacpy("A", *k, n, W, ldw, Z, ldz);
            }
        }
    }
    ...
    zgeev("N", jobzl, *k, S, lds, eigs, W, ldw, W, ldw, zwork, lzwork,
          &rwork[n], &info1);

    if (info1 > 0) {
        /* ZGEEV failed to compute the eigenvalues and eigenvectors. */
        *info = 3;
        return;
    }
    ...
    if (wntvec || wntex) {
        if (wntres) {
            if (wntref) {
                ...
                zlacpy("A", m, *k, Z, ldz, Y, ldy);
                if (wntex) zlacpy("A", m, *k, Z, ldz, B, ldb);
            }
        } else if (wntex) {
            cblas_zgemm(CblasColMajor,
                        (t_or_n == 'C') ? CblasConjTrans : CblasNoTrans,
        ...
        if (wntres) {
            for (i = 0; i < *k; i++) {
                c128 neg = -eigs[i];
                cblas_zaxpy(m, &neg, &Z[i * ldz], 1, &Y[i * ldy], 1);
                res[i] = cblas_dznrm2(m, &Y[i * ldy], 1);
            }
        }
    }
