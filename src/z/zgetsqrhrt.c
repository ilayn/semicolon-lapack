 ...
 *
 * @param[out] T
 *          Double complex array, dimension (ldt, n).
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    zlatsqr(m, n, mb1, nb1local, A, lda, work, ldwt, &work[lwt], lw1, &iinfo);

    for (j = 0; j < n; j++) {
        cblas_zcopy(j + 1, &A[0 + j * lda], 1, &work[lwt + n * j], 1);
    }

    zungtsqr_row(m, n, mb1, nb1local, A, lda, work, ldwt, &work[lwt + n * n], lw2, &iinfo);

    zunhr_col(m, n, nb2local, A, lda, T, ldt, &work[lwt + n * n], &iinfo);
    ...
    for (i = 0; i < n; i++) {
        if (work[lwt + n * n + i] == -CONE) {
            ...
            cblas_zcopy(n - i, &work[lwt + n * i + i], n, &A[i + i * lda], lda);
        }
    }
