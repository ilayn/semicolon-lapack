/**
 * @file zgelsd.c
 * @brief ZGELSD computes the minimum norm solution to a complex linear least
...
/**
 * ZGELSD computes the minimum-norm solution to a complex linear least
 * squares problem:
    ...
    *info = 0;
    ...
    if (m >= n) {
        ...
        if (m >= mnthr) {
            ...
            /* Compute A=Q*R */
            zgeqrf(m, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &iinfo);
        ...
        zlalsd("U", smlsiz, n, nrhs, S, &rwork[ie], B, ldb, rcond, rank,
        ...
        zunmbr("P", "L", "N", n, nrhs, n, A, lda, &work[itaup],
               B, ldb, &work[nwork], lwork - nwork, &iinfo);
        ...
        /* Compute A=L*Q */
        zgelqf(m, n, A, lda, &work[itau], &work[nwork], lwork - nwork, &iinfo);
        il = nwork;
        ...
        /* Solve the bidiagonal least squares problem */
        zlalsd("U", smlsiz, m, nrhs, S, &rwork[ie], B, ldb, rcond, rank,
               &work[nwork], &rwork[nrwork], iwork, info);
        if (*info != 0) {
            goto cleanup;
        }
        ...
        zunmbr("P", "L", "N", m, nrhs, m, &work[il], ldwork, &work[itaup],
               B, ldb, &work[nwork], lwork - nwork, &iinfo);
        ...
        /* Solve the bidiagonal least squares problem */
        zlalsd("L", smlsiz, m, nrhs, S, &rwork[ie], B, ldb, rcond, rank,
               &work[nwork], &rwork[nrwork], iwork, info);
        if (*info != 0) {
            goto cleanup;
        }
        ...
        zunmbr("P", "L", "N", n, nrhs, m, A, lda, &work[itaup],
               B, ldb, &work[nwork], lwork - nwork, &iinfo);
    }
