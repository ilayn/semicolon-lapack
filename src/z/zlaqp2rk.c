/**
 * @file zlaqp2rk.c
 * @brief ZLAQP2RK computes truncated QR factorization with column pivoting of a complex matrix block using Level 2 BLAS.
 */
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    for (kk = 0; kk < kmax; kk++) {
        ...
        if (kp != kk) {
            cblas_zswap(m, &A[0 + kp * lda], 1, &A[0 + kk * lda], 1);
            vn1[kp] = vn1[kk];
            vn2[kp] = vn2[kk];
            itemp = jpiv[kp];
            jpiv[kp] = jpiv[kk];
        ...
        if (i < m - 1) {
            zlarfg(m - i, &A[i + kk * lda], &A[(i + 1) + kk * lda], 1, &tau[kk]);
        } else {
            tau[kk] = CMPLX(0.0, 0.0);
        }

        if (disnan(creal(tau[kk]))) {
            taunan = creal(tau[kk]);
        } else if (disnan(cimag(tau[kk]))) {
            taunan = cimag(tau[kk]);
        } else {
            taunan = 0.0;
        }
        ...
        if (kk < minmnupdt - 1) {
            zlarf1f("L", m - i, n + nrhs - kk - 1, &A[i + kk * lda], 1,
                    conj(tau[kk]), &A[i + (kk + 1) * lda], lda, &work[0]);
        }
        ...
        if (kk < minmnfact - 1) {
            ...
            for (j = kk + 1; j < n; j++) {
                if (vn1[j] != 0.0) {
                    ...
                    temp = 1.0 - pow(cabs(A[i + j * lda]) / vn1[j], 2.0);
                    temp = (temp > 0.0) ? temp : 0.0;
                    temp2 = temp * pow(vn1[j] / vn2[j], 2.0);
                    if (temp2 <= tol3z) {

                        vn1[j] = cblas_dznrm2(m - i - 1, &A[(i + 1) + j * lda], 1);
                        vn2[j] = vn1[j];
