/**
 * @file zunbdb4.c
 * @brief ZUNBDB4 simultaneously bidiagonalizes the blocks of a tall and skinny
...
void zunbdb4(const int m, const int p, const int q,
             ...
             double complex* const restrict phantom,
    ...
    *info = 0;
    ...
    for (i = 0; i < m - q; i++) {
        ...
        if (i == 0) {
            ...
            zunbdb5(p, m - p, q, &phantom[0], 1, &phantom[p], 1,
                    X11, ldx11, X21, ldx21, &work[iorbdb5],
                    lorbdb5, &childinfo);
            cblas_zscal(p, &NEGONE, &phantom[0], 1);
            zlarfgp(p, &phantom[0], &phantom[1], 1, &taup1[0]);
            zlarfgp(m - p, &phantom[p], &phantom[p + 1], 1,
                    &taup2[0]);
            theta[i] = atan2(creal(phantom[0]), creal(phantom[p]));
            c = cos(theta[i]);
            s = sin(theta[i]);
            double complex conjtaup1 = conj(taup1[0]);
            zlarf1f("L", p, q, &phantom[0], 1, conjtaup1,
                    X11, ldx11, &work[ilarf]);
            double complex conjtaup2 = conj(taup2[0]);
            zlarf1f("L", m - p, q, &phantom[p], 1,
                    conjtaup2,
                    X21, ldx21, &work[ilarf]);
        } else {
            zunbdb5(p - i, m - p - i, q - i,
                    &X11[i + (i - 1) * ldx11], 1,
                    &X21[i + (i - 1) * ldx21], 1,
                    &X11[i + i * ldx11], ldx11,
                    &X21[i + i * ldx21], ldx21,
                    &work[iorbdb5], lorbdb5, &childinfo);
            cblas_zscal(p - i, &NEGONE, &X11[i + (i - 1) * ldx11], 1);
            zlarfgp(p - i, &X11[i + (i - 1) * ldx11],
                    &X11[(i + 1) + (i - 1) * ldx11], 1,
                    &taup1[i]);
            zlarfgp(m - p - i, &X21[i + (i - 1) * ldx21],
                    &X21[(i + 1) + (i - 1) * ldx21], 1,
                    &taup2[i]);
            theta[i] = atan2(creal(X11[i + (i - 1) * ldx11]),
                             creal(X21[i + (i - 1) * ldx21]));
            c = cos(theta[i]);
            s = sin(theta[i]);
        ...
        zlarfgp(q - i, &X21[i + i * ldx21],
                &X21[i + (i + 1) * ldx21], ldx21, &tauq1[i]);
        c = creal(X21[i + i * ldx21]);
        zlarf1f("R", p - i - 1, q - i, &X21[i + i * ldx21], ldx21,
                tauq1[i], &X11[(i + 1) + i * ldx11], ldx11,
                &work[ilarf]);
        zlarf1f("R", m - p - i - 1, q - i, &X21[i + i * ldx21], ldx21,
                tauq1[i], &X21[(i + 1) + i * ldx21], ldx21,
                &work[ilarf]);
        zlacgv(q - i, &X21[i + i * ldx21], ldx21);
        if (i < m - q - 1) {
            double nrm1 = cblas_dznrm2(p - i - 1, &X11[(i + 1) + i * ldx11], 1);
            double nrm2 = cblas_dznrm2(m - p - i - 1, &X21[(i + 1) + i * ldx21], 1);
    ...
    for (i = m - q; i < p; i++) {
        ...
        zlarf1f("R", p - i - 1, q - i, &X11[i + i * ldx11], ldx11,
                tauq1[i], &X11[(i + 1) + i * ldx11], ldx11,
                &work[ilarf]);
        zlarf1f("R", q - p, q - i, &X11[i + i * ldx11], ldx11,
    ...
    for (i = p; i < q; i++) {
        zlacgv(q - i, &X21[(m - q + i - p) + i * ldx21], ldx21);
        zlarfgp(q - i, &X21[(m - q + i - p) + i * ldx21],
