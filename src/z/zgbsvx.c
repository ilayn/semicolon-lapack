/**
 * @file zgbsvx.c
 * @brief ZGBSVX computes the solution to a complex system of linear equations
    ...
    *info = 0;
    ...
    if (nofact || equil) {
        ...
        for (j = 0; j < n; j++) {
            ...
            cblas_zcopy(len, &AB[ku + j1 - j + j * ldab], 1,
        ...
        if (*info > 0) {
            ...
            rpvgrw = zlantb("M", "U", "N", *info, k_val, &AFB[start_offset], ldafb, rwork);
            if (rpvgrw == ZERO) {
                rpvgrw = ONE;
            } else {
                rpvgrw = anorm / rpvgrw;
