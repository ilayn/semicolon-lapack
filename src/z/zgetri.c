    ...
    *info = 0;
    ...
    if (nb < nbmin || nb >= n) {
        ...
        for (j = n - 1; j >= 0; j--) {
            ...
            if (j < n - 1) {
                cblas_zgemv(CblasColMajor, CblasNoTrans, n, n - j - 1, &NEG_ONE,
        ...
        for (j = nn; j >= 0; j -= nb) {
            ...
            cblas_ztrsm(CblasColMajor, CblasRight, CblasLower, CblasNoTrans, CblasUnit,
    ...
    for (j = n - 2; j >= 0; j--) {
        jp = ipiv[j];
        if (jp != j) {
            cblas_zswap(n, &A[j * lda], 1, &A[jp * lda], 1);
        }
    }
