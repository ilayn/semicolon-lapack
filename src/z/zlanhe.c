/**
 * @file zlanhe.c
 * @brief ZLANHE returns the value of the 1-norm, Frobenius norm,
    ...
    if (norm[0] == 'M' || norm[0] == 'm') {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                ...
                work[j] = sum + fabs(creal(A[j + j * lda]));
            }
            for (i = 0; i < n; i++) {
                sum = work[i];
                if (value < sum || isnan(sum)) value = sum;
            ...
            for (j = 0; j < n; j++) {
                sum = work[j] + fabs(creal(A[j + j * lda]));
                for (i = j + 1; i < n; i++) {
                    absa = cabs(A[i + j * lda]);
                    sum += absa;
                    work[i] += absa;
        ...
        sum = 2.0 * sum;
        for (i = 0; i < n; i++) {
            if (creal(A[i + i * lda]) != 0.0) {
                absa = fabs(creal(A[i + i * lda]));
                if (scale < absa) {
