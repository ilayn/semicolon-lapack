/**
 * @file zlarrv.c
 * @brief ZLARRV computes the eigenvectors of the tridiagonal matrix
            ...
            f64* work, INT* iwork, INT* info)
{
    ...
    *info = 0;
    ...
    zlaset("Full", n, zusedw, CZERO, CZERO, &Z[zusedl * ldz], ldz);
    ...
    for (jblk = 0; jblk <= iblock[m - 1]; jblk++) {
        ...
        if (ibegin == iend) {
            ...
            Z[ibegin + wbegin * ldz] = CMPLX(ONE, 0.0);
        ...
        cblas_dcopy(im, &W[wbegin], 1, &work[wbegin], 1);
        ...
        while (idone < im) {
            ...
            for (i = 0; i < oldncl; i++) {
                ...
                if (ndepth > 0) {
                    ...
                    for (k = 0; k < in - 1; k++) {
                        D[ibegin + k] = creal(Z[ibegin + k + j * ldz]);
                        L[ibegin + k] = creal(Z[ibegin + k + (j + 1) * ldz]);
                    }
                    D[iend] = creal(Z[iend + j * ldz]);
                    sigma = creal(Z[iend + (j + 1) * ldz]);

                    /* Set the corresponding entries in Z to zero */
                    zlaset("Full", in, 2, CZERO, CZERO, &Z[ibegin + j * ldz], ldz);
                }
                ...
                if (ndepth > 0) {
                     ...
                     * WBEGIN+OLDLST are correctly computed in DLARRB.
                ...
                for (j = oldfst; j <= oldlst; j++) {
                    ...
                    if (newsiz > 1) {
                         ...
                         * workspace */
                        for (k = 0; k < in - 1; k++) {
                            Z[ibegin + k + newftt * ldz] = CMPLX(work[indin1 + k], 0.0);
                            Z[ibegin + k + (newftt + 1) * ldz] = CMPLX(work[indin2 + k], 0.0);
                        }
                        Z[iend + newftt * ldz] = CMPLX(work[indin1 + in - 1], 0.0);
                        if (iinfo == 0) {
                            /* a new RRR for the cluster was found by DLARRF
                             * update shift and store it */
                            ssigma = sigma + tau;
                            Z[iend + (newftt + 1) * ldz] = CMPLX(ssigma, 0.0);
                            /* WORK() are the midpoints and WERR() the semi-width
                             * Note that the entries in W are unchanged. */
                            for (k = newfst; k <= newlst; k++) {
                                fudge = THREE * eps * fabs(work[wbegin + k]);
                                work[wbegin + k] = work[wbegin + k] - tau;
                                fudge = fudge + FOUR * eps * fabs(work[wbegin + k]);
                                /* Fudge errors */
                        ...
                        if (k == 0) {
                            ...
                            lgap = eps * fmax(fabs(left), fabs(right));
                        } else {
                            lgap = wgap[windmn];
                        }
                        if (k == im - 1) {
                            ...
                            rgap = eps * fmax(fabs(left), fabs(right));
                        } else {
                            rgap = wgap[windex];
                        }
...
label_120:
                        ...
                        zlar1v(in, 0, in - 1, lambda, &D[ibegin],
                         ...
                         */
                        if (resid > tol * gap && fabs(rqcorr) > rqtol * fabs(lambda)
                            && !usedbs) {
                        ...
                        cblas_zdscal(zto - zfrom + 1, nrminv,
