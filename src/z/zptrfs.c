/**
 * @file zptrfs.c
 * @brief ZPTRFS improves the computed solution to a system of linear equations
 ...
 * @param[in]     EF    Double complex array, dimension (n-1).
 ...
 * @param[in,out] X     Double complex array, dimension (ldx, nrhs).
...
refine:
        if (upper) {
            if (n == 1) {
                ...
                for (i = 1; i < n - 1; i++) {
                    ...
                    work[i] = bi - cx - dx - ex;
                    rwork[i] = cabs1(bi) +
                               cabs1(E[i - 1]) * cabs1(X[(i - 1) + j * ldx]) +
                               cabs1(dx) + cabs1(E[i]) *
                               cabs1(X[(i + 1) + j * ldx]);
                }
                bi = B[(n - 1) + j * ldb];
                cx = conj(E[n - 2]) * X[(n - 2) + j * ldx];
                dx = D[n - 1] * X[(n - 1) + j * ldx];
                work[n - 1] = bi - cx - dx;
                rwork[n - 1] = cabs1(bi) + cabs1(E[n - 2]) *
                               cabs1(X[(n - 2) + j * ldx]) + cabs1(dx);
            }
        } else {
            if (n == 1) {
                bi = B[0 + j * ldb];
        ...
        rwork[0] = ONE;
        for (i = 1; i < n; i++) {
            rwork[i] = ONE + rwork[i - 1] * cabs(EF[i - 1]);
        }
        ...
        for (i = n - 2; i >= 0; i--) {
            rwork[i] = rwork[i] / DF[i] + rwork[i + 1] * cabs(EF[i]);
        }

        /* Compute norm(inv(A)) = max(x(i)), 0<=i<n. */

        ix = cblas_idamax(n, rwork, 1);
        ferr[j] = ferr[j] * fabs(rwork[ix]);

        /* Normalize error. */

        lstres = ZERO;
        for (i = 0; i < n; i++) {
            lstres = (lstres > cabs(X[i + j * ldx])) ? lstres : cabs(X[i + j * ldx]);
        }
        if (lstres != ZERO) {
