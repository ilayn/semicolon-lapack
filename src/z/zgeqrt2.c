/**
 * @file zgeqrt2.c
 * @brief ZGEQRT2 computes a QR factorization of a general complex matrix using
    ...
    *info = 0;
    ...
    for (i = 0; i < k; i++) {
        ...
        if (i < n - 1) {
            ...
            /* W(0:n-i-2) := A(i:m-1, i+1:n-1)^H * A(i:m-1, i)
            ...
            /* A(i:m-1, i+1:n-1) += alpha * A(i:m-1, i) * W^H */
            alpha = -conj(T[i + 0 * ldt]);
    ...
    for (i = 1; i < n; i++) {
        ...
        /* T(0:i-1, i) := -tau(i) * A(i:m-1, 0:i-1)^H * A(i:m-1, i) */
        alpha = -T[i + 0 * ldt];
        cblas_zgemv(CblasColMajor, CblasConjTrans,
                    m - i, i, &alpha,
                    &A[i + 0 * lda], lda,
