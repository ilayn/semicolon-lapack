/**
 * @file zlatrs3.c
 * @brief ZLATRS3 solves a triangular system with scale factors to prevent overflow.
 */
...
#define NBMAX 64
    ...
    double anrm, bignum, bnrm, rscal, scal, scaloc;
    double scamin, smlnum, tmax;
    ...
    *info = 0;
    ...
    if (nrhs < NRHSMIN) {
        zlatrs(uplo, trans, diag, normin, n, A, lda, &X[0], &scale[0], cnorm, info);
        for (k = 1; k < nrhs; k++) {
            zlatrs(uplo, trans, diag, "Y", n, A, lda, &X[k * ldx], &scale[k], cnorm, info);
        }
        return;
    }
    ...
    for (j = 0; j < nba; j++) {
        ...
        for (i = ifirst; i <= ilast; i++) {
            ...
            if (notran) {
                anrm = zlange("I", i2 - i1, j2 - j1, &A[i1 + j1 * lda], lda, W);
                work[awrk + i + j * nba] = anrm;
            } else {
                anrm = zlange("1", i2 - i1, j2 - j1, &A[i1 + j1 * lda], lda, W);
    ...
    if (!(tmax <= dlamch("O"))) {
        ...
        for (k = 0; k < nrhs; k++) {
            zlatrs(uplo, trans, diag, "N", n, A, lda, &X[k * ldx], &scale[k], cnorm, info);
        }
        return;
    }
    ...
    for (k = 0; k < nbx; k++) {
        ...
        for (j = jfirst; (jinc > 0) ? (j <= jlast) : (j >= jlast); j += jinc) {
            ...
            for (kk = 0; kk < k2 - k1; kk++) {
                rhs = k1 + kk;
                if (kk == 0) {
                    zlatrs(uplo, trans, diag, "N", j2 - j1,
                           &A[j1 + j1 * lda], lda, &X[j1 + rhs * ldx],
                           &scaloc, cnorm, info);
                } else {
                    zlatrs(uplo, trans, diag, "Y", j2 - j1,
                           &A[j1 + j1 * lda], lda, &X[j1 + rhs * ldx],
                           &scaloc, cnorm, info);
                }

                /* Find largest absolute value entry in X( J1:J2-1, RHS ) */
                XNRM[kk] = zlange("I", j2 - j1, 1, &X[j1 + rhs * ldx], ldx, W);
                ...
                if (scaloc == ZERO) {
                    ...
                    if (XNRM[kk] * rscal <= bignum) {
                        ...
                        cblas_zdscal(j2 - j1, rscal, &X[j1 + rhs * ldx], 1);
                        scaloc = ONE;
                    } else {
                        scale[rhs] = ZERO;
                        for (ii = 0; ii < n; ii++) {
            ...
            for (i = ifirst; (iinc > 0) ? (i <= ilast) : (i >= ilast); i += iinc) {
                ...
                for (kk = 0; kk < k2 - k1; kk++) {
                    ...
                    bnrm = zlange("I", i2 - i1, 1, &X[i1 + rhs * ldx], ldx, W);
                    ...
                    if (scal != ONE) {
                        cblas_zdscal(i2 - i1, scal, &X[i1 + rhs * ldx], 1);
                        work[i + (kk + 1) * lds] = scamin * scaloc;
                    }

                    scal = (scamin / work[j + (kk + 1) * lds]) * scaloc;
                    if (scal != ONE) {
                        cblas_zdscal(j2 - j1, scal, &X[j1 + rhs * ldx], 1);
