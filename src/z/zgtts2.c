/**
 * @file zgtts2.c
 * @brief ZGTTS2 solves a system of linear equations with a tridiagonal matrix
    ...
    const int ldb)
{
    ...
    if (itrans == 0) {
        ...
        if (nrhs <= 1) {
            for (j = 0; j < nrhs; j++) {
                /* Solve U**H * x = b. */
                B[0 + j * ldb] = B[0 + j * ldb] / conj(D[0]);
                if (n > 1) {
                    B[1 + j * ldb] = (B[1 + j * ldb] - conj(DU[0]) * B[0 + j * ldb]) / conj(D[1]);
                }
                for (i = 2; i < n; i++) {
                    B[i + j * ldb] = (B[i + j * ldb] - conj(DU[i - 1]) * B[(i - 1) + j * ldb]
                                      - conj(DU2[i - 2]) * B[(i - 2) + j * ldb]) / conj(D[i]);
                }
