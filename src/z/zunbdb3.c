/**
 * @file zunbdb3.c
 * @brief ZUNBDB3 simultaneously bidiagonalizes the blocks of a tall and skinny
             ...
             int* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < m - p; i++) {
        ...
        zlarf1f("R", p - i, q - i, &X21[i + i * ldx21], ldx21, tauq1[i],
        ...
        if (i < m - p - 1) {
            zlarfgp(m - p - i - 1, &X21[(i + 1) + i * ldx21],
                    &X21[((i + 2) < (m - p) ? (i + 2) : (i + 1)) + i * ldx21],
                    1, &taup2[i]);
            phi[i] = atan2(creal(X21[(i + 1) + i * ldx21]),
                           creal(X11[i + i * ldx11]));
            c = cos(phi[i]);
            s = sin(phi[i]);
            {
                double complex conjtaup2 = conj(taup2[i]);
                zlarf1f("L", m - p - i - 1, q - i - 1,
