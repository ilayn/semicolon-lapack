    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (k + 1 > nb) {
                ...
                zlasyf_rook(uplo, k + 1, nb, &kb, A, lda, ipiv, work, ldwork, &iinfo);

            } else {

                zsytf2_rook(uplo, k + 1, A, lda, ipiv, &iinfo);
                kb = k + 1;
            }
        ...
        while (k < n) {
            ...
            if (k <= n - 1 - nb) {
                ...
                zlasyf_rook(uplo, n - k, nb, &kb, &A[k + k * lda], lda,
                            &ipiv[k], work, ldwork, &iinfo);

            } else {

                zsytf2_rook(uplo, n - k, &A[k + k * lda], lda, &ipiv[k], &iinfo);
                kb = n - k;
            }
