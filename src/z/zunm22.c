/**
 * @file zunm22.c
 * @brief ZUNM22 multiplies a general matrix by a banded unitary matrix.
 */
 ...
 * @param[in]     Q       Double complex array, dimension (ldq, m) if SIDE = 'L',
 *                        (ldq, n) if SIDE = 'R'.
    ...
    if (left) {
        if (notran) {
            for (i = 0; i < n; i += nb) {
                ...
                zlacpy("A", n1, len, &C[n2 + i * ldc], ldc,
                       work, ldwork);
                ...
                zlacpy("A", m, len, work, ldwork, &C[i * ldc], ldc);
            }
        } else {
            for (i = 0; i < n; i += nb) {
                len = nb < (n - i) ? nb : (n - i);
                ...
                zlacpy("A", n2, len, &C[n1 + i * ldc], ldc,
                       work, ldwork);
                cblas_ztrmm(CblasColMajor, CblasLeft, CblasUpper,
        ...
        if (notran) {
            for (i = 0; i < m; i += nb) {
                ...
                zlacpy("A", len, n2, &C[i + n1 * ldc], ldc,
                       work, ldwork);
                ...
                /* Copy everything back. */
                zlacpy("A", len, n, work, ldwork, &C[i], ldc);
            }
        } else {
            for (i = 0; i < m; i += nb) {
                len = nb < (m - i) ? nb : (m - i);
                ...
                zlacpy("A", len, n1, &C[i + n2 * ldc], ldc,
                       work, ldwork);
                cblas_ztrmm(CblasColMajor, CblasRight, CblasLower,
                            CblasConjTrans, CblasNonUnit,
                            len, n1, &ONE, &Q[n2 * ldq], ldq,
