/**
 * @file zggev.c
 * @brief ZGGEV computes the eigenvalues and, optionally, the left and/or
           ...
           double* rwork, int* info)
{
    ...
    *info = 0;
    ...
    if (ilv) {
        zgghrd(jobvl, jobvr, n, ilo, ihi, A, lda, B, ldb, VL,
               ldvl, VR, ldvr, &ierr);
    } else {
        zgghrd("N", "N", irows, 1, irows, &A[(ilo - 1) + (ilo - 1) * lda],
               lda, &B[(ilo - 1) + (ilo - 1) * ldb], ldb, VL, ldvl,
               VR, ldvr, &ierr);
    }
