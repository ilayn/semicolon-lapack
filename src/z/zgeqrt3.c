/**
 * @file zgeqrt3.c
 * @brief ZGEQRT3 recursively computes a QR factorization of a general
             ...
             INT* info)
{
    ...
    *info = 0;
    ...
    if (n == 1) {
        ...
        zgeqrt3(m - n1, n2, &A[n1 + n1 * lda], lda,
        ...
        for (i = 0; i < n1; i++) {
            for (j = 0; j < n2; j++) {
                T[i + (j + n1) * ldt] = conj(A[(j + n1) + i * lda]);
            }
        }
