 ...
 *
 * @param[in,out] B
 *          Double complex array, dimension (ldb, n).
    ...
    INT* info)
{
    ...
    *info = 0;
    ...
    if (p > 0) {
        ...
        cblas_zcopy(p, D, 1, &X[n - p], 1);

        cblas_zgemv(CblasColMajor, CblasNoTrans, n - p, p, &neg_cone,
                    &A[0 + (n - p) * lda], lda, D, 1, &cone, C, 1);
    }

    if (n > p) {
        ztrtrs("U", "N", "N", n - p, 1, A, lda, C, n - p, info);
        ...
        cblas_zcopy(n - p, C, 1, X, 1);
    }

    if (m < n) {
        nr = m + p - n;
        if (nr > 0) {
            cblas_zgemv(CblasColMajor, CblasNoTrans, nr, n - m, &neg_cone,
    ...
    if (nr > 0) {
        ...
        cblas_zaxpy(nr, &neg_cone, D, 1, &C[n - p], 1);
    }
