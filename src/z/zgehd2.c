/**
 * @file zgehd2.c
 * @brief ZGEHD2 reduces a general square matrix to upper Hessenberg form
 ...
 *                       are normally set by a previous call to ZGEBAL;
            ...
            c128* work, int* info)
{
    ...
    *info = 0;
    ...
    for (i = ilo; i <= ihi - 1; i++) {
        ...
        zlarfg(len, &A[(i + 1) + i * lda], &A[start + i * lda], 1, &tau[i]);

        /* Apply H(i) to A(0:ihi, i+1:ihi) from the right */
        zlarf1f("Right", ihi + 1, len, &A[(i + 1) + i * lda], 1, tau[i],
                &A[(i + 1) * lda], lda, work);
