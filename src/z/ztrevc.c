 ...
 * @param[out] work   Double complex array, dimension (2*n).
        ...
        for (ki = n - 1; ki >= 0; ki--) {
            ...
            if (ki > 0) {
                zlatrs("Upper", "No transpose", "Non-unit", "Y",
                       ...
                       info);
                work[ki] = CMPLX(scale, 0.0);
            }

            /* Copy the vector x or Q*x to VR and normalize. */
            if (!over) {
                cblas_zcopy(ki + 1, &work[0], 1, &VR[is * ldvr], 1);
...
L80:        ;
    ...
    if (leftv) {
        ...
        for (ki = 0; ki < n; ki++) {
            ...
            if (!over) {
                cblas_zcopy(n - ki, &work[ki], 1, &VL[ki + is * ldvl], 1);
