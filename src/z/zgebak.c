/**
 * @file zgebak.c
 * @brief ZGEBAK forms the right or left eigenvectors of a complex general
 ...
 * @param[in]     ilo    The integers ILO and IHI determined by ZGEBAL.
 *                       0 <= ILO <= IHI <= N-1, if N > 0; ILO=0 and IHI=-1, if N=0.
 *                       Uses 0-based indexing.
 * @param[in]     ihi    See ILO. Uses 0-based indexing.
 * @param[in]     scale  Double precision array, dimension (N).
 *                       Details of the permutation and scaling factors, as returned
 *                       by ZGEBAL. Permutation indices are 0-based.
 ...
 */
void zgebak(const char* job, const char* side, const int n, const int ilo,
            const int ihi, const double* scale, const int m, double complex* V,
    ...
    *info = 0;
    ...
    if (job[0] == 'S' || job[0] == 's' || job[0] == 'B' || job[0] == 'b') {
        if (rightv) {
            for (i = ilo; i <= ihi; i++) {
                s = scale[i];
                cblas_zdscal(m, s, &V[i + 0 * ldv], ldv);
            }
        }
    ...
    if (job[0] == 'P' || job[0] == 'p' || job[0] == 'B' || job[0] == 'b') {
        if (rightv) {
            for (ii = 0; ii < n; ii++) {
                ...
                k = (int)scale[i];
                if (k == i) continue;
                cblas_zswap(m, &V[i + 0 * ldv], ldv, &V[k + 0 * ldv], ldv);
            }
        }
