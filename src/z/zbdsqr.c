/**
 * @file zbdsqr.c
 * @brief ZBDSQR computes the SVD of a real bidiagonal matrix using the
...
void zbdsqr(const char* uplo, const int n, const int ncvt, const int nru,
            const int ncc, double* const restrict D, double* const restrict E,
            ...
            double* const restrict rwork, int* info)
{
    ...
    double oldcs, oldsn, r, shift, sigmn, sigmx, sinl, sinr;
    double sll, smax, smin, sminoa, sn, thresh, tol, tolmul, unfl;
    ...
    *info = 0;
    ...
    if (lower) {
        ...
        if (nru > 0) {
            zlasr("R", "V", "F", nru, n, rwork, &rwork[nm1], U, ldu);
        }
        if (ncc > 0) {
            zlasr("L", "V", "F", n, ncc, rwork, &rwork[nm1], C, ldc);
        }
    }
    ...
    tolmul = (TEN > (HNDRD < pow(eps, MEIGTH) ? HNDRD : pow(eps, MEIGTH)))
             ? TEN
             : (HNDRD < pow(eps, MEIGTH) ? HNDRD : pow(eps, MEIGTH));
    tol = tolmul * eps;

    /* Compute approximate maximum, minimum singular values */
    smax = ZERO;
    for (i = 0; i < n; i++) {
        smax = (smax > fabs(D[i])) ? smax : fabs(D[i]);
    }
    for (i = 0; i < n - 1; i++) {
        smax = (smax > fabs(E[i])) ? smax : fabs(E[i]);
    }
    ...
    L50:
        ...
        thresh = (fabs(tol) * smax > MAXITR * (n * (n * unfl)))
                 ? fabs(tol) * smax
                 : MAXITR * (n * (n * unfl));
    }
...
L90:
    ...
    if (ll == m - 2) {
        ...
        if (ncvt > 0) {
            cblas_zdrot(ncvt, &VT[m - 2], ldvt, &VT[m - 1], ldvt,
                       cosr, sinr);
        }
        if (nru > 0) {
            cblas_zdrot(nru, &U[(m - 2) * ldu], 1, &U[(m - 1) * ldu], 1, cosl, sinl);
        }
        if (ncc > 0) {
            cblas_zdrot(ncc, &C[m - 2], ldc, &C[m - 1], ldc, cosl, sinl);
        }
        m = m - 2;
        goto L60;
    }
    ...
    if (shift == ZERO) {
        if (idir == 1) {
            ...
            if (ncvt > 0) {
                zlasr("L", "V", "F", m - ll, ncvt, rwork,
                      &rwork[nm1], &VT[ll], ldvt);
            }
            if (nru > 0) {
                zlasr("R", "V", "F", nru, m - ll,
                      &rwork[nm12], &rwork[nm13], &U[ll * ldu], ldu);
            }
            if (ncc > 0) {
                zlasr("L", "V", "F", m - ll, ncc,
                      &rwork[nm12], &rwork[nm13], &C[ll], ldc);
            }
            ...
            /* Update singular vectors */
            if (ncvt > 0) {
                zlasr("L", "V", "B", m - ll, ncvt,
                      &rwork[nm12], &rwork[nm13], &VT[ll], ldvt);
            }
            if (nru > 0) {
                zlasr("R", "V", "B", nru, m - ll, rwork,
                      &rwork[nm1], &U[ll * ldu], ldu);
            }
            if (ncc > 0) {
                zlasr("L", "V", "B", m - ll, ncc, rwork,
                      &rwork[nm1], &C[ll], ldc);
            }
...
L160:
    for (i = 0; i < n; i++) {
        ...
        if (D[i] < ZERO) {
            ...
            if (ncvt > 0) {
                cblas_zdscal(ncvt, NEGONE, &VT[i], ldvt);
            }
        }
    }
    ...
    for (i = 0; i < n - 1; i++) {
        ...
        if (isub != n - 1 - i) {
            ...
            D[n - 1 - i] = smin;
            if (ncvt > 0) {
                cblas_zswap(ncvt, &VT[isub], ldvt, &VT[n - 1 - i], ldvt);
            }
            if (nru > 0) {
                cblas_zswap(nru, &U[isub * ldu], 1, &U[(n - 1 - i) * ldu], 1);
            }
            if (ncc > 0) {
                cblas_zswap(ncc, &C[isub], ldc, &C[n - 1 - i], ldc);
            }
        }
    }
    return;
