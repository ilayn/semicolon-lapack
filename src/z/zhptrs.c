/**
 * @file zhptrs.c
 * @brief ZHPTRS solves a system of linear equations with a Hermitian matrix
    ...
    if (upper) {
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                akm1k = AP[kc + k - 1];
                akm1 = AP[kc - 1] / akm1k;
                ak = AP[kc + k] / conj(akm1k);
                denom = akm1 * ak - ONE;
                for (j = 0; j < nrhs; j++) {
                    bkm1 = B[(k - 1) + j * ldb] / akm1k;
                    bk = B[k + j * ldb] / conj(akm1k);
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                ...
                if (k > 0) {
                    zlacgv(nrhs, &B[k], ldb);
                    cblas_zgemv(CblasColMajor, CblasConjTrans, k, nrhs, &NEG_ONE, B,
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                ...
                /* Multiply by the inverse of the diagonal block. */
                akm1k = AP[kc + 1];
                akm1 = AP[kc] / conj(akm1k);
                ak = AP[kc + n - k] / akm1k;
                denom = akm1 * ak - ONE;
                for (j = 0; j < nrhs; j++) {
                    bkm1 = B[k + j * ldb] / conj(akm1k);
        ...
        while (k >= 0) {
            ...
            if (ipiv[k] >= 0) {
                ...
                if (k < n - 1) {
                    zlacgv(nrhs, &B[k], ldb);
                    cblas_zgemv(CblasColMajor, CblasConjTrans, n - k - 1, nrhs, &NEG_ONE,
