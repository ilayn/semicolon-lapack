/**
 * @file zlagtm.c
 * @brief ZLAGTM performs a matrix-matrix product of the form C = alpha*A*B + beta*C,
    ...
    const INT ldb)
{
    ...
    if (alpha == ONE) {
        if (trans[0] == 'N' || trans[0] == 'n') {
            ...
            for (j = 0; j < nrhs; j++) {
                if (n == 1) {
                    B[0 + j * ldb] = B[0 + j * ldb] + conj(D[0]) * X[0 + j * ldx];
                } else {
                    B[0 + j * ldb] = B[0 + j * ldb] + conj(D[0]) * X[0 + j * ldx]
                                     + conj(DL[0]) * X[1 + j * ldx];
                    B[(n - 1) + j * ldb] = B[(n - 1) + j * ldb]
                                           + conj(DU[n - 2]) * X[(n - 2) + j * ldx]
                                           + conj(D[n - 1]) * X[(n - 1) + j * ldx];
                    for (i = 1; i < n - 1; i++) {
                        B[i + j * ldb] = B[i + j * ldb]
                                         + conj(DU[i - 1]) * X[(i - 1) + j * ldx]
                                         + conj(D[i]) * X[i + j * ldx]
                                         + conj(DL[i]) * X[(i + 1) + j * ldx];
                    }
                }
            }
        }
