/**
 * @file zgeql2.c
 * @brief ZGEQL2 computes a QL factorization of a general M-by-N matrix
            ...
            int* info)
{
    ...
    *info = 0;
    ...
    for (i = k - 1; i >= 0; i--) {
        ...
        zlarfg(m - k + i + 1, &A[(m - k + i) + (n - k + i) * lda],
               &A[0 + (n - k + i) * lda], 1, &tau[i]);

        if (n - k + i > 0) {
            /* Apply H(i)**H to A(0:m-k+i, 0:n-k+i-1) from the left */
            aii = A[(m - k + i) + (n - k + i) * lda];
            A[(m - k + i) + (n - k + i) * lda] = CMPLX(1.0, 0.0);
            zlarf1l("L", m - k + i + 1, n - k + i,
                    &A[0 + (n - k + i) * lda], 1, conj(tau[i]),
                    A, lda, work);
