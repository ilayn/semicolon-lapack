/**
 * @file zgbbrd.c
 * @brief ZGBBRD reduces a complex general band matrix to real upper bidiagonal form.
 */
 ...
 * @param[out]    rwork Double precision array, dimension (max(m,n)).
 * @param[out]    info
 *                         - = 0: successful exit.
            ...
            f64* restrict rwork, int* info)
{
    ...
    *info = 0;
    ...
    if (wantq) {
        zlaset("Full", m, m, CZERO, CONE, Q, ldq);
    }
    if (wantpt) {
        zlaset("Full", n, n, CZERO, CONE, PT, ldpt);
    }
    ...
    if (kl + ku > 1) {
        ...
        for (int i = 0; i < minmn; i++) {
            ...
            for (int kk = 0; kk < kb; kk++) {
                ...
                if (nr > 0) {
                    zlargv(nr, &AB[(klu1 - 1) + (j1 - klm - 1) * ldab], inca,
                ...
                for (int l = 0; l < kb; l++) {
                    ...
                    if (nrt > 0) {
                        zlartv(nrt, &AB[(klu1 - l - 2) + (j1 - klm + l) * ldab], inca,
                ...
                if (nr > 0) {
                    zlargv(nr, &AB[(j1 + kun - 1) * ldab], inca,
                ...
                for (int l = 0; l < kb; l++) {
                    ...
                    if (nrt > 0) {
                        zlartv(nrt, &AB[(l + 1) + (j1 + kun - 1) * ldab], inca,
    ...
    if (ku == 0 && kl > 0) {
        for (int i = 0; i < MIN(m - 1, n); i++) {
            ...
            if (wantq) {
                zrot(m, &Q[i * ldq], 1, &Q[(i + 1) * ldq], 1, rc, conj(rs));
            }
            if (wantc) {
                zrot(ncc, &C[i], ldc, &C[i + 1], ldc, rc, rs);
            }
        }
    } else if (ku > 0 && m < n) {
