/**
 * @file zgelsy.c
 * @brief ZGELSY computes the minimum-norm solution to a complex linear least
            ...
            int* info)
{
    ...
    *info = 0;
    ...
    while (*rank < mn) {
        ...
        zlaic1(IMIN, *rank, &work[ismin], smin,
               &A[0 + i * lda], A[i + i * lda],
               &sminpr, &s1, &c1);
        zlaic1(IMAX, *rank, &work[ismax], smax,
               &A[0 + i * lda], A[i + i * lda],
               &smaxpr, &s2, &c2);
    ...
    if (*rank < n) {
        ...
        ztzrzf(*rank, n, A, lda, &work[mn], &work[2 * mn],
               lwork - 2 * mn, &iinfo);
    }
     ...
     * sub-workspace in work[2*mn..] */
    if (*rank < n) {
        zunmrz("L", "C", n, nrhs, *rank, n - *rank, A, lda,
    ...
    for (int j = 0; j < nrhs; j++) {
        ...
        cblas_zcopy(n, work, 1, &B[j * ldb], 1);
    }

    /* Undo scaling */
    if (iascl == 1) {
        zlascl("G", 0, 0, anrm, smlnum, n, nrhs, B, ldb, &iinfo);
        zlascl("U", 0, 0, smlnum, anrm, *rank, *rank, A, lda, &iinfo);
    } else if (iascl == 2) {
        zlascl("G", 0, 0, anrm, bignum, n, nrhs, B, ldb, &iinfo);
        zlascl("U", 0, 0, bignum, anrm, *rank, *rank, A, lda, &iinfo);
    }
    if (ibscl == 1) {
