/**
 * @file zlamtsqr.c
 * @brief ZLAMTSQR overwrites the general complex M-by-N matrix C with
 *        Q*C, Q^H*C, C*Q, or C*Q^H using the blocked reflectors from ZLATSQR.
 */
 ...
 *                       The blocked elementary reflectors as returned by ZLATSQR.
 * @param[in]     lda    The leading dimension of A.
 *                       If SIDE = 'L', lda >= max(1, m);
 *                       if SIDE = 'R', lda >= max(1, n).
 * @param[in]     T      Complex*16 array containing the block reflectors.
 ...
 *                       If SIDE = 'R', lwork >= max(1, m*nb).
 *                       If lwork = -1, workspace query is assumed.
 * @param[out]    info
 *                         - = 0: successful exit
 *                         - < 0: if info = -i, the i-th argument had an illegal value
 */
void zlamtsqr(const char* side, const char* trans,
    ...
    *info = 0;
    ...
    if (left && notran) {
        ...
        if (kk > 0) {
            ...
            ztpmqrt("L", "N", kk, n, k, 0, nb, &A[ii], lda,
        ...
        for (i = ii - (mb - k); i >= mb; i -= (mb - k)) {
             ...
             */
            ctr = ctr - 1;
            ztpmqrt("L", "N", mb - k, n, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[i], ldc, work, info);
        }
         ...
         */
        zgemqrt("L", "N", mb, n, k, nb, &A[0], lda, T, ldt, &C[0], ldc, work, info);

    } else if (left && tran) {
        /*
         * Multiply Q^H to the first block of C
         */
        kk = (m - k) % (mb - k);
        ii = m - kk;  /* 0-based */
        ctr = 1;
        zgemqrt("L", "C", mb, n, k, nb, &A[0], lda, T, ldt, &C[0], ldc, work, info);

        /* Fortran: DO I=MB+1,II-MB+K,(MB-K) */
        /* C 0-based: i starts at mb, goes up to ii-mb+k-1, step (mb-k) */
        for (i = mb; i <= ii - mb + k; i += (mb - k)) {
            /*
             * Multiply Q^H to the current block of C
             */
            ztpmqrt("L", "C", mb - k, n, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[i], ldc, work, info);
            ctr = ctr + 1;
        }
        ...
        if (ii < m) {
            /*
             * Multiply Q^H to the last block of C
             */
            ztpmqrt("L", "C", kk, n, k, 0, nb, &A[ii], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[ii], ldc, work, info);
        }
        ...
        /*
         * Multiply Q^H from the right to the last block of C
         */
        kk = (n - k) % (mb - k);
        ctr = (n - k) / (mb - k);
        if (kk > 0) {
            ii = n - kk;  /* 0-based */
            ztpmqrt("R", "C", m, kk, k, 0, nb, &A[ii], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
        ...
        for (i = ii - (mb - k); i >= mb; i -= (mb - k)) {
             ...
             */
            ctr = ctr - 1;
            ztpmqrt("R", "C", m, mb - k, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
         ...
         * Multiply Q^H to the first block of C
         */
        zgemqrt("R", "C", m, mb, k, nb, &A[0], lda, T, ldt, &C[0], ldc, work, info);
        ...
        ii = n - kk;  /* 0-based */
        ctr = 1;
        zgemqrt("R", "N", m, mb, k, nb, &A[0], lda, T, ldt, &C[0], ldc, work, info);
        ...
        for (i = mb; i <= ii - mb + k; i += (mb - k)) {
             ...
             * Multiply Q to the current block of C
             */
            ztpmqrt("R", "N", m, mb - k, k, 0, nb, &A[i], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[0 + i * ldc], ldc, work, info);
            ctr = ctr + 1;
        }
        ...
        if (ii < n) {
             ...
             * Multiply Q to the last block of C
             */
            ztpmqrt("R", "N", m, kk, k, 0, nb, &A[ii], lda,
                    &T[0 + ctr * k * ldt], ldt, &C[0], ldc,
                    &C[0 + ii * ldc], ldc, work, info);
        }
    }
