/**
 * @file ztgevc.c
 * @brief ZTGEVC computes eigenvectors of a pair of complex upper triangular matrices.
 */
 ...
 * @param[out]    rwork   Double precision workspace array, dimension (2*n).
 * @param[out]    info
 *                         - = 0: successful exit.
    ...
    int* info)
{
    ...
    *m = im;
    ...
    /* Compute the 1-norm of each column of the strictly upper triangular
       ...
       solver. */
    anorm = cabs1(S[0 + 0 * lds]);
    bnorm = cabs1(P[0 + 0 * ldp]);
    rwork[0] = ZERO;
    rwork[n] = ZERO;
    for (j = 1; j < n; j++) {
        rwork[j] = ZERO;
        ...
        anorm = fmax(anorm, rwork[j] + cabs1(S[j + j * lds]));
        bnorm = fmax(bnorm, rwork[n + j] + cabs1(P[j + j * ldp]));
    }

    ascale = ONE / fmax(anorm, safmin);
    bscale = ONE / fmax(bnorm, safmin);
    ...
    if (compl_) {
        ...
        for (je = 0; je < n; je++) {
            ...
            if (ilcomp) {
                ...
                if (cabs1(S[je + je * lds]) <= safmin &&
                    fabs(creal(P[je + je * ldp])) <= safmin) {
                ...
                temp = ONE / fmax(fmax(cabs1(S[je + je * lds]) * ascale,
                             fabs(creal(P[je + je * ldp])) * bscale), safmin);
                salpha = (temp * S[je + je * lds]) * ascale;
                sbeta = (temp * creal(P[je + je * ldp])) * bscale;
                acoeff = sbeta * ascale;
                bcoeff = salpha * bscale;

                /* Scale to avoid underflow */
                lsa = fabs(sbeta) >= safmin && fabs(acoeff) < small_;
                lsb = cabs1(salpha) >= safmin && cabs1(bcoeff) < small_;

                scale = ONE;
                if (lsa)
                    scale = (small_ / fabs(sbeta)) * fmin(anorm, big);
                if (lsb)
                    scale = fmax(scale, (small_ / cabs1(salpha)) * fmin(bnorm, big));
                if (lsa || lsb) {
                    scale = fmin(scale, ONE /
                            (safmin * fmax(fmax(ONE, fabs(acoeff)),
                            cabs1(bcoeff))));
                    if (lsa) {
                ...
                for (jr = 0; jr < n; jr++)
                    work[jr] = CZERO;
                work[je] = CONE;
                dmin_ = fmax(fmax(ulp * acoefa * anorm, ulp * bcoefa * bnorm), safmin);
                ...
                for (j = je + 1; j < n; j++) {
                    ...
                    sum_ = acoeff * suma - conj(bcoeff) * sumb;
                    ...
                    work[j] = zladiv(-sum_, d);
                    xmax = fmax(xmax, cabs1(work[j]));
                }

                /* Back transform eigenvector if HOWMNY='B'. */
                if (ilback) {
                    cblas_zgemv(CblasColMajor, CblasNoTrans, n, n - je, &CONE,
                                &VL[0 + je * ldvl], ldvl,
                ...
                xmax = ZERO;
                for (jr = ibeg; jr < n; jr++)
                    xmax = fmax(xmax, cabs1(work[(isrc - 1) * n + jr]));
    ...
    if (compr) {
        ...
        for (je = n - 1; je >= 0; je--) {
            ...
            if (ilcomp) {
                ...
                /* Back transform eigenvector if HOWMNY='B'. */
                if (ilback) {
                    cblas_zgemv(CblasColMajor, CblasNoTrans, n, je + 1, &CONE,
