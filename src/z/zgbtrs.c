/**
 * @file zgbtrs.c
 * @brief ZGBTRS solves a system of linear equations with a general band
 ...
 *                        as computed by ZGBTRF. U is stored as an upper triangular
    ...
    if (notran) {
        ...
        if (lnoti) {
            for (j = 0; j < n - 1; j++) {
                ...
                l = ipiv[j];
                if (l != j) {
                    cblas_zswap(nrhs, &B[l], ldb, &B[j], ldb);
                }
                cblas_zgeru(CblasColMajor, lm, nrhs, &NEG_ONE,
                            &AB[kd + 1 + j * ldab], 1,
                            &B[j], ldb,
                            &B[j + 1], ldb);
            }
         ...
         */
        for (i = 0; i < nrhs; i++) {
            cblas_ztbsv(CblasColMajor, CblasUpper, CblasTrans, CblasNonUnit,
                        n, kl + ku, AB, ldab, &B[i * ldb], 1);
        }
        ...
        if (lnoti) {
            for (j = n - 2; j >= 0; j--) {
                lm = (kl < n - 1 - j) ? kl : n - 1 - j;
                cblas_zgemv(CblasColMajor, CblasTrans, lm, nrhs, &NEG_ONE,
                            &B[j + 1], ldb,
                            &AB[kd + 1 + j * ldab], 1,
                            &ONE, &B[j], ldb);
                l = ipiv[j];
        ...
        if (lnoti) {
            for (j = n - 2; j >= 0; j--) {
                lm = (kl < n - 1 - j) ? kl : n - 1 - j;
                zlacgv(nrhs, &B[j], ldb);
                cblas_zgemv(CblasColMajor, CblasConjTrans, lm, nrhs, &NEG_ONE,
                            &B[j + 1], ldb,
                            &AB[kd + 1 + j * ldab], 1,
                            &ONE, &B[j], ldb);
                zlacgv(nrhs, &B[j], ldb);
