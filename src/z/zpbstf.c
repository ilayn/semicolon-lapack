/**
 * @file zpbstf.c
 * @brief ZPBSTF computes a split Cholesky factorization of a Hermitian positive definite band matrix.
 */
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        for (j = n - 1; j >= m; j--) {
            ...
            if (km > 0) {
                cblas_zdscal(km, ONE / ajj, &AB[kd - km + j * ldab], 1);
                cblas_zher(CblasColMajor, CblasUpper, km, -ONE,
                           &AB[kd - km + j * ldab], 1,
                           &AB[kd + (j - km) * ldab], kld);
            }
        }
        ...
        for (j = 0; j < m; j++) {
            ...
            if (km > 0) {
                cblas_zdscal(km, ONE / ajj, &AB[kd - 1 + (j + 1) * ldab], kld);
                zlacgv(km, &AB[kd - 1 + (j + 1) * ldab], kld);
                cblas_zher(CblasColMajor, CblasUpper, km, -ONE,
        ...
        for (j = n - 1; j >= m; j--) {
            ...
            // the trailing submatrix within the band
            if (km > 0) {
                cblas_zdscal(km, ONE / ajj, &AB[km + (j - km) * ldab], kld);
                zlacgv(km, &AB[km + (j - km) * ldab], kld);
                cblas_zher(CblasColMajor, CblasLower, km, -ONE,
                           &AB[km + (j - km) * ldab], kld,
                           &AB[0 + (j - km) * ldab], kld);
                zlacgv(km, &AB[km + (j - km) * ldab], kld);
            }
        ...
        for (j = 0; j < m; j++) {
            ...
            // the trailing submatrix within the band
            if (km > 0) {
                cblas_zdscal(km, ONE / ajj, &AB[1 + j * ldab], 1);
                cblas_zher(CblasColMajor, CblasLower, km, -ONE,
                           &AB[1 + j * ldab], 1,
                           &AB[0 + (j + 1) * ldab], kld);
            }
