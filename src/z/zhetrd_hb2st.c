/**
 * @file zhetrd_hb2st.c
 * @brief ZHETRD_HB2ST reduces a complex Hermitian band matrix to real symmetric tridiagonal form.
 */
 ...
 *                          band using the zhetrd_he2hb routine was not called
 ...
 *                          band using the zhetrd_he2hb routine has been called
 ...
 * @param[out]    hous    Double complex array, dimension (MAX(1,LHOUS)).
                  ...
                  c128* work, const INT lwork, INT* info)
{
    ...
    *info = 0;
    ...
    if (kd == 1) {
        ...
        if (upper) {
            for (i = 0; i < n - 1; i++) {
                ...
                abstmp = cabs(tmp);
                AB[abofdpos + (i + 1) * ldab] = CMPLX(abstmp, 0.0);
                E[i] = abstmp;
                if (abstmp != rzero) {
                    tmp = tmp / CMPLX(abstmp, 0.0);
                } else {
                    tmp = one;
                }
                if (i < n - 2)
            ...
            for (i = 0; i < n - 1; i++) {
                ...
                abstmp = cabs(tmp);
                AB[abofdpos + i * ldab] = CMPLX(abstmp, 0.0);
                E[i] = abstmp;
                if (abstmp != rzero) {
                    tmp = tmp / CMPLX(abstmp, 0.0);
                } else {
            ...
            for (m = 1; m <= stepercol; m++) {
                ...
                for (sweepid = st; sweepid <= ed; sweepid++) {
                    for (k = 1; k <= grsiz; k++) {
                        ...
                        zhb2st_kernels(uplo, wantq, ttype,
    ...
    for (i = 0; i < n; i++) {
        D[i] = creal(work[dpos + i * lda]);
    }

    if (upper) {
        for (i = 0; i < n - 1; i++) {
            E[i] = creal(work[ofdpos + (i + 1) * lda]);
        }
    } else {
        for (i = 0; i < n - 1; i++) {
            E[i] = creal(work[ofdpos + i * lda]);
        }
    }
