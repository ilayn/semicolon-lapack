            ...
            const int nh, double complex* WH, const int ldwh)
{
    ...
    for (incol = ktop - 2 * nbmps + 1; incol <= kbot - 2; incol += 2 * nbmps) {
        ...
        ndcol = incol + kdu;
        if (accum)
            zlaset("ALL", kdu, kdu, czero, cone, U, ldu);
        ...
        for (krcol = incol; krcol <= (incol + 2 * nbmps - 1 < kbot - 2 ?
            ...
            if (bmp22) {
                k = krcol + 2 * (m22 - 1);
                if (k == ktop - 1) {
                    zlaqr1(2, &H[(k + 1) + (k + 1) * ldh], ldh,
                           S[2 * m22 - 2], S[2 * m22 - 1],
                           &V[(m22 - 1) * ldv]);
                    beta = V[(m22 - 1) * ldv];
                    zlarfg(2, &beta, &V[1 + (m22 - 1) * ldv], 1,
                           &V[(m22 - 1) * ldv]);
                } else {
                    beta = H[(k + 1) + k * ldh];
                ...
                for (j = k + 1; j <= jbot; j++) {
                    refsum = H[(k + 1) + j * ldh] +
                             conj(V[1 + (m22 - 1) * ldv]) * H[(k + 2) + j * ldh];
                ...
                if (k >= ktop) {
                    if (H[(k + 1) + k * ldh] != czero) {
                        ...
                        if (cabs1(H[(k + 1) + k * ldh]) <=
                            (smlnum > ulp * tst1 ? smlnum : ulp * tst1)) {
                            h12 = cabs1(H[(k + 1) + k * ldh]) > cabs1(H[k + (k + 1) * ldh]) ?
                                  cabs1(H[(k + 1) + k * ldh]) : cabs1(H[k + (k + 1) * ldh]);
                            h21 = cabs1(H[(k + 1) + k * ldh]) < cabs1(H[k + (k + 1) * ldh]) ?
                                  cabs1(H[(k + 1) + k * ldh]) : cabs1(H[k + (k + 1) * ldh]);
                            h11 = cabs1(H[(k + 1) + (k + 1) * ldh]) >
                                  cabs1(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]) ?
                                  cabs1(H[(k + 1) + (k + 1) * ldh]) :
                                  cabs1(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]);
                            h22 = cabs1(H[(k + 1) + (k + 1) * ldh]) <
                                  cabs1(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]) ?
                                  cabs1(H[(k + 1) + (k + 1) * ldh]) :
                                  cabs1(H[k + k * ldh] - H[(k + 1) + (k + 1) * ldh]);
                            scl = h11 + h12;
            ...
            for (m = mbot; m >= mtop; m--) {
                ...
                if (k == ktop - 1) {
                    ...
                    if (H[(k + 3) + k * ldh] != czero ||
                        ...
                        zlaqr1(3, &H[(k + 1) + (k + 1) * ldh], ldh,
                               S[2 * m - 2], S[2 * m - 1], vt);
                        alpha = vt[0];
                        zlarfg(3, &alpha, &vt[1], 1, &vt[0]);
                        t1 = conj(vt[0]);
                        t2 = t1 * vt[1];
                        t3 = t1 * vt[2];
                        refsum = H[(k + 1) + k * ldh] + conj(vt[1]) * H[(k + 2) + k * ldh];

                        if (cabs1(H[(k + 2) + k * ldh] - refsum * t2) +
                            cabs1(refsum * t3) > ulp * (cabs1(H[k + k * ldh]) +
                            cabs1(H[(k + 1) + (k + 1) * ldh]) +
                            cabs1(H[(k + 2) + (k + 2) * ldh]))) {
                            H[(k + 1) + k * ldh] = beta;
                            H[(k + 2) + k * ldh] = czero;
                            ...
                            H[(k + 1) + k * ldh] = H[(k + 1) + k * ldh] - refsum * conj(vt[0]);
                            H[(k + 2) + k * ldh] = czero;
                            H[(k + 3) + k * ldh] = czero;
                            V[(m - 1) * ldv] = vt[0];
                            V[1 + (m - 1) * ldv] = vt[1];
                ...
                refsum = H[(k + 1) + (k + 1) * ldh] +
                         conj(V[1 + (m - 1) * ldv]) * H[(k + 2) + (k + 1) * ldh] +
                         conj(V[2 + (m - 1) * ldv]) * H[(k + 3) + (k + 1) * ldh];
            ...
            for (m = mbot; m >= mtop; m--) {
                ...
                for (j = (ktop > krcol + 2 * m ? ktop : krcol + 2 * m); j <= jbot; j++) {
                    refsum = H[(k + 1) + j * ldh] +
                             conj(V[1 + (m - 1) * ldv]) * H[(k + 2) + j * ldh] +
                             conj(V[2 + (m - 1) * ldv]) * H[(k + 3) + j * ldh];
                    H[(k + 1) + j * ldh] = H[(k + 1) + j * ldh] - refsum * t1;
                    H[(k + 2) + j * ldh] = H[(k + 2) + j * ldh] - refsum * t2;
        ...
        if (accum) {
            ...
            for (jcol = (ndcol < kbot ? ndcol : kbot) + 1; jcol <= jbot; jcol += nh) {
                ...
                zlacpy("ALL", nu, jlen, WH, ldwh, &H[(incol + k1) + jcol * ldh], ldh);
            }
            ...
            for (jrow = jtop; jrow <= (ktop > incol ? ktop : incol) - 1; jrow += nv) {
                ...
                zlacpy("ALL", jlen, nu, WV, ldwv, &H[jrow + (incol + k1) * ldh], ldh);
            }
            ...
            if (wantz) {
                for (jrow = iloz; jrow <= ihiz; jrow += nv) {
                    ...
                    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                               ...
                               &U[(k1 - 1) + (k1 - 1) * ldu], ldu, &czero, WV, ldwv);
                    zlacpy("ALL", jlen, nu, WV, ldwv, &Z[jrow + (incol + k1) * ldz], ldz);
                }
            }
        }
    }
