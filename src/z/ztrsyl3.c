/**
 * @file ztrsyl3.c
 * @brief ZTRSYL3 solves the complex Sylvester matrix equation (blocked version).
 */
    ...
    *scale = ONE;
    ...
    for (k = 0; k < nba; k++) {
        ...
        for (l = k; l < nba; l++) {
            ...
            if (notrna) {
                swork[k + (awrk + l) * ldswork] = zlange("I", k2 - k1, l2 - l1,
                                                          &A[k1 + l1 * lda], lda, wnrm);
            } else {
                swork[l + (awrk + k) * ldswork] = zlange("1", k2 - k1, l2 - l1,
                                                          &A[k1 + l1 * lda], lda, wnrm);
            }
        }
    }
    ...
    for (k = 0; k < nbb; k++) {
        ...
        for (l = k; l < nbb; l++) {
            ...
            if (notrnb) {
                swork[k + (bwrk + l) * ldswork] = zlange("I", k2 - k1, l2 - l1,
                                                          &B[k1 + l1 * ldb], ldb, wnrm);
            } else {
                swork[l + (bwrk + k) * ldswork] = zlange("1", k2 - k1, l2 - l1,
                                                          &B[k1 + l1 * ldb], ldb, wnrm);
            }
        }
    }
    ...
    if (notrna && notrnb) {
        ...
        for (k = nba - 1; k >= 0; k--) {
            ...
            for (l = 0; l < nbb; l++) {
                ...
                ztrsyl(trana, tranb, isgn, k2 - k1, l2 - l1,
                       &A[k1 + k1 * lda], lda,
                       &B[l1 + l1 * ldb], ldb,
                ...
                *info = (*info > iinfo) ? *info : iinfo;
                ...
                xnrm = zlange("I", k2 - k1, l2 - l1, &C[k1 + l1 * ldc], ldc, wnrm);
                ...
                for (i = k - 1; i >= 0; i--) {
                    ...
                    cnrm = zlange("I", i2 - i1, l2 - l1, &C[i1 + l1 * ldc],
                                  ldc, wnrm);
                    ...
                    if (scal != ONE) {
                        for (jj = l1; jj < l2; jj++) {
                            cblas_zdscal(k2 - k1, scal, &C[k1 + jj * ldc], 1);
                        }
                    }
                    ...
                    if (scal != ONE) {
                        for (ll = l1; ll < l2; ll++) {
                            cblas_zdscal(i2 - i1, scal, &C[i1 + ll * ldc], 1);
                        }
                    }
                ...
                for (j = l + 1; j < nbb; j++) {
                    ...
                    cnrm = zlange("I", k2 - k1, j2 - j1, &C[k1 + j1 * ldc],
                                  ldc, wnrm);
                    ...
                    if (scal != ONE) {
                        for (ll = l1; ll < l2; ll++) {
                            cblas_zdscal(k2 - k1, scal, &C[k1 + ll * ldc], 1);
                        }
                    }
