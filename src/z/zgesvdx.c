/**
 * @file zgesvdx.c
 * @brief ZGESVDX computes the singular value decomposition (SVD) of a complex
 ...
 * @param[out]    S       Double precision array (min(m,n)). Singular values in descending order.
 ...
 * @param[out]    rwork   Double precision workspace array.
    ...
    *info = 0;
    ...
    if (m >= n) {
        ...
        if (m >= mnthr) {
            ...
            itau = 0;
            itemp = itau + n;
            zgeqrf(m, n, A, lda, &work[itau], &work[itemp], lwork - itemp, &ierr);
            ...
            zgebrd(n, n, &work[iqrf], n, &rwork[id], &rwork[ie],
            ...
            dbdsvdx("U", jobz_str, rngtgk_str, n, &rwork[id], &rwork[ie],
            ...
            if (wantu) {
                ...
                for (i = 0; i < *ns; i++) {
                    for (j = 0; j < n; j++) {
                        U[j + i * ldu] = CMPLX(rwork[k], 0.0);
                        k++;
                    }
                    k += n;
                }
                zlaset("A", m - n, *ns, CZERO, CZERO, &U[n], ldu);

                /* Call ZUNMBR to compute QB*UB */
                zunmbr("Q", "L", "N", n, *ns, n, &work[iqrf], n,
                       &work[itauq], U, ldu, &work[itemp],
                       lwork - itemp, &ierr);

                /* Call ZUNMQR to compute Q*(QB*UB) */
                zunmqr("L", "N", m, *ns, n, A, lda, &work[itau],
            ...
            if (wantvt) {
                ...
                for (i = 0; i < *ns; i++) {
                    for (j = 0; j < n; j++) {
                        VT[i + j * ldvt] = CMPLX(rwork[k], 0.0);
                        k++;
                    }
                    k += n;
                }

                /* Call ZUNMBR to compute VB**H * PB**H */
                zunmbr("P", "R", "C", *ns, n, n, &work[iqrf], n,
            ...
            ie = id + n;
            itgkz = ie + n;
            zgebrd(m, n, A, lda, &rwork[id], &rwork[ie],
                   &work[itauq], &work[itaup], &work[itemp],
                   lwork - itemp, &ierr);
            itempr = itgkz + n * (2 * n + 1);
            ...
            if (wantu) {
                ...
                /* Call ZUNMBR to compute QB*UB */
                zunmbr("Q", "L", "N", m, *ns, n, A, lda, &work[itauq],
                       U, ldu, &work[itemp], lwork - itemp, &ierr);
            }
            ...
            if (wantvt) {
                ...
                /* Call ZUNMBR to compute VB**H * PB**H */
                zunmbr("P", "R", "C", *ns, n, n, A, lda, &work[itaup],
        ...
        if (n >= mnthr) {
            ...
            itau = 0;
            itemp = itau + m;
            zgelqf(m, n, A, lda, &work[itau], &work[itemp], lwork - itemp, &ierr);
            ...
            zgebrd(m, m, &work[ilqf], m, &rwork[id], &rwork[ie],
                   &work[itauq], &work[itaup], &work[itemp],
                   lwork - itemp, &ierr);
            ...
            char jobz_str[2] = {jobz, '\0'};
            char rngtgk_str[2] = {rngtgk, '\0'};
            dbdsvdx("U", jobz_str, rngtgk_str, m, &rwork[id], &rwork[ie],
            ...
            if (wantu) {
                ...
                /* Call ZUNMBR to compute QB*UB */
                zunmbr("Q", "L", "N", m, *ns, m, &work[ilqf], m,
                       &work[itauq], U, ldu, &work[itemp],
                       lwork - itemp, &ierr);
            }
            ...
            if (wantvt) {
                ...
                zlaset("A", *ns, n - m, CZERO, CZERO, &VT[m * ldvt], ldvt);

                /* Call ZUNMBR to compute (VB**H)*(PB**H) */
                zunmbr("P", "R", "C", *ns, m, m, &work[ilqf], m,
                       &work[itaup], VT, ldvt, &work[itemp],
                       lwork - itemp, &ierr);

                /* Call ZUNMLQ to compute ((VB**H)*(PB**H))*Q */
                zunmlq("R", "N", *ns, n, m, A, lda, &work[itau],
                       VT, ldvt, &work[itemp], lwork - itemp, &ierr);
            }
        } else {
            /* Path 2t (N greater than M, but not much larger)
            ...
            char jobz_str[2] = {jobz, '\0'};
            char rngtgk_str[2] = {rngtgk, '\0'};
            dbdsvdx("L", jobz_str, rngtgk_str, m, &rwork[id], &rwork[ie],
                    vl, vu, iltgk, iutgk, ns, S, &rwork[itgkz], 2 * m,
                    &rwork[itempr], iwork, info);
            ...
            if (wantvt) {
                ...
                /* Call ZUNMBR to compute VB**H * PB**H */
                zunmbr("P", "R", "C", *ns, n, m, A, lda, &work[itaup],
                       VT, ldvt, &work[itemp], lwork - itemp, &ierr);
            }
        }
    }
