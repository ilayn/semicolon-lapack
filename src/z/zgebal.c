 ...
 */
void zgebal(const char* job, const int n, double complex* A, const int lda,
    ...
    *info = 0;
    ...
    if (!(job[0] == 'S' || job[0] == 's')) {
        ...
        while (noconv) {
            ...
            for (i = l; i >= 0; i--) {
                canswap = 1;
                for (j = 0; j <= l; j++) {
                    if (i != j && (creal(A[i + j * lda]) != ZERO ||
                                   cimag(A[i + j * lda]) != ZERO)) {
                        canswap = 0;
                        break;
                    }
                }
                ...
                if (canswap) {
                    ...
                    if (i != l) {
                        ...
                        cblas_zswap(l + 1, &A[0 + i * lda], 1, &A[0 + l * lda], 1);
                        /* Swap rows i and l from column k onwards */
                        cblas_zswap(n - k, &A[i + k * lda], lda, &A[l + k * lda], lda);
                    }
                    noconv = 1;
            ...
            for (j = k; j <= l; j++) {
                ...
                if (canswap) {
                    ...
                    if (j != k) {
                        ...
                        cblas_zswap(l + 1, &A[0 + j * lda], 1, &A[0 + k * lda], 1);
                        /* Swap rows j and k from column k onwards */
                        cblas_zswap(n - k, &A[j + k * lda], lda, &A[k + k * lda], lda);
                    }
                    noconv = 1;
    ...
    while (noconv) {
        ...
        for (i = k; i <= l; i++) {
            ...
            c = cblas_dznrm2(l - k + 1, &A[k + i * lda], 1);
            /* Compute row norm (columns k to l) */
            r = cblas_dznrm2(l - k + 1, &A[i + k * lda], lda);
            /* Find max absolute value in column (rows 0 to l) */
            ica = cblas_izamax(l + 1, &A[0 + i * lda], 1);
            ca = cabs(A[ica + i * lda]);
            /* Find max absolute value in row (columns k to n-1) */
            ira = cblas_izamax(n - k, &A[i + k * lda], lda);
            ra = cabs(A[i + (ira + k) * lda]);
            ...
            /* Scale row i */
            cblas_zdscal(n - k, g, &A[i + k * lda], lda);
            /* Scale column i */
            cblas_zdscal(l + 1, f, &A[0 + i * lda], 1);
        }
    }
