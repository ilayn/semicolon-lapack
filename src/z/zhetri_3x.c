/**
 * @file zhetri_3x.c
 * @brief ZHETRI_3X computes the inverse of a complex Hermitian indefinite matrix using the factorization computed by ZHETRF_RK or ZHETRF_BK (blocked algorithm).
 */
    ...
    int* info)
{
    ...
    double ak, akp1, t;
    double complex akkp1, d, u01_i_j, u01_ip1_j, u11_i_j, u11_ip1_j;
    int ldwork = n + nb + 1;
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (k < n) {
            if (ipiv[k] >= 0) {
                work[k + invd * ldwork] = 1.0 / creal(A[k + k * lda]);
                work[k + (invd + 1) * ldwork] = CZERO;
            } else {
                t = cabs(work[k + 1 + 0 * ldwork]);
                ak = creal(A[k + k * lda]) / t;
                akp1 = creal(A[k + 1 + (k + 1) * lda]) / t;
        ...
        while (k >= 0) {
            if (ipiv[k] >= 0) {
                ...
                t = cabs(work[k - 1 + 0 * ldwork]);
                ak = creal(A[k - 1 + (k - 1) * lda]) / t;
                akp1 = creal(A[k + k * lda]) / t;
                akkp1 = work[k - 1 + 0 * ldwork] / t;
                d = t * (ak * akp1 - CONE);
