/**
 * @file zhpevd.c
 * @brief ZHPEVD computes the eigenvalues and, optionally, eigenvectors for a
    ...
    int* info)
{
    ...
    *info = 0;
    ...
    /* For eigenvalues only, call DSTERF.  For eigenvectors, first call
       ZSTEDC to generate the eigenvectors from the tridiagonal form,
       then call ZUPMTR to multiply by the Householder transformations. */

    if (!wantz) {
        dsterf(n, W, &rwork[inde], info);
    } else {
        zstedc("I", n, W, &rwork[inde], Z, ldz,
               &work[indwrk], llwrk, &rwork[indrwk], llrwk, iwork, liwork,
               info);
        zupmtr("L", uplo, "N", n, n, AP, &work[indtau], Z, ldz,
               &work[indwrk], &iinfo);
    }
