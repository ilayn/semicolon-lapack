/**
 * @file zlaqps.c
 * @brief ZLAQPS computes a step of QR factorization with column pivoting
            ...
            c128* const restrict F, const int ldf)
{
    ...
    while (k < nb && lsticc == 0) {
         ...
         */
        if (rk < m) {
            zlarfg(m - rk + 1, &A[(rk - 1) + (k - 1) * lda],
                   &A[rk + (k - 1) * lda], 1, &tau[k - 1]);
        } else {
            zlarfg(1, &A[(rk - 1) + (k - 1) * lda],
        ...
        if (rk < lastrk) {
            for (j = k + 1; j <= n; j++) {
                if (vn1[j - 1] != 0.0) {
                     ...
                     */
                    temp = cabs(A[(rk - 1) + (j - 1) * lda]) / vn1[j - 1];
