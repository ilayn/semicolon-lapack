/**
 * @file zlaesy.c
 * @brief ZLAESY computes the eigenvalues and eigenvectors of a 2-by-2
    ...
    c128* sn1)
{
    ...
    if (cabs(b) == ZERO) {
        ...
        *sn1 = (*rt1 - a) / b;
        tabs = cabs(*sn1);
        if (tabs > ONE) {
            t = tabs * csqrt((ONE / tabs) * (ONE / tabs) + (*sn1 / tabs) * (*sn1 / tabs));
        } else {
            t = csqrt(CONE + (*sn1) * (*sn1));
        }
        evnorm = cabs(t);
        if (evnorm >= THRESH) {
            *evscal = CONE / t;
