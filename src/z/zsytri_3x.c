 ...
 *                      and factors U or L as computed by `zsytrf_rk` or
 *                      `zsytrf_bk`:
 *
 ...
 *                      of D as determined by `zsytrf_rk` or `zsytrf_bk`.
    ...
    *info = 0;
    ...
    if (upper) {
        ...
        while (cut < n) {
            ...
            cblas_ztrmm(CblasColMajor, CblasLeft, CblasLower, CblasTrans, CblasUnit,
                        nnb, nnb, &ONE, &A[cut + cut * lda], lda, &work[u11 + 0 * ldwork], ldwork);
