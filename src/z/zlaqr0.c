/**
 * @file zlaqr0.c
 * @brief ZLAQR0 computes eigenvalues of a Hessenberg matrix using small-bulge
 ...
 *
 * ZLAQR0 is identical to ZLAQR4 except that it calls ZLAQR3
 * instead of ZLAQR2.
 *
    ...
    *info = 0;
    ...
    if (n <= ntiny) {
        ...
        *info = 0;
        ...
        zlaqr3(wantt, wantz, n, ilo, ihi, nwr + 1, H, ldh, iloz,
        ...
        for (it = 1; it <= itmax; it++) {
            ...
            /* Aggressive early deflation - uses ZLAQR3 (not ZLAQR2) */
                ...
                if ((ndfl % kexsh) == 0) {
                    ...
                    if (kbot - ks + 1 <= ns / 2) {
                        ...
                        zlacpy("A", ns, ns, &H[ks + ks * ldh], ldh,
                               &H[kt], ldh);
                        if (ns > nmin) {
                            zlaqr4(0, 0, ns, 0, ns - 1, &H[kt], ldh, &W[ks],
