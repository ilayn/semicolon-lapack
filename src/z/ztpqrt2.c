/**
 * @file ztpqrt2.c
 * @brief ZTPQRT2 computes a QR factorization of a complex "triangular-pentagonal"
             ...
             c128* restrict T, const INT ldt, INT* info)
{
    ...
    *info = 0;
    ...
    for (i = 0; i < n; i++) {
        ...
        zlarfg(p + 1, &A[i + i * lda], &B[i * ldb], 1, &T[i]);

        if (i < n - 1) {
            for (j = 0; j < n - i - 1; j++) {
                T[j + (n - 1) * ldt] = conj(A[i + (i + 1 + j) * lda]);
            }
            cblas_zgemv(CblasColMajor, CblasConjTrans, p, n - i - 1, &ONE,
            ...
            for (j = 0; j < n - i - 1; j++) {
                A[i + (i + 1 + j) * lda] = A[i + (i + 1 + j) * lda] +
                                           alpha * conj(T[j + (n - 1) * ldt]);
            }
            cblas_zgerc(CblasColMajor, p, n - i - 1, &alpha, &B[i * ldb], 1,
    ...
    for (i = 1; i < n; i++) {
        ...
        if (i - p > 0) {
            cblas_zgemv(CblasColMajor, CblasConjTrans, l, i - p, &alpha,
                        &B[mp + (np - 1) * ldb], ldb, &B[mp + i * ldb], 1, &ZERO,
                        &T[(np - 1) + i * ldt], 1);
        }

        if (m - l > 0) {
            cblas_zgemv(CblasColMajor, CblasConjTrans, m - l, i, &alpha,
