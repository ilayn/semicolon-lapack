/**
 * @file zlamswlq.c
 * @brief ZLAMSWLQ overwrites the general complex M-by-N matrix C with
 ...
 * elementary reflectors computed by short wide LQ factorization (ZLASWLQ)
 *
 ...
 *                       blocked elementary reflector H(i), as returned by ZLASWLQ.
 ...
 */
void zlamswlq(const char* side, const char* trans,
              ...
              int* info)
{
    ...
    *info = 0;
    ...
    if (left && tran) {
        ...
        zgemlqt("L", "C", nb, n, k, mb, &A[0 + 0 * lda], lda, T, ldt,
        ...
        zgemlqt("L", "N", nb, n, k, mb, &A[0 + 0 * lda], lda, T, ldt,
                &C[0 + 0 * ldc], ldc, work, info);

        for (i = nb; i <= ii - nb + k; i += (nb - k)) {
            ztpmlqt("L", "N", nb - k, n, k, 0, mb,
                    &A[0 + i * lda], lda,
                    &T[0 + ctr * k * ldt], ldt,
                    &C[0 + 0 * ldc], ldc,
                    &C[i + 0 * ldc], ldc, work, info);
        ...
        zgemlqt("R", "N", m, nb, k, mb, &A[0 + 0 * lda], lda, T, ldt,
                &C[0 + 0 * ldc], ldc, work, info);
        ...
        zgemlqt("R", "C", m, nb, k, mb, &A[0 + 0 * lda], lda, T, ldt,
                &C[0 + 0 * ldc], ldc, work, info);

        for (i = nb; i <= ii - nb + k; i += (nb - k)) {
            ztpmlqt("R", "C", m, nb - k, k, 0, mb,
                    &A[0 + i * lda], lda,
                    &T[0 + ctr * k * ldt], ldt,
                    &C[0 + 0 * ldc], ldc,
                    &C[0 + i * ldc], ldc, work, info);
