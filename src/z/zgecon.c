    ...
    int* info)
{
    ...
    *rcond = ZERO;
    ...
    while (1) {
        ...
        if (kase == kase1) {
            // Multiply by inv(L)
            zlatrs("L", "N", "U", &normin, n, A, lda, work, &sl, rwork, &linfo);

            // Multiply by inv(U)
            zlatrs("U", "N", "N", &normin, n, A, lda, work, &su, &rwork[n], &linfo);
        } else {
            // Multiply by inv(U**H)
            zlatrs("U", "C", "N", &normin, n, A, lda, work, &su, &rwork[n], &linfo);

            // Multiply by inv(L**H)
            zlatrs("L", "C", "U", &normin, n, A, lda, work, &sl, rwork, &linfo);
        }
