/**
 * @file zggevx.c
 * @brief ZGGEVX computes the eigenvalues and, optionally, the left and/or
...
/**
 * ZGGEVX computes for a pair of N-by-N complex nonsymmetric matrices (A,B)
...
void zggevx(const char* balanc, const char* jobvl, const char* jobvr,
            ...
            c128* restrict alpha,
            c128* restrict beta,
            c128* restrict VL, const int ldvl,
            c128* restrict VR, const int ldvr,
            int* ilo, int* ihi,
            ...
            int* info)
{
    ...
    *abnrm = zlange("1", n, n, A, lda, rwork);
    if (ilascl) {
        rwork[0] = *abnrm;
        dlascl("G", 0, 0, anrmto, anrm, 1, 1, rwork, 1, &ierr);
        *abnrm = rwork[0];
    }

    *bbnrm = zlange("1", n, n, B, ldb, rwork);
    if (ilbscl) {
        rwork[0] = *bbnrm;
        dlascl("G", 0, 0, bnrmto, bnrm, 1, 1, rwork, 1, &ierr);
        *bbnrm = rwork[0];
    }
    ...
    zhgeqz(chtemp, jobvl, jobvr, n, *ilo, *ihi, A, lda, B, ldb,
    ...
    if (ilv || !wantsn) {
        ...
        if (!wantsn) {
            ...
            for (i = 0; i < n; i++) {
                ...
                if (wantse || wantsb) {
                    ztgevc("B", "S", bwork, n, A, lda, B, ldb,
                ...
                ztgsna(sense, "S", bwork, n, A, lda, B, ldb,
    ...
    if (ilvl) {
        ...
        for (jc = 0; jc < n; jc++) {
            temp = ZERO;
            for (jr = 0; jr < n; jr++) {
                temp = temp > cabs1(VL[jr + jc * ldvl]) ?
                       temp : cabs1(VL[jr + jc * ldvl]);
            }
            if (temp < smlnum)
                continue;
            temp = ONE / temp;
    ...
    if (ilvr) {
        ...
        for (jc = 0; jc < n; jc++) {
            temp = ZERO;
            for (jr = 0; jr < n; jr++) {
                temp = temp > cabs1(VR[jr + jc * ldvr]) ?
                       temp : cabs1(VR[jr + jc * ldvr]);
            }
            if (temp < smlnum)
                continue;
            temp = ONE / temp;
