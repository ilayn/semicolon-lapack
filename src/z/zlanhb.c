/**
 * @file zlanhb.c
 * @brief ZLANHB returns the value of the 1-norm, Frobenius norm, infinity norm, or max element of a Hermitian band matrix.
 */
    ...
    if (n == 0) {
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                ...
                sum = fabs(creal(AB[k + j * ldab]));
                if (value < sum || disnan(sum)) value = sum;
            }
        } else {
            for (j = 0; j < n; j++) {
                sum = fabs(creal(AB[j * ldab]));
                if (value < sum || disnan(sum)) value = sum;
        ...
        if (uplo[0] == 'U' || uplo[0] == 'u') {
            for (j = 0; j < n; j++) {
                ...
                work[j] = sum + fabs(creal(AB[k + j * ldab]));
            }
            for (i = 0; i < n; i++) {
                sum = work[i];
                if (value < sum || disnan(sum)) value = sum;
            ...
            for (j = 0; j < n; j++) {
                sum = work[j] + fabs(creal(AB[j * ldab]));
                l = -j;
                INT iend = (n < j + k + 1) ? n : j + k + 1;
                for (i = j + 1; i < iend; i++) {
                    absa = cabs(AB[l + i + j * ldab]);
        ...
        for (j = 0; j < n; j++) {
            if (creal(AB[l + j * ldab]) != ZERO) {
                absa = fabs(creal(AB[l + j * ldab]));
                if (scale < absa) {
