/**
 * @file zgedmdq.c
 * @brief ZGEDMDQ computes the Dynamic Mode Decomposition (DMD) for a pair of
...
/**
 * ZGEDMDQ computes the Dynamic Mode Decomposition (DMD) for a pair of data
 * snapshot matrices, using a QR factorization based compression of the data.
 * For the input matrix that contains a sequence of snapshots, ZGEDMDQ computes
 ...
 *                              converge.
 *                        = 4 : data inconsistency warning (see zgedmd).
 */
void zgedmdq(const char* jobs, const char* jobz, const char* jobr,
             const char* jobq, const char* jobt, const char* jobf,
             const INT whtsvd, const INT m, const INT n, c128* restrict F,
             const INT ldf, c128* restrict X, const INT ldx, c128* restrict Y,
             const INT ldy, const INT nrnk, const f64 tol, INT* k,
             c128* restrict eigs, c128* restrict Z, const INT ldz,
             f64* restrict res, c128* restrict B, const INT ldb,
             c128* restrict V, const INT ldv, c128* restrict S, const INT lds,
             c128* restrict zwork, const INT lzwork, f64* restrict work,
    ...
    *info = 0;
    ...
    if (*info == 0) {
        ...
        if (wntvec || wntvcf) {
            ...
            if (lquery) {
                zunmqr("L", "N", m, n, minmn, F, ldf, NULL, Z, ldz, zwork, -1, &info1);
                olwmqr = (INT)creal(zwork[0]);
        ...
        if (wantq) {
            ...
            if (lquery) {
                zungqr(m, minmn, minmn, F, ldf, NULL, zwork, -1, &info1);
                olwgqr = (INT)creal(zwork[0]);
    ...
    zgeqrf(m, n, F, ldf, zwork, &zwork[minmn], lzwork - minmn, &info1);

    /* Define X and Y as the snapshots representations in the orthogonal basis
     * computed in the QR factorization. */
    zlaset("L", minmn, n - 1, CZERO, CZERO, X, ldx);
    zlacpy("U", minmn, n - 1, F, ldf, X, ldx);
    zlacpy("A", minmn, n - 1, &F[ldf], ldf, Y, ldy);
    if (m >= 3) {
        zlaset("L", minmn - 2, n - 2, CZERO, CZERO, &Y[2], ldy);
    }

    /* Compute the DMD of the projected snapshot pairs (X,Y) */
    zgedmd(jobs, jobvl, jobr, jobf, whtsvd, minmn, n - 1, X, ldx, Y, ldy, nrnk,
    ...
    if (wntvec) {
        ...
        if (m > minmn)
            zlaset("A", m - minmn, *k, CZERO, CZERO, &Z[minmn], ldz);
        zunmqr("L", "N", m, *k, minmn, F, ldf, zwork, Z, ldz,
               &zwork[minmn], lzwork - minmn, &info1);
    } else if (wntvcf) {
        /* Return the Ritz vectors (eigenvectors) in factored form Z*V. */
        zlacpy("A", n, *k, X, ldx, Z, ldz);
        if (m > n)
            zlaset("A", m - n, *k, CZERO, CZERO, &Z[n], ldz);
        zunmqr("L", "N", m, *k, minmn, F, ldf, zwork, Z, ldz,
               &zwork[minmn], lzwork - minmn, &info1);
    }
    ...
    if (wnttrf) {
        zlaset("A", minmn, n, CZERO, CZERO, Y, ldy);
    ...
    if (wantq) {
        zungqr(m, minmn, minmn, F, ldf, zwork, &zwork[minmn], lzwork - minmn, &info1);
    }
}
