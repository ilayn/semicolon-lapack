/**
 * @file zunglq.c
 * @brief ZUNGLQ generates all or part of the unitary matrix Q from
 *        an LQ factorization determined by ZGELQF (blocked algorithm).
 */
    ...
    *info = 0;
    ...
    if (kk > 0) {
        ...
        for (i = ki; i >= 0; i -= nb) {
            ...
            /* Apply H**H to columns i:n-1 of current block */
            zungl2(ib, n - i, ib, &A[i + i * lda], lda,
                   &tau[i], work, &iinfo);
