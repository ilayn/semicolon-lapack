            ...
            c128* Z, const INT ldz)
{
    ...
    if (k + 1 == ihi) {
        ...
        zlartg(B[ihi + ihi * ldb], B[ihi + (ihi - 1) * ldb], &c, &s,
               &temp);
        B[ihi + ihi * ldb] = temp;
        B[ihi + (ihi - 1) * ldb] = CZERO;
        zrot(ihi - istartm, &B[istartm + ihi * ldb], 1,
             &B[istartm + (ihi - 1) * ldb], 1, c, s);
        zrot(ihi - istartm + 1, &A[istartm + ihi * lda], 1,
             &A[istartm + (ihi - 1) * lda], 1, c, s);
        if (ilz) {
            zrot(nz, &Z[(ihi - zstart) * ldz], 1,
        ...
        zlartg(B[(k + 1) + (k + 1) * ldb], B[(k + 1) + k * ldb], &c, &s,
               &temp);
        B[(k + 1) + (k + 1) * ldb] = temp;
        B[(k + 1) + k * ldb] = CZERO;
        zrot(k - istartm + 3, &A[istartm + (k + 1) * lda], 1,
             &A[istartm + k * lda], 1, c, s);
        zrot(k - istartm + 1, &B[istartm + (k + 1) * ldb], 1,
             &B[istartm + k * ldb], 1, c, s);
        if (ilz) {
            zrot(nz, &Z[(k - zstart + 1) * ldz], 1,
                 &Z[(k - zstart) * ldz], 1, c, s);
        }

        /* Apply transformation from the left */

        zlartg(A[(k + 1) + k * lda], A[(k + 2) + k * lda], &c, &s, &temp);
        A[(k + 1) + k * lda] = temp;
        A[(k + 2) + k * lda] = CZERO;
        zrot(istopm - k, &A[(k + 1) + (k + 1) * lda], lda,
             &A[(k + 2) + (k + 1) * lda], lda, c, s);
        zrot(istopm - k, &B[(k + 1) + (k + 1) * ldb], ldb,
             &B[(k + 2) + (k + 1) * ldb], ldb, c, s);
        if (ilq) {
            zrot(nq, &Q[(k - qstart + 1) * ldq], 1,
                 &Q[(k - qstart + 2) * ldq], 1, c, conj(s));
        }
